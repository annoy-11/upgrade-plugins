...
class Sesvideosell_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function getGateway()
  {
    if( null === $this->_gateway ) {
      ...
      $gateway = new $class(array(
        ...
        'currency' => Engine_Api::_()->sesbasic()->getCurrentCurrency(),
      ));
  ...
  public function createOrderTransaction($viewer, $order, $user, array $params = array()) {  
    ...
    $videoItem = Engine_Api::_()->getItem('sesvideo_video', $orderSubscriber->video_id);
...
//     }
    ...
    if( $params['state'] == 'cancel' ) {
      ...
      if(empty($orderSubscriber->state) || empty($orderSubscriber->order_id))
        return;
      Engine_Api::_()->getDbtable('orders', 'sesvideosell')->update(array('state' => 'failed'), array('order_id =?' => $orderSubscriber->order_id));
      // Error
    ...
    // Insert transaction
    $transactionsTable = Engine_Api::_()->getDbtable('transactions', 'sesvideosell');
    $transactionsTable->insert(array(
      ...
      'video_id' => $orderSubscriber->video_id,
    ));
    
    // Get benefit setting
    $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sesvideosell')->getBenefitStatus($user); 
    ...
    if( $paymentStatus == 'okay' || ($paymentStatus == 'pending' && $giveBenefit) ) {
      ...
      $select = $tableRemaining->select()->from($tableName)->where('user_id =?',$videoItem->owner_id);
      $select = $tableRemaining->fetchAll($select);
      if(count($select)){
        $tableRemaining->update(array('remaining_payment' => new Zend_Db_Expr("remaining_payment + $orderAmount")),array('user_id =?'=>$videoItem->owner_id));
      } else {
        $tableRemaining->insert(array(
          'remaining_payment' => $orderAmount,
          'user_id' => $videoItem->owner_id,
        ));
      }

      Engine_Api::_()->getDbtable('orders', 'sesvideosell')->update(array('state' => 'complete'), array('order_id =?' => $orderSubscriber->order_id));
      ...
      if( $orderSubscriber->state == 'complete' ) {
        ...
        $video = Engine_Api::_()->getItem('sesvideo_video', $orderSubscriber->video_id);
        
        $total_amount = Engine_Api::_()->sesbasic()->getCurrencyPrice($orderSubscriber->total_amount, $orderSubscriber->currency_symbol);
        $total_useramount = Engine_Api::_()->sesbasic()->getCurrencyPrice($orderSubscriber->total_useramount, $orderSubscriber->currency_symbol);
        $commission_amount = Engine_Api::_()->sesbasic()->getCurrencyPrice($orderSubscriber->commission_amount, $orderSubscriber->currency_symbol);
        $videoPhoto = ( isset($_SERVER["HTTPS"]) && (strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] .Zend_Registry::get('StaticBaseUrl') . $video->getPhotoUrl();
        
        $body = '<table cellpadding="0" cellspacing="0" style="background:#f0f4f5;border:3px solid #f1f4f5;border-collapse:collapse;border-spacing:0;float:none;margin:0 auto;max-width:608px;padding:0;text-align:center;vertical-align:top;width:450px;max-width:100%;font-family: Arial,Helvetica,sans-serif;"><tr><td style="text-align:center;padding:10px;"><table cellpadding="0" cellspacing="0" style="width:100%;"><tr><td style="padding:10px 0;color:#555;font-size:13px;">Free stock photos of video · Pexels</td></tr><tr><td style="background-color:#fff;text-align:center;"><div style="width:150px;height:150px;float:left;"><a href="'.$video->getHref().'"><img src="'.$videoPhoto.'" alt="" style="width:100%;height:100%;object-fit:cover;" /></a></div><div style="display: block; overflow: hidden; padding: 10px; text-align: left;"><a href="'.$video->getHref().'" style="color: rgb(85, 85, 85); font-size: 17px; text-decoration: none; font-weight: bold;">'.$video->getTitle().'</a></div></td></tr><tr><td style="height:20px;"></td></tr><tr><td><table style="color:#555;border:1px solid #ddd;background-color: rgb(255, 255, 255); width: 100%;font-size:13px;" cellspacing="0" cellpadding="10"><tbody><tr>';
        ...
        //Notification send to Video Owner When some one purchased video
        $user = Engine_Api::_()->getItem('user', $orderSubscriber->user_id);
        $owner = $video->getOwner();
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $user, $video, 'sesvideosell_purchased_videoowner');
        
        //Ticket invoice mail to buyer
        Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'sesvideosell_videoinvoice_buyer', array('invoice_body' => $body, 'host' => $_SERVER['HTTP_HOST']));

        //Video Purchased Mail to Video Owner
        $owner = Engine_Api::_()->getItem('user', $video->owner_id);
        Engine_Api::_()->getApi('mail', 'core')->sendSystem($owner, 'sesvideosell_purchased_videoowner', array('video_title' => $video->getTitle(), 'object_link' => $user->getHref(), 'buyer_name' => $user->getTitle(), 'host' => $_SERVER['HTTP_HOST']));
        
        //Video Purchased Mail send to admin
        ...
        foreach($datas as $data) {
          ...
          Engine_Api::_()->getApi('mail', 'core')->sendSystem($adminUser, 'sesvideosell_purchased_adminemail', array('video_title' => $video->getTitle(), 'object_link' => $user->getHref(), 'buyer_name' => $user->getTitle(), 'host' => $_SERVER['HTTP_HOST'], 'total_amount' => $total_amount, 'total_useramount' => $total_useramount, 'commission_amount' => $commission_amount));
        }
      ...
      Engine_Api::_()->getDbtable('orders', 'sesvideosell')->update(array('state' => 'pending'), array('order_id =?' => $orderSubscriber->order_id));
  ...
  {
    return new Sesvideosell_Form_Admin_Gateway_PayPal();
  }
