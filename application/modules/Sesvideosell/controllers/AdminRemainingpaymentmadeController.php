...
class Sesvideosell_AdminRemainingpaymentmadeController extends Core_Controller_Action_Admin {
  
  public function indexAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesvideosell_admin_main', array(), 'sesvideosell_admin_remainingpaymentmade');

    $userTableName = Engine_Api::_()->getItemTable('sesvideosell_remainingpayment')->info('name');
    ...
    $transactionsTableName = $transactionsTable->info('name');

    $select = $ordersTable->select()
            ->setIntegrityCheck(false)
            ->from($ordersTableName, array('totalAmount' => new Zend_Db_Expr("(sum(amount))")))
            ->joinLeft($userTableName, "$userTableName.remainingpayment_id = $ordersTableName.source_id", 'user_id')
            ->joinLeft($transactionsTableName, "$transactionsTableName.order_id = $ordersTableName.order_id", 'amount')
            ->where($ordersTableName . '.state = ?', 'complete')
            ->where($ordersTableName.'.source_type =?', 'sesvideosell_remainingpayment')
            ->group($ordersTableName.'.source_id');
  ...
  public function approveAction() {
	  ...
	  $remainingpaymentItem = Engine_Api::_()->getItem('sesvideosell_remainingpayment', $this->getRequest()->getParam('id'));  
    ...
    $this->_helper->layout->setLayout('admin-simple');
      ...
      //Make form
      $this->view->form = $form = new Sesvideosell_Form_Admin_Payment_Approve();
      ...
      if($this->_getParam('id',false)) {
        $item = Engine_Api::_()->getItem('sesvideosell_remainingpayment', $this->_getParam('id'));
        if($item) {
...
//       }
      ...
      $db = Engine_Api::_()->getDbtable('remainingpayments', 'sesvideosell')->getAdapter();
      $db->beginTransaction();
      try {
        ...
        return $this->_forward('success', 'utility', 'core', array(
              'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'sesvideosell', 'controller' => 'paymentmade', 'action' => 'process', 'remainingpayment_id' => $remainingpaymentItem->remainingpayment_id),'admin_default',true),
...
// 			return $this->_forward('requireauth', 'error', 'core');
		...
		$item = Engine_Api::_()->getItem('sesvideosell_remainingpayment', $remainingpayment_id);
    ...
    $ordersTable->insert(array(
        ...
        'source_type' => 'sesvideosell_remainingpayment',
        'source_id' => $remainingpayment_id,
    ));
    
		$session = new Zend_Session_Namespace();
    $session->sesvideosell_order_id = $order_id = $ordersTable->getAdapter()->lastInsertId(); 
		$session->sesvideosell_item_id = $remainingpayment_id; //$item->getIdentity();    
    // Prepare host info
    $schema = 'http://';
    ...
    $params['return_url'] = $schema . $host
      .  $this->view->url(array('action' => 'return', 'controller' => 'paymentmade', 'module' => 'sesvideosell'), 'admin_default', true)
    ...
    // Get order
		...
		$item_id = $session->sesvideosell_item_id ; 
		$item = Engine_Api::_()->getItem('sesvideosell_remainingpayment', $item_id);
    if (!$orderPayment || ($orderId != $orderPaymentId) ||
			 ($orderPayment->source_type != 'sesvideosell_remainingpayment') ||
  ...
  public function finishAction() {
    ...
    $session = new Zend_Session_Namespace();
    if (!empty($session->sesvideosell_order_id))
      $session->sesvideosell_order_id = '';
		//if(empty($session->sesvideosell_order_id))
			//return $this->_forward('notfound', 'error', 'core');
    $orderTrabsactionDetails = array('state' => $this->_getParam('state'), 'errorMessage' => $session->errorMessage);
    $session->sesvideosell_order_details = $orderTrabsactionDetails;
	  ...
	  if(!$state)
		...
		$session->unsetAll();
		if($state == 'active')
      return $this->_helper->redirector->gotoRoute(array('controller' => 'paymentmade', 'module' => 'sesvideosell'), 'admin_default', true);
  } 
