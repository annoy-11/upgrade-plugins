...
<?php include APPLICATION_PATH . '/application/modules/Sesdiscussion/views/scripts/_showDiscussionListGrid.tpl'; ?>
...
<?php if(!$this->is_ajax):?>
  <script type="application/javascript"> 
	...
	requestTab_<?php echo $randonNumber; ?> = new Request.HTML({
	  method: 'post',
	  'url': en4.core.baseUrl+"widget/index/mod/sesdiscussion/name/<?php echo $this->widgetName; ?>/openTab/"+valueTab,
	  'data': {
	    format: 'html',  
	  ...
	  onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
	    ...
	    if(sesJqueryObject('.pin_selectView_<?php echo $randonNumber;?>').hasClass('active')) {
	      if(document.getElementById('sesdiscussion_pinboard_view_<?php echo $randonNumber;?>'))
	      document.getElementById('sesdiscussion_pinboard_view_<?php echo $randonNumber;?>').style.display = 'block';
