...
<script type="text/javascript">
  ...
  en4.core.runonce.add(function() {
    ...
    <?php if( !$this->renderOne ): ?>
      ...
      $('sesdiscussion_profile_previous').style.display = '<?php echo ( $this->paginator->getCurrentPageNumber() == 1 ? 'none' : '' ) ?>';
      $('sesdiscussion_profile_next').style.display = '<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() ? 'none' : '' ) ?>';

      $('sesdiscussion_profile_previous').removeEvents('click').addEvent('click', function() {
      ...
      $('sesdiscussion_profile_next').removeEvents('click').addEvent('click', function(){
        en4.core.request.send(new Request.HTML({
          url : en4.core.baseUrl + 'widget/index/content_id/' + <?php echo sprintf('%d', $this->identity) ?>,
          data : {
            format : 'html',
...
<?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
    ...
    <ul class="prelative sesdiscussions_listing sesbasic_pinboard_<?php echo $randonNumber ; ?>" style="min-height:50px;" id="tabbed-widget_<?php echo $randonNumber; ?>" >
              ...
              <div id="sesdiscussion_description_content_<?php echo $item->getIdentity(); ?>" class='sesdiscussion_feed_discussion _des'>
                ...
                <?php if(in_array('title', $this->allParams['stats']) && !empty($item->title)) { ?>
                  <div class="sesdiscussion_title">  
                    ...
                    <?php } else { ?>
                      <a data-url='sesdiscussion/index/discussion-popup/discussion_id/<?php echo $item->getIdentity(); ?>' class="sessmoothbox" href="javascript:;"><?php echo $item->title; ?></a>
                    <?php } ?>
                  </div>
                <?php } ?>
                <div class="sesdiscussion_discussion">
                  <?php echo nl2br($item->discussiontitle); ?>
                </div>
    ...
    <div>
      <div id="sesdiscussion_profile_previous" class="paginator_previous">
        <?php echo $this->htmlLink('javascript:void(0);', $this->translate('Previous'), array(
          'onclick' => '',
          'class' => 'buttonlink icon_previous'
        )); ?>
      </div>
      <div id="sesdiscussion_profile_next" class="paginator_next">
...
<?php else:?>
  <div class="sesbasic_tip">
    ...
    <span>
      <?php echo $this->translate('There are no discussion entry yet.'); ?>
    </span>
  </div>
<?php endif; ?>
...
<script type="application/javascript">
  ...
  sesJqueryObject(document).on('click', '.sesdiscussion_expand', function() {

    sesJqueryObject(this).parent().find('._des').css('max-height','none');
    //sesJqueryObject(this).hide();
    sesJqueryObject(this).closest('.sesdiscussions_list_item').addClass('sesdiscussion_content_open');
