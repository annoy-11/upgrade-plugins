...
<script type="application/javascript">
  ...
  function submitCompliment(formObject) {
    ...
    sesJqueryObject.ajax({
      url: "sescrowdfundingteam/index/edit-designation/",
      type: "POST",
      contentType:false,
      processData: false,
      cache: false,
...
//           }
          if(sesJqueryObject('#sescrowdfunding_dashboard_team_desinations').length) {
            if(sesJqueryObject('#team_tip').length)
              sesJqueryObject('#team_tip').hide();
            sesJqueryObject('#sescrowdfunding_dashboard_team_desinations').show();
            sesJqueryObject('#sescrowdfunding_dashboard_team_desinations').html(result.message);
          }
        }
      }
    });
