 ...
 */
class Sescrowdfundingteam_Model_DbTable_Designations extends Engine_Db_Table {

  protected $_rowClass = "Sescrowdfundingteam_Model_Designation";

  public function getDesignations($params = array()) {

    $select = $this->select()
            ->from($this->info('name'), array('designation_id', 'designation'))
            ->where('crowdfunding_id =?', 0);
  ...
  public function getAllDesignations($params = array()) {
    ...
    $select = $this->select()
                  ->from($this->info('name'))
                  ->order('order ASC');
    if(isset($params['crowdfunding_id']) && !empty($params['crowdfunding_id'])) {
