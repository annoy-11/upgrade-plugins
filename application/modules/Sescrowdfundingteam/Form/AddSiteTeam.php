 ...
 */
class Sescrowdfundingteam_Form_AddSiteTeam extends Engine_Form {
  ...
  public function init() {
    ...
    $crowdfunding_id = Zend_Controller_Front::getInstance()->getRequest()->getParam('crowdfunding_id');

    $this
            ->setMethod('POST')
            ->setAttrib('name', 'sescrowdfundingteam_addteam')
    ...
    if ($type == 'nonsitemember') {
      ...
      $team = Engine_Api::_()->getItem('sescrowdfundingteam_team', $team_id);
      $photo_id = 0;
    ...
    $designations = Engine_Api::_()->getDbtable('designations', 'sescrowdfundingteam')->getDesignations();

    $allCrowdfundingDesignations = Engine_Api::_()->getDbtable('designations', 'sescrowdfundingteam')->getAllDesignations(array('crowdfunding_id' => $crowdfunding_id));
    if(count($allCrowdfundingDesignations) > 0) {
      foreach ($allCrowdfundingDesignations as $designation) {
    ...
    $this->addElement('Textarea', "description", array(
        ...
        'description' => "Enter short description about the team member. [The description should be of 300 characters only. This description would be shown on the Team Crowdfunding.]",
        'maxlength' => '300',
        'filters' => array(
            'StripTags',
    ...
    $this->addElement('TinyMce', 'detail_description', array(
        ...
        'description' => 'Enter detailed description about the team member. [This description would be shown on the Member Profile crowdfunding.]',
        'editorOptions' => $editorOptions,
    ));

    $this->addElement('Text', "email", array(
        'label' => 'Email',
        'description' => "Enter email address of the team member which will be displayed on the team crowdfunding.",
    ));
    ...
    $this->addElement('Text', "facebook", array(
        'label' => 'Facebook URL',
        'description' => "Enter URL of the Facebook Profile or Crowdfunding of the team member.",
    ));
