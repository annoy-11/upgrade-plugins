...
<table class='admin_table'>
  <thead>
    <tr>
      ...
      <th><?php echo $this->translate("Reset Links") ?></th>
    </tr>
  </thead>
  <tbody>
    <?php foreach ($this->pagesArray as $item):
    $corePages = Engine_Api::_()->sesqa()->getwidgetizePage(array('name' => $item));
    ...
    <tr>
      ...
      <td>
        <?php if($corePages->name != 'sesqa_index_view' && $corePages->name != 'sesqa_category_index'):?>
      ...
      <td>
          <a title="<?php echo $this->translate('Reset Page'); ?>" href="<?php echo $this->url(array('module'=> 'sesqa', 'controller' => 'settings', 'action' => 'reset-page-settings', 'page_id' => $corePages->page_id, 'page_name' => $item,'format' => 'smoothbox'),'admin_default',true); ?>" class=" smoothbox"><?php echo $this->translate('Reset Page'); ?></a>
        </td>
    </tr>
    <?php $results = ''; ?>
    <?php endforeach; ?>
