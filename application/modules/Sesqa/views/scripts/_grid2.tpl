 ...
 * @version    $Id: _grid2.tpl  2018-10-15 00:00:00 SocialEngineSolutions $
...
<?php if(!$this->is_ajax){ ?>
<div class="sesqa_grid_views_container sesbasic_clearfix sesbasic_bxs">
...
<?php } ?>
    <?php if(count($this->paginator)){ ?>
    	<?php foreach($this->paginator as $question){ ?>
    		<li class="sesqa_gridview_item sesbasic_bg bottom-border sesbasic_cleardix" style="<?php if($this->width){ ?>width:<?php echo $this->width.'px;'; ?> <?php } ?><?php if($this->height){ ?>height:<?php echo $this->height.'px;'; ?> <?php } ?>">
          ...
          <ul class="sesqa_views_box">
            <?php $votecount = $question->upvote_count - $question->downvote_count; ?>
            <?php if(in_array('vote',$this->showOptions)){ ?>
              <li><p><span class="round-grey"><?php echo $votecount; ?></span><span><?php echo $votecount == 1 ? $this->translate('Vote') : $this->translate('Votes'); ?></span></p></li>
            <?php } ?>
            <?php if(in_array('answerCount',$this->showOptions)){ ?>
              <li class="<?php if($question->best_answer){ ?>sesqa_list_best<?php } ?>"><p><span class="round-grey"><?php echo $question->answer_count; ?></span><span><?php echo $question->answer_count == 1 ? $this->translate('Answer') : $this->translate('Answers'); ?></span></p></li>
            <?php } ?>   
            <?php if(in_array('view',$this->showOptions)){ ?>
              <li><p><span class="round-grey"><?php echo $question->view_count; ?></span><span><?php echo $question->view_count == 1 ? $this->translate('View') : $this->translate('Views'); ?></span></p></li>
            <?php } ?> 
          </ul>
       ...
       <?php 
        ...
        <div class="sesqa_toggle_option">
          ...
          <div class="sesbasic_pulldown_options">
            <ul class="_isicon">
             <?php if($canEdit){ ?>
             <li><a href="<?php echo $this->url(array('action' => 'edit','question_id' => $question->getIdentity()),'sesqa_general'); ?>"><i class="fa fa-edit"></i><?php echo $this->translate("SESEdit"); ?></a></li>
            <?php } ?>
            <?php if($canDelete){ ?>
    ...
    <?php } ?>
    <?php }else{ ?>
    <div class="sesqa_noquestion_tip_wrapper norecord<?php echo $this->widgetIdentity; ?>" id="norecord<?php echo $this->widgetIdentity; ?>">
      <div class="sesqa_noquestion_tip clearfix"> <img src="<?php echo Engine_Api::_()->sesqa()->getFileUrl(Engine_Api::_()->getApi('settings', 'core')->getSetting('qanda_notip_image', 'application/modules/Sesqa/externals/images/notip.png')); ?>" alt="" /> 
