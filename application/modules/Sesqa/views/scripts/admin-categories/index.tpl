...
<?php $this->headScript()->appendFile($this->layout()->staticBaseUrl . 'externals/ses-scripts/odering.js'); ?>
<style>
.error {
	color:#FF0000;
}
...
<div class='sesqa-form sesqa-categories-form'>
  <div>
    ...
    <div class="sesqa-form-cont">
      ...
      <p class="description"> Question categories can be managed here. To create new categories, use "Add New Category" form below. Here, you can also choose Slug URL, Title, Description, Profile Type to be associated with the category, Icon and Thumbnail. You can also map Categories with the Profile Types, so that questions belonging to the mapped Profile Type will appear to users while creating / editing Questions when they choose the associated Category.
<br><br>
To create 2nd-level categories and 3rd-level categories, choose respective 1st-level and 2nd-level category from “Parent Category” dropdown. Choose this carefully as you will not be able to edit Parent Category later.
<br><br>
To reorder the categories, click on their names or row and drag them up or down.
</p>
      <div class="sesqa-categories-add-form">
        <h4 class="bold">Add New Category</h4>
        <form id="addcategory" method="post" enctype="multipart/form-data">
          <div class="sesqa-form-field" id="name-required">
            <div class="sesqa-form-field-label">
              <label for="tag-name">Name</label>
            </div>
            <div class="sesqa-form-field-element">
              <input name="category_name" autocomplete="off" id="tag-name" type="text"  size="40" >
              <p>The name is how it appears on your site.</p>
            </div>
          </div>
          <div class="sesqa-form-field" id="slug-required">
            <div class="sesqa-form-field-label">
...
<!--          <div class="sesqa-form-field">
            ...
            <div class="sesqa-form-field-element">
              ...
              <p>This will be category color & will appear in Grid View of qas.</p>
            </div>
          </div>-->
          <div class="sesqa-form-field">
            <div class="sesqa-form-field-label">
            ...
            <div class="sesqa-form-field-element">
              <select name="parent" id="parent" class="postform">
               ...
               <?php foreach ($this->categories as $category): ?>
                ...
                <?php 
                  $subcategory = Engine_Api::_()->getDbtable('categories', 'sesqa')->getModuleSubcategory(array('column_name' => "*", 'category_id' => $category->category_id));          foreach ($subcategory as $sub_category):  
                ?>
          ...
          <div class="sesqa-form-field">
            <div class="sesqa-form-field-label">
              <label for="tag-memberlevel">Member Levels</label>
            </div>
            <div class="sesqa-form-field-element">
              <select name="member_levels[]" id="tag-memberlevel" multiple="multiple">
                <?php 
                  foreach($levels as $level){ ?>
                    <option value="<?php echo $level->getIdentity(); ?>"><?php echo $level->getTitle(); ?></option>
                  <?php
                  }
                ?>
              </select>
          ...
          <div class="sesqa-form-field">
            <div class="sesqa-form-field-label">
              <label>Upload Icon</label><span style="font-size: 11px;"> [Recommended size is: 24px * 24px.]</span>
            </div>
            <div class="sesqa-form-field-element">
          ...
          <div class="sesqa-form-field">
            <div class="sesqa-form-field-label">
              <label>Upload Big Icons</label><span style="font-size: 11px;"> [Recommended size is: 250px * 250px.]</span>
            </div>
            <div class="sesqa-form-field-element">
          ...
          <div class="sesqa-form-field">
            <div class="sesqa-form-field-label">
              <label>Upload Banner Photo</label><span style="font-size: 11px;"> [Recommended size is: 950px * 500px.] </span>
            </div>
            <div class="sesqa-form-field-element">
              <input type="file" name="colored_icon" id="colored_icon" alt="Colored Icon" onchange="readImageUrl(this,'colored_icon_preview')" />
              <span style="display:none" class="error" id="colored_icon-msg"></span>
            </div>
          </div>
          <div class="form-wrapper" id="colored_icon_preview-wrapper" style="display: none;">
          	<div class="form-label" id="colored_icon_preview-label">&nbsp;</div>
            <div class="form-element" id="colored_icon_preview-element">
            	<input width="100" type="image" height="100" alt="Icon" src="" id="colored_icon_preview" name="colored_icon_preview">
            </div>
          </div>
          
          <div class="submit sesqa-form-field">
            <button type="button" id="submitaddcategory" class="upload_image_button button">Add New Category</button>
          </div>
        </form>
        <div class="sesqa-categories-add-form-overlay" id="add-category-overlay" style="display:none"></div>
      </div>
      <div class="sesqa-categories-listing">
            ...
            <tbody>
              ...
              <tr id="categoryid-<?php echo $category->category_id; ?>" data-article-id="<?php echo $category->category_id; ?>">
                ...
                <td>
                  <?php if($category->cat_icon): ?>
                    <?php $icon = Engine_Api::_()->storage()->get($category->cat_icon); 
                    if($icon) { 
                      $icon = $icon->getPhotoUrl('thumb.icon'); ?>
                      <img class="sesqa-category-icon" src="<?php echo $icon; ?>" />
                    <?php } else { ?>
                      <?php echo "---"; ?>
                    <?php } ?>
                  <?php else: ?>
                ...
                <td><?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesqa', 'controller' => 'categories', 'action' => 'edit-category', 'id' => $category->category_id), $this->translate('Edit'), array()) ?> 
                <?php if($category->category_id != 0){ ?>
              ...
              <tr id="categoryid-<?php echo $sub_category->category_id; ?>" data-article-id="<?php echo $sub_category->category_id; ?>">
                ...
                <td>
                  <?php if($sub_category->cat_icon): ?>
                    <?php $cat_icon = Engine_Api::_()->storage()->get($sub_category->cat_icon); $catURL = ''; ?>
                    <?php if($cat_icon) { 
                      $catURL = $cat_icon->getPhotoUrl(); 
                    ?>
                      <img class="sesqa-category-icon" src="<?php echo $catURL; ?>" />
                    <?php } else { ?>
                      <?php echo "---"; ?>
                    <?php } ?>
                  <?php else: ?>
                ...
                <td><?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesqa', 'controller' => 'categories', 'action' => 'edit-category', 'id' => $sub_category->category_id), $this->translate('Edit'), array()) ?> | <?php echo $this->htmlLink('javascript:void(0);', $this->translate('Delete'), array('class' => 'deleteCat','data-url'=>$sub_category->category_id)) ?> 		</td>
              </tr>
              <?php 
                		//SubSubcategory Work
              ...
              <tr id="categoryid-<?php echo $subsub_category->category_id; ?>" data-article-id="<?php echo $subsub_category->category_id; ?>">
                ...
                <td><?php if($subsub_category->cat_icon): ?>
                    <?php $cat_icon = Engine_Api::_()->storage()->get($subsub_category->cat_icon); $catURL = ''; ?>
                    <?php if($cat_icon) { 
                      $catURL = $cat_icon->getPhotoUrl(); 
                    ?>
                      <img class="sesqa-category-icon" src="<?php echo $catURL; ?>" />
                ...
                <td><?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesqa', 'controller' => 'categories', 'action' => 'edit-category', 'id' => $subsub_category->category_id), $this->translate('Edit'), array()) ?> | <?php echo $this->htmlLink('javascript:void(0);', $this->translate('Delete'), array('class' => 'deleteCat','data-url'=>$subsub_category->category_id)) ?>
              </tr>
              <?php endforeach; ?>
              <?php endforeach; ?>
              <?php endforeach; ?>
...
ajaxurl = en4.core.baseUrl+"admin/sesqa/categories/change-order";
...
jqueryObjectOfSes (document).ready(function (e) {
    jqueryObjectOfSes ('#addcategory').on('submit',(function(e) {
            ...
            scrollTop: jqueryObjectOfSes('#addcategory').offset().top },
            1000
       		 );
					error = true;
			}else{
        ...
        jqueryObjectOfSes .ajax({
            ...
            success:function(data){
                jqueryObjectOfSes('#colored_icon_preview-wrapper').css('display','none');
                ...
                parent = jqueryObjectOfSes('#parent').val();
								...
								console.log(scrollUpTo,jqueryObjectOfSes(scrollUpTo).offset().top);
								jqueryObjectOfSes('html, body').animate({
									scrollTop: jqueryObjectOfSes(scrollUpTo).offset().top },
									1000
								 );
