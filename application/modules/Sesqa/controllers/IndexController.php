...
//       }
  ...
  public function editAction(){
    ...
    try {
      ...
      if(!empty($_FILES['photo']['name']) && $values['mediatype'] == 1)
          $question->setPhoto($_FILES['photo']);
      ...
      //update location data in sesbasic location table
      $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
      $dbGetInsert->query('DELETE FROM `engine4_sesbasic_locations` WHERE `engine4_sesbasic_locations`.`resource_type` = "'.$question->getType().'" AND `engine4_sesbasic_locations`.`resource_id` = "'.$question->getIdentity().'";');
      $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type, country, state, city, zip) VALUES ("' . $question->getIdentity() . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","'.$question->getType().'", "' . $_POST['country'] . '", "' . $_POST['state'] . '", "' . $_POST['city'] . '", "' . $_POST['zip'] . '")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
