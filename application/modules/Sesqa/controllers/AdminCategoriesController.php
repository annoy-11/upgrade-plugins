 ...
 * @version    $Id: AdminCategoriesController.php  2018-10-15 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */


class Sesqa_AdminCategoriesController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    if (isset($_POST['selectDeleted']) && $_POST['selectDeleted']) {
      ...
      if (isset($_POST['data']) && is_array($_POST['data'])) {
        ...
        $qaTable = Engine_Api::_()->getDbtable('questions', 'sesqa');
        foreach ($_POST['data'] as $key => $valueSelectedcategory) {

          $categoryDelete = Engine_Api::_()->getItem('sesqa_category', $valueSelectedcategory);

          $deleteCategory = Engine_Api::_()->getDbtable('categories', 'sesqa')->deleteCategory($categoryDelete);

          if ($deleteCategory) {
            $deleteCategoryIds[] = $categoryDelete->category_id;
						//delete all category level

							$qaTable->update(array(
								'category_id' => 0,
							), array(
    ...
    if (isset($_POST['is_ajax']) && $_POST['is_ajax'] == 1) {
      ...
      $slugExists = Engine_Api::_()->getDbtable('categories', 'sesqa')->slugExists($value['slug']);
      if (!$slugExists) {
        echo json_encode(array('slugError' => true));
        die;
      }
      if ($cat_id != -1) {
        $categoryData = Engine_Api::_()->getItem('sesqa_category', $cat_id);
        if ($categoryData->subcat_id == 0) {
          $value['subcat_id'] = $cat_id;
          $seprator = '&nbsp;&nbsp;&nbsp;';
          $tableSeprator = '-&nbsp;';
          $parentId = $cat_id;
          $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesqa')->orderNext(array('subcat_id' => $cat_id));
        } else {
          $value['subsubcat_id'] = $cat_id;
          $seprator = '3';
          $tableSeprator = '--&nbsp;';
          $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesqa')->orderNext(array('subsubcat_id' => $cat_id));
          $parentId = $cat_id;
        }
      } else {
        $parentId = 0;
        $seprator = '';
        $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesqa')->orderNext(array('category_id' => true));
        $tableSeprator = '';
      }
      $db = Engine_Db_Table::getDefaultAdapter();
      $db->beginTransaction();
      try {
        $categoriesTable = Engine_Api::_()->getDbtable('categories', 'sesqa');
        //Create row in categories table
        $row = $categoriesTable->createRow();
        $row->setFromArray($value);
        $memberLevels = implode($_POST['member_levels'],',');
        if($memberLevels){
          $row->member_levels = $memberLevels.',';
        }else{
          $row->member_levels = '';
        }
        $row->save();
  ...
  public function changeOrderAction() {
    if ($this->_getParam('id', false) || $this->_getParam('nextid', false)) {
      ...
      $categoryTypeId = '';
      $checkTypeCategory = $dbObject->query("SELECT * FROM engine4_sesqa_categories WHERE category_id = " . $category_id)->fetchAll();
      ...
      if ($checkTypeCategory)
        $currentOrder = Engine_Api::_()->getDbtable('categories', 'sesqa')->order($categoryType, $categoryTypeId);
      ...
      $categoryTable = Engine_Api::_()->getDbtable('categories', 'sesqa');
      ...
      $checkCategoryChildrenCondition = $dbObject->query("SELECT * FROM engine4_sesqa_categories WHERE subcat_id = '" . $id . "' || subsubcat_id = '" . $id . "' || subcat_id = '" . $nextid . "' || subsubcat_id = '" . $nextid . "'")->fetchAll();
  ...
  public function editCategoryAction() {
    ...
    $this->view->form = $form = new Sesqa_Form_Admin_Category_Edit();

    $cat_id = $this->_getParam('id');

    $category = Engine_Api::_()->getItem('sesqa_category', $cat_id);
    $form->populate($category->toArray());
    if($category->member_levels){
      $form->member_levels->setValue(explode(',',$category->member_levels));
    }
...
//       $slugExists = Engine_Api::_()->getDbtable('categories', 'sesqa')->slugExists($value['slug'], $cat_id);
//       if (!$slugExists) {
...
//     }
    ...
    try {
      ...
      $memberLevels = implode($values['member_levels'],',');
      if($memberLevels){
          $category->member_levels = $memberLevels.',';
      }else{
        $category->member_levels = '';
      }
      ...
      if (isset($_FILES['colored_icon']) && !empty($_FILES['colored_icon']['name'])) {
        $coloredIconId = $this->setPhoto($form->colored_icon, $cat_id);

        if (!empty($coloredIconId)) {
          if ($previousColoredIcon && $deleteCI) {
            $coloredIcon = Engine_Api::_()->getItem('storage_file', $previousColoredIcon);
            if($coloredIcon)
               $coloredIcon->delete();
          }
          $category->colored_icon = $coloredIconId;
          $category->save();
    ...
    return $this->_helper->redirector->gotoRoute(array('module' => 'sesqa', 'action' => 'index', 'controller' => 'categories'), 'admin_default', true);
  }
  ...
  protected function setPhoto($photo, $cat_id, $resize = false) {
    ...
    try {
      ...
      if ($e->getCode() == Storage_Model_DbTable_Files::SPACE_LIMIT_REACHED_CODE) {
        throw new Sesqa_Model_Exception($e->getMessage(), $e->getCode());
      } else {
        throw $e;
      }
    }
