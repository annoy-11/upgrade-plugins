...
<div class="sessportz_banner_container_wrapper sesbasic_bxs<?php if($this->is_full){ ?> isfull<?php } ?> " style="height:<?php echo $this->height.'px'; ?>;">
	<div class="sessportz_banner_container">
    <div class="sessportz_banner_img_container" style="height:<?php echo $this->height.'px'; ?>;">
        <?php if($this->banner_image): ?>
	        <img src="<?php echo Engine_Api::_()->sessportz()->getFileUrl($this->banner_image) ?>" alt="" />
        <?php else: ?>
	        <img src="<?php echo $this->baseUrl() . '/application/modules/Sessportz/externals/images/paralex-background.jpg' ?>" alt="" />
        <?php endif; ?>
    </div>
  <div class="sessportz_banner_content" style="height:<?php echo $this->height.'px'; ?>;<?php if(!$this->is_pattern){ ?>background:none; <?php } ?>">
  	<div class="sessportz_banner_content_inner">
      <?php if($this->banner_title != '' || $this->description  != '') { ?>	
        <?php if($this->banner_title != ''){ ?>
          <h2 class="sessportz_banner_title" style="color:#<?php echo $this->title_button_color; ?>"><?php echo $this->banner_title; ?></h2>
        <?php } ?>
      <?php } ?>
      <?php if($this->description  != ''){ ?>
        <p class="sessportz_banner_des" style="color:#<?php echo $this->description_button_color; ?>"><?php echo $this->description ; ?></p>
      <?php } ?>
      <?php if($this->button1 || $this->button2 || $this->button3){ ?>
        <div class="sessportz_banner_btns">
          <?php if($this->button1){ ?>
