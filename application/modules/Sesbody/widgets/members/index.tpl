...
<?php $this->headScript()->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sesbody/externals/scripts/owl.carousel.min.js'); ?>


<?php $baseUrl = $this->layout()->staticBaseUrl; ?>
<div class="sesbody_lp_members_block sesbasic_bxs">
	<div class="sesbody_lp_members_block_content">
    <?php if($this->heading): ?>
      <h2><?php echo $this->translate("%s members and counting ...", $this->member_count); ?></h2>
    <?php endif; ?>
    <?php if(!$this->showType): ?>
	    <ul class="clearfix sesbody_lp_members_block_list">
	      <?php foreach( $this->paginator as $user ): ?>
	        <li class="sesbody_lp_members_thumb" style="height:<?php echo $this->height ?>px; width:<?php echo $this->width ?>px;">
    ...
    <?php else: ?>
	    <div class="clearfix sesbody_lp_members_carousel" id="sesbody_lp_members_carousel">
	      <?php foreach( $this->paginator as $user ): ?>
	        <div class="sesbody_lp_members_thumb">
	          <a href="<?php echo $user->getHref() ?>" class ="item_thumb" title="<?php echo $user->displayname ?>">
	            <?php $url = $user->getPhotoUrl('thumb.profile'); ?>
	            <?php if ($url == ''){$url = $this->layout()->staticBaseUrl ."application/modules/User/externals/images/nophoto_user_thumb_profile.png";
	            } ?>
...
<script>
  jquery1_8_2SesObject(document).ready(function() {
		jquery1_8_2SesObject("#sesbody_lp_members_carousel").owlCarousel({
		navigation: true,
		pagination: false,
