 ...
 * @version    $Id: AdminCustomThemeController.php  2019-02-16 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesbody_AdminCustomThemeController extends Core_Controller_Action_Admin {

  public function indexAction() {
    
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesbody_admin_main', array(), 'sesbody_admin_main_customcss');
    
    $writeable = array();
    $writeable['sesbody'] = false;
    try {
      foreach(array('theme.css', 'constants.css', 'media-queries.css', 'sesbody-custom.css') as $file) {
        if(!file_exists(APPLICATION_PATH . "/application/themes/sesbody/$file")) {
          throw new Core_Model_Exception('Missing file');
        } else {
          $this->checkWriteable(APPLICATION_PATH . "/application/themes/sesbody/$file");
        }
      }
      $writeable['sesbody'] = true;
    } catch(Exception $e) {
      $this->view->errorMessage = $e->getMessage();
    }
    
    $this->view->writeable = $writeable;
    $this->view->activeFileContents = file_get_contents(APPLICATION_PATH . '/application/themes/sesbody/sesbody-custom.css');
  }
  ...
  public function saveAction() {
    ...
    //Check file
    $basePath = APPLICATION_PATH . '/application/themes/sesbody';
