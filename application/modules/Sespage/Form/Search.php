 ...
 * @version    $Id: Search.php  2018-04-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sespage_Form_Search extends Engine_Form {
  ...
  public function init() {
    ...
    $hideClass = 'sespage_widget_advsearch_hide_' . $identity;
    ...
    $this->setAction($view->url(array('module' => 'sespage', 'controller' => 'index', 'action' => 'index'), 'default', true));
    $viewer = Engine_Api::_()->user()->getViewer();
    if (in_array('searchPageTitle', $show_criterias)) {
      $this->addElement('Text', 'search', array(
          'label' => 'Search Pages',
          'class' => in_array('searchPageTitle', $show_advSearch) ? $hideClass : '',
      ));
    }
    ...
    if (isset($show_criterias) && in_array('browseBy', $show_criterias)) {
      switch ($params['default_search_type']) {
        ...
        case 'mostSPcommented':
          ...
          break;
        case 'mostSPrated':
          $default_search_type = 'rating DESC';
          break;
        case 'lowestSPrated':
          $default_search_type = 'rating ASC';
          break;
      ...
      foreach ($params['search_type'] as $key => $widgetOption) {
        ...
        switch ($columnValue) {
          ...
          case 'mostSPcommented':
            ...
            break;
           case 'mostSPrated':
            $columnValue = 'rating DESC';
            $value = 'Rating: High to Low';
            break;
           case 'lowestSPrated':
            $columnValue = 'rating ASC';
    ...
    if (in_array('view', $show_criterias)) {
      ...
      foreach ($params['criteria'] as $key => $viewOption) {
        ...
        switch ($columnValue) {
          case '0':
            $value = 'All Page Directories';
            break;
          case '1':
            $value = 'Only My Friend\'s Page Directories';
            break;
          case '2':
            $value = 'Only My Network Page Directories';
            break;
          case '3':
            $value = 'Only My Pages Directories';
            break;
          case '4':
            $value = 'Open Pages';
            break;
          case '5':
            $value = 'Closed Page Directories';
            break;
          case '6':
            $value = 'Only 5 Star Reviews';
            break;
          case '7':
            $value = 'Only 4 Star Reviews';
            break;
          case '8':
            $value = 'Only 3 Star Reviews';
            break;
          case '9':
            $value = 'Only 2 Star Reviews';
            break;
          case '10':
            $value = 'Only 1 Star Reviews';
            break;
        }
        $filterOptions[$columnValue] = ucwords($value);
      }
    ...
    if (in_array('location', $show_criterias) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sespage_enable_location', 1)) {
      ...
      if (isset($params['kilometer_miles']) && $params['kilometer_miles'] && Engine_Api::_()->getApi('settings', 'core')->getSetting('enableglocation', 1)) {
    ...
    if (in_array('closepage', $show_criterias)) {
      $this->addElement('Checkbox', 'is_close_page', array(
          'label' => 'Include Closed Page Directories',
          'class' => in_array('closepage', $show_advSearch) ? $hideClass : '',
              //'decorators' => array(
//              'ViewHelper',
//              array('Description', array()),
//             // array('Label', array('placement' => 'APPEND', 'tag' => 'label')),
//              // array('HtmlTag', array('tag' => 'li', 'class' => 'is_close_page'))
//          ),
      ));
    }
