...
<div class="sesbasic_cloud_widget sesbasic_clearfix">
        ...
        <ul>
          <?php foreach($this->paginator as $valueTags):?>
            <?php if($valueTags['text'] == '' || empty($valueTags['text'] )):?>
              <?php continue; ?>
            <?php endif;?>
            <li><a href="<?php echo $this->url(array('module' =>'sespage','controller' => 'index', 'action' => 'browse'),'sespage_general',true).'?tag_id='.$valueTags['tag_id'].'&tag_name='.$valueTags['text']  ;?>"><?php echo $valueTags['text'] ?></a></li>
          <?php endforeach;?>
        </ul>
        </canvas>
    </div>
  <?php else:?>
    <div class="sespage_tags_cloud_page sesbasic_bxs ">
      <ul class="sespage_tags_cloud_list">
        <?php foreach($this->paginator as $valueTags):?>
          <?php if($valueTags['text'] == '' || empty($valueTags['text'] )):?>
            <?php continue; ?>
          <?php endif;?>
  ...
  <?php endif;?>
  <a href="<?php echo $this->url(array('action' => 'tags'),'sespage_general',true);?>" class="sesbasic_more_link clear"><?php echo $this->translate("See All Tags");?> &raquo;</a>
</div>
