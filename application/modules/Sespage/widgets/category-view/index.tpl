...
<?php if($this->viewer()->getIdentity() && Engine_Api::_()->getApi('settings', 'core')->getSetting('sespage.allowfollow.category', 1)):?>
 <?php $showFollowButton = 1;?>
<?php endif;?>
<?php if($showFollowButton):?>
  <?php $followStatus = Engine_Api::_()->getDbTable('followers', 'sespage')->isFollow(array('resource_id' => $this->category->category_id,'resource_type' => 'sespage_category')); ?>
...
<?php if(isset($this->category->thumbnail) && !empty($this->category->thumbnail)){ ?>
  <div class="sespage_category_cover sesbasic_bxs sesbm sesbasic_bxs">
    <div class="sespage_category_cover_inner">
    	<div class="sespage_category_cover_img" style="background-image:url(<?php echo  Engine_Api::_()->storage()->get($this->category->thumbnail)->getPhotoUrl('thumb.thumb'); ?>);"></div>
      <div class="sespage_category_cover_content">
        <div class="sespage_category_cover_breadcrumb">
          <!--breadcrumb -->
          <a href="<?php echo $this->url(array('action' => 'browse'), "sespage_category"); ?>"><?php echo $this->translate("Categories"); ?></a>&nbsp;&raquo;
          ...
          <?php if($showFollowButton && isset($this->params['show_follow_button']) && $this->params['show_follow_button']):?>
            <a href='javascript:;' data-url='<?php echo $this->category->category_id; ?>'  data-status='<?php echo $followStatus;?>' class="sesbasic_animation sesbasic_link_btn sespage_category_follow sespage_category_follow_<?php echo $this->category->category_id; ?>"><i class='fa <?php echo $followClass ; ?>'></i><span><?php echo $followText; ?></span></a> 
          <?php endif;?>
        </div>
        <div class="sespage_category_cover_blocks">
          ...
          <div class="sespage_category_cover_block_info">
            ...
            <?php if(count($this->paginatorc)){ ?>
              <div class="sespage_category_cover_pages">
              	<div class="sespage_category_cover_pages_head">
                 	<?php echo $this->params['pop_title']; ?>
                </div>
               	<?php foreach($this->paginatorc as $pagesCri){ ?>
...
<?php } else { ?>
  ...
  <div class="sespage_browse_cat_top sesbm">
    <?php if(isset($this->category->title) && !empty($this->category->title)): ?>
      <div class="sespage_catview_title centerT"> 
        <?php echo $this->category->title; ?>
      </div>
    <?php endif; ?>
    <?php if(isset($this->category->description) && !empty($this->category->description)): ?>
      <div class="sespage_catview_des centerT">
        <?php echo nl2br($this->category->description);?>
      </div>
    <?php endif; ?>
    <?php if($showFollowButton && isset($this->params['show_follow_button']) && $this->params['show_follow_button']):?>
      <div class="sespage_catview_button centerT">
        <a href='javascript:;' data-url='<?php echo $this->category->category_id; ?>'  data-status='<?php echo $followStatus;?>' class="sesbasic_animation sesbasic_link_btn sespage_category_follow sespage_category_follow_<?php echo $this->category->category_id; ?>"><i class='fa <?php echo $followClass ; ?>'></i><span><?php echo $followText; ?></span></a>
      </div>
    <?php endif;?>
  </div>
  <?php if(count($this->paginatorc)){ ?>
      ...
      <ul class="clear sesbasic_clearfix _iscatitem">
      	<?php foreach($this->paginatorc as $page){ ?>
...
<?php if($this->params['show_subcat'] == 1 && count($this->innerCatData)>0){ ?>
	<div class="sespage_catview_categories sesbasic_clearfix">
    <div class="sespage_catview_list_title clear sesbasic_clearfix">
      <span class="_title"><?php echo $this->params['subcategory_title']; ?></span>
    </div>
    <div class="sesbasic_clearfix">
      <ul class="sespage_category_grid_listing sesbasic_clearfix clear sesbasic_bxs">	
        <?php foreach( $this->innerCatData as $page ): ?>
          <li class="sespage_category_grid sesbm" style="height:<?php echo is_numeric($this->params['heightSubcat']) ? $this->params['heightSubcat'].'px' : $this->params['heightSubcat'] ?>;width:<?php echo is_numeric($this->params['widthSubcat']) ? $this->params['widthSubcat'].'px' : $this->params['widthSubcat'] ?>;">
            <a href="<?php echo $page->getHref(); ?>">
              <div class="sespage_category_grid_img">
                <?php if($page->thumbnail != '' && !is_null($page->thumbnail) && intval($page->thumbnail)){ ?>
                  <span class="sesbasic_animation" style="background-image:url(<?php echo  Engine_Api::_()->storage()->get($page->thumbnail)->getPhotoUrl('thumb.thumb'); ?>);"></span>
                <?php } ?>
              </div>
              <div class="sespage_category_grid_overlay sesbasic_animation"></div>
              <div class="sespage_category_grid_info">
                <div>
                  <div class="sespage_category_grid_details">
                    <?php if(isset($this->iconSubcatActive) && $page->cat_icon != '' && !is_null($page->cat_icon) && intval($page->cat_icon)){ ?>
                      <img src="<?php echo  Engine_Api::_()->storage()->get($page->cat_icon)->getPhotoUrl('thumb.icon'); ?>" />
                    <?php } ?>
                    <?php if(isset($this->titleSubcatActive)){ ?>
                    <span><?php echo $page->category_name; ?></span>
                    <?php } ?>
                    <?php if(isset($this->countPagesSubcatActive)){ ?>
                      <span class="sespage_category_grid_stats"><?php echo $this->translate(array('%s page', '%s pages', $page->total_pages_categories), $this->locale()->toNumber($page->total_pages_categories))?></span>
                    <?php } ?>
                  </div>
                </div>
              </div>
...
<div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" class="sesbasic_clearfix sesbasic_bxs clear">    
  <div class="sespage_catview_list_title clear sesbasic_clearfix">
    <span class="_title"><?php echo $this->params['page_title']; ?></span>
  </div>
   <ul class="sespage_cat_page_listing sesbasic_clearfix clear _iscatitem" id="tabbed-widget_<?php echo $randonNumber; ?>">
<?php } ?>
    ...
    <?php  if(  $totalCount == 0){  ?>
      <div class="tip">
        <span>
        	<?php echo $this->translate("No pages in this  category."); ?>
          <?php if (!$this->can_edit):?>
                    <?php echo $this->translate('Be the first to %1$spost%2$s one in this category!', '<a href="'.$this->url(array('action' => 'create'), "sespage_general").'">', '</a>'); ?>
          <?php endif; ?>
        </span>
      </div>
    <?php } ?>    
...
function dynamicWidth(){
	var objectClass = jqueryObjectOfSes('.sespage_cat_page_list_info');
	for(i=0;i<objectClass.length;i++){
			jqueryObjectOfSes(objectClass[i]).find('div').find('.sespage_cat_page_list_content').find('.sespage_cat_page_list_title').width(jqueryObjectOfSes(objectClass[i]).width());
	}
}
dynamicWidth();
<?php } ?>
