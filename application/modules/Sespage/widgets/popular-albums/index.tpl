...
<ul class="sespage_album_listings sesbasic_bxs">
  <?php $limit = 0; ?>
  <?php foreach( $this->paginator as $item ): ?>
    ...
    <li id="thumbs-photo-<?php echo $item->photo_id ?>" class="sespage_album_list_grid_thumb sespage_album_list_grid sespa-i-<?php echo (isset($widgetParams['insideOutside']) && $widgetParams['insideOutside'] == 'outside') ? 'outside' : 'inside'; ?> sespa-i-<?php echo (isset($widgetParams['fixHover']) && $widgetParams['fixHover'] == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($widgetParams['width']) ? $widgetParams['width'].'px' : $widgetParams['width'] ?>;">  
    	<article>
        <a class="sespage_album_list_grid_img" href="<?php echo $item->getHref(); ?>" style="height:<?php echo is_numeric($widgetParams['height']) ? $widgetParams['height'].'px' : $widgetParams['height'] ?>;">
        ...
        <?php  if(in_array('socialSharing', $widgetParams['show_criteria']) || in_array('likeButton', $widgetParams['show_criteria']) || in_array('favouriteButton', $widgetParams['show_criteria'])){  ?>
          <span class="sespage_album_list_grid_btns">
            ...
            if(in_array('likeButton', $widgetParams['show_criteria']) && Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && $canComment){  ?>
              ...
              <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $item->getType(); ?>" data-url='<?php echo $item->album_id; ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_like_btn sespage_albumlike <?php echo ($albumLikeStatus) ? 'button_active' : '' ; ?>">
            ...
            if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && in_array('favouriteButton', $widgetParams['show_criteria'])) {
              $albumFavStatus = Engine_Api::_()->getDbTable('favourites', 'sespage')->isFavourite(array('resource_type'=>$item->getType(),'resource_id'=>$item->getIdentity())); ?>
              <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $item->getType(); ?>" data-url='<?php echo $item->album_id; ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_fav_btn sespage_albumFav <?php echo ($albumFavStatus)>0 ? 'button_active' : '' ; ?>">
        ...
        <?php if(in_array('like', $widgetParams['show_criteria']) || in_array('comment', $widgetParams['show_criteria']) || in_array('view', $widgetParams['show_criteria']) || in_array('title', $widgetParams['show_criteria']) || in_array('photoCount', $widgetParams['show_criteria']) || in_array('favouriteCount', $widgetParams['show_criteria']) || in_array('by', $widgetParams['show_criteria']) || in_array('pagename', $widgetParams['show_criteria'])){ ?>
          <p class="sespage_album_list_grid_info sesbasic_clearfix<?php if(!in_array('photoCount', $widgetParams['show_criteria'])) { ?> nophotoscount<?php } ?>">
            ...
            <span class="sespage_album_list_grid_stats">
              <?php if(in_array('pagename', $widgetParams['show_criteria'])) { ?>
              ...
              <?php } ?>
              <?php if(SESPAGESHOWUSERDETAIL == 1 && in_array('by', $widgetParams['show_criteria'])) { ?>
                <span class="sespage_album_list_grid_owner">
