...
class Sespage_Widget_photoViewPageController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
		...
		if(Engine_Api::_()->core()->hasSubject('sespage_photo') && !$is_ajax)
      $photo = Engine_Api::_()->core()->getSubject('sespage_photo');
		else if(isset($_POST['photo_id'])) {
      $photo = Engine_Api::_()->getItem('sespage_photo',$_POST['photo_id']);
      Engine_Api::_()->core()->setSubject($photo); 
      $photo = Engine_Api::_()->core()->getSubject();
		} else
      return $this->setNoRender();
		
		$this->view->page =	$page = Engine_Api::_()->getItem('sespage_page', $photo->page_id);
      ...
      $this->view->canEdit = $canEdit = $editPageRolePermission ? $editPageRolePermission : $page->authorization()->isAllowed($viewer, 'edit');
      $this->view->canComment = $canComment = $page->authorization()->isAllowed($viewer, 'comment');
      $this->view->canDelete = $canDelete = $deletePageRolePermission ? $deletePageRolePermission : $page->authorization()->isAllowed($viewer, 'delete');
      $this->view->canTag = $canTag = $page->authorization()->isAllowed($viewer, 'tag');
      $this->view->canCommentMemberLevelPermission = Engine_Api::_()->authorization()->getPermission($viewer, 'sespage_page', 'comment');
		}
