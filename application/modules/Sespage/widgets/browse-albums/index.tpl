...
<?php if(isset($this->identityForWidget) && !empty($this->identityForWidget)){
 ...
 <div class="sespagea_album_search_result sesbasic_clearfix sesbm" id="<?php echo !$this->is_ajax ? 'paginator_count_sespage' : 'paginator_count_ajax_sespage' ?>">	
  <?php if($this->paginator->getTotalItemCount() > 0) { ?>
  <span id="total_item_count_sespage" style="display:inline-block;"><?php echo $this->paginator->getTotalItemCount(); ?></span> <?php echo $this->translate("albums found."); ?>
  <?php } ?>
 </div>
 <?php if(!$this->is_ajax){ ?>
 ...
 <?php }
    foreach( $this->paginator as $album ): ?>
      <?php $page = Engine_Api::_()->getItem('sespage_page', $album->page_id); ?>
      <?php if($this->view_type == 1){ ?>
        <li id="thumbs-photo-<?php echo $album->photo_id ?>" class="sespage_album_list_grid_thumb sespage_album_list_grid sespa-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sespa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">  
            ...
            <span class="sespage_album_list_grid_btns">
             ...
             ?>
              ...
              if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->likeButton) && $canComment){  ?>
                <!--Album Like Button-->
                <?php $albumLikeStatus = Engine_Api::_()->sespage()->getLikeStatus($album->getIdentity(), $album->getType()); ?>
                <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $album->getType(); ?>" data-url='<?php echo $album->album_id; ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_like_btn sespage_albumlike <?php echo ($albumLikeStatus) ? 'button_active' : '' ; ?>">
                  <i class="fa fa-thumbs-up"></i>
                  <span><?php echo $album->like_count; ?></span>
                </a>
              <?php } 
              if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->favouriteButton)){
                $albumFavStatus = Engine_Api::_()->getDbTable('favourites', 'sespage')->isFavourite(array('resource_type'=>'sespage_album','resource_id'=>$album->getIdentity())); ?>
                <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $album->getType(); ?>" data-url='<?php echo $album->getIdentity(); ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_fav_btn sespage_albumFav <?php echo ($albumFavStatus) ? 'button_active' : '' ; ?>">
         ...
         <?php } ?>
         <?php if(isset($this->like) || isset($this->comment) || isset($this->view) || isset($this->title) || isset($this->rating) || isset($this->photoCount) || isset($this->favouriteCount) || isset($this->downloadCount)  || isset($this->by) || isset($this->pagename)){ ?>
              ...
              <span class="sespage_album_list_grid_stats">
                <?php if(isset($this->pagename)) { ?>
                  <span class="sespage_album_list_grid_owner">
                    <?php echo $this->translate('in ');?>
                   <?php echo $this->htmlLink($page->getHref(), $page->getTitle(), array('class' => 'thumbs_author')) ?>
                  </span>
                <?php }?>
              ...
              <span class="sespage_album_list_grid_stats sesbasic_text_light">
               ...
               <?php if(isset($this->favouriteCount)) { ?>
                  <span class="sespage_album_list_grid_fav" title="<?php echo $this->translate(array('%s favourite', '%s favourites', $album->favourite_count), $this->locale()->toNumber($album->favourite_count))?>">
                    <i class="fa fa-heart"></i> 
      ...
      <?php }else{ ?>
      		<?php $image = Engine_Api::_()->sespage()->getAlbumPhoto($album->getIdentity(),$album->photo_id,3);  ?>
          ...
          <li class="sespage_album_list_thumbnail_view" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">
            <div class="sespage_album_thumbnail_view_main_img" style="height:<?php echo is_numeric($heightDiv) ? $heightDiv.'px' : $heightDiv ?>;">
              <?php $albumPhoto = Engine_Api::_()->getItem('sespage_photo', $album->photo_id); ?>
              <a class="ses-image-viewer" onclick="openLightBoxForSesPlugins('<?php echo $albumPhoto->getHref()	; ?>', '<?php echo $album->getPhotoUrl(); ?>')" href="<?php echo $albumPhoto->getHref(); ?>"><span class="sespage_album_thumbnail_view_thumb" style="background-image:url(<?php echo $albumPhoto->getPhotoUrl('thumb.normalmain'); ?>);"></span></a>
            </div>
						<?php 
            		if(count($image) != 0) { ?>
                <div class="sespage_album_thumbnail_view_thumbs thumbs<?php echo count($image); ?> clear sesbasic_clearfix">
             		<?php	foreach($image as $key=>$valuePhoto) { ?>
                <div>
                  <?php $imageURL = $valuePhoto->getPhotoUrl('thumb.normalmain'); ?>
                  <a onclick="openLightBoxForSesPlugins('<?php echo $valuePhoto->getHref(); ?>', '<?php echo $valuePhoto->getPhotoUrl()	; ?>')" href="<?php echo $valuePhoto->getHref(); ?>" class="ses-image-viewer"><span class="sespage_album_thumbnail_view_thumb" style="background-image:url(<?php echo $imageURL;  ?>);"></span></a>
                </div>
            <?php } ?>
            		</div>
            <?php	} ?>
            <div class="sespage_album_thumbnail_view_btm clear sesbasic_clearfix">
              <div class="sespage_album_thumbnail_owner_photo">
              	<?php $user = Engine_Api::_()->getItem('user',$album->owner_id) ?>
                <a href="<?php echo $user->getHref();; ?>">
                  <?php echo $this->itemPhoto($user, 'thumb.profile'); ?>
                 </a>          
              </div>
              <div class="sespage_album_thumbnail_album_info">
                <p class="sespage_album_thumbnail_album_name">
                  <?php if(isset($this->title)) { ?>
                   <?php echo $this->htmlLink($album, $this->string()->truncate($album->getTitle(), $this->title_truncation),array('title'=>$album->getTitle())) ; ?>
                  <?php } ?>
                </p>
                <p class="sespage_album_thumbnail_album_by sesbasic_clearfix sesbasic_text_light"> 
                  <?php if(isset($this->pagename)) { ?>
                    <span>
                      <?php echo $this->translate('in ');?>
                       <?php echo $this->htmlLink($page->getHref(), $page->getTitle(), array('class' => 'thumbs_author')) ?>
               ...
               <?php } ?>
                <p class="sespage_album_thumbnail_album_stats sespage_album_list_stats sesbasic_clearfix sesbasic_text_light">
                	<?php if(isset($this->like)) { ?>
            ...
            <?php  if(isset($this->socialSharing) || isset($this->likeButton) || isset($this->favouriteButton)){  ?>
            	<p class="sespage_album_list_btns sespage_album_thumbnail_album_btns">
             	<?php if(isset($this->socialSharing)){ 
              //album viewpage link for sharing
                $urlencode = urlencode(((!empty($_SERVER["HTTPS"]) &&  strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $album->getHref());
             ?>
              ...
              $canComment = $album->authorization()->isAllowed($this->viewer, 'comment');
                if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->likeButton) && $canComment){  ?>
              	...
              	<?php } 
                 if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->favouriteButton)){
                  $albumFavStatus = Engine_Api::_()->getDbTable('favourites', 'sespage')->isFavourite(array('resource_type'=>'album','resource_id'=>$album->album_id)); ?>
                    <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $album->getType(); ?>" data-url='<?php echo $album->album_id; ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_fav_btn sespage_albumFav <?php echo ($albumFavStatus) ? 'button_active' : '' ; ?>">
                      <i class="fa fa-heart"></i>
                      <span><?php echo $album->favourite_count; ?></span>
                    </a>
                <?php } ?>
...
function paggingNumber<?php echo $randonNumber; ?>(pageNum){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "widget/index/mod/sespage/name/browse-albums",
      'data': {
        format: 'html',
        page: pageNum,    
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        ...
        document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML =  responseHTML;
				sesJqueryObject('#paginator_count_sespage').find('#total_item_count_sespage').html(sesJqueryObject('#paginator_count_ajax_sespage').find('#total_item_count_sespage').html());
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/sespage/name/browse-albums/index/',
      'data': {
        format: 'html',
