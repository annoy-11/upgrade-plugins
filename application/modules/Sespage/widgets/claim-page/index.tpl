...
<div class="sespage_profile_tabs_top claim_request active sesbasic_clearfix">
...
<?php if($checkClaimRequest):?>
	<div class="sespage_profile_tabs_top claim_new_page sesbasic_clearfix">
...
<script type="text/javascript">
	en4.core.runonce.add(function() {
		var contentAutocomplete = new Autocompleter.Request.JSON('title', "<?php echo $this->url(array('module' => 'sespage', 'controller' => 'index', 'action' => 'get-pages'), 'default', true) ?>", {
			'postVar': 'text',
			'minLength': 1,
			'selectMode': 'pick',
	...
	function validateForm(){
		...
		counter = 0;
		sesJqueryObject('#sespage_claim_create input, #sespage_claim_create select,#sespage_claim_create checkbox,#sespage_claim_create textarea,#sespage_claim_create radio').each(
			function(index){
	...
	en4.core.runonce.add(function() {
		sesJqueryObject('#sespage_claim_create').submit(function(e){
	...
	function sendDataToServer(object){
		...
		sesJqueryObject.ajax({
			type:'POST',
			url: en4.core.baseUrl + "widget/index/mod/sespage/name/claim-page/",
			data:formData,
			cache:false,
			contentType: false,
			processData: false,
			success:function(data){
				...
				window.location.href = '<?php echo $this->url(array("action"=>"claim-requests"),"sespage_general",true); ?>';
			},
			error: function(data){
			}
		});
