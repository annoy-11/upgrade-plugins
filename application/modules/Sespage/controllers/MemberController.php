 ...
 */
class Sespage_MemberController extends Core_Controller_Action_Standard {
  ...
  public function joinAction() {
    ...
    // Make form
    $this->view->form = $form = new Sespage_Form_Member_Join();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      try {
        ...
        if(Engine_Api::_()->getDbTable('notifications','sespage')->getNotifications(array('page_id'=>$subject->getIdentity(), 'type'=>'notification_type','role'=>'new_joinee','notification_type'=>'site_notification', 'user_id' => $owner->getIdentity()))) {
          Engine_Api::_()->getDbTable('notifications', 'activity')->addNotification($owner, $viewer, $subject, 'sespage_page_join');
        }
        
        if(Engine_Api::_()->getDbTable('notifications','sespage')->getNotifications(array('page_id'=>$subject->getIdentity(), 'type'=>'notification_type','role'=>'new_joinee','notification_type'=>'email_notification', 'user_id' => $owner->getIdentity()))) {
          Engine_Api::_()->getApi('mail', 'core')->sendSystem($subject->getOwner(), 'notify_sespage_page_pagejoined', array('page_title' => $subject->getTitle(), 'sender_title' => $viewer->getOwner()->getTitle(), 'object_link' => $subject->getHref(), 'host' => $_SERVER['HTTP_HOST']));
        }
        
        // Page admin notifications and email work
        $getAllPageAdmins = Engine_Api::_()->getDbTable('pageroles', 'sespage')->getAllPageAdmins(array('page_id' => $subject->getIdentity(), 'user_id' => $subject->owner_id));
        foreach($getAllPageAdmins as $getAllPageAdmin) {
          $pageadmin = Engine_Api::_()->getItem('user', $getAllPageAdmin->user_id);
          if(Engine_Api::_()->getDbTable('notifications','sespage')->getNotifications(array('page_id'=>$subject->getIdentity(), 'type'=>'notification_type','role'=>'new_joinee','notification_type'=>'site_notification', 'user_id' => $pageadmin->getIdentity()))) {
            Engine_Api::_()->getDbTable('notifications', 'activity')->addNotification($owner, $viewer, $subject, 'sespage_page_join');
          }
          if(Engine_Api::_()->getDbTable('notifications','sespage')->getNotifications(array('page_id'=>$subject->getIdentity(), 'type'=>'notification_type','role'=>'new_joinee','notification_type'=>'email_notification', 'user_id' => $pageadmin->getIdentity()))) {
            Engine_Api::_()->getApi('mail', 'core')->sendSystem($subject->getOwner(), 'notify_sespage_page_pagejoined', array('page_title' => $subject->getTitle(), 'sender_title' => $viewer->getOwner()->getTitle(), 'object_link' => $subject->getHref(), 'host' => $_SERVER['HTTP_HOST']));
          }
        }

        //Send to all joined members
        $joinedMembers = Engine_Api::_()->sespage()->getallJoinedMembers($subject);
        foreach($joinedMembers as $joinedMember) {
          ...
          Engine_Api::_()->getDbTable('notifications', 'activity')->addNotification($joinedMember, $viewer, $subject, 'sespage_page_pagesijoinedjoin');
          
          Engine_Api::_()->getApi('mail', 'core')->sendSystem($subject->getOwner(), 'notify_sespage_page_joinpagejoined', array('page_title' => $subject->getTitle(), 'sender_title' => $viewer->getTitle(), 'object_link' => $subject->getHref(), 'host' => $_SERVER['HTTP_HOST']));
        }
        
        $followerMembers = Engine_Api::_()->getDbTable('followers', 'sespage')->getFollowers($subject->getIdentity());
        foreach($followerMembers as $followerMember) {
          ...
          Engine_Api::_()->getDbTable('notifications', 'activity')->addNotification($followerMember, $viewer, $subject, 'sespage_page_pagesifollowedjoin');
          
          Engine_Api::_()->getApi('mail', 'core')->sendSystem($subject->getOwner(), 'notify_sespage_page_joinedpagefollowed', array('page_title' => $subject->getTitle(), 'sender_title' => $viewer->getTitle(), 'object_link' => $subject->getHref(), 'host' => $_SERVER['HTTP_HOST']));
        }
      ...
      return $this->_forward('success', 'utility', 'core', array(
                  'messages' => array(Zend_Registry::get('Zend_Translate')->_('Page joined')),
  ...
  public function requestAction() {
    ...
    // Make form
    $this->view->form = $form = new Sespage_Form_Member_Request();
  ...
  public function cancelAction() {
    ...
    // Make form
    $this->view->form = $form = new Sespage_Form_Member_Cancel();
  ...
  public function leaveAction() {
    ...
    // Make form
    $this->view->form = $form = new Sespage_Form_Member_Leave();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      return $this->_forward('success', 'utility', 'core', array(
                  'messages' => array(Zend_Registry::get('Zend_Translate')->_('Page left')),
                  'layout' => 'default-simple',
                  'parentRefresh' => true,
      ));
    }
  ...
  public function acceptAction() {
    ...
    if ($this->_helper->contextSwitch->getCurrentContext() == "smoothbox") {
      return $this->_forward('success', 'utility', 'core', array(
                  'messages' => array(Zend_Registry::get('Zend_Translate')->_('Page invite accepted')),
                  'layout' => 'default-simple',
                  'parentRefresh' => true,
      ));
    }
  ...
  public function rejectAction() {
    ...
    // Make form
    $this->view->form = $form = new Sespage_Form_Member_Reject();
  ...
  public function removeAction() {
    ...
    if (!$page->membership()->isMember($user)) {
      throw new Sespage_Model_Exception('Cannot remove a non-member');
    }

    // Make form
    $this->view->form = $form = new Sespage_Form_Member_Remove();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      return $this->_forward('success', 'utility', 'core', array(
                  'messages' => array(Zend_Registry::get('Zend_Translate')->_('Page member removed.')),
                  'layout' => 'default-simple',
                  'parentRefresh' => true,
      ));
    }
  ...
  public function approveAction() {
    ...
    // Make form
    $this->view->form = $form = new Sespage_Form_Member_Approve();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      return $this->_forward('success', 'utility', 'core', array(
                  'messages' => array(Zend_Registry::get('Zend_Translate')->_('Page request approved')),
                  'layout' => 'default-simple',
                  'parentRefresh' => true,
      ));
    }
