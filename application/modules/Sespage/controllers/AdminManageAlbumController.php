 ...
 */
class Sespage_AdminManageAlbumController extends Core_Controller_Action_Admin {

  public function indexAction() {
  
   $this->view->navigation  = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespage_admin_main', array(), 'sespage_admin_main_mgalbumphoto');
    
   $this->view->subNavigation  = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespage_admin_main_mgalbumphoto', array(), 'sespage_admin_main_managealbum');
      
		$this->view->formFilter = $formFilter = new Sespage_Form_Admin_Manage_FilterAlbum();
    ...
    if( $this->getRequest()->isPost() ) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $album = Engine_Api::_()->getItem('sespage_album', $value);
          $album->delete();
        }
      }
    }
    
		$tableAlbum = Engine_Api::_()->getDbTable('albums', 'sespage');
    ...
    $select = $tableAlbum->select()
												 ...
												 ->join($tablePageName, "$tablePageName.page_id = $tableAlbumName.page_id", null)
    ...
    if( !empty($_GET['pagetitle']) ) 
      $select->where($tablePageName.'.title LIKE ?', '%' . $values['pagetitle'] . '%');
      
    if( !empty($values['creation_date']) ) 
      $select->where($tablePageName.'.date(creation_date) = ?', $values['creation_date'] );
  ...
  public function featuredalbumAction() {
    ...
    if (isset($_SERVER['HTTP_REFERER']))
      ...
      $url = 'admin/sespage/manage-album';
    $this->_redirect($url);
  }
  ...
  public function sponsoredalbumAction() {
   ...
   $this->view->subNavigation  = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespage_admin_main_mgalbumphoto', array(), 'sespage_admin_main_managephotos');
    
		$this->view->formFilter = $formFilter = new Sespage_Form_Admin_Manage_FilterAlbum(array('albumTitle' =>'yes'));
    ...
    foreach( $_GET as $key => $value ) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $photo = Engine_Api::_()->getItem('sespage_photo', $value);
          $photo->delete();
        }
      }
    }
				
		$tablePhoto = Engine_Api::_()->getDbTable('photos', 'sespage');
    ...
    $select =  $tablePhoto->select()
													...
													->joinLeft($tablePageName, "$tablePageName.page_id = $tablePhotoName.page_id", null)
  ...
  public function sponsoredphotoAction() {
    ...
    if( $this->getRequest()->isPost())
    {
      ...
      try
      {
        $photo = Engine_Api::_()->getItem('sespage_photo', $id);
  ...
  public function deleteAction()
  {
    ...
    if( $this->getRequest()->isPost())
    {
      ...
      try
      {
        $album = Engine_Api::_()->getItem('sespage_album', $id);
