 ...
 * @version    $Id: AlbumController.php  2018-04-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sespage_AlbumController extends Core_Controller_Action_Standard {
  ...
  public function createAction() {
    ...
    if($album_id) {
    	$this->view->album = $album = Engine_Api::_()->getItem('sespage_album', $album_id);
			$this->view->page_id = $page_id = $album->page_id;
		} else {
      $this->view->page_id = $page_id = $page_id;
		}
    ...
    $this->view->current_count = Engine_Api::_()->getDbTable('albums', 'sespage')->getUserAlbumCount($values);
    $this->view->quota = $quota = 0;

    $sespage_sespagealbumcreate = Zend_Registry::isRegistered('sespage_sespagealbumcreate') ? Zend_Registry::get('sespage_sespagealbumcreate') : null;
    if(!empty($sespage_sespagealbumcreate)) {
      // Get form
      $this->view->form = $form = new Sespage_Form_Album();
    }
...
//     if (!$this->_helper->requireAuth()->setAuthParams($page, null, 'album')->isValid()) {
//       return;
//     }
    ...
    try {
      ...
      $photo->setFromArray(array(
        ...
        if($type == 'wall') {
            $viewer = Engine_Api::_()->user()->getViewer();
                $table = Engine_Api::_()->getDbTable('albums', 'sespage');
            $album = $table->getSpecialAlbum($viewer, $type,$page->page_id);
        }
      $photo->save();
      ...
      $db->commit();
    }catch (Sespage_Model_Exception $e) {
      $db->rollBack();
      $this->view->status = false;
  ...
  public function viewAction() {
    ...
    if ($viewer->getIdentity() != 0 && isset($album->album_id)) {
      ...
      $dbObject->query('INSERT INTO engine4_sespage_recentlyviewitems (resource_id, resource_type,owner_id,creation_date ) VALUES ("' . $album->album_id . '", "sespage_album","' . $viewer->getIdentity() . '",NOW())	ON DUPLICATE KEY UPDATE	creation_date = NOW()');
    }
	...
	public function homeAction() {
			// Render
    $sespage_sespagealbumhome = Zend_Registry::isRegistered('sespage_sespagealbumhome') ? Zend_Registry::get('sespage_sespagealbumhome') : null;
    if(!empty($sespage_sespagealbumhome)) {
    ...
    // Render
    $sespage_sespagebrowsealbum = Zend_Registry::isRegistered('sespage_sespagebrowsealbum') ? Zend_Registry::get('sespage_sespagebrowsealbum') : null;
    if(!empty($sespage_sespagebrowsealbum)) {
      $this->_helper->content->setEnabled();
    }
	}
  ...
  public function getAlbumAction() {
    ...
    $albums = Engine_Api::_()->getDbTable('albums', 'sespage')->getAlbumsAction($value);
  ...
  public function editAction() {
    ...
    $this->view->album = $album = Engine_Api::_()->getItem('sespage_album', $album_id);
			...
			return;
		$this->view->page = $page = Engine_Api::_()->getItem('sespage_page', $album->page_id);
		if ($page) {
    ...
    // Make form
    $this->view->form = $form = new Sespage_Form_Album_Edit();
  ...
  public function deleteAction() {
    ...
    $this->view->form = $form = new Sespage_Form_Album_Delete();
    if (!$album) {
      $this->view->status = false;
    ...
    return $this->_forward('success', 'utility', 'core', array(
      'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('id'=>$page->custom_url), 'sespage_profile', true),
  ...
  public function editphotosAction() {
    ...
    $pageNumber = isset($_POST['page']) ? $_POST['page'] : 1;
		...
		if($album_id){
		 $this->view->album =	$album = Engine_Api::_()->getItem('sespage_album', $album_id);
			$this->view->page_id = $page_id = $album->page_id;
			$page = $this->view->sespage = Engine_Api::_()->getItem('sespage_page', $page_id);
		}else{
    ...
    if (!$is_ajax) {
      ...
      foreach ($navigation->getPages() as $page) {
        if ($page->route != 'sespage_general' || $page->action != 'manage')
          continue;
        $page->active = true;
      }
    }
    ...
    // Get albums
    $myAlbums = Engine_Api::_()->getDbTable('albums', 'sespage')->editPhotos();
    $albumOptions = array('' => '');
    ...
    // Make form
    $this->view->form = $form = new Sespage_Form_Album_Photos();
    foreach ($paginator as $photo) {
      $subform = new Sespage_Form_Album_EditPhoto(array('elementsBelongTo' => $photo->getGuid()));
      ...
      foreach ($paginator as $photo) {
        ...
        if (isset($values['delete']) && $values['delete'] == '1') {
					if($album->photo_id == $photo->photo_id){
						$album->photo_id = 0;
						$photoCn = Engine_Api::_()->getDbTable('photos', 'sespage')->getPhotoSelect(array('album_id'=>$album->album_id,'fetchAll'=>true,'limit_data'=>1));
						if(count($photoCn)){
          ...
          if (($nextPhoto instanceof Sespage_Model_Photo) &&
    ...
    return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'album_id' => $album->album_id), 'sespage_specific_album', true);
  }
  ...
  public function editPhotoAction() {
    ...
    $this->view->photo = Engine_Api::_()->getItem('sespage_photo', $photo_id);
  }
  ...
  public function saveInformationAction() {
    ...
    Engine_Api::_()->getDbTable('photos', 'sespage')->update(array('title' => $title, 'description' => $description), array('photo_id = ?' => $photo_id));
  }
