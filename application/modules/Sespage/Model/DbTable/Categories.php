 ...
 * @version    $Id: Categories.php  2018-04-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sespage_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Sespage_Model_Category';
  ...
  public function getCategory($params = array())
  {
    ...
    $pageTableName = Engine_Api::_()->getDbTable('pages', 'sespage')->info('name');
    $category_select = $category_select->setIntegrityCheck(false);
    $category_select = $category_select->joinLeft($pageTableName, "$pageTableName.category_id=$tableName.category_id AND (`is_approved` = 1 AND `" . $pageTableName . "`.`draft` != 0)", array("total_page_categories" => "COUNT($pageTableName.page_id)"));
    $category_select = $category_select->group("$tableName.category_id");
    if (isset($params['criteria']) && $params['criteria'] == 'most_page')
      $category_select->order('total_page_categories DESC');
    if (isset($params['criteria']) && $params['criteria'] != 'most_page')
    ...
    if (isset($params['hasPage'])) {
      ...
      $category_select = $category_select->having('total_page_categories > 0');
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('sespage_category');
  ...
  public function getCategoriesAssoc($params = array()) {
    ...
    if ($viewerId && @$params['member_levels'] == 1) {
      ...
      $stmt->where('CONCAT(engine4_sespage_categories.member_levels," ") LIKE "%' . $levelId . '%"');
    }
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countPages'])) {
      $pageTable = Engine_Api::_()->getDbTable('pages', 'sespage')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($pageTable, "$pageTable.subcat_id=$tableName.category_id", array("total_pages_categories" => "COUNT($pageTable.photo_id)"));
      $category_select = $category_select->group("$tableName.category_id");
      $category_select->order('total_pages_categories DESC');
    }
    $category_select = $category_select->order('order DESC');
    if (isset($params['limit']))
      $category_select->limit($params['limit']);
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countPages'])) {
      $pageTable = Engine_Api::_()->getDbTable('pages', 'sespage')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($pageTable, "$pageTable.subsubcat_id=$tableName.category_id", array("total_pages_categories" => "COUNT($pageTable.page_id)"));
