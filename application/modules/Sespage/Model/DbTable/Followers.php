 ...
 */
class Sespage_Model_DbTable_Followers extends Engine_Db_Table {

  protected $_rowClass = "Sespage_Model_Follower";
  protected $_name = "sespage_followers";
  ...
  function getCategoryFollowers($resourceId = "") {
    $select = $this->select()
            ->from($this->info('name'), 'owner_id')
            ->where('resource_type = ?', 'sespage_category')
            ->where('resource_id = ?', $resourceId);
    return $this->fetchAll($select);
  }
