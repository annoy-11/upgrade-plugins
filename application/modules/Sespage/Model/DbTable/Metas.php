 ...
 */
class Sespage_Model_DbTable_Metas extends Engine_Db_Table {

  protected $_name = 'sespage_page_fields_meta';
  protected $_rowClass = 'Sespage_Model_Meta';
	
	public function getMetaData($options_id = null){
		if(!$options_id)
			return null;
		$tableName = $this->info('name');
		$mapTableName = Engine_Api::_()->getDbTable('maps','sespage')->info('name');
  ...
  public function profileFieldId() {
	...
	public function reviewProfileFieldId() {
	  $table = Engine_Api::_()->fields()->getTable('sespage_review', 'metas');
		return $table->select()
