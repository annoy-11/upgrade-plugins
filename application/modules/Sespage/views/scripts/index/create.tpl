 ...
 * @version    $Id: create.tpl  2018-04-23 00:00:00 SocialEngineSolutions $
...
<?php if($this->createLimit == 1):?>
  ...
  <?php }else{ ?>
    <script type="application/javascript">
      Sessmoothbox.css.push("<?php echo $this->layout()->staticBaseUrl . 'application/modules/Sespage/externals/styles/styles.css'; ?>");
  ...
  <?php } ?>
  <?php if(count($_POST) == 0 && Engine_Api::_()->getApi('settings','core')->getSetting('sespage.category.selection', 0)):?>
    <?php if (Engine_Api::_()->core()->hasSubject('sespage_page')):?>
      <?php $page = Engine_Api::_()->core()->getSubject();?>
    <?php endif;?>
    <div class="sespage_create_step_container sesbasic_bxs sesbasic_clearfix">
      <h3><?php echo $this->translate('Create New Page');?></h3>
      <p><?php echo $this->translate("It's easy to set up. Just choose a Page category to get started.");?></p>
      <?php $iconType = Engine_Api::_()->getApi('settings','core')->getSetting('sespage.category.icon');?>
      <?php if($iconType == 0):?>
      ...
      <?php elseif($iconType == 2):?>
        <?php $icon = 'thumbnail';?>
      <?php endif;?>
      <div class="sespage_create_categories_listing">
        <?php foreach($this->categories as $category):?>
          <div class="sespage_create_category">
            <section class="">
              <div class="_inner">
                <div class="_step1">
                  <?php if($this->quickCreate):?>
                    <a href="javascript:;" class="sesbasic_linkinherit sespage_create_category_toggle">
                  <?php else:?>
                  ...
                  <?php else:?>
                    <i style="background-image:url(application/modules/Sespage/externals/images/page-icon-big.png);"></i>
                  <?php endif;?>
                ...
                <?php if($this->quickCreate):?>
                  <div class="_step2">
                    <form class='quick_page_create' action="sespage/index/create/parent_id/<?php echo $this->parent_id;?>" id="quick_page_create_<?php echo $category->category_id;?>" method="post" enctype="multipart/form-data">
                    ...
                    <div><input type="text" class="sespage_title" id="sespage_title_<?php echo $category->category_id;?>" name='sespage_title' class="reset_form_value" placeholder="Page Title" /></div>
                    <?php $custom_url_value = isset($page->custom_url) ? $page->custom_url : (isset($_POST["custom_url"]) ? $_POST["custom_url"] : "");;?>
                    <div class="_url">
                      <input type="text" class="sespage_custom_url reset_form_value" name="custom_url" id="custom_url_<?php echo $category->category_id;?>" value="<?php echo $custom_url_value;?>">
                      <span class="sespage_check_availability_btn">
                          <button id="check_custom_url_availability" class="check_custom_url_availability" type="button" name="check_availability" ><i class="fa fa-check" id="sespage_custom_url_correct" style=""></i><i class="fa fa-close" id="sespage_custom_url_wrong" style="display:none;"></i><img src="application/modules/Core/externals/images/loading.gif" id="sespage_custom_url_loading" alt="Loading" style="display:none;" />
                        <samp class="availability_tip"><?php echo $this->translate("Check Availability");?></samp></button>
                      </span>
                     </div>
                    <?php $subcategory = Engine_Api::_()->getDbTable('categories','sespage')->getModuleSubcategory(array('category_id' => $category->category_id, 'column_name' => '*'));?>
                    ...
                    <?php endif;?>
                    <div style='display:none;' class='quick_page_subsubcat'><select></select></div>
                    <div style='display:none;' class="_loading" id='sespage_sussubcat_loading'><i class="fa fa-spinner fa-spin"></i></div>
    ...
    <script type="text/javascript">
      //sesJqueryObject(document).ready(function(){
        sesJqueryObject(document).on('click',".sespage_create_category_toggle",function(){
          checkTitleChnage = false; 
          changeTitle = true;
          sesJqueryObject('.reset_form_value').val('');
          sesJqueryObject('.reset_form_value').trigger('change');
          sesJqueryObject('.sespage_create_category').removeClass('openbox');
          sesJqueryObject(this).closest('.sespage_create_category').toggleClass("openbox");
        });
     // });
    </script>    
  <?php endif; ?>
  <div class="sespage_create_container <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sespage.create.form', 1)):?>sespage_create_form<?php endif;?>">
    <?php echo $this->form->render();?>
    <div class="sespage_create_loading sespage_create_overlay">
      <div class="sespage_create_overlay_cont">
        <i class="fa fa-spinner fa-pulse fa-3x fa-fw margin-bottom"></i>
        <span class="_text"><?php echo $this->translate('Creating Page ...');?></span>
      </div>
    </div>
    <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sespage.guidelines', 1) && !empty(Engine_Api::_()->getApi('settings', 'core')->getSetting('sespage.message.guidelines', ''))):?>
      <div id="sespage_create_tips" class="sespage_create_tips">
        ...
        <div class="create_tips_bottom_sec">
          <div class="sesbasic_html_block">
            <?php echo Engine_Api::_()->getApi('settings','core')->getSetting('sespage.message.guidelines', '');?>
          </div>
        </div>
      </div>
    <?php endif;?>
    ...
    <script type="application/javascript">
      ...
      en4.core.runonce.add(function(){
         if(sesJqueryObject('.sespage_create_step_container').length > 0){
           sesJqueryObject('.sespage_create_container').hide();
           sesJqueryObject('.sespage_create_step_container').show();
         }else{
           sesJqueryObject('.sespage_create_tips').show();
           sesJqueryObject('.sespage_create_container').show();
           sesJqueryObject('.sespage_create_step_container').hide();
         }
      });
      ...
      sesJqueryObject(function() {
        ...
        var $el = sesJqueryObject('#sespage_create_tips');
        var positionInitial = sesJqueryObject('#title').offsetTop;
        sesJqueryObject('<style>#sespage_create_tips{top:'+positionInitial+'px;}</style>').appendTo(document.head);
      ...
      en4.core.runonce.add(function() {
        ...
        sesJqueryObject("#title, .sespage_title").keyup(function(){
          ...
          if(sesJqueryObject(this).closest('.quick_page_create').length){
              sesJqueryObject(sesJqueryObject(this).closest('.quick_page_create').find('._url').find('.sespage_custom_url')).val(Text);   
      }
      else {
          sesJqueryObject("#custom_url").val(Text);   
      }
        });
        sesJqueryObject("#custom_url, .sespage_custom_url").keydown(function(){
         checkTitleChnage = true; 
         changeTitle = false;
        });
        var checkTitleChnage = false;
        sesJqueryObject("#title,.sespage_title").blur(function(){
        ...
        sesJqueryObject("#custom_url, .sespage_custom_url").blur(function(){
          validUrl = false;
          if(!checkTitleChnage)
           changeTitle = false;
      ...
      sesJqueryObject('#check_custom_url_availability, .check_custom_url_availability').click(function(){
        ...
        if(!custom_url_value)
            return;
        sesJqueryObject(this).find('#sespage_custom_url_wrong').hide();
        sesJqueryObject(this).find('#sespage_custom_url_correct').hide();
        sesJqueryObject(this).find('#sespage_custom_url_loading').css('display','inline-block');
        var obj = this;
        sesJqueryObject.post('<?php echo $this->url(array('controller' => 'ajax','module'=>'sespage', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value},function(response){
                    sesJqueryObject(obj).find('#sespage_custom_url_loading').hide();
                    response = sesJqueryObject.parseJSON(response);
                    if(response.error){
                        validUrl = false;
                        sesJqueryObject(obj).find('#sespage_custom_url_correct').hide();
                        sesJqueryObject(obj).find('#sespage_custom_url_wrong').css('display','inline-block');
                            ...
                            sesJqueryObject(obj).find('#sespage_custom_url_wrong').hide();
                            sesJqueryObject(obj).find('#sespage_custom_url_correct').css('display','inline-block');
    ...
    <script type="text/javascript">
      ...
      sesJqueryObject(document).on('submit','.quick_page_create',function(e) {
        var cat_id = sesJqueryObject(this).attr('id').replace('quick_page_create_','');
        if(validation == true) {
          return true;
        }
        e.preventDefault();
        if(sesJqueryObject('#sespage_title_'+cat_id).val() == "") {
          alert('Please complete the title field, it is required.');
          return false;
        }
        if(sesJqueryObject('#custom_url_'+cat_id).val() == "") {
          alert('Please complete the custom URl field, it is required.');
          return false;
        }
        ...
        sesJqueryObject.post('<?php echo $this->url(array('controller' => 'ajax','module'=>'sespage', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value},function(response){
          ...
          if(response.error) {
           ...
           validation = true;
           sesJqueryObject('#quick_page_create_'+cat_id).trigger('submit');
           return true;
          }
        });
        return false;
      ...
      var getProfileType = function(category_id) {
        var mapping = <?php echo Zend_Json_Encoder::encode(Engine_Api::_()->getDbTable('categories', 'sespage')->getMapping(array('category_id', 'profile_type'))); ?>;
      ...
      function showSubCategory(cat_id,selectedId) {
        ...
        var url = en4.core.baseUrl + 'sespage/ajax/subcategory/category_id/' + cat_id;
        new Request.HTML({
          url: url,
          data: {
                    'selected':selected
      ...
      function showQuickSubSubCategory(cat_id,object) {
       sesJqueryObject('#sespage_sussubcat_loading').show();
        var url = en4.core.baseUrl + 'sespage/ajax/subsubcategory/subcategory_id/' + cat_id;
        (new Request.HTML({
          url: url,
          data: {
            'selected':'',
          ...
          onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
            if(responseHTML != ''){
              sesJqueryObject('.quick_page_subsubcat').show();
              sesJqueryObject(object).parent().parent().find('.quick_page_subsubcat').html("<select><option value=''>"+en4.core.language.translate("3rd-level Category")+"</option>"+responseHTML+"></select>");
            }
            else {
              sesJqueryObject(object).parent().parent().find('.quick_page_subsubcat').html("");
              sesJqueryObject('.quick_page_subsubcat').hide();
            }
            sesJqueryObject('#sespage_sussubcat_loading').hide();
          }
        })).send();  
      }
       ...
       en4.core.runonce.add(function(){
                formObj = sesJqueryObject('#sespage_create_form').find('div').find('div').find('div');
    ...
    obj.on('drop', function (e) 
    {
         ...
         handleFileBackgroundUpload(files,'page_main_photo_preview');
    });
   ...
   var sespageidparam = "";
       ...
       /* var reader = new FileReader();
        ...
        if (files) {
           orientation(files, function(base64img, value) {
             ...
             sesJqueryObject(sespageidparam).closest('.form-wrapper').show();;
             var rotated = sesJqueryObject(sespageidparam).attr('src', base64img);
             if (value) {
               sesJqueryObject(sespageidparam).css('transform', rotation[value]);
             }
           });
         }
        ...
        $('page_main_photo_preview').style.display = 'block';
        $('page_main_photo_preview-wrapper').style.display = 'block';
    ...
    function removeImage() {
        ...
        $('page_main_photo_preview').style.display = 'none';
        $('page_main_photo_preview-wrapper').style.display = 'none';
        $('page_main_photo_preview').src = '';
        $('MAX_FILE_SIZE').value = '';
        $('removeimage2').value = '';
        $('photo').value = '';
    }
    ...
    function validateForm(){
            ...
            counter = 0;
            sesJqueryObject('#sespage_create_form input, #sespage_create_form select,#sespage_create_form checkbox,#sespage_create_form textarea,#sespage_create_form radio').each(
                    function(index){
      ...
      en4.core.runonce.add(function() {
        sesJqueryObject('#sespage_create_form').submit(function(e){
          ...
          if(validationFm) {
            ...
            if(typeof objectError != 'undefined'){
             ...
             <?php }else{ ?>
              sesJqueryObject('#sespage_create_form').animate({scrollTop: errorFirstObject.offset().top}, 2000);
             <?php } ?>
            }
            return false;	
          }
            ...
            if(showErrorMessage != ''){
              sesJqueryObject('#page_error_time-wrapper').show();
              sesJqueryObject('#page_error_time-element').text(showErrorMessage);
              var errorFirstObject = sesJqueryObject('.sespage_choose_date');
              <?php if(!$this->typesmoothbox){ ?>
              sesJqueryObject('html, body').animate({scrollTop: errorFirstObject.offset().top}, 2000);
             <?php }else{ ?>
             ...
             <?php } ?>
              ...
              sesJqueryObject('#page_error_time-wrapper').hide();
            }
            if(!validUrl){
             ...
             <?php } ?>
              ...
              sesJqueryObject('.sespage_create_loading').show();
              sesJqueryObject('.sespage_create_form').addClass('_success');
  ...
  sesJqueryObject(document).on('change','input[type=radio][name=enable_lock]',function(){
    if (this.value == 1)
    sesJqueryObject('#page_password-wrapper').show();
    else
    sesJqueryObject('#page_password-wrapper').hide();
  });
  ...
  <?php if($this->typesmoothbox) { ?>
   <script type="application/javascript">
     ...
     en4.core.runonce.add(function() {
       ...
       sesJqueryObject('#sespage_create_form').attr('action',sesJqueryObject('#sespage_create_form').attr('action').replace('?format=html&typesmoothbox=sessmoothbox',''));   
       tinymce.init({
 ...
 <?php die;} ?>
<?php else:?>
  <div class="sesbasic_tip clearfix sespage_error">
    <img src="application/modules/Sespage/externals/images/page-icon-error.png" alt="">
    <span><?php echo $this->translate("You have reached the limit of page creation. Please contact to the site administrator.");?></span>
  </div>
<?php endif;?>
