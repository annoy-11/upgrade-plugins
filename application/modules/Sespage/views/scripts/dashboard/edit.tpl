...
<?php } ?>
    <div class="sespage_dashboard_form <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sespage.create.form', 1)):?>sespage_create_form<?php endif;?>">
...
<script type="text/javascript">
  ...
  if(document.getElementById('page_password-wrapper') && sesJqueryObject('#enable_lock').val() == 0){
    document.getElementById('page_password-wrapper').style.display = 'none';	
    sesJqueryObject('#page_password').val('');
  }
  else if(document.getElementById('page_password-wrapper')){
    document.getElementById('page_password-wrapper').style.display = 'block';	
    sesJqueryObject('#page_password').val('<?php echo $this->page->page_password; ?>');
  }
  //trim last -
  ...
  jqueryObjectOfSes('#check_custom_url_availability').click(function(){
    ...
    var obj = this;
    jqueryObjectOfSes.post('<?php echo $this->url(array('controller' => 'ajax','module'=>'sespage', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value,page_id:<?php echo $this->page->page_id ?>},function(response){
      sesJqueryObject('#sespage_custom_url_loading').hide();
      ...
      if(response.error){
         ...
         jqueryObjectOfSes(obj).find('#sespage_custom_url_wrong').css('display','inline-block');
      }else{
         jqueryObjectOfSes('#custom_url').css('border-color','green');
         jqueryObjectOfSes(obj).find('#sespage_custom_url_wrong').hide();
         jqueryObjectOfSes(obj).find('#sespage_custom_url_correct').css('display','inline-block');
      }
   });
  });
  //tags
  en4.core.runonce.add(function(){
    new Autocompleter.Request.JSON('tags', '<?php echo $this->url(array('controller' => 'tag', 'action' => 'suggest'), 'default', true) ?>', {
      ...
      'injectChoice': function(token){
        ...
        this.addChoicePages(choice).inject(this.choices);
