...
?>
<?php if(!$this->is_ajax){ echo $this->partial('dashboard/left-bar.tpl', 'sespage', array('page' => $this->page));?>
  <div class="sespage_dashboard_content sesbm sesbasic_clearfix sesbasic_bxs">
    <div class="sespage_dashboard_content_header sesbasic_clearfix">
      <h3><?php echo $this->translate('Page Statstics'); ?></h3>
      <p><?php echo $this->translate(''); ?></p>
    </div>
<?php }	?>
<?php if(!$this->is_ajax):?>
  <div class="sespage_dashboard_graph sesbasic_bxs">
    <div class="sesbasic_filter_tabs sesbasic_clearfix">
      <ul>
        <?php if(1):?>
          <li><a href="javascript:;" rel="hourly" onclick="showIntervalData('hourly', this)" id="page-static-hourly" class="<?php if($this->view_type == 'hourly'):?>active<?php endif;?>"><?php echo $this->translate('Hourly');?></a></li>
        <?php endif;?>
        <?php if(1):?>
          <li><a href="javascript:;" rel="daily" onclick="showIntervalData('daily', this)" id="page-static-daily" class="<?php if($this->view_type == 'daily'):?>active<?php endif;?>"><?php echo $this->translate('Daily');?></a></li>
        <?php endif;?>
        <?php if(1):?>
          <li><a href="javascript:;" rel="weekly" onclick="showIntervalData('weekly', this)" id="page-static-weekly" class="<?php if($this->view_type == 'weekly'):?>active<?php endif;?>"><?php echo $this->translate('Weekly');?></a></li>
        <?php endif;?>
        <?php if(1):?>
          <li><a href="javascript:;" rel="monthly" onclick="showIntervalData('monthly', this)" id="page-static-monthly" class="<?php if($this->view_type == 'monthly'):?>active<?php endif;?>"><?php echo $this->translate('Monthly');?></a></li>
        <?php endif;?>
      </ul>
    </div>
    <div id="chartxcontainer" class="clear sespage_dashboard_graph_box"><div class="sesbasic_loading_container" style="height:300px;"></div></div>
    <div id="sespage-graph-loading" class="sesbasic_loading_container sespage_entry_view_graph_box clear" style="display: none;"></div>
  </div>
...
//]]> 
  function showIntervalData(interval, obj) {
    ...
    sesJqueryObject('#sespage-graph-loading').show();
    sesJqueryObject('#page-static-monthly').removeClass('active');
    sesJqueryObject('#page-static-weekly').removeClass('active');
    sesJqueryObject('#page-static-daily').removeClass('active');
    sesJqueryObject('#page-static-hourly').removeClass('active');
    sesJqueryObject(obj).addClass('active');
    var url = "<?php echo $this->url(array('page_id' => $this->page->custom_url,'action'=>'insights'), 'sespage_dashboard', true); ?>"+'/interval/'+interval;
    var request = new Request.JSON({
      ...
      onSuccess : function(responseJSON) {
        ...
        sesJqueryObject('#sespage-graph-loading').hide();
