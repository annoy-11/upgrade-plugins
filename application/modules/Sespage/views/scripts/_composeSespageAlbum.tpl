...
<?php
  ...
  $this->headLink()
    ...
    ->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sespage/externals/styles/styles.css');
...
<?php if(!Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('sesadvancedactivity')) { ?>
  en4.core.runonce.add(function() {
    if (Composer.Plugin.Sespagephoto)
      return;

    Asset.javascript('<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sespage/externals/scripts/composer_photo.js', {
      onLoad:  function() {
        var type = 'wall';
        if (composeInstance.options.type) type = composeInstance.options.type;
        composeInstance.addPlugin(new Composer.Plugin.Sespagephoto({
          ...
          requestOptions : {
            'url'  : en4.core.baseUrl + 'sespage/album/upload-photo/type/'+type+'/page_id/<?php echo $subject->getIdentity(); ?>',
          },
          fancyUploadOptions : {
            'url'  : en4.core.baseUrl + 'sespage/album/upload-photo/format/json/type/'+type+'/page_id/<?php echo $subject->getIdentity(); ?>',
...
<?php }else{  ?>  
<?php $this->headScript()->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sespage/externals/scripts/composer_photo.js'); ?>
...
obj.on('drop', function (e) 
{
   ...
   handleFileUploadSespage(files,obj);
});
...
sesJqueryObject(document).on('click','div[id^="abortPhoto_"]',function(){
		...
		if(typeof jqXHR[id] != 'undefined'){
				...
				executeuploadSespage();
		}else{
				delete filesArray[id];	
				sesJqueryObject(this).parent().remove();
		}
    if(isMessagePage && (sesJqueryObject('#toValues-wrapper').length > 0 || sesJqueryObject('#submit-wrapper').length > 0)){
...
function createStatusbar(obj,file)
{
     ...
     if(rowCount %2 ==0) row ="even";
		  var checkedId = sesJqueryObject("input[name=cover]:checked");
			...
			this.abort = sesJqueryObject('<div class="abort sespage_upload_item_abort" id="abortPhoto_'+countUploadSes+'"><span><?php echo $this->translate("Cancel Uploading"); ?></span></div>').appendTo(this.objectInsert);
			...
			this.src = sesJqueryObject('<img src="'+en4.core.baseUrl+'application/modules/Sespage/externals/images/blank-img.gif">').appendTo(this.imageContainer);
			this.infoContainer = sesJqueryObject('<div class=advact_compose_photo_item_info sesbasic_clearfix"></div>').appendTo(this.objectInsert);
			 this.size = sesJqueryObject('<span class="sespage_upload_item_size sesbasic_text_light"></span>').appendTo(this.infoContainer);
			 this.filename = sesJqueryObject('<span class="sespage_upload_item_name"></span>').appendTo(this.infoContainer);
			this.option = sesJqueryObject('<div class="sespage_upload_item_options clear sesbasic_clearfix"><span class="sespage_upload_item_radio"></span><a class="edit_image_upload" href="javascript:void(0);"><i class="fa fa-pencil"></i></a><a class="delete_image_upload" href="javascript:void(0);"><i class="fa fa-close"></i></a></div>').appendTo(this.objectInsert);
...
var fdSes = new Array();
function handleFileUploadSespage(files,obj)
{
...
var execute = true;
function executeuploadSespage(){
	if(Object.keys(filesArray).length == 0 && sesJqueryObject('#show_photo').html() != ''){
   if(isMessagePage && (sesJqueryObject('#toValues-wrapper').length > 0 || sesJqueryObject('#submit-wrapper').length > 0)){}else{
    ...
    {
     	sendFileToServerSespage(filesArray[i],statusArray[i],filesArray[i],'upload',i);
			break;
    }			
	 }
	}
...
var jqXHR = new Array();
function sendFileToServerSespage(formData,status,file,isURL,i)
{
		execute = false;
    ...
    var uploadURL =en4.core.baseUrl + 'sespage/album/upload-photo/type/'+type+'/page_id/<?php echo $subject->getIdentity(); ?>';
...
function readImageUrlSespage(input) {
	handleFileUploadSespage(input.files,sesJqueryObject('#dragandrophandler'));
}
...
sesJqueryObject(document).on('click','.delete_image_upload',function(e){
    ...
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'sespage', 'controller' => 'index', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'sespage', 'controller' => 'index', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
</script>
