 ...
 * @version    $Id: edit-ticket.tpl 2016-07-26 00:00:00 SocialEngineSolutions $
...
<?php }else{ ?>
	sesJqueryObject('#price-label').append('<span> (<?php echo Engine_Api::_()->sesevent()->defaultCurrency(); ?>)</span>');
<?php } ?>
sesJqueryObject('#timezone-wrapper').hide();
sesJqueryObject('#event_start_time-element').append('<span style="line-height:25px"><?php echo $this->event_timezone[$this->event->timezone]; ?></span>');
sesJqueryObject('#event_end_time-element').append('<span style="line-height:25px"><?php echo $this->event_timezone[$this->event->timezone]; ?></span>');
function eventType(value){
 if(value == 'paid'){
		sesJqueryObject('#price').show();
		sesJqueryObject('#free_ticket_label').remove();
		sesJqueryObject('#price-element').find('#sesevent_currency_coverter').show();
		...
		sesJqueryObject('#sesevent_currency_coverter').css('display','none');
...
sesJqueryObject('#manage-ticket').parent().addClass('active');
sesJqueryObject('#manage-ticket').parent().parent().parent().find('a.sesbasic_dashboard_nopropagate').trigger('click');
sesJqueryObject(document).on('click','.seseventticket_back_manage',function(e){
	sesJqueryObject('#manage-ticket').parent().removeClass('active');
