 ...
 * @version    $Id: browse-host.tpl 2016-07-26 00:00:00 SocialEngineSolutions $
...
<?php if( count($this->paginator) > 0 ): ?>
  <ul class='sesevents_browse'>
    <?php foreach( $this->paginator as $event ): ?>
      <li>
        <div class="sesevents_photo">
          <?php echo $this->htmlLink($event->getHref(), $this->itemPhoto($event, 'thumb.normal')) ?>
        </div>
        <div class="sesevents_options">
        </div>
        <div class="sesevents_info">
          <div class="sesevents_title">
            <h3><?php echo $this->htmlLink($event->getHref(), $event->getTitle()) ?></h3>
          </div>
      <div class="sesevents_members">
