   ...
   $this->view->form = $form = new Sesevent_Form_Admin_Payment_Approve(array('eventId'=>$event->event_id));
    ...
    if (!$form->isValid($this->getRequest()->getPost()))
      ...
      if($_POST['gateway_type'] == "paytm") {
        ...
        return $this->_forward('success', 'utility', 'core', array(
                    'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('route' => 'default','module' => 'sesevent', 'controller' => 'payment', 'action' => 'process','gateway_id'=>$_POST['usergateway_id']),'admin_default',true),
                    'messages' => array($this->view->message)
        ));
      }
		}catch (Exception $e) {
    ...
    // Get gateway
    if(!($gateway = Engine_Api::_()->getItem('sesevent_usergateway',$this->_getParam('gateway_id', null))) ||
            !($gateway->enabled) ) {
    ...
    // Process transaction
    if($gateway->plugin == "Sesevent_Plugin_Gateway_Event_Stripe") {
