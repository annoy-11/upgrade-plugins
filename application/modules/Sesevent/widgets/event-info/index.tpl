    ...
    <ul class="sesevent_profile_info_row_info">
     ...
     <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesevent_enable_location', 1)){ ?>
      <li class="sesbasic_clearfix">
        ...
        <span> <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('enableglocation', 1)) { ?><a href='<?php echo $this->url(array('resource_id' => $this->subject->event_id,'resource_type'=>'sesevent_event','action'=>'get-direction'), 'sesbasic_get_direction', true); ?>' class="openSmoothbox"><?php echo $this->subject->location; ?> </a><?php } else { ?><?php echo $this->subject->location; ?><?php } ?> </span>
      </li>
      <?php } ?>
    </ul>
  </div>
