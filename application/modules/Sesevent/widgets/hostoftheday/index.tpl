	...
	<li class="sesevent_host_list sesevent_grid_btns_wrap sesbasic_clearfix <?php if($this->contentInsideOutside == 'in'): ?> sesevent_host_list_in <?php else: ?> sesevent_host_list_out <?php endif; ?> <?php if($this->mouseOver): ?> sesae-i-over <?php endif; ?>" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">
      ...
      $urlencode = urlencode(((!empty($_SERVER["HTTPS"]) &&  strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $href); ?>
        <div class="sesevent_grid_btns"> 
          ...
          ?>
          <?php
          if(in_array('likeButton', $this->content_show) && isset($host->like_count)){ ?>
          <?php $LikeStatus = Engine_Api::_()->sesevent()->getLikeStatusEvent($host->$getId,$host->getType()); ?>
            <a href="javascript:;" data-url="<?php echo $host->$getId ; ?>" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn sesevent_like_<?php echo $itemtype; ?> <?php echo ($LikeStatus) ? 'button_active' : '' ; ?>"> <i class="fa fa-thumbs-up"></i><span><?php echo $host->like_count; ?></span></a>
            <?php } 
      ...
      <div class="sesevent_host_list_info sesbasic_clearfix">
        ...
        <?php //endif; ?>
	      <div class="sesevent_host_list_stats sesevent_list_stats">
	        ...
	        <?php } ?>
          <?php if(in_array('like', $this->content_show) && isset($host->like_count)) { ?>
            <span title="<?php echo $this->translate(array('%s like', '%s likes', $host->like_count), $this->locale()->toNumber($host->like_count))?>"><i class="fa fa-thumbs-up sesbasic_text_light"></i><?php echo $host->like_count; ?></span>
          <?php } ?>
