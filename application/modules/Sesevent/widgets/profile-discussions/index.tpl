...
<?php if( $this->viewer()->getIdentity() || $this->paginator->count() > 1 ): ?>
  <div class="sesbasic_profile_tabs_top sesbasic_clearfix">
    <?php if( $this->viewer()->getIdentity() && $this->canTopicCreate):?>
       <a href="#" class="sesbasic_button" onclick="opensmoothboxurl('<?php echo $this->url(array('action' => 'create','controller'=>'topic','subject' => $this->subject()->getGuid()),'sesevent_extended',true); ?>');return false;" >
    <i class="sesbasic_icon_add"></i><?php echo $this->translate('Post New Topic') ?>
    </a>
    <?php endif;?>
