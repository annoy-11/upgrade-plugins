...
class Sesevent_Model_Gateway extends Core_Model_Item_Abstract
{
  ...
  public function getPlugin($type = 'ticket')
  {
        ...
        $class = $this->plugin;
        if($this->plugin == "Payment_Plugin_Gateway_PayPal"){
         $class = str_replace('Payment','Sesevent',$class);
        } elseif($this->plugin == "Sesadvpmnt_Plugin_Gateway_Stripe") {
          $class = str_replace('Sesadvpmnt','Sesevent',$class);
        }
