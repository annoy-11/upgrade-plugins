...
class Sesevent_Plugin_Core {
  public function onStatistics($event) {
	...
	public function onRenderLayoutDefault($event){
		...
		$checkPopup = Engine_Api::_()->getApi('settings', 'core')->getSetting('page.popup.create.event',0);
		$viewer = Engine_Api::_()->user()->getViewer();		

		$checkWelcomeEnableLoggedIN = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesevent.enable.welcome.logged.in',1);
		$checkWelcomeEnableNonloggedIN = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesevent.enable.welcome.nonlogged.in',1);
		
		if($actionName == 'welcome' && $controllerName == 'index' && $moduleName == 'sesevent'){
		  $redirector = Zend_Controller_Action_HelperBroker::getStaticHelper('redirector');
		  if($viewer->getIdentity() != 0){
			if($checkWelcomeEnableLoggedIN == 0) 
		  	$redirector->gotoRoute(array('module' => 'sesevent', 'controller' => 'index', 'action' => 'home'), 'sesevent_general', false);
			else if($checkWelcomeEnableLoggedIN == 2) 
				$redirector->gotoRoute(array('module' => 'sesevent', 'controller' => 'index', 'action' => 'browse'), 'sesevent_general', false);
			else if($checkWelcomeEnableLoggedIN == 3) 
				$redirector->gotoRoute(array('module' => 'sesevent', 'controller' => 'list', 'action' => 'browse'), 'sesevent_list', false);
			else if($checkWelcomeEnableLoggedIN == 4) 
				$redirector->gotoRoute(array('module' => 'sesevent', 'controller' => 'category', 'action' => 'browse'), 'sesevent_category', false);
			else if($checkWelcomeEnableLoggedIN == 5) 
				$redirector->gotoRoute(array('module' => 'sesevent', 'controller' => 'index', 'action' => 'browse-host'), 'sesevent_general', false);
			}
			if($viewer->getIdentity() == 0){
			if($checkWelcomeEnableNonloggedIN == 0) 
		  	$redirector->gotoRoute(array('module' => 'sesevent', 'controller' => 'index', 'action' => 'home'), 'sesevent_general', false);
			else if($checkWelcomeEnableNonloggedIN == 2) 
				$redirector->gotoRoute(array('module' => 'sesevent', 'controller' => 'index', 'action' => 'browse'), 'sesevent_general', false);
			else if($checkWelcomeEnableNonloggedIN == 3) 
				$redirector->gotoRoute(array('module' => 'sesevent', 'controller' => 'list', 'action' => 'browse'), 'sesevent_list', false);
			else if($checkWelcomeEnableNonloggedIN == 4) 
				$redirector->gotoRoute(array('module' => 'sesevent', 'controller' => 'category', 'action' => 'browse'), 'sesevent_category', false);
			else if($checkWelcomeEnableNonloggedIN == 5) 
				$redirector->gotoRoute(array('module' => 'sesevent', 'controller' => 'index', 'action' => 'browse-host'), 'sesevent_general', false);
			}
		}
...
";
		if($viewer->getIdentity() != 0 && Engine_Api::_()->authorization()->isAllowed('sesevent_event', $viewer, 'create')){
			...
			if(sesJqueryObject("body").attr("id").search("sesevent") > -1 && typeof showAddnewEventIconShortCut != "undefined" && showAddnewEventIconShortCut ){
				sesJqueryObject("<a class=\'sesbasic_create_button sesevent_quick_create sesevent_quick_create_button sesbasic_animation\' href=\''.$view->url(array('action'=>'create'), 'sesevent_general').'\' title=\'Add New Event\'><i class=\'fa fa-plus\'></i></a>").appendTo("body");
			}
			if("'.$checkPopup.'" == "1"){
