              ...
              <?php if($this->invitecountActive || $this->friendcountActive) { ?>
                <div class='sesinviter_topinviters_friends'>
                  <?php if($this->invitecountActive) { ?>
                    <span class="_des sesbasic_text_light"><?php echo $this->translate(array('<span>%s</span> invite', '<span>%s</span> invites', $result->affiliates_count),$this->locale()->toNumber($result->affiliates_count)) ?></span>
                  <?php } ?>
                  <?php if($this->memberjoincountActive) { ?>
                    <?php $joinedCount = Engine_Api::_()->getDbTable('invites', 'sesinviter')->getrefJoinedCount($user->user_id); ?>
                    <span class="_des sesbasic_text_light"><?php echo $this->translate(array('<span>%s</span> members', '<span>%s</span> members', $joinedCount),$this->locale()->toNumber($joinedCount)) ?></span>
                  <?php } ?>
