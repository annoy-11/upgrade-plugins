 ...
 */
 ?>
<?php  
    //check sesmusic enable or not
    if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesmusic') && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sessiteiframe')){ 
    if(empty($_COOKIE['sessiteiframeCookieValue'])){ 
      setcookie('sessiteiframeCookieValue', 'sessiteiframe', time() + (86400 * 30), '/');
?>
  <?php $actual_link = (stripos($_SERVER['SERVER_PROTOCOL'],'https') === true || $_SERVER['SERVER_PORT'] == "443" ? 'https://' : 'http://')."$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
 ...
 <html xmlns="http://www.w3.org/1999/xhtml" xml:lang="<?php echo $locale ?>" lang="<?php echo $locale ?>" dir="<?php echo $orientation ?>">
  <head>
    ...
    <script type="text/javascript">
      function removeMusicCookie(cname, cvalue, expires){
        setMusicCookie(cname,0,0);
      } 
     ...
     window.addEventListener('popstate', function(event) {
      ...
      <?php if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sestweet')) { ?>
        sesJqueryObject(document).ready(function(){
          document.getElementById("sessiteiframeCookieValue").contentWindow.addEventListener('mousemove', function( event ) {
            var boundingClientRect = document.getElementById("sessiteiframeCookieValue").getBoundingClientRect();
            var evt = new CustomEvent( 'mousemove', {bubbles: true, cancelable: false})
            //evt.clientX = event.clientX + boundingClientRect.left;
            //evt.clientY = event.clientY + boundingClientRect.top;  
            document.getElementById("sessiteiframeCookieValue").contentWindow.m_MouseDrag = true;
            document.getElementById("sessiteiframeCookieValue").contentWindow.tweetx = event.clientX  - 10;
            document.getElementById("sessiteiframeCookieValue").contentWindow.tweety = event.clientY + 10;
            document.getElementById("sessiteiframeCookieValue").dispatchEvent( evt );
           });  
          });
      <?php } ?>
    </script>
...
<body style="padding:0; margin:0;" id="global_page_<?php echo $identity ?>"<?php if (!$this->viewer()->getIdentity()): ?> class="guest-user"<?php endif; ?>>
 ...
 <?php echo $this->content()->renderWidget('sesmusic.player',array('fromIframe'=>true)); ?>
</body>
</html>
<?php }else{ ?>
<?php echo $this->doctype()->__toString() ?>
...
<script type="application/javascript">
  ...
  sesJqueryObject(document).click(function(){
    parent.setMusicCookie("sessiteiframeCookieValue", 'sessiteiframe', 1);
  });
  sesJqueryObject(document).on("click", function(event) {
    if (event.ctrlKey || event.shiftKey || event.metaKey || event.which == 2) {
        if(typeof parent.removeMusicCookie != "undefined")
        parent.removeMusicCookie("sessiteiframeCookieValue", 'sessiteiframe', 'Thu, 01 Jan 1970 00:00:01 GMT;');
    }
  ...
  sesJqueryObject(document).ready(function(){
    ...
    <?php } ?>
    if(typeof window.parent.changeHashState != "undefined")
    window.parent.changeHashState();
    parent.history.pushState(null, null, '<?php echo str_replace(array('?sessiteiframeCookieValue=true', 'sessiteiframeCookieValue=true'), array('', ''),  (stripos($_SERVER['SERVER_PROTOCOL'],'https') === true || $_SERVER['SERVER_PORT'] == "443" ? "https://" : "http://")."$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]"); ?>');
    });
