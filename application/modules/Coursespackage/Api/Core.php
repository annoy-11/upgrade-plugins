...
class coursespackage_Api_Core extends Core_Api_Abstract {
  ...
  public function getCustomFieldMapData($page) {
    if ($page) {
      ...
      return $db->query("SELECT GROUP_CONCAT(value) AS `valuesMeta`,IFNULL(TRIM(TRAILING ', ' FROM GROUP_CONCAT(DISTINCT(engine4_courses_page_fields_options.label) SEPARATOR ', ')),engine4_courses_page_fields_values.value) AS `value`, `engine4_courses_page_fields_meta`.`label`, `engine4_courses_page_fields_meta`.`type` FROM `engine4_courses_page_fields_values` LEFT JOIN `engine4_courses_page_fields_meta` ON engine4_courses_page_fields_meta.field_id = engine4_courses_page_fields_values.field_id LEFT JOIN `engine4_courses_page_fields_options` ON engine4_courses_page_fields_values.value = engine4_courses_page_fields_options.option_id AND `engine4_courses_page_fields_meta`.`type` = 'multi_checkbox' WHERE (engine4_courses_page_fields_values.item_id = " . $page->getIdentity() . ") AND (engine4_courses_page_fields_values.field_id != 1) GROUP BY `engine4_courses_page_fields_meta`.`field_id`,`engine4_courses_page_fields_options`.`field_id`")->fetchAll();
    }
    return array();
  }
