 ...
 * @version    $Id: PayPal.php 2019-11-05 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */

class Coursespackage_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function getGateway() {
    if (null === $this->_gateway) {
      ...
      $gateway = new $class(array(
          ...
          'currency' => Engine_Api::_()->coursespackage()->getCurrentCurrency(),
      ));
  ...
  public function createCourseTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $item, Coursespackage_Model_Package $package, array $params = array()) {
    // Process description
    ...
    $currentCurrency = Engine_Api::_()->coursespackage()->getCurrentCurrency();
    $defaultCurrency = Engine_Api::_()->coursespackage()->defaultCurrency();
  ...
  public function onCourseTransactionReturn(
  Payment_Model_Order $order, array $params = array(),$bankTransfer = false) {
    // Check that gateways match
    if ($order->gateway_id != $this->_gatewayInfo->gateway_id && !$bankTransfer) {
    ...
    // Check params
    if (empty($params['token']) && !$bankTransfer) {
    ...
    if ($package->isOneTime()) {
      ...
      $this->getGateway()->getLog()->log('DoExpressCheckoutPayment: '
        ...
        $rdata['PAYMENTINFO'][0]['PAYMENTSTATUS'] = 'pending';
        $rdata['PAYMENTINFO'][0]['TRANSACTIONID'] = '';
      }
      // Get payment state
      $paymentStatus = null;
      $orderStatus = null;
      ...
      $orderPackage = Engine_Api::_()->getItem('coursespackage_orderspackage', $orderPackageId);
      if (!$orderPackageId || !$orderPackage) {
        $transactionsOrdersTable = Engine_Api::_()->getDbtable('orderspackages', 'coursespackage');
      ...
      $session = new Zend_Session_Namespace('Payment_Coursespackage');
      ...
      $tablePage = Engine_Api::_()->getDbTable('classroom', 'eclassroom');
      ...
      $packageOrder = Engine_Api::_()->getItem('coursespackage_orderspackage', $orderPackageId);
      $packageOrder->item_count = $leftPage;
      $packageOrder->save();

      $transactionsTable = Engine_Api::_()->getDbtable('transactions', 'coursespackage');
      $transactionsTable->insert(array(
          ...
          'change_rate' => $rate,
          'gateway_type' => !$bankTransfer ? 'Paypal' : 'Bank Transfer',
      ...
      // Get benefit setting
      $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'coursespackage')
              ->getBenefitStatus($user);
      // Check payment status
      if (($paymentStatus == 'okay' || $paymentStatus == 'active' ||
              ($paymentStatus == 'pending' && $giveBenefit)) || ($bankTransfer)   ) {

        //Update subscription info
        $transaction->gateway_id = !$bankTransfer ? $this->_gatewayInfo->gateway_id : 10;
        $transaction->gateway_profile_id = !$bankTransfer ? $rdata['PAYMENTINFO'][0]['TRANSACTIONID'] : '';
        $transaction->save();
        // Payment success
        if(!$bankTransfer)
          ...
          if ($package->isOneTime()) {
            switch ($rawData['payment_status']) {
              ...
              case 'Pending':
                ...
                // Get benefit setting
                $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'coursespackage')->getBenefitStatus($user);
                if ($giveBenefit) {
                  $item->onPaymentSuccess();
                } else {
                  $item->onPaymentPending();
  ...
  public function cancelPage($transactionId, $note = null) {
    $profileId = null;
    if ($transactionId instanceof Coursespackage_Model_Transaction) {
  ...
  public function onIpn(Engine_Payment_Ipn $ipn) {
    ...
    if ($order) {
      $ipnProcessed = false;
      // Subscription IPN
      if ($order->source_type == 'classroom') {
