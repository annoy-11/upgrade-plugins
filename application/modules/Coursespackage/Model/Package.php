 ...
 * @version    $Id: Package.php 2019-11-05 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */

class Coursespackage_Model_Package extends Core_Model_Item_Abstract {
  ...
  public function getProduct() {
    if (null === $this->_product) {
      ...
      $this->_product = $productsTable->fetchRow($productsTable->select()
                      ->where('extension_type = ?', 'sespage_page')
  ...
  public function getPackageDescription() {
    ...
    $priceStr = Engine_Api::_()->sespagepackage()->getCurrencyPrice($this->price,'','');
  ...
  public function allowUploadVideo($item, $left = false) {
    ...
    else if ($item && $item instanceof Sespage_Model_Page)
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('video', $modulesEnable['modules'])) {
      ...
      //free package
      if ($orderspackage_id) {
        $itemTable = Engine_Api::_()->getDbTable('pages', 'sespage');
        $itemTableName = $itemTable->info('name');
        $subselect = $itemTable->select()->from($itemTableName, 'page_id')->where('orderspackage_id =?', $orderspackage_id);
        $select->where('parent_id IN (?)', $subselect)->where('parent_type =?', 'sespage_page');
      }
  ...
  public function allowUploadMusic($item, $left = false) {
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('music', $modulesEnable['modules'])) {
      ...
      $select = $albumSongTable->select()->from($albumSongTableName, array('total' => 'COUNT(albumsong_id)'))
              ...
              ->joinLeft($albumTableName, $albumSongTableName . '.album_id = ' . $albumTableName . '.album_id AND ' . $albumTableName . '.resource_type = "sespage_page"', null);
      //free package
      if ($orderspackage_id) {
        $itemTable = Engine_Api::_()->getDbTable('pages', 'sespage');
        $itemTableName = $itemTable->info('name');
  ...
  public function allowUploadPhoto($item, $left = false) {
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('photo', $modulesEnable['modules'])) {
      ...
      if ($orderspackage_id) {
        ...
        $itemTableName = $itemTable->info('name');
        $subselect = $itemTable->select()->from($itemTableName, 'page_id')->where('orderspackage_id =?', $orderspackage_id);
        $select->where('page_id IN (?)', $subselect);
      }
