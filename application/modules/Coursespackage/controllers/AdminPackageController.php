...
class Coursespackage_AdminPackageController extends Core_Controller_Action_Admin {

  public function indexAction() {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('courses_admin_main', array(), 'courses_admin_main_package');

    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_package', array(), 'courses_admin_main_packman');
    ...
    // Make form
    $this->view->formFilter = $formFilter = new Coursespackage_Form_Admin_Package_Filter();
    ...
    // Initialize select
    $table = Engine_Api::_()->getDbtable('packages', 'coursespackage');
    $select = $table->select();
    ...
    $paginator->setCurrentPageNumber($this->_getParam('courses', 1));

    // Get courses totals for each plan
    $pageCounts = array();
    foreach ($paginator as $item) {
      $pageCounts[$item->package_id] = Engine_Api::_()->getDbtable('courses', 'courses')
              ->select()
              ->from('engine4_courses_courses', new Zend_Db_Expr('COUNT(*)'))
              ->where('package_id = ?', $item->package_id)
              ->where('is_approved = ?', true)
              ->query()
              ->fetchColumn();
    }
    $this->view->pageCounts = $pageCounts;
  }
  ...
  public function settingsAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_package', array(), 'courses_admin_main_packageset');

    // Make form
    $this->view->form = $form = new Coursespackage_Form_Admin_Package_Settings();
  ...
  public function createAction() {
    ...
    // Make form
    $this->view->form = $form = new Coursespackage_Form_Admin_Package_Create(array('customFields' => $customFields));
    ...
    $packageTable = Engine_Api::_()->getDbtable('packages', 'coursespackage');
...
//       if (!empty($values['sespage_page_text_photo']))
//         $params['sespage_page_text_photo'] = $values['sespage_page_text_photo'];
//       if (!empty($values['sespage_page_photo_photo']))
//         $params['sespage_page_photo_photo'] = $values['sespage_page_photo_photo'];
//       if (!empty($values['sespage_page_music_photo']))
//         $params['sespage_page_music_photo'] = $values['sespage_page_music_photo'];
//       if (!empty($values['sespage_page_video_photo']))
//         $params['sespage_page_video_photo'] = $values['sespage_page_video_photo'];
//       if (!empty($values['sespage_page_text_coverphoto']))
//         $params['sespage_page_text_coverphoto'] = $values['sespage_page_text_coverphoto'];
//       if (!empty($values['sespage_page_photo_coverphoto']))
//         $params['sespage_page_photo_coverphoto'] = $values['sespage_page_photo_coverphoto'];
//       if (!empty($values['sespage_page_music_coverphoto']))
//         $params['sespage_page_music_coverphoto'] = $values['sespage_page_music_coverphoto'];
//       if (!empty($values['sespage_page_video_coverphoto']))
//         $params['sespage_page_video_coverphoto'] = $values['sespage_page_video_coverphoto'];
//
//
//
...
//       $params['page_service'] = $values['page_service'];
...
//
//       if (!empty($values['sespage_admin_commission']))
//         $params['sespage_admin_commission'] = $values['sespage_admin_commission'];
//       if (!empty($values['sespage_commission_value']))
//         $params['sespage_commission_value'] = $values['sespage_commission_value'];
//       if (!empty($values['sespage_threshold_amount']))
//         $params['sespage_threshold_amount'] = $values['sespage_threshold_amount'];
//
//       $values['params'] = json_encode($params);
    ...
    // Redirect
    return $this->_helper->redirector->gotoRoute(array('module' => 'coursespackage', 'controller' => 'package'), "admin_default", true);
  }
  ...
  public function editAction() {
    ...
    if (null === ($packageIdentity = $this->_getParam('package_id')) ||
            !($package = Engine_Api::_()->getDbtable('packages', 'coursespackage')->find($packageIdentity)->current())) {
    ...
    $this->view->customFields = $customFields;
    // Make form
    $this->view->form = $form = new Coursespackage_Form_Admin_Package_Edit(array('customFields' => $customFields));
    // Populate form
  ...
  public function deleteAction() {
    ...
    $this->view->form = $form = new Coursespackage_Form_Admin_Package_Delete();
    // Check method/data
    if (!$this->getRequest()->isPost()) {
      return;
    }
  ...
  public function highlightAction() {
    ...
    if (!empty($package_id)) {
      $package = Engine_Api::_()->getItem('coursespackage_package', $package_id);
  ...
  public function approvedAction() {
    $package_id = $this->_getParam('package_id');
    if (!empty($package_id)) {
      $package = Engine_Api::_()->getItem('courses_package', $package_id);
  ...
  $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_package', array(), 'courses_admin_main_packtran');

    $this->view->formFilter = $formFilter = new Coursespackage_Form_Admin_Filter();
    ...
    $tableTransaction = Engine_Api::_()->getItemTable('coursespackage_transaction');
    $tableTransactionName = $tableTransaction->info('name');
    $pageTable = Engine_Api::_()->getDbTable('classrooms', 'eclassroom');
    ...
    $select = $tableTransaction->select()
            ...
            ->joinLeft($pageTableName, "$tableTransactionName.transaction_id = $pageTableName.transaction_id", 'classroom_id')
            ->where($pageTableName . '.classroom_id IS NOT NULL')
            ->order((!empty($_GET['order']) ? $_GET['order'] : 'transaction_id' ) . ' ' . (!empty($_GET['order_direction']) ? $_GET['order_direction'] : 'DESC' ));

    if (!empty($_GET['title'])) {
      $select->where('(' . $tableTransactionName . '.gateway_transaction_id LIKE ? || ' .
  ...
  public function detailAction() {
    if (!($transaction_id = $this->_getParam('transaction_id')) ||
            !($transaction = Engine_Api::_()->getItem('coursespackage_transaction', $transaction_id))) {
      return;
    }
    ...
    $this->view->item = Engine_Api::_()->getItem('classroom', $this->_getParam('classroom_id'));
  ...
  public function showReceiptAction() {
    ...
    $nama_fail = Engine_Api::_()->getItem('courses_transaction', $transactionId)->file_path;
    $file = $nama_fail;
    if (file_exists($file)) {
  ...
  public function approveAction() {
    $transactionId = $this->_getParam('transaction_id',0);
    $transaction = Engine_Api::_()->getItem('courses_transaction', $transactionId);
    $orderPackageId = $transaction->orderspackage_id;
    $package = Engine_Api::_()->getItem('courses_package',$transaction->package_id);
    $orderPackage = Engine_Api::_()->getItem('courses_orderspackage', $orderPackageId);
    $pageTable = Engine_Api::_()->getDbTable('pages','sespage');
    $pageId = $pageTable->select()->from($pageTable->info('name'),'course_id')->where('transaction_id =?',$transactionId)->where('orderspackage_id =?',$orderPackageId)->query()->fetchColumn();
    $item = Engine_Api::_()->getItem('sespage_page',$pageId);
    $daysLeft = 0;
    ...
    if (isset($_SERVER['HTTP_REFERER']))
      ...
      $url = 'admin/courses/manage-transaction';
    $this->_redirect($url);
  }
