...
<?php foreach( $this->paginator as $album ){
			$counterAlbum = 0;
?>
<div class="sesusercoverphoto_update_album_row sesbasic_clearfix">
	<div id="sesusercoverphoto_photo_content_<?php echo $album->album_id; ?>">
	<?php $photos = Engine_Api::_()->sesusercoverphoto()->getPhotoSelect(array('album_id'=>$album->album_id,'pagNator'=>true)); 
                  $photos->setItemCountPerPage($this->limit);
                  $photos->setCurrentPageNumber(1);
        if($photos->getTotalItemCount() > 0){
          foreach($photos as $photo){ ?>
          <?php if($counterAlbum == 0){ ?>
            <span class="sesusercoverphoto_name"><?php echo $album->title; ?></span>
          <?php } ?>
            <div class="sesusercoverphoto_thumb">
              
            	<a href="javascript:void(0);" id="sesusercoverphoto_<?php echo $this->coverphoto ?>_upload_existing_photos_<?php echo $photo->photo_id; ?>" data-src="<?php echo $photo->photo_id; ?>" class="sesusercoverphoto_thumb_img">
        ...
        <div class="album_more_photos floatR clear">
          <a href="javascript:;" id="sesusercoverphoto_<?php echo $this->coverphoto ?>_existing_album_see_more_<?php echo $album->album_id; ?>" data-src="1">
            <?php echo $this->translate("See More"); ?> &raquo;
          </a>
        </div>
      <?php } ?>
      <div class="clear" style="text-align:center;display:none;" id="sesusercoverphoto_existing_album_see_more_loading_<?php echo $album->album_id; ?>">
