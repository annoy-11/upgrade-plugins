...
class Sesusercoverphoto_Widget_SesusercoverphotoCoverController extends Engine_Content_Widget_Abstract {
  public function indexAction() {
    ...
    $level = $subject;
	 	...
	 	$tab = Engine_Api::_()->authorization()->getPermission($level, 'sesusercover', 'tab');
		if($tab == ' ')
			$this->view->tab = 'inside';
		else
			$this->view->tab = $tab;

		$options = Engine_Api::_()->authorization()->getPermission($level, 'sesusercover','option');
 	  ...
 	  $this->view->can_edit = $canEdit = $subject->authorization()->isAllowed($viewer, 'edit');
		...
		$height = Engine_Api::_()->authorization()->getPermission($level, 'sesusercover', 'height');
		if($height == '')
		...
		$is_fullwidth = Engine_Api::_()->authorization()->getPermission($level, 'sesusercover', 'is_fullwidth');
		if($is_fullwidth == 0)
    ...
    $showicontype = Engine_Api::_()->authorization()->getPermission($level, 'sesusercover', 'showicontype');
		if($showicontype == 0)
		...
		$userphotoround = Engine_Api::_()->authorization()->getPermission($level, 'sesusercover', 'userphotoround');
		if($userphotoround == 0)
    ...
    $canCreate = Engine_Api::_()->authorization()->getPermission($level, 'sesusercover', 'create');
		if($canCreate == 0)
			$this->view->canCreate = 0;
		else
			$this->view->canCreate = 1;
		$coverType = Engine_Api::_()->authorization()->getPermission($level, 'sesusercover', 'viewtype');
		if($coverType)
			$this->view->type = $coverType;
		else
			$this->view->type = 1;

    $canCreateVideo = Engine_Api::_()->authorization()->getPermission($level, 'sesusercovevideo', 'create');
		if($canCreateVideo == 0)
			$this->view->canCreateVideo = 0;
		else
			$this->view->canCreateVideo = 1;
    ...
    if(isset($_GET['type'])) {
      ...
      $this->view->type = $_GET['type'];
		...
		if($defaultCoverPhoto)
			...
			$this->view->defaultCoverPhoto = 'application/modules/Sesusercoverphoto/externals/images/default_cover.jpg';
	}
}
