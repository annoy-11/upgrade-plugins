 ...
 * @version    $Id: _type3.tpl 2016-05-06 00:00:00 SocialEngineSolutions $
...
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesusercoverphoto/externals/styles/cover_layout3.css'); ?>

<div class="sesusercover_main_container sesusercover_cover_type3 sesbasic_bxs <?php if($this->is_fullwidth){?>sesusercoverphoto_cover_wrapper_full<?php } ?>">
      ...
      <div class="sesusercoverphoto_cover_change_cover" id="sesusercoverphoto_cover_change">
        ...
        <div class="sesusercoverphoto_change_cover_options sesbasic_option_box"> 
          ...
          <?php if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesusercovervideo') && $this->canCreateVideo) { ?>
            <a href="<?php echo 'sesusercovervideo/index/upload/subject_id/'.$this->subject->getIdentity(); ?>" class="smoothbox" id="uploadCoverVideo" href="javascript:;"><i class="fa fa-plus"></i><?php echo ($this->subject->cover_video != 0 && $this->subject->cover_video != '') ? $this->translate('Change Cover Video') : $this->translate('Add Cover Video');; ?></a>
          <?php } ?>
  ...
  <div class="sesusercoverphoto_cover_information_block sesbasic_clearfix">
  	<div class="sesusercoverphoto_cover_information_block_inner sesbasic_clearfix">
     <?php if(in_array('photo',$this->option)){ ?>
      ...
      ?>
    	<div class="sesusercoverphoto_profile_img <?php if($this->userphotoround == 1): ?> isround <?php endif; ?>">
        ...
        <?php if($this->can_edit){ ?>
          ...
          <input type="file" id="uploadFileMainsesusercoverphoto" name="main_photo_cvr" onchange="uploadFileMainsesusercoverphoto(this);"  style="display:none" />
		...
		<?php } ?>
      <div class="sesusercoverphoto_cover_content sesbasic_clearfix">
      	 <?php if(in_array('title',$this->option)){ ?> 
        <div class="sesusercoverphoto_profile_title">
          <p>
    ...
    <?php if(in_array('membersince',$this->option)){ ?>     
        <p class="sesusercoverphoto_cover_field">
          <i class="fa fa-clock-o"></i>
          <span><?php echo  $this->translate('Member Since').': '.$this->timestamp($this->subject->creation_date) ; ?>
          </span>
        </p>
   <?php  } ?> 
         <?php if(in_array('location',$this->option) && $this->sesmemberenable && $getUserInfoItem->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmember_enable_location', 1)){ ?>
          <p class="sesusercoverphoto_cover_field sesbasic_clearfix"> 
           ...
           ?>
            <?php $music_count =  Engine_Api::_()->getApi('core', 'sesusercoverphoto')->getModuleRecordsOfUser($musicItem,$this->subject->getIdentity()); ?>
              <div title="<?php echo $this->translate(array('%s music', '%s musics', $music_count), $this->locale()->toNumber($music_count))?>"> 
                ...
                <span class="sesusercoverphoto_cover_stat_txt"> <?php echo str_replace(',','',preg_replace('/[0-9]+/', '', $this->translate(array('%s Music', '%s Musics', $music_count), $this->locale()->toNumber($music_count)))); ?></span>
              </div>
         <?php } ?>         
...
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesmember/externals/styles/styles.css'); ?>
        ...
        <?php if(in_array('recentlyViewedBy',$this->option) && $this->sesmemberenable){ ?>
          <div class="sesusercoverphoto_users">
    ...
    <?php if(($this->module == 'user' || $this->module == 'sesmember') && $this->controller == 'profile' && $this->action == 'index'): ?>
      <?php if($this->tab == 'inside'){ ?>
        <div class="sesmember_tabs sesusercoverphoto_cover_tabs"></div>
      <?php } ?>
    <?php endif; ?>
  </div>
  </div>
...
var heightin = sesJqueryObject('.sesusercoverphoto_cover_information_block').height();
...
function uploadFileToServerMain(files){
	...
	sesJqueryObject('.sesusercoverphoto_cover_main_photo').append('<div id="sesusercoverphoto_cover_loading_main" class="sesbasic_loading_cont_overlay" style="display:block;border-radius:50%;"></div>');
	<?php }else{ ?>
		sesJqueryObject('.sesusercoverphoto_cover_main_photo').append('<div id="sesusercoverphoto_cover_loading_main" class="sesbasic_loading_cont_overlay" style="display:block;"></div>');
	<?php } ?>
	var formData = new FormData();
    ...
    processData: false,
		...
		success: function(response){
			response = sesJqueryObject.parseJSON(response);
			sesJqueryObject('#uploadFileMainsesusercoverphoto').val('');
