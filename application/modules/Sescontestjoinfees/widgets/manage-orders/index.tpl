...
function paggingNumbermanage_order(pageNum){
	 ...
	 var searchFormData = sesJqueryObject('#sescontest_search_ticket_search').serialize();
		requestPagging= (new Request.HTML({
			...
			onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
				...
				sesJqueryObject('#sescontest_manage_order_content').html(responseHTML);
			}
		}));
		requestPagging.send();
		return false;
...
sesJqueryObject(document).on('click','.sescontest_search_ticket_search',function(e){
	e.prcontestDefault();
	sendParamInSearch = sesJqueryObject(this).attr('data-rel');
	sesJqueryObject('#sescontest_search_ticket_search').trigger('click');
});
