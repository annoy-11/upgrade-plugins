<?php
class Esenangpay_Plugin_Gateway_Senangpay extends Engine_Payment_Plugin_Abstract
{
  ...
  public function __construct(Zend_Db_Table_Row_Abstract $gatewayInfo)
  {
    ...
    $senangPayUserDetails = (object) $gatewayInfo->config;
  ...
  public function getGateway()
  {
    if (null === $this->_gateway) {
      $class = 'Esenangpay_Gateways_Senangpay';
  ...
  public function createSubscriptionTransaction(
    ...
    if ($package->isOneTime()) {
      ...
      $params['secretKey'] = $params['gateway']->config['sesadvpmnt_stripe_secret'];
      return  $this->createOrderTransaction($params);
    } else {
      $settings = Engine_Api::_()->getApi('settings', 'core');
      $this->view->secretKey = $secretKey = $params['gateway']->config['sesadvpmnt_stripe_secret'];
  ...
  public function onSubscriptionReturn(
    ...
    if ($package->isOneTime()) {
      ...
      if (
        $paymentStatus == 'okay' || ($paymentStatus == 'pending' && $giveBenefit)
      ) {
  ...
  public function createOrderTransactionReturn($order, $transaction)
  {
    ...
    if ($transaction->metadata->type == "crowdfunding") {
      ...
      if (
        $paymentStatus == 'okay' || $paymentStatus == 'active' || ($paymentStatus == 'pending' && $giveBenefit)
      ) {
        // Payment success
        try {
    ...
    } else if ($transaction->metadata->type == "Payment_Sescredit") {
      // Update order with profile info and complete status?
      $order->state = $orderStatus;
      $order->gateway_transaction_id = $transaction->id;
      $order->save();
      ...
      $transactionsTable->insert(array(
        ...
        'change_rate' => $transaction->metadata->change_rate,
        'state' => 'initial',
        'gateway_type' => 'Stripe',
        'currency_symbol' => strtoupper($transaction->currency),
        'ip_address' => $_SERVER['REMOTE_ADDR'],
      ));
      $transaction_id = $transactionsTable->getAdapter()->lastInsertId();
  ...
  {
    return array(
      'USD' => 'USD', 'AED' => 'AED', 'AFN' => 'AFN', 'ALL' => 'ALL', 'AMD' => 'AMD', 'ANG' => 'ANG', 'AOA' => 'AOA', 'ARS' => 'ARS', 'AUD' => 'AUD', 'AWG' => 'AWG', 'AZN', 'BAM' => 'BAM', 'BBD' => 'BBD', 'BDT' => 'BDT', 'BGN' => 'BGN', 'BIF' => 'BIF', 'BMD' => 'BMD', 'BND' => 'BND', 'BOB' => 'BOB', 'BRL' => 'BRL',
  ...
  {
    return new Esenangpay_Form_Admin_Settings_Senangpay();
  }
  ...
  public function createAdTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $item, Sescommunityads_Model_Package $package, array $params = array())
  { }
  ...
  public function setSendPaymentDetails($request, $detail, $orderId, $amount)
  {
    ...
    $this->phone = (isset($request->phone)) ?  $request->phone : "";
    return $this;
  }
  ...
  {
    $requestType = $this->getGateway()->getGatewayMethod();
    $httpQuery = [
      'detail' => $this->detail,
      'amount' => $this->amount,
    ...
    if ($requestType == "GET")
      $data = http_build_query($httpQuery);
  ...
  /**
   * @description  This will SenangPay api Url.
   * @return main url OR test url.
   */
  public function getApiUrl()
  {
    return ($this->getGateway()->getTestMode()) ? 'https://sandbox.senangpay.my/payment/' : 'https://app.senangpay.my/payment/';
  }
  ...
  {
    $senangPayApiUrl = $this->getApiUrl();
    ...
    if ($requestType == "GET")
      $returnUrl = $senangPayApiUrl . $this->merchantId . "?" . $data;
    else if ($requestType == "POST")
      $returnUrl = array('url' => $senangPayApiUrl . $this->merchantId, 'data' => $data);
    //file_get_contents($senangPayApiUrl . $this->merchantId, null, $context);
    return $returnUrl;
  }
  ...
  public function onAdTransactionReturn(Payment_Model_Order $order, array $params = array())
  {
    // Check that gateways match
    if ($order->gateway_id != $this->_gatewayInfo->gateway_id) {
      throw new Engine_Payment_Plugin_Exception('Gateways do not match');
