...
class Sesgroupforum_PostController extends Core_Controller_Action_Standard
{
  public function init()
  {
    if( 0 !== ($post_id = (int) $this->_getParam('post_id')) &&
        null !== ($post = Engine_Api::_()->getItem('sesgroupforum_post', $post_id)) &&
        $post instanceof Sesgroupforum_Model_Post ) {
  ...
  public function deleteAction()
  {
    ...
    if( !$this->_helper->requireSubject('sesgroupforum_post')->isValid() ) {
      return;
    }
    $this->view->viewer = $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->post = $post = Engine_Api::_()->core()->getSubject('sesgroupforum_post');
    ...
    if( !$this->_helper->requireAuth()->setAuthParams($post, null, 'delete')->checkRequire() &&
        !$this->_helper->requireAuth()->setAuthParams($sesgroupforum, null, 'topic.delete')->checkRequire() ) {
      return $this->_helper->requireAuth()->forward();
    }

    $this->view->form = $form = new Sesgroupforum_Form_Post_Delete();
    ...
    $href = ( null === $topic ? $sesgroupforum->getHref() : $topic->getHref() );
  ...
  public function editAction()
  {
    ...
    if( !$this->_helper->requireAuth()->setAuthParams($post, null, 'edit')->checkRequire() &&
        !$this->_helper->requireAuth()->setAuthParams($sesgroupforum, null, 'topic.edit')->checkRequire() ) {
      return $this->_helper->requireAuth()->forward();
    }
    ...
    try
    {
      ...
      return $this->_helper->redirector->gotoRoute(array('post_id'=>$post->getIdentity(), 'topic_id' => $post->getParent()->getIdentity()), 'sesgroupforum_topic', true);
    }
