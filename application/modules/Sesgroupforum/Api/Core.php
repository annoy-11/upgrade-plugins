...
class Sesgroupforum_Api_Core extends Core_Api_Abstract
{

  public function getMaxCategoryOrder()
  {
    $table = Engine_Api::_()->getItemTable('sesgroupforum_category');
  ...
  public function setRating($forum_id, $user_id, $rating){
    ...
    if (empty($row)) {
      // create rating
      Engine_Api::_()->getDbTable('ratings', 'sesgroupforum')->insert(array(
        'forum_id' => $forum_id,
        'user_id' => $user_id,
        'rating' => $rating
      ));
 ...
 function tagCloudItemCore($fetchtype = '',$limit = null, $topic_id = '') {
    ...
    $selecttagged_photo = $tableTagmap->select()
            ->from($tableTagName)
            ->setIntegrityCheck(false)
            ->where('resource_type =?', 'sesgroupforum_topic')
