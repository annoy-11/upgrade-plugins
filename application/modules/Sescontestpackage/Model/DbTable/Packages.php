...
class Sescontestpackage_Model_DbTable_Packages extends Engine_Db_Table {

  protected $_rowClass = 'Sescontestpackage_Model_Package';
  ...
  public function cancelSubscription($params = array()) {
    ...
    $db->query("DELETE FROM engine4_sescontestpackage_orderspackages WHERE package_id = $packageId && owner_id = " . $viewerId);
    $db->query("DELETE FROM engine4_sescontestpackage_transactions WHERE package_id = $packageId && owner_id = " . $viewerId);
    //End Work of Package Related Data
    
    Engine_Api::_()->getDbtable('contests', 'sescontest')->update(array('is_approved' => 0), array('package_id =?' => $packageId, 'user_id =?' => $viewerId));

    $transactionTable = Engine_Api::_()->getDbTable('transactions', 'sescontestpackage');
