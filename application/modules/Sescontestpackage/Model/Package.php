...
class Sescontestpackage_Model_Package extends Core_Model_Item_Abstract {
  ...
  public function allowUploadVideo($item, $left = false) {
    ...
    else if ($item && $item instanceof Sescontest_Model_Contest)
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('video', $modulesEnable['modules'])) {
      ...
      //free package
      if ($orderspackage_id) {
        $itemTable = Engine_Api::_()->getDbTable('contests', 'sescontest');
        $itemTableName = $itemTable->info('name');
        $subselect = $itemTable->select()->from($itemTableName, 'contest_id')->where('orderspackage_id =?', $orderspackage_id);
        $select->where('parent_id IN (?)', $subselect)->where('parent_type =?', 'contest');
      }
  ...
  public function allowUploadMusic($item, $left = false) {
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('music', $modulesEnable['modules'])) {
      ...
      $select = $albumSongTable->select()->from($albumSongTableName, array('total' => 'COUNT(albumsong_id)'))
              ...
              ->joinLeft($albumTableName, $albumSongTableName . '.album_id = ' . $albumTableName . '.album_id AND ' . $albumTableName . '.resource_type = "contest"', null);
      //free package
      if ($orderspackage_id) {
        $itemTable = Engine_Api::_()->getDbTable('contests', 'sescontest');
        $itemTableName = $itemTable->info('name');
  ...
  public function allowUploadPhoto($item, $left = false) {
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('photo', $modulesEnable['modules'])) {
      ...
      $photoTable = Engine_Api::_()->getDbTable('photos', 'sescontest');
      ...
      if ($orderspackage_id) {
        ...
        $itemTableName = $itemTable->info('name');
        $subselect = $itemTable->select()->from($itemTableName, 'contest_id')->where('orderspackage_id =?', $orderspackage_id);
        $select->where('contest_id IN (?)', $subselect);
      }
