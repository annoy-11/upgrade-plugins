...
class Sescontestpackage_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function createContestTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $item, Sescontestpackage_Model_Package $package, array $params = array()) {
    // Process description
  ...
  public function onContestTransactionReturn(
  Payment_Model_Order $order, array $params = array()) {
    ...
    if ($package->isOneTime()) {
      ...
      if (!empty($oldOrderPackageId)) {
        ...
        $totalItemCreated = count($tableContest->fetchAll($select));
  ...
  public function onContestTransactionIpn(
  Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
          ...
          if ($package->isOneTime()) {
            switch ($rawData['payment_status']) {
              ...
              case 'Pending':
                ...
                // Get benefit setting
                $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sescontestpackage')->getBenefitStatus($user);
                if ($giveBenefit) {
                  $item->onPaymentSuccess();
                } else {
                  $item->onPaymentPending();
  ...
  public function cancelContest($transactionId, $note = null) {
    $profileId = null;
    if ($transactionId instanceof Sescontestpackage_Model_Transaction) {
  ...
  public function onIpn(Engine_Payment_Ipn $ipn) {
    ...
    if ($order) {
      $ipnProcessed = false;
      // Subscription IPN
      if ($order->source_type == 'contest') {
        $this->onContestTransactionIpn($order, $ipn);
        $ipnProcessed = true;
      }
