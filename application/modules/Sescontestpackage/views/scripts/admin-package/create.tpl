...
<div class="sesbasic_search_result">
  <?php echo $this->htmlLink($this->url(array('module' => 'sescontestpackage', 'controller' => 'package','action'=>'index')), $this->translate("Back to Manage Packages"), array('class' => 'buttonlink sesbasic_icon_back')); ?>
</div><br />
...
<script type="application/javascript">
  ...
  sesJqueryObject(document).on('change','input[type=radio][name=contest_choose_style]',function(){
    if (this.value == 1) {
      sesJqueryObject('#contest_chooselayout-wrapper').show();
      sesJqueryObject('#contest_style_type-wrapper').hide();
    }else{
      sesJqueryObject('#contest_style_type-wrapper').show();
      sesJqueryObject('#contest_chooselayout-wrapper').hide();
    }
  });
  ...
  window.addEvent('domready', function() {
   ...
   var valueStyle = sesJqueryObject('input[name=contest_choose_style]:checked').val();
    if(valueStyle == 1) {
      sesJqueryObject('#contest_chooselayout-wrapper').show();
      sesJqueryObject('#contest_style_type-wrapper').hide();
    }
