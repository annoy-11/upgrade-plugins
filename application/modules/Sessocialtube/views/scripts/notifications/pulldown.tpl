...
<ul class='' id="notifications_main" onclick="redirectPage(event);">
  ...
  <?php $user = Engine_Api::_()->getItem('user', $notification->subject_id);?>
    <li class="clearfix <?php if( !$notification->socialtube_read ): ?>pulldown_content_list_highlighted<?php endif; ?>"  value="<?php echo $notification->getIdentity();?>">
        ...
        url = url[0];
			...
			if( notificationD_li.get('class') == 'clearfix pulldown_content_list_highlighted' ){
				...
				en4.core.request.send(new Request.JSON({
					url : en4.core.baseUrl + 'sessocialtube/notifications/markread',
					data : {
						format     : 'json',
