 ...
 * @version    $Id: PayPal.php  2018-10-16 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesbusinesspackage_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function getGateway() {
    if (null === $this->_gateway) {
      ...
      $gateway = new $class(array(
          ...
          'currency' => Engine_Api::_()->sesbusinesspackage()->getCurrentCurrency(),
      ));
  ...
  public function createBusinessTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $item, Sesbusinesspackage_Model_Package $package, array $params = array()) {
    // Process description
    ...
    $defaultCurrency = Engine_Api::_()->sesbusinesspackage()->defaultCurrency();
  ...
  public function onBusinessTransactionReturn(
  Payment_Model_Order $order, array $params = array()) {
    ...
    if ($package->isOneTime()) {
      ...
      $orderPackage = Engine_Api::_()->getItem('sesbusinesspackage_orderspackage', $orderPackageId);
      ...
      $session = new Zend_Session_Namespace('Payment_Sesbusinesspackage');
      ...
      if (!empty($oldOrderPackageId)) {
        $select = $tableBusiness->select()->from($tableBusiness->info('name'))->where('orderspackage_id =?', $oldOrderPackageId);
        $totalItemCreated = count($tableBusiness->fetchAll($select));
        if ($package->item_count >= $totalItemCreated && $package->item_count)
          $leftBusiness = $package->item_count - $totalItemCreated;
        else if (!$package->item_count)
          $leftBusiness = -1;
        else
          $leftBusiness = 0;
      } else
        $leftBusiness = $package->item_count - 1;
      $tableBusiness->update(array('orderspackage_id' => $orderPackageId), array('orderspackage_id' => $oldOrderPackageId));
      $packageOrder = Engine_Api::_()->getItem('sesbusinesspackage_orderspackage', $orderPackageId);
      $packageOrder->item_count = $leftBusiness;
      $packageOrder->save();
      ...
      // Get benefit setting
      $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sesbusinesspackage')
  ...
  public function onBusinessTransactionIpn(
  Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
          ...
          if ($package->isOneTime()) {
            switch ($rawData['payment_status']) {
              ...
              case 'Pending':
                ...
                // Get benefit setting
                $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sesbusinesspackage')->getBenefitStatus($user);
                if ($giveBenefit) {
                  $item->onPaymentSuccess();
                } else {
                  $item->onPaymentPending();
  ...
  public function cancelBusiness($transactionId, $note = null) {
    $profileId = null;
    if ($transactionId instanceof Sesbusinesspackage_Model_Transaction) {
  ...
  /**
   * Generate href to a business detailing the order
   *
  ...
  /**
   * Generate href to a business detailing the transaction
   *
   * @param string $transactionId
   * @return string
   */
  ...
  public function onIpn(Engine_Payment_Ipn $ipn) {
    ...
    if ($order) {
      $ipnProcessed = false;
      // Subscription IPN
      if ($order->source_type == 'businesses') {
