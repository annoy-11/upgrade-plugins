...
class Sesbusinesspackage_Plugin_Gateway_Paytm extends Engine_Payment_Plugin_Abstract
{
  ...
  public function createBusinessTransaction(User_Model_User $user,
  ...
  public function onBusinessTransactionReturn(
    ...
    if($params['STATUS'] == 'TXN_FAILURE' ) {
      ...
      // Error
      throw new Payment_Model_Exception('Your payment has been cancelled and '.$params['RESPMSG']);
    }
    // Get payment state
    $paymentStatus = null;
    $orderStatus = null;
    ...
    if($package->isOneTime()) {
      ...
      $tablePage = Engine_Api::_()->getDbTable('businesses', 'sesbusiness');
    ...
    public function onIpnTransaction($subscription){
      ...
      if ($order) {
        ...
        if ($order->source_type == 'businesses') {
          $this->onBusinessTransactionIpn($order, $response);
          $ipnProcessed = true;
        }
  ...
  public function onBusinessTransactionIpn(Payment_Model_Order $order,  $rawData) { 
