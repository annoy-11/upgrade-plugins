 ...
 * @version    $Id: business.tpl  2018-10-16 00:00:00 SocialEngineSolutions $
...
<?php if(count($this->existingleftpackages)){ ?>
	<div class="sesbusiness_packages_main sesbasic_clearfix sesbasic_bxs">
    ...
    <div class="sesbusiness_packages_table_container">
      ...
      <?php foreach($this->existingleftpackages as $packageleft)	{
       ...
       ?>
        <?php include APPLICATION_PATH .  '/application/modules/Sesbusinesspackage/views/scripts/_packagesHorizontal.tpl';?>
      <?php } ?>
      </ul>
    </div>
  </div>
...
<?php if(count($this->package)){ ?>
	<div class="sesbusiness_packages_main sesbasic_clearfix sesbasic_bxs">
      ...
      <p><?php echo $this->translate('Select a package that suits you most to start creating businesses on this website.');?></p>
    </div>
    <div class="sesbusiness_packages_table_container">
      <ul class="sesbusiness_packages_table">
        <?php $existing = 0;?>
      	<?php foreach($this->package as $package)	{
              $enableModules = json_decode($package->params,true);
       	?>
         <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinesspackage.package.style', 1)):?>
           <?php include APPLICATION_PATH .  '/application/modules/Sesbusinesspackage/views/scripts/_packages.tpl';?>
         <?php else:?>
           <?php include APPLICATION_PATH .  '/application/modules/Sesbusinesspackage/views/scripts/_packagesHorizontal.tpl';?>
         <?php endif;?>
      	<?php } ?>
