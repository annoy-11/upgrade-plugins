...
class Sesbusinesspackage_PaymentController extends Core_Controller_Action_Standard
{
  /**
   * @var User_Model_User
   */
  ...
  public function init()
  {
    ...
    if( Engine_Api::_()->getDbtable('gateways', 'payment')->getEnabledGatewayCount() <= 0 ||
        Engine_Api::_()->getDbtable('packages', 'sesbusinesspackage')->getEnabledNonFreePackageCount() <= 0 ) {
    ...
    $this->_session = new Zend_Session_Namespace('Payment_Sesbusinesspackage');
		$this->_session->gateway_id = $this->_getParam('gateway_id',0);
    $this->_item = Engine_Api::_()->getItem('businesses',$this->_getParam('business_id'));
		if(!$this->_item)
  ...
  public function gatewayAction()
  {
    ...
    // For Coupon 
    $this->view->couponSessionCode = $package->getType().'-'.$package->package_id.'-'.$this->_item->getType().'-'.$this->_item->business_id.'-1';
  ...
  public function processAction()
  {
    ...
    if( !$gatewayId ||
        !($gateway = Engine_Api::_()->getDbtable('gateways', 'sesbusinesspackage')->find($gatewayId)->current()) ||
        !($gateway->enabled) ) {
    ...
    // For Coupon 
    $couponSessionCode = $package->getType().'-'.$package->package_id.'-'.$this->_item->getType().'-'.$this->_item->business_id.'-1';
    ...
    $this->_session->order_id = $order_id = $ordersTable->getAdapter()->lastInsertId();
		$this->_session->currency = $currentCurrency = Engine_Api::_()->sesbusinesspackage()->getCurrentCurrency();
		$settings = Engine_Api::_()->getApi('settings', 'core');
		$this->_session->change_rate = $settings->getSetting('sesmultiplecurrency.' . $currentCurrency) ;
    // Unset certain keys
    unset($this->_session->package_id);
    unset($this->_session->business_id);
    unset($this->_session->gateway_id);
    // Get gateway plugin
    ...
    $params['cancel_url'] = $schema . $host
      ...
      . '?order_id=' . $order_id
      . '&state=' . 'cancel';
    $params['ipn_url'] = $schema . $host . $this->view->url(array('action' => 'index', 'controller' => 'ipn', 'module' => 'sesbusinesspackage'), 'default') . '?order_id=' . $order_id.'&gateway_id='.$gatewayId;
    ...
    $creditCode =  'credit'.'-sesbusinesspackage-'.$package->package_id.'-'.$this->_item->business_id;
    ...
    if($gateway->plugin == "Sesadvpmnt_Plugin_Gateway_Stripe") {
          $this->view->currency =  $params['currency'] =  $currentCurrency = Engine_Api::_()->sesbusinesspackage()->getCurrentCurrency();
        ...
        $paytmParams = $plugin->createBusinessTransaction($this->_user, $this->_item, $package, $params);
        ...
        if($gateway->test_mode){
         ...
         $this->renderScript('/application/modules/Epaytm/views/scripts/payment/index.tpl');
          ...
          // Process transaction
        $transaction = $plugin->createBusinessTransaction($this->_user,
  ...
  public function returnAction()
  {
    ...
    if( !$this->_user ||
        ...
        $order->source_type != 'businesses' ||
        !($item = $order->getSource()) ||
        !($package = $this->_item->getPackage()) ||
        !($gateway = Engine_Api::_()->getItem('sesbusinesspackage_gateway', $order->gateway_id)) ) {
      return $this->_helper->redirector->gotoRoute(array('action'=>'manage'), 'sesbusiness_general', true);
    }
    // Get gateway plugin
    $this->view->gatewayPlugin = $gatewayPlugin = $gateway->getGateway();
    ...
    //For Coupon
    $couponSessionCode = $couponSessionCode = $package->getType().'-'.$package->package_id.'-'.$this->_item->getType().'-'.$this->_item->business_id.'-1';
    $params['amount'] = @isset($_SESSION[$couponSessionCode]) ? round($package->price - $_SESSION[$couponSessionCode]['discount_amount']) : $package->price;
    ...
    try {
      if($gateway->plugin == "Sesadvpmnt_Plugin_Gateway_Stripe") {
          if(isset($_POST['stripeToken'])){
            ...
            $params['type'] = "sesbusinesspackage_gateway";
            if(!$package->isOneTime()) {
            ...
            $transaction = $plugin->createBusinessTransaction($this->_user, $this->_item, $package, $params);
          }
          $params['transaction'] = $transaction;
         $status = $plugin->onBusinessTransactionReturn($order,$params);
      } else {
         $status = $plugin->onBusinessTransactionReturn($order,array_merge($this->_getAllParams(),$params));
      }
