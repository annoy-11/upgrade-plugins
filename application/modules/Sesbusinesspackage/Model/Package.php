...
class Sesbusinesspackage_Model_Package extends Core_Model_Item_Abstract {
  ...
  public function getProduct() {
    if (null === $this->_product) {
      ...
      $this->_product = $productsTable->fetchRow($productsTable->select()
                      ->where('extension_type = ?', 'businesses')
  ...
  public function getPackageDescription() {
    ...
    $priceStr = Engine_Api::_()->sesbusinesspackage()->getCurrencyPrice($this->price,'','');
  ...
  public function allowUploadVideo($item, $left = false) {
    ...
    else if ($item && $item instanceof Sesbusiness_Model_Business)
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('video', $modulesEnable['modules'])) {
      ...
      //free package
      if ($orderspackage_id) {
        $itemTable = Engine_Api::_()->getDbTable('businesses', 'sesbusiness');
        $itemTableName = $itemTable->info('name');
        $subselect = $itemTable->select()->from($itemTableName, 'business_id')->where('orderspackage_id =?', $orderspackage_id);
        $select->where('parent_id IN (?)', $subselect)->where('parent_type =?', 'businesses');
      }
  ...
  public function allowUploadMusic($item, $left = false) {
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('music', $modulesEnable['modules'])) {
      ...
      $select = $albumSongTable->select()->from($albumSongTableName, array('total' => 'COUNT(albumsong_id)'))
              ...
              ->joinLeft($albumTableName, $albumSongTableName . '.album_id = ' . $albumTableName . '.album_id AND ' . $albumTableName . '.resource_type = "businesses"', null);
      //free package
      if ($orderspackage_id) {
        $itemTable = Engine_Api::_()->getDbTable('businesses', 'sesbusiness');
        $itemTableName = $itemTable->info('name');
  ...
  public function allowUploadPhoto($item, $left = false) {
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('photo', $modulesEnable['modules'])) {
      ...
      if ($orderspackage_id) {
        ...
        $itemTableName = $itemTable->info('name');
        $subselect = $itemTable->select()->from($itemTableName, 'business_id')->where('orderspackage_id =?', $orderspackage_id);
        $select->where('business_id IN (?)', $subselect);
      }
