...
class Seseventcontact_Form_Admin_Mail extends Engine_Form {

  public function init() {
	
		$description = $this->getTranslator()->translate('Using this form, you will be able to send an email out to all event owners.  Emails are sent out using a queue system, so they will be sent out over time.  An email will be sent to you when all emails have been sent. <br>');
    ...
    $this->setTitle('Email All Event Owners')
...
//     ));
    ...
    $this->addElement('Multiselect', 'sesevent_memberlevels', array(
      'label' => 'Member Levels',
      'description' => 'Hold down the CTRL key to select or de-select specific Member Levels.',
      'required' => true,
      'allowEmpty' => false,
    ...
    $this->sesevent_memberlevels->getDecorator('Description')->setOptions(array('placement' => 'APPEND'));
