...
class Seselegant_AdminManageBannerController extends Core_Controller_Action_Admin {

  public function indexAction() {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('seselegant_admin_main', array(), 'seselegant_admin_main_managebanners');
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('banners', 'seselegant')->getBanner();
    if ($this->getRequest()->isPost()) {
      $db = Engine_Db_Table::getDefaultAdapter();
      $values = $this->getRequest()->getPost();
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $banner = Engine_Api::_()->getItem('seselegant_banner', $value)->delete();
          $db->query("DELETE FROM engine4_seselegant_slides WHERE banner_id = " . $value);
        }
      }
    }
  ...
  public function createSlideAction() {
    ...
    $this->view->form = $form = new Seselegant_Form_Admin_Createslide();
    if ($slide_id) {
      ...
      $slide = Engine_Api::_()->getItem('seselegant_slide', $slide_id);
      $form->populate($slide->toArray());
    }
    if ($this->getRequest()->isPost()) {
      if (!$form->isValid($this->getRequest()->getPost()))
        return;
      $db = Engine_Api::_()->getDbtable('slides', 'seselegant')->getAdapter();
      $db->beginTransaction();
      try {
        $table = Engine_Api::_()->getDbtable('slides', 'seselegant');
        ...
        $slide->save();
        $db->commit();
        $url = Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'seselegant', 'controller' => 'manage-banner', 'action' => 'manage', 'id' => $id), 'admin_default', true);
        header("Location:" . $url);
      } catch (Exception $e) {
        $db->rollBack();
        throw $e;
  ...
  public function deleteSlideAction() {
    ...
    // Check post
    if ($this->getRequest()->isPost()) {
      $slide = Engine_Api::_()->getItem('seselegant_slide', $id);
      if ($slide->thumb_icon) {
  ...
  public function manageAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $slide = Engine_Api::_()->getItem('seselegant_slide', $value);
          if ($slide->file_id) {
            $item = Engine_Api::_()->getItem('storage_file', $slide->file_id);
            if ($item->storage_path) {
              @unlink($item->storage_path);
    ...
    if (!$id)
      return;
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('slides', 'seselegant')->getSlides($id, 'show_all');
    $page = $this->_getParam('page', 1);
  ...
  public function orderAction() {
    ...
    $slidesTable = Engine_Api::_()->getDbtable('slides', 'seselegant');
  ...
  public function deleteBannerAction() {
    ...
    // Check post
    if ($this->getRequest()->isPost()) {
      $chanel = Engine_Api::_()->getItem('seselegant_banner', $id)->delete();
      $db = Engine_Db_Table::getDefaultAdapter();
      $db->query("DELETE FROM engine4_seselegant_slides WHERE banner_id = " . $id);
      $this->_forward('success', 'utility', 'core', array(
          'smoothboxClose' => 10,
          'parentRefresh' => 10,
  ...
  public function createBannerAction() {
    ...
    $this->view->form = $form = new Seselegant_Form_Admin_Banner();
    if ($id) {
      $form->setTitle("Edit Banner Name");
      $form->submit->setLabel('Save Changes');
      $banner = Engine_Api::_()->getItem('seselegant_banner', $id);
      $form->populate($banner->toArray());
    }
    if ($this->getRequest()->isPost()) {
      if (!$form->isValid($this->getRequest()->getPost()))
        return;
      $db = Engine_Api::_()->getDbtable('banners', 'seselegant')->getAdapter();
      $db->beginTransaction();
      try {
        $table = Engine_Api::_()->getDbtable('banners', 'seselegant');
        $values = $form->getValues();
        if (!$id)
  ...
  public function enabledAction() {
    ...
    if (!empty($id)) {
      if(!empty($banner_id))
      $item = Engine_Api::_()->getItem('seselegant_slide', $id);
      else
      $item = Engine_Api::_()->getItem('seselegant_banner', $id);
      $item->enabled = !$item->enabled;
      $item->save();
    }
    if(!empty($banner_id))
    $this->_redirect('admin/seselegant/manage-banner/manage/id/'.$banner_id);
    else
    $this->_redirect('admin/seselegant/manage-banner');
  }
