...
class Sescrowdfundingvideo_Form_Video extends Engine_Form {

  public function init() {

    $crowdfundingvideo_id = Zend_Controller_Front::getInstance()->getRequest()->getParam('crowdfundingvideo_id');
    ...
    $setting = Engine_Api::_()->getApi('settings', 'core');
		...
		$allowedUploadOption = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('crowdfundingvideo', $viewer, 'sesvdeo_upld');
    ...
    //My Computer
    if ($myComputer) {
      $allowed_upload = 1;//Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('crowdfundingvideo', $user, 'upload');
    ...
    $this->addElement('Text', 'title', array(
		...
		if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo_enable_location', 1)){
    ...
    $viewOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('crowdfundingvideo', $user, 'auth_view');
    ...
    // Comment
    $commentOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('crowdfundingvideo', $user, 'auth_comment');
    ...
    if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock')) && Engine_Api::_()->authorization()->getPermission($viewer, 'crowdfundingvideo', 'video_locked')) {
    ...
    $uploadoption = $settings->getSetting('sescrowdfundingvideo.uploadphoto', '0');
    if ($uploadoption == 1) {
      if (isset($video) && $video->photo_id) {
        ...
        if (isset($path) && !empty($path)) {
          $this->addElement('Image', 'cover_photo_preview sesbd', array(
              'src' => $path,
              'class' => 'sescrowdfundingvideo_channel_thumb_preview sesbd',
          ));
