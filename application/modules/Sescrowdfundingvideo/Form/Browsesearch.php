...
class Sescrowdfundingvideo_Form_Browsesearch extends Engine_Form {
  ...
  public function init() {
    parent::init();
		...
		if($this->getSearchFor() == 'video')
			$searchFor =$view->url(array('controller' => 'index', 'action' => 'browse','module'=>'sescrowdfundingvideo'), 'sescrowdfundingvideo_general', true);
    $this
            ->setAttribs(array(
                'id' => 'filter_form',
    ...
    if ($this->_searchTitle == 'yes') {
      ...
      $this->addElement('Text', 'search', array(
		...
		if ($this->_searchPageTitle == 'yes' || 1){
			 $valueD = Zend_Registry::get('Zend_Translate')->_('Page Title:');
      $this->addElement('Text', 'search_page', array(
          'label' => $valueD
      ));
    }
    ...
    $this->addElement('Button', 'submit', array(
		...
		$this->addElement('Dummy','loading-img-sescrowdfundingvideo', array(
        'content' => '<img src="application/modules/Core/externals/images/loading.gif" id="sescrowdfundingvideo-category-widget-img" alt="Loading" />',
   ));
  }
