...
Composer.Plugin.Sescrowdfundingvideo = new Class({
...
// 		}
    ...
    if (this.options.allowed == 1 && this.options.type != 'message' && this.options.myComputerCheck == 1){
      ...
      this.elements.previewDescription = new Element('div', {
        ...
        'html' : this._lang('To upload a video from your device, please use our <a href="'+en4.core.baseUrl+'sescrowdfundingvideo/create/type/3">full uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
    }
    else{
    if (this.options.allowed == 1 && this.options.type != 'message' && this.options.myComputerCheck == 1){
      ...
      this.elements.previewDescription = new Element('div', {
        'id' : 'compose-video-upload',
        'class' : 'compose-video-upload',
        'html' : this._lang('To upload a video from your computer, please use our <a href="'+en4.core.baseUrl+'sescrowdfundingvideo/create/type/3">full     uploader</a>.'),
        'style': 'display:none;'
      }).inject(this.elements.body);
    }
  ...
  deactivate : function() {
    ...
    if (this.params.crowdfundingvideo_id)
      new Request.JSON({
        url: en4.core.basePath + 'sescrowdfundingvideo/index/delete',
        data: {
          format: 'json',
          crowdfundingvideo_id: this.params.crowdfundingvideo_id
        }
      }).send();
    if( !this.active ) return;
    this.parent();
  ...
  doProcessResponse : function(responseJSON, responseText) {
    // Handle error
    if( ($type(responseJSON) != 'hash' && $type(responseJSON) != 'object') || $type(responseJSON.src) != 'string' || $type(parseInt(responseJSON.crowdfundingvideo_id)) != 'number' ) {
    ...
    this.params.set('crowdfundingvideo_id', responseJSON.crowdfundingvideo_id);
