...
var autoPlayId,firstVideoSrc=false,isOnRegister = false,IsfinishVideoNext = false,clearIntvalSescrowdfundingVideoSlideshow1,clearIntvalSescrowdfundingVideoSlideshow2 ;
jQuery(document).ready(function($){
		...
		sliderNav.on('click', 'li', function(event){
			...
			if(!selectedItem.hasClass('selected')) {
				...
				if(slidesWrapper.find('li.selected').hasClass('cd-bg-video')){
					 ...
					 clearInterval(clearIntvalSescrowdfundingVideoSlideshow1);
					 clearInterval(clearIntvalSescrowdfundingVideoSlideshow1);
				}
			}
		});
	...
	function checkVideo(hiddenSlide, container, n) {
		...
		if( visibleVideo.length > 0 ) {
			clearInterval(autoPlayId);
			clearInterval(clearIntvalSescrowdfundingVideoSlideshow2);
			clearInterval(clearIntvalSescrowdfundingVideoSlideshow1);
			if(!Modernizr.touch){
