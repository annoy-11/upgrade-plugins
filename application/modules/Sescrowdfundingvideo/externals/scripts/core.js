...
var marker;
var mapLoad = true;
function initializeSesPageVideoMap() {
  var mapOptions = {
  ...
  google.maps.event.addDomListener(window, 'load', initializeSesPageVideoMap);
}
function editMarkerOnMapSesPageVideoEdit(){
...
function editSetMarkerOnMapSesPageVideo(){
	geocoder = new google.maps.Geocoder();
...
//list page map
function initializeSesPageVideoMapList () {
  ...
  if(mapLoad){
	...
	if(mapLoad){
		google.maps.event.addDomListener(window, 'load', initializeSescrowdfundingVideoMapList);
	}
}

function editSetMarkerOnMapListSesPageVideo() {
...
sesJqueryObject(document).on('click','#openPageVideoInLightbox',function(){
	var getImageHref = window.location.href;
	videoURLsesbasic = videoURLsescrowdfundingvideo;
	moduleName = 'sescrowdfundingvideo';
	...
	var image = sesJqueryObject('#sescrowdfundingvideo_image_video_url').attr('data-src');
...
function checkFunctionEmbed(){
	if( typeof flashembed == 'function'){
		//silence
		 ...
		 if(sesJqueryObject('.sescrowdfundingvideo_attachment_info').length){
				var href = sesJqueryObject('.sescrowdfundingvideo_attachment_info').find('a').attr('href');
...
sesJqueryObject(document).on('click','.sescrowdfundingvideo_list_option_toggle',function(){
...
sesJqueryObject(document).on('click','.sescrowdfundingvideo_watch_later',function(){
		var that = this;
		...
		if(sesJqueryObject(this).hasClass('selectedWatchlater')){
		 ...
		 (new Request.HTML({
      method: 'post',
      'url':  en4.core.baseUrl + 'sescrowdfundingvideo/watchlater/add/',
      'data': {
        format: 'html',
...
function sescrowdfundingvideo_like_favourite_data(element,functionName,itemType,likeNoti,unLikeNoti,className){
		...
		if(sesJqueryObject(element).hasClass('button_active')){
		 ...
		 (new Request.HTML({
      method: 'post',
      'url':  en4.core.baseUrl + 'sescrowdfundingvideo/index/'+functionName,
      'data': {
        format: 'html',
...
sesJqueryObject(document).on('click','.sescrowdfundingvideo_favourite_crowdfundingvideo',function(){
	sescrowdfundingvideo_like_favourite_data(this,'favourite','crowdfundingvideo','<i class="fa fa-heart"></i><span>'+(en4.core.language.translate("Video added as Favourite successfully"))+'</span>','<i class="fa fa-heart"></i><span>'+(en4.core.language.translate("Video Unfavourited successfully"))+'</span>','sesbasic_favourites_notification');
});
sesJqueryObject(document).on('click','.sescrowdfundingvideo_like_crowdfundingvideo',function(){
	sescrowdfundingvideo_like_favourite_data(this,'like','crowdfundingvideo','<i class="fa fa-thumbs-up"></i><span>'+(en4.core.language.translate("Video Liked successfully"))+'</span>','<i class="fa fa-thumbs-up"></i><span>'+(en4.core.language.translate("Video Unliked successfully"))+'</span>','sesbasic_liked_notification');
});
sesJqueryObject(document).on("click", '.sescrowdfundingvideo_thumb_img', function (e) {
