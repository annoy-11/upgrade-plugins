...
class Sescrowdfundingvideo_Plugin_Core {
  public function onStatistics($event) {
    $table = Engine_Api::_()->getDbTable('videos', 'sescrowdfundingvideo');
    $select = new Zend_Db_Select($table->getAdapter());
    $select->from($table->info('name'), 'COUNT(*) AS count');
    $event->addResponse($select->query()->fetchColumn(0), 'crowdfundingvideo');
  }
...
EOF;
    $script .=
						"var videoURLsescrowdfundingvideo = '" . Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.video.manifest', 'crowdfundingvideo') . "';
