...
class Sescrowdfundingvideo_Model_Crowdfundingvideo extends Core_Model_Item_Abstract {

  protected $_parent_type = 'user';
  protected $_owner_type = 'user';
  protected $_parent_is_owner = true;
  protected $_type = 'crowdfundingvideo';
  ...
  public function getRichContent($view = false, $params = array(), $map = false,$autoplay = true) {
    ...
    if ($this->type == 1) {
      $videoEmbedded = $this->compileYouTube($this->crowdfundingvideo_id, $this->code, $view, $mobile, $map,$autoplay);
    }
    // if video type is vimeo
    if ($this->type == 2) {
      $videoEmbedded = $this->compileVimeo($this->crowdfundingvideo_id, $this->code, $view, $mobile, $map,$autoplay);
    }
    // if video type is uploaded
    if ($this->type == 3) {
      $video_location = Engine_Api::_()->getItem('crowdfundingvideo', $this->crowdfundingvideo_id)->getHref();
      $videoEmbedded = $this->compileFlowPlayer($video_location, $view, $map,$autoplay);
    }
    // if video type is dailymotion
    if ($this->type == 4) {
      $videoEmbedded = $this->compileDailymotion($this->crowdfundingvideo_id, $this->code, $view, $mobile, $map,$autoplay);
    }
    ...
    if ($view == false) {
      ...
      if ($this->duration) {
        ...
        $video_duration = "<span class='sescrowdfundingvideo_length'>" . $duration . "</span>";
      }
			 $watchLater = '';
			$watchLaterId = Engine_Api::_()->sescrowdfundingvideo()->getWatchLaterId($this->crowdfundingvideo_id);
           ...
           $watchLater =   '<a href="javascript:;" class="sescrowdfundingvideo_watch_later_btn sescrowdfundingvideo_watch_later '.$watchLaterActive.'" title = "'.$watchLaterText.'" data-url="'.$this->crowdfundingvideo_id.'"></a>';
       }
			 ...
			 $urlencode = urlencode(((!empty($_SERVER["HTTPS"]) &&  strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $this->getHref());
			  $buttons = '<div class="sescrowdfundingvideo_thumb_btns">
                ...
                if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0 ){
											$thistype = 'crowdfundingvideo';
											$getId = 'crowdfundingvideo_id';
                      $canComment =  $this->authorization()->isAllowed(Engine_Api::_()->user()->getViewer(), 'comment');
                      if($canComment){
                $LikeStatus = Engine_Api::_()->sescrowdfundingvideo()->getLikeStatusVideo($this->$getId,$this->getType());
								$likeText = ($LikeStatus) ? 'button_active' : '';
                $buttons  .= ' <a href="javascript:;" data-url="'. $this->$getId .'" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn sescrowdfundingvideo_like_crowdfundingvideo '.$likeText.'"> <i class="fa fa-thumbs-up"></i><span>'. $this->like_count.'</span></a>';
                     } ;
                     if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.enable.favourite', 1) && isset($this->favourite_count)){
                   		$favStatus = Engine_Api::_()->getDbtable('favourites', 'sescrowdfundingvideo')->isFavourite(array('resource_type'=>$thistype,'resource_id'=>$this->$getId));
											$favText = ($favStatus)  ? 'button_active' : '';
                   		$buttons .= '<a href="javascript:;" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_fav_btn sescrowdfundingvideo_favourite_crowdfundingvideo '.$favText .'"  data-url="'.$this->$getId.'"><i class="fa fa-heart"></i><span>'. $this->favourite_count.'</span></a>';
                  	}
                }
             $buttons .= ' </div>';
      // prepare the thumbnail
      $thumb = Zend_Registry::get('Zend_View')->itemPhoto($this, 'thumb.video.activity');
      $thumb = '<a class="sescrowdfundingvideo_thumb_img sescrowdfundingvideo_attachment_thumb_img" href="'.$this->getHref().'" data-url="video"><span style="background-image:url(' . $this->getPhotoUrl() . ');"></span></a><a class="sescrowdfundingvideo_play_btn fa fa-play-circle sescrowdfundingvideo_thumb_img" href="'.$this->getHref().'" data-url="video"><span style="background-image:url(' . $this->getPhotoUrl() . ');display:none"></span></a>'.$video_duration.$watchLater.$buttons;
      if (!$mobile) {
        $thumb = '<div class="sescrowdfundingvideo_thumb sescrowdfundingvideo_attachment_thumb sescrowdfundingvideo_play_btn_wrap sescrowdfundingvideo_activity_video sesbasic_bxs" id="video_thumb_' . $this->crowdfundingvideo_id . '" >' . $thumb  . '</div>';
      } else {
        $thumb = '<div class="sescrowdfundingvideo_thumb sescrowdfundingvideo_attachment_thumb sescrowdfundingvideo_play_btn_wrap sescrowdfundingvideo_activity_video sesbasic_bxs" id="video_thumb_' . $this->crowdfundingvideo_id . '" href="' . $this->getHref() . '">' . $thumb  . '</div>';
      }
      // prepare title and description
      $title = "<a href='" . $this->getHref($params) . "'>$this->title</a>";
      $tmpBody = strip_tags($this->description);
      $description = "<div class='sescrowdfundingvideo_attachment_desc'>" . (Engine_String::strlen($tmpBody) > 255 ? Engine_String::substr($tmpBody, 0, 255) . '...' : $tmpBody) . "</div>";
      $videoEmbedded = $thumb . '<div id="video_object_' . $this->crowdfundingvideo_id . '" data-rel="'.$this->type.'" class="sescrowdfundingvideo_object">' . $videoEmbedded . '</div><div class="sescrowdfundingvideo_attachment_info">' . $title . $description . '</div>';
    }
  ...
  public function getPhotoUrl($type = null) {
    ...
    if (!$photo_id && !$this->is_locked)
      return 'application/modules/Sescrowdfundingvideo/externals/images/video.png';
    if ($this->is_locked)
      return 'application/modules/Sescrowdfundingvideo/externals/images/locked-video.jpg';
  ...
  public function getEmbedCode(array $options = null) {
    ...
    $url = 'http://' . $_SERVER['HTTP_HOST']
            . Zend_Controller_Front::getInstance()->getRouter()->assemble(array(
                ...
                'controller' => 'video',
                'action' => 'external',
                'crowdfundingvideo_id' => $this->getIdentity(),
	...
	public function compileFromUrl($crowdfundingvideo_id, $code, $view, $mobile = false, $map = false){
    ...
    id="videoFrame' . $crowdfundingvideo_id . '"
    ...
    <script type="text/javascript">
        ...
        var doResize = function() {
            var aspect = 16 / 9;
            var el = document.id("videoFrame' . $crowdfundingvideo_id . '");
  ...
  public function compileDailymotion($crowdfundingvideo_id, $code, $view, $mobile = false, $map = false,$autoPlayView) {
    $autoplay = !$mobile && $view;
		if(!$autoPlayView)
  ...
  public function compileYouTube($crowdfundingvideo_id, $code, $view, $mobile = false, $map = false,$autoPlayView) {
    $autoplay = !$mobile && $view;
		if(!$autoPlayView)
			$autoplay = $autoPlayView;
    if ($map)
  ...
  public function compileVimeo($crowdfundingvideo_id, $code, $view, $mobile = false, $map = false,$autoPlayView) {
    $autoplay = !$mobile && $view;
		if(!$autoPlayView)
			$autoplay = $autoPlayView;
    if ($map)
  ...
  public function compileFlowPlayer($location, $view, $map = false) {
    ...
    $embedded = "
    <div id='videoFrame" . $this->crowdfundingvideo_id . "'></div>
    <script type='text/javascript'>
    en4.core.runonce.add(function(){\$('video_thumb_" . $this->crowdfundingvideo_id . "').removeEvents('click').addEvent('click', function(){checkFunctionEmbed();flashembed('videoFrame$this->crowdfundingvideo_id',{src: '" . Zend_Registry::get('StaticBaseUrl') . $flowplayer."', width: " . ($view ? "480" : "420") . ", height: " . ($view ? "386" : "326") . ", wmode: 'opaque'},{config: {clip: {url: '$location',autoPlay: " . ($view ? "false" : "true") . ", duration: '$this->duration', autoBuffering: true},plugins: {controls: {background: '#000000',bufferColor: '#333333',progressColor: '#444444',buttonColor: '#444444',buttonOverColor: '#666666'}},canvas: {backgroundColor:'#000000'}}});})});
    </script>";
