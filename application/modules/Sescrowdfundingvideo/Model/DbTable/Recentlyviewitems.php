 ...
 * @version    $Id: Recentlyviewitems.php 2018-07-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sescrowdfundingvideo_Model_DbTable_Recentlyviewitems extends Engine_Db_Table {
  protected $_name = 'sescrowdfundingvideo_recentlyviewitems';
  protected $_rowClass = 'Sescrowdfundingvideo_Model_Recentlyviewitem';
  public function getitem($params = array()) {

      $itemTable = Engine_Api::_()->getItemTable('crowdfundingvideo');
      $itemTableName = $itemTable->info('name');
      $fieldName = 'crowdfundingvideo_id';
      $not = true;
    ...
    $select = $this->select()
            ...
            ->from(array('engine4_sescrowdfundingvideo_recentlyviewitems' => $subquery))
            ->where($this->info('name') .'.resource_type = ?', $params['type'])
            ->order('engine4_sescrowdfundingvideo_recentlyviewitems.creation_date DESC')
    ...
    $select->joinLeft($itemTableName, $itemTableName . ".$fieldName =  " . $this->info('name') . '.resource_id', array('*'));
		if (isset($not)) {
				...
				$select = $select->joinLeft($watchLaterTable, '(' . $watchLaterTable . '.crowdfundingvideo_id = ' . $this->info('name') . '.resource_id AND ' . $watchLaterTable . '.owner_id = ' . $user_id . ')', array('watchlater_id'));
			}
    }
