 ...
 * @version    $Id: IndexController.php 2018-07-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sescrowdfundingvideo_IndexController extends Core_Controller_Action_Standard {
  ...
  public function searchAction() {
    ...
    $sescrowdfundingvideo_commonsearch = $this->_getParam('sescrowdfundingvideo_commonsearch', 'video');
    if ($sescrowdfundingvideo_commonsearch && $actonType == 'browse') {
      $type = $sescrowdfundingvideo_commonsearch;
    } else {
      if (isset($_COOKIE['sescrowdfundingvideo_commonsearch']))
        $type = $_COOKIE['sescrowdfundingvideo_commonsearch'];
      else
        $type = 'video';
    }
    $table = Engine_Api::_()->getDbTable('videos', 'sescrowdfundingvideo');
    $tableName = $table->info('name');
    $id = 'crowdfundingvideo_id';
  ...
  public function editDetailAction() {
    ...
    $status = true;
    $error = false;
    $crowdfundingvideo_id = $this->_getParam('crowdfundingvideo_id', false);
    $video = Engine_Api::_()->getItem('crowdfundingvideo', $crowdfundingvideo_id);
    if (!$this->_helper->requireAuth()->setAuthParams(null, null, 'edit')->isValid() || !$crowdfundingvideo_id || !$video) {
      $status = false;
      $error = true;
    }
    ...
    if ($status && !$error) {
      ...
      if ($_POST['lat'] != '' && $_POST['lng'] != '') {
        ...
        $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $_POST['crowdfundingvideo_id'] . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","crowdfundingvideo")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }
  ...
  public function locationAction() {
    ...
    $this->view->crowdfundingvideo_id = $crowdfundingvideo_id = $this->_getParam('crowdfundingvideo_id');
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->video = $video = Engine_Api::_()->getItem('crowdfundingvideo', $crowdfundingvideo_id);
    if (!$video)
      return;
    $this->view->form = $form = new Sescrowdfundingvideo_Form_Location();
    ...
    if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
      $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
      $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $crowdfundingvideo_id . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","crowdfundingvideo")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
    }
    $db = $video->getTable()->getAdapter();
    $db->beginTransaction();
    try {
  ...
  public function imageviewerdetailAction() {
    $this->view->crowdfundingvideo_id = $crowdfundingvideo_id = $this->getRequest()->getParam('crowdfundingvideo_id', '0');
    ...
    $this->view->user = $user = Engine_Api::_()->getItem('user', $user_id);
		...
		if (!is_file($privateImageURL))
			$privateImageURL = 'application/modules/Sescrowdfundingvideo/externals/images/private-video.jpg';
    ...
    if ($viewer->getIdentity() != 0 && isset($video->crowdfundingvideo_id)) {
      $dbObject = Engine_Db_Table::getDefaultAdapter();
      $dbObject->query('INSERT INTO engine4_sescrowdfundingvideo_recentlyviewitems (resource_id, resource_type,owner_id,creation_date ) VALUES ("' . $video->crowdfundingvideo_id . '", "crowdfundingvideo","' . $viewer->getIdentity() . '",NOW())	ON DUPLICATE KEY UPDATE	creation_date = NOW()');
    }
		...
		$this->view->customParamsArray = $customParamsArray;
		 // get next video URL
    $this->view->nextVideo = Engine_Api::_()->getDbTable('videos', 'sescrowdfundingvideo')->videoLightBox($video, '>','','',$type,$this->_getParam('item_id',''));
    // get previous video URL
    $this->view->previousVideo = Engine_Api::_()->getDbTable('videos', 'sescrowdfundingvideo')->videoLightBox($video, '<','','',$type,$this->_getParam('item_id',''));
    ...
    if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('seslock'))) {
      ...
      if ($viewer->getIdentity() == 0)
        ...
        $level = $viewer;
      if (!Engine_Api::_()->authorization()->getPermission($level, 'crowdfundingvideo', 'locked') && $video->is_locked) {
      ...
      $this->view->locked = false;
    // rating code
    $this->view->allowShowRating = $allowShowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.ratevideo.show', 1);
    $this->view->allowRating = $allowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.video.rating', 1);
    ...
    if ($showRating != 0) {
      $this->view->canRate = $canRate = Engine_Api::_()->authorization()->isAllowed('crowdfundingvideo', $viewer, 'rating');
      $this->view->allowRateAgain = $allowRateAgain = Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.ratevideo.again', 1);
      $this->view->allowRateOwn = $allowRateOwn = Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.ratevideo.own', 1);
      ...
      $this->view->rating_type = $rating_type = 'crowdfundingvideo';
      $this->view->rating_count = $rating_count = Engine_Api::_()->getDbTable('ratings', 'sescrowdfundingvideo')->ratingCount($video->getIdentity(), $rating_type);
      $this->view->rated = $rated = Engine_Api::_()->getDbTable('ratings', 'sescrowdfundingvideo')->checkRated($video->getIdentity(), $viewer->getIdentity(), $rating_type);
      $rating_sum = Engine_Api::_()->getDbTable('ratings', 'sescrowdfundingvideo')->getSumRating($video->getIdentity(), $rating_type);
      if ($rating_count != 0)
  ...
  public function getVideoAction() {
    ...
    $value['search'] = 1;
    $videos = Engine_Api::_()->getDbtable('videos', 'sescrowdfundingvideo')->getVideo($value);
    foreach ($videos as $video) {
      $video_icon = $this->view->itemPhoto($video, 'thumb.icon');
      $sesdata[] = array(
          'id' => $video->crowdfundingvideo_id,
          'crowdfundingvideo_id' => $video->crowdfundingvideo_id,
          'label' => $video->title,
          'photo' => $video_icon
      );
    }
  ...
  function likeAction() {
    ...
    $type = 'crowdfundingvideo';
    $dbTable = 'videos';
    $resorces_id = 'crowdfundingvideo_id';
    ...
    $itemTable = Engine_Api::_()->getDbtable($dbTable, 'sescrowdfundingvideo');
  ...
  function favouriteAction() {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $Fav = Engine_Api::_()->getDbTable('favourites', 'sescrowdfundingvideo')->getItemfav($type, $item_id);
    $favItem = Engine_Api::_()->getDbtable($dbTable, 'sescrowdfundingvideo');
    if (count($Fav) > 0) {
      //delete
      ...
      //update
      $db = Engine_Api::_()->getDbTable('favourites', 'sescrowdfundingvideo')->getAdapter();
      $db->beginTransaction();
      try {
        $fav = Engine_Api::_()->getDbTable('favourites', 'sescrowdfundingvideo')->createRow();
      ...
      //send notification and activity feed work.
      $item = Engine_Api::_()->getItem(@$type, @$item_id);
      if ($this->_getParam('type') != 'sescrowdfundingvideo_artist') {
        $subject = $item;
        $owner = $subject->getOwner();
        if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
          $activityTable = Engine_Api::_()->getDbtable('actions', 'activity');
  ...
  public function allVideosAction() {
    ...
    //FETCH photos
		...
		$this->view->customParamsArray = $customParamsArray;
    $paginator = $this->view->allVideos = Engine_Api::_()->getDbTable('videos', 'sescrowdfundingvideo')->videoLightBox($video, '', true, true,$type,$this->_getParam('item_id',''));
  ...
  public function viewAction() {
    $video = Engine_Api::_()->core()->getSubject('crowdfundingvideo');
    $viewer = Engine_Api::_()->user()->getViewer();
    if( !$this->_helper->requireAuth()->setAuthParams('crowdfundingvideo', null, 'view')->isValid() ) return;
    // Render
    $this->_helper->content->setEnabled();
  }
  ...
  public function rateAction() {
    ...
    $table = Engine_Api::_()->getDbtable('ratings', 'sescrowdfundingvideo');
    $db = $table->getAdapter();
    $db->beginTransaction();
    try {
      Engine_Api::_()->getDbtable('ratings', 'sescrowdfundingvideo')->setRating($resource_id, $user_id, $rating, $resource_type);

      if ($resource_type && $resource_type == 'crowdfundingvideo')
        $item = Engine_Api::_()->getItem('crowdfundingvideo', $resource_id);

      $item->rating = Engine_Api::_()->getDbtable('ratings', 'sescrowdfundingvideo')->getRating($item->getIdentity(), $resource_type);
      $item->save();
      if ($resource_type == 'crowdfundingvideo') {
    ...
    $total = Engine_Api::_()->getDbtable('ratings', 'sescrowdfundingvideo')->ratingCount($item->getIdentity(), $resource_type);
    $rating_sum = Engine_Api::_()->getDbtable('ratings', 'sescrowdfundingvideo')->getSumRating($item->getIdentity(), $resource_type);
    $data = array();
  ...
  public function shareAction() {
    ...
    $typeItem = ucwords(str_replace(array('sescrowdfundingvideo_'), '', $attachment->getType()));
  ...
  public function importVideoFromYoutubePlaylist($playlistId = null, $leftvideos, $values, $form, $limitYoutubePlaylist, $googleApiKey,$approve) {
    ...
    $videoIds = array();
    $counter = 1;
    $key = Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.youtube.apikey');
  ...
  public function uploadYoutubePlaylistVideos($values, $form, $leftVideos, $data, $limitYoutubePlaylist,$approve) {
    ...
    $values['parent_type'] = $parent_type = 'crowdfunding';
    $db = Engine_Api::_()->getDbtable('videos', 'sescrowdfundingvideo')->getAdapter();
    $db->beginTransaction();
    try {
      //Create video
      $table = Engine_Api::_()->getDbtable('videos', 'sescrowdfundingvideo');
      ...
      foreach ($data['items'] as $videoId) {
        ...
        if ($valid_thumb && $thumbnail && $ext && $thumbnail_parsed && in_array($ext, array('jpg', 'jpeg', 'gif', 'png'))) {
          ...
          $video->save();
            ...
            $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
            $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $video->crowdfundingvideo_id . '", "' . $values['lat'] . '","' . $values['lng'] . '","crowdfundingvideo")	ON DUPLICATE KEY UPDATE	lat = "' . $values['lat'] . '" , lng = "' . $values['lng'] . '"');
          }
  ...
  public function createAction() {
    ...
    $parent_type = $parent_type = 'crowdfunding';
    ...
    $paginator = Engine_Api::_()->getApi('core', 'sescrowdfundingvideo')->getVideosPaginator($values);

    $this->view->quota = $quota = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'crowdfundingvideo', 'max');
    ...
    //Create form
    $this->view->form = $form = new Sescrowdfundingvideo_Form_Video();
    ...
    try {
			$viewer = Engine_Api::_()->user()->getViewer();
			$isApproveUploadOption = Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('crowdfundingvideo', $viewer, 'video_approve');
			$approveUploadOption = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('crowdfundingvideo', $viewer, 'video_approve_type');
			$approve = 1;
      ...
      $table = Engine_Api::_()->getDbtable('videos', 'sescrowdfundingvideo');
        ...
        $video = Engine_Api::_()->getItem('crowdfundingvideo', $this->_getParam('id'));
      } else
        $video = $table->createRow();
      if ($values['type'] == 3 && isset($_FILES['photo_id']['name']) && $_FILES['photo_id']['name'] != '') {
        $values['photo_id'] = $this->setPhoto($form->photo_id, $video->crowdfundingvideo_id, true);
      }
        ...
        if(isset($_FILES['photo_id']['name']) && $_FILES['photo_id']['name'] != '' && $values['type'] != 3 ){
            $video->photo_id = $this->setPhoto($form->photo_id, $video->crowdfundingvideo_id, true);
            $video->save();
        ...
        if($values['type'] == 'iframely') {
            ...
            $insert_action = true;
				...
				if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
            $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
            $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $video->crowdfundingvideo_id . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","crowdfundingvideo")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
          }
    ...
    try {
      if ($approve) {
        ...
        if($parent_id && $parent_type) {
		      $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($owner, $parentItem, 'sescrowdfunding_page_editeventvideo');
	        if ($action != null) {
	          Engine_Api::_()->getDbtable('actions', 'activity')->attachActivity($action, $video);
	        }
        } else {
    ...
    $widgetId = Engine_Api::_()->sescrowdfunding()->getIdentityWidget('sescrowdfundingvideo.profile-videos','widget','sescrowdfunding_index_view');
    if($widgetId)
  ...
  protected function setPhoto($photo, $id) {
    ...
    try {
      ...
      if ($e->getCode() == Storage_Model_DbTable_Files::SPACE_LIMIT_REACHED_CODE) {
        throw new Sescrowdfundingvideo_Model_Exception($e->getMessage(), $e->getCode());
      } else {
        throw $e;
      }
    }
  ...
  public function uploadVideoAction() {
    ...
    try {
      ...
      $video = Engine_Api::_()->sescrowdfundingvideo()->createVideo($params, $_FILES['Filedata'], $values);
      $this->view->status = true;
      $this->view->name = $_FILES['Filedata']['name'];
      $this->view->code = $video->code;
      $this->view->crowdfundingvideo_id = $video->crowdfundingvideo_id;
      ...
      $video->save();
      $db->commit();
      return $video->crowdfundingvideo_id;
    } catch (Exception $e) {
      $db->rollBack();
      $this->view->status = false;
  ...
  public function deleteAction() {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $video = Engine_Api::_()->getItem('crowdfundingvideo', $this->getRequest()->getParam('crowdfundingvideo_id'));
    $resourceItem = Engine_Api::_()->getItem('crowdfunding', $video->parent_id);
		if(!$resourceItem)
			return;
    ...
    return $this->_forward('success', 'utility', 'core', array(
                'parentRedirect' => $resourceItem->getHref(),
  ...
  public function editAction() {
    ...
    if (!$this->_helper->requireUser()->isValid())
      return;
    $viewer = Engine_Api::_()->user()->getViewer();
    $video = Engine_Api::_()->getItem('crowdfundingvideo', $this->_getParam('crowdfundingvideo_id'));

    // Render
    $this->_helper->content->setEnabled();

    $this->view->parentItem = $resourceItem = Engine_Api::_()->getItem('crowdfunding', $video->parent_id);
    ...
    $this->view->form = $form = new Sescrowdfundingvideo_Form_Edit();

		$latLng = Engine_Api::_()->getDbTable('locations', 'sesbasic')->getLocationData('crowdfundingvideo',$video->crowdfundingvideo_id);
		if($latLng){
    ...
    try {
      ...
      if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
        $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
        $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $this->_getParam('crowdfundingvideo_id') . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","crowdfundingvideo")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }
      $video->setFromArray($values);
      $video->save();
      // Add fields
    ...
    return $this->_helper->redirector->gotoRoute(array('user_id' => $viewer->getIdentity(), 'crowdfundingvideo_id' => $video->getIdentity(), 'slug' => $video->getSlug()), 'sescrowdfundingvideo_view', true);
  }
  public function lastElementDataAction() {
		$this->view->type = $this->_getParam('type','crowdfundingvideo');
...
//     }
    ...
    $values['user_id'] = $viewer->getIdentity();
    $paginator = Engine_Api::_()->getApi('core', 'sescrowdfundingvideo')->getVideosPaginator($values);
    $quota = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'crowdfundingvideo', 'max');
    ...
    if (($current_count >= $quota) && !empty($quota)) {
      ...
      try {
        ...
        $video->save();
        // Now try to create thumbnail
        $thumbnail = $thumbnail = $information['thumbnail']; //$this->handleThumbnail($video->type, $video->code);
...
//     $this->view->valid = $valid;
  ...
  public function checkYouTubePlaylist($code) {
    ...
    require_once 'Google/Service/YouTube.php';
    $client = new Google_Client();
    $client->setDeveloperKey(Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.youtube.apikey', 0));
    $youtube = new Google_Service_YouTube($client);
    $nextPageToken = '';
    $playlistItemsResponse = $youtube->playlistItems->listPlaylistItems('snippet', array(
        'playlistId' => $code,
  ...
  public function favouriteItemAction() {
    ...
    if ($item_type == 'video'){
      ...
      $paginator = Engine_Api::_()->getDbTable('videos', 'sescrowdfundingvideo')->getFavourite($param);
		}
    else
      $paginator = Engine_Api::_()->getDbTable('chanels', 'sescrowdfundingvideo')->getFavourite($param);
  ...
  public function likeItemAction() {
    ...
    $paginator = Engine_Api::_()->sescrowdfundingvideo()->likeItemCore($param);
    $this->view->item_id = $item_id;
    $this->view->paginator = $paginator;
    // Set item count per page and current page number
    $paginator->setItemCountPerPage(10);
