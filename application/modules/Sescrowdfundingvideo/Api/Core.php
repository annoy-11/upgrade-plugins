 ...
 * @version    $Id: Core.php 2018-07-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sescrowdfundingvideo_Api_Core extends Core_Api_Abstract {

  public function getRow($params = array()) {

    $table = Engine_Api::_()->getDbtable($params['table_name'], 'sescrowdfundingvideo');
  ...
  public function nextPhoto($photo = '', $params = array()) {
    return Engine_Api::_()->getDbTable('chanelphotos', 'sescrowdfundingvideo')->getPhotoCustom($photo, $params, '>');
  }
  ...
  public function getLikeStatusVideo($crowdfundingvideo_id = '', $moduleName = '') {
    if ($moduleName == '')
      $moduleName = 'crowdfundingvideo';
    if ($crowdfundingvideo_id != '') {
      $userId = Engine_Api::_()->user()->getViewer()->getIdentity();
      if ($userId == 0)
        return false;
      $coreLikeTable = Engine_Api::_()->getDbtable('likes', 'core');
      $total_likes = $coreLikeTable->select()->from($coreLikeTable->info('name'), new Zend_Db_Expr('COUNT(like_id) as like_count'))->where('resource_type =?', $moduleName)->where('poster_id =?', $userId)->where('poster_type =?', 'user')->where('	resource_id =?', $crowdfundingvideo_id)->limit(1)->query()->fetchColumn();
      if ($total_likes > 0) {
        return true;
      } else {
        return false;
  ...
  public function getSlug($str = null, $maxstrlen = 245) {
	...
	function getCustomLightboxHref($item,$params=array()){
		...
		$params = array_merge(array(
          'route' => 'sescrowdfundingvideo_lightbox',
          'controller' => 'index',
  ...
  public function addWatchlaterVideo($params = array()) {
    ...
    if (count($params['crowdfundingvideo_id'])) {
      $watchLater = Engine_Api::_()->getDbtable('watchlaters', 'sescrowdfundingvideo')->createRow();
      $watchLater->crowdfundingvideo_id = $params['crowdfundingvideo_id'];
  ...
  public function getWatchLaterId($crowdfundingvideo_id = false) {
    if ($crowdfundingvideo_id) {
      $user_id = Engine_Api::_()->user()->getViewer()->getIdentity();
      $watchlater = Engine_Api::_()->getDbtable('watchlaters', 'sescrowdfundingvideo');
      $select = $watchlater->select('watchlater_id')
              ->where('crowdfundingvideo_id = ?', $crowdfundingvideo_id)
  ...
  public function deleteWatchlaterVideo($params = array()) {
    ...
    if (count($params['crowdfundingvideo_id'])) {
      ...
      $object = $dbObject->query('INSERT INTO engine4_sescrowdfundingvideo_watchlaters (crowdfundingvideo_id, owner_id,modified_date,creation_date ) VALUES ("' . $params['crowdfundingvideo_id'] . '", "' . $user_id . '",NOW(),NOW())	ON DUPLICATE KEY UPDATE	modified_date = modified_date');
      if ($object->rowCount() > 0) {
        return array('status' => 'insert', 'error' => false);
      } else {
        $watchlaterid = $this->getWatchLaterId($params['crowdfundingvideo_id']);
  ...
  public function getVideosSelect($params = array()) {
    ...
    if (!empty($params['text'])) {
      ...
      $select
              ->joinRight($sName, $sName . '.id=' . $rName . '.crowdfundingvideo_id', null)
              ->where($sName . '.type = ?', 'crowdfundingvideo')
    ...
    if (!empty($params['tag'])) {
      $select
              ...
              // ->from($rName)
              ->joinLeft($tmName, "$tmName.resource_id = $rName.crowdfundingvideo_id", NULL)
  ...
  public function createVideo($params, $file, $values,$video_date = false) {
    if ($file instanceof Storage_Model_File) {
      ...
      // create video item
			if(!$video_date){
      	$video = Engine_Api::_()->getDbtable('videos', 'sescrowdfundingvideo')->createRow();
      ...
      $video->save();
			if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.direct.video', 0)){
			...
			/* this giving error so commented */
			$video->status = 1;
			$video->save();
      // Add to jobs
      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.html5', false)) {
        Engine_Api::_()->getDbtable('jobs', 'core')->addJob('sescrowdfundingvideo_encode', array(
            'crowdfundingvideo_id' => $video->getIdentity(),
            'type' => 'mp4',
        ));
      } else {
        Engine_Api::_()->getDbtable('jobs', 'core')->addJob('sescrowdfundingvideo_encode', array(
	...
	public function getVideoThumbnail($video,$thumb_splice,$file = false){
		$tmpDir = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'temporary' . DIRECTORY_SEPARATOR . 'crowdfundingvideo';
  ...
  public function deleteVideo($video) {
    ...
    Engine_Api::_()->getDbtable('ratings', 'sescrowdfundingvideo')->delete(array(
        'resource_id = ?' => $video->crowdfundingvideo_id,
    ));

    Engine_Api::_()->getDbtable('favourites', 'sescrowdfundingvideo')->delete(array(
        'resource_id = ?' => $video->crowdfundingvideo_id,
    ));

    Engine_Api::_()->getDbtable('recentlyviewitems', 'sescrowdfundingvideo')->delete(array(
        'resource_id = ?' => $video->crowdfundingvideo_id,
    ));

    Engine_Api::_()->getDbtable('watchlaters', 'sescrowdfundingvideo')->delete(array(
    ...
    $item = Engine_Api::_()->getItem('crowdfundingvideo', $video->crowdfundingvideo_id);
    if ($item) {
      $item->delete();
    }
  }
