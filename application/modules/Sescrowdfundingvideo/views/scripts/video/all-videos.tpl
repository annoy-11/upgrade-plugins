 ...
 * @version    $Id: all-videos.tpl 2018-07-04 00:00:00 SocialEngineSolutions $
...
<div class="ses_ml_photos_panel_wrapper sesbasic_clearfix sesbasic_bxs" id="ses_ml_photos_panel_wrapper">
  ...
  <div class="ses_ml_photos_panel_content" style="height:200px;">
      ...
      <div class="ses_media_lightbox_all_photo" id="ses_media_lightbox_all_crowdfundingvideo_id">
<?php } ?>
       <?php foreach($this->allVideos as $valueVideo){ 
       		  if (!$valueVideo instanceof Sescrowdfundingvideo_Model_Video)
            	$valueVideo = Engine_Api::_()->getItem('crowdfundingvideo',$valueVideo->crowdfundingvideo_id);
       ?>
          <?php $imageURL = $valueVideo->getHref($this->customParamsArray); ?>
          <a id="video-lightbox-id-<?php echo $valueVideo->crowdfundingvideo_id; ?>" href="<?php echo $valueVideo->getHref($this->customParamsArray); ?>" class="ses-video-viewer ses_ml_photos_panel_photo_thumb sescrowdfundingvideo_thumb_img">
...
function <?php echo $randonNumber; ?> (){
    (new Request.HTML({
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        document.getElementById('ses_media_lightbox_all_crowdfundingvideo_id').innerHTML = document.getElementById('ses_media_lightbox_all_crowdfundingvideo_id').innerHTML + responseHTML;
