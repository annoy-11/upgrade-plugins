 ...
 * @version    $Id: image-viewer-details-advance.tpl 2018-07-04 00:00:00 SocialEngineSolutions $
...
?>
	<script type="application/javascript">
     function flashembedAttach(){
      flashembed("sesbasic_lightbox_content", {
        src: "<?php echo $this->layout()->staticBaseUrl. $flowplayer; ?>",
        width: 480,
        height: 386,
        wmode: 'transparent',
				onFail: function()  {
							document.getElementById("sesbasic_lightbox_content").innerHTML =
...
<script type="text/javascript">
  en4.core.runonce.add(function() {
    ...
    var rated_viewer = '90';
		<?php } ?>
    var resource_id_viewer = <?php echo $this->video->crowdfundingvideo_id;?>;
    ...
    <div id="flexcroll" >
      <div class="ses_pswp_info" id="ses_pswp_info">
        ...
        <div class="ses_pswp_location"> <span class="sesbasic_text_light" id="seslocationIn">
          ...
          <span>
            <a id="ses_location_data" href="javascript:;" onclick="openURLinSmoothBox('<?php echo $this->url(array("module"=> "sescrowdfundingvideo", "controller" => "index", "action" => "location",  "crowdfundingvideo_id" => $this->video->getIdentity(),'type'=>'video_location'),'default',true); ?>');return false;"><?php echo $this->video->location; ?></a>
         </span>
        </div>
        <?php } ?>
        <?php if(($this->getAllowRating == 0 && $this->allowShowRating == 1 && $this->total_rating_average != 0 ) || ($this->getAllowRating == 1) ){ ?>
      ...
      <div class="ses_pswp_item_edit_form" id="editDetailsFormVideo" style="display:none;">
        <form id="changePhotoDetailsVideo">
          ...
          <input type="hidden" id="crowdfundingvideo_id_ses" name="photo_id" value="<?php echo $this->video->crowdfundingvideo_id; ?>" />
      ...
      <?php echo $this->action("list", "comment", "sesadvancedcomment", array("type" => "crowdfundingvideo", "id" => $this->video->getIdentity())); 
        }else{ echo $this->action("list", "comment", "core", array("type" => "crowdfundingvideo", "id" => $this->video->getIdentity())); } ?>
      </div>
    </div>
  </div>
</div>
...
<div class="pswp__top-bar" style="display:none" id="imageViewerId"> 
  ...
  <div class="pswp__top-bar-action">
    ...
    <div class="pswp__top-bar-share">
      <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0 && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbasic.add.share',1) == 1){ ?>
       <a href="javascript:;" class='smoothbox' onclick="openURLinSmoothBox('<?php echo $this->url(array("action" => "share","module" =>"sescrowdfundingvideo", "type" => "crowdfundingvideo", "id" => $this->video->getIdentity(),"format" => "smoothbox"), 'default', true); ?>')"><?php echo $this->translate('Share'); ?></a>
      <?php } ?>
    </div>
    ...
    <div class="pswp__top-bar-more"> 
      ...
      <div class="pswp__top-bar-more-tooltip" style="display:none">
        ...
        <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbasic.add.delete',1) == 1){ ?>
         <a href="javascript:;" class='smoothbox' onclick="openURLinSmoothBox('<?php echo $this->url(array('module' => 'sescrowdfundingvideo', 'action' => 'delete', 'crowdfundingvideo_id' => $this->video->getIdentity()), 'default', true); ?>')"><?php echo $this->translate('Delete'); ?></a>
        <?php } ?>
        <?php } ?>
        <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbasic.add.report',1) == 1 && Engine_Api::_()->user()->getViewer()->getIdentity() != 0){ ?>
    ...
    <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0){ ?>
      ...
      <a class="sesbasic_icon_btn sesalbum_icon_msg_btn smoothbox" href="<?php echo $this->url(array('module'=> 'sesbasic', 'controller' => 'index', 'action' => 'message','item_id' => $this->video->getIdentity(), 'type'=>$this->video->getType()),'default',true); ?>" onclick="openURLinSmoothBox(this.href);return false;"><i class="fa fa-envelope"></i></a>     
       <?php if($this->canComment){ ?>
        <?php $LikeStatus = Engine_Api::_()->sescrowdfundingvideo()->getLikeStatusVideo($this->video->crowdfundingvideo_id,'crowdfundingvideo');  ?>
        <a href="javascript:void(0);" id="sesLightboxLikeUnlikeButtonVideo" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn sesbasic_like_sesbasic_video<?php echo $LikeStatus === true ? ' button_active' : '' ;  ?>"><i class="fa fa-thumbs-up"></i><span id="like_unlike_count"><?php echo $this->video->like_count; ?></span></a>
        <?php } ?>
        <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo.enable.favourite', 1)) { ?>
          <?php $favStatus = Engine_Api::_()->getDbtable('favourites', 'sescrowdfundingvideo')->isFavourite(array('resource_type'=>'crowdfundingvideo','resource_id'=>$this->video->crowdfundingvideo_id)); ?>
          <a href="javascript:;" id="sesJqueryObject_favourite" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_fav_btn sesbasic_favourite_sesbasic_video<?php echo ($favStatus)  ? ' button_active' : '' ?>"  data-url="<?php echo $this->video->crowdfundingvideo_id; ?>"><i class="fa fa-heart"></i><span><?php echo $this->video->favourite_count; ?></span></a>
        <?php } ?>
      <?php } ?>
      </div>
  </div>
  <?php $className= '';
                if($this->locked){
                   $imageUrl = 'application/modules/Sescrowdfundingvideo/externals/images/locked-video.jpg';
 ...
 <div id="sesbasic_crowdfundingvideo_id_data_src" data-src="<?php echo $this->video->crowdfundingvideo_id; ?>" style="display:none;"></div>
