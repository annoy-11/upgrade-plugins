 ...
 * @version    $Id: view.tpl 2018-07-04 00:00:00 SocialEngineSolutions $
...
<div class="sesbasic_view_stats_popup">
  ...
  <table>
    ...
    <td><?php if(!is_null($this->item->getTitle()) && $this->item->getTitle() != '') {
  	...
  	<tr>
    	<td><?php echo $this->translate("Event"); ?></td>
    	<td>
    		<?php
        	$event = Engine_Api::_()->getItem('crowdfunding',$this->item->parent_id); ?>
   ...
   <?php } ?>
   <?php if($this->type != 'crowdfundingvideo'){ ?>
    <tr>
   ...
   <?php if(isset($this->item->ip_address)){ ?>
    ...
    <?php } ?>
    <?php if(isset($this->item->location) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sescrowdfundingvideo_enable_location', 1)){ ?>
    <tr>
