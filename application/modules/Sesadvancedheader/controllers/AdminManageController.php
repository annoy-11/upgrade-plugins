...
class Sesadvancedheader_AdminManageController extends Core_Controller_Action_Admin {
  ...
  public function miniMenuIconsAction() {
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesadvancedheader_admin_main', array(), 'sesadvancedheader_admin_main_menus');
    $this->view->form = $form = new Sesadvancedheader_Form_Admin_MiniMenuIcons();
  ...
  public function headerSettingsAction() {
    ...
    $this->view->form = $form = new Sesadvancedheader_Form_Admin_HeaderSettings();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      unset($values['minimenu_message_icons']);
      unset($values['minimenu_frrequest_icons']);
			unset($values['sesadvancedheader_noadcampaignid']);
      foreach ($values as $key => $value) {
        if ($key == 'sesadvheader_design' || $key == 'sesadvancedheader_header_textcolor') {
					if($key == 'sesadvancedheader_header_textcolor')
					 $value = '#'.$value;
	        Engine_Api::_()->sesadvancedheader()->readWriteXML($key, $value);
		 			Engine_Api::_()->getApi('settings', 'core')->setSetting($key, $value);
        } else {
          if ($key == 'sesadvancedheader_header_loggedin_options' || $key == 'sesadvancedheader_header_nonloggedin_options' || $key == 'sesadvancedheader_header_transparent') {
            if(Engine_Api::_()->getApi('settings', 'core')->hasSetting($key)){
              Engine_Api::_()->getApi('settings', 'core')->removeSetting($key);
            }
          }
					if($key == 'sesadvancedheader_menuinformation_img') {
						if(empty($value)) {
							$value = 'public/admin/blank.png';
            }
					}
					if($key == 'sesadvancedheader_menu_img') {
						if(empty($value))
							$value = 'public/admin/blank.png';
					}
					//Notification Icon
  ...
  public function setPhoto($photo) {
    ...
    $photo_params = array(
        'parent_id' => 1,
        'parent_type' => "sesadvancedheader_minimenu_icons",
    );
  ...
  public function uploadPhotoAction() {
    ...
    $admin_file = APPLICATION_PATH . '/public/adminsesadvancedheader';
    ...
    $this->view->photo_url = 'http://' . $_SERVER['HTTP_HOST'] . '/' . Zend_Controller_Front::getInstance()->getBaseUrl() . '/public/adminsesadvancedheader/' . $info['name'];
  }
  ...
  public function uploadIconAction() {
    ...
    $this->view->form = new Sesadvancedheader_Form_Admin_Icon();
    ...
    if ($this->getRequest()->isPost()) {
      ...
      if ($menuType == 'main')
        $redirectUrl = Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'sesadvancedheader', 'controller' => 'admin-manage', 'action' => 'index'), 'default', true);
      else
        $redirectUrl = Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'sesadvancedheader', 'controller' => 'admin-manage', 'action' => 'footer-menu'), 'default', true);
  ...
  public function deletePhotoAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $mainPhoto->delete();
        $slideImage = Engine_Api::_()->getItem('sesadvancedheader_slideimage', $id);
        $slideImage->delete();
        $db->commit();
      } catch (Exception $e) {
        $db->rollBack();
