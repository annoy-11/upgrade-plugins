...
<?php if($this->search_for == 'video'){ ?>
  <script type="application/javascript">var Searchurl = "<?php echo $this->url(array('module' =>'sespagevideo','controller' => 'index', 'action' => 'get-video'),'default',true); ?>";</script>
  <?php if($controllerName == 'index' && $actionName != 'locations'){ ?>

    <?php if($actionName == 'browse'): ?>
      <?php $identity = Engine_Api::_()->sespage()->getIdentityWidget('sespagevideo.browse-video','widget','sespagevideo_index_browse'); ?>
    <?php endif; ?>
    ...
    <?php if(@$identity) { ?>
      ...
      sesJqueryObject(document).ready(function(){
        sesJqueryObject('#filter_form').submit(function(e){
          e.preventDefault();
          if(sesJqueryObject('.sespagevideo_video_listing').length > 0){
            ...
            sesJqueryObject('#loadingimgsespagevideo-wrapper').show();
