...
class Sespagevideo_AdminSettingsController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespagevideo_admin_main', array(), 'sespagevideo_admin_main_settings');


    // Check ffmpeg path for correctness
    if (function_exists('exec')) {
      $ffmpeg_path = Engine_Api::_()->getApi('settings', 'core')->sespagevideo_ffmpeg_path;
      ...
      if ($ffmpeg_path != $ffmpeg_path_original) {
        Engine_Api::_()->getApi('settings', 'core')->sespagevideo_ffmpeg_path = $ffmpeg_path;
      }
    }

    // Make form
    $this->view->form = $form = new Sespagevideo_Form_Admin_Settings_Global();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      if (!empty($values['sespagevideo_youtube_apikey']) && $values['sespagevideo_youtube_apikey'] != $oldYoutubeApikey) {
        $response = $this->verifyYotubeApiKey($values['sespagevideo_youtube_apikey']);
      ...
      // Check ffmpeg path
      if (!empty($values['sespagevideo_ffmpeg_path'])) {
        if (function_exists('exec')) {
          $ffmpeg_path = $values['sespagevideo_ffmpeg_path'];
          $output = null;
          $return = null;
          exec($ffmpeg_path . ' -version', $output, $return);

          if ($return > 0 && $output != NULL) {
            $form->sespagevideo_ffmpeg_path->addError('FFMPEG path is not valid or does not exist');
            $values['sespagevideo_ffmpeg_path'] = '';
          }
        } else {
          $form->sespagevideo_ffmpeg_path->addError('The exec() function is not available. The ffmpeg path has not been saved.');
          $values['sespagevideo_ffmpeg_path'] = '';
        }
      }

      include_once APPLICATION_PATH . "/application/modules/Sespagevideo/controllers/License.php";
      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagevideo.pluginactivated')) {
  ...
  public function levelAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespagevideo_admin_main', array(), 'sespagevideo_admin_main_level');
    ...
    // Make form
    $this->view->form = $form = new Sespagevideo_Form_Admin_Settings_Level(array(
    ...
    $form->populate($permissionsTable->getAllowed('pagevideo', $id, array_keys($form->getValues())));
    // Check post
    ...
    try {
      // Set permissions
      $permissionsTable->setAllowed('pagevideo', $id, $values);
      // Commit
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
  ...
  public function utilityAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sespagevideo_admin_main', array(), 'sespagevideo_admin_main_utility');
    $ffmpeg_path = Engine_Api::_()->getApi('settings', 'core')->sespagevideo_ffmpeg_path;
  ...
  //site statis for sespagevideo plugin
  public function statisticAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespage_admin_main', array(), 'sespage_admin_main_sespagevideo');

    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespagevideo_admin_main', array(), 'sespagevideo_admin_main_statistic');

    $videoTable = Engine_Api::_()->getDbtable('videos', 'sespagevideo');
  ...
  public function manageWidgetizePageAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespagevideo_admin_main', array(), 'sespagevideo_admin_main_managepages');

    $this->view->pagesArray = array('sespagevideo_index_home','sespagevideo_index_browse','sespagevideo_index_create','sespagevideo_index_edit','sespagevideo_index_view');
  }
}
