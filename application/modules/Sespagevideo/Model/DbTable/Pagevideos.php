...
class Sespagevideo_Model_DbTable_Pagevideos extends Engine_Db_Table {

  protected $_rowClass = "Sespagevideo_Model_Pagevideo";
  protected $_name = 'sespagevideo_videos';
  ...
  public function getWatchLaterStatus($video_id) {
    ...
    $watchLaterTable = Engine_Api::_()->getDbTable('watchlaters', 'sespagevideo')->info('name');
  ...
  public function getVideo($params = array(), $paginator = true) {
    ...
    $select = $select->joinLeft($pageTable, '(' . $pageTable . '.page_id = ' . $tableName . '.parent_id )', array('page_id'));
		$select->where($pageTable.'.page_id !=?','');
    ...
    if (isset($params['lat']) && isset($params['miles']) && $params['miles'] != 0 && isset($params['lng']) && $params['lat'] != '' && $params['lng'] != '' && ((isset($params['location']) && $params['location'] != '') || isset($params['locationWidget']))) {
      ...
      $origLon = $params['lng'];
      //$select = $select->setIntegrityCheck(false);
      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagevideo.search.type', 1) == 1) {
        $searchType = 3956;
      } else
        $searchType = 6371;
      $dist = $params['miles']; // This is the maximum distance (in miles) away from $origLat, $origLon in which to search
      $select->joinLeft($tableLocationName, $tableLocationName . '.resource_id = ' . $tableName . '.video_id AND ' . $tableLocationName . '.resource_type = "pagevideo" ', array('distance'=> new Zend_Db_Expr($searchType . " * 2 * ASIN(SQRT( POWER(SIN(($origLat - abs(lat))*pi()/180/2),2) + COS($origLat*pi()/180 )*COS(abs(lat)*pi()/180) *POWER(SIN(($origLon-lng)*pi()/180/2),2)))"), 'lat', 'lng'));
    ...
    if (!empty($params['parent_id']) && $params['parent_type'] == 'sespage_page') {
  ...
  public function peopleAlsoLiked($id = 0) {
    ...
    $select = $this->select()
            ...
            ->where($likesTableName . '.resource_type = ?', 'pagevideo')
            ->where('l2.resource_type = ?', 'pagevideo')
...
public function getFavourite($params = array()){
		$tableFav = Engine_Api::_()->getDbtable('favourites', 'sespagevideo');
