...
class Sespagevideo_Model_DbTable_Recentlyviewitems extends Engine_Db_Table {
  protected $_name = 'sespagevideo_recentlyviewitems';
  protected $_rowClass = 'Sespagevideo_Model_Recentlyviewitem';
  public function getitem($params = array()) {

      $itemTable = Engine_Api::_()->getItemTable('pagevideo');
    ...
    $select = $this->select()
            ...
            ->from(array('engine4_sespagevideo_recentlyviewitems' => $subquery))
            ->where($this->info('name') .'.resource_type = ?', $params['type'])
            ->order('engine4_sespagevideo_recentlyviewitems.creation_date DESC')
