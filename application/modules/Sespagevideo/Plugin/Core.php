...
class Sespagevideo_Plugin_Core {
  public function onStatistics($event) {
    $table = Engine_Api::_()->getDbTable('videos', 'sespagevideo');
    $select = new Zend_Db_Select($table->getAdapter());
    $select->from($table->info('name'), 'COUNT(*) AS count');
    $event->addResponse($select->query()->fetchColumn(0), 'pagevideo');
  }
...
EOF;
    $script .=
						"var videoURLsespagevideo = '" . Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagevideo.video.manifest', 'pagevideo') . "';
