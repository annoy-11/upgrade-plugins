...
class Sespagevideo_Plugin_Job_Encode extends Core_Plugin_Job_Abstract {
  ...
  protected function _execute() {
    ...
    if (!$video || !($video instanceof Sespagevideo_Model_Video)) {
  ...
  private function getFFMPEGPath() {
    ...
    if (!function_exists('shell_exec')) {
      throw new Sespagevideo_Model_Exception('Unable to execute shell commands using shell_exec(); the function is disabled.');
    }

    if (!function_exists('exec')) {
      throw new Sespagevideo_Model_Exception('Unable to execute shell commands using exec(); the function is disabled.');
    }

    // Make sure FFMPEG path is set
    $ffmpeg_path = Engine_Api::_()->getApi('settings', 'core')->sespagevideo_ffmpeg_path;
    if (!$ffmpeg_path) {
      throw new Sespagevideo_Model_Exception('Ffmpeg not configured');
    }
    ...
    if (!@file_exists($ffmpeg_path) || !@is_executable($ffmpeg_path)) {
      ...
      if ($return > 0) {
        throw new Sespagevideo_Model_Exception('Ffmpeg found, but is not executable');
      }
    }
  ...
  private function getTmpDir() {
    ...
    if (!is_dir($tmpDir) && !mkdir($tmpDir, 0777, true)) {
      throw new Sespagevideo_Model_Exception('Video temporary directory did not exist and could not be created.');
    }

    if (!is_writable($tmpDir)) {
      throw new Sespagevideo_Model_Exception('Video temporary directory is not writable.');
    }
  ...
  private function getVideo($video) {
    ...
    if (is_numeric($video)) {
      $video = Engine_Api::_()->getItem('pagevideo', $video);
    }

    if (!($video instanceof Sespagevideo_Model_Video)) {
      throw new Sespagevideo_Model_Exception('Argument was not a valid video');
    }
  ...
  private function getStorageObject($video) {
    ...
    if (!$storageObject) {
      throw new Sespagevideo_Model_Exception('Video storage file was missing');
    }
  ...
  private function getOriginalPath($storageObject) {
    ...
    if (!file_exists($originalPath)) {
      throw new Sespagevideo_Model_Exception('Could not pull to temporary file');
    }
    ...
    if ($video->rotation > 0) {
      ...
      if ($video->rotation == 180)
        ...
        if (empty($transpose[$video->rotation]))
          throw new Sespagevideo_Model_Exception('Invalid rotation value');
  ...
  private function notifyOwner($video, $owner) {
    ...
    Engine_Api::_()->getDbtable('notifications', 'activity')
            ->addNotification($owner, $owner, $video, 'sespgvido_pro_fail', array(
                ...
                'message_link' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'manage'), 'sespagevideo_general', true),
    ));
  ...
  protected function _process($video, $type, $compatibilityMode = false) {
    ...
    if (!$success) {
      ...
      throw new Sespagevideo_Model_Exception($exceptionMessage);
    }
    ...
    // Success
      ...
      try {
        ...
        // notify the owner
        Engine_Api::_()->getDbtable('notifications', 'activity')
                ->addNotification($owner, $owner, $video, 'sespagevideo_processed');
