...
class Sespagevideo_Api_Core extends Core_Api_Abstract {

  public function getRow($params = array()) {

    $table = Engine_Api::_()->getDbtable($params['table_name'], 'sespagevideo');
  ...
  public function nextPhoto($photo = '', $params = array()) {
    return Engine_Api::_()->getDbTable('chanelphotos', 'sespagevideo')->getPhotoCustom($photo, $params, '>');
  }
  ...
  public function addWatchlaterVideo($params = array()) {
    ...
    if (count($params['video_id'])) {
      $watchLater = Engine_Api::_()->getDbtable('watchlaters', 'sespagevideo')->createRow();
  ...
  public function getWatchLaterId($video_id = false) {
    if ($video_id) {
      $user_id = Engine_Api::_()->user()->getViewer()->getIdentity();
      $watchlater = Engine_Api::_()->getDbtable('watchlaters', 'sespagevideo');
  ...
  public function deleteWatchlaterVideo($params = array()) {
    ...
    if (count($params['video_id'])) {
      ...
      $object = $dbObject->query('INSERT INTO engine4_sespagevideo_watchlaters (video_id, owner_id,modified_date,creation_date ) VALUES ("' . $params['video_id'] . '", "' . $user_id . '",NOW(),NOW())	ON DUPLICATE KEY UPDATE	modified_date = modified_date');
  ...
  public function getVideosSelect($params = array()) {
    ...
    if (!empty($params['tag'])) {
      $select
              ...
              ->where($tmName . '.resource_type = ?', 'pagevideo')
  ...
  public function createVideo($params, $file, $values,$video_date = false) {
    if ($file instanceof Storage_Model_File) {
      ...
      // create video item
			if(!$video_date){
      	$video = Engine_Api::_()->getDbtable('videos', 'sespagevideo')->createRow();
      ...
      $video->save();
			if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagevideo.direct.video', 0)){
			...
			$video->save();
      // Add to jobs
      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagevideo.html5', false)) {
        Engine_Api::_()->getDbtable('jobs', 'core')->addJob('sespagevideo_encode', array(
	...
	public function getVideoThumbnail($video,$thumb_splice,$file = false){
		$tmpDir = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'temporary' . DIRECTORY_SEPARATOR . 'pagevideo';
  ...
  public function deleteVideo($video) {
    ...
    Engine_Api::_()->getDbtable('ratings', 'sespagevideo')->delete(array(
        'resource_id = ?' => $video->video_id,
    ));

    Engine_Api::_()->getDbtable('favourites', 'sespagevideo')->delete(array(
        'resource_id = ?' => $video->video_id,
    ));

    Engine_Api::_()->getDbtable('recentlyviewitems', 'sespagevideo')->delete(array(
        'resource_id = ?' => $video->video_id,
    ));

    Engine_Api::_()->getDbtable('watchlaters', 'sespagevideo')->delete(array(
    ...
    $item = Engine_Api::_()->getItem('pagevideo', $video->video_id);
    if ($item) {
      $item->delete();
    }
  }
