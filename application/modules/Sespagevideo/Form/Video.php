...
class Sespagevideo_Form_Video extends Engine_Form {
  ...
  public function init() {
    ...
    $setting = Engine_Api::_()->getApi('settings', 'core');
		...
		$allowedUploadOption = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('pagevideo', $viewer, 'sesvdeo_upld');
    ...
    //My Computer
    if ($myComputer) {
      $allowed_upload = 1;//Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('pagevideo', $user, 'upload');
    ...
    $viewOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('pagevideo', $user, 'auth_view');
    ...
    // Comment
    $commentOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('pagevideo', $user, 'auth_comment');
    ...
    if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock')) && Engine_Api::_()->authorization()->getPermission($viewer, 'pagevideo', 'video_locked')) {
    ...
    $uploadoption = $settings->getSetting('sespagevideo.uploadphoto', '0');
    if ($uploadoption == 1) {
      if (isset($video) && $video->photo_id) {
        ...
        if (isset($path) && !empty($path)) {
          $this->addElement('Image', 'cover_photo_preview sesbd', array(
              'src' => $path,
              'class' => 'sespagevideo_channel_thumb_preview sesbd',
          ));
