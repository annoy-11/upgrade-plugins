...
var autoPlayId,firstVideoSrc=false,isOnRegister = false,IsfinishVideoNext = false,clearIntvalSespageVideoSlideshow1,clearIntvalSespageVideoSlideshow2 ;
jQuery(document).ready(function($){
		...
		sliderNav.on('click', 'li', function(event){
			...
			if(!selectedItem.hasClass('selected')) {
				...
				if(slidesWrapper.find('li.selected').hasClass('cd-bg-video')){
					 ...
					 clearInterval(clearIntvalSespageVideoSlideshow1);
					 clearInterval(clearIntvalSespageVideoSlideshow1);	
				}
			}
		});
	...
	function checkVideo(hiddenSlide, container, n) {
		...
		if( visibleVideo.length > 0 ) { 
			clearInterval(autoPlayId);
			clearInterval(clearIntvalSespageVideoSlideshow2);
			clearInterval(clearIntvalSespageVideoSlideshow1);
			if(!Modernizr.touch){
