  ...
  <div id="heightOfImageViewerContent">
    <div id="flexcroll" >
      <div class="ses_pswp_info" id="ses_pswp_info">
        ...
        <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagevideo_enable_location', 1)) { ?>
        <div class="ses_pswp_location"> <span class="sesbasic_text_light" id="seslocationIn">
          ...
          <span>
            <a id="ses_location_data" href="javascript:;" onclick="openURLinSmoothBox('<?php echo $this->url(array("module"=> "sespagevideo", "controller" => "index", "action" => "location",  "video_id" => $this->video->getIdentity(),'type'=>'video_location'),'default',true); ?>');return false;"><?php echo $this->video->location; ?></a>
         </span>
        </div>
        <?php } ?>
        <?php if(($this->getAllowRating == 0 && $this->allowShowRating == 1 && $this->total_rating_average != 0 ) || ($this->getAllowRating == 1) ){ ?>
      ...
      <?php echo $this->action("list", "comment", "sesadvancedcomment", array("type" => "pagevideo", "id" => $this->video->getIdentity())); 
        }else{ echo $this->action("list", "comment", "core", array("type" => "pagevideo", "id" => $this->video->getIdentity())); } ?>
      </div>
    </div>
  </div>
</div>
...
<div class="pswp__top-bar" style="display:none" id="imageViewerId"> 
  ...
  <div class="pswp__top-bar-action">
    ...
    <div class="pswp__top-bar-share">
      <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0 && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbasic.add.share',1) == 1){ ?>
       <a href="javascript:;" class='smoothbox' onclick="openURLinSmoothBox('<?php echo $this->url(array("action" => "share","module" =>"sespagevideo", "type" => "pagevideo", "id" => $this->video->getIdentity(),"format" => "smoothbox"), 'default', true); ?>')"><?php echo $this->translate('Share'); ?></a>
      <?php } ?>
    </div>
    ...
    <div class="pswp__top-bar-more"> 
      ...
      <div class="pswp__top-bar-more-tooltip" style="display:none">
        ...
        <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbasic.add.delete',1) == 1){ ?>
         <a href="javascript:;" class='smoothbox' onclick="openURLinSmoothBox('<?php echo $this->url(array('module' => 'sespagevideo', 'action' => 'delete', 'video_id' => $this->video->getIdentity()), 'default', true); ?>')"><?php echo $this->translate('Delete'); ?></a>
        <?php } ?>
        <?php } ?>
    ...
    <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0){ ?>
      ...
      <a class="sesbasic_icon_btn sesalbum_icon_msg_btn smoothbox" href="<?php echo $this->url(array('module'=> 'sesbasic', 'controller' => 'index', 'action' => 'message','item_id' => $this->video->getIdentity(), 'type'=>$this->video->getType()),'default',true); ?>" onclick="openURLinSmoothBox(this.href);return false;"><i class="fa fa-envelope"></i></a>     
       <?php if($this->canComment){ ?>
        <?php $LikeStatus = Engine_Api::_()->sespagevideo()->getLikeStatusVideo($this->video->video_id,'pagevideo');  ?>
        ...
        <?php } ?>
        <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagevideo.enable.favourite', 1)) { ?>
          <?php $favStatus = Engine_Api::_()->getDbtable('favourites', 'sespagevideo')->isFavourite(array('resource_type'=>'pagevideo','resource_id'=>$this->video->video_id)); ?>
  ...
  <?php $className= '';
                if($this->locked){
                   $imageUrl = 'application/modules/Sespagevideo/externals/images/locked-video.jpg';
