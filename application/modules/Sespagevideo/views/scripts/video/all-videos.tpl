...
<?php } ?>
       <?php foreach($this->allVideos as $valueVideo){ 
       		  if (!$valueVideo instanceof Sespagevideo_Model_Video)
            	$valueVideo = Engine_Api::_()->getItem('pagevideo',$valueVideo->video_id);
       ?>
          <?php $imageURL = $valueVideo->getHref($this->customParamsArray); ?>
          <a id="video-lightbox-id-<?php echo $valueVideo->video_id; ?>" href="<?php echo $valueVideo->getHref($this->customParamsArray); ?>" class="ses-video-viewer ses_ml_photos_panel_photo_thumb sespagevideo_thumb_img">
