...
<ul class='sesbasic_clearfix sesbasic_bxs sesbasic_sidebar_block sespageoffer_sidebar_info_block'>
  <?php if(in_array('by',$this->criteria)){ ?>
    <li class="sesbasic_clearfix sespageoffer_list_stats sespageoffer_list_created_by"> <i title='<?php echo $this->translate("Created by"); ?>' class="fa fa-user"></i> <span><a href="<?php echo $this->subject->getOwner()->getHref(); ?>"><?php echo $this->translate('');?><?php echo $this->subject->getOwner()->getTitle(); ?></a></span> </li>
  <?php } ?>
  <?php if(in_array('creationDate',$this->criteria)){ ?>
  <li class="sesbasic_clearfix sespageoffer_list_stats sespageoffer_list_created_on"> <i title='<?php echo $this->translate("Created on"); ?>' class="fa fa-clock-o"></i> <span><?php echo $this->translate('%1$s', $this->timestamp($this->subject->creation_date)); ?></span> </li>
  <?php } ?>
  <?php if(in_array('pagename',$this->criteria)) { ?>
    <?php $page = Engine_Api::_()->getItem('sespage_page',$this->subject->parent_id); ?>
    <?php if($page) { ?>
      <li class="sesbasic_clearfix sespageoffer_list_stats sespageoffer_list_category"> <i title="<?php echo $this->translate("Page"); ?>" class="fa fa-long-arrow-right"></i> <span><a href="<?php echo $page->getHref(); ?>"><?php echo $page->getTitle(); ?></a></span> </li>
    <?php } ?>          
  <?php } ?>
