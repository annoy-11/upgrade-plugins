  ...
  <?php } elseif($this->view_type == 'map') {
   ...
   <?php $rating = ''; ?>
    <?php if(isset($this->ratingActive)){ ?>
      <?php $rating .= '<div class="sesproduct_list_rating">
            <div class="sesproduct_list_ratings">';
              for( $x=1; $x<=(int)$item->rating; $x++ ):
                $rating .= '<span class="sesproduct_rating_star_small"></span>';
              endfor;
              if( (round($item->rating) - (int)$item->rating) > 0):
                $rating .= '<span class="sesproduct_rating_star_small far fa-star-half"></span>';
              endif;
              for( $x=5; $x>round($item->rating); $x-- ):
                $rating .= '<span class="sesproduct_rating_star_small sesproduct_rating_star_disable"></span>';
              endfor;
            $rating .= '</div>
          </div>'; 
    ?>
    ...
    <?php if(isset($this->addCompareActive)){
        ...
        $addCompare .=' value="1" data-attr='. $compareData.' /><span class="">Add to Compare</span><span class="checkmark"></span></label>';
     }
    }?>
    <?php $addWishlist = ''; ?>
    ...
    <?php $productPrice = ''; ?>
     ...
     <?php if(isset($this->categoryActive)){
            ...
            $categoryItem = Engine_Api::_()->getItem('sesproduct_category', $item->category_id);
                if($categoryItem):
                    $category .='<div class="sesproduct_product_stat sesbasic_text_light">
                        <span> <i class="far fa-folder-open" title="'.$this->translate('Category').'"></i> <a href="'.$categoryItem->getHref().'">'. $categoryItem->category_name.'</a> </span>
                        </div>';
                endif;
        endif;
    } ?>
        ...
        <?php $creationDate .='<div class="sesproduct_product_stat sesbasic_text_light"> <span><i class="far fa-calendar"></i>';
    ...
    <?php if(isset($this->storeNameActive) && count($store)){ 
            ...
            $storeInfo ='<div class="sesproduct_store_name"><i class="fa fa-store"></i><div class="store_name sesbasic_text_light">'.$storeTitle.'</div></div>'; 
    ...
    <?php endif;?>
      ...
      $counter = 0;
			$owner =  '<div class="sesproduct_grid_date sesproduct_list_stats sesbasic_text_light"><span><i class="far fa-user"></i>'.$this->translate("by ") .$this->htmlLink($owner->getHref(),$owner->getTitle() ).'</span></div>';
...
<?php if(!$this->is_ajax && Engine_Api::_()->getApi('settings', 'core')->getSetting('enableglocation', 1)): ?>
	<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesbasic/externals/styles/styles.css'); ?>
  <script type="text/javascript" src="<?php echo $this->layout()->staticBaseUrl; ?>application/modules/Sesbasic/externals/scripts/richMarker.js?c=2"></script>
  <script type="text/javascript" src="<?php echo $this->layout()->staticBaseUrl; ?>application/modules/Sesbasic/externals/scripts/marker.js"></script>
	...
	<script type="application/javascript">
        ...
        var rating = markerArrayData_<?php echo $randonNumber?>[i]['rating'];
