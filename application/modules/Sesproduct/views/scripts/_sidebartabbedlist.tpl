 ...
 * @version    $Id: _sidebartabbedlist.tpl  2019-03-04 00:00:00 SocialEngineSolutions $
...
<div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" class="sesbasic_clearfix sesbasic_bxs clear">
  <ul class="sesproduct_products_listing sesbasic_clearfix clear" id="sidebar-tabbed-widget_<?php echo $randonNumber; ?>" style="min-height:50px;">
    <?php } ?>
    ...
    <li class="sesproduct_sidebar_product_list sesbasic_clearfix">
      <div class="sesproduct_sidebar_product_list_img" style="height:<?php echo is_numeric($this->height_list) ? $this->height_list.'px' : $this->height_list ?>;width:<?php echo is_numeric($this->width_list) ? $this->width_list.'px' : $this->width_list ?>;">
        <?php $href = $item->getHref();$imageURL = $photoPath;?>
        <a href="<?php echo $href; ?>" data-url = "<?php echo $item->getType() ?>"> <span style="background-image:url(<?php echo $imageURL; ?>);"></span> </a> </div>
      <div class="sesproduct_sidebar_product_list_cont">
        <div class="sesproduct_sidebar_product_list_title">
        ...
        <div class="sesproduct_sidebar_product_list_date">
          ...
          <span title="<?php echo date('M d, Y',strtotime($item->publish_date));?>"> <?php echo date('M d, Y',strtotime($item->publish_date));?> </span>
          <?php } ?>
        </div>
        <?php } ?>
        ...
        <?php if($categoryItem):?>
        <div class="sesproduct_sidebar_product_list_date"> <i class="fa fa-folder-open sesbasic_text_light" title="<?php echo $this->translate('Category'); ?>"></i> <a href="<?php echo $categoryItem->getHref(); ?>"><?php echo $this->translate($categoryItem->category_name); ?></a> </div>
        <?php endif;?>
        <?php endif;?>
        <?php } ?>
        ...
        <div class="sesproduct_sidebar_product_list_date"> <i class="fa fa-map-marker sesbasic_text_light"></i> <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('enableglocation', 1)) { ?><a href="<?php echo $this->url(array('resource_id' => $item->product_id,'resource_type'=>'sesproduct','action'=>'get-direction'), 'sesbasic_get_direction', true) ;?>" class="opensmoothboxurl" title="<?php echo $item->location;?>"><?php echo $item->location;?></a><?php } else { ?><?php echo $item->location;?><?php } ?> </div>
        <?php } ?>
    ...
    <li class="sesproduct_grid sesbasic_bxs <?php if((isset($this->my_products) && $this->my_products)){ ?>isoptions<?php } ?>" style="width:<?php echo is_numeric($this->width_grid) ? $this->width_grid.'px' : $this->width_grid ?>;">
      <div class="sesproduct_grid_inner sesproduct_thumb">
        <div class="sesproduct_grid_thumb" style="height:<?php echo is_numeric($this->height_grid) ? $this->height_grid.'px' : $this->height_grid ?>;">
          ...
          <div class="sesproduct_img_thumb_over"> <a href="<?php echo $href; ?>" data-url = "<?php echo $item->getType() ?>"></a>
            <div class="sesproduct_list_grid_thumb_btns">
              ...
              <?php endif;?>
              <?php endif;?>
              <div class="sesproduct_quick_view"> <a href="javascript:;" data-url="<?php echo "sesproduct/index/quick-view/product_id/". $item->product_id; ?>" class="quick_vbtn sessmoothbox"><?php echo $this->translate('Quick View');?></a> </div>
            </div>
          </div>
        </div>
        <div class="sesproduct_grid_info clear clearfix sesbm">
          ...
          <div class="sesproduct_grid_meta_block">
            ...
            <?php if(isset($this->creationDateActive)): ?>
            <div class="sesproduct_product_stat sesbasic_text_light"> <span><i class=" fa fa-clock-o"></i> <?php echo date('M d',strtotime($item->publish_date));?></span> </div>
            <?php endif;?>
            <?php if(isset($this->locationActive) && isset($item->location) && $item->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.enable.location', 1)){ ?>
          ...
          <div class="sesproduct_listing_item_des"> <?php echo $this->string()->truncate($this->string()->stripTags($item->body), $this->description_truncation_grid) ?> </div>
          <?php } ?>
    ...
    <div class="tip"> <span> <?php echo $this->translate('Nobody has posted a product with that criteria.');?>
      <?php if ($this->can_create):?>
      <?php echo $this->translate('Be the first to %1$spost%2$s one!', '<a href="'.$this->url(array('action' => 'create'), "sesproduct_general").'">', '</a>'); ?>
      <?php endif; ?>
      </span> </div>
    <?php else:?>
    <div class="tip"> <span> <?php echo $this->translate('Nobody has created a product yet.');?>
      <?php if ($this->can_create):?>
      <?php echo $this->translate('Be the first to %1$spost%2$s one!', '<a href="'.$this->url(array('action' => 'create'), "sesproduct_general").'">', '</a>'); ?>
      <?php endif; ?>
      </span> </div>
  ...
  <?php if($this->loadOptionData != 'pagging' && !($this->show_limited_data)):?>
  <div class="sesbasic_view_more" id="view_more_<?php echo $randonNumber;?>" onclick="viewMore_<?php echo $randonNumber; ?>();" > <?php echo $this->htmlLink('javascript:void(0);', $this->translate('View More'), array('id' => "feed_viewmore_link_$randonNumber", 'class' => 'buttonlink fa fa-arrow-circle-o-down')); ?> </div>
