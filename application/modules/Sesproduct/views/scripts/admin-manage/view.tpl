...
<?php if(isset($this->item->store_id)){ ?>
    ...
    <?php } ?>
    <?php
        if($this->item->category_id != '' && intval($this->item->category_id) && !is_null($this->item->category_id)):
            $categoryItem = Engine_Api::_()->getItem('sesproduct_category', $this->item->category_id);
    ?>
    <tr>
    ...
    <?php } ?>
    <?php if(isset($this->item->location) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.enable.location', 1)){ ?>
    <tr>
    ...
    <tr>
      <td><?php echo $this->translate('Discount') ?>:</td>
        <?php if($this->item->discount_type == 1){ ?>
            <td><?php echo $this->item->fixed_discount_value != 0 ? Engine_Api::_()->sesproduct()->getCurrencyPrice($this->item->fixed_discount_value) : 'No'; ?></td>
        <?php } else { ?>
            <td><?php echo $this->item->percentage_discount_value != 0 ? $this->item->percentage_discount_value .'%' : 'No'; ?></td>
         <?php } ?>
    </tr>
    <?php } ?>
    <?php if(isset($this->item->location) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.enable.location', 1)){ ?>
    ...
    <?php } ?>
	  ...
	  <?php $settings = Engine_Api::_()->getApi('settings', 'core'); ?> 
		<?php if(isset($this->item->ip_address) && $settings->getSetting('sesproduct.displayips', '1')){ ?>
			<tr>
