        ...
        <table class='admin_table'>
          ...
          <tbody>
            ...
            <tr>
              ...
              <td><?php echo $this->htmlLink($store->getHref(), $this->translate(Engine_Api::_()->sesbasic()->textTruncation($store->getTitle(),16)), array('title' => $store->getTitle(), 'target' => '_blank')); ?></td>
          <?php  $owner = Engine_Api::_()->getItem('user', $store->owner_id); ?>
          <td><?php echo $this->htmlLink($owner->getHref(), $this->translate(Engine_Api::_()->sesbasic()->textTruncation($owner->getTitle(),16)), array('title' => $owner->getTitle(), 'target' => '_blank')); ?></td>
              <td><?php echo Engine_Api::_()->sesproduct()->getCurrencyPrice($item->requested_amount,$defaultCurrency); ?></td>
              <td><?php echo Engine_Api::_()->sesproduct()->dateFormat($item->creation_date	); ?></td>
              ...
              <td>
                <?php if ($item->state == 'pending'){ ?>
                    <?php echo $this->htmlLink($this->url(array('route' => 'default', 'module' => 'sesproduct', 'controller' => 'payment','store_id' => $store->store_id,'action'=>'approve','id'=>$item->userpayrequest_id)), $this->translate("Approve"), array('class' => 'smoothbox')); ?> |
                    <?php echo $this->htmlLink($this->url(array('route' => 'default', 'module' => 'sesproduct', 'controller' => 'payment','action' => 'cancel', 'id' => $item->userpayrequest_id, 'store_id' => $store->store_id)), $this->translate("Reject"), array('class' => 'smoothbox')); ?> |
                <?php } ?>
                <?php echo $this->htmlLink($this->url(array('action' => 'payment-requests', 'store_id' => $store->custom_url), 'estore_dashboard', true), $this->translate("Edit"), array('class' => '','target'=>'_blank')); ?> |
                    <?php echo $this->htmlLink($this->url(array('action' => 'detail-payment', 'id' => $item->userpayrequest_id, 'store_id' => $store->custom_url), 'estore_dashboard', true), $this->translate("Details"), array('class' => 'smoothbox')); ?>
              </td>
            </tr>
            <?php endforeach; ?>
          </tbody>
