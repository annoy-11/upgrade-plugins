  ...
  <div class="sesproduct_pinboard_list_item <?php if((isset($this->my_products) && $this->my_products)){ ?>isoptions<?php } ?>">
    ...
    <div class="sesproduct_pinboard_info_product sesbasic_clearfix">
      <div class="sesproduct_pinboard_list_item_cont sesbasic_clearfix">
        ...
        <?php if($categoryItem):?>
        <div class="sesproduct_pinboard_categry sesproduct_list_stats sesbasic_text_light"> <span> <a href="<?php echo $categoryItem->getHref(); ?>"><i class="fa fa-folder-open" title="<?php echo $this->translate('Category'); ?>"></i> <span><?php echo $categoryItem->category_name; ?></span></a> </span> </div>
        <?php endif;?>
        <?php endif;?>
        <?php } ?>
        ...
        <?php endif;?>
          <div class="sesproduct_pinboard_meta_block">
            <?php if(isset($this->storeNameActive)):?>
            <?php $store = Engine_Api::_()->getItem('stores',$item->store_id); ?>
              <?php if(!empty($store)) { ?>
              <div class="sesproduct_product_stat sesbasic_text_dark sesbasic_clearfix">
            ...
            <div class="sesproduct_product_stat sesbasic_text_dirk">
              <?php if($item->publish_date): ?>
              <span><i class="fa fa-calendar"></i> <a href="<?php echo $this->url(array('action'=>'browse'),'sesproduct_general',true).'?date='.date('Y-m-d',strtotime($item->publish_date)); ?>"><?php echo date('M d',strtotime($item->publish_date));?></a></span>
              <?php else: ?>
              <span><i class="fa fa-calendar"></i> <a href="<?php echo $this->url(array('action'=>'browse'),'sesproduct_general',true).'?date='.date('Y-m-d',strtotime($item->creation_date)); ?>"><?php echo date('M d',strtotime($item->creation_date));?></a></span>
              <?php endif; ?>
            </div>
            <?php } ?>
            ...
            <?php if(isset($this->locationActive) && isset($item->location) && $item->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct_enable_location', 1)){ ?>
            <div class="sesproduct_product_stat _location"> <span> <i class="fa fa-map-marker"></i> <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('enableglocation', 1)) { ?><a href="<?php echo $this->url(array('resource_id' => $item->product_id,'resource_type'=>'sesproduct','action'=>'get-direction'), 'sesbasic_get_direction', true) ;?>" class="opensmoothboxurl" title="<?php echo $item->location;?>"><?php echo $item->location;?></a><?php } else { ?><?php echo $item->location;?><?php } ?> </span> </div>
            <?php } ?>
        </div>  
        <?php if(isset($this->descriptionActive)){ ?>
        <div class="sesproduct_listing_item_des"> <?php echo $this->string()->truncate($this->string()->stripTags($item->body), $this->description_truncation_pinboard) ?> </div>
        <?php } ?>
