 ...
 * @version    $Id: create.tpl  2019-03-04 00:00:00 SocialEngineSolutions $
...
<?php } else { ?>
  <script type="application/javascript">
    ...
    Sessmoothbox.css.push("<?php echo $this->layout()->staticBaseUrl . 'application/modules/Sesproduct/externals/styles/styles.css'; ?>");
  ...
  ?>
	<script type="application/javascript">
	 window.addEvent('domready',function(){
		sesJqueryObject('.sesproduct_type').hide();
		sesJqueryObject('.sesproduct_create').show();
		});
	</script>
  <div class="estore_dashboard_content sesbm sesbasic_clearfix">
<?php 		
}
$mainPhotoEnable = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.photo.mandatory', '1');
if ($mainPhotoEnable == 1) {
	$required = true; ?>
  <style type='text/css'>
  .sesproduct_create #tabs_form_productcreate-label label:after{content: '*';color: #F00;}
  </style>
<?php } else {
	$required = false;
}
...
<script type="text/javascript">
  ...
  function checkAvailsbility(submitform) {
    ...
    return;
    jqueryObjectOfSes('#sesproduct_custom_url_wrong').hide();
    jqueryObjectOfSes('#sesproduct_custom_url_correct').hide();
    jqueryObjectOfSes('#sesproduct_custom_url_loading').css('display','inline-block');
    jqueryObjectOfSes.post('<?php echo $this->url(array('controller' => 'index','module'=>'sesproduct', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value},function(response){
      jqueryObjectOfSes('#sesproduct_custom_url_loading').hide();
      response = jqueryObjectOfSes.parseJSON(response);
      if(response.error){
        jqueryObjectOfSes('#sesproduct_custom_url_correct').hide();
        jqueryObjectOfSes('#sesproduct_custom_url_wrong').css('display','inline-block');
        ...
        jqueryObjectOfSes('#sesproduct_custom_url_wrong').hide();
        jqueryObjectOfSes('#sesproduct_custom_url_correct').css('display','inline-block');
        if(typeof submitform != 'undefined') {
  ...
  en4.core.runonce.add(function() {
    ...
    if(jqueryObjectOfSes('input[name="show_end_time"]').length && jqueryObjectOfSes('input[name="show_end_time"]:checked').val() == ''){
      sesJqueryObject('#event_end_time-wrapper').hide();
    }
  ...
  <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct_enable_location', 1)){ ?>
    en4.core.runonce.add(function() {
      ...
      jqueryObjectOfSes('#map-canvas').css('height','0px');
      jqueryObjectOfSes('#map-canvas').css('width','0px');
...
<?php if (($this->current_count >= $this->quota) && !empty($this->quota)): ?>
  <div class="tip">
    <span>
      ...
      <?php echo $this->translate('If you would like to upload a new entry, please <a href="%1$s">delete</a> an old one first.', $this->url(array('action' => 'manage'), 'sesproduct_general'));?>
    </span>
  </div>
  <br/>
<?php else:?>
<div class="sesproduct_type" style="display:<?php echo !empty($this->showType) ? 'block' : 'none'; ?>">
    <ul>
    <?php foreach($this->allowedTypes as $allowedTypes){ ?>
      <?php if($allowedTypes == "simpleProduct"){ ?>
        <li><span class="product_type" data-rel="<?php echo $allowedTypes; ?>"><div class="product_type_img sesbasic_bg"><img src="application/modules/Sesproduct/externals/images/product_type/single.png" /></div><?php echo $this->translate("Simple Product") ?></span></li>
      <?php //}else if($allowedTypes == "groupedProduct"){  ?>
    <!-- <li><span class="product_type" data-rel="<?php //echo $allowedTypes; ?>"><div class="product_type_img sesbasic_bg"><img src="application/modules/Sesproduct/externals/images/product_type/grouped.png" /></div><?php //echo $this->translate("Grouped Product") ?></span></li>-->
      <?php /*}else if($allowedTypes == "bundledProduct"){ */ ?>
       <!--  <li><span class="product_type" data-rel="<?php //echo $allowedTypes; ?>"><div class="product_type_img sesbasic_bg"><img src="application/modules/Sesproduct/externals/images/product_type/bundled.png" /></div><?php //echo $this->translate("Bundled Product") ?></span></li>-->
      <?php //}else if($allowedTypes == "virtualProduct"){  ?>
   <!--  <li><span  class="product_type" data-rel="<?php //echo $allowedTypes; ?>"><div class="product_type_img sesbasic_bg"><img src="application/modules/Sesproduct/externals/images/product_type/virtual.png" /></div><?php //echo $this->translate("Virtual Product") ?></span></li>-->
      <?php }else if($allowedTypes == "configurableProduct"){  ?>
        <li><span class="product_type" data-rel="<?php echo $allowedTypes; ?>"><div class="product_type_img sesbasic_bg"><img src="application/modules/Sesproduct/externals/images/product_type/variable.png" /></div><?php echo $this->translate("Configurable/Variable Product") ?></span></li>
      <?php //}else if($allowedTypes == "downloadableProduct"){  ?>
<!--//       <li><span href="javascipt:void(0);" class="product_type" data-rel="<?php //echo $allowedTypes; ?>"><div class="product_type_img sesbasic_bg"><img src="application/modules/Sesproduct/externals/images/product_type/download.png" /></div><?php //echo $this->translate("Downloadable Product") ?></span></li>-->
      <?php //}else if($allowedTypes == "externalProduct"){  ?>
<!--   <li><span href="javascipt:void(0);" class="product_type" data-rel="<?php //echo $allowedTypes; ?>"><div class="product_type_img sesbasic_bg"><img src="application/modules/Sesproduct/externals/images/product_type/external.png" /></div><?php //echo $this->translate("External / Affiliate Product") ?></span></li>-->
      <?php } ?>
    <?php } ?>
    </ul>
	<?php if(empty($this->edit)){ ?>
    <script type="application/javascript">
    en4.core.runonce.add(function() {
      sesJqueryObject(document).on('click','.product_type',function(e){
        sesJqueryObject('.sesproduct_type').hide();
        sesJqueryObject('.sesproduct_create').show();
        sesJqueryObject('#type').val(sesJqueryObject(this).attr('data-rel'));
        sesJqueryObject('#product_type-element').html("<b>"+sesJqueryObject(this).text()+"</b>");
      });
      sesJqueryObject('#product_type').remove();
    });
    
       window.addEvent('domready',function(){
        <!--sesJqueryObject('.sesproduct_type').hide();
        sesJqueryObject('.sesproduct_create').hide();
        sesJqueryObject('#type').val(sesJqueryObject(this).attr('data-rel'));
        sesJqueryObject('#product_type-element').html("<b>"+sesJqueryObject(this).text()+"</b>");
        sesJqueryObject('#product_type').remove();
        });
 ...
 <div class="sesproduct_create_container">
  <div class="sesproduct_create sesbasic_bxs" style="position:relative;">
    <?php echo $this->form->render($this);?>
    <div class="sesbasic_loading_cont_overlay" style="display:none;"></div>
       <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.guidline', 1) && !empty(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.message.guidelines', ''))):?>
        <div id="estore_create_tips" class="estore_create_tips">
          ...
          <div class="create_tips_bottom_sec">
            <div class="sesbasic_html_block">
              <?php echo Engine_Api::_()->getApi('settings','core')->getSetting('sesproduct.message.guidelines', '');?>
            </div>
          </div>
        </div>
      <?php endif;?>
...
<?php } ?>
<?php if($this->subject() || count($_POST)){ ?>
<script type="application/javascript">
  en4.core.runonce.add(function() {
      var productType = "<?php echo $this->subject() ? $this->subject()->type : $_POST['type']; ?>";
      <!--sesJqueryObject('.sesproduct_type').hide();
      sesJqueryObject('#product_type-element').html("<b>"+sesJqueryObject('.product_type[data-rel="'+productType+'"]').text()+"</b>");
      sesJqueryObject('#product_type').remove();
      <?php if($this->subject()){ ?>
        sesJqueryObject('#sku-element').html("<b><?php echo $this->subject()->sku; ?></b>");
        sesJqueryObject('#sku').remove();
      <?php } ?>
  });
...
$profile_type = 2;
?>
<?php echo $this->partial('_customFields.tpl', 'sesproduct', array()); ?>
...
<?php if(Engine_Api::_()->estore()->isMultiCurrencyAvailable()){ ?>
	sesJqueryObject('#price-element').append('<span class="fa fa-retweet estore_convert_icon estore_link_btn" id="estore_currency_coverter" title="<?php echo $this->translate("Convert to %s",Engine_Api::_()->estore()->defaultCurrency());?>"></span>');
	sesJqueryObject('#price-label').append('<span> (<?php echo Engine_Api::_()->estore()->defaultCurrency(); ?>)</span>');
<?php }else{ ?>
	sesJqueryObject('#price-label').append('<span> (<?php echo Engine_Api::_()->estore()->defaultCurrency(); ?>)</span>');
<?php } ?>
  ...
  var getProfileType = function(category_id) {
    var mapping = <?php echo Zend_Json_Encoder::encode(Engine_Api::_()->getDbTable('categories', 'sesproduct')->getMapping(array('category_id', 'profile_type'))); ?>;
  ...
  jqueryObjectOfSes("#sesproducts_create").bind("keypress", function (e) {
  ...
  function validateForm() {
    ...
    jqueryObjectOfSes('#sesproducts_create input, #sesproducts_create select,#sesproducts_create checkbox,#sesproducts_create textarea,#sesproducts_create radio').each(
  function(index){
      ...
      if(jqueryObjectOfSes(this).closest('div').parent().css('display') != 'none' && jqueryObjectOfSes(this).closest('div').parent().find('.form-label').find('label').first().hasClass('required') && jqueryObjectOfSes(this).prop('type') != 'hidden' && jqueryObjectOfSes(this).closest('div').parent().attr('class') != 'form-elements'){	
          if(jqueryObjectOfSes(this).prop('type') == 'checkbox'){
            ...
            error = false;
          }else if(jqueryObjectOfSes(this).prop('type') == 'textarea' && jqueryObjectOfSes(this).prop('id') == 'body' && typeof tinyMCE != "undefined"){
    ...
    if(errorPresent == false && sesJqueryObject('#tabs_form_productcreate-wrapper').length && sesJqueryObject('.sesproduct_upload_item_photo').length == 0){
      <?php if($required):?>
        objectError = sesJqueryObject('.sesproduct_create_form_tabs');
        error = true;
		errorPresent = true;
      <?php endif;?>
    }
  ...
  var isSubmit = false;
en4.core.runonce.add(function() {
  jqueryObjectOfSes('#sesproducts_create').submit(function(e) {
    if(isSubmit == true)
      return true;
    e.preventDefault();
    ...
    if(validationFm) {
      ...
      if(typeof objectError != 'undefined'){
        var elem = sesJqueryObject(objectError).closest('.sesproduct_cnt').prev('h4').trigger('click');
				setTimeout(function(){
    ...
    }else if(sesJqueryObject('.sesproduct_upload_item_abort').length){
    ...
    <?php if($settings->getSetting('sesproduct.enable.wysiwyg',1)) { ?>
...
en4.core.runonce.add(function() {
  var sesproduct_create_form_tabsSesproduct = jqueryObjectOfSes('#sesproduct_create_form_tabs li a');
  sesproduct_create_form_tabsSesproduct.click(function() {
...
function createStatusbar(obj,file)
{
     ...
     if(rowCount %2 ==0) row ="even";
		  var checkedId = jqueryObjectOfSes("input[name=cover]:checked");
			this.objectInsert = jqueryObjectOfSes('<div class="sesproduct_upload_item sesbm '+row+'"></div>');
			this.overlay = jqueryObjectOfSes("<div class='overlay sesproduct_upload_item_overlay'></div>").appendTo(this.objectInsert);
			this.abort = jqueryObjectOfSes('<div class="abort sesproduct_upload_item_abort" id="abortPhoto_'+countUploadSes+'"><span><?php echo $this->translate("Cancel Uploading"); ?></span></div>').appendTo(this.objectInsert);
			this.progressBar = jqueryObjectOfSes('<div class="overlay_image progressBar"><div></div></div>').appendTo(this.objectInsert);
			this.imageContainer = jqueryObjectOfSes('<div class="sesproduct_upload_item_photo"></div>').appendTo(this.objectInsert);
			this.src = jqueryObjectOfSes('<img src="'+en4.core.baseUrl+'application/modules/Sesproduct/externals/images/blank-img.gif">').appendTo(this.imageContainer);
			this.infoContainer = jqueryObjectOfSes('<div class=sesproduct_upload_photo_info sesbasic_clearfix"></div>').appendTo(this.objectInsert);
			this.size = jqueryObjectOfSes('<span class="sesproduct_upload_item_size sesbasic_text_light"></span>').appendTo(this.infoContainer);
			this.filename = jqueryObjectOfSes('<span class="sesproduct_upload_item_name"></span>').appendTo(this.infoContainer);
			this.option = jqueryObjectOfSes('<div class="sesproduct_upload_item_options clear sesbasic_clearfix"><span class="sesproduct_upload_item_radio"><input type="radio" id="main_photo_id'+rowCount+'" name="cover"><label for="main_photo_id'+rowCount+'"><?php echo $this->translate("Main Photo"); ?></label></span><a class="edit_image_upload" href="javascript:void(0);"><i class="fa fa-edit"></i></a><a class="delete_image_upload" href="javascript:void(0);"><i class="fa fa-trash"></i></a></div>').appendTo(this.objectInsert);
		...
		if (1) {
			if(jqueryObjectOfSes('#show_photo').children('div').length == 1) {
				var idPhoto = jqueryObjectOfSes('#show_photo').eq(0).find('.sesproduct_upload_item_radio').find('input').attr('id');
...
function sendFileToServer(formData,status,file,isURL,i)
{
		...
		var url = '&isURL='+urlIs;
    var uploadURL = en4.core.baseUrl + 'sesproduct/photo/upload' + '?ul=1'+url; //Upload URL
    ...
    processData: false,
		...
		success: function(response){
					...
					if (response.status) {
							...
							status.option.find('.sesproduct_upload_item_radio').find('input').attr('value',response.photo_id);
...
var dragandrophandlerSesproduct = sesJqueryObject('#dragandrophandler');
dragandrophandlerSesproduct.click(function(){
	document.getElementById('file_multi').click();	
});
...
jqueryObjectOfSes(document).on('click','.edit_image_upload',function(e){
  e.preventDefault();
  var photo_id = jqueryObjectOfSes(this).closest('.sesproduct_upload_item_options').attr('data-src');
  if(photo_id){
    editImage(photo_id);
  }else
    return false;
...
jqueryObjectOfSes(document).on('click','.delete_image_upload',function(e){
	e.preventDefault();
	jqueryObjectOfSes(this).parent().parent().find('.sesproduct_upload_item_overlay').css('display','block');
	var sesthat = this;
	var isCover = jqueryObjectOfSes(this).closest('.sesproduct_upload_item_options').find('.sesproduct_upload_item_radio').find('input').prop('checked');
	var photo_id = jqueryObjectOfSes(this).closest('.sesproduct_upload_item_options').attr('data-src');
	if(photo_id){
		request = new Request.JSON({
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'sesproduct', 'controller' => 'index', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'sesproduct', 'controller' => 'index', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
  
  function showPreview(value) {
    if(value == 1)
    en4.core.showError('<div class="sesproduct_design_popup"><a class="sesproduct_icon_close"><i class="fa fa-times"></i></a> <p class="popup_design_title">'+en4.core.language.translate("Design 1")+'</p><img class="popup_img" src="./application/modules/Sesproduct/externals/images/layout_1.jpg" alt="" /></div>');
    else if(value == 2)
    en4.core.showError('<div class="sesproduct_design_popup"><a class="sesproduct_icon_close"><i class="fa fa-times"></i></a> <p class="popup_design_title">'+en4.core.language.translate("Design 2")+'</p><img src="./application/modules/Sesproduct/externals/images/layout_2.jpg" alt="" /></div>');
    else if(value == 3)
    en4.core.showError('<div class="sesproduct_design_popup"><a class="sesproduct_icon_close"><i class="fa fa-times"></i></a> <p class="popup_design_title">'+en4.core.language.translate("Design 3")+'</p><img src="./application/modules/Sesproduct/externals/images/layout_3.jpg" alt="" /></div>');
    else if(value == 4)
    en4.core.showError('<div class="sesproduct_design_popup"><a class="sesproduct_icon_close"><i class="fa fa-times"></i></a> <p class="popup_design_title">'+en4.core.language.translate("Design 4")+'</p><img src="./application/modules/Sesproduct/externals/images/layout_4.jpg" alt="" /></div>');
    return;
  }
  $$('.core_main_sesproduct').getParent().addClass('active');
  jqueryObjectOfSes(document).on('click','.sesproduct_icon_close',function(){
    Smoothbox.close();
  });
  ...
  <?php if(!empty($this->edit)){ ?>
    showEndDate(<?php echo $this->product->show_end_time; ?>);
  <?php } else { ?>
    showEndDate(<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('show.end.time', '1'); ?>);
  <?php } ?>
  ...
  sesJqueryObject(document).on('change','input[name=discount]',function(e){
   ...
   en4.core.runonce.add(function() {
     ...
     sesJqueryObject('#manage_stock').trigger('change');
  });
  sesJqueryObject(document).on('change','#manage_stock',function(e){
      var value = sesJqueryObject(this).val();
      if(value == 1){
        sesJqueryObject('#stock_quatity-wrapper').show();
        sesJqueryObject('#show_outof_stock-wrapper').show();
        sesJqueryObject('#enable_backin_stock-wrapper').show();  
      }else{
        sesJqueryObject('#stock_quatity-wrapper').hide();
        sesJqueryObject('#show_outof_stock-wrapper').hide();
        sesJqueryObject('#enable_backin_stock-wrapper').hide();  
      }
  ...
  en4.core.runonce.add(function () {
    ...
    contentAutocompleteUpsell = new Autocompleter.Request.JSON('upsell', "<?php echo $this->url(array('module' => 'sesproduct', 'controller' => 'index', 'action' => 'upsell-product','product_id'=>($this->subject() ? $this->subject()->getIdentity() : "")), 'default', true) ?>", {
      'postVar': 'text',
      'minLength': 1,
      'delay' : 250,
      'selectMode': 'pick',
    ...
    contentAutocompleteUpsell.addEvent('onSelection', function(element, selected, value, input) {
      ...
      if($('upsell_id').value) {
        ...
        if(notAddagain == false) {
          ...
          var shareItemmyElementPrivate = new Element('span', {'id' : 'upsell_remove_'+shareItem.id, 'class' : 'sesproduct_tag tag', 'html' :  shareItem.label  + ' <a href="javascript:void(0);" ' + 'onclick="removeFromToValueUpsell('+shareItem.id+');">x</a>'
          });
  ...
  en4.core.runonce.add(function () {
    ...
    <?php } ?>
    contentAutocompleteCrossSell = new Autocompleter.Request.JSON('crosssell', "<?php echo $this->url(array('module' => 'sesproduct', 'controller' => 'index', 'action' => 'upsell-product','product_id'=>($this->subject() ? $this->subject()->getIdentity() : "")), 'default', true) ?>", {
      'postVar': 'text',
      'minLength': 1,
      'delay' : 250,
      'selectMode': 'pick',
    ...
    contentAutocompleteCrossSell.addEvent('onSelection', function(element, selected, value, input) {
      ...
      if($('crosssell_id').value) {
        ...
        if(notAddagain == false) {
          ...
          var shareItemmyElementPrivate = new Element('span', {'id' : 'crosssell_remove_'+shareItem.id, 'class' : 'sesproduct_tag tag', 'html' :  shareItem.label  + ' <a href="javascript:void(0);" ' + 'onclick="removeFromToValueCrossSell('+shareItem.id+');">x</a>'
          });
  ...
  function hideShow(obj){
    ...
    if(sesJqueryObject('.content_'+id).css('display') == 'block')
      return;  
    sesJqueryObject('#img_'+id).find('img').attr('src','application/modules/Sesproduct/externals/images/downarrow.png');
    sesJqueryObject('.content_'+id).slideDown('slow');
    for(i=1;i<=maxDiv;i++){
      if(sesJqueryObject('.content_'+i).css('display') == 'block' && i != id){  
         sesJqueryObject('#img_'+i).find('img').attr('src','application/modules/Sesproduct/externals/images/leftarrow.png');
...
<?php if($this->typesmoothbox) { ?>
	...
	function showHideOptionsSesproduct(display){
		var elem = sesJqueryObject('.sesproduct_hideelement_smoothbox');
		for(var i = 0 ; i < elem.length ; i++){
				sesJqueryObject(elem[i]).parent().parent().css('display',display);
		}
	}
	function checkSettingSesproduct(first){
		var hideShowOption = sesJqueryObject('#advanced_sesproductoptions').hasClass('active');
			if(hideShowOption){
					showHideOptionsSesproduct('none');
					if(typeof first == 'undefined'){
						sesJqueryObject('#advanced_sesproductoptions').html("<i class='fa fa-plus-circle'></i><?php echo $this->translate('Show Advanced Settings') ?>");
					}
					sesJqueryObject('#advanced_sesproductoptions').removeClass('active');
			}else{
					showHideOptionsSesproduct('block');
					sesJqueryObject('#advanced_sesproductoptions').html("<i class='fa fa-minus-circle'></i><?php echo $this->translate('Hide Advanced Settings') ?>");
						sesJqueryObject('#advanced_sesproductoptions').addClass('active');
			}	
	}
	en4.core.runonce.add(function()
  {
		sesJqueryObject('#advanced_sesproductoptions').click(function(e){
			checkSettingSesproduct();
		});
		sesJqueryObject('#advanced_sesproductoptions').html("<i class='fa fa-plus-circle'></i><?php echo $this->translate('Show Advanced Settings') ?>");
		checkSettingSesproduct('true');
