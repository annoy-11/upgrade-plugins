 ...
 * @version    $Id: _reviewsidebarWidgetData.tpl  2019-03-04 00:00:00 SocialEngineSolutions $
...
<?php if($this->view_type == 'list'){ ?>
  <li class="sesproduct_review_sidebar_list  <?php if($this->image_type == 'rounded'):?>sesproduct_sidebar_image_rounded<?php endif;?> sesbasic_clearfix">
    <?php echo $this->htmlLink($reviewOwner, $this->itemPhoto($reviewOwner, 'thumb.icon')); ?>
    <div class="sesproduct_review_sidebar_list_info">
    	<?php  if(isset($this->titleActive)){ ?>
      	<div class="sesproduct_review_sidebar_list_title">
      ...
      <?php } ?>
        ...
        <?php $reviewTaker = Engine_Api::_()->getItem('sesproduct', $review->product_id);?>
        <?php if(isset($this->byActive)):?>
        <div class="sesproduct_review_sidebar_list_stat sesbasic_text_light">
      ...
      <?php endif;?>  
			<div class="sesproduct_list_stats sesproduct_review_sidebar_list_stat">
        ...
        <?php if(isset($this->viewActive) && isset($review->view_count)) { ?>
         ...
         <?php } ?>
        <?php if(Engine_Api::_()->getApi('core', 'sesproduct')->allowReviewRating() && $this->ratingActive){
      ...
      <?php if(isset($this->ratingActive)): ?>
        <div class="sesproduct_list_rating sesproduct_review_sidebar_list_stat clear">
					<?php $ratingCount = $review->rating; $x=0; ?>
					<?php if( $ratingCount > 0 ): ?>
						<?php for( $x=1; $x<=$ratingCount; $x++ ): ?>
							<span id="" class="sesproduct_rating_star_small"></span>
						<?php endfor; ?>
						<?php if( (round($ratingCount) - $ratingCount) > 0){ ?>
							<span class="sesproduct_rating_star_small sesproduct_rating_star_small_half"></span>
						<?php }else{ $x = $x - 1;} ?>
						<?php if($x < 5){ 
						for($j = $x ; $j < 5;$j++){ ?>
    ...
    <?php if($this->descriptionActive && $review->getDescription()):?>
      <div class="sesproduct_review_sidebar_list_body clear">
    ...
    <?php endif;?>
     <?php if(isset($this->featuredLabelActive) || ($review->featured)  ||  ($this->verifiedLabelActive) || ($review->verified)):?>
        <div class="sesproduct_review_sidebar_featured_list">
...
<?php }else{ ?>
  <li class="sesproduct_review_grid sesbasic_clearfix sesbasic_bxs sesproduct_grid_btns_wrap">
    <div class="sesproduct_review_grid_thumb floatL" style="height:<?php echo is_numeric($this->height) ? $this->height.'px' : $this->height ?>;">
      <?php $href = $reviewOwner->getHref();$imageURL = $reviewOwner->getPhotoUrl('thumb.profile');?>
      <a href="<?php echo $href; ?>" class="sesproduct_review_grid_thumb_img floatL">
      ...
      <?php } ?>
      <div class="sesproduct_review_grid_thumb_cont">
      	<?php if(isset($this->titleActive) ){ ?>
          <div class="sesproduct_review_grid_title">
            <?php if(strlen($review->getTitle()) > $this->title_truncation){ 
            ...
            <i class="sesproduct_verified_sign fa fa-check-circle" title="<?php echo $this->translate('Verified');?>"></i>
            <?php } ?>
          </div>
      	<?php } ?>
      </div>
        <?php if((isset($this->socialSharingActive)  && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.enable.sharing', 1)) || isset($this->likeButtonActive)) {
      ...
      <?php } ?>
		...
		<div class="sesproduct_review_grid_info sesbasic_clearfix clear">
			<?php $reviewCrator = Engine_Api::_()->getItem('user', $review->owner_id);?> 
			<?php $reviewTaker = Engine_Api::_()->getItem('sesproduct', $review->product_id);?>
			<div class="sesproduct_review_grid_stat sesproduct_sidebar_image_rounded floatL sesbasic_text_light">
				<?php echo $this->htmlLink($reviewTaker, $this->itemPhoto($reviewTaker, 'thumb.icon'), array('class' => 'sesproduct_reviw_prof_img floatL')); ?>
				<div class="sesproduct_review_title_block">
				  <?php if(isset($this->byActive)):?>
					...
					<div class="sesproduct_list_stats sesproduct_review_grid_stat clear">
						<?php if(isset($this->likeActive) && isset($review->like_count)) { ?>
						<span title="<?php echo $this->translate(array('%s like', '%s likes', $review->like_count), $this->locale()->toNumber($review->like_count)); ?>"><i class="fa fa-thumbs-up sesbasic_text_light"></i><?php echo $review->like_count; ?></span>
						<?php } ?>
						<?php if(isset($this->viewActive) && isset($review->view_count)) { ?>
				...
				<?php if(isset($this->ratingActive)): ?>
					<div class="sesproduct_list_rating sesproduct_review_sidebar_list_stat floatR">
						<?php $ratingCount = $review->rating; $x=0; ?>
						<?php if( $ratingCount > 0 ): ?>
							<?php for( $x=1; $x<=$ratingCount; $x++ ): ?>
								<span id="" class="sesproduct_rating_star_small"></span>
