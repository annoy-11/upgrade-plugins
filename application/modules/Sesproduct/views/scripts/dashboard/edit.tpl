...
<?php } ?>
<div class="estore_dashboard_form estore_create_form sesbasic_bxs">
  <?php echo $this->sesproductform->render(); ?>
  </div>
<?php if(!$this->is_ajax){ ?>
    </div>
      </div>
...
//prproduct form submit on enter
...
var counter = 0;
  function validateForm(){
    ...
    jqueryObjectOfSes('#sesproducts_edit input, #sesproducts_edit select,#sesproducts_edit checkbox,#sesproducts_edit textarea,#sesproducts_edit radio').each(
      function(index){
  ...
  jqueryObjectOfSes('#sesproducts_edit').submit(function(e){
    var validationFm = validateForm();
		if(!validationFm && sesJqueryObject('#sesproduct_schedule_time').length > 0 ) {
			var lastTwoDigitStart = sesJqueryObject('#sesproduct_schedule_time').val().slice('-2');
			var startDate = new Date(sesJqueryObject('#sesproduct_schedule_date').val()+' '+sesJqueryObject('#sesproduct_schedule_time').val().replace(lastTwoDigitStart,'')+':00 '+lastTwoDigitStart);
			var error = checkDateTime(startDate);
			if(error != ''){
				sesJqueryObject('#event_error_time-wrapper').show();
				sesJqueryObject('#sesproduct_schedule_error_time-element').text(error);
