...
<?php foreach($this->paginator as $productAdmin):?>
	<div class="admin_manage" id="admin_manage_<?php echo $productAdmin->role_id;?>">
		...
		<?php if($productAdmin->user_id != $this->owner_id):?>
			<a class="remove_product" href="javascript:void(0);" onclick="removeUser('<?php echo $productAdmin->product_id;?>','<?php echo $productAdmin->role_id;?>');"><i class="fa fa-close"></i></a>
		<?php endif;?>
		<br />
	</div>
<?php endforeach;?>
