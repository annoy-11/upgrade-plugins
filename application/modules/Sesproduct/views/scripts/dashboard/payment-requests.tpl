 ...
 * @version    $Id: payment-requests.tpl  2019-03-04 00:00:00 SocialEngineSolutions $
...
<div class="estore_dashboard_content_header sesbasic_clearfix">	
  <h3><?php echo $this->translate("Make Payment Request"); ?></h3>
    ...
    <div class="sesproduct_dashboard_threshold_amt"><?php echo $this->translate("Threshold Amount:"); ?> <b><?php echo Engine_Api::_()->sesevent()->getCurrencyPrice($this->thresholdAmount,$defaultCurrency); ?></b></div>
    <?php } ?>
</div>
<?php if(!count($this->userGateway)){ ?>
	<div class="tip">
  <span>    
    <?php echo $this->translate('Payment details not submitted yet %1$sClick Here%2$s to submit.', '<a href="'.$this->url(array('event_id' => $this->event->custom_url,'action'=>'account-details'), 'sesevent_dashboard', true).'">', '</a>'); ?>
  </span>
</div>
<?php } ?>
<?php $orderDetails = $this->orderDetails; ?>
<div class="sesproduct_sale_stats_container sesbasic_bxs sesbasic_clearfix">
	<div class="sesproduct_sale_stats">
...
<div class="sesproduct_request_payment_link ">	
	<a href="<?php echo $this->url(array('event_id' => $this->event->custom_url,'action'=>'payment-request'), 'sesevent_dashboard', true); ?>" class="openSmoothbox sesbasic_button fa fa-money"><?php echo $this->translate("Make Request For Payment."); ?></a>
</div>
<?php } ?>
        ...
        <tr>
          ...
          <td class="table_options">
          	<?php if ($item->state == 'pending'){ ?>
                <?php echo $this->htmlLink($this->url(array('event_id' => $this->event->custom_url,'action'=>'payment-request','id'=>$item->userpayrequest_id), 'sesevent_dashboard', true), $this->translate(""), array('class' => 'openSmoothbox fa fa-pencil','title'=>$this->translate("Edit Request"))); ?>
                <?php echo $this->htmlLink($this->url(array('action' => 'delete-payment', 'id' => $item->userpayrequest_id, 'event_id' => $this->event->custom_url), 'sesevent_dashboard', true), $this->translate(""), array('class' => 'openSmoothbox fa fa-trash','title'=>$this->translate("Delete Request"))); ?>
            <?php } ?>
            		<?php echo $this->htmlLink($this->url(array('action' => 'detail-payment', 'id' => $item->userpayrequest_id, 'event_id' => $this->event->custom_url), 'sesevent_dashboard', true), $this->translate(""), array('class' => 'openSmoothbox fa fa-eye','title'=>$this->translate("View Details"))); ?>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
