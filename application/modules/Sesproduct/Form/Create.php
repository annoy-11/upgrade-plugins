...
if($settings->getSetting('sesproduct.start.date', 1) || $settings->getSetting('sesproduct.end.date', 1))  {
 ...
 $this->upsell->getDecorator("Description")->setOption("placement", "append");
  ...
  if(empty($product)){
    ...
    $productdesign = Engine_Api::_()->authorization()->getPermission($viewer, 'sesproduct', 'productdesign');
		$chooselayoutVal = json_decode(Engine_Api::_()->authorization()->getPermission($viewer, 'sesproduct', 'chooselayout'));
		$chooseVal = Engine_Api::_()->authorization()->getPermission($viewer, 'sesproduct', 'defaultlayout');

    if($productdesign) {
      $designoptions = array();
      if(in_array(1, $chooselayoutVal)) {
        $designoptions[1] = '<a href="" onclick="showPreview(1);return false;"><img src="./application/modules/Sesproduct/externals/images/layout_1.jpg" alt="" /></a> '.$translate->translate("Design 1");
      }
      if(in_array(2, $chooselayoutVal)) {
        $designoptions[2] = '<a href="" onclick="showPreview(2);return false;"><img src="./application/modules/Sesproduct/externals/images/layout_2.jpg" alt="" /></a> '.$translate->translate("Design 2");
      }
      if(in_array(3, $chooselayoutVal)) {
        $designoptions[3] = '<a href="" onclick="showPreview(3);return false;"><img src="./application/modules/Sesproduct/externals/images/layout_3.jpg" alt="" /></a> '.$translate->translate("Design 3");
      }
      if(in_array(4, $chooselayoutVal)) {
        $designoptions[4] = '<a href="" onclick="showPreview(4);return false;"><img src="./application/modules/Sesproduct/externals/images/layout_4.jpg" alt="" /></a> '.$translate->translate("Design 4");
      }

      $this->addElement('Radio', 'productstyle', array(
        'label' => 'Product Layout',
        'description' => 'Choose a Template for your product.',
        'multiOptions' => $designoptions,
        'escape' => false,
        'value' => $chooseVal,
      ));
		} else {
      $this->addElement('Hidden', 'productstyle', array(
        'value' => $chooseVal,
        'order'=>$hiddenElement++
      ));
		}
