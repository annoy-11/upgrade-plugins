            ...
            try {
                    ...
                    foreach ($getAdminnSuperAdmins as $getAdminnSuperAdmin) {
                        ...
                        foreach($storeIds as $storeid) {
                            ...
                            Engine_Api::_()->getApi('mail', 'core')->sendSystem($user, 'sesproduct_product_orderplaced', array('sender_title' => $store->getOwner()->getTitle(),'order_id'=>$paymentOrder->order_id,'store_name'=>$store->getTitle(), 'object_link' => $store->getHref(),'gateway_name'=>'paypal','buyer_name'=>$viewer->getTitle(),'host' => $_SERVER['HTTP_HOST']));
                            ...
                            if($counter)
                                Continue;
                            Engine_Api::_()->getApi('mail', 'core')->sendSystem($store->getOwner()->email, 'sesproduct_product_orderplaced', array('host' => $_SERVER['HTTP_HOST'], 'order_id'=>$paymentOrder->order_id,'store_name'=>$store->getTitle(),'gateway_name'=>'paypal','buyer_name'=>$viewer->getTitle(),'object_link' => $store->getHref()));
                        }
                        $counter++;
                }
            }catch(Exception $e){}
