...
class Sesproduct_AdminPaymentController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
           ...
           $this->view->disable_gateway = false;
            ...
            if (!$form->isValid($this->getRequest()->getPost()))
              return;
                ...
                if(!($gateway = Engine_Api::_()->getItem('sesproduct_usergateway',$_POST['usergateway_id'])) || !($gateway->enabled)) {
                    $form->addError('Please Select Any Payment Gateway.');
                    return;
                }
                ...
                try{
                    ...
                    if($gateway->gateway_type == "paytm") {
                        ...
                        return $this->_forward('success', 'utility', 'core', array(
                                    'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('route' => 'default','module' => 'sesproduct', 'controller' => 'payment', 'action' => 'process','gateway_id'=>$_POST['usergateway_id']),'admin_default',true),
                                    'messages' => array($this->view->message)
                        ));
                    }
                }catch (Exception $e) {
	...
	public function processAction(){
        ...
        // Get gateway
        if(!($gateway = Engine_Api::_()->getItem('sesproduct_usergateway',$this->_getParam('gateway_id', null))) ||
            !($gateway->enabled) ) {
        ...
        // Process transaction
        if($gateway->plugin == "Sesproduct_Plugin_Gateway_Store_Stripe") {
            $currentCurrency = Engine_Api::_()->sesproduct()->getCurrentCurrency();
            $this->view->publishKey = $gateway->config['sesadvpmnt_stripe_publish']; 
            $this->view->session = $plugin->createOrderTransaction($item,$store,$params);
	...
	public function returnAction() {
        ...
        if(!($gateway = Engine_Api::_()->getItem('sesproduct_usergateway',$orderPayment->gateway_id)) ||
            !($gateway->enabled) ) {
           return $this->_helper->redirector->gotoRoute(array(), 'admin_default', true);
        }
        // Get gateway plugin
        ...
        try {
         //get all params
          $status = $plugin->orderTransactionReturn($orderPayment, $this->_getAllParams(),$item);
