 ...
 * @version    $Id: OrderController.php  2019-03-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesproduct_OrderController extends Core_Controller_Action_Standard {
	public function init() {
	 if (!$this->_helper->requireUser->isValid())
			return;
    $id = $this->_getParam('order_id', null);
    $order = Engine_Api::_()->getItem('sesproduct_order', $id);
    if ($order) {
    ...
    $product_id = Engine_Api::_()->getDbtable('sesproducts', 'sesproduct')->getEventId($id);
		$product = null;
    if ($product_id) {
      $product = Engine_Api::_()->getItem('product', $product_id);
      if ($product) {
		...
		$this->view->ticketDetail = $ticketDetails = $order->getTicket(array('order_id'=>$order->order_id,'user_id'=>$viewer->getIdentity(),'product_id'=>$product->product_id));
		if(!$ticketDetails)
			return $this->_forward('requireauth', 'error', 'core');
	}
    ...
    if ($product_id) {
      ...
      if ($product) {
		...
		if(is_array($postTicketUser) && count($postTicketUser)){
			...
			$db->query("DELETE FROM engine4_sesproduct_orderticketdetails WHERE order_id = ".$order->order_id);
			...
			foreach($postTicketUser['firstName'] as $key=>$val){
				...
				foreach($val as $keyTic =>$valTk){
					...
					$ticketCode = Engine_Api::_()->sesproduct()->generateTicketCode(8,'orderticketdetails');
					...
					$db->query("INSERT INTO engine4_sesproduct_orderticketdetails (first_name,last_name,email,ticket_id,ticket_number,order_id,mobile,registration_number) VALUES('".$valTk."','".$lastName."','".$email."','".$key."','".$keyTic."','".$order->order_id."','".$mobile."','".$registration_number."')");
					$counter++;
				}
			}
		}
    ...
    if ($product_id) {
      ...
      if ($event) {
		...
		if($order->state == 'complete')
			return $this->_forward('notfound', 'error', 'core');
		$this->view->viewer = $viewer = Engine_Api::_()->user()->getViewer();
		Engine_Api::_()->getDbtable('orderTickets', 'sesproduct')->updateTicketOrderState(array('order_id'=>$order->order_id,'state'=>'complete'));
		// success
    ...
    //send email
		...
		$orderDetails = Engine_Api::_()->getDbTable('orderticketdetails', 'sesproduct')->orderTicketDetails(array('order_id'=>$order->order_id));

		//Ticket Details
    $ticketsContent = '';
		$pdfCreate = false;
		//send pdf ticket if sesproductpdf extention enabled and activated
		if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesproductpdfticket') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproductpdfticket.pluginactivated')){
			 try{
				...
				$mail->addTo($order->getOwner()->email);
				 foreach($orderDetails as $keyDet => $item) {
						...
						$pdfname =	Engine_Api::_()->getApi('core', 'sesproductpdfticket')->createPdfFile($item,$event,$eventOrder,$user);
						if(!$pdfname){
								$pdfCreate = false;
								break;
						}else{
						 try{
							$pdfTicketFile = APPLICATION_PATH . '/public/sesproduct_ticketpdf/'.$pdfname;
	    ...
	    foreach($orderDetails as $keyDet => $item) {
		    ...
		    if($event->location && !$event->is_webinar && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.enable.location', 1)) {
			    $venue_name = '';
					if($event->venue_name){
		    ...
		    $ticketsContent .= 'on ' . Engine_Api::_()->sesproduct()->dateFormat($eventOrder->creation_date);
		    $ticketsContent .= '</div></div>';
	      ...
	      $ticketsContent .= '</div></div><div style="display:block;float:left;position:relative;text-align:center;width:100%;">';
				...
				if(!file_exists(APPLICATION_PATH . DIRECTORY_SEPARATOR . 'public/sesproduct_qrcode/'.$fileName)){
					$fileName = Engine_Api::_()->sesproduct()->generateQrCode($item->registration_number,$fileName);
				} else{
					$fileName = ( isset($_SERVER["HTTPS"]) && (strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] .Zend_Registry::get('StaticBaseUrl') .'/public/sesproduct_qrcode/'.$fileName;
				}
			}else
				$qrCode = '';
		...
		if(!$pdfCreate){
	    //Tickets Details
	    Engine_Api::_()->getApi('mail', 'core')->sendSystem($order->getOwner(), 'sesproduct_tikets_details', array('host' => $_SERVER['HTTP_HOST'], 'ticket_body' => $ticketsContent, 'event_title' => $event->getTitle()));
		}
	...
	public function processAction()
  {
    ...
    $event_id = Engine_Api::_()->getDbtable('events', 'sesproduct')->getEventId($id);
		$event = null;
    if ($event_id) {
      ...
      if ($event) {
		...
		$this->view->viewer = $viewer = Engine_Api::_()->user()->getViewer();
    if( !$gatewayId ||
        !($gateway = Engine_Api::_()->getItem('sesproduct_gateway', $gatewayId)) ||
        !($gateway->enabled) ) {
    ...
    if (!$orderPayment || ($orderId != $orderPayment->source_id) ||
			 ($orderPayment->source_type != 'sesproduct_order') ||
			 !($user_order = $orderPayment->getSource()) ) {
			return $this->_helper->redirector->gotoRoute(array(), 'sesproduct_general', true);
    }
    $gateway = Engine_Api::_()->getItem('sesproduct_gateway', $orderPayment->gateway_id);
    if( !$gateway )
      return $this->_helper->redirector->gotoRoute(array(), 'sesproduct_general', true);
    // Get gateway plugin
    ...
    try {
			...
			if(!$order->ragistration_number){
				$order->ragistration_number = Engine_Api::_()->sesproduct()->generateTicketCode(8);
				$order->save();
			}
  ...
  protected function _finishPayment($state = 'active',$orderPaymentId) {
    ...
    if ($state == 'free') {
			$session->unsetAll();
      return $this->_helper->redirector->gotoRoute(array('sesproduct_event'), 'default', true);
    } else {
  ...
  public function finishAction() {
	...
	public function checkorderAction(){
		$order_id = $this->_getParam('order_id',null);
		$checkOrderStatus = Engine_Api::_()->getDbtable('orders', 'sesproduct')->getOrderStatus($order_id);
		if($checkOrderStatus){
	...
	public function viewAction(){
		...
		$id = $this->_getParam('product_id', null);
      $value['product_id'] =  $product_id = Engine_Api::_()->getDbtable('sesproducts', 'sesproduct')->getProductId($id);
		$product = null;
    if ($product_id) {
      $product = Engine_Api::_()->getItem('sesproduct', $product_id);
      ...
      if ($product) {
		...
		$this->view->viewer = $viewer = Engine_Api::_()->user()->getViewer();
		$this->view->orders = $orders = Engine_Api::_()->getDbtable('orders', 'sesproduct')->productsOrders($value);
    $this->renderScript('/application/modules/Estore/views/scripts/order/view.tpl');
	}
}
