 ...
 * @version    $Id: AdminIntegratteothermoduleController.php  2019-03-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesproduct_AdminIntegrateothermoduleController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesproduct_admin_main', array(), 'sesproduct_admin_main_integrateothermodule');

    $this->view->enabledModules = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();

    $select = Engine_Api::_()->getDbtable('integrateothermodules', 'sesproduct')->select();
  ...
  public function addmoduleAction() {
    ...
    $this->view->form = $form = new Sesproduct_Form_Admin_Manage_Add();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      $integrateothermoduleTable = Engine_Api::_()->getDbtable('integrateothermodules', 'sesproduct');
      ...
      try {
        ...
        $dbInsert->query('INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `enabled`, `custom`, `order`) VALUES ("sesproduct_main_browseproduct_'.$row->getIdentity().'", "'.$modulename.'", "Browse Products", "", \'{"route":"sesproduct_browseproduct_'.$row->getIdentity().'","action":"browse-products", "resource_type":"'.$values['content_type'].'"}\', "'.$modulename.'_main", "", 1, 0, 999)');

        $this->createBrowseProductPage($modulename, $row->getIdentity());
  ...
  function createBrowseProductPage($modulename, $id) {
    ...
    $page_id = $db->select()
      ...
      ->where('name = ?', 'sesproduct_index_'.$id)
      ->limit(1)
      ->query()
      ->fetchColumn();
    ...
    if( !$page_id ) {
      ...
      $db->insert('engine4_core_pages', array(
        'name' => 'sesproduct_index_'.$id,
        'displayname' => 'SES - Advanced Products - '.ucfirst($modulename).' Products Browse Page',
        'title' => ucfirst($modulename) .' Product Browse',
        'description' => 'This page lists products.',
        'custom' => 0,
      ));
  ...
  public function deleteAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $inttable = Engine_Api::_()->getItem('sesproduct_integrateothermodule', $this->_getParam('integrateothermodule_id'));
        $pageName = "sesproduct_index_".$this->_getParam('integrateothermodule_id');
        if (!empty($pageName)) {
          $page_id = $db->select()
                  ->from('engine4_core_pages', 'page_id')
                  ->where('name = ?', $pageName)
        ...
        Engine_Api::_()->getDbtable('menuItems', 'core')->delete(array('name =?' => 'sesproduct_main_browseproduct_' . $this->_getParam('integrateothermodule_id')));

        $integrateothermodule = Engine_Api::_()->getItem('sesproduct_integrateothermodule', $this->_getParam('integrateothermodule_id'));
  ...
  public function enabledAction() {
    $db = Engine_Db_Table::getDefaultAdapter();
    $db->beginTransaction();
    $content = Engine_Api::_()->getItemTable('sesproduct_integrateothermodule')->fetchRow(array('integrateothermodule_id = ?' => $this->_getParam('integrateothermodule_id')));
    try {

      Engine_Api::_()->getDbtable('menuItems', 'core')->update(array('enabled' => !$content->enabled), array('name =?' => 'sesproduct_main_browseproduct_' . $this->_getParam('integrateothermodule_id')));
