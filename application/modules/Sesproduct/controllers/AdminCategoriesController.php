 ...
 * @version    $Id: AdminCategoriesController.php  2019-03-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesproduct_AdminCategoriesController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    if (isset($_POST['selectDeleted']) && $_POST['selectDeleted']) {
      if (isset($_POST['data']) && is_array($_POST['data'])) {
        ...
        foreach ($_POST['data'] as $key => $valueSelectedcategory) {
          $categoryDelete = Engine_Api::_()->getItem('sesproduct_category', $valueSelectedcategory);
          $deleteCategory = Engine_Api::_()->getDbtable('categories', 'sesproduct')->deleteCategory($categoryDelete);
          if ($deleteCategory) {
    ...
    if (isset($_POST['is_ajax']) && $_POST['is_ajax'] == 1) {
      ...
      $slugExists = Engine_Api::_()->getDbtable('categories', 'sesproduct')->slugExists($value['slug']);
      if (!$slugExists) {
        echo json_encode(array('slugError' => true));
        die;
      }
      if ($cat_id != -1) {
        $categoryData = Engine_Api::_()->getItem('sesproduct_category', $cat_id);
        if ($categoryData->subcat_id == 0) {
          $value['subcat_id'] = $cat_id;
          $seprator = '&nbsp;&nbsp;&nbsp;';
          $tableSeprator = '-&nbsp;';
          $parentId = $cat_id;
          $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesproduct')->orderNext(array('subcat_id' => $cat_id));
        } else {
          $value['subsubcat_id'] = $cat_id;
          $seprator = '3';
          $tableSeprator = '--&nbsp;';
          $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesproduct')->orderNext(array('subsubcat_id' => $cat_id));
          $parentId = $cat_id;
        }
      } else {
        $parentId = 0;
        $seprator = '';
        $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesproduct')->orderNext(array('category_id' => true));
        $tableSeprator = '';
      }
   ...
   $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('estore_admin_main', array(), 'estore_admin_main_categories');
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('estore_admin_main_categories', array(), 'sesproduct_admin_main_categories');
    $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesproduct_admin_main_categories', array(), 'sesproduct_admin_main_subcategories');
    //profile types
    $profiletype = array();
    $topStructure = Engine_Api::_()->fields()->getFieldStructureTop('sesproduct');
    if (count($topStructure) == 1 && $topStructure[0]->getChild()->type == 'profile_type') {
      $profileTypeField = $topStructure[0]->getChild();
      $options = $profileTypeField->getOptions();
      $options = $profileTypeField->getElementParams('sesproduct');
    ...
    //Get all categories
    $this->view->categories = Engine_Api::_()->getDbtable('categories', 'sesproduct')->getCategory(array('column_name' => '*', 'profile_type' => true));
  }
  ...
  public function changeOrderAction() {
    ...
    if ($this->_getParam('id', false) || $this->_getParam('nextid', false)) {
      ...
      $categoryTypeId = '';
      $checkTypeCategory = $dbObject->query("SELECT * FROM engine4_sesproduct_categories WHERE category_id = " . $category_id)->fetchAll();
      ...
      if ($checkTypeCategory)
        $currentOrder = Engine_Api::_()->getDbtable('categories', 'sesproduct')->order($categoryType, $categoryTypeId);
      ...
      $checkCategoryChildrenCondition = $dbObject->query("SELECT * FROM engine4_sesproduct_categories WHERE subcat_id = '" . $id . "' || subsubcat_id = '" . $id . "' || subcat_id = '" . $nextid . "' || subsubcat_id = '" . $nextid . "'")->fetchAll();
  ...
  public function editCategoryAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('estore_admin_main', array(), 'estore_admin_main_categories');
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('estore_admin_categories', array(), 'estore_admin_main_subcategories');

    $this->view->form = $form = new Sesproduct_Form_Admin_Category_Edit();

    $cat_id = $this->_getParam('id');
    $category = Engine_Api::_()->getItem('sesproduct_category', $cat_id);
    ...
    if (!$form->isValid($this->getRequest()->getPost())) {
      if (isset($_POST['slug'])) {
        $slugExists = Engine_Api::_()->getDbtable('categories', 'sesproduct')->slugExists($value['slug'], $cat_id);
        if (!$slugExists) {
    ...
    return $this->_helper->redirector->gotoRoute(array('module' => 'sesproduct', 'action' => 'index', 'controller' => 'categories'), 'admin_default', true);
  }
