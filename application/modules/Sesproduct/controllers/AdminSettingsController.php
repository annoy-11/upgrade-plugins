 ...
 * @version    $Id: AdminSettingsController.php  2019-03-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesproduct_AdminSettingsController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('estore_admin_main_setting', array(), 'sesproduct_admin_main_settings');

    $this->view->form  = $form = new Sesproduct_Form_Admin_Global();
    ...
    if( $this->getRequest()->isPost() && $form->isValid($this->_getAllParams()) ) {
      ...
      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.pluginactivated')) {
        ...
        if (isset($values['chooselayout']))
          $values['chooselayout'] = serialize($values['chooselayout']);
        else
          $values['chooselayout'] = serialize(array());

        //Start Landing page set
        if (isset($_POST['sesproduct_changelanding']) && $_POST['sesproduct_changelanding'] == 1) {
          $this->landingPageSetup();
				}
        //End Landing Page set

        //START TEXT CHNAGE WORK IN CSV FILE
        $oldSigularWord = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.text.singular', 'product');
        $oldPluralWord = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.text.plural', 'sesproducts');
        $newSigularWord = $values['sesproduct_text_singular'] ? $values['sesproduct_text_singular'] : 'product';
        $newPluralWord = $values['sesproduct_text_plural'] ? $values['sesproduct_text_plural'] : 'sesproducts';
    ...
    public function pagecreateAction() {
        ...
        $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('estore_admin_main_create', array(), 'sesproduct_admin_main_pagecreate');
        $settings = Engine_Api::_()->getApi('settings', 'core');
        $this->view->form = $form = new Sesproduct_Form_Admin_CreatePageSettings();
	...
	public function uploadDefaultCategory(){
		...
		foreach($catgoryData as $data){
			 $catId = $db->query("SELECT category_id FROM `engine4_sesproduct_categories` WHERE category_name = '" . $data[0] . "' AND (thumbnail = 0 OR cat_icon = 0)")->fetchAll();
			 ...
			 $db->query("UPDATE `engine4_sesproduct_categories` SET `cat_icon` = '" . $cat_icon . "',`thumbnail` = '" . $thumbnail_icon . "' WHERE category_id = " . $catId);
			 $db->query("UPDATE `engine4_sesproduct_categories` set `title` = category_name WHERE category_id = ".$catId ." AND ( title IS NULL OR title = '')");
       $db->query("UPDATE `engine4_sesproduct_categories` set `slug` = LOWER(REPLACE(REPLACE(REPLACE(REPLACE(category_name,'&',''),'  ',' '),' ','-'),'/','-'))  WHERE category_id = ".$catId ." AND ( title IS NULL OR slug = '')");
       $db->query("UPDATE `engine4_sesproduct_categories` SET `order` = `category_id`  WHERE category_id = ".$catId ." AND ( `order` IS NULL OR `order` = 0)");
  ...
  function setCategoryPhoto($file, $cat_id, $resize = false) {
    ...
    $params = array(
        'parent_type' => 'sesproduct_category',
        'parent_id' => $cat_id,
  ...
  public function categoriesAction()
  {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
      ->getNavigation('sesproduct_admin_main', array(), 'sesproduct_admin_main_categories');

    $this->view->categories = Engine_Api::_()->getItemTable('sesproduct_category')->fetchAll();
  }
  ...
  public function addCategoryAction()
  {
    ...
    $form = $this->view->form = new Sesproduct_Form_Admin_Category();
    ...
    $categoryTable = Engine_Api::_()->getItemTable('sesproduct_category');
  ...
  public function editCategoryAction()
  {
    ...
    $this->_helper->layout->setLayout('admin-simple');
    $category_id = $this->_getParam('id');
    $this->view->product_id = $this->view->category_id = $id;
    $categoriesTable = Engine_Api::_()->getDbtable('categories', 'sesproduct');
    $category = $categoriesTable->find($category_id)->current();
  ...
  //site statis for sesproduct plugin
  public function statisticAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('estore_admin_main_statistics', array(), 'sesproduct_admin_main_statistic');

    $productTable = Engine_Api::_()->getDbtable('sesproducts', 'sesproduct');
    $productTableName = $productTable->info('name');

    $productPhotoTable = Engine_Api::_()->getDbtable('photos', 'sesproduct');
    $productPhotoTableName = $productPhotoTable->info('name');

    $productAlbumTable = Engine_Api::_()->getDbtable('albums', 'sesproduct');
    $productAlbumTableName = $productAlbumTable->info('name');

    //Total Products
    $select = $productTable->select()->from($productTableName, 'count(*) AS totalproduct');
    $this->view->totalproduct = $select->query()->fetchColumn();

    //Total approved product
    $select = $productTable->select()->from($productTableName, 'count(*) AS totalapprovedproduct')->where('is_approved =?', 1);
    $this->view->totalapprovedproduct = $select->query()->fetchColumn();

    //Total verified product
    $select = $productTable->select()->from($productTableName, 'count(*) AS totalverified')->where('verified =?', 1);
    $this->view->totalproductverified = $select->query()->fetchColumn();

    //Total featured product
    $select = $productTable->select()->from($productTableName, 'count(*) AS totalfeatured')->where('featured =?', 1);
    $this->view->totalproductfeatured = $select->query()->fetchColumn();

    //Total sponsored product
    $select = $productTable->select()->from($productTableName, 'count(*) AS totalsponsored')->where('sponsored =?', 1);
    $this->view->totalproductsponsored = $select->query()->fetchColumn();

    //Total albums of product
    $select = $productAlbumTable->select()->from($productAlbumTableName, 'count(*) AS totalalbums');
    $this->view->totalalbums = $select->query()->fetchColumn();

    //Total photos of product
    $select = $productPhotoTable->select()->from($productPhotoTableName, 'count(*) AS totalphotos');
    ...
    if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesvideo')) {
			...
			//Total videos of product
			$select = $videoTable->select()->from($videoTableName, 'count(*) AS totalvideos')->where('parent_type =?', 'sesproduct');
			$this->view->totalvideos = $select->query()->fetchColumn();
    }

    //Total favourite product
    $select = $productTable->select()->from($productTableName, 'count(*) AS totalfavourite')->where('favourite_count <>?', 0);
    $this->view->totalproductfavourite = $select->query()->fetchColumn();

    //Total comments product
    $select = $productTable->select()->from($productTableName, 'count(*) AS totalcomment')->where('comment_count <>?', 0);
    $this->view->totalproductcomments = $select->query()->fetchColumn();

     //Total view product
    $select = $productTable->select()->from($productTableName, 'count(*) AS totalview')->where('view_count <>?', 0);
    $this->view->totalproductviews = $select->query()->fetchColumn();

     //Total like product
    $select = $productTable->select()->from($productTableName, 'count(*) AS totallike')->where('like_count <>?', 0);
    $this->view->totalproductlikes = $select->query()->fetchColumn();

    //Total rated product
    $select = $productTable->select()->from($productTableName, 'count(*) AS totalrated')->where('rating <>?', 0);
    $this->view->totalproductrated = $select->query()->fetchColumn();
  }
  ...
  public function manageWidgetizePageAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('estore_admin_main_widgetizepage', array(), 'sesproduct_admin_main_managepages');

    $this->view->pagesArray = array('sesproduct_index_welcome', 'sesproduct_index_home', 'sesproduct_index_browse', 'sesproduct_category_browse', 'sesproduct_index_locations', 'sesproduct_review_browse', 'sesproduct_index_manage', 'sesproduct_index_claim', 'sesproduct_index_create', 'sesproduct_index_view_1', 'sesproduct_index_view_2','sesproduct_index_view_3','sesproduct_index_view_4', 'sesproduct_index_tags', 'sesproduct_review_view', 'sesproduct_album_view', 'sesproduct_photo_view', 'sesproduct_index_claim-requests', 'sesproduct_index_list', 'sesproduct_category_index');
  }
  ...
  public function landingPageSetup() {
    ...
    $db->insert('engine4_core_content', array(
      ...
      'order' => $widgetOrder++,
      'params' => '{"bodysimple":"<div class=\"sesproduct_welcome_text_block_wrapper sesbasic_bxs sesbasic_clearfix\">\r\n\t<div class=\"sesproduct_welcome_text_block\">\r\n  \t<h2 class=\"sesproduct_welcome_text_block_maintxt\">SHARE YOUR  IDEAS & STORIES  WITH THE WORLD<\/h2>\r\n    <div class=\"sesproduct_welcome_text_block_buttons sesbasic_clearfix\">\r\n    \t<a href=\"products\/home\" class=\"estore_link_btn sesbasic_animation\">Explore Popular Products<\/a>\r\n      <a href=\"products\/create\" class=\"estore_link_btn sesbasic_animation\">Create Your Unique Product<\/a>\r\n      <a href=\"products\/categories\" class=\"estore_link_btn\">Explore By Category<\/a>\r\n    <\/div>\r\n<div style=\"text-align: center;margin-top:30px; box-shadow:inset 0 1px 0 rgba(255,255,255,.1),0 1px 0 rgba(8,32,84,.1);padding-bottom: 6px;width: 1200px; margin-left: 50px;\">\r\n<\/div>\r\n<div style=\"font-size: 24px;margin-bottom: 30px;  margin-top: 25px;text-align: center;\">Read our Sponsored Products!<\/div>\r\n  <\/div>\r\n<\/div>","en_US_bodysimple":"<div class=\"sesproduct_welcome_text_block_wrapper sesbasic_bxs sesbasic_clearfix\">\r\n\t<div class=\"sesproduct_welcome_text_block\">\r\n  \t<h2 class=\"sesproduct_welcome_text_block_maintxt\">SHARE YOUR  \u2022  IDEAS & STORIES  \u2022 WITH THE WORLD<\/h2>\r\n    <div class=\"sesproduct_welcome_text_block_buttons sesbasic_clearfix\">\r\n    \t<a href=\"products\/home\" class=\"estore_link_btn sesbasic_animation\">Explore Popular Products<\/a>\r\n      <a href=\"products\/create\" class=\"estore_link_btn sesbasic_animation\">Create Your Unique Product<\/a>\r\n      <a href=\"products\/categories\" class=\"estore_link_btn sesbasic_animation\">Explore By Category<\/a>\r\n    <\/div>\r\n    <p class=\"sesproduct_welcome_text_block_subtxt\">Read our Sponsored Products!<\/p>\r\n  <\/div>\r\n<\/div>","show_content":"1","title":"","nomobile":"0","name":"sesbasic.simple-html-block"}',
    ));
    $db->insert('engine4_core_content', array(
      'page_id' => $page_id,
      'type' => 'widget',
    ...
    $db->insert('engine4_core_content', array(
      ...
      'order' => $widgetOrder++,
      'params' => '{"bodysimple":"<div style=\"font-size: 34px;margin-bottom: 50px;margin-top:50px;text-align: center;\">Featured Posts -  Heads up productgers!<\/div>","en_US_bodysimple":"<div style=\"font-size: 34px;margin-bottom: 50px;margin-top:50px;text-align: center;\">Featured Posts -  Heads up productgers!<\/div>","show_content":"1","title":"","nomobile":"0","name":"sesbasic.simple-html-block"}',
    ));
    $db->insert('engine4_core_content', array(
      'page_id' => $page_id,
      'type' => 'widget',
      'name' => 'sesproduct.featured-sponsored-verified-random-product',
      'parent_content_id' => $main_middle_id,
      'order' => $widgetOrder++,
      'params' => '{"category":"0","criteria":"1","order":"","show_criteria":["like","comment","favourite","view","title","by","rating","ratingStar","verifiedLabel","favouriteButton","likeButton","category","socialSharing","creationDate"],"title":"","nomobile":"0","name":"sesproduct.featured-sponsored-verified-random-product"}',
    ));
    $db->insert('engine4_core_content', array(
      'page_id' => $page_id,
      'type' => 'widget',
    ...
    $db->insert('engine4_core_content', array(
      ...
      'parent_content_id' => $main_middle_id,
      'order' => $widgetOrder++,
      'params' => '{"height":"180","width":"196","limit":"12","video_required":"1","criteria":"admin_order","show_criteria":["title","countProducts"],"title":"","nomobile":"0","name":"sesproduct.product-category"}',
    ));
    $db->insert('engine4_core_content', array(
      'page_id' => $page_id,
      'type' => 'widget',
      ...
      'parent_content_id' => $main_middle_id,
      'order' => $widgetOrder++,
      'params' => '{"bodysimple":"<div style=\"text-align: center;margin-top:30px; box-shadow:inset 0 1px 0 rgba(255,255,255,.1),0 1px 0 rgba(8,32,84,.1);padding-bottom: 30px;\">\r\n\t<a class=\"sesproduct_welcome_btn sesbasic_animation\" href=\"products\/categories\">Browse All Categories\r\n<\/a><\/div>\r\n<div style=\"font-size: 34px;margin-bottom: 30px;  margin-top: 30px;text-align: center;\">Meet our Top Productgers!\r\n<\/span><\/div>","en_US_bodysimple":"<div style=\"text-align: center;margin-top:30px; box-shadow:inset 0 1px 0 rgba(255,255,255,.1),0 1px 0 rgba(8,32,84,.1);padding-bottom: 30px;\">\r\n\t<a class=\"sesproduct_welcome_btn sesbasic_animation\" href=\"products\/categories\">Browse All Categories\r\n<\/a><\/div>\r\n<div style=\"font-size: 34px;margin-bottom: 30px;  margin-top: 30px;text-align: center;\">Meet our Top Productgers!\r\n<\/span><\/div>","show_content":"1","title":"","nomobile":"0","name":"sesbasic.simple-html-block"}',
    ));
    $db->insert('engine4_core_content', array(
      'page_id' => $page_id,
      'type' => 'widget',
      'name' => 'sesproduct.top-productgers',
      'parent_content_id' => $main_middle_id,
      'order' => $widgetOrder++,
      'params' => '{"view_type":"horizontal","show_criteria":["count","ownername"],"height":"180","width":"234","showLimitData":"0","limit_data":"5","title":"","nomobile":"0","name":"sesproduct.top-productgers"}',
    ));
    $db->insert('engine4_core_content', array(
      'page_id' => $page_id,
      'type' => 'widget',
      ...
      'parent_content_id' => $main_middle_id,
      'order' => $widgetOrder++,
      'params' => '{"is_full":"1","is_pattern":"1","banner_image":"public\/admin\/banner_final.jpg","banner_title":"Start by creating your Unique Product","title_button_color":"FFFFFF","description":"Publish your personal or professional products at your desired date and time!","description_button_color":"FFFFFF","button1":"1","button1_text":"Get Started","button1_text_color":"0295FF","button1_color":"FFFFFF","button1_mouseover_color":"EEEEEE","button1_link":"products\/create","button2":"0","button2_text":"Button - 2","button2_text_color":"FFFFFF","button2_color":"0295FF","button2_mouseover_color":"067FDE","button2_link":"","button3":"0","button3_text":"Button - 3","button3_text_color":"FFFFFF","button3_color":"F25B3B","button3_mouseover_color":"EA350F","button3_link":"","height":"400","title":"","nomobile":"0","name":"sesspectromedia.banner"}',
    ));
  }
   ...
   $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('estore_admin_main', array(), 'estore_admin_main_dashboards');
     $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('estore_admin_main_dashboards', array(), 'estore_admin_main_productdashboards');

    $this->view->storage = Engine_Api::_()->storage();
    $this->view->paginator = Engine_Api::_()->getDbTable('dashboards', 'sesproduct')->getDashboardsItems();
  }
   public function enabledAction() {

    $id = $this->_getParam('dashboard_id');
    if (!empty($id)) {
      $item = Engine_Api::_()->getItem('sesproduct_dashboards', $id);
      $item->enabled = !$item->enabled;
      $item->save();
    }
    $this->_redirect('admin/sesproduct/settings/manage-dashboards');
  }
    public function editDashboardsSettingsAction() {

    $dashboards = Engine_Api::_()->getItem('sesproduct_dashboards', $this->_getParam('dashboard_id'));
    $this->_helper->layout->setLayout('admin-simple');
  ...
  public function resetPageSettingsAction(){
    ...
    try {
      ...
      include_once APPLICATION_PATH . "/application/modules/Sesproduct/controllers/resetPage.php";
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    ...
    if( $this->getRequest()->getParam('format') == 'smoothbox' ) {
      return $this->_forward('success', 'utility', 'core', array(
        'messages' => array(Zend_Registry::get('Zend_Translate')->_('This Page has been reset successfully.')),
         'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'sesproduct', 'controller' => 'settings', 'action' => 'manage-widgetize-page'), 'admin_default', true),
        'smoothboxClose' => true,
      ));
    }
  }
