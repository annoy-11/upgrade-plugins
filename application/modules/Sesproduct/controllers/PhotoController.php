 ...
 * @version    $Id: PhotoController.php  2019-03-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesproduct_PhotoController extends Core_Controller_Action_Standard {

  public function init() {
    if (!Engine_Api::_()->core()->hasSubject()) {
      if (0 !== ($photo_id = (int) $this->_getParam('photo_id')) &&
              null !== ($photo = Engine_Api::_()->getItem('sesproduct_photo', $photo_id))) {
        Engine_Api::_()->core()->setSubject($photo);
      } else if (0 !== ($product_id = (int) $this->_getParam('product_id')) &&
              null !== ($product = Engine_Api::_()->getItem('sesproduct', $product_id))) {
...
//rotate photo action from lightbox and photo view page
  public function rotateAction() {
    if (!$this->_helper->requireSubject('sesproduct_photo')->isValid())
      return;
// 		$product_id = $this->_getParam('product_id');
// 		$product = Engine_Api::_()->getItem('sesproduct', $product_id);
//     if (!$this->_helper->requireAuth()->setAuthParams($product, null, 'edit')->isValid())
//       return;
  ...
  public function flipAction() {
   if (!$this->_helper->requireSubject('sesproduct_photo')->isValid())
      return;
// 		$product_id = $this->_getParam('photo_id');
// 		$product = Engine_Api::_()->getItem('sesproduct', $product_id);
//     if (!$this->_helper->requireAuth()->setAuthParams($product, null, 'edit')->isValid())
//       return;
    if (!$this->getRequest()->isPost()) {
    ...
    $this->view->href = $photo->getPhotoUrl();
  	...
  	public function correspondingImageAction(){
		...
		$this->view->paginator = $paginator = Engine_Api::_()->getDbtable('photos', 'sesproduct')->getPhotoSelect(array('album_id'=>$album_id,'limit_data'=>100));
	}
  public function uploadAction() {

    if (isset($_GET['ul']) || isset($_FILES['Filedata']))
    return $this->_forward('upload-photo', null, null, array('format' => 'json', 'product_id'=> 0));
  }
  public function viewAction() {
    ...
    $product_id = $this->_getParam('product_id');
    $product = Engine_Api::_()->getItem('sesproduct', $product_id);
    if (!$this->_helper->requireAuth()->setAuthParams($product, null, 'view')->isValid()) {
      return;
    }
  ...
  public function uploadPhotoAction() {
    ...
    $sesproductPhotoTable = Engine_Api::_()->getDbtable('photos', 'sesproduct');

    $db = $sesproductPhotoTable->getAdapter();
    $db->beginTransaction();
		$session = new Zend_Session_Namespace();
    try {
      $viewer = Engine_Api::_()->user()->getViewer();
      if(empty($session->album_id)) {
			$album = Engine_Api::_()->getItemTable('sesproduct_album')->createRow();
      ...
      $photo = Engine_Api::_()->sesbasic()->setPhoto($uploadSource, $isURL,false,'sesproduct','sesproduct',$params,$album);
  ...
  public function editAction()
  {
    ...
    $this->view->form = $form = new Sesproduct_Form_Photo_Edit();
  ...
  public function deleteAction() {
    $photo = Engine_Api::_()->core()->getSubject();
//     $product = $photo->getParent('sesproduct');
		$album_id = $photo->album_id;
//     if (!$this->_helper->requireAuth()->setAuthParams($product, null, 'edit')->isValid()) {
//       return;
//     }

    $this->view->form = $form = new Sesproduct_Form_Photo_Delete();
