 ...
 * @version    $Id: AdminReviewCategoriesController.php  2019-03-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesproduct_AdminReviewCategoriesController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesproduct_admin_main_reviewsettings', array(), 'sesproduct_admin_main_review_categories');
    $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesproduct_admin_main_review_categories', array(), 'sesproduct_admin_main_review_subcategories');
    //profile types
    $profiletype = array();
    $topStructure = Engine_Api::_()->fields()->getFieldStructureTop('sesproductreview');
    if (count($topStructure) == 1 && $topStructure[0]->getChild()->type == 'profile_type') {
      $profileTypeField = $topStructure[0]->getChild();
      $options = $profileTypeField->getOptions();
      $options = $profileTypeField->getElementParams('sesproduct_event');
  ...
  public function editCategoryAction() {
    ...
    $this->view->form = $form = new Sesproduct_Form_Admin_Review_Category_Edit();

    $category = Engine_Api::_()->getItem('sesproduct_category', $this->_getParam('id'));
    ...
    return $this->_helper->redirector->gotoRoute(array('module' => 'sesproduct', 'action' => 'index', 'controller' => 'review-categories'), 'admin_default', true);
  }
