	...
	<div class="sesbasic_cloud_widget sesbasic_clearfix">  
		...
		<ul>
			...
			<?php foreach($this->paginator as $value):?> 	
				...
				<?php endif;?>	
				<li>
					<a title="<?php echo $value['category_name'] ?>" href="<?php echo $this->url(array('module' =>'sesproduct','controller' => 'index', 'action' => 'browse'),'sesproduct_general',true).'?category_id='.$value['category_id']; ?>">
...
<?php elseif($this->showType = 'simple'):?>  
	<ul class="sesproduct_sidebar_categroies_list">    
		<?php foreach( $this->paginator as $item ): ?>
			<li>
				<?php $subcategory = Engine_Api::_()->getDbtable('categories', 'sesproduct')->getModuleSubcategory(array('column_name' => "*", 'category_id' => $item->category_id, 'countProducts' => true)); ?>
				<?php if(counT($subcategory) > 0): ?>  
					<a id="sesproduct_toggle_<?php echo $item->category_id ?>" class="cattoggel cattoggelright" href="javascript:void(0);" onclick="showCategory('<?php echo $item->getIdentity()  ?>')">
					</a>
				<?php endif; ?>     
				<a class="catlabel' ?>" href="<?php echo $this->url(array('module' =>'sesproduct','controller' => 'index', 'action' => 'browse'),'sesproduct_general',true).'?category_id='.$item->category_id; ?>"> 
					...
					<?php else:?>
						<img src="application/modules/Sesproduct/externals/images/category/icons/image.png" />
					<?php endif;?>
					<span><?php echo $this->translate($item->category_name); ?>
						<?php echo ' ('.$item->total_products_categories.')';?>
					</span>
				</a>
				<ul id="subcategory_<?php echo $item->getIdentity() ?>" style="display:none;">   	
					<?php foreach( $subcategory as $subCat ): ?>
						<li>
							<?php $subsubcategory = Engine_Api::_()->getDbtable('categories', 'sesproduct')->getModuleSubsubcategory(array('column_name' => "*", 'category_id' => $subCat->category_id, 'countProducts' => true)); ?>
							<?php if(count($subsubcategory) > 0): ?>   
								<a id="sesproduct_subcat_toggle_<?php echo $subCat->category_id ?>" class="cattoggel cattoggelright" href="javascript:void(0);" onclick="showCategory('<?php echo $subCat->getIdentity()  ?>')"></a>
							<?php endif; ?> 
							<a class="catlabel" href="<?php echo $this->url(array('action' => 'browse'), $route, true).'?category_id='.urlencode($item->category_id) . '&subcat_id='.urlencode($subCat->category_id) ; ?>">
								...
								<span>
									...
									<?php echo ' ('.$subCat->total_products_categories.')';?>
								</span>
							</a>                  
									...
									<li>
										<a class="catlabel" href="<?php echo $this->url(array('action' => 'browse'), $route, true).'?category_id='.urlencode($item->category_id) . '&subcat_id='.urlencode($subCat->category_id) .'&subsubcat_id='.urlencode($subSubCat->category_id) ; ?>">
											...
											<span>
												...
												<?php echo ' ('.$subSubCat->total_products_categories.')';?>
											</span>
										</a>
									</li>
								<?php endforeach; ?>                
...
function showCategory(id) {  
	if($('subcategory_' + id)) {  
      ...
      $('sesproduct_toggle_' + id).removeClass('cattoggel cattoggeldown');
      $('sesproduct_toggle_' + id).addClass('cattoggel cattoggelright'); 
			$('subcategory_' + id).style.display = 'none';   
		} else {  
			$('sesproduct_toggle_' + id).removeClass('cattoggel cattoggelright');
      $('sesproduct_toggle_' + id).addClass('cattoggel cattoggeldown');
      $('subcategory_' + id).style.display = 'block';
		} 
	}    
	if($('subsubcategory_' + id)) {   
		if ($('subsubcategory_' + id).style.display == 'block') { 
			$('sesproduct_subcat_toggle_' + id).removeClass('cattoggel cattoggeldown');
      $('sesproduct_subcat_toggle_' + id).addClass('cattoggel cattoggelright');           
			$('subsubcategory_' + id).style.display = 'none';   
		} else {  
			$('sesproduct_subcat_toggle_' + id).removeClass('cattoggel cattoggelright'); 
			$('sesproduct_subcat_toggle_' + id).addClass('cattoggel cattoggeldown');  
