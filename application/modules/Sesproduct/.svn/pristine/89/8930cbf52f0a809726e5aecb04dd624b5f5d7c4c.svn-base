 ...
 */
class Sesproduct_Widget_ManageOrdersController extends Engine_Content_Widget_Abstract {

  public function indexAction() {
    $value = array();
    $this->view->product_id = $product_id = $value['product_id'] = $this->_getParam('product_id', false);
    if (!$product_id)
      return $this->setNoRender();
    $this->view->is_search_ajax = $is_search_ajax = isset($_POST['is_search_ajax']) ? $_POST['is_search_ajax'] : false;
    if (!$is_search_ajax) {
      $this->view->searchForm = $searchForm = new Sesproduct_Form_Searchorder();
    }
    $this->view->product = $product = Engine_Api::_()->getItem("sesproduct_order", $product_id);
    ...
    $this->view->paginator = $orders = Engine_Api::_()->getDbtable('orders', 'sesproduct')->productsOrders($value);
