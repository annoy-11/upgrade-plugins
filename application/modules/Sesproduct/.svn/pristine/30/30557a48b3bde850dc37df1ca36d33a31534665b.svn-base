 ...
 * @version    $Id: create-album.tpl  2019-03-04 00:00:00 SocialEngineSolutions $
...
<?php } ?>
<div class="sesproduct_album_form"> <?php echo $this->form->render($this) ?> </div>
<?php if(!$this->is_ajax){ ?>
  </div>
</div>
</div>
...
<script type="text/javascript">
  ...
  jqueryObjectOfSes('#sesproduct_create_form_tabs li a').on('click',function(){
...
function sendFileToServer(formData,status,file,isURL,i)
{
    ...
    var uploadURL =$('form-upload').action + '?ul=1'+url+'&product_id='+product_id; //Upload URL
...
jqueryObjectOfSes(document).on('click','.delete_image_upload',function(e){
    ...
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'sesproduct', 'controller' => 'album', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'sesproduct', 'controller' => 'album', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
</script>
