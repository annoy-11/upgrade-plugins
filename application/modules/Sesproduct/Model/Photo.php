 ...
 * @version    $Id: Photo.php  2019-03-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesproduct_Model_Photo extends Core_Model_Item_Collectible
{
  protected $_parent_type = 'sesproduct_album';

  protected $_owner_type = 'user';

  protected $_collection_type = 'sesproduct_album';
  ...
  public function setAlbumPhoto($photo,$isURL = false,$isUploadDirect = false) {
   ...
   public function getAlbum() {
    return Engine_Api::_()->getItem('sesproduct_album', $this->album_id);
  }
    //get next photo
  ...
  */
  public function getAlbumPhotoUrl($type = null) {
    ...
    if( !$photo_id ) {
      return 'application/modules/Sesproduct/externals/images/nophoto_album_thumb_normal.pngc=direct';
    }
  ...
  public function getSesproduct()
  {
    return Engine_Api::_()->getItem('sesproduct', $this->product_id);
  ...
  protected function _postDelete()
  {
    ...
    try
    {
      ...
      $album = $this->getCollection();
	    $productItem = Engine_Api::_()->getItem("sesproduct", $this->product_id);
      if((int)$album->photo_id == (int) $this->getIdentity()){
        if($this->getNextCollectible())
          $album->photo_id = $this->getNextCollectible()->getIdentity();
        $album->save();
      }
      $photoItem = Engine_Api::_()->getItem("sesproduct_photo", $productItem->photo_id);
      if(!$photoItem) {
        $productItem->photo_id = 0;
        $productItem->save();
      }
    }
    catch( Exception $e )
