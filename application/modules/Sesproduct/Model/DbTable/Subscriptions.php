 ...
 * @version    $Id: Subscriptions.php  2019-03-04 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesproduct_Model_DbTable_Subscriptions extends Engine_Db_Table {

  public function sendNotifications(Sesproduct_Model_Sesproduct $sesproduct) {

    if( !empty($sesproduct->draft) || $sesproduct->owner_type != 'user' ) {
      return $this;
    }

    // Get sesproduct owner
    $owner = $sesproduct->getOwner('user');
    ...
    $identities = $this->select()
      ...
      ->where('user_id = ?', $sesproduct->owner_id)
      ->query()
    ...
    foreach( $users as $user ) {
      $notificationTable->addNotification($user, $owner, $sesproduct, 'sesproduct_subscribed_new');
    }
