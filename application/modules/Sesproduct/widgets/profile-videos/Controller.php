 ...
 */
class Sesproduct_Widget_ProfileVideosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($_POST['is_ajax'])) {
      ...
      if (!$subject->authorization()->isAllowed($viewer, 'view'))
        return $this->setNoRender();
			...
			$subject = Engine_Api::_()->getItem('sesproduct', $params['parent_id']);
		}
		$this->view->allow_create = true;
		if(!$is_ajax && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesproduct') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproductpackage.enable.package', 1)){
    ...
    $this->view->description_truncation_list = $description_truncation_list = isset($params['description_truncation_list']) ? $params['description_truncation_list'] : $this->_getParam('description_truncation_list', '100');
      $viewer = Engine_Api::_()->user()->getViewer();
      $this->view->canUpload = $canUpload = Engine_Api::_()->sesproduct()->isProductAdmin($subject, 'create');
      ...
      $this->view->isProductAdmin = Engine_Api::_()->sesproduct()->checkProductAdmin($subject);
