...
<?php } ?>
<div class="sesproduct_socila_share_button">
...
<div id="sesbasic_popup_slide" class="well" style="display:none">
  <div class="sesbasic_popup sesbasic_bxs">
    <div class="sesbasic_popup_title">
       <?php echo $this->translate("Share This %s",ucfirst(str_replace(array('sesproduct_',''),'',$this->subject()->getTitle()))); ?>
    ...
    <div class="sesbasic_popup_content">
      <div class="sesbasic_share_popup_content_row clear sesbasic_clearfix">
          ...
          <?php if(in_array('privateMessage',$this->allowAdvShareOptions)){ ?>
            ...
            <a href="javascript:void(0)" class="sesbasic_button" onClick="opensmoothboxurl('<?php echo $this->url(array('module'=> 'sesproduct', 'controller' =>'index','action' => 'share','type' => $this->subject()->getType(),'id' => $this->subject()->getIdentity(),'format' => 'smoothbox'),'default',true); ?>')"> <?php echo $this->translate("Share on Site"); ?></a>
            <?php } ?>
             <?php if(in_array('quickShare',$this->allowAdvShareOptions)){ ?>
            <a href="javascript:void(0)" class="sesbasic_button" onClick="sesproductsendQuickShare('<?php echo $this->url(array('module'=> 'sesproduct', 'controller' =>'index','action' => 'share','type' => $this->subject()->getType(),'id' => $this->subject()->getIdentity()),'default',true); ?>');return false;"> <?php echo $this->translate("Quick Share on Site"); ?></a>
          <?php } ?>
           <?php //if(in_array('socialSharing',$this->allowAdvShareOptions)){ ?>
            <!--<a href="javascript:void(0)" class="sesbasic_button" onClick="sesproductsendQuickShare('<?php //echo $this->url(array('module'=> 'sesproduct', 'controller' =>'index','action' => 'share','type' => $this->subject()->getType(),'id' => $this->subject()->getIdentity()),'default',true); ?>');return false;"> <?php //echo $this->translate("Social Share on Site"); ?></a>-->
          <?php //} ?>
        </div>
      </div>
      <?php if($addThisCode && in_array('addThis',$this->allowAdvShareOptions)){ ?>
...
<script type="text/javascript">
  ...
  jquery1_8_2SesObject(document).ready(function () {
    ...
    <?php if(!empty($_SESSION['newProduct'])){ ?>
        sesJqueryObject('.sesbasic_popup_slide_open').trigger('click');
        <?php unset($_SESSION['newProduct']); ?>
    <?php } ?>
  });
</script>
