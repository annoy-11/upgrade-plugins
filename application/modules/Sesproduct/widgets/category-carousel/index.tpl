...
<div class="slide sesbasic_clearfix sesbasic_bxs sesproduct_category_carousel_wrapper <?php echo $this->isfullwidth ? 'isfull_width' : '' ; ?>" style="height:<?php echo $this->height ?>px;">
  <div class="productslide_<?php echo $this->identity; ?>">
    <?php foreach( $this->paginator as $item): ?>
    <div class="sesproduct_category_carousel_item sesbasic_clearfix sesproduct_grid_btns_wrap" style="height:<?php echo $this->height ?>px;width:<?php echo $this->width ?>px;">
      <div class="sesproduct_category_carousel_item_thumb" style="height:<?php echo $this->height ?>px;width:<?php echo $this->width ?>px;">       
        <?php
        $href = $item->getHref();
        ...
        <a href="<?php echo $href; ?>" class="sesproduct_list_thumb_img">
          ...
          <div class="sesproduct_grid_btns"> 
            <?php if(isset($this->socialshareActive)  && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.enable.sharing', 1)){ ?>
        ...
        <div class="sesproduct_category_carousel_item_info sesbasic_clearfix centerT">
          <?php if(isset($this->titleActive) ){ ?>
            <span class="sesproduct_category_carousel_item_info_title">
          ...
          <?php } ?>
           ...
           <span class="sesproduct_category_carousel_item_info_des">
          ...
          <?php } ?>
          <?php if(isset($this->countProductsActive) ){ ?>
            <span class="sesproduct_category_carousel_item_info_stat">
              <?php echo $this->translate(array('%s PRODUCT', '%s PRODUCTS',$item->total_products_categories), $this->locale()->toNumber($item->total_products_categories)); ?>
            </span>
          <?php } ?>
        </div>
    	</div>
...
<script type="text/javascript">
  window.addEvent('domready', function () {
		<?php if($this->isfullwidth){ ?>
			...
			htmlElement.addClass('sesproduct_category_carousel');
		<?php } ?>
