...
<div class="sesproduct_custom_offers" id="sesproduct_custom_offers<?php echo $this->offer_id; ?>">
  <!--Design 1-->
  <?php if($this->design == 'design1') { ?>
    <section class="sesproduct_welcome_offerblk sesbasic_clearfix sesbasic_bxs">
      <?php if($this->show_timer == 'yes' || !empty($this->heading)) { ?>
        <div class="sesproduct_custom_dealtime">
          <h2><?php echo $this->heading; ?> </h2>
         <?php $offer = Engine_Api::_()->getItem('estore_offer',$this->offer_id); ?>
          <div class="deal_time" id ="deal_time<?php echo $this->offer_id; ?>" data-start-date ="<?php echo $offer->offer_startdate; ?>" data-end-date ="<?php echo $offer->offer_enddate; ?>">
            ...
            <?php if($this->show_timer == 'yes') { ?>
                <div class="_img">
                    <img src="./application/modules/Sesproduct/externals/images/clock.png"/>
                </div>
                <div class="_time">
          ...
          <?php foreach($this->paginator as $offers) { ?>
            <a href="<?php echo $offers->url; ?>" target="_blank">
              ...
              <div class="_desc">
                ...
                <i class="fa fa-plus"></i>&nbsp;<?php echo $this->button_title; ?>
              </div>
            </div>
            </a>
          <?php } ?>
  ...
  <?php if($this->design == 'design2') { ?>
    <div class="sesproduct_welcome_branddiscount sesbasic_clearfix sesbasic_bxs">
       <?php if($this->show_timer == 'yes' || !empty($this->heading)) { ?>
      <div class="sesproduct_custom_dealtime">
        <h2><?php echo $this->heading; ?> </h2>
        <?php $offer = Engine_Api::_()->getItem('estore_offer',$this->offer_id); ?>
  ...
  <section class="sesproduct_products_carousel sesbasic_clearfix sesbasic_bxs">
  <?php if($this->show_timer == 'yes' || !empty($this->heading)) { ?>
    <div class="sesproduct_custom_dealtime">
      <h2><?php echo $this->heading; ?> </h2>
      <?php $offer = Engine_Api::_()->getItem('estore_offer',$this->offer_id); ?>
   ...
   <?php } ?>
    <div class="sesproduct_browseprdt">
      <div class="sesproduct_welcomep_products owl-carousel owl-theme">
        <?php foreach($this->paginator as $offers) { ?>
  ...
  <?php if($this->design == 'design4') { ?>
    <section class="sesproduct_carousel_deals sesbasic_clearfix sesbasic_bxs">
      <div class="sesproduct_welcome_offer">
        <?php if($this->show_timer == 'yes' || !empty($this->heading)) { ?>
        <div class="sesproduct_custom_dealtime">
          <h2><?php echo $this->heading; ?> </h2>    
            <?php $offer = Engine_Api::_()->getItem('estore_offer',$this->offer_id); ?>
        ...
        <?php  } ?>
        <div class="sesproduct_welcomep_deals owl-carousel owl-theme">
         <?php foreach($this->paginator as $offers) { ?>
          <div class="item" style="height:<?php echo is_numeric($this->height) ? $this->height.'px' : $this->height ?>;">
            <img src="<?php echo $offers->getPhotoUrl(); ?>"/>
            <div class="_desc">
  ...
  <div class="sesproduct_welcome_discoverproducts sesbasic_clearfix sesbasic_bxs">
    <?php if($this->show_timer == 'yes' || !empty($this->heading)) { ?>
      <div class="sesproduct_custom_dealtime">
        <h2><?php echo $this->heading; ?> </h2>    
            <?php $offer = Engine_Api::_()->getItem('estore_offer',$this->offer_id); ?>
...
sesJqueryObject(document).ready(function(){
 ...
 if(now > endOffertime) {
    ...
    sesJqueryObject("#sesproduct_custom_offers<?php echo $this->offer_id; ?>").html(null);
    return false;
 }
...
function endOffer(offerId)
  {
    ...
    offerEndRequest = (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/sesproduct/name/custom-offer',
      'data': {
      offerId: offerId,
      is_offer_end : 1,
...
sesproductJqueryObject('.sesproduct_welcomep_products').owlCarousel({
  margin: 10,
  nav: true,
  autoplay:true,
  loop:false,
...
sesproductJqueryObject('.sesproduct_welcomep_deals').owlCarousel({
  margin: 10,
  nav: true,
  autoplay:true,
  loop:false,
