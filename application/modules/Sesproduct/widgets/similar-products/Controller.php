...
class Sesproduct_Widget_SimilarProductsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (!$is_ajax) {
      ...
      if(!$category_id)
        return $this->setNoRender();
			...
			$subject = Engine_Api::_()->getItem('sesproduct', $params['product_id']);
      ...
      if(!$category_id)
		...
		$this->view->widgetName = 'similar-products';
        ...
        $this->view->description_truncation_limit = isset($params['description_truncation_limit']) ? $params['description_truncation_limit'] : $this->_getParam('description_truncation_limit', '45');

    $show_criterias = isset($params['show_criterias']) ? $params['show_criterias'] : $this->_getParam('show_criteria', array('like', 'comment', 'title', 'socialSharing', 'view', 'featuredLabel', 'sponsoredLabel', 'verifiedLabel', 'rating', 'storeName', 'favourite','category','favouriteButton','likeButton'));
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('sesproducts', 'sesproduct')->getSesproductsPaginator($value);
