...
class Sesproduct_Widget_ProductReviewsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (!$is_ajax) {
      ...
      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproduct.allow.owner', 1)) {
        ...
        if ($subject->product_id == $viewer->getIdentity())
          $allowedCreate = false;
        else
          $allowedCreate = true;
      }
      ...
      if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('sesproductreview', 'view'))
        return $this->setNoRender();
      $this->view->isReview = $hasReview = $reviewTable->isReview(array('product_id' => $subject->getIdentity(), 'column_name' => 'review_id'));
      $this->view->cancreate = Engine_Api::_()->sesbasic()->getViewerPrivacy('sesproductreview', 'create');

      if ($hasReview && Engine_Api::_()->sesbasic()->getViewerPrivacy('sesproductreview', 'edit')) {
        ...
        $this->view->form = $form = new Sesproduct_Form_Review_Create(array( 'reviewId' => $reviewObject->review_id, 'productItem' =>$subject));
        ...
        $this->view->form = $form = new Sesproduct_Form_Review_Create(array('productItem' =>$subject));
      }
    }
    ...
    $this->view->product_id = $params['product_id'] = isset($_POST['product_id']) ? $_POST['product_id'] : $subject->getIdentity();
