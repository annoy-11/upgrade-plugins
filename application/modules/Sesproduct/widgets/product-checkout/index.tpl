...
<?php if(!empty($this->noPaymentGatewayEnableByAdmin)){ ?>
<div class="tip">
	<span><?php echo $this->translate("No Payment Gateway enabled by admin. So you cannot proceed with the checkout."); ?></span>
</div>
<?php return; ?>
<?php } ?>
<div class="sesproduct_checkout sesbasic_clearfix sesbasic_bxs">
	<div class="sesproduct_check_address">
	 <form method="post" autocomplete="off" id="sesproduct_checkout_form">
		<div class="sesproduct_checkout_container">
			<div class="sesproduct_checkout_tabs sesproduct_checkout_tabs_step sesbasic_clearfix ">
			  <?php
			  	$counter = 1;
			  	if(!$this->viewer()->getIdentity()){ ?>
				  <div class="sesproduct_checkout_head _active">
					  <span class="no_value"><?php echo $counter;$counter++; ?></span><?php echo $this->translate("Login"); ?>
				  </div>
			  <?php } ?>
			  <div class="sesproduct_checkout_head<?php if($this->viewer()->getIdentity()){ ?> _active<?php } ?>">
				  <span class="no_value"><?php echo $counter++; ?></span><?php echo $this->translate("Billing/Shipping Address"); ?>
			  </div>
              <div class="sesproduct_checkout_head">
				  <span class="no_value"><?php echo $counter++; ?></span><?php echo $this->translate("Shipping Method"); ?>
			  </div>
              <div class="sesproduct_checkout_head">
				  <span class="no_value"><?php echo $counter++; ?></span><?php echo $this->translate("Payment Method"); ?>
			  </div>
              <div class="sesproduct_checkout_head">
				  <span class="no_value"><?php echo $counter++; ?></span><?php echo $this->translate('Order Review'); ?>
			  </div>
			</div>
            <div class="sesproduct_checkout_tabcontent">

				<?php if(!$this->viewer()->getIdentity()){ ?>
				<div class="sesproduct_billing_details sesproduct_checkout_step">
				<?php //Login ?>
				  <div class="sesbasic_clearfix">

						<div class="sesproduct_checkout_body" style="display: block;">
							<div class="sesproduct_checkout_head"><?php echo $this->translate('Login'); ?></div>
							<div class="form-group">
								<input type="radio" id="guestuser" value="guest" name="login_type" checked/>
								<label for="guestuser"><?php echo $this->translate('Checkout as Guest'); ?></label>
							</div>
							<div class="form-group">
								<input type="radio" id="login_signup" value="login" name="login_type"/>
								<label for="login_signup"><?php echo $this->translate("Login or Signup to Checkout"); ?></label>
							</div>
							<div class="submit">
								<a href="javascript:;" class="info_btn auth_logincheck"><?php echo $this->translate("Save & Continue"); ?></a>
								<a href="javascript:void(0);" class="nextprevious firststep" style="display: none;"  data-next="1" data-previous="0"></a>
							</div>
						</div>
				  </div>
				</div>
                ...
                <?php //Address ?>
                <div class="sesproduct_billing_details sesproduct_checkout_step">
                    <?php include APPLICATION_PATH.'/application/modules/Sesproduct/views/scripts/cart/_billingInformation.tpl'; ?>
                </div>
    			<?php //Shipping Methods ?>
    			<div class="sesproduct_checkout_step sesbasic_clearfix shipping_method">			  
    			   <?php //include APPLICATION_PATH.'/application/modules/Sesproduct/views/scripts/cart/shipping-method.tpl'; ?>			  
    			</div>
    			<?php //Payment ?>
    			<div class="sesproduct_checkout_step sesbasic_clearfix payment_methods">    			
    			</div>
    			<?php //Order Review ?>
    			<div class="sesproduct_checkout_step sesbasic_clearfix order_review">			  
    			</div>
            </div>
		</div>
		</form>
	</div>
<!--	<div class="sesproduct_checkout_total">
		<div class="sesproduct_checkout_help">
			<h4><?php //echo $this->translate("Need help?"); ?></h4>
			<ul>
				<li><a href="#"><?php //echo $this->translate("What Payment Methods can i use?"); ?></a></li>
				<li><a href="#"><?php //echo $this->translate("How is My Order Secure?"); ?></a></li>
				<li><a href="#"><?php //echo $this->translate("How do i Know If my Ppromocode Worked?"); ?></a></li>
				<li><a href="#"><?php //echo $this->translate("How do I redeem my gift Card?"); ?></a></li>
			</ul>
		</div>
	</div>-->
</div>
<script type="application/javascript">
    ...
    sesJqueryObject(document).on('change','input[name=payment_type]',function () {
        if(sesJqueryObject(this).val() == "cheque"){
            sesJqueryObject('#sesproduct_check_cnt').show();
        }else {
            sesJqueryObject('#sesproduct_check_cnt').hide();
        }
         if(sesJqueryObject(this).val() == "stripe"){ 
            sesJqueryObject('#sesproduct_stripe_pay').show();
        }else {
            sesJqueryObject('#sesproduct_stripe_pay').hide();
        }
    });
	...
	sesJqueryObject(document).on('click','.back_check',function (e) {
        ...
        sesJqueryObject('.sesproduct_checkout_head').removeClass('_active');
        sesJqueryObject('.sesproduct_checkout_tabs').find('.sesproduct_checkout_head').each(function (index) {
...
sesJqueryObject(document).on('click','.nextprevious',function (e) {
    var parentElem = sesJqueryObject(this).closest('.sesproduct_checkout_step');
    var allChildrens = sesJqueryObject('.sesproduct_checkout_tabcontent').find('.sesproduct_checkout_step');
    var currentIndex = allChildrens.index(parentElem);
    ...
    function paymentMethods(currentIndex) {
		...
		var noShipping = "";
		if(sesJqueryObject("input:checkbox[name=no_shipping]").length) {
            sesJqueryObject("input:checkbox[name=no_shipping]:checked").each(function () {
                noShipping = sesJqueryObject(this).val() + ',';
            });
        }
        ...
        sesJqueryObject.post('sesproduct/cart/place-order',{billingAddress:billingAddress,shippingAddress:shippingAddress,shipping:shipping,checkbox:checkbox,order:2,no_shipping:noShipping},function (response) { console.log(response);
            if(response == "return_to_cart"){
                window.location.href = cartUrl;
                return;
            }
	...
	function order_review(currentIndex,obj){
        ...
        var shipping = sesJqueryObject('.shipping_method').find('input[type=radio]').serialize();
        sesJqueryObject.post('sesproduct/cart/place-order',{order:3,shipping:shipping,billingAddress:billingAddress,shippingAddress:shippingAddress,checkbox:checkbox},function (response) {
            if(response == "return_to_cart"){
                window.location.href = cartUrl;
                return;
            }
...
sesJqueryObject(document).on('click','.sesproduct_checkout_head',function (e) {
    var elems = sesJqueryObject('.sesproduct_checkout_tabs').find('.sesproduct_checkout_head').index(sesJqueryObject(this));


    var parentElem = sesJqueryObject('.sesproduct_checkout_tabcontent').find('.sesproduct_checkout_step').eq(elems);
    var allChildrens = sesJqueryObject('.sesproduct_checkout_tabcontent').find('.sesproduct_checkout_step');
    var currentIndex = allChildrens.index(parentElem);
