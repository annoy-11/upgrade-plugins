 ...
 */
class Sesproduct_Widget_ProfilePhotosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->album_parent_id =  $id = isset($params['album_parent_id']) ? $params['album_parent_id'] : Zend_Controller_Front::getInstance()->getRequest()->getParam('product_id', null);

    $value['product_id'] = $this->view->product_id =  $product_id = $id = Engine_Api::_()->getDbtable('sesproducts', 'sesproduct')->getProductId($id);
    $this->view->product  = $subject = Engine_Api::_()->getItem('sesproduct',$id);
    $value['identityForWidget'] = $this->view->identityForWidget = isset($_POST['identity']) ? $_POST['identity'] : $this->_getParam('widget_id','');
		$this->view->allow_create = true;
    $this->view->widgetId = $this->_getParam('widget_id','');
		if(!$value['is_ajax'] && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesproductpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesproductpackage.enable.package', 1)){
    ...
    $this->view->limit_data = $value['limit_data'] = isset($params['limit_data']) ? $params['limit_data'] : $this->_getParam('limit_data', '10');
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('albums', 'sesproduct')->getAlbumSelect($value);
