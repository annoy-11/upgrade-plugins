 ...
 */
?>
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Seshtmlbackground/externals/styles/animate.min.css'); ?>
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Seshtmlbackground/externals/styles/design7.css'); ?>
  <div class="seshtmlbackground_slideshow_container">
    ...
    <?php 
       ...
       <li class="<?php echo $counter == 0 ? 'selected' : '' ; ?>" style="background-image:url('<?php echo $item->getFilePath('file_id') ?>');">
        ...
        <div class="cd_bg_box">
        <?php if($item->youtube_video_code){  ?>
          <div width="100%" height="100%" class="youtubevideo">
            <?php echo $item->youtube_video_code; ?>
          </div>
       <?php } ?>
        </div>
        </div>
