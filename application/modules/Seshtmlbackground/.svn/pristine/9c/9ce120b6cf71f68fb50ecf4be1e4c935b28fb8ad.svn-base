    ...
    <?php echo $this->translate(array('%s video / photo found.', '%s videos / photos found.', $this->paginator->getTotalItemCount()), $this->locale()->toNumber($this->paginator->getTotalItemCount())) ?>
        ...
        <?php if(count($this->paginator) > 0):?>
              ...
              <li class="item_label" id="slide_<?php echo $item->slide_id ?>">
                ...
                <div style="width:15%;">
                  <?php if($item->thumb_icon): ?>
	                  <img height="100px;" width="100px;" alt="" src="<?php echo Engine_Api::_()->storage()->get($item->thumb_icon, '')->getPhotoUrl(); ?>" style="object-fit:cover; display:block; margin:0 auto;"/>
                  <?php else: ?>
	                  <?php echo "---"; ?>
                  <?php endif; ?>
                </div>
                <div style="width:10%;" class="admin_table_centered">
                  <?php echo strtolower($item->file_type) == 'jpeg' || strtolower($item->file_type) == 'jpg' || strtolower($item->file_type) == 'png' || strtolower($item->file_type) == 'gif' ? 'Photo' : 'Video'; ?>
                </div>
                <div style="width:7.5%;" class="admin_table_centered">
                  <?php 
                  	switch ($item->status){
            ...
            <?php echo $this->htmlLink(
                array('route' => 'admin_default', 'module' => 'seshtmlbackground', 'controller' => 'manage-slide', 'action' => 'delete-slide', 'id' => $item->slide_id, 'type' => strtolower($item->file_type) == 'jpeg' || strtolower($item->file_type) == 'jpg' || strtolower($item->file_type) == 'png' || strtolower($item->file_type) == 'gif' ? 'Photo' : 'Video'),
