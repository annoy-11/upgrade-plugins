...
<?php 
  ...
  $this->headScript()->appendFile($baseURL . 'application/modules/Sesjob/externals/scripts/jquery.js');
  $this->headScript()->appendFile($baseURL . 'application/modules/Sesjob/externals/scripts/owl.carousel.js'); 
?>
<div class="sesjob_jobs_slideshow_wrapper sesbasic_clearfix sesbasic_bxs">
  <div class="sesjob_jobs_slideshow_container sesbasic_clearfix" id="sesjob_jobs_slideshow_<?php echo $this->identity; ?>">
    ...
    ?>
      <div class="_left_col <?php if(empty($this->enableSlideshow)) {?>_norightblock<?php } ?>">
        <?php foreach( $this->paginatorLeft as $item): ?>
          <div class="sesjob_jobs_slideshow_left_item">
            <div class="sesjob_jobs_slideshow_left_item_thumb" style="height:<?php echo $height ?>px;">       
              <?php
              $href = $item->getHref();
              $imageURL = $item->getPhotoUrl('');
              ?>
              <a href="<?php echo $href; ?>" class="sesjob_jobs_slideshow_left_item_thumb_img">
              ...
              <?php $urlencode = urlencode(((!empty($_SERVER["HTTPS"]) &&  strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $item->getHref()); ?>
                <div class="sesjob_jobs_slideshow_item_btns"> 
            ...
            <div class="sesjob_jobs_slideshow_left_item_cont">             
              <?php if(isset($this->titleActive) ){ ?>
                <div class="sesjob_jobs_slideshow_left_item_title">
              ...
              <div class="sesjob_jobs_slideshow_item_stats">
            ...
            <?php if(isset($this->categoryActive)){ ?>
              <?php if($item->category_id != '' && intval($item->category_id) && !is_null($item->category_id)):?> 
                ...
                <?php if($categoryItem):?>
                  <div class="sesjob_jobs_slideshow_cat">
    ...
    <?php if($this->enableSlideshow){ ?>
      <div class="_right_col <?php if(empty($this->leftJob)) {?>_noleftblock<?php } ?>">
        <div autoplay = '<?php echo $this->autoplay ?>' autoplayTimeout = '<?php echo $this->speed ?>' class="sesjob_jobs_slideshow" style="height:<?php echo $this->height?>px;">
          <?php foreach( $this->paginatorRight as $item): ?>
          <div class="sesjob_jobs_slideshow_item sesbasic_clearfix">
            <div class="sesjob_jobs_slideshow_thumb sesjob_thumb" style="height:<?php echo $this->height?>px;">       
              <?php
              $href = $item->getHref();
              $imageURL = $item->getPhotoUrl('');
              ?>
              <a href="<?php echo $href; ?>" class="sesjob_jobs_slideshow_thumb_img">
                <span style="background-image:url(<?php echo $imageURL; ?>);"></span>
              </a>
              <?php if((isset($this->socialSharingActive)  && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesjob.enable.sharing', 1)) || isset($this->likeButtonActive) || isset($this->favouriteButtonActive)):?>
              <?php $urlencode = urlencode(((!empty($_SERVER["HTTPS"]) &&  strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $item->getHref()); ?>
            ...
            <div class="sesjob_jobs_slideshow_cont">             
              <?php if(isset($this->titleActive) ){ ?>
                <div class="sesjob_jobs_slideshow_title">
                  <?php if(strlen($item->getTitle()) > $this->title_truncation){ 
                    $title = mb_substr($item->getTitle(),0,$this->title_truncation).'...';
                    echo $this->htmlLink($item->getHref(),$title) ?>
                  <?php }else{ ?>
              ...
              <?php if($this->descriptionActive): ?>
                <div class="sesjob_jobs_slideshow_des">
