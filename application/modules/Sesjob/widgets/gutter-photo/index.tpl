 ...
 */
 ...
 ?>
<!--sesjob onear photo three-->
<div class="sesjobs_owner_photo">
<div>
  <?php if($this->photoviewtype == 'square'): ?>
    <?php echo $this->htmlLink($this->owner->getHref(), $this->itemPhoto($this->owner),array('class' => 'sesjobs_gutter_photo_square')) ?>
  <?php else: ?>
    <?php echo $this->htmlLink($this->owner->getHref(), $this->itemPhoto($this->owner),array('class' => 'sesjobs_gutter_photo')) ?>
	<?php endif; ?>
  </div>
  <div>
	<?php $user = $this->owner;?>
	...
	<?php echo $this->htmlLink($this->owner->getHref(), $this->owner->getTitle(), array('class' => 'sesjobs_gutter_name')) ?>
    <?php if(isset($data[0]['valuesMeta'])):?>
      <p class="about_content"><?php echo $this->string()->truncate($this->string()->stripTags($data[0]['valuesMeta']), $this->user_description_limit) ?></p>
    <?php endif;?>
    </div>
</div>
