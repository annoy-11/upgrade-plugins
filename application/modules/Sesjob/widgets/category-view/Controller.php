...
class Sesjob_Widget_CategoryViewController extends Engine_Content_Widget_Abstract {
  public function indexAction() {
    ...
    $this->view->limit_data = $limit_data = isset($params['job_limit']) ? $params['job_limit'] : $this->_getParam('job_limit', '10');
    ...
    $show_subcatcriterias = isset($params['show_subcatcriteria']) ? $params['show_subcatcriteria'] : $this->_getParam('show_subcatcriteria', array('countJob', 'icon', 'title'));
    ...
    $this->view->height = $height = isset($params['height']) ? $params['height'] : $this->_getParam('height', '160px');
		$this->view->textJob = $textJob = isset($params['textJob']) ? $params['textJob'] : $this->_getParam('textJob', 'Jobs we love');
    $params = array('viewType' => $this->view->viewType,'job_limit' => $limit_data, 'description_truncation' => $descriptionLimit, 'pagging' => $loadOptionData, 'show_criterias' => $show_criterias,'category_id' => $categoryId, 'width' => $width, 'height' => $height, 'show_subcat' => $show_subcat, 'show_subcatcriteria' => $show_subcatcriterias, 'widthSubcat' => $widthSubcat, 'heightSubcat', $heightSubcat,'textJob'=>$textJob);
    if (Engine_Api::_()->core()->hasSubject()) {
      $this->view->category = $category = Engine_Api::_()->core()->getSubject();
      $category_id = $category->category_id;
    } else {
      $this->view->category = $category = Engine_Api::_()->getItem('sesjob_category', $params['category_id']);
    ...
    if (!$is_ajax) {
      if ($category->subcat_id == 0 && $category->subsubcat_id == 0) {
        ...
        $innerCatData = Engine_Api::_()->getDbtable('categories', 'sesjob')->getModuleSubcategory(array('category_id' => $category->category_id, 'column_name' => '*', 'countJobs' => true));
        $columnCategory = 'category_id';
      } else if ($category->subsubcat_id == 0) {
        $innerCatData = Engine_Api::_()->getDbtable('categories', 'sesjob')->getModuleSubsubcategory(array('countJobs' => true, 'category_id' => $category->category_id, 'column_name' => '*'));
        $columnCategory = 'subcat_id';
      } else
        $columnCategory = 'subsubcat_id';
      $this->view->innerCatData = $innerCatData;
      //breadcum
      $this->view->breadcrumb = $breadcrumb = Engine_Api::_()->getDbtable('categories', 'sesjob')->getBreadcrumb($category);
    }
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('jobs', 'sesjob')->getSesjobsPaginator(array($columnCategory => $category->category_id, 'status' => 1));
