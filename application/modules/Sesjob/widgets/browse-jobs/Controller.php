 ...
 */
class Sesjob_Widget_BrowseJobsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $form = new Sesjob_Form_Search(array('searchTitle' => 'yes','browseBy' => 'yes','categoriesSearch' => 'yes','searchFor'=>'job','FriendsSearch'=>'yes','defaultSearchtype'=>'mostSPliked','locationSearch' => 'yes','kilometerMiles' => 'yes','hasPhoto' => 'yes'));
    ...
    $value['company_id'] =  isset($searchArray['company_id']) ? $searchArray['company_id'] : (isset($_GET['company_id']) ? $_GET['company_id'] : (isset($params['company_id']) ? $params['company_id'] : ''));

    $value['industry_id'] =  isset($searchArray['industry_id']) ? $searchArray['industry_id'] : (isset($_GET['industry_id']) ? $_GET['industry_id'] : (isset($params['industry_id']) ? $params['industry_id'] : ''));

    $value['employment_id'] =  isset($searchArray['employment_id']) ? $searchArray['employment_id'] : (isset($_GET['employment_id']) ? $_GET['employment_id'] : (isset($params['employment_id']) ? $params['employment_id'] : ''));
    ...
    if (!$is_ajax) {
        $this->view->optionsEnable = $optionsEnable = $this->_getParam('enableTabs', array('list','grid', 'pinboard', 'map'));
    ...
    $type = '';
    $page_id = Engine_Api::_()->sesjob()->getWidgetPageId($identityForWidget);
    if($page_id) {
      ...
      if($pageName) {
        ...
        if(is_numeric($explode[1])) {
          $type = Engine_Db_Table::getDefaultAdapter()->select()
                ->from('engine4_sesjob_integrateothermodules', 'content_type')
    ...
    $value['status'] = 1;
    $value['search'] = 1;
    $this->view->widgetName = 'browse-jobs';
    ...
    if($value['moduleName'] == 'core') {
      unset($params['text']);
      $params['tag'] = Engine_Api::_()->getDbTable('tags', 'core')->getTagId($text);
    }
    
    // Get jobs
    $paginator = Engine_Api::_()->getDbtable('jobs', 'sesjob')->getSesjobsPaginator($params, $form->getValues());
