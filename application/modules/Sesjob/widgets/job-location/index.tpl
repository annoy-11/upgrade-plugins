...
<?php $randonNumber = "location_widget_sesjob"; ?>
...
<?php foreach( $this->paginator as $job ): ?>
  ...
  <?php $location = '';?>
    ...
    <?php $socialShare = $this->partial('_socialShareIcons.tpl','sesbasic',array('resource' => $job, 'param' => 'feed', 'socialshare_enable_plusicon' => $this->socialshare_enable_plusicon, 'socialshare_icon_limit' => $this->socialshare_icon_limit)); ?>

    

     <?php  $socialshare = '<div class="sesjob_list_grid_thumb_btns">'.$socialShare.$likeButton.$favouriteButton.'</div>';?>
...
<?php if(!$this->is_ajax){ ?>
  ...
  <script type="text/javascript">
	...
	'<div class="sesjob_large_map_content_title">'+markerArrayData_<?php echo $randonNumber?>[i]['title']+'</div>'+ owner + location+stats +'<div class="sesjob_list_stats_btn clearfix clear"><span>'+'</span><span>'+'</span><span>'+'</span><span>'+'</span></div></div></div>'+'<a class="icn close" href="javascript:;" title="Close"><i class="fa fa-close"></i></a>' + 
...
<?php if(!$this->is_ajax){ ?>
	...
	<div id="map-data_location_widget_sesjob" style="display:none;"><?php echo json_encode($locationArray,JSON_HEX_QUOT | JSON_HEX_TAG); ?></div>

	<div id="map-canvas-location_widget_sesjob" class="map sesbasic_large_map sesbm sesbasic_bxs sesjob_browse_map"></div>
