...
<?php if(!$this->is_ajax){ ?>
  <div class="sesbasic_tabs_container sesbasic_clearfix sesbasic_bxs">
    <div class="sesbasic_tabs sesbasic_clearfix" <?php if(count($this->defaultOptions) ==1){ ?> style="display:none" <?php } ?>> 
    	<ul id="tab-widget-sesjob-<?php echo $randonNumber; ?>">
         <?php 
        ...
        <?php if($this->can_create) { ?>
          <li>
            <a href="<?php echo $this->url(array('action' => 'create', 'resource_type' => $this->resource_type, 'resource_id' => $this->resource_id), "sesjob_general"); ?>"><?php echo $this->translate("Post New Job"); ?></a>
          </li>
        <?php } ?>
      </ul>
    </div>
  ...
  <script type="application/javascript"> 
	...
	requestTab_<?php echo $randonNumber; ?> = new Request.HTML({
	  ...
	  onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
	    ...
	    if(document.getElementById('sesjob_pinboard_view_<?php echo $randonNumber;?>') && sesJqueryObject('.sesbasic_view_type_options_<?php echo $randonNumber; ?>').find('.active').attr('rel') == 'pinboard') {
