 ...
 * @version    $Id: install.php  2019-03-27 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
 
class Sesjob_Installer extends Engine_Package_Installer_Module {
  ...
  protected function _addUserProfileContent() {
    ...
    $select
      ...
      ->where('type = ?', 'widget')
      ->where('name = ?', 'sesjob.profile-sesjobs')
      ;
    $info = $select->query()->fetch();
    ...
    if( empty($info) ) {
      ...
      $db->insert('engine4_core_content', array(
        ...
        'order'   => 6,
        'params'  => '{"title":"Sesjobs","titleCount":true}',
      ));
  ...
  public function onPostInstall() {
    ...
    $singularWord = $db->select()
                      ...
                      ->where('name = ?', 'sesjob.text.singular')
                      ->limit(1)
                      ->query()
                      ->fetchColumn();
	  if(!$singularWord)
