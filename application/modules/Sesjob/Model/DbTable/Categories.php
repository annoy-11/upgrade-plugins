 ...
 * @version    $Id: Categories.php  2019-03-27 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesjob_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Sesjob_Model_Category';
  ...
  public function getCategoriesAssoc($params = array()) {
    ...
    if ($viewerId && isset($params['member_levels']) && $params['member_levels'] == 1) {
      ...
      $stmt->where('CONCAT(engine4_sesjob_categories.member_levels," ") LIKE "%' . $levelId . '%"');
    }
  ...
  public function getUserCategoriesAssoc($user) {
    ...
    $stmt = $this->getAdapter()
            ->select()
            ->from('engine4_sesjob_categories', array('category_id', 'category_name'))
            ->joinLeft('engine4_sesjob_jobs', "engine4_sesjob_jobs.category_id = engine4_sesjob_categories.category_id")
            ->group("engine4_sesjob_categories.category_id")
            ->where('engine4_sesjob_jobs.owner_id = ?', $user)
            ->where('engine4_sesjob_jobs.draft = ?', "0")
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countJobs'])) {
      $sesjobTable = Engine_Api::_()->getDbTable('jobs', 'sesjob')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($sesjobTable, "$sesjobTable.subcat_id=$tableName.category_id", array("total_jobs_categories" => "COUNT(job_id)"));
      $category_select = $category_select->group("$tableName.category_id");
      $category_select->order('total_jobs_categories DESC');
    }
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countJobs'])) {
      $sesjobTable = Engine_Api::_()->getDbTable('jobs', 'sesjob')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($sesjobTable, "$sesjobTable.subsubcat_id=$tableName.category_id", array("total_jobs_categories" => "COUNT(job_id)"));
  ...
  public function getCategory($params = array(), $customParams = array(), $searchParams = array()) {
    ...
    $jobTable = Engine_Api::_()->getDbTable('jobs', 'sesjob')->info('name');

    if ((isset($params['hasJob']) && $params['hasJob']) || isset($params['countJobs']) || (isset($params['criteria']) && $params['criteria'] == 'most_job')) {
      $take = true;
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($jobTable, "$jobTable.category_id=$tableName.category_id", array("total_jobs_categories" => "COUNT(job_id)"));
      if (empty($params['countJobs'])) {
        $category_select = $category_select->having("COUNT($jobTable.category_id) > 0")
                ->group("$jobTable.category_id");
        if (isset($params['jobDesc'])) {
          if ($params['criteria'] && $params['criteria'] == 'most_job') {
            $category_select->order('total_jobs_categories DESC');
          }
        }
      } else {
...
//             ->joinLeft($jobTableName, "$jobTableName.category_id=$tableName.category_id", array("total_jobs_categories" => "COUNT(job_id)"))
//             ->where($tableName . '.subcat_id = ?', 0)
//             ->where($tableName . '.subsubcat_id = ?', 0)
//             ->group("$jobTableName.category_id")
            ->order('order DESC');
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('sesjob_category');
  ...
  public function hasCategoryId($params = array()) {
    ...
    $category_select = $this->select()
            ->from($tableName, 'category_id')
            ->where('sejob_categoryid = ?', $params['sejob_categoryid']);
    return $category_select->query()->fetchColumn();
  }
