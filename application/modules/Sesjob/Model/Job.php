 ...
 * @version    $Id: Job.php  2019-03-27 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesjob_Model_Job extends Core_Model_Item_Abstract
{
  // Properties
  ...
  public function getHref($params = array()) {
    ...
    $params = array_merge(array(
      'route' => 'sesjob_entry_view',
      'reset' => true,
     // 'user_id' => $this->owner_id,
      'job_id' => $this->custom_url,
      //'slug' => $slug,
    ), $params);
    $route = $params['route'];
    $reset = $params['reset'];
    ...
    if ($photo instanceof Zend_Form_Element_File) {
      ...
      throw new Sesjob_Model_Exception('invalid argument passed to setPhoto');
    }
    ...
    $this->save();
		if($direct == ''){
			...
			$jobAlbum = $this->getSingletonAlbum();
			$jobAlbum->title = Zend_Registry::get('Zend_Translate')->_('Untitled');
			$jobAlbum->owner_id = Engine_Api::_()->user()->getViewer()->getIdentity();
			$jobAlbum->save();
			$photoItem = $photoTable->createRow();
			$photoItem->setFromArray(array(
					'job_id' => $this->getIdentity(),
					'album_id' => $jobAlbum->getIdentity(),
					'user_id' => $viewer->getIdentity(),
					'file_id' => $iMain->getIdentity(),
					'collection_id' => $jobAlbum->getIdentity(),
					'user_id' => $viewer->getIdentity(),
			));
			$photoItem->save();
		}
    ...
    $table = Engine_Api::_()->getItemTable('sesjob_album');
    $select = $table->select()
	...
	public function cancel()
  {
	 ...
	 //update transaction_id to other job of same package job
	 if($this->transaction_id){
		$transaction = $this->getTransaction();
		$table = Engine_Api::_()->getDbTable('jobs','sesjob');
		$tableName = $table->info('name');
		//select job in package with our transaction id.
		...
		if($job){
			$job->transaction_id = $this->transaction_id;
			$job->save();
			//update order
			$order_id = $transaction->order_id;
			$order = Engine_Api::_()->getItem('payment_order',$order_id);
			if($order){
				$order->source_id = $job->getIdentity();
				$order->save();
			}
			//update item count in order package
			$orderpackage = Engine_Api::_()->getItem('sesjobpackage_orderspackage',$this->orderspackage_id);
    ...
    if( !empty($transaction->gateway_id) && !empty($transaction->gateway_profile_id) && empty($transaction->gateway_transaction_id) ) {
      try {
        ...
        $gateway = Engine_Api::_()->getItem('sesjobpackage_gateway', $transaction->gateway_id);
        if( $gateway ) {
          $gatewayPlugin = $gateway->getPlugin();
          if( method_exists($gatewayPlugin, 'cancelJob') ) {
            $gatewayPlugin->cancelJob($transaction->gateway_profile_id);
          }
        }
      } catch( Exception $e ) {
        // Silence?
  ...
  public function getPackage(){
		return Engine_Api::_()->getItem('sesjobpackage_package',$this->package_id);
	}
	 public function getTransaction(){
		return Engine_Api::_()->getItem('sesjobpackage_transaction',$this->transaction_id);
	}
	 // Events
  ...
  public function setActive($flag = true, $deactivateOthers = null)
  {
	...
	public function changeApprovedStatus($approved = 0){
		...
		if($transaction && $orderPackageId){
				...
				$this->save();
				Engine_Api::_()->getDbtable('jobs', 'sesjob')->update(array('is_approved' =>$approved),array('orderspackage_id'=>$orderPackageId));
		}
	}
     ...
     if( in_array($transaction->state, array('initial', 'trial', 'pending', 'active')) ) {
      ...
      if( $expiration ) {
        $expiration_date = date('Y-m-d H:i:s', $expiration);
				...
				//make it a future job(never expired)
  ...
  public function getPhotoUrl($type = null) {
    ...
    if ($photo_id) {
      $file = Engine_Api::_()->getItemTable('storage_file')->getFile($this->photo_id, $type);
		...
		$defaultPhoto = Zend_Registry::get('StaticBaseUrl').$settings->getSetting('sesjob_job_default_photo', 'application/modules/Sesjob/externals/images/nophoto_job_thumb_profile.png');
		return $defaultPhoto;
  }
