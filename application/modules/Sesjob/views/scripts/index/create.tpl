 ...
 * @version    $Id: create.tpl  2019-03-27 00:00:00 SocialEngineSolutions $
...
<?php if(!$this->typesmoothbox){ ?>
  <script type="application/javascript">
    window.addEvent('load', function(){
      ...
      document.getElementById('description').value = desVal.match(/[^<p>].*[^</p>]/g);
    });
    <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesjob.company', 1)) { ?>
    window.addEvent('load', function(){   
      tinymce.execCommand('mceRemoveEditor',true, 'company_description');
      var desVal = document.getElementById('company_description').value;
      document.getElementById('company_description').value = desVal.match(/[^<p>].*[^</p>]/g);
    });
    <?php } ?>
  </script>
  <?php
...
<?php } else { ?>
  <script type="application/javascript">
    ...
    Sessmoothbox.css.push("<?php echo $this->layout()->staticBaseUrl . 'application/modules/Sesjob/externals/styles/styles.css'; ?>");
...
<script type="text/javascript">
  ...
  function choosecompany(value) {
    if(value == 0) {
      sesJqueryObject('#company_name').val('');
      sesJqueryObject('#company_websiteurl').val('');
      sesJqueryObject('#company_description').val('');
      sesJqueryObject('#industry_id').val('');
    } else {
      var url = en4.core.baseUrl + 'sesjob/company/getcompanyinformation/company_id/' + value;
      new Request.JSON({
        url: url,
        data: {'company_id':value},
        onSuccess: function(responseJSON) {
          sesJqueryObject('#company_name').val(responseJSON.company_name);
          sesJqueryObject('#company_websiteurl').val(responseJSON.company_websiteurl);
          sesJqueryObject('#company_description').val(responseJSON.company_description);
          sesJqueryObject('#industry_id').val(responseJSON.industry_id);
        }
      }).send();
    }
  }
  ...
  en4.core.runonce.add(function() {
      var contentAutocomplete = new Autocompleter.Request.JSON('job_contact_name', "<?php echo $this->url(array('module' => 'sesjob', 'controller' => 'index', 'action' => 'getusers'), 'default', true) ?>", {
        'postVar': 'text',
        'minLength': 1,
        'selectMode': 'pick',
      ...
      contentAutocomplete.addEvent('onSelection', function(element, selected, value, input) {
        $('job_contact_email').value = selected.retrieve('autocompleteChoice').email;
        $('job_contact_email').value = selected.retrieve('autocompleteChoice').email;
      });
    });
</script>
...
if ($mainPhotoEnable == 1) {
	$required = true; ?>
  <style type='text/css'>
  .sesjob_create #tabs_form_jobcreate-label label:after{content: '*';color: #F00;}
  </style>
<?php } else {
	$required = false;
}
...
<script type="text/javascript">
  ...
  function checkAvailsbility(submitform) {
    ...
    return;
    jqueryObjectOfSes('#sesjob_custom_url_wrong').hide();
    jqueryObjectOfSes('#sesjob_custom_url_correct').hide();
    jqueryObjectOfSes('#sesjob_custom_url_loading').css('display','inline-block');
    jqueryObjectOfSes.post('<?php echo $this->url(array('controller' => 'index','module'=>'sesjob', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value},function(response){
      jqueryObjectOfSes('#sesjob_custom_url_loading').hide();
      response = jqueryObjectOfSes.parseJSON(response);
      if(response.error){
        jqueryObjectOfSes('#sesjob_custom_url_correct').hide();
        jqueryObjectOfSes('#sesjob_custom_url_wrong').css('display','inline-block');
        ...
        jqueryObjectOfSes('#sesjob_custom_url_wrong').hide();
        jqueryObjectOfSes('#sesjob_custom_url_correct').css('display','inline-block');
        if(typeof submitform != 'undefined') {
  ...
  <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesjob_enable_location', 1)){ ?>
    en4.core.runonce.add(function() {
...
<?php if (($this->current_count >= $this->quota) && !empty($this->quota)):?>
  <div class="tip">
    <span>
      ...
      <?php echo $this->translate('If you would like to upload a new entry, please <a href="%1$s">delete</a> an old one first.', $this->url(array('action' => 'manage'), 'sesjob_general'));?>
    </span>
  </div>
  <br/>
<?php else:?>
<div class="sesjob_create sesbasic_bxs">
...
<?php echo $this->partial('_customFields.tpl', 'sesjob', array()); ?>
...
<script type="text/javascript">
  ...
  var getProfileType = function(category_id) {
    var mapping = <?php echo Zend_Json_Encoder::encode(Engine_Api::_()->getDbTable('categories', 'sesjob')->getMapping(array('category_id', 'profile_type'))); ?>;
  ...
  function showSubCategory(cat_id,selectedId) {
    ...
    var url = en4.core.baseUrl + 'sesjob/index/subcategory/category_id/' + cat_id;
    new Request.HTML({
      url: url,
  ...
  function showSubSubCategory(cat_id,selectedId,isLoad) {
    ...
    if(selectedId != '')
    var selected = selectedId;
    var url = en4.core.baseUrl + 'sesjob/index/subsubcategory/subcategory_id/' + cat_id;
    (new Request.HTML({
      url: url,
      data: {'selected':selected},
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
  ...
  jqueryObjectOfSes("#sesjobs_create").bind("keypress", function (e) {
  ...
  function validateForm() {
    ...
    jqueryObjectOfSes('#sesjobs_create input, #sesjobs_create select,#sesjobs_create checkbox,#sesjobs_create textarea,#sesjobs_create radio').each(
      function(index){
	...
	if(jqueryObjectOfSes(this).closest('div').parent().css('display') != 'none' && jqueryObjectOfSes(this).closest('div').parent().find('.form-label').find('label').first().hasClass('required') && jqueryObjectOfSes(this).prop('type') != 'hidden' && jqueryObjectOfSes(this).closest('div').parent().attr('class') != 'form-elements'){	
	  ...
	  if(error){
	    ...
	    counter++
			...
			if(sesJqueryObject('#tabs_form_jobcreate-wrapper').length && sesJqueryObject('.sesjob_upload_item_photo').length == 0){
				<?php if($required):?>
					objectError = sesJqueryObject('.sesjob_create_form_tabs');
					error = true;
				<?php endif;?>
			}		
	  }
...
en4.core.runonce.add(function() {
  jqueryObjectOfSes('#sesjobs_create').submit(function(e) {
    var validationFm = validateForm();
    if(!validationFm) {
			var lastTwoDigitStart = sesJqueryObject('#sesjob_schedule_time').val().slice('-2');
			var startDate = new Date(sesJqueryObject('#sesjob_schedule_date').val()+' '+sesJqueryObject('#sesjob_schedule_time').val().replace(lastTwoDigitStart,'')+':00 '+lastTwoDigitStart);
			var error = checkDateTime(startDate);
			if(error != ''){
				sesJqueryObject('#event_error_time-wrapper').show();
				sesJqueryObject('#sesjob_schedule_error_time-element').text(error);
    ...
    if(validationFm) {
      ...
      return false;	
    }else if(sesJqueryObject('.sesjob_upload_item_abort').length){
...
en4.core.runonce.add(function() {
  var sesjob_create_form_tabsSesjob = jqueryObjectOfSes('#sesjob_create_form_tabs li a');
  sesjob_create_form_tabsSesjob.click(function() {
...
function createStatusbar(obj,file)
{
     ...
     if(rowCount %2 ==0) row ="even";
		  var checkedId = jqueryObjectOfSes("input[name=cover]:checked");
			this.objectInsert = jqueryObjectOfSes('<div class="sesjob_upload_item sesbm '+row+'"></div>');
			this.overlay = jqueryObjectOfSes("<div class='overlay sesjob_upload_item_overlay'></div>").appendTo(this.objectInsert);
			this.abort = jqueryObjectOfSes('<div class="abort sesjob_upload_item_abort" id="abortPhoto_'+countUploadSes+'"><span><?php echo $this->translate("Cancel Uploading"); ?></span></div>').appendTo(this.objectInsert);
			this.progressBar = jqueryObjectOfSes('<div class="overlay_image progressBar"><div></div></div>').appendTo(this.objectInsert);
			this.imageContainer = jqueryObjectOfSes('<div class="sesjob_upload_item_photo"></div>').appendTo(this.objectInsert);
			this.src = jqueryObjectOfSes('<img src="'+en4.core.baseUrl+'application/modules/Sesjob/externals/images/blank-img.gif">').appendTo(this.imageContainer);
			this.infoContainer = jqueryObjectOfSes('<div class=sesjob_upload_photo_info sesbasic_clearfix"></div>').appendTo(this.objectInsert);
			this.size = jqueryObjectOfSes('<span class="sesjob_upload_item_size sesbasic_text_light"></span>').appendTo(this.infoContainer);
			this.filename = jqueryObjectOfSes('<span class="sesjob_upload_item_name"></span>').appendTo(this.infoContainer);
			this.option = jqueryObjectOfSes('<div class="sesjob_upload_item_options clear sesbasic_clearfix"><span class="sesjob_upload_item_radio"><input type="radio" id="main_photo_id'+rowCount+'" name="cover"><label for="main_photo_id'+rowCount+'"><?php echo $this->translate("Main Photo"); ?></label></span><a class="edit_image_upload" href="javascript:void(0);"><i class="fa fa-pencil"></i></a><a class="delete_image_upload" href="javascript:void(0);"><i class="fa fa-trash"></i></a></div>').appendTo(this.objectInsert);
		...
		if (1) {
			if(jqueryObjectOfSes('#show_photo').children('div').length == 1) {
				var idPhoto = jqueryObjectOfSes('#show_photo').eq(0).find('.sesjob_upload_item_radio').find('input').attr('id');
...
function sendFileToServer(formData,status,file,isURL,i)
{
		...
		var url = '&isURL='+urlIs;
    var uploadURL = en4.core.baseUrl + 'sesjob/photo/upload' + '?ul=1'+url; //Upload URL
    ...
    processData: false,
		...
		success: function(response){
					...
					if (response.status) {
							...
							status.option.find('.sesjob_upload_item_radio').find('input').attr('value',response.photo_id);
...
var dragandrophandlerSesjob = sesJqueryObject('#dragandrophandler');
dragandrophandlerSesjob.click(function(){
	document.getElementById('file_multi').click();	
});
...
jqueryObjectOfSes(document).on('click','.edit_image_upload',function(e){
  e.preventDefault();
  var photo_id = jqueryObjectOfSes(this).closest('.sesjob_upload_item_options').attr('data-src');
  if(photo_id){
    editImage(photo_id);
  }else
    return false;
...
jqueryObjectOfSes(document).on('click','.delete_image_upload',function(e){
	e.preventDefault();
	jqueryObjectOfSes(this).parent().parent().find('.sesjob_upload_item_overlay').css('display','block');
	var sesthat = this;
	var isCover = jqueryObjectOfSes(this).closest('.sesjob_upload_item_options').find('.sesjob_upload_item_radio').find('input').prop('checked');
	var photo_id = jqueryObjectOfSes(this).closest('.sesjob_upload_item_options').attr('data-src');
	if(photo_id){
		request = new Request.JSON({
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'sesjob', 'controller' => 'index', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
   'onSuccess' : function(responseJSON) {
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'sesjob', 'controller' => 'index', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
  
  function showPreview(value) {
    if(value == 1)
    en4.core.showError('<a class="icon_close"><i class="fa fa-close"></i></a> <p class="popup_design_title">'+en4.core.language.translate("Design 1")+'</p><img class="popup_img" src="./application/modules/Sesjob/externals/images/layout_1.jpg" alt="" />');
    else if(value == 2)
    en4.core.showError('<a class="icon_close"><i class="fa fa-close"></i></a> <p class="popup_design_title">'+en4.core.language.translate("Design 2")+'</p><img src="./application/modules/Sesjob/externals/images/layout_2.jpg" alt="" />');
    else if(value == 3)
    en4.core.showError('<a class="icon_close"><i class="fa fa-close"></i></a> <p class="popup_design_title">'+en4.core.language.translate("Design 3")+'</p><img src="./application/modules/Sesjob/externals/images/layout_3.jpg" alt="" />');
    else if(value == 4)
    en4.core.showError('<a class="icon_close"><i class="fa fa-close"></i></a> <p class="popup_design_title">'+en4.core.language.translate("Design 4")+'</p><img src="./application/modules/Sesjob/externals/images/layout_4.jpg" alt="" />');
    return;
  }
  $$('.core_main_sesjob').getParent().addClass('active');
...
<?php if($this->typesmoothbox) { ?>
	...
	function showHideOptionsSesjob(display){
		var elem = sesJqueryObject('.sesjob_hideelement_smoothbox');
		for(var i = 0 ; i < elem.length ; i++){
				sesJqueryObject(elem[i]).parent().parent().css('display',display);
		}
	}
	function checkSettingSesjob(first){
		var hideShowOption = sesJqueryObject('#advanced_sesjoboptions').hasClass('active');
			if(hideShowOption){
					showHideOptionsSesjob('none');
					if(typeof first == 'undefined'){
						sesJqueryObject('#advanced_sesjoboptions').html("<i class='fa fa-plus-circle'></i><?php echo $this->translate('Show Advanced Settings') ?>");
					}
					sesJqueryObject('#advanced_sesjoboptions').removeClass('active');
			}else{
					showHideOptionsSesjob('block');
					sesJqueryObject('#advanced_sesjoboptions').html("<i class='fa fa-minus-circle'></i><?php echo $this->translate('Hide Advanced Settings') ?>");
						sesJqueryObject('#advanced_sesjoboptions').addClass('active');
			}	
	}
	en4.core.runonce.add(function()
  {
		sesJqueryObject('#advanced_sesjoboptions').click(function(e){
			checkSettingSesjob();
		});
		sesJqueryObject('#advanced_sesjoboptions').html("<i class='fa fa-plus-circle'></i><?php echo $this->translate('Show Advanced Settings') ?>");
		checkSettingSesjob('true');
