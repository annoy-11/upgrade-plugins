 ...
 * @version    $Id: _showCompanyListGrid.tpl  2019-03-27 00:00:00 SocialEngineSolutions $
...
<?php foreach( $this->paginator as $item ):?>
  ...
  <?php if($this->view_type == 'grid'){ ?>
    <?php include APPLICATION_PATH .  '/application/modules/Sesjob/views/scripts/company/_gridView.tpl';?>
  <?php }else if($this->view_type == 'list'){ ?>
    <?php include APPLICATION_PATH .  '/application/modules/Sesjob/views/scripts/company/_listView.tpl';?>
  <?php } ?>
<?php endforeach; ?>
...
<?php  if(  $this->paginator->getTotalItemCount() == 0 &&  (empty($this->widgetType))){  ?>
  <?php if( isset($this->category) || isset($this->tag) || isset($this->text) ):?>
    <div class="tip">
      <span>
	<?php echo $this->translate('Nobody has created a companies with that criteria.');?>
      </span>
    </div>
  <?php else:?>
    <div class="tip">
      <span>
	<?php echo $this->translate('Nobody has created a companies yet.');?>
      </span>
    </div>
  <?php endif; ?>
