 ...
 * @version    $Id: job-role.tpl  2019-03-27 00:00:00 SocialEngineSolutions $
...
<div class="sesbasic_dashboard_content sesjob_manage_role_form sesbm sesbasic_clearfix">
	<div class="sesjob_manage_role_form_top sesbasic_clearfix">
		<p class="heading_desc"><?php echo $this->translate('Below, you can add admins to your job who all will be able to do anything on your job as you do including editing, creating sub job, etc.');?></p>
		<?php endif; ?>
		<form id="job_admin_form" action="<?php echo $this->url(array('action' => 'save-job-admin', 'job_id' => $this->job->job_id), 'sesjob_dashboard', true) ?>" method="post">
			<div id="manage_admin_input">
				<div class="sesjob_manage_roles_item">
					<span class="show_img" id="show_default_img"></span>
					<input type='text' id="job_admin" name='job_admin' size='20' placeholder='<?php echo $this->translate('Type Member Name') ?>' />
					<input type="hidden" id="user_id" name="job_admins[]" value=""/>
				</div>
			</div>
...
<?php if(!$this->is_ajax){ ?>
	...
	<div class="sesjob_footer_contant">
		...
		<p><?php echo $this->translate('Here, you can see a list of admins added by you for this job.');?></p>
		<div id="manage_admin">
          ...
          <?php if($jobAdmin->user_id != $this->job->owner_id):?>
...
<script type="text/javascript">
	function showAutosuggest(jobAdmin, imageId) {
	  var contentAutocomplete1 =  'contentAutocomplete-'+jobAdmin
		contentAutocomplete1 = new Autocompleter.Request.JSON(jobAdmin, "<?php echo $this->url(array('module' => 'sesjob', 'controller' => 'dashboard', 'action' => 'get-members', 'job_id' => $this->job->job_id), 'default', true) ?>", {
			'postVar': 'text',
			'minLength': 1,
			'selectMode': '',
      ...
      $(imageId).innerHTML = selected.retrieve('autocompleteChoice').photo;
			sesJqueryObject('#'+jobAdmin).attr('rel', selected.retrieve('autocompleteChoice').id);
			sesJqueryObject('#save_button_admin').removeAttr('disabled');
		});
	}
	en4.core.runonce.add(function() {
	  showAutosuggest('job_admin','show_default_img');
	});
	
	function saveForm() {
	  var UserIds = $('user_id').value;
		new Request.HTML({
			url : en4.core.baseUrl + 'sesjob/dashboard/save-job-admin/job_id/' + <?php echo $this->job->job_id ?>,
			method: 'post',
			data : {
				format : 'html',
				data: UserIds,
	...
	sesJqueryObject(document).on('keyup', 'input[id^="job_admin"]', function(event) {
  ...
  function removeUser(jobId, roleId) {
		new Request.JSON({
			url : en4.core.baseUrl + 'sesjob/dashboard/delete-job-admin',
			method: 'post',
			data : {
				format : 'json',
				role_id: roleId,
