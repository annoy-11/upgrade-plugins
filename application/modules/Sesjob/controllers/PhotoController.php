 ...
 * @version    $Id: PhotoController.php  2019-03-27 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesjob_PhotoController extends Core_Controller_Action_Standard {

  public function init() {
    if (!Engine_Api::_()->core()->hasSubject()) {
      if (0 !== ($photo_id = (int) $this->_getParam('photo_id')) &&
              null !== ($photo = Engine_Api::_()->getItem('sesjob_photo', $photo_id))) {
        Engine_Api::_()->core()->setSubject($photo);
      } else if (0 !== ($job_id = (int) $this->_getParam('job_id')) &&
              null !== ($job = Engine_Api::_()->getItem('sesjob_job', $event_id))) {
  ...
  public function rotateAction() {
    if (!$this->_helper->requireSubject('sesjob_photo')->isValid())
      return;
		$job_id = $this->_getParam('job_id');
		$job = Engine_Api::_()->getItem('sesjob_job', $job_id);
    if (!$this->_helper->requireAuth()->setAuthParams($job, null, 'edit')->isValid())
      return;
    ...
    $photo = Engine_Api::_()->core()->getSubject('sesjob_photo');
   ...
   if (!$this->_helper->requireSubject('sesjob_photo')->isValid())
    ...
    $this->view->href = $photo->getPhotoUrl();
  	...
  	public function correspondingImageAction(){
		...
		$this->view->paginator = $paginator = Engine_Api::_()->getDbtable('photos', 'sesjob')->getPhotoSelect(array('album_id'=>$album_id,'limit_data'=>100));
	}
  public function uploadAction() {

    if (isset($_GET['ul']) || isset($_FILES['Filedata']))
    return $this->_forward('upload-photo', null, null, array('format' => 'json', 'job_id'=> 0));
  }
  public function viewAction() {
    ...
    $this->view->photo = $photo = Engine_Api::_()->core()->getSubject();
		$job_id = $this->_getParam('job_id');
		$job = Engine_Api::_()->getItem('sesjob_job', $job_id);
    if (!$this->_helper->requireAuth()->setAuthParams($job, null, 'view')->isValid()) {
      return;
    }
  ...
  public function uploadPhotoAction() {
 		 if (!$this->_helper->requireAuth()->setAuthParams('sesjob_job', null, 'create')->isValid())
      return;
    ...
    $sesjobPhotoTable = Engine_Api::_()->getDbtable('photos', 'sesjob');

    $db = $sesjobPhotoTable->getAdapter();
    $db->beginTransaction();
		$session = new Zend_Session_Namespace();
    try {
      $viewer = Engine_Api::_()->user()->getViewer();
      if(empty($session->album_id)) {
			$album = Engine_Api::_()->getItemTable('sesjob_album')->createRow();
      	...
      	$album_id = $session->album_id;
				$album = Engine_Api::_()->getItem('sesjob_album', $album_id);
		 }
      $params = array(
      ...
      $photo = Engine_Api::_()->sesbasic()->setPhoto($uploadSource, $isURL,false,'sesjob','sesjob_job',$params,$album);
  ...
  public function editAction()
  {
    ...
    $this->view->form = $form = new Sesjob_Form_Photo_Edit();
  ...
  public function deleteAction() {
    $photo = Engine_Api::_()->core()->getSubject();
    $job = $photo->getParent('sesjob_job');
		$album_id = $photo->album_id;
    if (!$this->_helper->requireAuth()->setAuthParams($job, null, 'edit')->isValid()) {
      return;
    }

    $this->view->form = $form = new Sesjob_Form_Photo_Delete();
