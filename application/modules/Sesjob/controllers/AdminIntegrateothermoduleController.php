...
class Sesjob_AdminIntegrateothermoduleController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesjob_admin_main', array(), 'sesjob_admin_main_integrateothermodule');

    $this->view->enabledModules = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();

    $select = Engine_Api::_()->getDbtable('integrateothermodules', 'sesjob')->select();
  ...
  public function addmoduleAction() {
    ...
    $this->view->form = $form = new Sesjob_Form_Admin_Manage_Add();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      $integrateothermoduleTable = Engine_Api::_()->getDbtable('integrateothermodules', 'sesjob');
      ...
      try {
        ...
        $dbInsert->query('INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `enabled`, `custom`, `order`) VALUES ("sesjob_main_browsejob_'.$row->getIdentity().'", "'.$modulename.'", "Browse Jobs", "", \'{"route":"sesjob_browsejob_'.$row->getIdentity().'","action":"browse-jobs", "resource_type":"'.$values['content_type'].'"}\', "'.$modulename.'_main", "", 1, 0, 999)');

        $this->createBrowseJobPage($modulename, $row->getIdentity());
  ...
  function createBrowseJobPage($modulename, $id) {
    ...
    if( !$page_id ) {
      ...
      $db->insert('engine4_core_pages', array(
        'name' => 'sesjob_index_'.$id,
        'displayname' => 'SES - Advanced Jobs - '.ucfirst($modulename).' Jobs Browse Page',
        'title' => ucfirst($modulename) .' Job Browse',
        'description' => 'This page lists jobs.',
        'custom' => 0,
      ));
  ...
  public function deleteAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $inttable = Engine_Api::_()->getItem('sesjob_integrateothermodule', $this->_getParam('integrateothermodule_id'));
        $pageName = "sesjob_index_".$this->_getParam('integrateothermodule_id');
        if (!empty($pageName)) {
          $page_id = $db->select()
                  ->from('engine4_core_pages', 'page_id')
                  ->where('name = ?', $pageName)
        ...
        Engine_Api::_()->getDbtable('menuItems', 'core')->delete(array('name =?' => 'sesjob_main_browsejob_' . $this->_getParam('integrateothermodule_id')));

        $integrateothermodule = Engine_Api::_()->getItem('sesjob_integrateothermodule', $this->_getParam('integrateothermodule_id'));
  ...
  public function enabledAction() {
    $db = Engine_Db_Table::getDefaultAdapter();
    $db->beginTransaction();
    $content = Engine_Api::_()->getItemTable('sesjob_integrateothermodule')->fetchRow(array('integrateothermodule_id = ?' => $this->_getParam('integrateothermodule_id')));
    try {

      Engine_Api::_()->getDbtable('menuItems', 'core')->update(array('enabled' => !$content->enabled), array('name =?' => 'sesjob_main_browsejob_' . $this->_getParam('integrateothermodule_id')));
