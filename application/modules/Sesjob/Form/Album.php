...
class Sesjob_Form_Album extends Engine_Form
{
  public function init()
  {
    ...
    $this
      ...
      ;
    // Init album
    $jobId = Zend_Controller_Front::getInstance()->getRequest()->getParam('job_id', null);
		$albumId = Zend_Controller_Front::getInstance()->getRequest()->getParam('album_id', null);
		if($albumId){
			$job =  Engine_Api::_()->getItem('sesjob_album', $albumId)->getParent();
			$jobId = Engine_Api::_()->getItem('sesjob_album', $albumId)->job_id;
			if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesjobpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesjobpackage.enable.package', 1)) {
				...
				$photoLeft = $package->allowUploadPhoto($job->orderspackage_id,true);
  ...
  public function saveValues() {
    ...
    $params = array();
    if ((empty($values['job_id'])) || (empty($values['user_id']))) {
      ...
      throw new Zend_Exception("Non-user album owners not yet implemented");
    	...
    	$album = Engine_Api::_()->getItem('sesjob_album', $album_id);
			$this->view->job_id = $job_id = $album->job_id;
    ...
    if( ($values['album'] == 0) ) {
      ...
      $params['search'] = 1;
      $album = Engine_Api::_()->getDbtable('albums', 'sesjob')->createRow();
      ...
      if (!isset($album)) {
        $album = Engine_Api::_()->getItem('sesjob_album', $values['album']);
      }
    }
