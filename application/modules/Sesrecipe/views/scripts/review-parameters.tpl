 ...
 * @version    $Id: review-parameters.tpl 2018-05-07 00:00:00 SocialEngineSolutions $
...
if($this->item->category_id){
	$reviewParameters = Engine_Api::_()->getDbtable('parameters', 'sesrecipe')->getParameterResult(array('category_id'=>$this->item->category_id));
  if(count($reviewParameters)){
  foreach($reviewParameters as $value){
?>
<div class="form-wrapper sesrecipe_form_review_star">
	<div class="form-label"><label><?php echo $this->translate($value['title']); ?></label></div>
  <div id="sesrecipe_review_rating" class="sesbasic_rating_parameter sesrecipe_rating_star_element" onmouseout="rating_out_review(<?php echo $value['parameter_id'] ?>);">
...
function ratingTextReview(rating){
  ...
  window.addEvent('domready', function() {
		var countExistsParam = sesJqueryObject('.sesrecipe_review_values');
