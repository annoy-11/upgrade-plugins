 ...
 * @version    $Id: upgrade.tpl 2018-05-07 00:00:00 SocialEngineSolutions $
...
<?php 
$information = array('description'=>'Package Description','featured'=>'Featured','sponsored'=>'Sponsored','verified'=>'Verified','location'=>'Location','modules'=>'Modules','editor'=>'Rich Editor','custom_fields'=>'Custom Fields','recipecount'=>'Recipe Count');
$showinfo = array('description','featured','sponsored','verified','location','modules','editor','custom_fields','recipecount');//Engine_Api::_()->getApi('settings', 'core')->getSetting('sesrecipepackage.package.information', array_keys($information));
 ?>
<?php $currentCurrency =  Engine_Api::_()->sesrecipe()->getCurrentCurrency(); ?>
<?php if($this->currentPackage){ 
	$package = Engine_Api::_()->getItem('sesrecipepackage_package',$this->currentPackage->package_id);
  if($package){
?>
<div class="sesrecipepackage_recipe_list sesbasic_clearfix sesbasic_bxs">
	<div class="sesrecipepackage_slect_plan_recipe">
  <div class="heading_recipe">
  	<p class="headding"><?php echo $this->translate("Current Package"); ?></p>
    </div>
    <div class="select">
    	<ul>
      <?php if(in_array('recipecount',$showinfo)){ ?>
        <li><?php echo $this->translate("Recipe Count"); ?></li>
     	<?php }else{ ?>
      	<li><?php echo $this->translate("Recipe Left"); ?></li>
      <?php } ?>
  ...
  <div class="package_catogery_list">
    ...
    <?php 
            ...
            $curruncySymbol = str_replace('.','',preg_replace('/\d+/u', '', Engine_Api::_()->sesrecipe()->getCurrencyPrice($package->price)));
     ?>
    	<li class="<?php ($package->highlight) ? 'active' : '' ?>">
      	<div class="package-ditals_recipe">
        	<div class="headding_recipe">
          	<div class="title">
            ...
            <div class="amount">
            	<?php if($package->price > 0){ ?>
            		<span><?php echo $curruncySymbol; ?></span><?php echo Engine_Api::_()->sesrecipe()->getCurrencyPrice($package->price,'','',true); ?><br />
     ...
     ?>
    	<li class="<?php echo ($package->highlight) ? 'active' : '' ?>">
          ...
          <?php if(in_array('description',$showinfo)){ ?> 
            <p class="footer_discprition"><?php echo $this->translate($package->description); ?> </p>
          <?php } ?>
          <a href="<?php echo $this->url(array('recipe_id' => $this->recipe->recipe_id,'action'=>'confirm-upgrade','package_id'=>$package->package_id), 'sesrecipepackage_general', true); ?>" class="smoothbox">Upgrade Package</a>
          </div>
        </div>
      </li>      
    <?php } ?>
