...
<div class="clear sesrecipe_order_view_top" style="margin:10px;">
      <a href="<?php echo $this->recipe->getHref(); ?>" class="buttonlink sesbasic_icon_back"><?php echo $this->translate("Back To Recipe"); ?></a>
    </div>
<div class="sesrecipe_album_form"> <?php echo $this->form->render($this) ?> </div>
...
<script type="text/javascript">
  ...
  jqueryObjectOfSes('#sesrecipe_create_form_tabs li a').on('click',function(){
...
function sendFileToServer(formData,status,file,isURL,i)
{
		...
		var url = '&isURL='+urlIs;
		var recipe_id = '<?php  echo $this->recipe_id;?>';
    var uploadURL =$('form-upload').action + '?ul=1'+url+'&recipe_id='+recipe_id; //Upload URL
...
jqueryObjectOfSes(document).on('click','.delete_image_upload',function(e){
    ...
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'sesrecipe', 'controller' => 'album', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'sesrecipe', 'controller' => 'album', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
</script>