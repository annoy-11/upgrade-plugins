 ...
 * @version    $Id: _advgridView.tpl 2018-05-07 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

?>
<li class="sesrecipe_last_grid_block sesbasic_bxs <?php if((isset($this->my_recipes) && $this->my_recipes)){ ?>isoptions<?php } ?>" style="width:<?php echo is_numeric($this->width_advgrid) ? $this->width_advgrid.'px' : $this->width_advgrid ?>;">
		<div class="sesrecipe_grid_inner">
			<div class="sesrecipe_gird-top_recipe">
			  <?php if(isset($this->categoryActive)){ ?>
			    <?php if($item->category_id != '' && intval($item->category_id) && !is_null($item->category_id)):?> 
			      <?php $categoryItem = Engine_Api::_()->getItem('sesrecipe_category', $item->category_id);?>
					  <?php if($categoryItem):?>
							<div class="sesrecipe_grid_info_title floatL"><a href="<?php echo $categoryItem->getHref(); ?>"><?php echo $categoryItem->category_name; ?></a></div>
					  <?php endif;?>
					<?php endif;?>
				<?php } ?>
				<?php if(isset($this->creationDateActive)){ ?>
					<div class="sesrecipe_grid_date_recipe floatR">
		...
		<div class="sesrecipe_grid_thumb sesrecipe_thumb" style="height:<?php echo is_numeric($this->height_advgrid) ? $this->height_advgrid.'px' : $this->height_advgrid ?>;">
		...
		<div class="sesrecipe_grid_info clear clearfix sesbm">
			<div class="sesrecipe_grid_meta_block">
				...
				<?php if(isset($this->locationActive) && isset($item->location) && $item->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesrecipe.enable.location', 1)){ ?>
					<div class="sesrecipe_list_stats sesrecipe_list_location sesbasic_text_dark">
						<span>
			...
			<?php if(Engine_Api::_()->getApi('core', 'sesrecipe')->allowReviewRating() && isset($this->ratingStarActive)):?>
					<?php echo $this->partial('_recipeRating.tpl', 'sesrecipe', array('rating' => $item->rating, 'class' => 'sesrecipe_list_rating sesrecipe_list_view_ratting floatR', 'style' => 'margin:0px;'));?>
				<?php endif;?>
      <?php if(isset($this->titleActive)): ?>
				<div class="sesrecipe_grid_three_info_title">
			...
			<?php  if(isset($this->descriptionadvgridActive)){?>
				<div class="sesrecipe_grid_des clear">
					<?php echo $this->string()->truncate($this->string()->stripTags($item->body), $this->description_truncation_advgrid) ?>
					<span></span>
				</div>
			<?php } ?>
			...
			<?php if(isset($this->readmoreActive)):?>
				<div class="sesrecipe_second_readmore_link floatR">
