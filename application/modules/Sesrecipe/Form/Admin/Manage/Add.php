<?php
class Sesrecipe_Form_Admin_Manage_Add extends Engine_Form {
  public function init() {
    ...
    if (!$integrateothermoduleId) {
      ...
      if (!empty($module)) {
        ...
        $integrateothermodule = Engine_Api::_()->sesrecipe()->getPluginItem($module);
    ...
    if ($param) {
      if (!$integrateothermoduleId) {
        $this->addElement('Select', 'content_type', array(
            ...
            'description' => 'Select the item type of the above chosen plugin which is defined in its manifest.php file. [This item type is the parent to which recipes are associated. For example for groups in SocialEngine Groups plugin, simply choose "groups".]',
            'multiOptions' => $integrateothermodule,
        ));
        
        $this->addElement('Text', 'content_url', array(
          'label' => 'Item Type URL for Browse Recipes Page',
          'description' => 'Enter the Item Type URL for the Browse Recipes Page. This is the URL slug which will come after your site url and before the search word browse-recipes like: "http://www.yourwebsite.com/ITEM_TYPE_URL_SLUG/browse-recipes". For example for SE - Groups Plugin, enter "groups", so that the URL will be "http://www.yourwebsite.com/groups/browse-recipes".',
          'required' => true,
          'allowEmpty' => false,
        ));
