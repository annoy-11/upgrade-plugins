...
class Sesrecipe_Widget_ProfileMusicalbumsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($_POST['is_ajax'])) {
      ...
      $subject_recipe = $coreApi->getSubject('sesrecipe_recipe');
      $this->view->recipe_id = $subject_recipe->recipe_id;

      $subject = $coreApi->getSubject();
			
			$this->view->allow_create = true;
			if(isset($value['is_ajax']) && !$value['is_ajax'] && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesrecipepackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesrecipepackage.enable.package', 1)){
			...
			$this->view->canCreate  = Engine_Api::_()->sesrecipe()->isRecipeAdmin($subject, 'music_create');
    }
    ...
    $this->view->recipe_id = $recipe_id = isset($params['recipe_id']) ? $params['recipe_id'] : $subject->recipe_id;
    ...
    $params = $this->view->params = array('height' => $defaultHeight, 'width' => $defaultWidth, 'limit_data' => $limit_data, 'albumPhotoOption' => 'album', 'openTab' => $defaultOpenTab, 'pagging' => $loadOptionData, 'show_criterias' => $show_criterias, "informationAlbum" => $informationAlbum, 'recipe_id' => $recipe_id);
    ...
    $select = $table->select()
            ...
            ->where($tableName . '.resource_id = ?', $recipe_id)
            ->where($tableName . '.resource_type = ?', 'sesrecipe_recipe')
