 ...
 */
class Sesrecipe_Widget_ProfileVideosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($_POST['is_ajax'])) {
      ...
      if (!$subject->authorization()->isAllowed($viewer, 'view'))
        return $this->setNoRender();
			...
			$subject = Engine_Api::_()->getItem('sesrecipe_recipe', $params['parent_id']);	
		}
    ...
    $this->view->description_truncation_list = $description_truncation_list = isset($params['description_truncation_list']) ? $params['description_truncation_list'] : $this->_getParam('description_truncation_list', '100');
		$viewer = Engine_Api::_()->user()->getViewer();
		$this->view->canUpload = $canUpload = Engine_Api::_()->sesrecipe()->isRecipeAdmin($subject, 'create');
    ...
    $this->view->isRecipeAdmin = Engine_Api::_()->sesrecipe()->checkRecipeAdmin($subject);
