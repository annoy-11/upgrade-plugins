 ...
 */
class Sesrecipe_Widget_photoViewPageController extends Engine_Content_Widget_Abstract
{ 
  public function indexAction()
  {
		...
		if(Engine_Api::_()->core()->hasSubject('sesrecipe_photo') && !$is_ajax)
	 	 $photo = Engine_Api::_()->core()->getSubject('sesrecipe_photo');
		else if(isset($_POST['photo_id'])){
		 $photo = Engine_Api::_()->getItem('sesrecipe_photo',$_POST['photo_id']);
		 Engine_Api::_()->core()->setSubject($photo); 
		 $photo = Engine_Api::_()->core()->getSubject();
		}else
			 return $this->setNoRender();
		
		$this->view->recipe =	$recipe = Engine_Api::_()->getItem('sesrecipe_recipe', $photo->recipe_id);
	  ...
	  if($viewer->getIdentity()>0){
			$this->view->canEdit = $canEdit = $recipe->authorization()->isAllowed($viewer, 'edit');
			$this->view->canComment = $canComment = $recipe->authorization()->isAllowed($viewer, 'comment');
			$this->view->canDelete = $canDelete = $recipe->authorization()->isAllowed($viewer, 'delete');
			$this->view->canTag = $canTag = $recipe->authorization()->isAllowed($viewer, 'tag');
			$this->view->canCommentMemberLevelPermission = Engine_Api::_()->authorization()->getPermission($viewer, 'sesrecipe_recipe', 'comment');
		}
