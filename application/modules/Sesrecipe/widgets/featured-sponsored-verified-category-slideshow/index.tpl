...
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesrecipe/externals/styles/slideshow.css'); ?>
<div class="sesrecipe_slideshow_recipe_wrapper sesbasic_clearfix sesbasic_bxs <?php if($this->navigation != 'nextprev'){ echo " isbulletnav " ; } echo $this->isfullwidth ? 'isfull_width' : '' ; ?>" style="height:<?php echo $this->height ?>px;">
	<div class="sesrecipe_slideshow_recipe_container" style="height:<?php echo $this->height ?>px;">
  	<div class="sesrecipe_recipe_slideshow">
    	<div class="sesrecipe_slideshow_recipe" id="sesrecipe_slideshow_<?php echo $this->identity; ?>">
      <ul class="bjqs">
        <?php foreach( $this->paginator as $item): ?>
        <li class="sesrecipe_slideshow_inner_view sesbasic_clearfix " style="height:<?php echo $this->height ?>px;">
          <div class="sesrecipe_slideshow_inside">
          <div class="sesrecipe_slideshow_thumb sesrecipe_thumb" style="height:<?php echo $this->height ?>px;">       
            <?php
            $href = $item->getHref();
            $imageURL = $item->getPhotoUrl('');
            ?>
            <a href="<?php echo $href; ?>" class="sesrecipe_slideshow_thumb_img">
            ...
            <div class="sesrecipe_slideshow_inside_contant">
            	<div class="slideshow_contant">
              <div class="sesrecipe_slideshow_info sesbasic_clearfix ">
             
              <?php if(isset($this->titleActive) ){ ?>
                <span class="sesrecipe_slideshow_info_title">
        ...
        <?php endif;?>
            ...
            <div class="admin_teg sesrecipe_list_stats sesbasic_text_dark">
          ...
          <?php } ?>
           <?php if(isset($this->creationDateActive)){ ?>
				<div class="admin_teg sesrecipe_list_stats sesbasic_text	_dark"><span> <i class="fa fa-calendar"></i> <?php echo ' '.date('M d, Y',strtotime($item->publish_date));?> |</span></div>
			<?php } ?>
          ...
          <?php include APPLICATION_PATH .  '/application/modules/Sesrecipe/views/scripts/_recipeRatingStat.tpl';?>
 			   ...
 			   <div class="sesrecipe_slideshow-des">
         	<p><?php echo $this->string()->truncate($this->string()->stripTags($item->body), 150) ?></p>
...
<?php
$this->headScript()->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sesrecipe/externals/scripts/slideshow/bjqs-1.3.min.js');
?>
<script type="text/javascript">
  window.addEvent('domready', function () {
		<?php if($this->isfullwidth){ ?>
			var htmlElement = document.getElementsByTagName("body")[0];
			htmlElement.addClass('sesrecipe_category_slideshow');
		<?php } ?>
		...
		<?php } ?>
			...
			var	width = sesJqueryObject('#sesrecipe_slideshow_<?php echo $this->identity; ?>').outerWidth();
			var	heigth = '<?php echo $this->height ?>';
			sesJqueryObject('#sesrecipe_slideshow_<?php echo $this->identity; ?>').bjqs({
...
sesBasicAutoScroll('.recipeslide_<?php echo $this->identity; ?>').on('init', function(event, slick, currentSlide, nextSlide){
  sesBasicAutoScroll('#sesrecipe_carousel_<?php echo $this->identity; ?>').show();
});
</script>