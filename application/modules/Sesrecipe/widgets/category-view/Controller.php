...
class Sesrecipe_Widget_CategoryViewController extends Engine_Content_Widget_Abstract {
  public function indexAction() {
    ...
    $this->view->limit_data = $limit_data = isset($params['recipe_limit']) ? $params['recipe_limit'] : $this->_getParam('recipe_limit', '10');
    ...
    $show_subcatcriterias = isset($params['show_subcatcriteria']) ? $params['show_subcatcriteria'] : $this->_getParam('show_subcatcriteria', array('countRecipe', 'icon', 'title'));
    ...
    if (!$is_ajax) {
      if ($category->subcat_id == 0 && $category->subsubcat_id == 0) {
        ...
        $innerCatData = Engine_Api::_()->getDbtable('categories', 'sesrecipe')->getModuleSubcategory(array('category_id' => $category->category_id, 'column_name' => '*', 'countRecipes' => true));
        $columnCategory = 'category_id';
      } else if ($category->subsubcat_id == 0) {
        $innerCatData = Engine_Api::_()->getDbtable('categories', 'sesrecipe')->getModuleSubsubcategory(array('countRecipes' => true, 'category_id' => $category->category_id, 'column_name' => '*'));
        $columnCategory = 'subcat_id';
      } else
        $columnCategory = 'subsubcat_id';
      $this->view->innerCatData = $innerCatData;
      //breadcum
      $this->view->breadcrumb = $breadcrumb = Engine_Api::_()->getDbtable('categories', 'sesrecipe')->getBreadcrumb($category);
    }
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('recipes', 'sesrecipe')->getSesrecipesPaginator(array($columnCategory => $category->category_id, 'status' => 1));
