...
<?php if(!$this->widgetIdentity) { ?>
	<div class="sesrecipe_browse_reviews_search sesbasic_bxs sesbasic_clearfix <?php echo $this->view_type == 'horizontal' ? 'sesrecipe_browse_review_search_horizontal' : 'sesrecipe_browse_review_search_vertical'; ?>">
<?php } ?>
...
<?php if($controllerName == 'review' && $actionName == 'browse'){ ?>
  <?php $identity = Engine_Api::_()->sesbasic()->getIdentityWidget('sesrecipe.browse-reviews','widget','sesrecipe_review_browse'); ?>
	<?php if($identity):?>
		<script type="application/javascript">
			sesJqueryObject(document).ready(function(){
				sesJqueryObject('#filter_form_review').submit(function(e){	
					if(sesJqueryObject('.sesrecipe_review_listing').length > 0){
						e.preventDefault();
						sesJqueryObject('#loadingimgsesrecipereview-wrapper').show();
						...
						if(typeof paggingNumber<?php echo $identity; ?> == 'function'){
							...
							sesJqueryObject('#sesrecipe_review_listing').html('');
...
<script type="text/javascript">
	var Searchurl = "<?php echo $this->url(array('module' =>'sesrecipe','controller' => 'index', 'action' => 'get-review'),'default',true); ?>";
