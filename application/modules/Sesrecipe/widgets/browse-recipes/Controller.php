 ...
 */
class Sesrecipe_Widget_BrowseRecipesController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $type = '';
    $page_id = Engine_Api::_()->sesrecipe()->getWidgetPageId($identityForWidget);
    if($page_id) {
      ...
      if($pageName) {
        ...
        if(is_numeric($explode[1])) {
          $type = Engine_Db_Table::getDefaultAdapter()->select()
                ->from('engine4_sesrecipe_integrateothermodules', 'content_type')
    ...
    $value['status'] = 1;
    $value['search'] = 1;
    $this->view->widgetName = 'browse-recipes';			
    $params = array_merge($params, $value,array('search'=>addslashes($text)));

    $value['draft'] = "0";
    $value['visible'] = "1";
    // Get recipes
    $paginator = Engine_Api::_()->getDbtable('recipes', 'sesrecipe')->getSesrecipesPaginator($params, $form->getValues());
