...
class Sesrecipe_Widget_BrowseSearchController extends Engine_Content_Widget_Abstract
{
  public function indexAction() {
    ...
    $this->view->search_for = $search_for = $this-> _getParam('search_for', 'recipe');
    $default_search_type = $this-> _getParam('default_search_type', 'mostSPliked');
	
    if($this->_getParam('location','yes') == 'yes' && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesrecipe_enable_location', 1))
    $location = 'yes';	
    else
    $location = 'no';
    ...
    // Recipe browse page work
    $page_id = Engine_Api::_()->sesrecipe()->getWidgetPageId($this->view->identity);
    if($page_id) {
      ...
      if($pageName) {
        ...
        $explode = explode('sesrecipe_index_', $pageName);
