 ...
 */
class Sesrecipe_Widget_ManageRecipesController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (!$is_ajax && is_array($defaultOptionsArray)) {
      foreach ($defaultOptionsArray as $key => $defaultValue) {
				if(!Engine_Api::_()->getApi('settings', 'core')->getSetting('sesrecipe.enable.favourite', 1) && $defaultValue == 'mostSPfavourite')
					continue;
    ...
    $this->view->widgetName = 'manage-recipes';			
    $params = array_merge($params, $value, array('search'=>addslashes($text)));

    // Get Recipes
    $paginator = Engine_Api::_()->getDbtable('recipes', 'sesrecipe')->getSesrecipesPaginator($value);
