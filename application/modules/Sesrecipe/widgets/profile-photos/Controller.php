 ...
 */
class Sesrecipe_Widget_ProfilePhotosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->album_parent_id =  $id = isset($params['album_parent_id']) ? $params['album_parent_id'] : Zend_Controller_Front::getInstance()->getRequest()->getParam('recipe_id', null);
		
    $value['recipe_id'] = $this->view->recipe_id =  $recipe_id = $id = Engine_Api::_()->getDbtable('recipes', 'sesrecipe')->getRecipeId($id);
    $this->view->recipe  = $subject = Engine_Api::_()->getItem('sesrecipe_recipe',$id);
    $value['identityForWidget'] = $this->view->identityForWidget = isset($_POST['identity']) ? $_POST['identity'] : '';
		$this->view->allow_create = true;
		if(!$value['is_ajax'] && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesrecipepackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesrecipepackage.enable.package', 1)){
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('albums', 'sesrecipe')->getAlbumSelect($value);
