...
class Sesrecipe_AdminIntegrateothermoduleController extends Core_Controller_Action_Admin {

  public function indexAction() {
  
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesrecipe_admin_main', array(), 'sesrecipe_admin_main_integrateothermodule');
    
    $this->view->enabledModules = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();
    
    $select = Engine_Api::_()->getDbtable('integrateothermodules', 'sesrecipe')->select();
  ...
  public function addmoduleAction() {
    ...
    $this->view->form = $form = new Sesrecipe_Form_Admin_Manage_Add();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      $integrateothermoduleTable = Engine_Api::_()->getDbtable('integrateothermodules', 'sesrecipe');
      ...
      try {
        ...
        $dbInsert->query('INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `enabled`, `custom`, `order`) VALUES ("sesrecipe_main_browserecipe_'.$row->getIdentity().'", "'.$modulename.'", "Browse Recipes", "", \'{"route":"sesrecipe_browserecipe_'.$row->getIdentity().'","action":"browse-recipes", "resource_type":"'.$values['content_type'].'"}\', "'.$modulename.'_main", "", 1, 0, 999)');
        
        $this->createBrowseRecipePage($modulename, $row->getIdentity());
  ...
  function createBrowseRecipePage($modulename, $id) {
    ...
    $page_id = $db->select()
      ...
      ->where('name = ?', 'sesrecipe_index_'.$id)
      ->limit(1)
      ->query()
      ->fetchColumn();
    ...
    if( !$page_id ) {
      ...
      $db->insert('engine4_core_pages', array(
        'name' => 'sesrecipe_index_'.$id,
        'displayname' => 'SES - Advanced Recipes - '.ucfirst($modulename).' Recipes Browse Page',
        'title' => ucfirst($modulename) .' Recipe Browse',
        'description' => 'This page lists recipes.',
        'custom' => 0,
      ));
  ...
  public function deleteAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $inttable = Engine_Api::_()->getItem('sesrecipe_integrateothermodule', $this->_getParam('integrateothermodule_id'));
        $pageName = "sesrecipe_index_".$this->_getParam('integrateothermodule_id');
        if (!empty($pageName)) {
          $page_id = $db->select()
                  ->from('engine4_core_pages', 'page_id')
                  ->where('name = ?', $pageName)
        ...
        Engine_Api::_()->getDbtable('menuItems', 'core')->delete(array('name =?' => 'sesrecipe_main_browserecipe_' . $this->_getParam('integrateothermodule_id')));
      
        $integrateothermodule = Engine_Api::_()->getItem('sesrecipe_integrateothermodule', $this->_getParam('integrateothermodule_id'));
  ...
  public function enabledAction() {
    $db = Engine_Db_Table::getDefaultAdapter();
    $db->beginTransaction();
    $content = Engine_Api::_()->getItemTable('sesrecipe_integrateothermodule')->fetchRow(array('integrateothermodule_id = ?' => $this->_getParam('integrateothermodule_id')));
    try {
      
      Engine_Api::_()->getDbtable('menuItems', 'core')->update(array('enabled' => !$content->enabled), array('name =?' => 'sesrecipe_main_browserecipe_' . $this->_getParam('integrateothermodule_id')));
