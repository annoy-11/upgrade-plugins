 ...
 * @version    $Id: PhotoController.php 2018-05-07 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesrecipe_PhotoController extends Core_Controller_Action_Standard {

  public function init() {
    if (!Engine_Api::_()->core()->hasSubject()) {
      if (0 !== ($photo_id = (int) $this->_getParam('photo_id')) &&
              null !== ($photo = Engine_Api::_()->getItem('sesrecipe_photo', $photo_id))) {
        Engine_Api::_()->core()->setSubject($photo);
      } else if (0 !== ($recipe_id = (int) $this->_getParam('recipe_id')) &&
              null !== ($recipe = Engine_Api::_()->getItem('sesrecipe_recipe', $event_id))) {
...
//rotate photo action from lightbox and photo view page
  public function rotateAction() {
    if (!$this->_helper->requireSubject('sesrecipe_photo')->isValid())
      return;
		$recipe_id = $this->_getParam('recipe_id');
		$recipe = Engine_Api::_()->getItem('sesrecipe_recipe', $recipe_id);
    if (!$this->_helper->requireAuth()->setAuthParams($recipe, null, 'edit')->isValid())
      return;
    ...
    $photo = Engine_Api::_()->core()->getSubject('sesrecipe_photo');
   ...
   if (!$this->_helper->requireSubject('sesrecipe_photo')->isValid())
    ...
    $this->view->href = $photo->getPhotoUrl();
  	...
  	public function correspondingImageAction(){
		...
		$this->view->paginator = $paginator = Engine_Api::_()->getDbtable('photos', 'sesrecipe')->getPhotoSelect(array('album_id'=>$album_id,'limit_data'=>100));
	}
  public function uploadAction() {

    if (isset($_GET['ul']) || isset($_FILES['Filedata']))
    return $this->_forward('upload-photo', null, null, array('format' => 'json', 'recipe_id'=> 0));
  }
  ...
  public function uploadPhotoAction() {
    ...
    $sesrecipePhotoTable = Engine_Api::_()->getDbtable('photos', 'sesrecipe');
    
    $db = $sesrecipePhotoTable->getAdapter();
    $db->beginTransaction();
		$session = new Zend_Session_Namespace();
    try {
      $viewer = Engine_Api::_()->user()->getViewer();
      if(empty($session->album_id)) {
			$album = Engine_Api::_()->getItemTable('sesrecipe_album')->createRow();
      ...
      $photo = Engine_Api::_()->sesbasic()->setPhoto($uploadSource, $isURL,false,'sesrecipe','sesrecipe_recipe',$params,$album);
  ...
  public function editAction()
  {
    ...
    $this->view->form = $form = new Sesrecipe_Form_Photo_Edit();
  ...
  public function deleteAction() {
    $photo = Engine_Api::_()->core()->getSubject();
    $recipe = $photo->getParent('sesrecipe_recipe');
		$album_id = $photo->album_id;
    if (!$this->_helper->requireAuth()->setAuthParams($recipe, null, 'edit')->isValid()) {
      return;
    }

    $this->view->form = $form = new Sesrecipe_Form_Photo_Delete();
