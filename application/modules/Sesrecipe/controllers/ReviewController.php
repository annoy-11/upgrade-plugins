 ...
 * @version    $Id: ReviewController.php 2018-05-07 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesrecipe_ReviewController extends Core_Controller_Action_Standard {
	...
	public function init() {
		...
		if($recipe_id = $this->_getParam('recipe_id')){
			$recipe = Engine_Api::_()->getItem('sesrecipe_recipe',$recipe_id);
		}
		 
		//Get subject
		if (null !== ($review_id = $this->_getParam('review_id')) && null !== ($review = Engine_Api::_()->getItem('sesrecipe_review', $review_id)) && $review instanceof Sesrecipe_Model_Review && !Engine_Api::_()->core()->hasSubject()) {
			$recipe = $review->getParent();
			Engine_Api::_()->core()->setSubject($review);
		}
		if(isset($recipe) && $recipe && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesrecipepackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesrecipepackage.enable.package', 1)){
  ...
  public function createAction() {
		...
		$viewer = Engine_Api::_()->user()->getViewer();
		$this->view->item = $item = Engine_Api::_()->getItem('sesrecipe_recipe', $this->getParam('recipe_id'));
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('sesrecipe_review', 'create'))
		return $this->_forward('notfound', 'error', 'core');
		if(!$item)
		return $this->_forward('notfound', 'error', 'core');
		//check review exists
		$isReview = Engine_Api::_()->getDbtable('reviews', 'sesrecipe')->isReview(array('recipe_id' => $item->getIdentity()));
    ...
    $this->view->form = $form = new Sesrecipe_Form_Review_Create(array('defaultProfileId' => $defaultProfileId));
    ...
    $values['recipe_id'] = $item->getIdentity();
    $reviews_table = Engine_Api::_()->getDbtable('reviews', 'sesrecipe');
    ...
    try {
      ...
      $review->save();
		  $dbObject = Engine_Db_Table::getDefaultAdapter();
			...
			$table = Engine_Api::_()->getDbtable('parametervalues', 'sesrecipe');
			...
			foreach($_POST as $key => $reviewC){
				...
				if(!is_numeric($key))
					continue;
				$parameter = Engine_Api::_()->getItem('sesrecipe_parameter',$key);
			...
			if(isset($item->rating)){
				$item->rating = Engine_Api::_()->getDbtable('reviews', 'sesrecipe')->getRating($review->recipe_id);
				$item->save();
			}
      ...
      $customfieldform->saveValues();
			$review->save();
      $db->commit();
      return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'review_id' => $review->review_id, 'slug' => $review->getSlug()), 'sesrecipereview_view', true);
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    }
  ...
  public function editAction() {
    ...
    $review_id = $this->_getParam('review_id', null);
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('sesrecipe_review', 'edit'))
    return $this->_forward('notfound', 'error', 'core');
		$this->view->item = $item = Engine_Api::_()->getItem('sesrecipe_recipe', $subject->recipe_id);
		if (isset($item->category_id) && $item->category_id != 0)
		$this->view->category_id = $item->category_id;
		else
		$this->view->category_id = 0;
    ...
    if(!$review_id || !$subject)
	  return $this->_forward('notfound', 'error', 'core');
    $this->view->defaultProfileId = $defaultProfileId = Engine_Api::_()->getDbTable('reviewmetas', 'sesrecipe')->profileFieldId();
    $this->view->form = $form = new Sesrecipe_Form_Review_Edit(array('defaultProfileId' => $defaultProfileId));
    ...
    try {
      ...
      $subject->save();
			...
			if(isset($item->rating)){
				$item->rating = Engine_Api::_()->getDbtable('reviews', 'sesrecipe')->getRating($subject->recipe_id);
				$item->save();
			}
			 //Add fields
      $customfieldform = $form->getSubForm('fields');
      ...
      $customfieldform->saveValues();
			$subject->save();
      $db->commit();
      return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'review_id' => $subject->review_id, 'slug' => $subject->getSlug()), 'sesrecipereview_view', true);
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    }
  ...
  public function deleteAction() {
    $viewer = Engine_Api::_()->user()->getViewer();
    $review = Engine_Api::_()->getItem('sesrecipe_review', $this->getRequest()->getParam('review_id'));
    $content_item = Engine_Api::_()->getItem('sesrecipe_recipe', $review->recipe_id);
  ...
  public function viewAction() {
		...
		return $this->_forward('notfound', 'error', 'core');
		$review_id = $this->_getParam('review_id', null);
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('sesrecipe_review', 'view'))
		return $this->_forward('notfound', 'error', 'core');
  ...
  function likeAction() {
    ...
    $itemTable = Engine_Api::_()->getItemTable('sesrecipe_review');
    $tableLike = Engine_Api::_()->getDbtable('likes', 'core');
    $tableMainLike = $tableLike->info('name');
    $select = $tableLike->select()
            ->from($tableMainLike)
            ->where('resource_type = ?', 'sesrecipe_review')
    ...
    if (count($result) > 0) {
      ...
      try {
        ...
        $like->resource_type = 'sesrecipe_review';
