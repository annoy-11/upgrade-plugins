 ...
 * @version    $Id: AdminReviewController.php 2018-05-07 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesrecipe_AdminReviewController extends Core_Controller_Action_Admin {

  public function reviewSettingsAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesrecipe_admin_main', array(), 'sesrecipe_admin_main_reviewsettings');
    
    $this->view->subnavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesrecipe_admin_main_reviewsettings', array(), 'sesrecipe_admin_main_review_settings');

    $this->view->form = $form = new Sesrecipe_Form_Admin_Review_ReviewSettings();
  ...
  public function manageReviewsAction() {
    ...
    $this->view->subnavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesrecipe_admin_main_reviewsettings', array(), 'sesrecipe_admin_main_managereview');

    $this->view->formFilter = $formFilter = new Sesrecipe_Form_Admin_Review_Filter();
    ...
    if ($this->getRequest()->isPost()) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $video = Engine_Api::_()->getItem('sesrecipe_review', $value)->delete();
        }
      }
    }

    $table = Engine_Api::_()->getDbtable('reviews', 'sesrecipe');
  ...
  public function viewAction() {
    $this->view->item = Engine_Api::_()->getItem('sesrecipe_review', $this->_getParam('id', null));
  }
  ...
  public function deleteReviewAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db->beginTransaction();
      try {
        $review = Engine_Api::_()->getItem('sesrecipe_review', $this->_getParam('review_id'));
        $review->delete();
        $db->commit();
      } catch (Exception $e) {
        $db->rollBack();
  ...
  public function profileTypeMappingAction() {
    ...
    $this->view->module_name = 'sesrecipe';

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesrecipe_admin_main', array(), 'sesrecipe_admin_main_reviewsettings');
    
    $this->view->subnavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesrecipe_admin_main_reviewsettings', array(), 'sesrecipe_admin_main_subprofiletypemapping');

    $categories_table = Engine_Api::_()->getDbtable('categories', 'sesrecipe');
  ...
  public function categoryMappingAction() {
    ...
    $this->view->form = $form = new Sesrecipe_Form_Admin_Review_CategoryMapping();

    $categorymapping_table = Engine_Api::_()->getDbtable('categorymappings', 'sesrecipe');
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      try {
        ...
        if (empty($isCatMapped)) {
          ...
          $categorymapping = Engine_Api::_()->getItem('sesrecipe_categorymapping', $isCatMapped);
  ...
  public function removeCategoryMappingAction() {
    ...
    $categorymapping = Engine_Api::_()->getItem('sesrecipe_categorymapping', $categorymapping_id);
    ...
    public function levelSettingsAction() {
		...
		$this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesrecipe_admin_main', array(), 'sesrecipe_admin_main_reviewsettings');
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesrecipe_admin_main_reviewsettings', array(), 'sesrecipe_admin_main_levelsettings');
    ...
    //Make form
    $this->view->form = $form = new Sesrecipe_Form_Admin_Review_Level(array(
        'public' => ( in_array($level->type, array('public')) ),
        'moderator' => ( in_array($level->type, array('admin', 'moderator')) ),
    ));
    $form->level_id->setValue($id);

    $content_type = 'sesrecipe_review';
    $module_name = $this->_getParam('module_name', null);
  ...
  public function featuredAction() {
    ...
    if (!empty($id)) {
      $item = Engine_Api::_()->getItem('sesrecipe_review', $id);
      $item->featured = !$item->featured;
      $item->save();
    }
    $this->_redirect('admin/sesrecipe/review/manage-reviews');
  }
  ...
  public function ofthedayAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db->update('engine4_sesrecipe_reviews', array('starttime' => $values['starttime'], 'endtime' => $values['endtime']), array("review_id = ?" => $id));
      if (isset($values['remove']) && $values['remove']) {
        $db->update('engine4_sesrecipe_reviews', array('oftheday' => 0), array("review_id = ?" => $id));
      } else {
        $db->update('engine4_sesrecipe_reviews', array('oftheday' => 1), array("review_id = ?" => $id));
      }
