 ...
 * @version    $Id: AlbumController.php 2018-05-07 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesrecipe_AlbumController extends Core_Controller_Action_Standard {	
	
	public function init() {
    $id = $this->_getParam('album_id', $this->_getParam('id', null));
    if ($id) {
      $album = Engine_Api::_()->getItem('sesrecipe_album', $id);
      if ($album) {
  ...
  public function createAction() {
    ...
    return $this->_forward('upload-photo', null, null, array('format' => 'json'));
     $recipe_id = $this->_getParam('recipe_id',false);
    ...
    if($album_id){
    	$album = Engine_Api::_()->getItem('sesrecipe_album', $album_id);
			$this->view->recipe_id = $recipe_id = $album->recipe_id;
				...
				$this->view->recipe_id = $recipe_id = $recipe_id;
		}
		$recipe = $this->view->recipe = Engine_Api::_()->getItem('sesrecipe_recipe', $recipe_id);
		
		$isRecipeAdmin = Engine_Api::_()->sesrecipe()->checkRecipeAdmin($recipe);
    if(!$isRecipeAdmin)
    return $this->_forward('notfound', 'error', 'core');
		
		if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesrecipepackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesrecipepackage.enable.package', 1) && empty($_POST)){
			$package = $recipe->getPackage();
			$this->view->photoLeft = $photoLeft = $recipe->getPackage()->allowUploadPhoto($recipe->orderspackage_id,true);
    ...
    $this->view->current_count =Engine_Api::_()->getDbtable('albums', 'sesrecipe')->getUserAlbumCount($values);
    $this->view->quota = $quota = 0;
    // Get form
    $this->view->form = $form = new Sesrecipe_Form_Album();
    // Render		
    ...
    $db = Engine_Api::_()->getItemTable('sesrecipe_album')->getAdapter();
    $db->beginTransaction();
    try {
...
//     }
    ...
    try {
      $viewer = Engine_Api::_()->user()->getViewer();
      $photoTable = Engine_Api::_()->getDbtable('photos', 'sesrecipe');
	...
	return;
      ...
      $db->commit();
    }catch (Sesrecipe_Model_Exception $e) {
      $db->rollBack();
      $this->view->status = false;
  ...
  public function getAlbumAction() {
    ...
    $albums = Engine_Api::_()->getDbTable('albums', 'sesrecipe')->getAlbumsAction($value);
  ...
  public function editAction() {
    ...
    return;
		...
		$album_id = $this->_getParam('album_id',false);
    if($album_id)
    $this->view->album = $album = Engine_Api::_()->getItem('sesrecipe_album', $album_id);
	  else
		return;
		
		$this->view->recipe = $recipe = Engine_Api::_()->getItem('sesrecipe_recipe', $album->recipe_id);
		if ($recipe)
    ...
    // Make form
    $this->view->form = $form = new Sesrecipe_Form_Album_Edit();  
  ...
  public function deleteAction() {
    ...
    // In smoothbox
    $this->view->form = $form = new Sesrecipe_Form_Album_Delete();
    if (!$album) {
      $this->view->status = false;
    ...
    try {
      ...
      throw $e;
		...
		$tab_id = Engine_Api::_()->sesbasic()->getWidgetTabId(array('name' => 'sesrecipe.profile-photos'));
		return $this->_forward('success' ,'utility', 'core', array('parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'view','recipe_id'=>$recipe->custom_url, 'tab' => $tab_id), 'sesrecipe_entry_view', true), 'messages' => Array($this->view->message)
		));
  }
  ...
  public function editphotosAction() {
    ...
    // Get albums
    $myAlbums = Engine_Api::_()->getDbtable('albums', 'sesrecipe')->editPhotos();
    $albumOptions = array('' => '');
    ...
    // Make form
    $this->view->form = $form = new Sesrecipe_Form_Album_Photos();
    foreach ($paginator as $photo) {
      $subform = new Sesrecipe_Form_Album_EditPhoto(array('elementsBelongTo' => $photo->getGuid()));
    ...
    try {
      ...
      foreach ($paginator as $photo) {
        ...
        if (isset($values['delete']) && $values['delete'] == '1') {
          ...
          if (($nextPhoto instanceof Sesrecipe_Model_Photo) &&
