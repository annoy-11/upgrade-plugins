 ...
 * @version    $Id: Recipe.php 2018-05-07 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesrecipe_Model_Recipe extends Core_Model_Item_Abstract
{
  // Properties
  ...
  protected $_type = 'sesrecipe_recipe';
  ...
  public function getHref($params = array()) {
    ...
    $this->save();
		if($direct == ''){
			...
			$recipeAlbum->title = Zend_Registry::get('Zend_Translate')->_('Untitled');
			$recipeAlbum->owner_id = Engine_Api::_()->user()->getViewer()->getIdentity();
			$recipeAlbum->save();
    ...
    $table = Engine_Api::_()->getItemTable('sesrecipe_album');
    $select = $table->select()
	...
	public function cancel()
  {
	 ...
	 //update transaction_id to other recipe of same package recipe
	 if($this->transaction_id){
		$transaction = $this->getTransaction();
		$table = Engine_Api::_()->getDbTable('recipes','sesrecipe');
		$tableName = $table->info('name');
		//select recipe in package with our transaction id.
		$select = $table->select()->from($tableName)->where('transaction_id =?','')->where('orderspackage_id =?',$this->orderspackage_id);
		$recipe = $table->fetchRow($select);
		if($recipe){
			$recipe->transaction_id = $this->transaction_id;
			$recipe->save();
			//update order
			$order_id = $transaction->order_id;
			$order = Engine_Api::_()->getItem('payment_order',$order_id);
			if($order){
				$order->source_id = $recipe->getIdentity();
				$order->save();
			}
			//update item count in order package
			$orderpackage = Engine_Api::_()->getItem('sesrecipepackage_orderspackage',$this->orderspackage_id);
    ...
    if( !empty($transaction->gateway_id) && !empty($transaction->gateway_profile_id) && empty($transaction->gateway_transaction_id) ) {
      try {
        ...
        $gateway = Engine_Api::_()->getItem('sesrecipepackage_gateway', $transaction->gateway_id);
        if( $gateway ) {
          $gatewayPlugin = $gateway->getPlugin();
          if( method_exists($gatewayPlugin, 'cancelRecipe') ) {
            $gatewayPlugin->cancelRecipe($transaction->gateway_profile_id);
          }
        }
      } catch( Exception $e ) {
        // Silence?
  ...
  public function getPackage(){
		return Engine_Api::_()->getItem('sesrecipepackage_package',$this->package_id);
	}
	 public function getTransaction(){
		return Engine_Api::_()->getItem('sesrecipepackage_transaction',$this->transaction_id);
	}
	 // Events
  ...
  public function setActive($flag = true, $deactivateOthers = null)
  {
	...
	public function changeApprovedStatus($approved = 0){
		...
		if($transaction && $orderPackageId){
				...
				$this->save();
				Engine_Api::_()->getDbtable('recipes', 'sesrecipe')->update(array('is_approved' =>$approved),array('orderspackage_id'=>$orderPackageId));
		}			
	}
     ...
     if( in_array($transaction->state, array('initial', 'trial', 'pending', 'active')) ) {
      ...
      if( $expiration ) {
        $expiration_date = date('Y-m-d H:i:s', $expiration);
				...
				//make it a future recipe(never expired)
  ...
  public function getPhotoUrl($type = null) {
    ...
    if ($photo_id) {
      $file = Engine_Api::_()->getItemTable('storage_file')->getFile($this->photo_id, $type);
		...
		$defaultPhoto = Zend_Registry::get('StaticBaseUrl').$settings->getSetting('sesrecipe_recipe_default_photo', 'application/modules/Sesrecipe/externals/images/nophoto_recipe_thumb_profile.png');
		return $defaultPhoto;
  }
