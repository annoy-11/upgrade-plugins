 ...
 */
class Coursesalbum_Model_DbTable_Albums extends Engine_Db_Table {
  protected $_rowClass = "Coursesalbum_Model_Album";
  protected $_name = 'courses_albums';
  public function getUserAlbumCount($params = array()){
    ...
    public function getUserClassroomAlbumCount($params = array()){
        ...
        if ($params['course_id'])
            $count->where('course_id =?', $params['course_id']);
  ...
  public function editPhotos(){
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
		...
		$tableCourse = Engine_Api::_()->getDbTable('courses', 'courses');
        ...
        $select = $this->select()
            ...
            ->join($tableCourseName, $tableCourseName . '.course_id = ' . $albumTableName . '.course_id',array('is_approved','draft','owner_id'));
    ...
    if ((!$subject->is_approved || !$subject->draft) && $level_id != 1 && $level_id != 2) {
        $select->where("CASE WHEN ".$tableCourseName.".is_approved = '0' OR  ".$tableCourseName.".draft = '0' THEN ".$tableCourseName.".owner_id = ".$viewer->getIdentity()." ELSE TRUE END");
    }
    ...
    if(isset($value['course_id']) && intval($value['course_id']))
    $select->where($albumTableName.'.course_id = ?',$value['course_id']);
  ...
  public function getSpecialAlbum(User_Model_User $user, $type,$course_id)
  {
