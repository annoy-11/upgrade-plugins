 ...
 */
class Coursesalbum_AlbumController extends Core_Controller_Action_Standard {
  ...
  public function createAction() {
    ...
    if($album_id) {
    	$this->view->album = $album = Engine_Api::_()->getItem('coursesalbum_album', $album_id);
        $this->view->course_id = $course_id = $album->course_id;
    } else {
      $this->view->course_id = $course_id = $course_id;
    }
    $this->view->course = Engine_Api::_()->getItem('courses', $course_id);
    // set up data needed to check quota
    $viewer = Engine_Api::_()->user()->getViewer();
    $values['user_id'] = $viewer->getIdentity();
    $this->view->current_count = Engine_Api::_()->getDbTable('albums', 'coursesalbum')->getUserAlbumCount($values);
    $this->view->quota = $quota = 0;
      // Get form
      $this->view->form = $form = new Coursesalbum_Form_Album();
   // }
    // Render
    ...
    if (!$form->isValid($this->getRequest()->getPost()))
      return;
    $db = Engine_Api::_()->getItemTable('coursesalbum_album')->getAdapter();
    $db->beginTransaction();
    try {
...
//     if (!$this->_helper->requireAuth()->setAuthParams('courses', null, 'album')->isValid()) {
//       return;
//     }
    ...
    $db = Engine_Api::_()->getDbTable('photos', 'coursesalbum')->getAdapter();
    $db->beginTransaction();
    try {
      $viewer = Engine_Api::_()->user()->getViewer();
      $photoTable = Engine_Api::_()->getDbTable('photos', 'coursesalbum');
      ...
      $photo->setFromArray(array(
			...
			if($type == 'wall') {
				$viewer = Engine_Api::_()->user()->getViewer();
				 $table = Engine_Api::_()->getDbTable('albums', 'coursesalbum');
				$album = $table->getSpecialAlbum($viewer, $type,$course->course_id);
			}
      $photo->save();
  ...
  public function viewAction() {
		...
		$album = null;
    if ($album_id) {
      $album = Engine_Api::_()->getItem('coursesalbum_album', $album_id);
      if ($album) {
    ...
    if ($viewer->getIdentity() != 0 && isset($album->album_id)) {
      ...
      $dbObject->query('INSERT INTO engine4_courses_recentlyviewitems (resource_id, resource_type,owner_id,creation_date ) VALUES ("' . $album->album_id . '", "coursesalbum_album","' . $viewer->getIdentity() . '",NOW())	ON DUPLICATE KEY UPDATE	creation_date = NOW()');
    }
    // Render
  ...
  public function getAlbumAction() {
    ...
    $albums = Engine_Api::_()->getDbTable('albums', 'coursesalbum')->getAlbumSelect($value,array('album_id','title','photo_id'));
  ...
  public function editAction() {
    ...
    if($album_id)
        ...
        return;
    $this->view->course = $course = Engine_Api::_()->getItem('courses', $album->course_id);
    if ($course) {
  ...
  public function deleteAction() {
    ...
    if($album_id)
        ...
        return;
    $course = Engine_Api::_()->getItem('courses', $album->course_id);
    if ($course) {
        Engine_Api::_()->core()->setSubject($course);
    } else {
        return;
    }
    if (!$this->_helper->requireAuth()->setAuthParams('coursesalbum', null, 'edit')->isValid())
      return;
    // In smoothbox
    $this->_helper->layout->setLayout('default-simple');
    $this->view->form = $form = new Courses_Form_Album_Delete();
    if (!$album) {
      $this->view->status = false;
    ...
    try {
      $album->delete();
      $coursesPhotoTable = Engine_Api::_()->getDbtable('photos', 'coursesalbum');
      $coursesPhotoTable->delete(array(
    ...
    return $this->_forward('success', 'utility', 'core', array(
      'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('id'=>$course->custom_url), 'coursesalbum_profile', true),
  ...
  public function editphotosAction() {
    ...
    if($album_id){
        $this->view->album =	$album = Engine_Api::_()->getItem('coursesalbum_album', $album_id);
        $this->view->course_id = $course_id = $album->course_id;
        $course = $this->view->course = Engine_Api::_()->getItem('courses', $course_id);
    }else{
...
//       // Hack navigation
//       foreach ($navigation->getClassrooms() as $course) {
//         if ($course->route != 'coursesalbum_general' || $course->action != 'manage')
//           continue;
//         $course->active = true;
//       }
    }
    ...
    // Get albums
    $myAlbums = Engine_Api::_()->getDbTable('albums', 'coursesalbum')->editPhotos();
    $albumOptions = array('' => '');
      ...
      foreach ($paginator as $photo) {
        ...
        if (isset($values['delete']) && $values['delete'] == '1') {
					if($album->photo_id == $photo->photo_id){
						$album->photo_id = 0;
						$photoCn = Engine_Api::_()->getDbTable('photos', 'coursesalbum')->getPhotoSelect(array('album_id'=>$album->album_id,'fetchAll'=>true,'limit_data'=>1));
						if(count($photoCn)){
    ...
    return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'album_id' => $album->album_id), 'coursesalbum_specific_album', true);
  }
  ...
  public function editPhotoAction() {
    ...
    $this->view->photo = Engine_Api::_()->getItem('coursesalbum_photo', $photo_id);
  }
  public function saveInformationAction() {
    ...
    Engine_Api::_()->getDbTable('photos', 'coursesalbum')->update(array('title' => $title, 'description' => $description), array('photo_id = ?' => $photo_id));
  }
