 ...
 */
class Coursesalbum_AdminManageAlbumController extends Core_Controller_Action_Admin {

  public function indexAction() {
   $this->view->navigation  = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main', array(), 'coursesalbum_admin_main_mgphlbum');
   $this->view->subNavigation  = Engine_Api::_()->getApi('menus', 'core')->getNavigation('coursesalbum_admin_main_mgphlbum', array(), 'courses_admin_main_managealbum');
    ...
    if( $this->getRequest()->isPost() ) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $album = Engine_Api::_()->getItem('coursesalbum_album', $value);
          $album->delete();
        }
      }
    }
    $tableAlbum = Engine_Api::_()->getDbTable('albums', 'coursesalbum');
    $tableAlbumName = $tableAlbum->info('name');
    $tableCourses = Engine_Api::_()->getDbTable('courses', 'courses');
    $tableCoursesName = $tableCourses->info('name');
    $tableUserName = Engine_Api::_()->getItemTable('user')->info('name');
    $select = $tableAlbum->select()
                        ->from($tableAlbumName)
                        ->setIntegrityCheck(false)
                        ->joinLeft($tableUserName, "$tableUserName.user_id = $tableAlbumName.owner_id", 'username')
                        ->join($tableCoursesName, "$tableCoursesName.course_id = $tableAlbumName.course_id", null)
                        ->order($tableAlbumName.'.album_id DESC');

    if( !empty($_GET['title']) )
      $select->where($tableAlbumName.'.title LIKE ?', '%' . $values['title'] . '%');
    if( !empty($_GET['coursestitle']) )
      $select->where($tableCoursesName.'.title LIKE ?', '%' . $values['coursestitle'] . '%');
  ...
  public function featuredalbumAction() {
    ...
    if (isset($_SERVER['HTTP_REFERER']))
      ...
      $url = 'admin/coursesalbum/manage-album';
    $this->_redirect($url);
  }
  //Sponsored Action
  ...
  public function photosAction() {
   ...
   $this->view->subNavigation  = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_mgphlbum', array(), 'courses_admin_main_managephotos');
    ...
    if( $this->getRequest()->isPost() ) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $photo = Engine_Api::_()->getItem('coursesalbum_photo', $value);
          $photo->delete();
        }
      }
    }
    $tablePhoto = Engine_Api::_()->getDbTable('photos', 'coursesalbum');
    ...
    $select =  $tablePhoto->select()
                        ...
                        ->joinLeft($tableCoursesName, "$tableCoursesName.course_id = $tablePhotoName.course_id", null)
  ...
  public function deletePhotoAction() {
    ...
    if( $this->getRequest()->isPost())
    {
      ...
      try
      {
        $photo = Engine_Api::_()->getItem('coursesalbum_photo', $id);
  ...
  public function deleteAction()
  {
    ...
    if( $this->getRequest()->isPost())
    {
      ...
      try
      {
        $album = Engine_Api::_()->getItem('coursesalbum_album', $id);
