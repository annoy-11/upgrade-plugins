...
<?php endif;?>
<?php if(!$this->is_ajax): ?>
<div class="coursesalbum_profile_tab_wrapper coursesalbum_profile_photos sesbasic_bxs">
    <div class="coursesalbum_profile_content_search sesbasic_clearfix">
      <div class="_input">
      ...
      <?php if((!$this->is_ajax) && ($this->paginator->count() > 1 || $this->canUpload )  && (!$this->course_album_count || $this->paginator->getTotalItemCount() <= $this->course_album_count)):  ?>
      <?php if( $this->canUpload ): ?>
          <div class="_addbtn">
            <?php echo $this->htmlLink(array(
              'route' => 'coursesalbum_extended',
              'controller' => 'album',
              'action' => 'create',
              'course_id' => $this->course_id,
              ), $this->translate('Add New Album'), array(
              'class' => 'sesbasic_button sesbasic_icon_add coursesalbum_cbtn'
            )) ?>
          </div>  
        <?php endif; ?>
      <?php endif; ?>
      ...
      <li id="thumbs-photo-<?php echo $album->photo_id ?>" class="coursesalbum_album_list_grid_thumb coursesalbum_album_list_grid sespa-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sespa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?>" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">
        ...
        <?php  if(isset($this->socialSharing) || isset($this->likeButton)){  ?>
          <span class="coursesalbum_album_list_grid_btns">
            ...
            <?php }
            $canComment = $this->courseItem->authorization()->isAllowed($this->viewer, 'comment');;
            if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0 && isset($this->likeButton) && $canComment){  ?>
              <!--Album Like Button-->
              <?php $albumLikeStatus = Engine_Api::_()->courses()->getLikeStatus($album->album_id,'coursesalbum_album'); ?>
              <a href="javascript:;" data-url='<?php echo $album->album_id; ?>' class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn coursesalbum_albumlike <?php echo ($albumLikeStatus) ? 'button_active' : '' ; ?>">
  ...
  <?php if(!$this->is_ajax){ ?>
    ...
    <?php if($this->load_content != 'pagging'){ ?>      
    	<div class="sesbasic_load_btn" id="view_more_<?php echo $randonNumber;?>" onclick="viewMore_<?php echo $randonNumber; ?>();" > 
  		...
  		<div class="sesbasic_load_btn sesbasic_view_more_loading_<?php echo $randonNumber;?>" id="loading_image_<?php echo $randonNumber; ?>" style="display: none;"><span class="coursesalbum_link_btn"><i class="fa fa-spinner fa-spin"></i></span> </div>  
    <?php } ?>
  <?php } ?>
</div>  
...
<script type="text/javascript">
  ...
  function paggingNumber<?php echo $randonNumber; ?>(pageNum){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "widget/index/mod/coursesalbum/name/profile-photos",
      'data': {
      format: 'html',
      page: pageNum,    
	...
	document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML =  responseHTML;	sesJqueryObject('#paginator_count_coursesalbum').find('#total_item_count_coursesalbum').html(sesJqueryObject('#paginator_count_ajax_coursesalbum').find('#total_item_count_coursesalbum').html());
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/coursesalbum/name/profile-photos/index/',
      'data': {
	format: 'html',
  ...
  en4.core.runonce.add(function() {
    var url = en4.core.baseUrl + 'widget/index/mod/coursesalbum/name/profile-photos/index/';
