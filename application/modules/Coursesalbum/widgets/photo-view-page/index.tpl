...
function doResizeForButton(){
	var width = sesJqueryObject('.coursesalbum_view_photo_container_wrapper').width();
  sesJqueryObject('#media_photo').css('max-width',width+'px');
	sesJqueryObject('#media_photo').css('max-height',maxHeight+'px');
	<?php if(Engine_Api::_()->user()->getViewer()->getIdentity() == '0'){ ?>
			return false;
	<?php } ?>
	var topPositionOfParentDiv =  sesJqueryObject(".coursesalbum_photo_view_option_btn").offset().top + 35;
	topPositionOfParentDiv = topPositionOfParentDiv+'px';
	var leftPositionOfParentDiv =  sesJqueryObject(".coursesalbum_photo_view_option_btn").offset().left - 135;
...
<?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != ''){ ?>
  en4.core.runonce.add(function() {
    var descEls = $$('.coursesalbum_view_photo_des');
    if( descEls.length > 0 ) {
    ...
    taggerInstance.addEvents({
      'onBegin' : function() {
				sesJqueryObject('.coursesalbum_photo_view_btns').hide();
        $('media_photo_next').erase('href');
      },
      'onEnd' : function() {
				sesJqueryObject('.coursesalbum_photo_view_btns').show();
...
<div class='coursesalbum_view_photo sesbasic_bxs sesbasic_clearfix'>
  <div class='coursesalbum_view_photo_container_wrapper sesbasic_clearfix'>
    <?php if( $this->album->count() > 1 ): ?>
      <div class="coursesalbum_view_photo_nav_btns">
        <?php 
        $photoPreviousData = Engine_Api::_()->coursesalbum()->getPreviousPhoto($this->album->album_id ,$this->photo->order ) ?  Engine_Api::_()->coursesalbum()->getPreviousPhoto($this->album->album_id,$this->photo->order) : null;
        echo $this->htmlLink((isset($photoPreviousData->album_id) ?  $photoPreviousData->getHref() : null ), '<i class="fa fa-angle-left"></i>', array('id' => 'photo_prev','data-url'=>$photoPreviousData->photo_id, 'class' => 'coursesalbum_view_photo_nav_prev_btn'));
        $photoNextData = Engine_Api::_()->coursesalbum()->getNextPhoto($this->album->album_id  ,$this->photo->order ) ?  Engine_Api::_()->coursesalbum()->getNextPhoto($this->album->album_id  ,$this->photo->order ) : null;
         ?>
        <?php echo $this->htmlLink(( isset($photoNextData->album_id) ?  $photoNextData->getHref() : null ), '<i class="fa fa-angle-right"></i>', array('id' => 'photo_next','data-url'=>$photoNextData->photo_id, 'class' => 'coursesalbum_view_photo_nav_nxt_btn')) ?>
      </div>
    <?php endif ?>
    <div class='coursesalbum_view_photo_container' id='media_photo_div'>
      <?php 
        $imageViewerURL = $this->photo->getHref();
        if($imageViewerURL != ''){
      ?>
        <a href="<?php echo $this->photo->getHref(); ?>" title="<?php echo $this->translate('Open image in image viewer'); ?>" onclick="openLightBoxForSesPlugins('<?php echo $imageViewerURL; ?>','<?php echo $this->photo->getPhotoUrl(); ?>');return false;" class="coursesalbum_view_photo_expend seslightbox_no_prop"><i class="fa fa-expand"></i></a>
      <?php } ?>
    ...
    <?php if( $this->canEdit ): ?>
      <div class="coursesalbum_view_photo_rotate_btns">          
        <a class="coursesalbum_icon_photos_rotate_ccw" id="ses-rotate-90" href="javascript:void(0)" onclick="sesPhotoRotate('<?php echo $this->photo->getIdentity() ?>','90')">&nbsp;</a>
        <a class="coursesalbum_icon_photos_rotate_cw" id="ses-rotate-270" href="javascript:void(0)" onclick="sesPhotoRotate('<?php echo $this->photo->getIdentity() ?>','270')">&nbsp;</a>
        <a class="coursesalbum_icon_photos_flip_horizontal" id="ses-rotate-horizontal"  href="javascript:void(0)" onclick="sesPhotoRotate('<?php echo $this->photo->getIdentity() ?>','horizontal')">&nbsp;</a>
        <a class="coursesalbum_icon_photos_flip_vertical" id="ses-rotate-vertical"  href="javascript:void(0)" onclick="sesPhotoRotate('<?php echo $this->photo->getIdentity() ?>','vertical')">&nbsp;</a>          
      </div>
    <?php endif ?>
    <?php
    ...
    ?>
      <div class="sesbasic_clearfix coursesalbum_photo_view_btns">
        <?php echo $this->partial('_socialShareIcons.tpl','sesbasic',array('resource' => $this->photo, 'param' => 'feed', 'socialshare_enable_plusicon' => $this->socialshare_enable_plusicon, 'socialshare_icon_limit' => $this->socialshare_icon_limit)); ?>
      <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != ''){ ?>
        <?php if($this->canTag){ ?>
        <a title="<?php echo $this->translate('Tag'); ?>" href="javascript:void(0);" onclick="taggerInstance.begin();" class="coursesalbum_view_tag_button"><i class="fa fa-tag"></i></a>
        <?php } ?>
        <span class="coursesalbum_photo_view_option_btn">
  ...
  <div class="coursesalbum_view_photo_count">
	...
	<div class="coursesalbum_photo_view_bottom_right">
    <?php if(isset($this->criteria)){ ?>
      <!-- Corresponding photo as per album id -->
      <div class="layout_coursesalbum_photo_strip">
        <div class="coursesalbum_photos_strip_slider sesbasic_clearfix clear">
          <a id="prevSlide" class="coursesalbum_photos_strip_slider_btn btn-prev"><i class="fa fa-angle-left"></i></a>
          <div class="coursesalbum_photos_strip_slider_content">
            <div id="coursesalbum_corresponding_photo" style="width:257px;">
            <?php if(!$this->is_ajax){ ?>
              <img id="coursesalbum_corresponding_photo_image" src="<?php echo $this->layout()->staticBaseUrl; ?>application/modules/Sesbasic/externals/images/loading.gif" alt="" style="margin-top:23px;" />
             <?php } ?>
            </div>
          </div>
          <a id="nextSlide" class="coursesalbum_photos_strip_slider_btn btn-nxt"><i class="fa fa-angle-right"></i></a>
        </div>
      </div>
    <?php } ?>
  </div>
	<div class="coursesalbum_photo_view_bottom_middle sesbasic_clearfix">
    <?php if( $this->photo->getTitle() ): ?>
      <div class="coursesalbum_view_photo_title">
        <?php echo $this->photo->getTitle(); ?>
      </div>
    <?php endif; ?>
    <div class="coursesalbum_view_photo_middle_box clear sesbasic_clearfix">
      <div class="coursesalbum_view_photo_owner_info sesbasic_clearfix">
        <div class="coursesalbum_view_photo_owner_photo">
        ...
        <div class="coursesalbum_view_photo_owner_details">
          <span class="coursesalbum_view_photo_owner_name sesbasic_text_light">
            by <?php echo $this->htmlLink($albumOwnerDetails->getHref(), $albumOwnerDetails->getTitle()); ?>
          </span>
          <span class="sesbasic_text_light coursesalbum_view_photo_date">
    ...
    <div class="coursesalbum_view_photo_info_left">
      <?php if( $this->photo->getDescription() ): ?>
        <div class="coursesalbum_view_photo_des">
          <b>Description</b>
          <?php echo nl2br($this->photo->getDescription()) ?>
        </div>
      <?php endif; ?>
      <div class="coursesalbum_view_photo_tags" id="media_tags" style="display: none;">
        <b><?php echo $this->translate('Tagged') ?></b>
      </div>
    </div>
    <!-- comment code-->
    <div class="coursesalbum_photo_view_bottom_comments layout_core_comments">
      <?php if(Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('sesadvancedcomment')){ ?>
      <?php echo $this->action("list", "comment", "sesadvancedcomment", array("type" => "coursesalbum_photo", "id" => $this->photo->getIdentity(),'is_ajax_load'=>true)); 
        }else{ echo $this->action("list", "comment", "core", array("type" => "coursesalbum_photo", "id" => $this->photo->getIdentity())); } ?> 
    </div>
  </div>
</div>
<script type="text/javascript">
var optionDataForButton;
optionDataForButton = '<div class="sesbasic_option_box"><?php if ($this->viewer()->getIdentity()):?><?php if( $this->canEdit ): ?><?php echo $this->htmlLink(array('route' => 'coursesalbum_extended', 'controller' => 'album', 'action' => 'edit-photo', 'photo_id' => $this->photo->getIdentity(), 'format' => 'smoothbox'), $this->translate('Edit'), array('class' => 'smoothboxOpenPhoto')) ?><?php endif; ?><?php if( $this->canDelete ): ?><?php echo $this->htmlLink(array('route' => 'coursesalbum_extended', 'controller' => 'photo', 'action' => 'delete', 'photo_id' => $this->photo->getIdentity(), 'format' => 'smoothbox'), $this->translate('Delete'), array('class' => 'smoothboxOpenPhoto')) ?><?php endif; ?><?php if( !$this->message_view ):?><?php echo $this->htmlLink(Array('module'=>'activity', 'controller'=>'index', 'action'=>'share', 'route'=>'default', 'type'=>$this->photo->getType(), 'id'=>$this->photo->getIdentity(), 'format' => 'smoothbox'), $this->translate("Share"), array('class' => 'smoothboxOpenPhoto')); ?><?php echo $this->htmlLink(Array('module'=>'core', 'controller'=>'report', 'action'=>'create', 'route'=>'default', 'subject'=>$this->photo->getGuid(), 'format' => 'smoothbox'), $this->translate("Report"), array('class' => 'smoothboxOpenPhoto')); ?><?php if( $this->canEdit ): ?><?php echo $this->htmlLink(array('route' => 'user_extended', 'module' => 'user', 'controller' => 'edit', 'action' => 'external-photo', 'photo' => $this->photo->getGuid(), 'format' => 'smoothbox'), $this->translate('Make Profile Photo'), array('class' => 'smoothboxOpenPhoto')) ?><?php endif; ?><?php endif;?><?php endif ?></div>';
...
function sesPhotoRotate(photo_id,rotateAngle){
	...
	if(rotateAngle == 90 || rotateAngle == 270){
		if(rotateAngle == 90)
			className = 'coursesalbum_icon_photos_rotate_ccw';
		else
			className = 'coursesalbum_icon_photos_rotate_cw';		
		rotatePhotoSes(photo_id,rotateAngle,className);
	}else{
		if(rotateAngle == 'horizontal')
			className = 'coursesalbum_icon_photos_flip_horizontal';
		else
			className = 'coursesalbum_icon_photos_flip_vertical';
...
function flipPhotoSes(photo_id,rotateAngle,className){
	request = new Request.JSON({
      url : en4.core.baseUrl + 'coursesalbum/photo/flip',
      data : {
        format : 'json',
        photo_id : photo_id,
...
function rotatePhotoSes(photo_id,rotateAngle,className){
	request = new Request.JSON({
      url : en4.core.baseUrl + 'coursesalbum/photo/rotate',
      data : {
        format : 'json',
        photo_id : photo_id,
        angle : rotateAngle
	...
	/*change next previous button click event*/
	 ...
	 function changeNextPrevious(thisObject){
			...
			var correspondingImageData = sesJqueryObject('#coursesalbum_corresponding_photo').html();
			var photo_id = sesJqueryObject(thisObject).attr('data-url');
			(new Request.HTML({
      method: 'post',
      'url':en4.core.baseUrl + 'widget/index/mod/coursesalbum/name/photo-view-page/',
      'data': {
        format: 'html',
				 photo_id : photo_id,
				params :'', 
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
				...
				<?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != ''){ ?>
					sesJqueryObject(optionDataForButton).appendTo('body');
				<?php } ?>
					sesJqueryObject('.layout_coursesalbum_photo_view_page').html(responseHTML);
					var width = sesJqueryObject('.coursesalbum_view_photo_container_wrapper').width();
					sesJqueryObject('#media_photo').css('max-width',width+'px');
					sesJqueryObject('#media_photo').css('max-height',maxHeight+'px');
					sesJqueryObject('#coursesalbum_corresponding_photo').html(correspondingImageData);
					sesJqueryObject('#coursesalbum_corresponding_photo > a').each(function(index){
					...
					sesJqueryObject('#coursesalbum_corresponding_photo > a').eq(3).addClass('slideuptovisible');
					sesJqueryObject('#coursesalbum_corresponding_photo').css('width',(countSlide*64)+'px');
      }
    })).send();
    return false;
	}
...
sesJqueryObject(document).on('click','.coursesalbum_corresponding_image_album',function(e){
		e.preventDefault();
...
function getCorrespondingImg(){
	(new Request.HTML({
      method: 'post',
      'url':en4.core.baseUrl + 'coursesalbum/photo/corresponding-image/album_id/<?php echo $this->album->album_id; ?>',
      'data': {
        format: 'html',
				is_ajax : 1,
      },
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
				if(responseHTML){
					sesJqueryObject('#coursesalbum_corresponding_photo').html(responseHTML);
					sesJqueryObject('#coursesalbum_corresponding_photo > a').each(function(index){
						sesJqueryObject(this).removeClass('slideuptovisible');
...
sesJqueryObject(document).on('mouseover','#prevSlide',function(e){
	var indexCurrent = 	sesJqueryObject('#coursesalbum_corresponding_photo > a.slideuptovisible').index();
...
sesJqueryObject(document).on('click','#nextSlide',function(){
	...
	if((countSlide-1) == indexCurrent || indexCurrent == '-1'){
		...
		var leftAttr = sesJqueryObject('#coursesalbum_corresponding_photo').css('left').replace('px','');
		leftAttr = leftAttr.replace('-','');		
		if(slideLeft>3){
			leftAttr = parseInt(leftAttr,10);
			sesJqueryObject('#coursesalbum_corresponding_photo').css('left','-'+(leftAttr+(64*4))+'px');
			sesJqueryObject('#coursesalbum_corresponding_photo > a').eq(indexCurrent).removeClass('slideuptovisible');
			sesJqueryObject('#coursesalbum_corresponding_photo > a').eq((indexCurrent+4)).addClass('slideuptovisible');
		}else{
			leftAttr = parseInt(64*slideLeft,10)+parseInt(leftAttr,10);
			sesJqueryObject('#coursesalbum_corresponding_photo').css('left','-'+leftAttr+'px');
			sesJqueryObject('#coursesalbum_corresponding_photo > a').eq(indexCurrent).removeClass('slideuptovisible');
			sesJqueryObject('#coursesalbum_corresponding_photo > a').eq((indexCurrent+slideLeft)).addClass('slideuptovisible');
		}
	}
});
sesJqueryObject(document).on('click','#prevSlide',function(){
 ...
 if(leftAttr == 0 || countSlide < 4 || indexCurrent == '-1'){
	...
	if(typeof type == 'number' && type > 3 ){
		...
		sesJqueryObject('#coursesalbum_corresponding_photo').css('left',leftAttr+'px');
		sesJqueryObject('#coursesalbum_corresponding_photo > a').eq(indexCurrent).removeClass('slideuptovisible');
		sesJqueryObject('#coursesalbum_corresponding_photo > a').eq(selectedindex).addClass('slideuptovisible');
	}
 }
	return false;
});
