 ...
 */
class Coursesalbum_Widget_AlbumViewPageController extends Engine_Content_Widget_Abstract {
  public function indexAction() {
    ...
    if(Engine_Api::_()->core()->hasSubject()){
      ...
      $album =  Engine_Api::_()->getItem('coursesalbum_album', $_POST['album_id']);
      $course =  Engine_Api::_()->getItem('coursesalbum_album', $_POST['course_id']);
    }
    $this->view->courseItem = $course;
    $this->view->album = $album;
    $this->view->album_id = $param['id'] = $album->album_id;
    $this->view->course_id = $param['course_id'] = $album->course_id;
    ...
    if($viewer->getIdentity() > 0) {
         ...
         $this->view->canEdit = $viewPermission = $editCourseRolePermission ? $editCourseRolePermission : $course->authorization()->isAllowed($viewer, 'edit');
         $this->view->canComment = $course->authorization()->isAllowed($viewer, 'comment');
         $this->view->canEditMemberLevelPermission = $editCourseRolePermission ? $editCourseRolePermission : $course->authorization()->isAllowed($viewer, 'edit');
         ...
         $this->view->canDeleteMemberLevelPermission  = $deleteCourseRolePermission ? $deleteCourseRolePermission : $course->authorization()->isAllowed($viewer, 'delete');
    }
    if(!$is_ajax) {
        // Prepare data
