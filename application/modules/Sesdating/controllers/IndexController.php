 ...
 * @version    $Id: IndexController.php  2018-09-21 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesdating_IndexController extends Core_Controller_Action_Standard {
  ...
  public function inboxAction() {
    ...
    Engine_Api::_()->getApi('message', 'sesdating')->setUnreadMessage($viewer);
  }
  ...
  public function friendshipRequestsAction() {
    ...
    $this->view->friendRequests = $newFriendRequests = Engine_Api::_()->getDbtable('notifications', 'sesdating')->getFriendrequestPaginator($viewer);
    $newFriendRequests->setCurrentPageNumber($this->_getParam('page'));
    Engine_Api::_()->getApi('message', 'sesdating')->setUnreadFriendRequest($viewer);
  ...
  public function newMessagesAction() {
    $this->view->messageCount = Engine_Api::_()->getApi('message', 'sesdating')->getMessagesUnreadCount(Engine_Api::_()->user()->getViewer());
  }

  public function markallmessageAction() {

    $viewer_id = Engine_Api::_()->user()->getViewer()->getIdentity();
    Engine_Api::_()->getDbtable('recipients', 'messages')->update(array('dating_read' => 1,  'inbox_read' => 1), array('`user_id` = ?' => $viewer_id));
