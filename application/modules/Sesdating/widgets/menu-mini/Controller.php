...
class Sesdating_Widget_MenuMiniController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if ($viewer->getIdentity()) {
      ...
      $this->view->messageCount = Engine_Api::_()->getApi('message', 'sesdating')->getMessagesUnreadCount($viewer);
      $this->view->requestCount = Engine_Api::_()->getDbtable('notifications', 'sesbasic')->hasNotifications($viewer, 'friend');
    }
    $this->view->poupup = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesdating.popupsign', 1);
    ...
    $this->view->loginsignup_logo = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesdating.loginsignuplogo', '');
    ...
    if($viewer_id != 0) {
		...
		$this->view->loginsignupbgimage = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesdating.loginsignupbgimage', '');
