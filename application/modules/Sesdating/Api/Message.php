 ...
 * @version    $Id: Message.php  2018-09-21 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesdating_Api_Message extends Core_Api_Abstract
{
  ...
  public function setUnreadMessage(Core_Model_Item_Abstract $user) {
    $userLastNotification = Engine_Api::_()->getApi('message', 'sesdating')->getUserLastMessage($user);
    ...
    //Engine_Api::_()->getDbtable('recipients', 'messages')->update(array('dating_read' => 1), array('`user_id` = ?' => $user->getIdentity(), 'dating_read = ?' => 0, 'inbox_read = ?' => 0));
  }
  ...
  public function setUnreadFriendRequest(Core_Model_Item_Abstract $user) {
   ...
   // Engine_Api::_()->getDbtable('notifications', 'activity')->update(array('view_notification' => 1), array('user_id = ?' => $user->getIdentity(), 'dating_read = ?' => 0, 'mitigated = ?' => 0, 'view_notification = ?' => 0));
  }
