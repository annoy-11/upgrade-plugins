 ...
 */
class Egroupjoinfees_Widget_ManageOrdersController extends Engine_Content_Widget_Abstract {
  public function indexAction() {
    $value = array();
    $this->view->group_id = $group_id = $value['group_id'] = $this->_getParam('group_id', false);
    if (!$group_id)
      return $this->setNoRender();
    $this->view->is_search_ajax = $is_search_ajax = isset($_POST['is_search_ajax']) ? $_POST['is_search_ajax'] : false;
    if (!$is_search_ajax) {
      $this->view->searchForm = $searchForm = new Egroupjoinfees_Form_Searchorder();
    }
    ...
    $this->view->orders = $orders = Engine_Api::_()->getDbtable('orders', 'egroupjoinfees')->manageOrders($value);
