...
class Egroupjoinfees_Model_DbTable_Orders extends Engine_Db_Table {
  protected $_rowClass = "Egroupjoinfees_Model_Order";
	...
	public function getOrders($params = array()){
		...
		$select = $this->select()->from($orderTableName)->setIntegrityCheck(false)
		 						->joinLeft($groupTableName, "$orderTableName.group_id = $groupTableName.group_id", array());
		$select->where('state =?','complete');
		 if(isset($params['viewer_id']))
		 	$select->where($orderTableName.'.owner_id =?',$params['viewer_id']);
		 if(isset($params['group_id']))
		 	$select->where($orderTableName.'.group_id =?',$params['group_id']);
		 if(isset($params['groupBy']))	
		 	$select->group($params['groupBy']);
			if (isset($params['view_type'])) {
				$now = date("Y-m-d H:i:s");
// 				if ($params['view_type'] == 'current')
// 						$select->where("$groupTableName.endtime >= '$now'");
// 			  else 
// 						$select->where("$groupTableName.endtime < ?", $now);
