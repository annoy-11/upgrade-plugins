...
<div class="sesbusiness_profile_tab_wrapper sesbusiness_profile_photos sesbasic_bxs">
    <div class="sesbusiness_profile_content_search sesbasic_clearfix">
      ...
      <?php if((!$this->is_ajax) && $this->canCreate && ($this->paginator->count() > 1 || $this->canUpload ) && (!$this->business_offer_count || $this->paginator->getTotalItemCount() <= $this->business_offer_count)): ?> 
        <?php if( $this->canUpload ): ?>
          <div class="_addbtn">
            <?php echo $this->htmlLink(array(
              'route' => 'sesbusinessoffer_general',
              'controller' => 'index',
              'action' => 'create',
              'parent_id' => $this->business_id,
    ...
    <div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" class="prelative">
      <ul class="sesbusinessoffer_profile_listings sesbasic_bxs sesbasic_clearfix" id="tabbed-widget_<?php echo $randonNumber; ?>">
  <?php endif;?>
  <?php if($this->paginator->getTotalItemCount() > 0) { ?>
		<?php foreach( $this->paginator as $item ): ?>
      <?php $resource = Engine_Api::_()->getItem('businesses', $item->parent_id); ?>
      <li class="sesbasic_bg">
        ...
        <div class="sesbusinessoffer_profile_top">
         <div class="sesbusinessoffer_profile_inner">
					<a href="<?php echo $item->getHref(); ?>" class="sesbusinessoffer_profile_img">
          ...
          <div class="sesbusinessoffer_profile_body">
              ...
              <span class="sesbusinessoffer_businessname"> 
                  <i class="fa fa-file"></i><a href="<?php echo $business->getHref(); ?>"><?php echo $business->getTitle(); ?></a>
              </span>
            <?php } ?>
							</span>
          ...
          <?php if($this->canEdit || $this->canDelete) { ?>
            <div class="sesbusinessoffer_control_btns">
              <?php if($this->canEdit && ($item->owner_id == $this->viewer->getIdentity() || $resource->owner_id == $this->viewer->getIdentity())) { ?>
              <?php echo $this->htmlLink(array('route' => 'sesbusinessoffer_general', 'controller' => 'index', 'action' => 'edit', 'parent_id' => $item->parent_id, 'businessoffer_id' => $item->getIdentity(), 'reset' => true,), $this->translate('<i class="fa fa-pencil"></i>'), array('class' => 'buttonlink', 'title' => 'Edit')); ?>
              <?php } ?>
              <?php if($this->canDelete && ($item->owner_id == $this->viewer->getIdentity() || $resource->owner_id == $this->viewer->getIdentity())) { ?>
              <?php echo $this->htmlLink(array('route' => 'sesbusinessoffer_general','controller' => 'index', 'action' => 'delete','parent_id' => $item->parent_id, 'businessoffer_id' => $item->getIdentity(), 'format' => 'smoothbox'), $this->translate('<i class="fa fa-trash"></i>'), array('class' => 'buttonlink smoothbox','title' => 'Delete')); ?>
              <?php } ?>
            </div>
          <?php } ?>
           <!-- Stats -->
...
<script type="text/javascript">
  ...
  function paggingNumber<?php echo $randonNumber; ?>(businessNum){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "widget/index/mod/sesbusinessoffer/name/profile-offers",
      'data': {
      format: 'html',
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/sesbusinessoffer/name/profile-offers/index/',
      'data': {
	format: 'html',
  ...
  en4.core.runonce.add(function() {
    var url = en4.core.baseUrl + 'widget/index/mod/sesbusinessoffer/name/profile-offers/index/';
