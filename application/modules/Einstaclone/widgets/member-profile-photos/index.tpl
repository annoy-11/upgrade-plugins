...
<script type="text/javascript">
  function loadMoreInstaProfilePhoto() {
    if ($('load_more'))
    ...
    en4.core.request.send(new Request.HTML({
      method: 'post',              
      'url': en4.core.baseUrl + 'widget/index/mod/einstaclone/name/member-profile-photos',
      'data': {
        format: 'html',
...
<?php } ?>
  ...
  <?php if (!empty($this->paginator) && $this->paginator->count() > 1): ?>
    <?php if ($this->paginator->getCurrentPageNumber() < $this->paginator->count()): ?>
      ...
      <div class="einstaclone_view_more einstaclone_load_btn" id="load_more" onclick="loadMoreInstaProfilePhoto();" style="display: block;">
...
<?php if($this->paginationType == 1): ?>
  <script type="text/javascript">    
    ...
    en4.core.runonce.add(function() {
      ...
      function ScrollLoaderProfilePhoto() { 
        var scrollTop = document.documentElement.scrollTop || document.body.scrollTop;
        if($('loadmore_list')) {
          if (scrollTop > 40)
            loadMoreInstaProfilePhoto();
        }
      }
      window.addEvent('scroll', function() { 
        ScrollLoaderProfilePhoto(); 
      });
    });
  </script>
<?php endif; ?>
