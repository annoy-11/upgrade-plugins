...
function doResizeForButton(){
	var width = sesJqueryObject('.sesnews_view_photo_container_wrapper').width();
  sesJqueryObject('#media_photo').css('max-width',width+'px');
	sesJqueryObject('#media_photo').css('max-height',maxHeight+'px');
	<?php if(Engine_Api::_()->user()->getViewer()->getIdentity() == '0'){ ?>
			return false;
	<?php } ?>
	var topPositionOfParentDiv =  sesJqueryObject(".sesnews_photo_view_option_btn").offset().top + 35;
	topPositionOfParentDiv = topPositionOfParentDiv+'px';
	var leftPositionOfParentDiv =  sesJqueryObject(".sesnews_photo_view_option_btn").offset().left - 135;
	leftPositionOfParentDiv = leftPositionOfParentDiv+'px';
	sesJqueryObject('.sesnews_album_option_box').css('top',topPositionOfParentDiv);
	sesJqueryObject('.sesnews_album_option_box').css('left',leftPositionOfParentDiv);
}
 var width = sesJqueryObject('.sesnews_view_photo_container_wrapper').width();
  sesJqueryObject('#media_photo').css('max-width',width+'px');
	sesJqueryObject('#media_photo').css('max-height',maxHeight+'px');
...
<?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != ''){ ?>
  en4.core.runonce.add(function() {
    ...
    taggerInstance.addEvents({
      'onBegin' : function() {
				sesJqueryObject('.sesnews_photo_view_btns').hide();
        $('media_photo_next').erase('href');
      },
      'onEnd' : function() {
				sesJqueryObject('.sesnews_photo_view_btns').show();
	...
	sesJqueryObject(document).click(function(news){
		if(news.target.id == 'parent_container_option' || news.target.id == 'fa-ellipsis-v'){
			if(sesJqueryObject('#parent_container_option').hasClass('active')){
				...
				sesJqueryObject('.sesnews_album_option_box').hide();	
			}else{
				sesJqueryObject('#parent_container_option').addClass('active');
				sesJqueryObject('.sesnews_album_option_box').show();	
		  }
		}else{
			sesJqueryObject('#parent_container_option').removeClass('active');
			sesJqueryObject('.sesnews_album_option_box').hide();	
...
function rotatePhotoSes(photo_id,rotateAngle,className){
	...
	/*change next previous button click event*/
	 ...
	 function changeNextPrevious(thisObject){
			...
			var correspondingImageData = sesJqueryObject('#sesnews_corresponding_photo').html();
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
				if(sesJqueryObject('.sesnews_album_option_box').length >0)
					sesJqueryObject('.sesnews_album_option_box').remove();
				<?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != ''){ ?>
					sesJqueryObject(optionDataForButton).appendTo('body');
				<?php } ?>
					sesJqueryObject('.layout_sesnews_photo_view_page').html(responseHTML);
					var width = sesJqueryObject('.sesnews_view_photo_container_wrapper').width();
					sesJqueryObject('#media_photo').css('max-width',width+'px');
					sesJqueryObject('#media_photo').css('max-height',maxHeight+'px');
					sesJqueryObject('#sesnews_corresponding_photo').html(correspondingImageData);
					sesJqueryObject('#sesnews_corresponding_photo > a').each(function(index){
					...
					sesJqueryObject('#sesnews_corresponding_photo > a').eq(3).addClass('slideuptovisible');
					sesJqueryObject('#sesnews_corresponding_photo').css('width',(countSlide*64)+'px');
      }
    })).send();
    return false;
	}
...
sesJqueryObject(document).on('click','.sesnews_corresponding_image_album',function(e){
		e.preventDefault();
...
function getCorrespondingImg(){
	(new Request.HTML({
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
				if(responseHTML){
					sesJqueryObject('#sesnews_corresponding_photo').html(responseHTML);
					sesJqueryObject('#sesnews_corresponding_photo > a').each(function(index){
						sesJqueryObject(this).removeClass('slideuptovisible');
...
sesJqueryObject(document).on('mouseover','#prevSlide',function(e){
	var indexCurrent = 	sesJqueryObject('#sesnews_corresponding_photo > a.slideuptovisible').index();
...
sesJqueryObject(document).on('click','#nextSlide',function(){
	...
	if((countSlide-1) == indexCurrent || indexCurrent == '-1'){
		...
		var leftAttr = sesJqueryObject('#sesnews_corresponding_photo').css('left').replace('px','');
		leftAttr = leftAttr.replace('-','');		
		if(slideLeft>3){
			leftAttr = parseInt(leftAttr,10);
			sesJqueryObject('#sesnews_corresponding_photo').css('left','-'+(leftAttr+(64*4))+'px');
			sesJqueryObject('#sesnews_corresponding_photo > a').eq(indexCurrent).removeClass('slideuptovisible');
			sesJqueryObject('#sesnews_corresponding_photo > a').eq((indexCurrent+4)).addClass('slideuptovisible');
		}else{
			leftAttr = parseInt(64*slideLeft,10)+parseInt(leftAttr,10);
			sesJqueryObject('#sesnews_corresponding_photo').css('left','-'+leftAttr+'px');
			sesJqueryObject('#sesnews_corresponding_photo > a').eq(indexCurrent).removeClass('slideuptovisible');
			sesJqueryObject('#sesnews_corresponding_photo > a').eq((indexCurrent+slideLeft)).addClass('slideuptovisible');
		}
	}
});
sesJqueryObject(document).on('click','#prevSlide',function(){
 ...
 if(leftAttr == 0 || countSlide < 4 || indexCurrent == '-1'){
	...
	if(typeof type == 'number' && type > 3 ){
		...
		sesJqueryObject('#sesnews_corresponding_photo').css('left',leftAttr+'px');
		sesJqueryObject('#sesnews_corresponding_photo > a').eq(indexCurrent).removeClass('slideuptovisible');
		sesJqueryObject('#sesnews_corresponding_photo > a').eq(selectedindex).addClass('slideuptovisible');
	}
 }
	return false;
});
