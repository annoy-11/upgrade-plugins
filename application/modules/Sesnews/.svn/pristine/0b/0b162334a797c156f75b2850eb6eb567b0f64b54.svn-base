...
<script type="text/javascript">
  ...
  function checkAvailsbility(submitform) {
    ...
    return;
    jqueryObjectOfSes('#sesnews_custom_url_wrong').hide();
    jqueryObjectOfSes('#sesnews_custom_url_correct').hide();
    jqueryObjectOfSes('#sesnews_custom_url_loading').css('display','inline-block');
    jqueryObjectOfSes.post('<?php echo $this->url(array('controller' => 'index','module'=>'sesnews', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value},function(response){
      jqueryObjectOfSes('#sesnews_custom_url_loading').hide();
      response = jqueryObjectOfSes.parseJSON(response);
      if(response.error){
        jqueryObjectOfSes('#sesnews_custom_url_correct').hide();
        jqueryObjectOfSes('#sesnews_custom_url_wrong').css('display','inline-block');
        ...
        jqueryObjectOfSes('#sesnews_custom_url_wrong').hide();
        jqueryObjectOfSes('#sesnews_custom_url_correct').css('display','inline-block');
        if(typeof submitform != 'undefined') {
...
<script type="text/javascript">
  ...
  jqueryObjectOfSes("#sesnews_create").bind("keypress", function (e) {
  ...
  function validateForm() {
    ...
    jqueryObjectOfSes('#sesnews_create input, #sesnews_create select,#sesnews_create checkbox,#sesnews_create textarea,#sesnews_create radio').each(
      function(index){
	  ...
	  if(error){
	    ...
	    counter++
			...
			if(sesJqueryObject('#tabs_form_newscreate-wrapper').length && sesJqueryObject('.sesnews_upload_item_photo').length == 0){
				<?php if($required):?>
					objectError = sesJqueryObject('.sesnews_create_form_tabs');
					error = true;
				<?php endif;?>
			}		
	  }
...
en4.core.runonce.add(function() {
  jqueryObjectOfSes('#sesnews_create').submit(function(e) {
...
en4.core.runonce.add(function() {
  var sesnews_create_form_tabsSesnews = jqueryObjectOfSes('#sesnews_create_form_tabs li a');
...
function createStatusbar(obj,file)
{
     ...
     if(rowCount %2 ==0) row ="even";
		  var checkedId = jqueryObjectOfSes("input[name=cover]:checked");
			this.objectInsert = jqueryObjectOfSes('<div class="sesnews_upload_item sesbm '+row+'"></div>');
			this.overlay = jqueryObjectOfSes("<div class='overlay sesnews_upload_item_overlay'></div>").appendTo(this.objectInsert);
			this.abort = jqueryObjectOfSes('<div class="abort sesnews_upload_item_abort" id="abortPhoto_'+countUploadSes+'"><span><?php echo $this->translate("Cancel Uploading"); ?></span></div>').appendTo(this.objectInsert);
			this.progressBar = jqueryObjectOfSes('<div class="overlay_image progressBar"><div></div></div>').appendTo(this.objectInsert);
			this.imageContainer = jqueryObjectOfSes('<div class="sesnews_upload_item_photo"></div>').appendTo(this.objectInsert);
			this.src = jqueryObjectOfSes('<img src="'+en4.core.baseUrl+'application/modules/Sesnews/externals/images/blank-img.gif">').appendTo(this.imageContainer);
			this.infoContainer = jqueryObjectOfSes('<div class=sesnews_upload_photo_info sesbasic_clearfix"></div>').appendTo(this.objectInsert);
			this.size = jqueryObjectOfSes('<span class="sesnews_upload_item_size sesbasic_text_light"></span>').appendTo(this.infoContainer);
			this.filename = jqueryObjectOfSes('<span class="sesnews_upload_item_name"></span>').appendTo(this.infoContainer);
			this.option = jqueryObjectOfSes('<div class="sesnews_upload_item_options clear sesbasic_clearfix"><span class="sesnews_upload_item_radio"><input type="radio" id="main_photo_id'+rowCount+'" name="cover"><label for="main_photo_id'+rowCount+'"><?php echo $this->translate("Main Photo"); ?></label></span><a class="edit_image_upload" href="javascript:void(0);"><i class="fa fa-pencil"></i></a><a class="delete_image_upload" href="javascript:void(0);"><i class="fa fa-trash"></i></a></div>').appendTo(this.objectInsert);
		...
		if (1) {
			if(jqueryObjectOfSes('#show_photo').children('div').length == 1) {
				var idPhoto = jqueryObjectOfSes('#show_photo').eq(0).find('.sesnews_upload_item_radio').find('input').attr('id');
...
var dragandrophandlerSesnews = sesJqueryObject('#dragandrophandler');
...
jqueryObjectOfSes(document).on('click','.edit_image_upload',function(e){
  e.preventDefault();
  var photo_id = jqueryObjectOfSes(this).closest('.sesnews_upload_item_options').attr('data-src');
  if(photo_id){
    editImage(photo_id);
  }else
    return false;
...
jqueryObjectOfSes(document).on('click','.delete_image_upload',function(e){
	e.preventDefault();
	jqueryObjectOfSes(this).parent().parent().find('.sesnews_upload_item_overlay').css('display','block');
	var sesthat = this;
	var isCover = jqueryObjectOfSes(this).closest('.sesnews_upload_item_options').find('.sesnews_upload_item_radio').find('input').prop('checked');
	var photo_id = jqueryObjectOfSes(this).closest('.sesnews_upload_item_options').attr('data-src');
	if(photo_id){
...
<?php if($this->typesmoothbox) { ?>
	...
	function showHideOptionsSesnews(display){
		var elem = sesJqueryObject('.sesnews_hideelement_smoothbox');
		for(var i = 0 ; i < elem.length ; i++){
				sesJqueryObject(elem[i]).parent().parent().css('display',display);
		}
	}
	function checkSettingSesnews(first){
		var hideShowOption = sesJqueryObject('#advanced_sesnewsoptions').hasClass('active');
			if(hideShowOption){
					showHideOptionsSesnews('none');
					if(typeof first == 'undefined'){
						sesJqueryObject('#advanced_sesnewsoptions').html("<i class='fa fa-plus-circle'></i><?php echo $this->translate('Show Advanced Settings') ?>");
					}
					sesJqueryObject('#advanced_sesnewsoptions').removeClass('active');
			}else{
					showHideOptionsSesnews('block');
					sesJqueryObject('#advanced_sesnewsoptions').html("<i class='fa fa-minus-circle'></i><?php echo $this->translate('Hide Advanced Settings') ?>");
						sesJqueryObject('#advanced_sesnewsoptions').addClass('active');
			}	
	}
	en4.core.runonce.add(function()
  {
		sesJqueryObject('#advanced_sesnewsoptions').click(function(e){
			checkSettingSesnews();
		});
		sesJqueryObject('#advanced_sesnewsoptions').html("<i class='fa fa-plus-circle'></i><?php echo $this->translate('Show Advanced Settings') ?>");
