...
<div class="sesbasic_dashboard_content sesnews_manage_role_form sesbm sesbasic_clearfix">
	...
	<div class="sesnews_footer_contant">
		...
		<div id="manage_admin">
          ...
          <?php if($newsAdmin->user_id != $this->news->owner_id):?>
						<a class="remove_news" href="javascript:void(0);" onclick="removeUser('<?php echo $newsAdmin->news_id;?>','<?php echo $newsAdmin->role_id;?>');"><i class="fa fa-close"></i></a>
          <?php endif;?>
...
<script type="text/javascript">
	function showAutosuggest(newsAdmin, imageId) {
	  var contentAutocomplete1 =  'contentAutocomplete-'+newsAdmin
		contentAutocomplete1 = new Autocompleter.Request.JSON(newsAdmin, "<?php echo $this->url(array('module' => 'sesnews', 'controller' => 'dashboard', 'action' => 'get-members', 'news_id' => $this->news->news_id), 'default', true) ?>", {
			'postVar': 'text',
			'minLength': 1,
			'selectMode': '',
      ...
      $(imageId).innerHTML = selected.retrieve('autocompleteChoice').photo;
			sesJqueryObject('#'+newsAdmin).attr('rel', selected.retrieve('autocompleteChoice').id);
	...
	sesJqueryObject(document).on('keyup', 'input[id^="news_admin"]', function(event) {
	...
	function addMore() {
	  ...
	  sesJqueryObject('#manage_admin_input').append('<div class="sesnews_manage_roles_item"><span class="show_img" id="show_default_img_'+count+'"'+'></span> <input type="text" placeholder="Type Member Name" size="20" name="'+ColumnId+'"' +'id="'+ColumnId+'"'+'autocomplete="off" rel="'+count+'"><a class="remove_icon" href="javascript:void(0);" onclick="removeInputForm('+"'"+ColumnId+"'"+');"><i class="fa fa-close" id="close_option_'+count+'"'+'></i></a></div>');
