...
<?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
  ...
  <li class="sesnews_owner_review sesbasic_clearfix">
    ...
    <div class="sesnews_review_listing_desc sesbasic_clearfix">
      <?php if(in_array('pros', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesnews.show.pros', 1) && $item->pros): ?>
      ...
      <?php endif; ?>
      <?php if(in_array('cons', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesnews.show.cons', 1) && $item->cons): ?>
        <p class="sesnews_review_listing_body">
          <b><?php echo $this->translate("Cons: "); ?></b>
          <?php echo $this->string()->truncate($this->string()->stripTags($item->cons), 300) ?>
        </p>
      <?php endif; ?>
      <?php if(in_array('description', $this->stats) && $item->description && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesnews.review.summary', 1)): ?>
      <p class='sesnews_review_listing_body'>
    ...
    <div class="sesnews_review_listing_footer clear sesbasic_clearfix ">
			...
			<div class='sesnews_review_listing_btn_right floatR '>
				<?php if($item->authorization()->isAllowed($this->viewer(), 'edit')) { ?>
					<a class="fa fa-pencil sesbasic_button sesbasic_button_icon " href="<?php echo $this->url(array('slug' => $item->getSlug(), 'action' => 'edit', 'review_id' => $item->getIdentity()), 'sesnewsreview_view', 'true');?>"><span><i class="fa fa-caret-down"></i><?php echo $this->translate('Edit Review');?></span></a>
				<?php } ?>
				...
				<?php } ?>
				<?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesnews.show.report', 1) && $this->viewer()->getIdentity() && in_array('report', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesnews.enable.report', 1)): ?>
				...
				<?php endif; ?>
				<?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesnews.allow.share', 1) && $this->viewer()->getIdentity() && in_array('share', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesnews.enable.sharing', 1)): ?>
		...
		function viewMore_<?php echo $randonNumber; ?> () {
			...
			requestViewMore_<?php echo $randonNumber; ?> = new Request.HTML({
				...
				onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
					sesJqueryObject('#sesnews_review_listing').append(responseHTML);
	...
	<?php }else{ ?>
		function paggingNumber<?php echo $randonNumber; ?>(pageNum){
			...
			requestViewMore_<?php echo $randonNumber; ?> = (new Request.HTML({
				...
				onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
					sesJqueryObject('#sesnews_review_listing').html(responseHTML);
