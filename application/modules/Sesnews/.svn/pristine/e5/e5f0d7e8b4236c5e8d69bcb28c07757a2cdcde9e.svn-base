	...
	<?php  if((isset($this->socialSharing)  && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesnews.enable.sharing', 1)) || isset($this->likeButton)){  ?>
	  <span class="sesnews_album_list_grid_btns">
	    ...
	    <?php } if(isset($this->favouriteButton) && Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesnews.enable.favourite', 1)){ ?>
	     <?php $favStatus = Engine_Api::_()->getDbtable('favourites', 'sesnews')->isFavourite(array('resource_type'=>'sesnews_album','resource_id'=>'album_id')); ?>
        <a href="javascript:;" data-url='<?php echo $album->album_id; ?>' class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn sesnews_albumfavourite <?php echo ($favStatus) ? 'button_active' : '' ; ?>">
...
<script type="text/javascript">
  ...
  function paggingNumber<?php echo $randonNumber; ?>(pageNum){
	...
	document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML =  responseHTML;	sesJqueryObject('#paginator_count_sesnews').find('#total_item_count_sesnews').html(sesJqueryObject('#paginator_count_ajax_sesnews').find('#total_item_count_sesnews').html());
	sesJqueryObject('#paginator_count_ajax_sesnews').remove();
      }
    })).send();
    return false;
  }
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
	...
	sesJqueryObject('#paginator_count_sesnews').find('#total_item_count_sesnews').html(sesJqueryObject('#paginator_count_ajax_sesnews').find('#total_item_count_sesnews'));
	sesJqueryObject('#paginator_count_ajax_sesnews').remove();
