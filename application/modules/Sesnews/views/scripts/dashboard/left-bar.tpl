...
function getDataThroughAjax(url){
	...
	if(typeof ajaxRequest != 'undefined')
		ajaxRequest.cancel();
	sesJqueryObject('.sesnews_dashboard_content').html('<div class="sesbasic_loading_container"></div>');
	ajaxRequest = new Request.HTML({
      ...
      onComplete: function(response) {
				sesJqueryObject('.sesnews_dashboard_content').html(response);
...
sesJqueryObject(document).on('click','.sesnews_ajax_delete',function(e){
  ...
  sesJqueryObject(document).on('submit','#sesnews_ajax_form_submit',function(e){
    ...
    if(data){
    sesJqueryObject('.sesnews_dashboard_content').html(data);
    }else{
    alert('Something went wrong,please try again later');	
    }
    }
