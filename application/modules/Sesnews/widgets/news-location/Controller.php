 ...
 */
class Sesnews_Widget_NewsLocationController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->lat = $value['lat'] = isset($searchArray['lat']) ? $searchArray['lat'] : (isset($_GET['lat']) ? $_GET['lat'] : (isset($params['lat']) ? $params['lat'] : ''));
    $value['show'] = isset($searchArray['show']) ? $searchArray['show'] : (isset($_GET['show']) ? $_GET['show'] : (isset($params['show']) ? $params['show'] : ''));
    $this->view->lng = $value['lng'] = isset($searchArray['lng']) ? $searchArray['lng'] : (isset($_GET['lng']) ? $_GET['lng'] : (isset($params['lng']) ? $params['lng'] : ''));
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('news', 'sesnews')->getSesnewsPaginator($value, true);
    $paginator->setItemCountPerPage($limit);
    $paginator->setCurrentPageNumber(1);
    $this->view->widgetName = 'news-location';
  }
