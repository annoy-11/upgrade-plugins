 ...
 * @version    $Id: AdminFieldsReviewController.php  2019-02-27 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesnews_AdminFieldsReviewController extends Fields_Controller_AdminAbstract {

  protected $_requireProfileType = true;
  protected $_fieldType = 'sesnews_review';

  public function indexAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesnews_admin_main', array(), 'sesnews_admin_main_reviewsettings');

    $this->view->subnavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesnews_admin_main_reviewsettings', array(), 'sesnews_admin_main_subreviewparameters');
  ...
  public function fieldEditAction() {
    ...
    // Create form
    $this->view->form = $form = new $adminFormClass();
    $form->setTitle('Edit News Category Review Profile Question');
    $form->removeElement('show');
    $form->removeElement('search');
    $form->addElement('hidden', 'show', array('value' => 0));
    $form->removeElement('display');
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Sesnews_Form_Admin_Review_Type();
    if (!$this->getRequest()->isPost()) {
      return;
    }
    if (!$form->isValid($this->getRequest()->getPost())) {
    ...
    if ($form->getValue('duplicate') != 'null') {
      ...
      $field_map_array = $db->select()
              ->from('engine4_sesnews_review_fields_maps')
      ...
      $field_meta_array = $db->select()
              ->from('engine4_sesnews_review_fields_meta')
              ->where('field_id IN (' . implode(', ', $child_id_array) . ')')
              ->query()
              ->fetchAll();
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_sesnews_review_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_sesnews_review_fields_maps', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      // Engine_Api::_()->getDbtable('categories', 'sesnews')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
