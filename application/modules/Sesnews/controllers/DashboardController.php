  ...
  public function editAction() {
    ...
    try
    {
      ...
      if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '' && $_POST['location']) {
        Engine_Db_Table::getDefaultAdapter()->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $sesnews->getIdentity() . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","sesnews_news") ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      } else {
        $sesnews->location = '';
        $sesnews->save();
        $dbInsert = Engine_Db_Table::getDefaultAdapter();
        $dbInsert->query('DELETE FROM `engine4_sesbasic_locations` WHERE `engine4_sesbasic_locations`.`resource_type` = "sesnews_news" AND `engine4_sesbasic_locations`.`resource_id` = "'.$sesnews->getIdentity().'";');
      }
