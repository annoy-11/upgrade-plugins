...
class Sesdocument_AdminCategoriesController extends Core_Controller_Action_Admin {

  public function indexAction() {
       $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesdocument_admin_main', array(), 'sesdocument_admin_main_category');
    ...
    if (isset($_POST['selectDeleted']) && $_POST['selectDeleted']) {
      ...
      if (isset($_POST['data']) && is_array($_POST['data'])) {
        ...
        $deleteCategoryIds = array();
				 $eventTable = Engine_Api::_()->getDbtable('sesdocuments', 'sesdocument');
        foreach ($_POST['data'] as $key => $valueSelectedcategory) {

          $categoryDelete = Engine_Api::_()->getItem('sesdocument_category', $valueSelectedcategory);

          $deleteCategory = Engine_Api::_()->getDbtable('categories', 'sesdocument')->deleteCategory($categoryDelete);
    ...
    if (isset($_POST['is_ajax']) && $_POST['is_ajax'] == 1) {
      ...
      $slugExists = Engine_Api::_()->getDbtable('categories', 'sesdocument')->slugExists($value['slug']);
      if (!$slugExists) {
        echo json_encode(array('slugError' => true));
        die;
      }
      if ($cat_id != -1) {
        $categoryData = Engine_Api::_()->getItem('sesdocument_category', $cat_id);
        if ($categoryData->subcat_id == 0) {
          $value['subcat_id'] = $cat_id;
          $seprator = '&nbsp;&nbsp;&nbsp;';
          $tableSeprator = '-&nbsp;';
          $parentId = $cat_id;
          $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesdocument')->orderNext(array('subcat_id' => $cat_id));
        } else {
          $value['subsubcat_id'] = $cat_id;
          $seprator = '3';
          $tableSeprator = '--&nbsp;';
          $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesdocument')->orderNext(array('subsubcat_id' => $cat_id));
          $parentId = $cat_id;
        }
      } else {
        $parentId = 0;
        $seprator = '';
        $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesdocument')->orderNext(array('category_id' => true));
        $tableSeprator = '';
      }
      $db = Engine_Db_Table::getDefaultAdapter();
      $db->beginTransaction();
      try {
        $categoriesTable = Engine_Api::_()->getDbtable('categories', 'sesdocument');
    ...
    //Get all categories
    $this->view->categories = Engine_Api::_()->getDbtable('categories', 'sesdocument')->getCategory(array('column_name' => '*', 'profile_type' => true));
  }

  public function changeOrderAction() {
    if ($this->_getParam('id', false) || $this->_getParam('nextid', false)) {
      $id = $this->_getParam('id', false);
      $order = $this->_getParam('sesdocument', false);
      $order = explode(',', $order);
      ...
      $categoryTypeId = '';
      $checkTypeCategory = $dbObject->query("SELECT * FROM engine4_sesdocument_categories WHERE category_id = " . $category_id)->fetchAll();
      ...
      if ($checkTypeCategory)
        $currentOrder = Engine_Api::_()->getDbtable('categories', 'sesdocument')->order($categoryType, $categoryTypeId);
      ...
      $checkCategoryChildrenCondition = $dbObject->query("SELECT * FROM engine4_sesdocument_categories WHERE subcat_id = '" . $id . "' || subsubcat_id = '" . $id . "' || subcat_id = '" . $nextid . "' || subsubcat_id = '" . $nextid . "'")->fetchAll();
  ...
  public function editCategoryAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesdocument_admin_main', array(), 'sesdocument_admin_main_categories');

    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesdocument_admin_categories', array(), 'sesdocument_admin_main_subcategories');
    $this->view->form = $form = new Sesdocument_Form_Admin_Category_Edit();
    $cat_id = $this->_getParam('id');
    $category = Engine_Api::_()->getItem('sesdocument_category', $cat_id);
    ...
    if (!$form->isValid($this->getRequest()->getPost())) {
      ...
      if (isset($_POST['slug'])) {
        $slugExists = Engine_Api::_()->getDbtable('categories', 'sesdocument')->slugExists($value['slug'], $cat_id);
        if (!$slugExists) {
    ...
    try {
      ...
      $category->save();
      $deleteIc = $deleteTh = $deleteCI = false;
    ...
    return $this->_helper->redirector->gotoRoute(array('module' => 'sesdocument', 'action' => 'index', 'controller' => 'categories'), 'admin_default', true);
  }
  ...
  protected function setPhoto($photo, $cat_id, $resize = false) {
    ...
    $params = array(
        'parent_type' => 'sesdocument_category',
        'parent_id' => $cat_id,
    ...
    try {
      ...
      if ($e->getCode() == Storage_Model_DbTable_Files::SPACE_LIMIT_REACHED_CODE) {
        throw new Sesdocument_Model_Exception($e->getMessage(), $e->getCode());
      } else {
        throw $e;
      }
    }
