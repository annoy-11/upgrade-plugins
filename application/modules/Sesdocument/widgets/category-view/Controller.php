<?php
class Sesdocument_Widget_CategoryViewController extends Engine_Content_Widget_Abstract {
  public function indexAction() { 
    ...
    $this->view->limit_data = $limit_data = isset($params['document_limit']) ? $params['document_limit'] : $this->_getParam('document_limit', '10');
    ...
    $show_subcatcriterias = isset($params['show_subcatcriteria']) ? $params['show_subcatcriteria'] : $this->_getParam('show_subcatcriteria', array('countDocuments', 'icon', 'title'));
    ...
    if (Engine_Api::_()->core()->hasSubject()) {
      ...
      $this->view->category = $category = Engine_Api::_()->getItem('sesdocument_category', $params['category_id']);
      $category_id = $params['category_id'];
    }
    $innerCatData = array();
    if (!$is_ajax) {
      if ($category->subcat_id == 0 && $category->subsubcat_id == 0) {
        $innerCatData = Engine_Api::_()->getDbtable('categories', 'sesdocument')->getModuleSubcategory(array('category_id' => $category->category_id, 'column_name' => '*', 'countDocuments' => true,'getcategory0'=>true));
        $columnCategory = 'category_id';
      } else if ($category->subsubcat_id == 0) {
        $innerCatData = Engine_Api::_()->getDbtable('categories', 'sesdocument')->getModuleSubsubcategory(array('countDocuments' => true, 'category_id' => $category->category_id, 'column_name' => '*', 'countDocuments' => true,'getcategory0'=>true));
        $columnCategory = 'subcat_id';
      } else
        $columnCategory = 'subsubcat_id';
      $this->view->innerCatData = $innerCatData;  
      //breadcum
      $this->view->breadcrumb = $breadcrumb = Engine_Api::_()->getDbtable('categories', 'sesdocument')->getBreadcrumb($category);
    }  
     $columnCategory = isset($params['columnCategory']) ? $params['columnCategory'] : $columnCategory;
     
    $params = array('document_limit' => $limit_data, 'pagging' => $loadOptionData, 'show_criterias' => $show_criterias, 'view_type' => $view_type, 'category_id' => $categoryId, 'width' => $width, 'height' => $height, 'show_subcat' => $show_subcat, 'show_subcatcriteria' => $show_subcatcriterias, 'widthSubcat' => $widthSubcat, 'heightSubcat' => $heightSubcat,'columnCategory'=>$columnCategory); 
    
		$this->view->document_title = $this->_getParam('document_title','');
		...
		if($this->view->showPopularDocuments){
			$this->view->paginatorc = Engine_Api::_()->getDbTable('sesdocuments', 'sesdocument')
							->getDocumentPaginator(array_merge($value,array('search'=>1,'fetchAll'=>true,'limit_data'=>3,$columnCategory=>$category->getIdentity())));
		}
		
  
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('sesdocuments', 'sesdocument')->getDocumentPaginator(array_merge(array($columnCategory => $category_id,'getcategory0'=>true),$value), false);
