...
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesdocument/externals/styles/style_cover.css'); ?>
      ...
      <div class="sesevent_cover_cont sesbasic_clearfix">
        ...
        <?php if($this->can_edit && $this->photo == 'mPhoto'){ ?>
            ...
            <input type="file" id="uploadFileMainsesevent" name="main_photo_cvr" onchange="uploadFileMainsesevent(this);"  style="display:none" />
              ...
              <div class="sesevent_cover_change_cover_options sesbasic_option_box">
                <i class="sesevent_cover_change_cover_options_arrow"></i>
                <a href="javascript:;" id="change_main_cvr_pht"><i class="fa fa-plus"></i><?php echo $this->subject->file_id  ?  $this->translate("Change Main Photo") : $this->translate("Add Main Photo"); ?></a>
                <a style="display:<?php echo $this->subject->file_id ? 'block !important' : 'none !important' ; ?>;" href="javascript:;" id="sesevent_main_photo_i"><i class="fa fa-trash"></i><?php echo $this->translate("Remove Main Photo"); ?></a>
              </div>
            </div>
        <?php } ?>
          </div>
        <?php } ?>
          ...
          <?php } ?>
           <?php if(in_array('createdon',$this->show_criterias)){ ?>
            <div class="sesevent_cover_date clear sesbasic_clearfix"> 
              <i title='<?php echo $this->translate("Created On"); ?>' class="fa fa-clock-o"></i>
              <span>
          ...
          <?php if(in_array('createdby',$this->show_criterias)){ ?>
           ...
           <?php if(in_array('startEndDate',$this->show_criterias)){ ?>
            <div class="sesevent_cover_date clear sesbasic_clearfix sesevent_cover_time">
              <i title="<?php echo $this->translate("Start & End Date"); ?>" class="fa fa-calendar"></i>
          ...
          <?php } ?>
            ...
            <div class="sesevent_cover_date clear sesbasic_clearfix sesevent_cover_location">
              <i title='<?php echo $this->translate("Location"); ?>' class="fa fa-map-marker"></i>
              <span> 
          ...
          <div class="sesevent_cover_footer">
            <div class="sesevent_cover_footer_inner sesbasic_clearfix">
              ...
              <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesevent.allowfavourite', 1) && in_array('favouriteBtn',$this->show_criterias) && Engine_Api::_()->user()->getViewer()->getIdentity() != 0){
                 ...
                 $favClass = ($favStatus)  ? 'fa-heart' : 'fa-heart-o';
              ?>
                <div>
...
function take_snapshot() {
	...
	Webcam.snap(function(data_uri) {
		...
		sesJqueryObject('.sesevent_cover_container').append('<div id="sesevent_cover_loading" class="sesbasic_loading_cont_overlay"></div>');
		 Webcam.upload( data_uri, en4.core.staticBaseUrl+'sesevent/index/upload-cover/event_id/<?php echo $this->subject->event_id ?>' , function(code, text) {
...
function uploadFileToServerMain(files){
	...
	uploadURL = en4.core.staticBaseUrl+'sesevent/index/upload-main/event_id/<?php echo $this->subject->event_id ?>';
...
sesJqueryObject('#sesevent_main_photo_i').click(function(){
	...
	<?php } ?>
		...
		uploadURL = en4.core.staticBaseUrl+'sesevent/index/remove-main/event_id/'+event_id;
		var jqXHR=sesJqueryObject.ajax({
			url: uploadURL,
			type: "POST",
			contentType:false,
...
sesJqueryObject('#coverRemovesesevent').click(function(){
		...
		var event_id = '<?php echo $this->subject->event_id; ?>';
		uploadURL = en4.core.staticBaseUrl+'sesevent/index/remove-cover/event_id/'+event_id;
		var jqXHR=sesJqueryObject.ajax({
			url: uploadURL,
			type: "POST",
			contentType:false,
...
function uploadFileToServer(files){
	...
	var formData = new FormData();
	formData.append('Filedata', files);
	uploadURL = en4.core.staticBaseUrl+'sesevent/index/upload-cover/event_id/<?php echo $this->subject->event_id ?>';
	var jqXHR=sesJqueryObject.ajax({
    url: uploadURL,
    type: "POST",
    contentType:false,
