...
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesdocument/externals/styles/slideshow.css'); ?>

<div class="sesdocument_slideshow_document_wrapper sesbasic_clearfix sesbasic_bxs <?php if($this->navigation != 'nextprev'){ echo " isbulletnav " ; } echo $this->isfullwidth ? 'isfull_width' : '' ; ?>" style="height:<?php echo $this->height ?>px;">
  <div class="sesdocument_slideshow_document_container" style="height:<?php echo $this->height ?>px;">
    <div class="sesdocument_document_slideshow">
      <div class="sesdocument_slideshow_document" id="sesdocument_slideshow_<?php echo $this->identity; ?>">
        <ul class="bjqs">
          <?php foreach( $this->paginator as $item):  ?>
          <li class="sesdocument_slideshow_inner_view sesbasic_clearfix " style="height:<?php echo $this->height ?>px;">
            <div class="sesdocument_slideshow_inside">
              <div class="sesdocument_slideshow_thumb sesdocument_thumb" style="height:<?php echo $this->height ?>px;">
                <?php
                  $href = $item->getHref();
                  $imageURL = $item->getPhotoUrl('');
                ?>
                <a href="<?php echo $href; ?>" class="sesdocument_slideshow_thumb_img"> <span style="background-image:url(<?php echo $imageURL; ?>);"></span> </a>
                ...
                <div class="sesdocument_list_grid_thumb_btns">
                  ...
                  <?php $favStatus = Engine_Api::_()->getDbtable('favourites', 'sesdocument')->isFavourite(array('resource_type'=>'sesdocument_document','resource_id'=>$item->sesdocument_id)); ?>
                  <a href="javascript:;" class="sesbasic_icon_btn sesbasic_icon_btn_count sesdocument_favourite_sesdocument_document_<?php echo $item->sesdocument_id ;?>  sesbasic_icon_fav_btn sesdocument_favourite_sesdocument_document <?php echo ($favStatus)  ? 'button_active' : '' ?>"  data-url="<?php echo $item->sesdocument_id ; ?>"><i class="fa fa-heart"></i><span><?php echo $item->favourite_count; ?></span></a>
                  <?php endif;?>
                  <?php endif;?>
                </div>
                <?php endif;?>
            ...
            <div class="sesdocument_slideshow_inside_contant">
              <div class="slideshow_contant">
                <div class="sesdocument_slideshow_info sesbasic_clearfix ">
                  <?php if(isset($this->titleActive) ){ ?>
                  <span class="sesdocument_slideshow_info_title">
                  ...
                  <div class="admin_teg sesdocument_list_stats sesbasic_text_dark">
                    <?php $owner = $item->getOwner(); ?>
                     <span><i class="fa fa-user"></i> <?php echo $this->htmlLink($owner->getHref(),$owner->getTitle() ) ?> </span> </div>
                  <?php } ?>
                  <?php if(isset($this->creationDateActive)){ ?>
                  <div class="admin_teg sesdocument_list_stats sesbasic_text	_dark"><span> <i class="fa fa-calendar"></i> <?php echo ' '.date('M d, Y',strtotime($item->creation_date));?></span></div>
                  <?php } ?>
                  ...
                  <div class="sesdocument_slideshow-des">
                    ...
                    <?php if($categoryItem):?>
                    <div class="category_tag sesbasic_clearfix"> <span><a href="<?php echo $categoryItem->getHref(); ?>"><i class="fa fa-folder-open"></i> <?php echo $categoryItem->category_name; ?></a></span> </div>
                    <?php endif;?>
                    <?php endif;?>
                    <?php } ?>
                  </div>
...
<?php
$this->headScript()->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sesdocument/externals/scripts/slideshow/bjqs-1.3.min.js');
?>
<script type="text/javascript">
  window.addEvent('domready', function () {
		<?php if($this->isfullwidth){ ?>
			var htmlElement = document.getElementsByTagName("body")[0];
			htmlElement.addClass('sesdocument_category_slideshow');
		<?php } ?>
		...
		<?php } ?>
			...
			var	width = sesJqueryObject('#sesdocument_slideshow_<?php echo $this->identity; ?>').outerWidth();
			var	heigth = '<?php echo $this->height ?>';
			sesJqueryObject('#sesdocument_slideshow_<?php echo $this->identity; ?>').bjqs({
...
sesBasicAutoScroll('.documentslide_<?php echo $this->identity; ?>').on('init', function(event, slick, currentSlide, nextSlide){
  sesBasicAutoScroll('#sesdocument_carousel_<?php echo $this->identity; ?>').show();
});
</script>
