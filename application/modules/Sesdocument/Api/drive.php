...
include APPLICATION_PATH.DS.'application'.DS.'modules'.DS.'Sesdocument'.DS.'Api'.DS.'vendor/autoload.php';
...
class Drive
{
    ...
    public function __construct( $serviceAccountName = "Google Drive Upload", $createLink = null) {
        ...
        $url = ((!empty($_ENV["HTTPS"]) && 'on' == strtolower($_ENV["HTTPS"])) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $base . '/admin/sesdocument/settings';
        $this->_client->setApplicationName($serviceAccountName);
        $this->_client->setClientId(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesdocument_client_id'));
        $this->_client->setClientSecret(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesdocument_secret_key'));
        ...
        if ($createLink) {
            ...
            $refreshToken = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesdocument.google.refreshtoken');
            if (isset($_GET['code']) && !$refreshToken) {
                ...
                Engine_Api::_()->getApi('settings', 'core')->setSetting('sesdocument.google.refreshtoken', $token['refresh_token']);
