...
class Sesdocument_Model_DbTable_Membership extends Core_Model_DbTable_Membership {

  protected $_type = 'sesdocument';
  ...
  public function isResourceApprovalRequired(Core_Model_Item_Abstract $resource) {
	...
	public function getMembership($params){
		 $select = $this->select()->from($this->info('name'), array('*'))->where('resource_id =?',$params['sesdocument_id']);
