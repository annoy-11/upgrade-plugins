...
class Sesdocument_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Sesdocument_Model_Category';
  protected $_name = 'sesdocument_categories';
  ...
  public function getCategory($params = array(), $customParams = array()) {
    ...
    if (isset($params['criteria']) && $params['criteria'] == 'alphabetical')
      ...
      $documentTable = Engine_Api::_()->getDbTable('sesdocuments', 'sesdocument')->info('name');
    ...
    $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($documentTable, "$documentTable.category_id=$tableName.category_id AND (`".$documentTable."`.`is_approved` = 1 AND
        `".$documentTable."`.`draft` != 0 AND `".$documentTable."`.`search` = 0)", array("total_document_categories" => "COUNT($documentTable.sesdocument_id)"));

      //$category_select = $category_select->where($documentTable.'.is_approved =?','1');
      //$category_select = $category_select->where($documentTable.'.draft =?','1');

      //$category_select = $category_select->where($documentTable.'.is_delete =?','0');
         $category_select  = $category_select->group("$tableName.category_id");


				 if(isset($params['criteria']) && $params['criteria'] == 'most_document')
				 	 $category_select->order('total_document_categories DESC');
		if(isset($params['criteria']) && $params['criteria'] != 'most_document')
    ...
    if (isset($params['category_id']) && !empty($params['category_id']))
      $category_select = $category_select->where($tableName . '.category_id = ?', $params['category_id']);
			if(isset($params['hasDocument'])){
				$category_select = $category_select->having('total_document_categories > 0');
			}
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('sesdocument_category');
  ...
  public function getCategoriesAssoc($params = array()) {
    ...
    if ($viewerId && $params['member_levels'] == 1) {
      ...
      $stmt->where('CONCAT(engine4_sesdocument_categories.member_levels," ") LIKE "%' . $levelId . '%"');
    }
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countDocuments'])) {
      $documentTable = Engine_Api::_()->getDbTable('sesdocuments', 'sesdocument')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($documentTable, "$documentTable.subcat_id=$tableName.category_id", array("total_documents_categories" => "COUNT($documentTable.file_id)"));
      $category_select = $category_select->group("$tableName.category_id");
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countDocuments'])) {
      $documentTable = Engine_Api::_()->getDbTable('sesdocuments', 'sesdocument')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($documentTable, "$documentTable.subsubcat_id=$tableName.category_id", array("total_documents_categories" => "COUNT($documentTable.sesdocument_id)"));
