 ...
 * @version    $Id: Package.php  2018-10-16 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesgrouppackage_Model_Package extends Core_Model_Item_Abstract {
  ...
  public function getProduct() {
    if (null === $this->_product) {
      ...
      $this->_product = $productsTable->fetchRow($productsTable->select()
                      ->where('extension_type = ?', 'sesgroup_group')
  ...
  public function getPackageDescription() {
    ...
    $priceStr = Engine_Api::_()->sesgrouppackage()->getCurrencyPrice($this->price,'','');
  ...
  public function allowUploadVideo($item, $left = false) {
    ...
    else if ($item && $item instanceof Sesgroup_Model_Group)
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('video', $modulesEnable['modules'])) {
      ...
      //free package
      if ($orderspackage_id) {
        $itemTable = Engine_Api::_()->getDbTable('groups', 'sesgroup');
        $itemTableName = $itemTable->info('name');
        $subselect = $itemTable->select()->from($itemTableName, 'group_id')->where('orderspackage_id =?', $orderspackage_id);
        $select->where('parent_id IN (?)', $subselect)->where('parent_type =?', 'sesgroup_group');
      }
  ...
  public function allowUploadMusic($item, $left = false) {
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('music', $modulesEnable['modules'])) {
      ...
      $select = $albumSongTable->select()->from($albumSongTableName, array('total' => 'COUNT(albumsong_id)'))
              ...
              ->joinLeft($albumTableName, $albumSongTableName . '.album_id = ' . $albumTableName . '.album_id AND ' . $albumTableName . '.resource_type = "sesgroup_group"', null);
      //free package
      if ($orderspackage_id) {
        $itemTable = Engine_Api::_()->getDbTable('groups', 'sesgroup');
        $itemTableName = $itemTable->info('name');
  ...
  public function allowUploadPhoto($item, $left = false) {
    ...
    if ($modulesEnable && array_key_exists('modules', $modulesEnable) && in_array('photo', $modulesEnable['modules'])) {
      ...
      if ($orderspackage_id) {
        ...
        $itemTableName = $itemTable->info('name');
        $subselect = $itemTable->select()->from($itemTableName, 'group_id')->where('orderspackage_id =?', $orderspackage_id);
        $select->where('group_id IN (?)', $subselect);
      }
