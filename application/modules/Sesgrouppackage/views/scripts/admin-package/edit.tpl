 ...
 * @version    $Id: edit.tpl  2018-10-16 00:00:00 SocialEngineSolutions $
...
<div class="sesbasic_search_result">
	<?php echo $this->htmlLink($this->url(array('module' => 'sesgrouppackage', 'controller' => 'package','action'=>'index')), $this->translate("Back to Manage Package"), array('class' => 'buttonlink sesbasic_icon_back')); ?>
</div><br />
...
<script type="text/javascript">
  var fetchLevelSettings = function(level_id) {
    window.location.href = en4.core.baseUrl + 'admin/sesgroup/settings/level/id/' + level_id;
    //alert(level_id);
  }
  ...
  sesJqueryObject(document).on('change','input[type=radio][name=group_choose_style]',function(){
    if (this.value == 1) {
      sesJqueryObject('#group_chooselayout-wrapper').show();
      sesJqueryObject('#group_style_type-wrapper').hide();
    }else{
      sesJqueryObject('#group_style_type-wrapper').show();
      sesJqueryObject('#group_chooselayout-wrapper').hide();
    }
  });
  window.addEvent('domready', function() {
   ...
   var valueStyle = sesJqueryObject('input[name=group_choose_style]:checked').val();
    if(valueStyle == 1) {
      sesJqueryObject('#group_chooselayout-wrapper').show();
      sesJqueryObject('#group_style_type-wrapper').hide();
    }
