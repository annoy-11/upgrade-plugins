...
<?php $options = Engine_Api::_()->getDbTable('options','sesgroup')->getAllOptions();
	if(count($options)){ ?>
<div id="custom-fields-container" style="display:none;">
  <?php foreach($options as $valueOptions){
      ...
      $metaValues = Engine_Api::_()->getDbTable('metas','sesgroup')->getMetaData($valueOptions['option_id']);
      if(!count($metaValues))
      	continue;
   ?>
   <div>
