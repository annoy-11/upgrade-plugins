...
class Sesgrouppackage_AdminPackageController extends Core_Controller_Action_Admin {

  public function indexAction() {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sesgroup_admin_main', array(), 'sesgroup_admin_packagesetting');

    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesgroup_admin_packagesetting', array(), 'sesgroup_admin_package');
    ...
    // Make form
    $this->view->formFilter = $formFilter = new Sesgrouppackage_Form_Admin_Package_Filter();
    ...
    // Initialize select
    $table = Engine_Api::_()->getDbtable('packages', 'sesgrouppackage');
    $select = $table->select();
    ...
    foreach ($paginator as $item) {
      $groupCounts[$item->package_id] = Engine_Api::_()->getDbtable('groups', 'sesgroup')
              ->select()
              ->from('engine4_sesgroup_groups', new Zend_Db_Expr('COUNT(*)'))
              ->where('package_id = ?', $item->package_id)
              ->where('is_approved = ?', true)
              ->query()
              ->fetchColumn();
    }
    $this->view->groupCounts = $groupCounts;
  }
  ...
  public function settingsAction() {
    ...
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sesgroup_admin_main', array(), 'sesgroup_admin_packagesetting');
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesgroup_admin_packagesetting', array(), 'sesgroup_admin_subpackagesetting');

    // Make form
    $this->view->form = $form = new Sesgrouppackage_Form_Admin_Package_Settings();

    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      $values = $form->getValues();
      unset($values['commision']);
      include_once APPLICATION_PATH . "/application/modules/Sesgrouppackage/controllers/License.php";
      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgrouppackage.pluginactivated')) {
  ...
  public function createAction() {
    ...
    // Make form
    $this->view->form = $form = new Sesgrouppackage_Form_Admin_Package_Create(array('customFields' => $customFields));
    ...
    $packageTable = Engine_Api::_()->getDbtable('packages', 'sesgrouppackage');
    ...
    try {
      ...
      $params['group_choose_style'] = $values['group_choose_style'];
      if ($values['group_choose_style'] && isset($values['group_chooselayout']))
        $params['group_chooselayout'] = $values['group_chooselayout'];
      else
        $params['group_style_type'] = $values['group_style_type'];
      if (!empty($values['sesgroup_group_text_photo']))
        $params['sesgroup_group_text_photo'] = $values['sesgroup_group_text_photo'];
      if (!empty($values['sesgroup_group_photo_photo']))
        $params['sesgroup_group_photo_photo'] = $values['sesgroup_group_photo_photo'];
      if (!empty($values['sesgroup_group_music_photo']))
        $params['sesgroup_group_music_photo'] = $values['sesgroup_group_music_photo'];
      if (!empty($values['sesgroup_group_video_photo']))
        $params['sesgroup_group_video_photo'] = $values['sesgroup_group_video_photo'];
      if (!empty($values['sesgroup_group_text_coverphoto']))
        $params['sesgroup_group_text_coverphoto'] = $values['sesgroup_group_text_coverphoto'];
      if (!empty($values['sesgroup_group_photo_coverphoto']))
        $params['sesgroup_group_photo_coverphoto'] = $values['sesgroup_group_photo_coverphoto'];
      if (!empty($values['sesgroup_group_music_coverphoto']))
        $params['sesgroup_group_music_coverphoto'] = $values['sesgroup_group_music_coverphoto'];
      if (!empty($values['sesgroup_group_video_coverphoto']))
        $params['sesgroup_group_video_coverphoto'] = $values['sesgroup_group_video_coverphoto'];
      ...
      $params['group_service'] = $values['group_service'];
      $params['auth_insightrpt'] = $values['auth_insightrpt'];
      $params['auth_addbutton'] = $values['auth_addbutton'];
      $params['auth_contactgp'] = $values['auth_contactgp'];
      $params['album'] = $values['album'];



      $params['group_approve'] = $values['group_approve'];
      $params['group_featured'] = $values['group_featured'];
      $params['group_sponsored'] = $values['group_sponsored'];
      $params['group_verified'] = $values['group_verified'];
      $params['group_hot'] = $values['group_hot'];
      $params['group_seo'] = $values['group_seo'];
      $params['group_overview'] = $values['group_overview'];
      $params['group_bgphoto'] = $values['group_bgphoto'];
      $params['group_contactinfo'] = $values['group_contactinfo'];
      $params['group_enable_contactparticipant'] = $values['group_enable_contactparticipant'];
      $params['custom_fields'] = $values['custom_fields'];

      if (!empty($values['sesgroup_admin_commission']))
        $params['sesgroup_admin_commission'] = $values['sesgroup_admin_commission'];
      if (!empty($values['sesgroup_commission_value']))
        $params['sesgroup_commission_value'] = $values['sesgroup_commission_value'];
      if (!empty($values['sesgroup_threshold_amount']))
        $params['sesgroup_threshold_amount'] = $values['sesgroup_threshold_amount'];
    ...
    // Redirect
    return $this->_helper->redirector->gotoRoute(array('module' => 'sesgrouppackage', 'controller' => 'package'), "admin_default", true);
  }
  ...
  public function editAction() {
    ...
    if (null === ($packageIdentity = $this->_getParam('package_id')) ||
            !($package = Engine_Api::_()->getDbtable('packages', 'sesgrouppackage')->find($packageIdentity)->current())) {
    ...
    $this->view->customFields = $customFields;
    // Make form
    $this->view->form = $form = new Sesgrouppackage_Form_Admin_Package_Edit(array('customFields' => $customFields));
    ...
    try {
      ...
      if (isset($values['upload_mainphoto']))
        $params['upload_mainphoto'] = $values['upload_mainphoto'];
      if (isset($values['group_choose_style']))
        $params['group_choose_style'] = $values['group_choose_style'];
      ...
      if (!empty($values['sesgroup_group_video_coverphoto']))
        $params['sesgroup_group_video_coverphoto'] = $values['sesgroup_group_video_coverphoto'];
      if (isset($values['group_approve']))
        $params['group_approve'] = $values['group_approve'];
      if (isset($values['group_featured']))
        $params['group_featured'] = $values['group_featured'];
      if (isset($values['group_sponsored']))
        $params['group_sponsored'] = $values['group_sponsored'];
      if (isset($values['group_verified']))
        $params['group_verified'] = $values['group_verified'];
      if (isset($values['group_hot']))
        $params['group_hot'] = $values['group_hot'];
      if (isset($values['group_seo']))
        $params['group_seo'] = $values['group_seo'];
      if (isset($values['group_overview']))
        $params['group_overview'] = $values['group_overview'];
      if (isset($values['group_bgphoto']))
        $params['group_bgphoto'] = $values['group_bgphoto'];
      if (isset($values['group_contactinfo']))
        $params['group_contactinfo'] = $values['group_contactinfo'];
      if (isset($values['group_enable_contactparticipant']))
        $params['group_enable_contactparticipant'] = $values['group_enable_contactparticipant'];
      $params['custom_fields'] = $values['custom_fields'];
      $values['member_level'] = ',' . implode(',', $values['member_level']) . ',';
      ...
      $params['auth_insightrpt'] = $values['auth_insightrpt'];
      $params['auth_addbutton'] = $values['auth_addbutton'];
      $params['auth_contactgroup'] = $values['auth_contactgroup'];
      $params['album'] = $values['album'];
  ...
  public function deleteAction() {
    ...
    $this->view->form = $form = new Sesgrouppackage_Form_Admin_Package_Delete();
    // Check method/data
    if (!$this->getRequest()->isPost()) {
      return;
    }
  ...
  public function manageTransactionAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesgroup_admin_packagesetting', array(), 'sesgrouppackage_admin_main_transaction');

    $this->view->formFilter = $formFilter = new Sesgrouppackage_Form_Admin_Filter();
  ...
  public function detailAction() {
    if (!($transaction_id = $this->_getParam('transaction_id')) ||
            !($transaction = Engine_Api::_()->getItem('sesgrouppackage_transaction', $transaction_id))) {
      return;
    }
    ...
    $this->view->item = Engine_Api::_()->getItem('sesgroup_group', $this->_getParam('group_id'));
