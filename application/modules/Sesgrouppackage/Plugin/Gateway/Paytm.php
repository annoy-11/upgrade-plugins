...
class Sesgrouppackage_Plugin_Gateway_Paytm extends Engine_Payment_Plugin_Abstract
{
  ...
  public function createGroupTransaction(User_Model_User $user,
...
//     // One-time
    if($package->isOneTime()) {
      ...
      $tablePage = Engine_Api::_()->getDbTable('groups', 'sesgroup');
      ...
      if (($paymentStatus == 'okay' || $paymentStatus == 'active' ||
        ...
        $item->onPaymentFailure();
        // Payment failed
        throw new Payment_Model_Exception($params['RESPMSG']);
      } else {
   ...
   public function onIpn(Engine_Payment_Ipn $ipn)
  {
    ...
    public function cancelSubscriptionOnExpiry($source, $package) {
      ...
      $paytmParams['body'] = array(
          "mid"			=> $this->_gatewayInfo->config['paytm_marchant_id'],
          "subsId"		=> $source->gateway_profile_id,
      );
      $checksum = getChecksumFromString(json_encode($paytmParams['body'], JSON_UNESCAPED_SLASHES), $this->_gatewayInfo->config['paytm_secret_key']);
      $paytmParams["head"] = array(
          "version" => "V1",
    ...
    public function onIpnTransaction($subscription){
      ...
      if ($order) {
        $ipnProcessed = false;
        // Subscription IPN
        if ($order->source_type == 'sesgrouppackage') {
          $this->onAdTransactionIpn($order, $response);
          $ipnProcessed = true;
        }
  ...
  public function onAdTransactionIpn(Payment_Model_Order $order,  $rawData) {
