...
class Sesgrouppackage_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function getGateway() {
    if (null === $this->_gateway) {
      ...
      $gateway = new $class(array(
          ...
          'currency' => Engine_Api::_()->sesgrouppackage()->getCurrentCurrency(),
      ));
  ...
  public function createGroupTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $item, Sesgrouppackage_Model_Package $package, array $params = array()) {
    // Process description
    ...
    $defaultCurrency = Engine_Api::_()->sesgrouppackage()->defaultCurrency();
  ...
  Payment_Model_Order $order, array $params = array()) {
    ...
    if ($package->isOneTime()) {
      ...
      $orderPackage = Engine_Api::_()->getItem('sesgrouppackage_orderspackage', $orderPackageId);
      ...
      $session = new Zend_Session_Namespace('Payment_Sesgrouppackage');
      ...
      if (!empty($oldOrderPackageId)) {
        $select = $tableGroup->select()->from($tableGroup->info('name'))->where('orderspackage_id =?', $oldOrderPackageId);
        $totalItemCreated = count($tableGroup->fetchAll($select));
        if ($package->item_count >= $totalItemCreated && $package->item_count)
          $leftGroup = $package->item_count - $totalItemCreated;
        else if (!$package->item_count)
          $leftGroup = -1;
        else
          $leftGroup = 0;
      } else
        $leftGroup = $package->item_count - 1;
      $tableGroup->update(array('orderspackage_id' => $orderPackageId), array('orderspackage_id' => $oldOrderPackageId));
      $packageOrder = Engine_Api::_()->getItem('sesgrouppackage_orderspackage', $orderPackageId);
      $packageOrder->item_count = $leftGroup;
      $packageOrder->save();

      $transactionsTable = Engine_Api::_()->getDbtable('transactions', 'sesgrouppackage');
      $transactionsTable->insert(array(
          'owner_id' => $order->user_id,
          'package_id' => $item->package_id,
          'item_count' => $leftGroup,
      ...
      // Get benefit setting
      $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sesgrouppackage')
          ...
          if ($package->isOneTime()) {
            switch ($rawData['payment_status']) {
              ...
              case 'Pending':
                ...
                // Get benefit setting
                $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sesgrouppackage')->getBenefitStatus($user);
                if ($giveBenefit) {
                  $item->onPaymentSuccess();
                } else {
                  $item->onPaymentPending();
  ...
  public function cancelGroup($transactionId, $note = null) {
    $profileId = null;
    if ($transactionId instanceof Sesgrouppackage_Model_Transaction) {
  ...
  public function onIpn(Engine_Payment_Ipn $ipn) {
    ...
    if ($order) {
      $ipnProcessed = false;
      // Subscription IPN
      if ($order->source_type == 'sesgroup_group') {
