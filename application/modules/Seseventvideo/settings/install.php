 ...
 * @version    $Id: install.php 2016-07-28 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seseventvideo_Installer extends Engine_Package_Installer_Module {
  ...
  public function onEnable() {
		$db = $this->getDb();
		$db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '1' WHERE `engine4_core_menuitems`.`name` = 'sesevent_admin_main_seseventvideo';");
    parent::onEnable();
  }
  
  public function onDisable() {
		$db = $this->getDb();
		$db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '0' WHERE `engine4_core_menuitems`.`name` = 'sesevent_admin_main_seseventvideo';");
    parent::onDisable();
  }
}
