...
var autoPlayId,firstVideoSrc=false,isOnRegister = false,IsfinishVideoNext = false,clearIntvalSeseventVideoSlideshow1,clearIntvalSeseventVideoSlideshow2 ;
jQuery(document).ready(function($){
		...
		sliderNav.on('click', 'li', function(event){
			...
			if(!selectedItem.hasClass('selected')) {
				...
				if(slidesWrapper.find('li.selected').hasClass('cd-bg-video')){
					 ...
					 clearInterval(clearIntvalSeseventVideoSlideshow1);
					 clearInterval(clearIntvalSeseventVideoSlideshow1);	
				}
			}
		});
	...
	function checkVideo(hiddenSlide, container, n) {
		...
		if( visibleVideo.length > 0 ) { 
			clearInterval(autoPlayId);
			clearInterval(clearIntvalSeseventVideoSlideshow2);
			clearInterval(clearIntvalSeseventVideoSlideshow1);
			if(!Modernizr.touch){
