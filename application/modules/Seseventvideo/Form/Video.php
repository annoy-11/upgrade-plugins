...
class Seseventvideo_Form_Video extends Engine_Form {
  ...
  public function init() {
    ...
    $setting = Engine_Api::_()->getApi('settings', 'core');
		...
		$allowedUploadOption = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('seseventvideo_video', $viewer, 'video_upload_option');
    foreach ($allowedUploadOption as $key => $valueUploadoption) {
      if ($valueUploadoption == 'youtube' && Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.youtube.apikey', false))
        $video_options[1] = "YouTube";
			if ($valueUploadoption == 'youtubePlaylist' && Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.youtube.apikey', false))
    ...
    //My Computer
    if ($myComputer) {
      $allowed_upload = Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('seseventvideo_video', $user, 'upload');
    ...
    $viewOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('seseventvideo_video', $user, 'auth_view');
    ...
    // Comment
    $commentOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('seseventvideo_video', $user, 'auth_comment');
    ...
    if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('seslock')) && Engine_Api::_()->authorization()->getPermission($viewer, 'seseventvideo_video', 'video_locked')) {
   ...
   $uploadoption = $settings->getSetting('seseventvideo.uploadphoto', '0');
    if ($uploadoption == 1) {
      if (isset($video) && $video->photo_id) {
        ...
        if (isset($path) && !empty($path)) {
          $this->addElement('Image', 'cover_photo_preview sesbd', array(
              'src' => $path,
              'class' => 'seseventvideo_channel_thumb_preview sesbd',
          ));
