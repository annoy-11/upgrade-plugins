 ...
 * @version    $Id: IndexController.php 2016-07-28 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
 
class Seseventvideo_IndexController extends Core_Controller_Action_Standard {
  ...
  public function searchAction() {
    ...
    $seseventvideo_commonsearch = $this->_getParam('seseventvideo_commonsearch', 'video');
    if ($seseventvideo_commonsearch && $actonType == 'browse') {
      $type = $seseventvideo_commonsearch;
    } else {
      if (isset($_COOKIE['seseventvideo_commonsearch']))
        $type = $_COOKIE['seseventvideo_commonsearch'];
      else
        $type = 'video';
    }
  ...
  public function editDetailAction() {
    ...
    $video = Engine_Api::_()->getItem('seseventvideo_video', $video_id);
    ...
    if ($status && !$error) {
      ...
      if ($_POST['lat'] != '' && $_POST['lng'] != '') {
        ...
        $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $_POST['video_id'] . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","seseventvideo_video")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }
  ...
  public function locationAction() {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->video = $video = Engine_Api::_()->getItem('seseventvideo_video', $video_id);
    if (!$video)
      return;
    $this->view->form = $form = new Seseventvideo_Form_Location();
    ...
    if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
      $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
      $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $video_id . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","seseventvideo_video")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
    }
    $db = $video->getTable()->getAdapter();
    $db->beginTransaction();
    try {
  ...
  public function imageviewerdetailAction() {
    ...
    $this->view->user = $user = Engine_Api::_()->getItem('user', $user_id);
		...
		if (!is_file($privateImageURL))
			$privateImageURL = 'application/modules/Seseventvideo/externals/images/private-video.jpg';
    ...
    if ($viewer->getIdentity() != 0 && isset($video->video_id)) {
      ...
      $dbObject->query('INSERT INTO engine4_seseventvideo_recentlyviewitems (resource_id, resource_type,owner_id,creation_date ) VALUES ("' . $video->video_id . '", "seseventvideo_video","' . $viewer->getIdentity() . '",NOW())	ON DUPLICATE KEY UPDATE	creation_date = NOW()');
    }
		...
		$this->view->customParamsArray = $customParamsArray;
		 // get next video URL
    $this->view->nextVideo = Engine_Api::_()->getDbTable('videos', 'seseventvideo')->videoLightBox($video, '>','','',$type,$this->_getParam('item_id',''));
    // get previous video URL
    $this->view->previousVideo = Engine_Api::_()->getDbTable('videos', 'seseventvideo')->videoLightBox($video, '<','','',$type,$this->_getParam('item_id',''));
    ...
    if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('seslock'))) {
      ...
      $this->view->locked = false;
    // rating code
    $this->view->allowShowRating = $allowShowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.ratevideo.show', 1);
    $this->view->allowRating = $allowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.video.rating', 1);
    ...
    if ($showRating != 0) {
      $this->view->canRate = $canRate = Engine_Api::_()->authorization()->isAllowed('seseventvideo_video', $viewer, 'rating');
      $this->view->allowRateAgain = $allowRateAgain = Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.ratevideo.again', 1);
      $this->view->allowRateOwn = $allowRateOwn = Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.ratevideo.own', 1);
      ...
      $this->view->rating_type = $rating_type = 'seseventvideo_video';
      $this->view->rating_count = $rating_count = Engine_Api::_()->getDbTable('ratings', 'seseventvideo')->ratingCount($video->getIdentity(), $rating_type);
      $this->view->rated = $rated = Engine_Api::_()->getDbTable('ratings', 'seseventvideo')->checkRated($video->getIdentity(), $viewer->getIdentity(), $rating_type);
  ...
  public function getVideoAction() {
    ...
    $value['search'] = 1;
    $videos = Engine_Api::_()->getDbtable('videos', 'seseventvideo')->getVideo($value);
  ...
  function likeAction() {
    ...
    $itemTable = Engine_Api::_()->getDbtable($dbTable, 'seseventvideo');
  ...
  function favouriteAction() {
    ...
    $dbTable = 'videos';
    $resorces_id = 'video_id';
    $notificationType = 'seseventvideo_video_favourite';
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $Fav = Engine_Api::_()->getDbTable('favourites', 'seseventvideo')->getItemfav($type, $item_id);
    $favItem = Engine_Api::_()->getDbtable($dbTable, 'seseventvideo');
    if (count($Fav) > 0) {
      //delete		
      ...
      //update
      $db = Engine_Api::_()->getDbTable('favourites', 'seseventvideo')->getAdapter();
      $db->beginTransaction();
      try {
        $fav = Engine_Api::_()->getDbTable('favourites', 'seseventvideo')->createRow();
      ...
      //send notification and activity feed work.
      $item = Engine_Api::_()->getItem(@$type, @$item_id);
      if ($this->_getParam('type') != 'seseventvideo_artist') {
        $subject = $item;
        $owner = $subject->getOwner();
        if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
          $activityTable = Engine_Api::_()->getDbtable('actions', 'activity');
  ...
  public function allVideosAction() {
    ...
    //FETCH photos
		...
		$this->view->customParamsArray = $customParamsArray;
    $paginator = $this->view->allVideos = Engine_Api::_()->getDbTable('videos', 'seseventvideo')->videoLightBox($video, '', true, true,$type,$this->_getParam('item_id',''));
...
//     }
  ...
  public function rateAction() {
    ...
    $total = Engine_Api::_()->getDbtable('ratings', 'seseventvideo')->ratingCount($item->getIdentity(), $resource_type);
    $rating_sum = Engine_Api::_()->getDbtable('ratings', 'seseventvideo')->getSumRating($item->getIdentity(), $resource_type);
    $data = array();
  ...
  public function shareAction() {
    ...
    $typeItem = ucwords(str_replace(array('seseventvideo_'), '', $attachment->getType()));
  ...
  public function uploadYoutubePlaylistVideos($values, $form, $leftVideos, $data, $limitYoutubePlaylist,$approve) {
    ...
    $values['parent_type'] = $parent_type = 'sesevent_event';
    $db = Engine_Api::_()->getDbtable('videos', 'seseventvideo')->getAdapter();
    $db->beginTransaction();
    try {
      //Create video
      $table = Engine_Api::_()->getDbtable('videos', 'seseventvideo');
      ...
      foreach ($data['items'] as $videoId) {
        ...
        if ($valid_thumb && $thumbnail && $ext && $thumbnail_parsed && in_array($ext, array('jpg', 'jpeg', 'gif', 'png'))) {
          ...
          $video->save();
            ...
            $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
            $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $video->video_id . '", "' . $values['lat'] . '","' . $values['lng'] . '","seseventvideo_video")	ON DUPLICATE KEY UPDATE	lat = "' . $values['lat'] . '" , lng = "' . $values['lng'] . '"');
          }
        ...
        $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($owner, $video, 'seseventvideo_video_create');
        if ($action != null) {
  ...
  public function createAction() {
    ...
    $this->view->quota = $quota = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'seseventvideo_video', 'max');
    ...
    //Create form
    $this->view->form = $form = new Seseventvideo_Form_Video();
    ...
    try {
      ...
      $table = Engine_Api::_()->getDbtable('videos', 'seseventvideo');
      if ($values['type'] == 3) {
        $video = Engine_Api::_()->getItem('seseventvideo_video', $this->_getParam('id'));
      } else
        $video = $table->createRow();
      ...
      if ($values['type'] == 5) {
        $limitYoutubePlaylist = Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.youtube.playlist', '25');
        $googleApiKey = Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.youtube.apikey', 0);
        if (!$googleApiKey)
          return;
          ...
          try {
            ...
            @unlink($tmp_file);
             ...
             @unlink($tmp_file);
			  ...
			  if($values['type'] == 16){
				 $videoUrl = Engine_Api::_()->seseventvideo()->createVideo(array(), $values['code'], $values,$video);
				 $video->status = 1;
				 $video->save();
			  }
				if($values['type'] != 3){
				...
				if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
            $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
            $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $video->video_id . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","seseventvideo_video")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
          }
    ...
    try {
      ...
      throw $e;
		...
		$widgetId = Engine_Api::_()->sesevent()->getIdentityWidget('seseventvideo.profile-videos','widget','sesevent_profile_index');
		if($widgetId)
  ...
  public function uploadVideoAction() {
    ...
    try {
      ...
      $video = Engine_Api::_()->seseventvideo()->createVideo($params, $_FILES['Filedata'], $values);
      $this->view->status = true;
  ...
  public function deleteAction() {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $video = Engine_Api::_()->getItem('seseventvideo_video', $this->getRequest()->getParam('video_id'));
  ...
  public function editAction() {
    ...
    $this->view->form = $form = new Seseventvideo_Form_Edit();
    ...
    try {
      ...
      if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
        $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
        $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $this->_getParam('video_id') . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","seseventvideo_video")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }
      $video->setFromArray($values);
      $video->save();
      // Add fields
    ...
    return $this->_helper->redirector->gotoRoute(array('user_id' => $viewer->getIdentity(), 'video_id' => $video->getIdentity(), 'slug' => $video->getSlug()), 'seseventvideo_view', true);
  }
  public function lastElementDataAction() {
		$this->view->type = $this->_getParam('type','seseventvideo_video');
  ...
  public function composeUploadAction() {
    ...
    $values['user_id'] = $viewer->getIdentity();
    $paginator = Engine_Api::_()->getApi('core', 'seseventvideo')->getVideosPaginator($values);
    $quota = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'seseventvideo_video', 'max');
  ...
  public function checkYouTubePlaylist($code) {
    ...
    require_once 'Google/Service/YouTube.php';
    $client = new Google_Client();
    $client->setDeveloperKey(Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.youtube.apikey', 0));
    $youtube = new Google_Service_YouTube($client);
    $nextPageToken = '';
    $playlistItemsResponse = $youtube->playlistItems->listPlaylistItems('snippet', array(
        'playlistId' => $code,
  ...
  public function favouriteItemAction() {
    ...
    if ($item_type == 'video'){
			$param['type'] = 'seseventvideo_video';
      $paginator = Engine_Api::_()->getDbTable('videos', 'seseventvideo')->getFavourite($param);
		}
    else
      $paginator = Engine_Api::_()->getDbTable('chanels', 'seseventvideo')->getFavourite($param);
  ...
  public function likeItemAction() {
    ...
    $paginator = Engine_Api::_()->seseventvideo()->likeItemCore($param);
    $this->view->item_id = $item_id;
    $this->view->paginator = $paginator;
    // Set item count per page and current page number
    $paginator->setItemCountPerPage(10);
