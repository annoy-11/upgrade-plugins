 ...
 * @version    $Id: AdminManageController.php 2016-07-28 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seseventvideo_AdminManageController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('seseventvideo_admin_main', array(), 'seseventvideo_admin_main_manage');

    $this->view->formFilter = $formFilter = new Seseventvideo_Form_Admin_Manage_Filter();
    ...
    if ($this->getRequest()->isPost()) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $video = Engine_Api::_()->getItem('seseventvideo_video', $value)->delete();
        }
      }
    }
    ...
    $select = $table->select()
            ...
            ->joinLeft($tableUserName, "$tableUserName.user_id = $tableName.owner_id", 'username')
						->joinLeft($tableEventName, "$tableEventName.event_id = $tableName.parent_id", 'event_id');
  ...
  public function hotAction() {
    ...
    $db->beginTransaction();
    try {
      Engine_Api::_()->getDbtable($table, 'seseventvideo')->update(array(
          'is_hot' => $status,
              ), array(
          $type_id . " = ?" => $id,
      ));
			...
			if($approve){
				...
				try {
					...
					Engine_Api::_()->getDbtable('notifications', 'activity')
									->addNotification($owner, $owner, $video, 'seseventvideo_approved');
					$db->commit();
				} catch (Exception $e) {
					$db->rollBack();
					throw $e; // throw
				...
				Engine_Api::_()->getApi('mail', 'core')->sendSystem($owner->email, 'NOTIFY_SESEVENTVIDEO_DISAPPROVED', array(
  ...
  public function featureSponsoredAction() {
    ...
    $db->beginTransaction();
    try {
      Engine_Api::_()->getDbtable($params, 'seseventvideo')->update(array(
          'is_' . $category => $status,
              ), array(
          "$col = ?" => $id,
      ));
  ...
  public function ofthedayAction() {
    ...
    $this->view->form = $form = new Seseventvideo_Form_Admin_Settings_Oftheday();
    $item = Engine_Api::_()->getItem('seseventvideo_video', $id);
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db->update('engine4_seseventvideo_videos', array('starttime' => $values['starttime'], 'endtime' => $values['endtime']), array("video_id = ?" => $id));
      if (isset($values['remove']) && $values['remove']) {
        $db->update('engine4_seseventvideo_videos', array('offtheday' => 0), array("video_id = ?" => $id));
      } else {
        $db->update('engine4_seseventvideo_videos', array('offtheday' => 1), array("video_id = ?" => $id));
      }
      $this->_forward('success', 'utility', 'core', array(
          'smoothboxClose' => 10,
          'parentRefresh' => 10,
