...
class Seseventvideo_Plugin_Core {
  public function onStatistics($event) {
    $table = Engine_Api::_()->getDbTable('videos', 'seseventvideo');
    $select = new Zend_Db_Select($table->getAdapter());
    $select->from($table->info('name'), 'COUNT(*) AS count');
    $event->addResponse($select->query()->fetchColumn(0), 'seseventvideo_video');
  }
...
EOF;
    $script .=
						"var videoURLseseventvideo = '" . Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.video.manifest', 'eventvideo') . "';
