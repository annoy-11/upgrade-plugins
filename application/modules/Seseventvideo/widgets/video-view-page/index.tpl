...
<script type="text/javascript">
	 var tagAction = window.tagAction = function(tag,name){
			var url = "<?php echo $this->url(array('module' => 'seseventvideo','action'=>'browse'), 'seseventvideo_general', true) ?>?tag_id="+tag+'&tag_name='+name;
  ...
  en4.core.runonce.add(function() {
    ...
    var rate = window.rate = function(rating) {
      ...
      (new Request.JSON({
        'format': 'json',
        'url' : '<?php echo $this->url(array('module' => 'seseventvideo', 'controller' => 'index', 'action' => 'rate'), 'default', true) ?>',
        'data' : {
          'format' : 'json',
          'rating' : rating,
...
<?php } ?>
<div class="seseventvideo_video_view_container clear sesbasic_clearfix sesbasic_bxs">
  <?php if( $this->video->type == 3 ): ?>
    <div id="video_embed" class="seseventvideo_view_embed clear sesbasic_clearfix">
  ...
  <?php if(in_array('openVideoLightbox',$this->allowOptions)){ ?>
  	<a href="javascript:;" id="openEventVideoInLightbox" class="fa fa-expand seseventvideo_view_openlightbox_link"><?php echo $this->translate("Open in Lightbox")?></a>
  <?php } ?>
  <h2 class="seseventvideo_view_title sesbasic_clearfix clear">
  ...
  <div class="seseventvideo_view_statics">
    <?php if(($this->getAllowRating == 0 && $this->allowShowRating == 1 && $this->total_rating_average != 0 ) || ($this->getAllowRating == 1) && in_array('rateCount',$this->allowOptions) ){ ?>
      <div id="album_rating" class="sesbasic_rating_star seseventvideo_view_rating" onMouseOut="rating_out();">
  ...
  <div class="seseventvideo_view_meta sesbasic_text_light clear sesbasic_clearfix">
    ...
    <?php endif; ?>
   	<?php if(!is_null($this->video->location) && $this->video->location != '' && Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo_enable_location', 1)){ ?>
      <span>
         <i class="fa fa-map-marker"></i> 
         <a href="javascript:;" onClick="openURLinSmoothBox('<?php echo $this->url(array("module"=> "seseventvideo", "controller" => "index", "action" => "location",  "video_id" => $this->video->getIdentity(),'type'=>'video_location'),'default',true); ?>');return false;"><?php echo $this->video->location; ?></a>
      </span>
    <?php } ?>
  </div>
  <div class="seseventvideo_view_desc clear">
    ...
    <?php if(Engine_Api::_()->user()->getViewer()->getIdentity()){ ?>
 		 <div class='seseventvideo_view_options'>
    <!--Like Button-->
	    <?php $LikeStatus = Engine_Api::_()->seseventvideo()->getLikeStatusVideo($this->video->getidentity(),$this->video->getType()); ?>
    	<a href="javascript:;" title="<?php echo $this->translate('Like'); ?>" data-url="<?php echo $this->video->getIdentity() ; ?>" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn seseventvideo_like_seseventvideo_video <?php echo ($LikeStatus) ? 'button_active' : '' ; ?>"> <i class="fa fa-thumbs-up"></i><span><?php echo $this->video->like_count; ?></span></a>
    
			<?php if(in_array('favouriteButton',$this->allowOptions) && Engine_Api::_()->user()->getViewer()->getIdentity()){ ?>
				<?php $favStatus = Engine_Api::_()->getDbtable('favourites', 'seseventvideo')->isFavourite(array('resource_type'=>'seseventvideo_video','resource_id'=>$this->video->getIdentity())); ?>
				<a href="javascript:;" title="<?php echo $this->translate('Favourite'); ?>" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_fav_btn seseventvideo_favourite_seseventvideo_video ?> <?php echo ($favStatus)  ? 'button_active' : '' ?>"  data-url="<?php echo $this->video->getIdentity(); ; ?>"><i class="fa fa-heart"></i><span><?php echo $this->video->favourite_count; ?></span></a>
			<?php } ?>
	    <?php if(in_array('shareAdvance',$this->allowOptions)){ ?>
      	...
      	<i class="fa fa-share"></i></a>
			<?php } ?>
	    <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.enable.watchlater', 1) && Engine_Api::_()->user()->getViewer()->getIdentity() && in_array('watchLater',$this->allowOptions)){
		    $item = Engine_Api::_()->getDbTable('videos', 'seseventvideo')->getWatchLaterStatus($this->video->video_id);
	    ?>
      <a href="javascript:;" class="sesbasic_icon_btn seseventvideo_watch_later <?php echo count($item)  ? 'selectedWatchlater' : '' ?>" title = "<?php echo count($item)  ? $this->translate('Remove from Watch Later') : $this->translate('Add to Watch Later') ?>" data-url="<?php echo $this->video->video_id ; ?>">
      	<i class="fa fa-clock-o"></i>
      </a>
    <?php } ?>
    <?php if( Engine_Api::_()->user()->getViewer()->getIdentity() ): ?>
			<?php if(in_array('shareSimple',$this->allowOptions)){ ?>
      	<a href="<?php echo $this->url(array('module'=> 'seseventvideo', 'controller' => 'index','action' => 'share','route' => 'default','type' => 'seseventvideo_video','id' => $this->video->getIdentity(),'format' => 'smoothbox'),'default',true); ?>" class="sesbasic_icon_btn initialism btn btn-success smoothbox"  title="<?php echo  $this->translate('Share'); ?>"><i class="fa fa-share"></i></a>
      <?php  } ?>
      <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.enable.report',1) && in_array('reportVideo',$this->allowOptions)){ ?>
    ...
    <?php if( $this->can_embed && in_array('embedVideo',$this->allowOptions)): ?>
    	<a href="<?php echo $this->url(array('module'=> 'seseventvideo','controller' => 'video','action' => 'embed','id' => $this->video->getIdentity(),'format' => 'smoothbox'),'default',true); ?>" class="sesbasic_icon_btn sesbasic_icon_embed_btn smoothbox"  title="<?php echo  $this->translate('Embed'); ?>"><i class="fa fa-code"></i></a>
    <?php endif;?>
     
    <?php if( $this->can_edit && in_array('editVideo',$this->allowOptions)): ?>
    	<a href="<?php echo $this->url(array('controller' => 'index','action' => 'edit','video_id' => $this->video->video_id),'seseventvideo_general',true) ?>" class="sesbasic_icon_btn sesbasic_icon_edit_btn"  title="<?php echo  $this->translate('Edit'); ?>" data-url="<?php echo $this->video->video_id ; ?>"><i class="fa fa-pencil"></i></a>
    <?php endif;?>
    
    <?php if( $this->can_delete && $this->video->status != 2 && in_array('deleteVideo',$this->allowOptions)): ?>
    	<a href="<?php echo $this->url(array('controller' => 'index', 'action' => 'delete', 'video_id' => $this->video->video_id),'seseventvideo_general',true); ?>" class="sesbasic_icon_btn sesbasic_icon_delete_btn smoothbox"  title="<?php echo  $this->translate('Delete'); ?>"><i class="fa fa-trash"></i></a>
    <?php endif;?>
  </div>  
 		<?php } ?>
</div>
<div id="seseventvideo_image_video_url" data-src="<?php echo $this->video->getPhotoUrl(); ?>" style="display:none"></div>
<!-- Slide in -->
    ...
    <div class="sesbasic_popup_content">
      <div class="sesbasic_share_popup_content_row clear sesbasic_clearfix">
          ...
          <?php if(in_array('privateMessage',$this->allowAdvShareOptions)){ ?>
            <a href="javascript:void(0)" class="sesbasic_button" onClick="opensmoothboxurl('<?php echo $this->url(array('module'=> 'sesbasic', 'controller' => 'index', 'action' => 'message','item_id' => $this->video->getIdentity(), 'type'=>'seseventvideo_video'),'default',true); ?>')"> <?php echo $this->translate("Private Message"); ?></a>
            <?php } ?>
             <?php if(in_array('siteShare',$this->allowAdvShareOptions)){ ?>
            <a href="javascript:void(0)" class="sesbasic_button" onClick="opensmoothboxurl('<?php echo $this->url(array('module'=> 'seseventvideo', 'controller' =>'index','action' => 'share','type' => 'seseventvideo_video','id' => $this->video->getIdentity(),'format' => 'smoothbox'),'default',true); ?>')"> <?php echo $this->translate("Share on Site"); ?></a>
            <?php } ?>
             <?php if(in_array('quickShare',$this->allowAdvShareOptions)){ ?>
            <a href="javascript:void(0)" class="sesbasic_button" onClick="sessendQuickShare('<?php echo $this->url(array('module'=> 'seseventvideo', 'controller' =>'index','action' => 'share','type' => 'seseventvideo_video','id' => $this->video->getIdentity()),'default',true); ?>');return false;"> <?php echo $this->translate("Quick Share on Site"); ?></a>
          <?php } ?>
      	</div>
      </div>
      <?php if($addThisCode && in_array('addThis',$this->allowAdvShareOptions)){ ?>
...
<div class="seseventvideo_view_bottom sesbasic_clearfix">
	<div class="seseventvideo_view_bottom_right sesbasic_bxs">
    <?php if(in_array('peopleLike',$this->allowOptions)){  ?>
      <div class="layout_seseventvideo_people_like_video">
        <?php echo $this->content()->renderWidget('seseventvideo.people-like-item',array('limit_data'=>$this->limitLike,'removeDecorator'=>'yes')); ?>
      </div>
    <?php } ?>
    <?php if(in_array('favourite',$this->allowOptions)){  ?>
      <div class="layout_seseventvideo_people_favourite_video">
        <?php echo $this->content()->renderWidget('seseventvideo.people-favourite-item',array('limit_data'=>$this->limitFavourite,'removeDecorator'=>'yes')); ?>
      </div>
    <?php } ?>
  </div>
  <div class="seseventvideo_view_bottom_left layout_core_comments">
