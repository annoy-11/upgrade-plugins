 ...
 */
class Seseventvideo_Widget_ProfileVideosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($_POST['is_ajax'])) {
      ...
      if (!$subject->authorization()->isAllowed($viewer, 'view'))
        return $this->setNoRender();
			...
			$subject = Engine_Api::_()->getItem('sesevent_event', $params['parent_id']);	
		}
	  //Privacy Check
    $this->view->allowVideo  = Engine_Api::_()->authorization()->isAllowed('sesevent_event', $viewer, 'event_video');
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
		$this->view->canUpload = $canUpload = $subject->authorization()->isAllowed(null, 'video');
    ...
    $this->view->loadMoreLink = $this->_getParam('openTab') != NULL ? true : false;
		...
		$options = array('loadJs' => true, 'paggindData' => true);
    ...
    $paginator = Engine_Api::_()->getDbTable('videos', 'seseventvideo')->getVideo(array('parent_id' => $parent_id, 'parent_type' => $parent_type));
