...
class Seseventvideo_Widget_VideoLocationController extends Engine_Content_Widget_Abstract {

  public function indexAction() {
      
		if(! Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo_enable_location', 1))
			return setNoRender();
    ...
    $value['text'] = $text = isset($searchArray['search']) ? $searchArray['search'] : (!empty($params['search']) ? $params['search'] : (isset($_GET['search']) && ($_GET['search'] != '') ? $_GET['search'] : ''));
		 $value['search_event'] = $text = isset($searchArray['search_event']) ? $searchArray['search_event'] : (!empty($params['search_event']) ? $params['search_event'] : (isset($_GET['search_event']) && ($_GET['search_event'] != '') ? $_GET['search_event'] : ''));
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('videos', 'seseventvideo')->getVideo($value, true);
  ...
  function makeAjaxCallData($paginator) {
    ...
    foreach ($paginator as $item) {
			if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0){
				$likeButton = '<a href="javascript:;" data-url="'.$item->getIdentity().'" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn seseventvideo_like_seseventvideo_video "> <i class="fa fa-thumbs-up"></i><span>'.$item->like_count.'</span></a>';
				$favouriteButton = '<a href="javascript:;" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_fav_btn seseventvideo_favourite_seseventvideo_video " data-url="'.$item->getIdentity().'"><i class="fa fa-heart"></i><span>'.$item->favourite_count.'</span></a>';
				$addToplaylist = '';
			}
