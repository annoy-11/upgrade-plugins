 ...
 * @version    $Id: all-videos.tpl 2016-07-28 00:00:00 SocialEngineSolutions $
...
<?php } ?>
       <?php foreach($this->allVideos as $valueVideo){ 
       		  if (!$valueVideo instanceof Seseventvideo_Model_Video)
            	$valueVideo = Engine_Api::_()->getItem('seseventvideo_video',$valueVideo->video_id);
       ?>
          <?php $imageURL = $valueVideo->getHref($this->customParamsArray); ?>
          <a id="video-lightbox-id-<?php echo $valueVideo->video_id; ?>" href="<?php echo $valueVideo->getHref($this->customParamsArray); ?>" class="ses-video-viewer ses_ml_photos_panel_photo_thumb seseventvideo_thumb_img">
