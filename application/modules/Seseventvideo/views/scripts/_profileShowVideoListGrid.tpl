 ...
 * @version    $Id: _profileShowVideoListGrid.tpl 2016-07-28 00:00:00 SocialEngineSolutions $
        ...
        <?php }else if(isset($this->view_type) && $this->view_type == 'pinboard'){ ?>
            ...
            <?php } ?>
                 ...
                 <?php if(isset($this->enableCommentPinboardActive)){ ?>
                    ...
                    <?php if(($itemType != '')){ ?>
                    	<?php echo Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesadvancedcomment') ? $this->action('list', 'comment', 'sesadvancedcomment', array('type' => $event->getType(), 'id' => $event->getIdentity(),'page'=>'')) : $this->action("list", "comment", "sesbasic", array("item_type" => 'seseventvideo_video', "item_id" =>  $item->video_id,"widget_identity"=>$randonNumber)); ?>
                      <?php } ?>
                    </div>
                 <?php } ?>
                  </div>
