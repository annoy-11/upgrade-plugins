 ...
 * @version    $Id: Videos.php 2016-07-28 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seseventvideo_Model_DbTable_Videos extends Engine_Db_Table {

  protected $_rowClass = "Seseventvideo_Model_Video";
  protected $_name = 'seseventvideo_videos';
  ...
  public function getWatchLaterStatus($video_id) {
    ...
    $watchLaterTable = Engine_Api::_()->getDbTable('watchlaters', 'seseventvideo')->info('name');
  ...
  public function getVideo($params = array(), $paginator = true) {
    ...
    if (Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventvideo.enable.watchlater', 1)) {
      $watchLaterTable = Engine_Api::_()->getDbTable('watchlaters', 'seseventvideo')->info('name');
      $select = $select->setIntegrityCheck(false);
      $select = $select->joinLeft($watchLaterTable, '(' . $watchLaterTable . '.video_id = ' . $tableName . '.video_id AND ' . $watchLaterTable . '.owner_id = ' . $user_id . ')', array('watchlater_id'));
    }
		$eventTable = Engine_Api::_()->getDbTable('events', 'sesevent')->info('name');
    $select = $select->joinLeft($eventTable, '(' . $eventTable . '.event_id = ' . $tableName . '.parent_id )', array('event_id'));
		$select->where($eventTable.'.event_id !=?','');
		if(!empty($params['search_event']))
			$select->where($eventTable . ".title LIKE ?", $params['search_event'] . '%');
    ...
    if (!empty($params['sameTag'])) {
      $select->joinLeft($tmName, "$tmName.resource_id=$tableName.video_id", null)
              ->where('resource_type = ?', 'seseventvideo_video')
              ->distinct(true)
  ...
  public function peopleAlsoLiked($id = 0) {
    ...
    $select = $this->select()
            ...
            ->where($likesTableName . '.resource_type = ?', 'seseventvideo_video')
            ->where('l2.resource_type = ?', 'seseventvideo_video')
