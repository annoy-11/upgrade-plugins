 ...
 */
class Sesteam_Widget_TeamPageController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->teampage_description = $this->_getParam('sesteam_teampage_description', 'Meet Team');
    ...
    $params['limit'] = $itemCount = $params['limit'] = isset($params['limit']) ? $params['limit'] : $this->_getParam('limit', '10');

    $this->view->paginationType = $params['paginationType'] = $paginationType = $params['paginationType'] = isset($params['paginationType']) ? $params['paginationType'] : $this->_getParam('paginationType', '1');
    $this->view->type = $params['type'] = isset($_GET['sesteam_type']) ? $_GET['sesteam_type'] : $this->_getParam('sesteam_type', 'teammember');
    $params['popularity'] = isset($_GET['popularity']) ? $_GET['popularity'] : $this->_getParam('popularity', '');
    $params['title'] = isset($_GET['sesteam_title']) ? $_GET['sesteam_title'] : '';
    $params['designation_id'] = isset($_GET['designation_id']) ? $_GET['designation_id'] : $this->_getParam('designation_id', 0);

    $params['widgetname'] = 'team-page';
