...
class Sessubscribeuser_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function createOrderTransaction($viewer,$order,$user,array $params = array(), $package) {  
	...
	public function orderSubcriberTransactionReturn($order,$params = array()) {
    ...
    // Get related info
    $user = $order->getUser();
    $orderSubscriber = $order->getSource(); //print_r($orderSubscriber->toarray());die;
    ...
    $packageId = $packageTable->select()
              ...
              ->where('user_id =?', $orderSubscriber->subject_id)
              ->query()
              ->fetchColumn();
    $package = Engine_Api::_()->getItem('sessubscribeuser_package', $packageId);
...
//     }
    ...
    $this->getGateway()->getLog()->log('ExpressCheckoutDetail: ' . print_r($data, true), Zend_Log::INFO);
        ...
        $transactionsTable->insert(array(
          ...
          'subject_id' => $orderSubscriber->subject_id,
        ));
        
        // Get benefit setting
        $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sessubscribeuser')->getBenefitStatus($user); 
        ...
        if( $paymentStatus == 'okay' ||
          ...
          $tableRemaining = Engine_Api::_()->getDbtable('remainingpayments', 'sessubscribeuser');
          $tableName = $tableRemaining->info('name');
          $select = $tableRemaining->select()->from($tableName)->where('user_id =?',$orderSubscriber->subject_id);
          $select = $tableRemaining->fetchAll($select);
          if(count($select)){
            $tableRemaining->update(array('remaining_payment' => new Zend_Db_Expr("remaining_payment + $orderAmount")),array('user_id =?'=>$orderSubscriber->subject_id));
          } else {
            $tableRemaining->insert(array(
              'remaining_payment' => $orderAmount,
              'user_id' => $orderSubscriber->subject_id,
            ));
          }
          ...
          if( $orderSubscriber->state == 'complete' ) {
            ...
            $subject = Engine_Api::_()->getItem('user', $orderSubscriber->subject_id);
            $user = Engine_Api::_()->getItem('user', $orderSubscriber->user_id);
            Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($subject, $user, $subject, 'sessubscribeuser_subscribe_owner');
          }
    ...
    // Recurring
      ...
      $transaction = Engine_Api::_()->getItem('sessubscribeuser_transaction',$transaction_id);
...
//           ));
  ...
  {
    return new Sessubscribeuser_Form_Admin_Gateway_PayPal();
  }
