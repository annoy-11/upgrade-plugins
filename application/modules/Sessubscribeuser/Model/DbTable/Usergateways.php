...
class Sessubscribeuser_Model_DbTable_Usergateways extends Engine_Db_Table
{
  protected $_rowClass = 'Sessubscribeuser_Model_Usergateway';
  ...
  public function getEnabledGatewayCount()
  {
    ...
    $select = $this->select()->from($this->info('name'))->where('subject_id =?',$params['user_id']);
    if(!isset($params['enabled']))
      $select->where('enabled =?','1');
    if(isset($params['user_id']))
      $select->where('subject_id =?',$params['user_id']);
