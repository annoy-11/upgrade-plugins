sesJqueryObject(document).on('click','.seswishe_likefavfollow',function(){
	seswishe_likefavourite_data(this,'seswishe_likefavfollow');
});

sesJqueryObject(document).on('submit', '#seswishes_create', function(e) {
  e.preventDefault();
  addWishe(this);
});
...
var objectError ;
var counter = 0;
function validateWisheForm() {
  
  var errorPresent = false; 
  sesJqueryObject('.seswishe_create_form input, .seswishe_create_form select, .seswishe_create_form checkbox, .seswishe_create_form textarea, .seswishe_create_form radio').each(
    function(index){
...
function seswishe_likefavourite_data(element) {
    ...
    if(clickType == 'like_entry_view') {
      ...
      functionName = 'like';
      itemType = 'seswishe_participant';
      var contentId = sesJqueryObject(element).attr('data-url');
      var elementId = '.seswishe_entry_like_'+contentId;
      ...
      functionName = 'like';
      itemType = 'seswishe_wishe';
      var contentId = sesJqueryObject(element).attr('data-url');
      var elementId = '.seswishe_like_'+contentId;
      if(sesJqueryObject(elementId).hasClass('button_active')) {
        sesJqueryObject(elementId).find('span').html(parseInt(sesJqueryObject(elementId).find('span').html())-1);
      }
      else {
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'seswishe/index/' + functionName,
      'data': {
            format: 'html',
            id: contentId,
            type: itemType,
        ...
        if (response.error)
          alert(en4.core.language.translate('Something went wrong,please try again later'));
              ...
              if(canIntegrate == 1 && response.vote_status) {
                sesJqueryObject('#seswishe_vote_button_'+contentId).html('<i class="fa fa-hand-o-up"></i><span>Voted</span>');
                sesJqueryObject('#seswishe_vote_button_'+contentId).addClass('disable');
              }
        return true;
      }
    })).send();
