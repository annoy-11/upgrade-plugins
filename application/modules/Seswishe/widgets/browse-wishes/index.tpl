...
<script type="text/javascript">
  ...
  function loadMoreWISHE() {
  
    if ($('wishe_view_more'))
      $('wishe_view_more').style.display = "<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() || $this->count == 0 ? 'none' : '' ) ?>";

    if(document.getElementById('wishe_view_more'))
      document.getElementById('wishe_view_more').style.display = 'none';
    
    if(document.getElementById('wishe_loading_image'))
     document.getElementById('wishe_loading_image').style.display = 'block';

    en4.core.request.send(new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/seswishe/name/browse-wishes',
      'data': {
        format: 'html',
        page: "<?php echo sprintf('%d', $this->paginator->getCurrentPageNumber() + 1) ?>",
        viewmore: 1,
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        ...
        if(document.getElementById('wishe_view_more'))
          document.getElementById('wishe_view_more').destroy();
        if(document.getElementById('wishe_view_more'))
          document.getElementById('wishe_view_more').style.display = 'block';
        if(document.getElementById('wishe_loading_image'))
          document.getElementById('wishe_loading_image').destroy();
        if(document.getElementById('wishe_loading_image'))
         document.getElementById('wishe_loading_image').style.display = 'none';
        if(document.getElementById('wishe_loadmore_list'))
          document.getElementById('wishe_loadmore_list').destroy();
      }
    }));
    return false;
  }
...
<?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
    <?php if (empty($this->viewmore)): ?>
      <div class="sesbasic_bxs sesbasic_clearfix">
        <ul class="prelative seswishes_listing sesbasic_pinboard_<?php echo $randonNumber ; ?>" style="min-height:50px;" id="tabbed-widget_<?php echo $randonNumber; ?>" >
    <?php endif; ?>
      <?php foreach( $this->paginator as $item ): ?>
        <li class="seswishes_list_item new_image_pinboard_<?php echo $randonNumber; ?>" >
        	<section>
            ...
            <div class='_content'>
              <div id="seswishe_description_content_<?php echo $item->getIdentity(); ?>" class='seswishe_feed_wishe _des'>
                ...
                <?php } ?>
                <?php if(in_array('title', $this->allParams['stats']) && !empty($item->wishetitle)) { ?>
              ...
              <?php if(in_array('likecount', $this->allParams['stats']) || in_array('commentcount', $this->allParams['stats']) || in_array('viewcount', $this->allParams['stats']) || in_array('category', $this->allParams['stats']) || in_array('permalink', $this->allParams['stats'])): ?>
                <div class="_stats sesbasic_text_light">
                  ...
                  <?php } ?>
                  <?php if(in_array('category', $this->allParams['stats']) && Engine_Api::_()->getApi('settings', 'core')->getSetting('seswishe.enablecategory', 1) && $item->category_id) { ?>
                    <span> 
            ...
            <div class="_footer sesbasic_clearfix seswishe_social_btns">
              <?php if(in_array('socialSharing', $this->allParams['stats']) && Engine_Api::_()->getApi('settings', 'core')->getSetting('seswishe.allowshare', 1)):?>
  ...
  <?php if ($allParams['pagging'] == 'button' && !empty($this->paginator) && $this->paginator->count() > 1): ?>
    <?php if ($this->paginator->getCurrentPageNumber() < $this->paginator->count()): ?>
      <div class="clr" id="wishe_loadmore_list"></div>
      <div class="sesbasic_load_btn" id="wishe_view_more" onclick="loadMoreWISHE();" style="display: block;">
      ...
      <div class="sesbasic_load_btn" id="wishe_loading_image" style="display: none;">
...
<?php elseif( $this->category || $this->show == 2 || $this->search ): ?>
  <div class="sesbasic_tip">
    <img src="<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('seswishe_wishe_no_photo', 'application/modules/Seswishe/externals/images/wishe-icon.png'); ?>" alt="" />
    <span>
      <?php echo $this->translate('Nobody has written a wishe entry with that criteria.');?>
      <?php if (TRUE): // @todo check if user is allowed to create a poll ?>
        <?php echo $this->translate('Be the first to %1$swrite%2$s one!', '<a class="smoothbox" href="'.$this->url(array('action' => 'create'), 'seswishe_general').'">', '</a>'); ?>
      <?php endif; ?>
    </span>
  </div>
<?php else:?>
  <div class="sesbasic_tip">
    <img src="<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('seswishe_wishe_no_photo', 'application/modules/Seswishe/externals/images/wishe-icon.png'); ?>" alt="" />
    <span>
      <?php echo $this->translate('Nobody has written a wishe entry yet.'); ?>
...
//   sesJqueryObject(document).on('click', '.seswishe_expand', function() {
// 
//     sesJqueryObject(this).parent().find('._des').css('max-height','none');
//     //sesJqueryObject(this).hide();
//     sesJqueryObject(this).closest('.seswishes_list_item').addClass('seswishe_content_open');
