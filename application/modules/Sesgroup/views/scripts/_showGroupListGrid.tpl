 ...
 * @version    $Id: _showGroupListGrid.tpl  2018-04-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

?>
<?php $randonNumber = $this->widgetId; ?>
<?php $widgetType = 'profile-group';?>
<?php $viewer = Engine_Api::_()->user()->getViewer();?>
<?php $viewerId = $viewer->getIdentity();?>
<?php $counter = 0;?>
<?php if(isset($this->group_id)):?>
  <?php $groupId = $this->group_id;?>
<?php else:?>
  <?php $groupId = 0;?>
<?php endif;?>
<?php if(!$this->is_ajax){ ?>
...
<?php } ?>
    <?php if(isset($this->params['show_item_count']) && $this->params['show_item_count']){ ?>
      <div class="sesbasic_clearfix sesbm sesgroup_search_result" style="display:<?php !$this->is_ajax ? 'block' : 'none'; ?>" id="<?php echo !$this->is_ajax ? 'paginator_count_sesgroup' : 'paginator_count_ajax_sesgroup_entry' ?>"><span id="total_item_count_sesgroup_entry" style="display:inline-block;"><?php echo $this->paginator->getTotalItemCount(); ?></span> <?php echo $this->paginator->getTotalItemCount() == 1 ?  $this->translate("group found.") : $this->translate("groups found."); ?></div>
    <?php } ?>
<?php if(!$this->is_ajax){ ?>
    <div class="sesbasic_view_type_options sesbasic_view_type_options_<?php echo $randonNumber; ?>">
      <?php if(is_array($this->optionsEnable) && in_array('list',$this->optionsEnable)){ ?>
        <a href="javascript:;" rel="list" id="sesgroup_list_view_<?php echo $randonNumber; ?>" class="listicon selectView_<?php echo $randonNumber; ?> <?php if($this->view_type == 'list') { echo 'active'; } ?>" title="<?php echo ((isset($this->htmlTitle) && !empty($this->htmlTitle)) || (empty($this->htmlTitle) && !isset($this->htmlTitle)) ) ? $this->translate('List View') : '' ; ?>"></a>
      <?php } ?>
      <?php if(is_array($this->optionsEnable) && in_array('advlist',$this->optionsEnable)){ ?>
        <a href="javascript:;" rel="advlist" id="sesgroup_advlist_view_<?php echo $randonNumber; ?>" class="a-listicon selectView_<?php echo $randonNumber; ?> <?php if($this->view_type == 'advlist') { echo 'active'; } ?>" title="<?php echo ((isset($this->htmlTitle) && !empty($this->htmlTitle)) || (empty($this->htmlTitle) && !isset($this->htmlTitle)) ) ? $this->translate('Advanced List View') : '' ; ?>"></a>
      <?php } ?>
      <?php if(is_array($this->optionsEnable) && in_array('grid',$this->optionsEnable)){ ?>
        <a href="javascript:;" rel="grid" id="sesgroup_grid_view_<?php echo $randonNumber; ?>" class="gridicon selectView_<?php echo $randonNumber; ?> <?php if($this->view_type == 'grid') { echo 'active'; } ?>" title="<?php echo ((isset($this->htmlTitle) && !empty($this->htmlTitle)) || (empty($this->htmlTitle) && !isset($this->htmlTitle)) ) ? $this->translate('Grid View') : '' ; ?>"></a>
      <?php } ?>
      <?php if(is_array($this->optionsEnable) && in_array('simplegrid',$this->optionsEnable)){ ?>
        <a href="javascript:;" rel="simplegrid" id="sesgroup_simplegrid_view_<?php echo $randonNumber; ?>" class="s-gridicon selectView_<?php echo $randonNumber; ?> <?php if($this->view_type == 'simplegrid') { echo 'active'; } ?>" title="<?php echo ((isset($this->htmlTitle) && !empty($this->htmlTitle)) || (empty($this->htmlTitle) && !isset($this->htmlTitle)) ) ? $this->translate('Plain Grid View') : '' ; ?>"></a>
      <?php } ?>
      <?php if(is_array($this->optionsEnable) && in_array('advgrid',$this->optionsEnable)){ ?>
        <a href="javascript:;" rel="advgrid" id="sesgroup_advgrid_view_<?php echo $randonNumber; ?>" class="a-gridicon selectView_<?php echo $randonNumber; ?> <?php if($this->view_type == 'advgrid') { echo 'active'; } ?>" title="<?php echo ((isset($this->htmlTitle) && !empty($this->htmlTitle)) || (empty($this->htmlTitle) && !isset($this->htmlTitle)) ) ? $this->translate('Advanced Grid View') : '' ; ?>"></a>
      <?php } ?>
      <?php if(is_array($this->optionsEnable) && in_array('pinboard',$this->optionsEnable)){ ?>
        <a href="javascript:;" rel="pinboard" id="sesgroup_pinboard_view_<?php echo $randonNumber; ?>" class="boardicon selectView_<?php echo $randonNumber; ?> <?php if($this->view_type == 'pinboard') { echo 'active'; } ?>" title="<?php echo ((isset($this->htmlTitle) && !empty($this->htmlTitle)) || (empty($this->htmlTitle) && !isset($this->htmlTitle)) ) ? $this->translate('Pinboard View') : '' ; ?>"></a>
     <?php } ?>
     <?php if(is_array($this->optionsEnable) && in_array('map',$this->optionsEnable) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup_enable_location', 1)){ ?>
        <a href="javascript:;" rel="map" id="sesgroup_map_view_<?php echo $randonNumber; ?>" class="mapicon map_selectView_<?php echo $randonNumber;?> selectView_<?php echo $randonNumber;?> <?php if($this->view_type == 'map') { echo 'active'; } ?>" title="<?php echo ((isset($this->htmlTitle) && !empty($this->htmlTitle)) || (empty($this->htmlTitle) && !isset($this->htmlTitle)) ) ? $this->translate('Map View') : '' ; ?>"></a>
     <?php } ?>
    </div>
  </div>
<?php } ?>
...
<div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" class="sesbasic_clearfix sesbasic_bxs clear">
  <ul class="sesgroup_group_listing sesbasic_clearfix clear <?php if($this->view_type == 'pinboard'):?>sesbasic_pinboard_<?php echo $randonNumber;?><?php endif;?>" id="tabbed-widget_<?php echo $randonNumber; ?>" style="min-height:50px;">
<?php } ?>
<?php foreach($this->paginator as $group):?>
  ...
  <?php endif;?> 
  <?php if(isset($this->widgetName) && $this->widgetName == 'manage-groups'):?>
   	<?php $height = $this->params['height'];?>
    <?php $width = $this->params['width'];?>
    <?php $viewTypeClass = "sesgroup_list_type";?>
    <?php include APPLICATION_PATH .  '/application/modules/Sesgroup/views/scripts/group/_managelistView.tpl';?>
  <?php else:?>
    ...
    <?php elseif($this->view_type == 'simplegrid'):?>
      ...
      <?php $viewTypeClass = "sesgroup_list_type";?>
      <?php include APPLICATION_PATH .  '/application/modules/Sesgroup/views/scripts/group/_simpleGridView.tpl';?>
    <?php elseif($this->view_type == 'advlist'):?>
      <?php $height = $this->params['height_advlist'];?>
      <?php $width = $this->params['width_advlist'];?>
      <?php $viewTypeClass = "sesgroup_list_type";?>
      <?php include APPLICATION_PATH .  '/application/modules/Sesgroup/views/scripts/group/_advListView.tpl';?>
    <?php elseif($this->view_type == 'list'):?>
      <?php $height = $this->params['height'];?>
      <?php $width = $this->params['width'];?>
      <?php $viewTypeClass = "sesgroup_list_type";?>
      <?php include APPLICATION_PATH .  '/application/modules/Sesgroup/views/scripts/group/_listView.tpl';?>
    <?php elseif($this->view_type == 'advgrid'):?>
      <?php $height = $this->params['height_advgrid'];?>
      <?php $width = $this->params['width_advgrid'];?>
      <?php $viewTypeClass = "sesgroup_list_type sesbasic_animation";?>
    ...
    <?php elseif($this->view_type == 'map'):?>
      ...
      <?php if($group->lat): ?>
		...
		<?php if(isset($this->featuredLabelActive) || isset($this->sponsoredLabelActive)):?>
          <?php $labels .= "<div class=\"sesgroup_list_labels\">";?>
		  <?php if(isset($this->featuredLabelActive) && $group->featured == 1):?>
            <?php $labels .= "<span class=\"sesgroup_label_featured\" title='".$this->translate('FEATURED')."'><i class='fa fa-star'></i></span>";?>
          <?php endif;?>
          <?php if(isset($this->sponsoredLabelActive) && $group->sponsored == 1):?>
            <?php $labels .= "<span class=\"sesgroup_label_sponsored\" title='".$this->translate('SPONSORED')."'><i class='fa fa-star'></i></span>";?>
          <?php endif;?>
          <?php $labels .= "<span class=\"sesgroup_label_hot\" title='".$this->translate('HOT')."'><i class='fa fa-star'></i></span>";?>
          <?php $labels .= "</div>";?>
		<?php endif;?>
		<?php $vlabel = '';?>
		<?php if(isset($this->verifiedLabelActive) && $group->verified == 1) :?>
          <?php $vlabel = "<i class=\"sesgroup_label_verified sesbasic_verified_icon\" title=\"VERIFIED\"></i>";?>
		<?php endif;?>
		<?php if($group->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.location', 1)):?>
          <?php $locationText = $this->translate('Location');?>
          <?php $locationvalue = $group->location;?>
          <?php if(Engine_Api::_()->getApi('settings','core')->getSetting('sesgroup.enable.map.integration', 1)):?>
           <?php $locationHref = "<a href='".$this->url(array('resource_id' => $group->group_id,'resource_type'=>'sesgroup_group','action'=>'get-direction'), 'sesbasic_get_direction', true)."' class=\"opensmoothboxurl\">$locationvalue</a>";?>
          <?php else:?>
		...
		<?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0 &&  isset($this->likeButtonActive)):?>
          ...
          <?php $likeButton = '<a href="javascript:;" data-url="'.$group->getIdentity().'" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn sesgroup_like_sesgroup_group_'. $group->group_id.' sesgroup_like_sesgroup_group '.$likeClass .'"> <i class="fa fa-thumbs-up"></i><span>'.$group->like_count.'</span></a>';?>
		<?php endif;?>
		<?php $favouriteButton = '';?>
		<?php if(isset($this->favouriteButtonActive) && Engine_Api::_()->user()->getViewer()->getIdentity() != 0 &&  isset($group->favourite_count) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.favourite', 1)){
          $favStatus = Engine_Api::_()->getDbTable('favourites', 'sesgroup')->isFavourite(array('resource_type'=>'sesgroup_group','resource_id'=>$group->group_id));
        $favClass = ($favStatus)  ? 'button_active' : '';
        $favouriteButton = '<a href="javascript:;" class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_fav_btn  sesgroup_favourite_sesgroup_group_'. $group->group_id.' sesgroup_favourite_sesgroup_group '.$favClass .'" data-url="'.$group->getIdentity().'"><i class="fa fa-heart"></i><span>'.$group->favourite_count.'</span></a>';
		}?>
		<?php $urlencode = urlencode(((!empty($_SERVER["HTTPS"]) &&  strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $group->getHref());?>
		<?php $stats = '<div class="_stats sesbasic_text_light">';
		if(isset($this->commentActive)){
          $stats .= '<span title="'.$this->translate(array('%s comment', '%s comments', $group->comment_count), $this->locale()->toNumber($group->comment_count)).'"><i class="fa fa-comment"></i>'.$group->comment_count.'</span>';
		}
		if(isset($this->favouriteActive) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.favourite', 1)){
          $stats .= '<span title="'.$this->translate(array('%s favourite', '%s favourites', $group->favourite_count), $this->locale()->toNumber($group->favourite_count)).'"><i class="fa fa-heart"></i>'. $group->favourite_count.'</span>';
		}
		if(isset($this->viewActive)){
			$stats .= '<span title="'. $this->translate(array('%s view', '%s views', $group->view_count), $this->locale()->toNumber($group->view_count)).'"><i class="fa fa-eye"></i>'.$group->view_count.'</span>';
		}
		if(isset($this->likeActive)){
			$stats .= '<span title="'.$this->translate(array('%s like', '%s likes', $group->like_count), $this->locale()->toNumber($group->like_count)).'"><i class="fa fa-thumbs-up"></i>'.$group->like_count.'</span> ';
		}
		?>
	  <?php $stats .= '</div>';?>
      <?php $socialshare = $this->partial('_dataSharing.tpl','sesgroup',array('mapgroup' => $group,'contactButton' => isset($this->contactButtonActive) ? 1 : 0, 'joinButton' => isset($this->joinButtonActive) ? 1 : 0, 'socialSharing' => isset($this->socialSharingActive) ? 1 : 0)); ?>
      <?php $buttons = '<div class="_btns sesbasic_animation">'.$likeButton.$favouriteButton.'</div>';?>
      <?php $date = $this->partial('_date.tpl','sesgroup',array('mapgroup' => $group)); ?>
      <?php $owner = $group->getOwner();
        ...
        $locationArray[$counter]['id'] = $group->getIdentity();
        ...
        $locationArray[$counter]['lat'] = $group->lat;
        $locationArray[$counter]['lng'] = $group->lng;
  $locationArray[$counter]['labels'] = $labels;
  $locationArray[$counter]['vlabel'] = $vlabel;
        $locationArray[$counter]['iframe_url'] = '';
        $locationArray[$counter]['image_url'] = $group->getPhotoUrl();
        $locationArray[$counter]['sponsored'] = $group->sponsored;
        $locationArray[$counter]['title'] = '<a href="'.$group->getHref().'">'.$group->getTitle().'</a>';  
        $counter++;?>
      <?php endif;?>
    <?php endif;?>
  <?php endif;?>
...
<?php if($this->view_type == 'map'):?>
  ...
  <?php if(!$this->view_more || $this->is_search):?>
    <li id="sesgroup_map_view_<?php echo $randonNumber;?>" style="width:100%">
  ...
  <div id="browse-widget_<?php echo $randonNumber;?>" style="width:100%;">
    <div id="error-message_<?php echo $randonNumber;?>">
      <div class="sesbasic_tip clearfix">
        ...
        <?php if($viewerId && Engine_Api::_()->core()->hasSubject('sesgroup_group')):?>
          <?php $subject = Engine_Api::_()->core()->getSubject('sesgroup_group');?>
          <?php if((Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'sesgroup_group', 'auth_subgroup') && (!$subject->parent_id)) && Engine_Api::_()->getDbTable('grouproles','sesgroup')->toCheckUserGroupRole($viewer->getIdentity(),$subject->getIdentity(),'post_behalf_group')):?>
            <div class="_btn centerT">
              <a href="<?php echo $this->url(array('action' => 'create','parent_id' => $subject->getIdentity()),'sesgroup_general',true);?>" class="sesbasic_link_btn"><?php echo $this->translate("Create New Associates Sub Group")?></a>
            </div>
          <?php endif;?>
        <?php endif;?>
      </div>
  ...
  sesJqueryObject(document).on('click','.selectView_<?php echo $randonNumber; ?>',function(){
    ...
    requestTab_<?php echo $randonNumber; ?> = (new Request.HTML({
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        ...
        var totalGroup= sesJqueryObject('#tabbed-widget_<?php echo $randonNumber; ?>').find("#paginator_count_ajax_sesgroup_entry");
        sesJqueryObject('.sesbasic_view_type_<?php echo $randonNumber; ?>').find('#paginator_count_sesgroup').html(totalGroup.html());
        totalGroup.remove();
  ...
  <?php if($this->params['pagging'] != 'pagging'){ ?>
    ...
    function viewMore_<?php echo $randonNumber; ?> (){
      ...
      requestViewMore_<?php echo $randonNumber; ?> = new Request.HTML({
        method: 'post',
        'url': en4.core.baseUrl + "widget/index/mod/sesgroup/id/<?php echo $this->widgetId; ?>/name/<?php echo $this->widgetName; ?>/openTab/" + openTab_<?php echo $randonNumber; ?>,
        'data': {
          format: 'html',
  ...
  <script type="application/javascript">
    ...
    function newMarkerLayout_<?php echo $randonNumber?>(dataLenth){
      ...
      for(i=0;i<markerArrayData_<?php echo $randonNumber?>.length;i++){
        var images = '<div class="sesgroup_thumb_img"><span style="background-image:url('+markerArrayData_<?php echo $randonNumber?>[i]['image_url']+');"  /></span></div>';		
        ...
        var allowBlounce = <?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.bounce', 1); ?>;
        ...
        '</div>' +
        '<div class="large map_large_marker sesgroup_map_item"><div class="sesgroup_map_thumb sesgroup_thumb">' +
