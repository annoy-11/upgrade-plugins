...
class Sesgroup_IndexController extends Sesapi_Controller_Action_Standard
{
    ...
    public function browsesearchAction()
    {
        ...
        $select = $corePagesTable->select()
            ...
            ->where($coreContentTableName . '.name=?', 'sesgroup.browse-search')
            ->joinLeft($coreContentTableName, $corePagesTableName . '.page_id = ' . $coreContentTableName . '.page_id', $coreContentTableName . '.content_id')
            ->where($corePagesTableName . '.name = ?', 'sesgroup_index_browse');
        $id = $select->query()->fetchColumn();
        $form = new Sesgroup_Form_Search(array('defaultProfileId' => 1, 'contentId' => $id));
    ...
    public function manageAction()
    {
        ...
        $paginator = Engine_Api::_()->getDbTable('groups', 'sesgroup')->getgroupPaginator($params);
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        $canFavourite = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup_allow_favourite', 0);
        $canFollow = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup_allow_follow', 0);
        $shareType = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.allow.share', 0);
		$viewerId = $viewer->getIdentity();
		$levelId = ($viewerId) ? $viewer->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
        $canJoin = $levelId ? Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'group_can_join') : 0;
        ...
        if ($paginator) {
            ...
            foreach ($paginator as $groups) {
                ...
                if (!$canFavourite)
                    unset($groupArray['favourite_count']);
                if (!$canFollow)
                    unset($groupArray['follow_count']);
                unset($groupArray['location']);
                $result[$groupCounter] = $groupArray;
                $statsCounter = 0;
                ...
                $isGroupEdit = Engine_Api::_()->getDbTable('grouproles', 'sesgroup')->toCheckUserGroupRole($viewer->getIdentity(), $groups->getIdentity(), 'manage_dashboard', 'edit');
                $isGroupDelete = Engine_Api::_()->getDbTable('grouproles', 'sesgroup')->toCheckUserGroupRole($viewer->getIdentity(), $groups->getIdentity(), 'manage_dashboard', 'delete');
                $buttonCounter = 0;
                if ($isGroupEdit) {
                    $result[$groupCounter]['buttons'][$buttonCounter]['name'] = 'edit';
                    $result[$groupCounter]['buttons'][$buttonCounter]['label'] = 'Edit';
                    $buttonCounter++;
                }
                if ($isGroupDelete) {
                    $result[$groupCounter]['buttons'][$buttonCounter]['name'] = 'delete';
                    $result[$groupCounter]['buttons'][$buttonCounter]['label'] = 'Delete';
                    $buttonCounter++;
                }
                if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesgrouppackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgrouppackage.enable.package', 0) && (_SESAPI_VERSION_ANDROID >= 2.5 || _SESAPI_VERSION_IOS >= 1.7)) {
                    $package = Engine_Api::_()->getItem('sesgrouppackage_package', $groups->package_id);
                    if($package){
                        if (!$package->isFree()) {
                            $transaction = Engine_Api::_()->getDbTable('transactions', 'sesgrouppackage')->getItemTransaction(array('order_package_id' => $groups->orderspackage_id, 'group' => $groups));
                            if ($transaction) {
                                    ...
                                    if ($package->is_renew_link) {
                                        if (!empty($transaction->expiration_date) && $transaction->expiration_date != '3000-00-00 00:00:00') {
                                            ...
                                            if ($daysLeft <= $renew_link_days || strtotime($transaction->expiration_date) <= time()) {
                                                $result[$groupCounter]['buttons'][$buttonCounter]['name'] = 'payment';
                                                $result[$groupCounter]['buttons'][$buttonCounter]['value'] = $this->getBaseUrl(true, $this->view->url(array('group_id' => $groups->group_id, 'action' => 'index'), 'sesgrouppackage_payment', true));
                                                $result[$groupCounter]['buttons'][$buttonCounter]['label'] = $this->view->translate("Renew Page Payment");
                                                $buttonCounter++;
                                            }
                                        }else {
                                            $result[$groupCounter]['buttons'][$buttonCounter]['name'] = 'package_state';
                                            $result[$groupCounter]['buttons'][$buttonCounter]['value'] = ucwords($transaction->state);
                                            $result[$groupCounter]['buttons'][$buttonCounter]['label'] = $this->view->translate("Payment Status");
                                            $buttonCounter++;
                                        }
                                    }
                                }
                                ...
                                $result[$groupCounter]['buttons'][$buttonCounter]['name'] = 'payment';
                                $result[$groupCounter]['buttons'][$buttonCounter]['value'] = $this->getBaseUrl(true, $this->view->url(array('group_id' => $groups->group_id, 'action' => 'index'), 'sesgrouppackage_payment', true));
                                $result[$groupCounter]['buttons'][$buttonCounter]['label'] = $this->view->translate("Make Payment");
                                $buttonCounter++;
                            }
                        }
                    }
    ...
    {
        $menus = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesgroup_main', array());
        $menu_counter = 0;
        foreach ($menus as $menu) {
            $class = end(explode(' ', $menu->class));
            if ($class != "sesgroup_main_categories" && $class != "sesgroup_main_manage_package" && $class != 'sesgroup_main_browse' && $class != 'sesgroup_main_featured' && $class != 'sesgroup_main_featured' && $class != 'sesgroup_main_verified' && $class != 'sesgroup_main_sponsored' && $class != 'sesgroup_main_hot' && $class != 'sesgroup_main_create' && $class != 'sesgroup_main_manage' && $class != 'sesgroup_main_grouppollhome' && 'sesgroupvideo_main_browsehome' != $class && 'sesgroup_main_grouppollbrowse' != $class)
                continue;
            if($class == "sesgroup_main_manage_package" && !($this->checkVersion(2.6,1.7)))
                continue;
            if($class == "sesgroupvideo_main_browsehome" && !($this->checkVersion(2.6,1.7)))
                continue;
            if($class == "sesgroup_main_grouppollhome" && !($this->checkVersion(2.6,1.7)))
                continue;
    ...
    public function browseAction(){
		...
		if(isset($_POST['filter_sort'])){
            ...
            case "sesgroup_main_featured":
               $value['sort'] = 'featured';
			    break;
            case "sesgroup_main_verified":
                $value['sort'] = 'verified';
				 break;
            case "sesgroup_main_sponsored":
                $value['sort'] = 'sponsored';
				 break;
            case "sesgroup_main_hot":
                $value['sort'] = 'hot';
				 break;
			}
		}
        ...
        if ($page == 1 && $_POST['filter_sort'] == 'sesgroup_main_browse') {
            $categories = Engine_Api::_()->getDbtable('categories', 'sesgroup')->getCategory(array('column_name' => '*', 'limit' => 25));
            $menus = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesgroup_main', array());
            $category_counter = 0;
            foreach ($categories as $category) {
                ...
                $result_category[$category_counter]['total_group_categories'] = $category->total_group_categories;
            ...
            if (!isset($params['category_id'])) {
                ...
                if($paginator->getTotalItemCount() > 0){
                  $popularGroups = $this->getPopularGroups($paginator);
                  if (count($popularGroups)) {
                      $result['popularGroups'] = $popularGroups;
                  }
                }
            }
        }
        $getGroups = $this->getGroups($paginator);
	...
	//it's related groups
    public function getPopularGroups($groupPaginator){
        $groupid = array();
        foreach ($groupPaginator as $groups) {
            $groupid[] = $groups->group_id;
        }
        $params['info'] = 'most_viewed';
        $params['notGroupId'] = $groupid;
        $paginator = Engine_Api::_()->getDbTable('groups', 'sesgroup')->getGroupPaginator($params);
        $paginator->setItemCountPerPage(5);
        $paginator->setCurrentPageNumber(1);
        $result = $this->getGroups($paginator);
        return $result;
    }
    ...
    public function getGroups($paginator){
        $result = array();
        $counter = 0;
        $canFavourite = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.allow.favourite', 0);
		    $likeFollowIntegrate = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.allow.integration', 0);
        $canFollow = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.allow.follow', 0);
        $shareType = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.allow.share', 1);
        $hideIdentity = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.show.userdetail', 0);
        $viewer = Engine_Api::_()->user()->getViewer();
        $viewerId = $viewer->getIdentity();
		    $levelId = ($viewerId) ? $viewer->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
        $canJoin = Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'group_can_join');
        foreach ($paginator as $groups) {
            $group = $groups->toArray();
            $result[$counter] = $group;
            $result[$counter]['likeFollowIntegrate'] = $likeFollowIntegrate?true:false;
            if(!$hideIdentity)
            $result[$counter]['owner_title'] = $groups->getOwner()->getTitle();
            ...
            if ($groups->category_id) {
                $category = Engine_Api::_()->getItem('sesgroup_category', $groups->category_id);
                if ($category) {
                    $result[$counter]['category_title'] = $category->category_name;
                    if ($groups->subcat_id) {
                        $subcat = Engine_Api::_()->getItem('sesgroup_category', $groups->subcat_id);
                        if ($subcat) {
                            $result[$counter]['subcategory_title'] = $subcat->category_name;
                            if ($groups->subsubcat_id) {
                                $subsubcat = Engine_Api::_()->getItem('sesgroup_category', $groups->subsubcat_id);
                                if ($subsubcat) {
            ...
            $tags = array();
            foreach ($groups->tags()->getTagMaps() as $tagmap) {
            ...
            $result[$counter]['images']['main']= $this->getBaseUrl(true, $groups->getPhotoUrl());
            $result[$counter]['cover_image']['main'] = $this->getBaseUrl(true, $groups->getCoverPhotoUrl());
            $result[$counter]['cover_images']['main'] = $result[$counter]['cover_image']['main'];
            $showLoginformFalse = false;
            if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.contact.details', 1) && $viewerId == 0) {
                $showLoginformFalse = true;
            }
            $i = 0;
            if ($groups->group_contact_email || $groups->group_contact_phone || $groups->group_contact_website) {
                if ($groups->group_contact_email) {

                    $result[$counter]['buttons'][$i]['name'] = 'mail';
                    $result[$counter]['buttons'][$i]['label'] = 'Send Email';
                    $result[$counter]['buttons'][$i]['value'] = $groups->group_contact_email;
                    $i++;
                }
                if ($groups->group_contact_phone) {
                    $result[$counter]['buttons'][$i]['name'] = 'phone';
                    $result[$counter]['buttons'][$i]['label'] = 'Call';
                    $result[$counter]['buttons'][$i]['value'] = $groups->group_contact_phone;
                    $i++;
                }
                if ($groups->group_contact_website) {
                    $result[$counter]['buttons'][$i]['name'] = 'website';
                    $result[$counter]['buttons'][$i]['label'] = 'Visit Website';
                    $result[$counter]['buttons'][$i]['value'] = $groups->group_contact_website;
                    $i++;
                }
            ...
            if ($groups->is_approved) {
				
                if ($shareType) {
                    $result[$counter]["share"]["imageUrl"] = $this->getBaseUrl(false, $groups->getPhotoUrl());
										$result[$counter]["share"]["url"] = $this->getBaseUrl(false,$groups->getHref());
                    $result[$counter]["share"]["title"] = $groups->getTitle();
                    $result[$counter]["share"]["description"] = strip_tags($groups->getDescription());
					$result[$counter]["share"]["setting"] = $shareType;
                    $result[$counter]["share"]['urlParams'] = array(
                        "type" => $groups->getType(),
                        "id" => $groups->getIdentity()
                    );
                }
            }
            if ($groups->is_approved) {
				if($viewerId != $groups->owner_id){
                ...
                if ($canJoin) {
                    //if ($viewerId) {
                        $row = $groups->membership()->getRow($viewer);
                        if (null === $row) {
                            if ($groups->membership()->isResourceApprovalRequired()) {
                                $result[$counter]['buttons'][$i]['name'] = 'request';
                                $result[$counter]['buttons'][$i]['label'] = 'Request Membership';
                                $i++;
                            } else {
                                $result[$counter]['buttons'][$i]['name'] = 'join';
                                $result[$counter]['buttons'][$i]['label'] = 'Join Group';
                                $i++;
                            }
                        } else if ($row->active) {
                            if (!$groups->isOwner($viewer)) {
                                $result[$counter]['buttons'][$i]['label'] = 'Leave Group';
            ...
            if ($viewerId != 0) {
                $result[$counter]['is_content_like'] = Engine_Api::_()->sesapi()->contentLike($groups);
                $result[$counter]['content_like_count'] = (int)Engine_Api::_()->sesapi()->getContentLikeCount($groups);
                if ($canFavourite) {
                    $result[$counter]['is_content_favourite'] = Engine_Api::_()->sesapi()->contentFavoutites($groups, 'favourites', 'sesgroup', 'sesgroup_group', 'owner_id');
                    $result[$counter]['content_favourite_count'] = (int)Engine_Api::_()->sesapi()->getContentFavouriteCount($groups, 'favourites', 'sesgroup', 'sesgroup_group', 'owner_id');
                }
                if ($canFollow) {
                    $result[$counter]['is_content_follow'] = $this->contentFollow($groups, 'followers', 'sesgroup', 'sesgroup_group', 'owner_id');
                    $result[$counter]['content_follow_count'] = (int)$this->getContentFollowCount($groups, 'followers', 'sesgroup', 'sesgroup_group', 'owner_id');
                }

            }
			
            if (isset($groups->location) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.location', 1)) {
                unset($groups->location);
                $location = Engine_Api::_()->getDbTable('locations', 'sesbasic')->getLocationData('sesgroup_group', $groups->getIdentity());
				
                if ($location) {
                    $result[$counter]['location'] = $location->toArray();
                    if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.map.integration', 1)) {
   ...
   public function getContentFollowCount($subject,$tableName = "",$modulename = "",$resources_type = "",$column_name = "resource_id"){
    ...
    {
        $params['sort'] = 'featured';
        $paginator = Engine_Api::_()->getDbTable('groups', 'sesgroup')
            ->getGroupPaginator($params);
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        ...
        $result['groups'] = $this->getGroups($paginator);
        $extraParams['pagging']['total_page'] = $paginator->getPages()->pageCount;
        $extraParams['pagging']['total'] = $paginator->getTotalItemCount();
        $extraParams['pagging']['current_page'] = $paginator->getCurrentPageNumber();
        $extraParams['pagging']['next_page'] = $extraParams['pagging']['current_page'] + 1;
    ...
    public function categoriesAction(){
        $paginator = Engine_Api::_()->getDbTable('categories', 'sesgroup')->getGroupPaginator($params);
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        if ($paginator->getCurrentPageNumber() == 1) {
            $categories = Engine_Api::_()->getDbtable('categories', 'sesgroup')->getCategory(array('column_name' => '*', 'limit' => 25));
    ...
    public function getCategory($categoryPaginator){
        ...
        foreach ($categoryPaginator as $categories) {
			...
			if($paginator->getTotalItemCount() > 1){
				$result[$counter] = $group;
				$result[$counter]['items'] = $this->getGroups($paginator);
    ...
    public function contactAction(){
        ...
        $form = new Sesgroup_Form_ContactOwner();
        $form->group_owner_id->setValue($this->_getParam('owner_id', 0));
    ...
    public function joinAction()
    {
        ...
        $viewer = Engine_Api::_()->user()->getViewer();
        $item = Engine_Api::_()->getItem('sesgroup_group', $group_id);
        ...
        $form = new Sesgroup_Form_Member_Join();
        //      $form->group_owner_id->setValue($this->_getParam('owner_id',0));
        if ($this->_getParam('getForm')) {
            $formFields = Engine_Api::_()->getApi('FormFields', 'sesapi')->generateFormFields($form);
            $this->generateFormFields($formFields, array('resources_type' => 'sesgroup_group'));
        ...
        try {
            ...
            $owner = $item->getOwner();
            Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $item, 'sesgroup_group_join');

            Engine_Api::_()->getApi('mail', 'core')->sendSystem($item->getOwner(), 'notify_sesgroup_group_groupjoined', array('group_title' => $item->getTitle(), 'sender_title' => $viewer->getOwner()->getTitle(), 'object_link' => $item->getHref(), 'host' => $_SERVER['HTTP_HOST']));
            ...
            foreach ($joinedMembers as $joinedMember) {
                ...
                Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($joinedMember, $viewer, $item, 'sesgroup_group_groupsijoinedjoin');

                Engine_Api::_()->getApi('mail', 'core')->sendSystem($item->getOwner(), 'notify_sesgroup_group_joingroupjoined', array('group_title' => $item->getTitle(), 'sender_title' => $viewer->getTitle(), 'object_link' => $item->getHref(), 'host' => $_SERVER['HTTP_HOST']));
            }
            ...
            foreach ($followerMembers as $followerMember) {
                ...
                Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($followerMember, $viewer, $item, 'sesgroup_group_groupsifollowedjoin');

                Engine_Api::_()->getApi('mail', 'core')->sendSystem($item->getOwner(), 'notify_sesgroup_group_joinedgroupfollowed', array('group_title' => $item->getTitle(), 'sender_title' => $viewer->getTitle(), 'object_link' => $item->getHref(), 'host' => $_SERVER['HTTP_HOST']));
            }

            // Add activity if membership status was not valid from before
            if (!$membership_status) {
                $activityApi = Engine_Api::_()->getDbtable('actions', 'activity');
                $action = $activityApi->addActivity($viewer, $item, 'sesgroup_group_join');
                if ($action) {
            ...
            $viewerId = $viewer->getIdentity();
            $result['message'] = $this->view->translate('Group Successfully Joined.');
    ...
    public function request()
    {
        ...
        // Make form
        $form = new Sesgroup_Form_Member_Request();
		if ($this->_getParam('getForm')) {
			$formFields = Engine_Api::_()->getApi('FormFields', 'sesapi')->generateFormFields($form);
			$this->generateFormFields($formFields, array('resources_type' => 'sesgroup_group'));
		}
    ...
    public function likeAction()
    {
        ...
        if (count($result) > 0) {
            ...
            try {
                $result->delete();
                $db->commit();
                $temp['data']['message'] = $this->view->translate('Group Successfully Unliked.');
            } catch (Exception $e) {
                $db->rollBack();
            ...
            try {
                ...
                //Commit
                $db->commit();
                $temp['data']['message'] = $this->view->translate('Group Successfully Liked.');
            } catch (Exception $e) {
                $db->rollBack();
                Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' =>$this->getMessage(), 'result' => array()));
            }
            ...
            if ($notificationType && $owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
                ...
                if ($type == 'sesgroup_group') {
                    ...
                    foreach ($joinedMembers as $joinedMember) {
                        $joinedMember = Engine_Api::_()->getItem('user', $joinedMember->user_id);
                        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($joinedMember, $viewer, $subject, 'sesgroup_group_groupsijoinedlike');
                    ...
                    foreach ($followerMembers as $followerMember) {
                        $followerMember = Engine_Api::_()->getItem('user', $followerMember->owner_id);
                        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($followerMember, $viewer, $subject, 'sesgroup_group_groupsifollowedlike');
    ...
    public function followAction()
    {
        ...
        $item_id = $this->_getParam('id',$this->_getParam('group_id',0));
        if (intval($item_id) == 0) {
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('Invalid argument supplied.'), 'result' => array()));
        ...
        if (count($Fav) > 0) {
            ...
            try {
                $Fav->delete();
                $db->commit();
                $temp['data']['message'] = 'Group Successfully Unfollowed.';
            ...
            try {
                ...
                // Commit
                $db->commit();
                $temp['data']['message'] = 'Group Successfully Followed.';
            } catch (Exception $e) {
    ...
    public function favouriteAction()
    {
        ...
        if (count($Fav) > 0) {
            ...
            try {
                $Fav->delete();
                $db->commit();
                $temp['data']['message'] = 'Group Successfully Unfavourited.';
            } catch (Exception $e) {
                $db->rollBack();
                $temp['data']['message'] = 'Database Error.';
                //                throw $e;
            ...
            try {
                ...
                // Commit
                $db->commit();
                $temp['data']['message'] = 'Group Successfully Favourited.';
            } catch (Exception $e) {
                $db->rollBack();
                $temp['data']['message'] = 'Database Error.';
                //                throw $e;
    ...
    public function getgroup($group)
    {
        ...
        if (!empty($group->category_id)) {
            $category = Engine_Api::_()->getDbtable('categories', 'sesgroup')->find($group->category_id)->current();
        }
        $groupTags = $group->tags()->getTagMaps();
		    $likeFollowIntegrate = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.allow.integration', 0);
        $canComment = $group->authorization()->isAllowed($viewer, 'comment');
        $shareType = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.allow.share', 1);
        $canFavourite = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup_allow_favourite', 0);
        $canFollow = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup_allow_follow', 0);
        $canJoin = Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'group_can_join');
        $isGroupEdit = Engine_Api::_()->sesgroup()->groupPrivacy($group, 'edit');
        $canUploadCover = Engine_Api::_()->authorization()->isAllowed('sesgroup_group', $viewer, 'upload_cover');
        $canUploadPhoto = Engine_Api::_()->authorization()->isAllowed('sesgroup_group', $viewer, 'upload_mainphoto');

        $isGroupDelete = Engine_Api::_()->sesgroup()->groupPrivacy($group, 'delete');
        $likeStatus = Engine_Api::_()->sesgroup()->getLikeStatus($group->group_id, $group->getType());
        $followStatus = Engine_Api::_()->getDbTable('followers', 'sesgroup')->isFollow(array('resource_id' => $group->group_id, 'resource_type' => $group->getType()));
        $favouriteStatus = Engine_Api::_()->getDbTable('favourites', 'sesgroup')->isFavourite(array('resource_id' => $group->group_id, 'resource_type' => $group->getType()));


        $owner = $group->getOwner();
        $hideIdentity = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup_show_userdetail', 0);
        if(!$hideIdentity)
        $groupdata['owner_title'] = $group->getOwner()->getTitle();
        $currency = Engine_Api::_()->getApi('settings', 'core')->getSetting('payment.currency', 'USD');
        $curArr = Zend_Locale::getTranslationList('CurrencySymbol');
        $groupdata['currency'] = $curArr[$currency];
		    $groupdata['likeFollowIntegrate'] = $likeFollowIntegrate?true:false;
        if ($likeStatus && $viewer_id) {
            $groupdata['is_content_like'] = true;
        } else {
            $groupdata['is_content_like'] = false;
        }
		if($canFollow){
			$groupdata['is_content_follow'] = $followStatus >0?true:false;
		}
		if($canFavourite){
			$groupdata['is_content_follow'] = $favouriteStatus >0?true:false;
		}
        if ($group->category_id) {
            $category = Engine_Api::_()->getItem('sesgroup_category', $group->category_id);
            if ($category) {
                $groupdata['category_title'] = $category->category_name;

                if ($group->subcat_id) {
                    $subcat = Engine_Api::_()->getItem('sesgroup_category', $group->subcat_id);
                    if ($subcat) {
                        $groupdata['subcategory_title'] = $subcat->category_name;
                        if ($group->subsubcat_id) {
                            $subsubcat = Engine_Api::_()->getItem('sesgroup_category', $group->subsubcat_id);
                            if ($subsubcat) {
                                $groupdata['subsubcategory_title'] = $subsubcat->category_name;
                            }
                        }
                    }
                }
        ...
        $item = Engine_Api::_()->getItem('sesgroup_group', $group->group_id);
        $joinedMembers = Engine_Api::_()->sesgroup()->getallJoinedMembers($item);
        ...
        $groupdata['images']['main'] = $this->getBaseUrl(true, $group->getPhotoUrl());
        $groupdata['cover_image']['main'] = $this->getBaseUrl(true, $group->getCoverPhotoUrl());
        $groupdata['cover_images']['main'] = $groupdata['cover_image']['main'];
        $showLoginformFalse = false;
        if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.contact.details', 1)) {
            $showLoginformFalse = true;
        }
        $l = 0;
        if ($group->group_contact_email || $group->group_contact_phone || $group->group_contact_website) {
            if ($group->group_contact_email) {

                $groupdata['buttons'][$l]['name'] = 'mail';
                $groupdata['buttons'][$l]['label'] = 'Send Email';
                $groupdata['buttons'][$l]['value'] = $group->group_contact_email;
                $l++;

            }
            if ($group->group_contact_phone) {
                $groupdata['buttons'][$l]['name'] = 'phone';
                $groupdata['buttons'][$l]['label'] = 'Call';
                $groupdata['buttons'][$l]['value'] = $group->group_contact_phone;
                $l++;
            }
            if ($group->group_contact_website) {

                $groupdata['buttons'][$l]['name'] = 'website';
                $groupdata['buttons'][$l]['label'] = 'Visit Website';
                $groupdata['buttons'][$l]['value'] = $group->group_contact_website;
                $l++;
            }

            $groupdata['showLoginForm'] = $showLoginformFalse;


        }

        $canCall = Engine_Api::_()->getDbTable('callactions', 'sesgroup')->getCallactions(array('group_id' => $group->getIdentity()));
        if ($canCall) {
        ...
        $groupdata['is_feed_allowed'] = true;
        if( !$group->authorization()->isAllowed($this->view->viewer(), 'view') )
           $groupdata['is_feed_allowed'] = false; 
        ...
        if ($group->group_contact_email || $group->group_contact_phone || $group->group_contact_website) {
            if ($group->group_contact_email) {
                ...
                $result['about'][$i]['value'] = $group->group_contact_email;
                $i++;
            ...
            if ($group->group_contact_phone) {
                ...
                $result['about'][$i]['value'] = $group->group_contact_phone;
                $i++;
            }
            if ($group->group_contact_website) {
                ...
                $result['about'][$i]['value'] = $group->group_contact_website;
                $i++;
            }
            if ($group->creation_date) {
                $result['about'][$i]['name'] = 'createDate';
                $result['about'][$i]['label'] = 'Create Date';
                $result['about'][$i]['value'] = $group->creation_date;
                $i++;
            }
            if ($group->category_id) {
                $category = Engine_Api::_()->getItem('sesgroup_category', $group->category_id);
                if ($category) {
                    ...
                    //                     $groupdata['about'][$i]['id'] = $group->category_id;
                }
                $i++;
            }
            if (count($tags)) {
        ...
        $relatedParams['category_id'] = $group->category_id;
        $groupid = array();
        $groupid[] = $group->group_id;
        $relatedParams['notGroupId'] = $groupid;
        if ($groups = $this->relatedgroups($relatedParams)) {
            $result['relatedGroups'] = $groups;
        }
        $result['photo'] = $this->photo($group->group_id);
        if ($group->is_approved) {
            if ($shareType) {

                $groupdata["share"]["imageUrl"] = $this->getBaseUrl(false, $group->getPhotoUrl());
								$groupdata["share"]["url"] = $this->getBaseUrl(false,$group->getHref());
                $groupdata["share"]["title"] = $group->getTitle();
                $groupdata["share"]["description"] = strip_tags($group->getDescription());
				$groupdata["share"]["setting"] = $shareType;
                $groupdata["share"]['urlParams'] = array(
                    "type" => $group->getType(),
        ...
        $m = 0;
        if ($group->is_approved) {
            if($viewerId != $group->owner_id) {
                $groupdata['buttons'][$m]['name'] = 'contact';
                $groupdata['buttons'][$m]['label'] = 'Contact';
                $m++;
            }
            if ($shareType) {
                $groupdata['buttons'][$m]['name'] = 'share';
                $groupdata['buttons'][$m]['label'] = 'Share';
                $m++;
            }
            ...
            if ($canJoin) {
               ...
               // if ($viewerId) {
                    //                    $m++;
                    $row = $group->membership()->getRow($viewer);
                    if (null === $row) {
                        if ($group->membership()->isResourceApprovalRequired()) {
                            $groupdata['join'][$joincounter]['name'] = 'request';
                            $groupdata['join'][$joincounter]['label'] = 'Request Membership';
                            $joincounter++;

                        } else {
                            $groupdata['join'][$joincounter]['name'] = 'join';
                            $groupdata['join'][$joincounter]['label'] = 'Join Group';
                            $joincounter++;
                        }
                    } else if ($row->active) {
                        if (!$group->isOwner($viewer)) {
                            $groupdata['join'][$joincounter]['label'] = 'Leave Group';
                            $groupdata['join'][$joincounter]['name'] = 'leave';
                            $joincounter++;
                        }
                    } else if (!$row->resource_approved && $row->user_approved) {
                        $groupdata['join'][$joincounter]['label'] = 'Cancel Membership Request';
                        $groupdata['join'][$joincounter]['name'] = 'cancel';
                        $joincounter++;

                    } else if (!$row->user_approved && $row->resource_approved) {
                        $groupdata['join'][$joincounter]['label'] = 'Accept Membership Request';
                        $groupdata['join'][$joincounter]['name'] = 'accept';
                        $joincounter++;
                        $groupdata['join'][$joincounter]['label'] = 'Ignore Membership Request';
                        $groupdata['join'][$joincounter]['name'] = 'reject';
                    }
              //  }
            }
        }
        ...
        if ($viewer->getIdentity() != 0) {
            $groupdata['is_content_like'] = Engine_Api::_()->sesapi()->contentLike($group);
            $groupdata['content_like_count'] = (int)Engine_Api::_()->sesapi()->getContentLikeCount($group);
            if ($canFavourite) {
                $groupdata['is_content_favourite'] = Engine_Api::_()->sesapi()->contentFavoutites($group, 'favourites', 'sesgroup', 'sesgroup_group', 'owner_id');
                $groupdata['content_favourite_count'] = (int)Engine_Api::_()->sesapi()->getContentFavouriteCount($group, 'favourites', 'sesgroup', 'sesgroup_group', 'owner_id');
            }
            if ($canFollow) {
                $groupdata['is_content_follow'] = $this->contentFollow($group, 'followers', 'sesgroup', 'sesgroup_group', 'owner_id');
                $groupdata['content_follow_count'] = (int)$this->getContentFollowCount($group, 'favourites', 'sesgroup', 'sesgroup_group', 'owner_id');
            }
        }

        if ($group->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.location', 1)) {
            unset($group['location']);
            $location = Engine_Api::_()->getDbTable('locations', 'sesbasic')->getLocationData('sesgroup_group', $group->getIdentity());
            if ($location) {
                $groupdata['location'] = $location->toArray();
                if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.enable.map.integration', 1)) {
                    $groupdata['location']['showMap'] = true;
                } else {
                    $groupdata['location']['showMap'] = false;
                }
            }
        }
        ...
        if ($isGroupDelete) {
            $groupdata['groupDelete'] = true;
        } else {
            $groupdata['groupDelete'] = false;
        }
        ...
        if ($isGroupEdit) {
            ...
            if ($canUploadCover) {
                $i = 0;
                if (isset($group->cover) && $group->cover != 0 && $group->cover != '') {
                    $groupdata['updateCoverPhoto'][$i]['label'] = $this->view->translate('Change Cover Photo');
                    $groupdata['updateCoverPhoto'][$i]['name'] = 'upload';
                    $i++;
                    $groupdata['updateCoverPhoto'][$i]['label'] = $this->view->translate('Remove Cover Photo');
                    $groupdata['updateCoverPhoto'][$i]['name'] = 'removePhoto';
                    $i++;
                    $groupdata['updateCoverPhoto'][$i]['label'] = $this->view->translate('View Cover Photo');
                    $groupdata['updateCoverPhoto'][$i]['name'] = 'view';
                    $i++;
                } else {
                    $groupdata['updateCoverPhoto'][$i]['label'] = $this->view->translate('Upload Cover Photo');
                    $groupdata['updateCoverPhoto'][$i]['name'] = 'upload';
                    $i++;
                }
                //$groupdata['updateCoverPhoto'][$i]['label'] = $this->view->translate('Choose From Albums');
                //$groupdata['updateCoverPhoto'][$i]['name'] = 'album';
            }
          ...
          if($canUploadPhoto){
            ...
            if (!empty($group->photo_id)) {
                $groupdata['updateProfilePhoto'][$j]['label'] = $this->view->translate('Change Photo');
                $groupdata['updateProfilePhoto'][$j]['name'] = 'upload';
                $j++;
                $groupdata['updateProfilePhoto'][$j]['label'] = $this->view->translate('Remove Photo');
                $groupdata['updateProfilePhoto'][$j]['name'] = 'removePhoto';
                
            } else {
                $groupdata['updateProfilePhoto'][$j]['label'] = $this->view->translate('Upload Photo');
                $groupdata['updateProfilePhoto'][$j]['name'] = 'upload';
                $j++;
        ...
        //navigation
        $result['options'] = $this->getNavigation($group,$viewer);
        $tabcounter = 0;
        ...
        if(Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'auth_subgroup') || Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'view')) {
            $result['menus'][$tabcounter]['name'] = 'associateGroups';
            $result['menus'][$tabcounter]['label'] = 'Associated Groups';
            $tabcounter++;
        }
        $group_allow_announcement = Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'group_allow_announcement');
        $group_service = Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'group_service');
        $group_overview = Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'group_overview');
        //if ($group_allow_announcement) {
        ...
        $tabcounter++; 
        if ($group_service && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.allow.service', 0)) {
		...
		if($viewer->getIdentity() > 0 && !$group->isOwner($viewer) && Engine_Api::_()->authorization()->getPermission($viewer, 'sesgroup_group', 'auth_claim')){
			$result['menus'][$tabcounter]['name'] = 'claim';
			$result['menus'][$tabcounter]['label'] = 'Claim Group';
			$tabcounter++;
		}
		if(Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('sesgrouppoll') && (_SESAPI_VERSION_ANDROID >= 2.6 || _SESAPI_VERSION_IOS >= 1.7)){
	...
	public function claimAction(){
		...
		if($group_title)
			$_POST['title'] = $group_title;
		$form = new Sesgroup_Form_Claim();
		if(isset($_POST))
		$form->populate($_POST);
		...
		$selectClaimTable = $groupClaimTable->select()
		  ...
		  ->where('user_id =?', $viewer->getIdentity());
		  $selectClaimTable->where('group_id =?', $group_id);
		$claimedGroups = $groupClaimTable->fetchAll($selectClaimTable);
		if(count($claimedGroups->toArray()) >0){
		...
		try {
			...
			$sesgroupClaim->group_id = $values['group_id'];
			$sesgroupClaim->title = $values['title'];
			$sesgroupClaim->user_email = $values['user_email'];
			$sesgroupClaim->user_name = $values['user_name'];
			$sesgroupClaim->contact_number = $values['contact_number'];
			$sesgroupClaim->description = $values['description'];
			$sesgroupClaim->save();
			// Commit
			$db->commit();
		}
		...
		$groupItem = Engine_Api::_()->getItem('sesgroup_group', $values['group_id']);
		...
		Engine_Api::_()->getApi('mail', 'core')->sendSystem($groupOwnerEmail, 'sesgroup_group_owner_claim', $mail_settings);
...
function getNavigation($group,$viewer){
    $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesgroup_profile');
    $navigationCounter = 0;
	$viewerId = $viewer->getIdentity();
	$levelId = ($viewerId) ? $viewer->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
	$canJoin = Engine_Api::_()->authorization()->getPermission($levelId, 'sesgroup_group', 'group_can_join');
  ...
  $class = end(explode(' ', $link->class));
   ...
   if ($class != "sesgroup_profile_addtoshortcut") {
            $action = '';
            if ($class == 'sesgroup_profile_dashboard') {
                $label = $label;
                $action = 'dashboard';
                $baseurl = $this->getBaseUrl();
                $custumurl = $group->custom_url;
                $pluralurl = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup_groups_manifest', null);
                if ($pluralurl) {
                ...
                $value = $url;
            } elseif ($class == 'sesgroup_profile_member') {
      $row = $group->membership()->getRow($viewer);
      if (null === $row) {
			if ($group->membership()->isResourceApprovalRequired()) {
					$action = 'request';
            ...
            } elseif ($class == 'sesgroup_profile_invite') {
                $action = 'invite';
            } elseif ($class == 'sesgroup_profile_report') {
       if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroup.allow.report', 1))
         continue;
                $action = 'report';
            } elseif ($class  == 'sesgroup_profile_share') {
                $action = 'share';
            } elseif ($class  == 'sesgroup_profile_member') {
                $action = 'join';
            } elseif ($class == 'sesgroup_profile_delete') {
                $action = 'delete';
            } elseif ($class == 'sesgroup_profile_subgroup') {
                $action = 'createAssociateGroup';
            } elseif ($class == 'sesgroup_profile_like') {
                $action = 'likeasyourgroup';
            } elseif ($class == 'sesgroup_profile_unlike') {
                $action = 'unlikeasyourgroup';
            }
            if ($class == 'sesgroup_profile_dashboard') {
                ...
                $navigationCounter++;
				if($this->_helper->requireAuth()->setAuthParams('sesgroup_group', null, 'edit')->isValid()){
					 $result[$navigationCounter]['label'] = $this->view->translate('Edit Group');
                $result[$navigationCounter]['name'] = 'edit';
                $navigationCounter++;
				}
            }elseif($class == 'sesgroup_profile_delete'){
				if(!$this->_helper->requireAuth()->setAuthParams('sesgroup_group', null, 'delete')->isValid())
					continue;
				$result[$navigationCounter]['label'] = $label;
                $result[$navigationCounter]['name'] = $action;
                $navigationCounter++;
  ...
  /*if ($canJoin) {
		...
		if (null === $row) {
			if ($group->membership()->isResourceApprovalRequired()) {
				...
				$result[$navigationCounter]['label'] = 'Join group';
				$joincounter++;
			}
		} else if ($row->active) {
			if (!$group->isOwner($viewer)) {
				$result[$navigationCounter]['label'] = 'Leave group';
	...
	*/
    ...
    public function relatedgroups($params)
    {
        $paginator = Engine_Api::_()->getDbTable('groups', 'sesgroup')
            ->getGroupPaginator($params);
    ...
    public function photo($groupid)
    {
        $params['group_id'] = $groupid;
        $paginator = Engine_Api::_()->getDbTable('photos', 'sesgroup')
    ...
    public function infoAction()
    {
        ...
        $result['information'] = $this->getInformation($group);
	...
	public function moreMembersAction(){
		...
		if($this->_getParam('type',null) == 'like'){
            ...
            ->where('resource_id =?', $group->group_id )
            ->where('resource_type =?', 'sesgroup_group');
		}else if($this->_getParam('type',null) == 'group'){
			$tableLikegroups = Engine_Api::_()->getDbTable('likegroups', 'sesgroup');
			$select = $tableLikegroups->select()->where('group_id =?', $group->group_id);
			$groupcheck = true;
      	}
	   ...
	   if(count($Members) && $groupcheck) {
			$likeGroupsCounter = 0;
			foreach ($Members as $likegroup) {	
				$item = Engine_Api::_()->getItem('sesgroup_group', $likegroup->like_group_id);
				if ($item) {
					$nameLike = $item->getTitle();;
					$image = Engine_Api::_()->sesapi()->getPhotoUrls($item, '', "");
					if ($image) {
						$result['group_liked_by_this_group'][$likeGroupsCounter]['images'] = $image;
					}
					if ($nameLike) {
						$result['group_liked_by_this_group'][$likeGroupsCounter]['name'] = $nameLike;
					}
					$result['group_liked_by_this_group'][$likeGroupsCounter]['group_id'] = $item->group_id;
				}
				$likeGroupsCounter++;
			}
		}
	 if (count($Members) && !$groupcheck && $this->_getParam('type',null) != 'like') {
  ...
  public function userAge($member){
    ...
    public function getInformation($groups)
    {
        ...
        $select = $openhourstable->select()
            ...
            ->where('group_id =?', $groups->getIdentity());
        ...
        $result['basicInformation'][$basicInformationCounter]['value'] = $groups->creation_date;
        ...
        $state[$statsCounter]['value'] = $groups->comment_count;
        $state[$statsCounter]['label'] = 'Comments';
        $statsCounter++;


        $state[$statsCounter]['name'] = 'like';
        $state[$statsCounter]['value'] = $groups->like_count;
        $state[$statsCounter]['label'] = 'Likes';
        $statsCounter++;


        $state[$statsCounter]['name'] = 'view';
        $state[$statsCounter]['value'] = $groups->view_count;
        ...
        if ($canFavourite) {
            ...
            $state[$statsCounter]['value'] = $groups->favourite_count;
        ...
        if ($canFollow) {
            ...
            $state[$statsCounter]['value'] = $groups->follow_count;
        ...
        $fieldStructure = Engine_Api::_()->fields()->getFieldsStructurePartial($groups);
        if (count($fieldStructure)) { // @todo figure out right logic
            $content = $this->view->fieldSesapiValueLoop($groups, $fieldStructure);;
            $counter = 0;
        ...
        $result['Detail'] = $groups->description;
        ...
        if ($groups->group_contact_phone)
            $result['contactInformation'][$contactInformationCounter]['value'] = $groups->group_contact_phone;
        ...
        if ($groups->group_contact_email)
            $result['contactInformation'][$contactInformationCounter]['value'] = $$groups->group_contact_email;
        $contactInformationCounter++;
        ...
        if ($groups->group_contact_website)
            $result['contactInformation'][$contactInformationCounter]['value'] = $groups->group_contact_website;
        $contactInformationCounter++;
        ...
        if ($groups->group_contact_facebook)
            $result['contactInformation'][$contactInformationCounter]['value'] = $groups->group_contact_facebook;
        $contactInformationCounter++;
        ...
        if ($groups->group_contact_linkedin)
            $result['contactInformation'][$contactInformationCounter]['value'] = $groups->group_contact_linkedin;
        $contactInformationCounter++;
        ...
        if ($groups->group_contact_twitter)
            $result['contactInformation'][$contactInformationCounter]['value'] = $groups->group_contact_twitter;
        $contactInformationCounter++;
        ...
        if ($groups->group_contact_instagram)
            $result['contactInformation'][$contactInformationCounter]['value'] = $groups->group_contact_instagram;
        $contactInformationCounter++;
        ...
        if ($groups->group_contact_pinterest)
            $result['contactInformation'][$contactInformationCounter]['value'] = $groups->group_contact_pinterest;

        $likeMembers = Engine_Api::_()->sesgroup()->getMemberByLike($groups->group_id);
        $favMembers = Engine_Api::_()->sesgroup()->getMemberFavourite($groups->group_id);
        $followMembers = Engine_Api::_()->sesgroup()->getMemberFollow($groups->group_id);
        $tableLikegroups = Engine_Api::_()->getDbTable('likegroups', 'sesgroup');
        $selelct = $tableLikegroups->select()->where('group_id =?', $groups->group_id);
        $likeGroupResult = $tableLikegroups->fetchAll($selelct);
        if (count($likeGroupResult)) {
            $likeGroupsCounter = 0;
            $result['total_group_liked_by_this_group'] = count($likeGroupResult) > 4 ? count($likeGroupResult) - 4 : 0;
            foreach ($likeGroupResult as $likegroup) {
              if($likeGroupsCounter > 3)
                break;
                $item = Engine_Api::_()->getItem('sesgroup_group', $likegroup->like_group_id);
                if ($item) {
                    $nameLike = $item->getTitle();;
                  ...
                  $result['total_group_liked_by_this_group'] = $result['total_group_liked_by_this_group'] > 0 ? $result['total_group_liked_by_this_group'] - 1 : 0;  
                }
                $likeGroupsCounter++;
            }
        }
    ...
    public function memberAction()
    {
        ...
        if ($fullMembers->getTotalItemCount() > 0 && ($viewer->getIdentity() && ($group->isOwner($viewer))) && $waiting) {
...
//         $member->userImage($member->getIdentity(),'thumb.profile');
            if ($group->isOwner($viewer) && !$group->isOwner($member)) {
                $optionCounter = 0;
				...
				$row = Engine_Api::_()->getDbTable('membership','sesgroup')->getRequestInfo(array('resource_id' => $group->group_id,'user_id' => $member->user_id));
					if($row->answerques1){
						$result['members'][$counterLoop]['question_1'] = $group->questitle1;
						$result['members'][$counterLoop]['answer_1'] = $row->answerques1;
					}
					if($row->answerques2){
						$result['members'][$counterLoop]['question_2'] = $group->questitle2;
						$result['members'][$counterLoop]['answer_2'] = $row->answerques2;
					}
					if($row->answerques3){
						$result['members'][$counterLoop]['question_3'] = $group->questitle3;
						$result['members'][$counterLoop]['answer_3'] = $row->answerques3;
					}
					if($row->answerques4){
						$result['members'][$counterLoop]['question_4'] = $group->questitle4;
						$result['members'][$counterLoop]['answer_4'] = $row->answerques4;
					}
					if($row->answerques5){
						$result['members'][$counterLoop]['question_5'] = $group->questitle5;
						$result['members'][$counterLoop]['answer_5'] = $row->answerques5;
					}
    ...
    public function announcementAction()
    {
        ...
        $paginator = Engine_Api::_()->getDbTable('announcements', 'sesgroup')->getGroupAnnouncementPaginator(array('group_id' => $group->group_id));
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        $result = array();
    ...
    public function servicesAction()
    {
        ...
        $paginator = Engine_Api::_()->getDbTable('services', 'sesgroup')->getServicePaginator(array('group_id' => $subject->getIdentity(), 'widgettype' => 'widget'));
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        //Manage Apps Check
    ...
    public function mapAction()
    {
        ...
        $group = Engine_Api::_()->core()->getSubject();
        $paginator = Engine_Api::_()->getDbTable('locations', 'sesgroup')->getGroupLocationPaginator(array('group_id' => $group->group_id));
        $paginator->setItemCountPerPage(5);
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        ...
        foreach ($paginator as $location) {
            ...
            $locationPhotos = Engine_Api::_()->getDbTable('locationphotos', 'sesgroup')->getLocationPhotos(array('group_id' => $group->group_id, 'location_id' => $location->location_id));
            $photosCounter = 0;
    ...
    public function associatedAction()
    {
        ...
        $group = Engine_Api::_()->core()->getSubject();
        $params['parent_id'] = $group->group_id;


        $paginator = $paginator = Engine_Api::_()->getDbTable('groups', 'sesgroup')
            ->getGroupPaginator($params);
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        $result['groups'] = $this->getGroups($paginator);
    ...
    public function albumviewAction()
    {
        ...
        if (Engine_Api::_()->core()->hasSubject()) {
            $group = Engine_Api::_()->core()->getSubject();
            $album = Engine_Api::_()->getItem('sesgroup_album', $albumid);
        } else {
            $album = Engine_Api::_()->getItem('sesgroup_album', $albumid);
        ...
        if ($viewer->getIdentity() > 0) {
            $canEdit = $editGroupRolePermission = Engine_Api::_()->sesgroup()->getGroupRolePermission($group->getIdentity(), 'allow_plugin_content', 'edit');
            $editGroupRolePermission = Engine_Api::_()->sesgroup()->getGroupRolePermission($group->getIdentity(), 'allow_plugin_content', 'edit');
            $canEditMemberLevelPermission = $editGroupRolePermission ? $editGroupRolePermission : $group->authorization()->isAllowed($viewer, 'edit');
            $deleteGroupRolePermission = Engine_Api::_()->sesgroup()->getGroupRolePermission($group->getIdentity(), 'allow_plugin_content', 'delete');
            $canDeleteMemberLevelPermission = $deleteGroupRolePermission ? $deleteGroupRolePermission : $group->authorization()->isAllowed($viewer, 'delete');
        }
        ...
        $paginator->setCurrentPageNumber('group_number', 1);
        ...
        foreach ($paginator as $photo) {
            ...
            $albumLikeStatus = Engine_Api::_()->sesgroup()->getLikeStatusGroup($photo->photo_id, 'sesgroup_photo');
        ...
        if(!$albumArtCover){
          $albumImage = Engine_Api::_()->sesgroup()->getAlbumPhoto($album->getIdentity(), 0, 3);
    ...
    public function editalbumAction()
    {
        ...
        // Make form
        $form = new Sesgroup_Form_Album_Edit();
        if ($this->_getParam('getForm')) {
            $formFields = Engine_Api::_()->getApi('FormFields', 'sesapi')->generateFormFields($form);
            $this->generateFormFields($formFields, array('resources_type' => 'sesgroup_group'));
        }
    ...
    public function deletealbumAction()
    {
        ...
        $form = new Sesgroup_Form_Album_Delete();
        if ($this->_getParam('getForm')) {
            $formFields = Engine_Api::_()->getApi('FormFields', 'sesapi')->generateFormFields($form);
            $this->generateFormFields($formFields, array('resources_type' => 'sesgroup_group'));
        }
    ...
    public function likeasgroupAction()
    {
        ...
        $group_ids = $this->_getParam('group_ids');
        if($group_ids){
            ...
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '0', 'error_message' => '', 'result' => array('success_message' => 'Groups liked succuessfully.')));
        }
        ...
        if ($group_id) {
            ...
            $row->user_id = $viewer->getIdentity();
            $row->save();
            $message = $this->view->translate('%s has been liked as Your group.', $group->getTitle());
        ...
        $myGroups = ($table->fetchAll($selelct));
        if (count($myGroups)) {
            $result = array();
            $result['title'] = 'Like ' . $group->getTitle() . ' as Your Group';
            $result['description'] = "Likes will show up on your Group's timeline. Which Group do you want to like " . $group->getTitle() . " as?";
            $result['image'] = Engine_Api::_()->sesapi()->getPhotoUrls($group, '', "");
            $counter = 0;
            foreach ($myGroups as $mygroup) {
                $group = Engine_Api::_()->getItem('sesgroup_group', $mygroup->group_id);
                if (!$group)
                    continue;
                $groupdata[$counter]['group_id'] = $group->group_id;
                $groupdata[$counter]['group_title'] = $group->getTitle();
                $counter++;
            }
            $result['group'] = $groupdata;
        }
    ...
    public function unlikeasgroupAction()
    {
        ...
        if($group_ids){
            ...
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '0', 'error_message' => '', 'result' => array('success_message' => 'Groups succuessfully removed.')));
        }
        if ($group_id) {
            $table = Engine_Api::_()->getDbTable('likegroups', 'sesgroup');
            $select = $table->select()->where('group_id =?', $group_id)->where('like_group_id =?', $group->getIdentity());
        ...
        if (count($myGroups)) {
            $result = array();
            $result['title'] = "Remove " . $group->getTitle() . " from my Group's favorites";
            $result['description'] = "For which group would you like to remove  " . $group->getTitle() . " from favorites?";
            $result['image'] = Engine_Api::_()->sesapi()->getPhotoUrls($group, '', "");
            $counter = 0;
            foreach ($myGroups as $mygroup) {
                $group = Engine_Api::_()->getItem('sesgroup_group', $mygroup->group_id);
    ...
    public function leaveAction()
    {
        ...
        if (1) {
            ...
            try {
                ...
                $db->commit();
                $result['message'] = $this->view->translate('You have successfully left this group.');
    ...
    public function inviteAction()
    {
        ...
        if ($this->_getParam('getForm')) {
            if ($form->getElement('all'))
                $form->getElement('all')->setName('sesgroup_choose_all');
...
//        }
      ...
      public function editoverviewAction(){
        ...
        if ($this->_getParam('getForm')) {
            ...
            $formFields[0]['label'] = "Group Overview";
        ...
        $subject->save();
        Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '0', 'error_message' => '', 'result' => array('message' => $this->view->translate('Group overview saved successfully.'))));
    }
    ...
    public function deleteAction()
    {
        ...
        if (!$sesgroup)
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('This group does not exist.'), 'result' => array()));
...
//    $this->_helper->layout->setLayout('default-simple');
        ...
        $form = new Sesgroup_Form_Delete();


        if (!$sesgroup) {
            $status['status'] = false;
            $error = Zend_Registry::get('Zend_Translate')->_("Group entry doesn't exist or not authorized to delete");
        ...
        try {
            ...
            $db->commit();
            $status['status'] = true;
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '0', 'error_message' => '', 'result' => array('success_message' => $this->view->translate('You have successfully deleted to this group.'), $status)));
    ...
    public function addmorephotosAction()
    {
        ...
        $photoTable = Engine_Api::_()->getDbTable('photos', 'sesgroup');
        ...
        $counter = 0;
        // Process
        $db = Engine_Api::_()->getDbtable('photos', 'sesgroup')->getAdapter();
        $db->beginTransaction();
        try {
    ...
    public function uploadphotoAction()
    {
        ...
        $file = array('main' => $this->getBaseUrl(true,$group->getPhotoUrl()));
...
protected function setPhoto($photo, $id) {
    ...
    try {
      ...
      if ($e->getCode() == Storage_Model_DbTable_Files::SPACE_LIMIT_REACHED_CODE) {
        throw new Sesgroupvideo_Model_Exception($e->getMessage(), $e->getCode());
      } else {
        throw $e;
      }
    }
    ...
    public function removephotoAction()
    {
        ...
        if (!$group)
            $group = Engine_Api::_()->getItem('sesgroup_group', $this->_getparam('group_id', null));
        ...
        $file = array('main' => $group->getPhotoUrl());
...
//    die;
    ...
    public function uploadcoverAction()
    {
      ...
      if($detail_id) {
        ...
        $file['main'] = $this->getBaseUrl(true,$group->getCoverPhotoUrl());
  ...
  public function editDescriptionAction() {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $photo = Engine_Api::_()->core()->getSubject('sesgroup_photo');
    if ($status && !$error) {
    ...
    public function lightboxAction()
    {
        ...
        $group_id = $this->_getparam('group_id', $photo->group_id);
        ...
        if ($album_id && null !== ($album = Engine_Api::_()->getItem('sesgroup_album', $album_id))) {
        } else {
...
//        if (!$this->_helper->requireSubject('sesgroup_photo')->isValid())
//          Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=>'permission_error', 'result' => array()));
        ...
        if ($viewer->getIdentity() > 0) {
            ...
            $canEdit = Engine_Api::_()->sesgroup()->getGroupRolePermission($group->getIdentity(), 'allow_plugin_content', 'edit');
            if ($canEdit) {
            ...
            $can_delete = Engine_Api::_()->sesgroup()->getGroupRolePermission($group->getIdentity(), 'allow_plugin_content', 'delete');
            if ($canEdit) {
	...
	function getVideos($paginator,$checkProfile){
		...
		foreach ($paginator as $item){
			...
			if($checkProfile['group_id'] > 0){
				$viewer = Engine_Api::_()->user()->getViewer();
				$viewerId = $viewer->getIdentity();
				$levelId = ($viewerId) ? $viewer->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
				if($viewerId>0) {
					$can_edit = Engine_Api::_()->authorization()->getPermission($levelId, 'groupvideo', 'edit');
					$can_delete = Engine_Api::_()->authorization()->getPermission($levelId, 'groupvideo', 'delete');
				...
				$group = Engine_Api::_()->getItem('sesgroup_group',$checkProfile['group_id']);
				$result[$counter]['group_title'] = $group->getTitle();
			...
			if($item->code && $item->type == 'iframely'){
			 ...
			 if($item->getType() == 'groupvideo'){
				$allowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo.video.rating',1);
				$allowShowPreviousRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo.ratevideo.show',1);
			if($allowRating == 0){
			...
			if( $item->duration >= 3600 ) {
			  ...
			  if($viewerId != 0 ){
				  ...
				  $LikeStatus = Engine_Api::_()->sesgroupvideo()->getLikeStatusVideo($item->video_id,'groupvideo');
			  ...
			  $itemtype = 'groupvideo';
              $getId = 'video_id';
			  if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo.enable.favourite', 1) && isset($item->favourite_count)){
				  $favStatus = Engine_Api::_()->getDbtable('favourites', 'sesgroupvideo')->isFavourite(array('resource_type'=>$itemtype,'resource_id'=>$item->$getId));
				  ...
				  $result[$counter]['can_favourite'] = false;
				  $result[$counter]['fovourite_show'] = false;
			  }
			  $owner = $item->getOwner();
			  $result[$counter]['owner_title'] = $ownerTitle = $owner->getTitle();
			  if($item->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo_enable_location', 1)){
	...
	public function browsevideoAction(){
		...
		$value['search'] = 1;
		$paginator = Engine_Api::_()->getDbTable('videos', 'sesgroupvideo')->getVideo($value,$paginator = true);
		$paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
		$checkProfile['group_id'] = null;
	...
	public function profileVideosAction(){
		...
		$search = $this->_getParam('search', null);
		$paginator = Engine_Api::_()->getDbTable('videos', 'sesgroupvideo')->getVideo(array('parent_id' => $group->getIdentity(), 'parent_type' =>$group->getType(), 'text' => $search, 'sort' => $sort));
		$paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
		$checkProfile['group_id'] = $group->getIdentity(); 
		$allowVideo  = Engine_Api::_()->authorization()->isAllowed('sesgroup_group', $viewer, 'video');
		$canUpload = $canUpload = $group->authorization()->isAllowed($viewer, 'video');
    ...
    $parent_type = $parent_type = 'sesgroup_group';
    if( $parent_id &&  $parent_type)
        $parentItem = Engine_Api::_()->getItem($parent_type, $parent_id);
    if(!$parentItem)
        Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('Data not found.'), 'result' => array()));
    $paginator = Engine_Api::_()->getApi('core', 'sesgroupvideo')->getVideosPaginator($values);
    $quota = $quota = Engine_Api::_()->authorization()->getPermission($levelId, 'groupvideo', 'max');
    ...
    //Create form
    $form = $form = new Sesgroupvideo_Form_Video();
    ...
    $insert_action = false;
    $db = Engine_Api::_()->getDbtable('videos', 'sesgroupvideo')->getAdapter();
    $db->beginTransaction();
    try {
		$viewer = Engine_Api::_()->user()->getViewer();
		$isApproveUploadOption = Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('groupvideo', $viewer, 'video_approve');
		$approveUploadOption = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('groupvideo', $viewer, 'video_approve_type');
		$approve = 1;
        ...
        if(isset($_FILES['photo_id']['name']) && $_FILES['photo_id']['name'] != '' && $values['type'] != 3 ){
          ...
          try {
            ...
            $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
            $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $video->video_id . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","groupvideo")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
          }
    ...
    try {
      if ($approve) {
        ...
        if($parent_id && $parent_type) {
		      $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($owner, $parentItem, 'sesgroup_group_editeventvideo');
	        if ($action != null) {
   ...
   public function uploadVideoAction() {
    ...
    try {
      ...
      $video = Engine_Api::_()->sesgroupvideo()->createVideo($params, $_FILES['Filedata'], $values);
      $result['status'] = true;
    ...
    public function viewVideoAction(){
        ...
        if (Engine_Api::_()->core()->hasSubject()){
            $video = Engine_Api::_()->core()->getSubject('groupvideo');
        }
        else if($videoid)
        {
            $video = Engine_Api::_()->getItem('groupvideo', $videoid);
        }else{
            Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=>'parameter_missing', 'result' => array()));
        }
		...
		$favStatus = Engine_Api::_()->getDbtable('favourites', 'sesgroupvideo')->isFavourite(array('resource_type'=>$video->getType(),'resource_id'=>$video->getIdentity()));
		$LikeStatus = Engine_Api::_()->sesgroupvideo()->getLikeStatusVideo($video->getIdentity(),$video->getType());
		...
		$viewer = Engine_Api::_()->user()->getViewer();
		 if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('seslock'))) {
			...
			$viewerId = $viewer->getIdentity();
			$levelId = ($viewerId) ? $viewer->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
			  if (!Engine_Api::_()->authorization()->getPermission($levelId, 'groupvideo', 'locked') && $video->is_locked) {
		 ...
		 $videoTags = $video->tags()->getTagMaps();
			...
			if ($video->status == 1) {
			  ...
			  $result['embedded'] = $embedded = $video->getRichContent(true,array(),'',$autoPlay);
			}
			...
			if ($video->type == 3 && $video->status == 1) {
			 ...
			 $result['allowShowRating']  = $allowShowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo.ratevideo.show', 1);
			$result['allowRating'] = $allowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo.video.rating', 1);
			...
			if ($showRating) {
			  $result['canRate'] = $canRate = Engine_Api::_()->authorization()->isAllowed('groupvideo', $viewer, 'rating');
			   $result['allowRateAgain'] = $allowRateAgain = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo.ratevideo.again', 1);
			  $result['allowRateOwn'] = $allowRateOwn = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo.ratevideo.own', 1);
			 ...
			 $result['allowRating']  = $allowRating;
			  $result['rating_type']  = $rating_type = 'groupvideo';
			  $result['rating_count'] = $rating_count = Engine_Api::_()->getDbTable('ratings', 'sesgroupvideo')->ratingCount($video->getIdentity(), $rating_type);
		  $result['rated'] = $rated = Engine_Api::_()->getDbTable('ratings', 'sesgroupvideo')->checkRated($video->getIdentity(), $viewer->getIdentity(), $rating_type);
		  $rating_sum = Engine_Api::_()->getDbTable('ratings', 'sesgroupvideo')->getSumRating($video->getIdentity(), $rating_type);
		  if ($rating_count != 0) {
		...
		$result['can_delete'] = 0;
			$videoCounter = 0;
		if($viewer->getIdentity() != 0){
			$resourceItem = Engine_Api::_()->getItem('sesgroup_group', $video->parent_id);
			...
			if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo.enable.report',1) && $viewer_id != $album->owner_id){
    ...
    public function geturlAction(){
        ...
        $video = Engine_Api::_()->getItem('groupvideo', $video_id);
        //$embe = $video->code;
	...
	if(!$rating || !$resource_id || !$resource_type)
		 Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'1','error_message'=>$this->view->translate('parameter_missing'), 'result' => array()));
    $table = Engine_Api::_()->getDbtable('ratings', 'sesgroupvideo');
    $db = $table->getAdapter();
    $db->beginTransaction();
    try {
      Engine_Api::_()->getDbtable('ratings', 'sesgroupvideo')->setRating($resource_id, $user_id, $rating, $resource_type);
      if ($resource_type && $resource_type == 'groupvideo')
        $item = Engine_Api::_()->getItem('groupvideo', $resource_id);
      $item->rating = Engine_Api::_()->getDbtable('ratings', 'sesgroupvideo')->getRating($item->getIdentity(), $resource_type);
      $item->save();
      if ($resource_type == 'groupvideo') {
    ...
    $total = Engine_Api::_()->getDbtable('ratings', 'sesgroupvideo')->ratingCount($item->getIdentity(), $resource_type);
    $rating_sum = Engine_Api::_()->getDbtable('ratings', 'sesgroupvideo')->getSumRating($item->getIdentity(), $resource_type);
    $data = array();
    ...
    if (!$this->_helper->requireUser()->isValid())
        Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => 'user_not_autheticate', 'result' => array()));
    $viewer = Engine_Api::_()->user()->getViewer();
    $video = Engine_Api::_()->getItem('groupvideo', $this->_getParam('video_id'));
    // Render
    $this->_helper->content->setEnabled();
    $this->view->parentItem = $resourceItem = Engine_Api::_()->getItem('sesgroup_group', $video->parent_id);
    $canEditParent = $resourceItem->authorization()->isAllowed($viewer, 'edit');
    $canEdit = $video->authorization()->isAllowed($viewer, 'edit');
    if(!$canEdit && !$canEditParent)
	   Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => 'user_not_autheticate', 'result' => array()));
    $form = new Sesgroupvideo_Form_Edit();

	$latLng = Engine_Api::_()->getDbTable('locations', 'sesbasic')->getLocationData('groupvideo',$video->video_id);
	//if($latLng){
		if($form->getElement('lat'))
			$form->removeElement('lat');
   ...
   // }
    ...
    try {
      ...
      if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
        $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
        $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $this->_getParam('video_id') . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","groupvideo")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }
      $video->setFromArray($values);
      $video->save();
      // Add fields
  ...
  //item liked as per item tye given
    ...
    $tableLike = Engine_Api::_()->getDbtable('likes', 'core');
    $tableMainLike = $tableLike->info('name');
    $itemTable = Engine_Api::_()->getDbtable($dbTable, 'sesgroupvideo');
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $Fav = Engine_Api::_()->getDbTable('favourites', 'sesgroupvideo')->getItemfav($type, $item_id);
    $favItem = Engine_Api::_()->getDbtable($dbTable, 'sesgroupvideo');
    if (count($Fav) > 0) {
      //delete
      $db = $Fav->getTable()->getAdapter();
      $db->beginTransaction();
      ...
      //update
      $db = Engine_Api::_()->getDbTable('favourites', 'sesgroupvideo')->getAdapter();
      $db->beginTransaction();
      try {
        $fav = Engine_Api::_()->getDbTable('favourites', 'sesgroupvideo')->createRow();
      ...
      //send notification and activity feed work.
      $item = Engine_Api::_()->getItem(@$type, @$item_id);
      if ($this->_getParam('type') != 'sesgroupvideo_artist') {
        $subject = $item;
        $owner = $subject->getOwner();
        if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
          $activityTable = Engine_Api::_()->getDbtable('actions', 'activity');
	...
	public function browsealbumAction(){
        ...
        $value['group_id'] = $this->_getParam('group_id',0);
        $paginator = Engine_Api::_()->getDbTable('albums', 'sesgroup')->getAlbumSelect($value);
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        $albumCounter = 0;
        foreach ($paginator as $item) {
            ...
            $result['albums'][$albumCounter]['images'] = Engine_Api::_()->sesapi()->getPhotoUrls($item, '', "") ?
                ...
                $result['members'][$counterLoop]['owner_photo'] =
                    $this->getBaseUrl(true, '/application/modules/User/externals/images/nophoto_user_thumb_icon.png');
            $result['albums'][$albumCounter]['user_title'] = $ownertitle;
            $albumLikeStatus = Engine_Api::_()->sesgroup()->getLikeStatus($item->getIdentity(), $item->getType());
            $albumFavStatus = Engine_Api::_()->getDbTable('favourites', 'sesgroup')->isFavourite(array('resource_type' => 'album', 'resource_id' => $item->album_id));
            if ($albumLikeStatus) {
    ...
    public function albumsearchformAction(){
        ...
        $searchForm = new Sesgroup_Form_AlbumSearch(array('searchTitle' => $this->_getParam('search_title', 'yes'), 'browseBy' => $this->_getParam('browse_by', 'yes'), 'searchFor' => $search_for, 'FriendsSearch' => $this->_getParam('friend_show', 'yes'), 'defaultSearchtype' => $default_search_type));
        ...
        $searchForm->removeElement('loading-img-sesgroup');
        if ($this->_getParam('getForm')) {
	...
	function getButtonMenus($groups){
        ...
        if ($groups->group_contact_email || $groups->group_contact_phone || $groups->group_contact_website) {
            if ($groups->group_contact_email) {
                ...
                $result[$i]['value'] = $groups->group_contact_email;
                $i++;
            ...
            if ($groups->group_contact_phone) {
                $result[$i]['name'] = 'phone';
                $result[$i]['label'] = 'Call';
                $result[$i]['value'] = $groups->group_contact_phone;
                $i++;
            }
            if ($groups->group_contact_website) {
                ...
                $result[$i]['value'] = $groups->group_contact_website;
                $i++;
            }
        }
        ...
        if ($canJoin) {
          ...
          if (null === $row) {
              if ($groups->membership()->isResourceApprovalRequired()) {
                  ...
                  $result[$i]['name'] = 'join';
                  $result[$i]['label'] = 'Join Group';
                  $i++;
              }
          } else if ($row->active) {
              if (!$groups->isOwner($viewer)) {
                  $result[$i]['label'] = 'Leave Group';
                  $result[$i]['name'] = 'leave';
                  $i++;
              }
          } else if (!$row->resource_approved && $row->user_approved) {
    ...
    public function createalbumAction(){
        ...
        $viewer = Engine_Api::_()->user()->getViewer();
        $values['user_id'] = $viewer->getIdentity();
        $current_count = Engine_Api::_()->getDbTable('albums', 'sesgroup')->getUserAlbumCount($values);
        $quota = $quota = 0;
        // Get form
        $form = new Sesgroup_Form_Album();
        ...
        $db = Engine_Api::_()->getItemTable('sesgroup_album')->getAdapter();
        $db->beginTransaction();
        try {
            $photoTable = Engine_Api::_()->getDbTable('photos', 'sesgroup');
    ...
    public function acceptAction()
    {
        ...
        try {
            ...
            $db->commit();
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '0', 'error_message' => '', 'result' => array('message' => $this->view->translate('You have accepted the invite to the group'),'menus'=>$this->getButtonMenus($subject))));
        } catch (Exception $e) {
            $db->rollBack();
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $e->getMessage(), 'result' => array()));
        }
    ...
    public function removeAction()
    {
        ...
        try {
            ...
            $db->commit();
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '', 'error_message' => '', 'result' => array('message' => $this->view->translate('The selected member has been removed from this group.'),'menus'=>$this->getButtonMenus($subject))));
        } catch (Exception $e) {
            $db->rollBack();
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $e->getMessage(), 'result' => array()));
        }
    ...
    public function approveAction()
    {
        ...
        try {
            ...
            $db->commit();
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '0', 'error_message' => '', 'result' => array('message' => $this->view->translate('Group request approved'),'menus'=>$this->getButtonMenus($subject))));
        } catch (Exception $e) {
            $db->rollBack();
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $e->getMessage(), 'result' => array()));
        }
