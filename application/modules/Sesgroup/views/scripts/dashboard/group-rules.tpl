 ...
 * @version    $Id: group-rules.tpl  2018-04-23 00:00:00 SocialEngineSolutions $
...
<div class="sesgroup_dashboard_content_header">
  <h3><?php echo $this->translate('Manage Rules') ?></h3>
    <p><?php echo $this->translate('Here, you can create rules to show them at the Group Joining. You can also edit and delete the rules listed on this Group.') ?></p>
</div>
<div class="sesgroup_dashboard_content_btns">
  <a href="<?php echo $this->url(array('group_id' => $this->group->custom_url,'action'=>'add-rule'), 'sesgroup_dashboard', true); ?>" class="sessmoothbox sesbasic_button"><i class="fa fa-file" aria-hidden="true"></i><span><?php echo $this->translate("Add New Rule");?></span></a>
</div>  
<?php echo $this->paginationControl($this->paginator); ?>
<?php if( count($this->paginator) ): ?>
  <div class="sesgroup_dashboard_table">
    <table class='admin_table'>
      <thead>
        <tr>
          <th style="width: 1%;" class="admin_table_short"><input type='checkbox' class='checkbox'></th>
          <th style="width: 1%;"><a href="javascript:void(0);" onclick="javascript:changeOrder('rule_id', '<?php if($this->orderby == 'rule_id') echo "DESC"; else echo "ASC"; ?>');">
      ...
      <tbody>
        ...
        <tr>
          <td><input type='checkbox' class='checkbox' value="<?php echo $item->rule_id?>"></td>
          <td><?php echo $item->rule_id ?></td>
          <td class="admin_table_bold"><?php echo $item->title ?></td>
          <td class="admin_table_options">
            <?php echo $this->htmlLink(
              array('action' => 'edit-rule', 'id' => $item->getIdentity(), 'reset' => false), $this->translate(''),array('class' => 'sessmoothbox sesbasic_button fa sesbasic_icon_edit')) ?>
            <?php echo $this->htmlLink(array('action' => 'delete-rule', 'id' => $item->getIdentity(), 'reset' => false),$this->translate(''),array('class' => 'sessmoothbox sesbasic_button fa sesbasic_icon_delete')) ?>
          </td>
        </tr>
        <?php endforeach; ?>
      </tbody>
...
<?php else:?>
  <div class="tip">
    <span>
      <?php echo $this->translate("There are currently no rules.") ?>
    </span>
  </div>
<?php endif; ?>
<?php if(!$this->is_ajax) { ?>
