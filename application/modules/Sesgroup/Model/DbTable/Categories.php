 ...
 */
class Sesgroup_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Sesgroup_Model_Category';
  ...
  public function getCategory($params = array())
  {
    ...
    $groupTableName = Engine_Api::_()->getDbTable('groups', 'sesgroup')->info('name');
    $category_select = $category_select->setIntegrityCheck(false);
    $category_select = $category_select->joinLeft($groupTableName, "$groupTableName.category_id=$tableName.category_id AND (`is_approved` = 1 AND `" . $groupTableName . "`.`draft` != 0)", array("total_group_categories" => "COUNT($groupTableName.group_id)"));
    $category_select = $category_select->group("$tableName.category_id");
    if (isset($params['criteria']) && $params['criteria'] == 'most_group')
      $category_select->order('total_group_categories DESC');
    if (isset($params['criteria']) && $params['criteria'] != 'most_group')
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('sesgroup_category');
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countGroups'])) {
      $groupTable = Engine_Api::_()->getDbTable('groups', 'sesgroup')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($groupTable, "$groupTable.subcat_id=$tableName.category_id", array("total_groups_categories" => "COUNT($groupTable.photo_id)"));
      $category_select = $category_select->group("$tableName.category_id");
      $category_select->order('total_groups_categories DESC');
    }
    $category_select = $category_select->order('order DESC');
    if (isset($params['limit']))
      $category_select->limit($params['limit']);
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countGroups'])) {
      $groupTable = Engine_Api::_()->getDbTable('groups', 'sesgroup')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($groupTable, "$groupTable.subsubcat_id=$tableName.category_id", array("total_groups_categories" => "COUNT($groupTable.group_id)"));
