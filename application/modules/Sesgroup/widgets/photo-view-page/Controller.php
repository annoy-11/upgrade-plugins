...
class Sesgroup_Widget_PhotoViewPageController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
		...
		if(Engine_Api::_()->core()->hasSubject('sesgroup_photo') && !$is_ajax)
      $photo = Engine_Api::_()->core()->getSubject('sesgroup_photo');
		else if(isset($_POST['photo_id'])) {
      $photo = Engine_Api::_()->getItem('sesgroup_photo',$_POST['photo_id']);
      Engine_Api::_()->core()->setSubject($photo); 
      $photo = Engine_Api::_()->core()->getSubject();
		} else
      return $this->setNoRender();
		
		$this->view->group =	$group = Engine_Api::_()->getItem('sesgroup_group', $photo->group_id);
      ...
      $this->view->canEdit = $canEdit = $editGroupRolePermission ? $editGroupRolePermission : $group->authorization()->isAllowed($viewer, 'edit');
      $this->view->canComment = $canComment = $group->authorization()->isAllowed($viewer, 'comment');
      $this->view->canDelete = $canDelete = $deleteGroupRolePermission ? $deleteGroupRolePermission : $group->authorization()->isAllowed($viewer, 'delete');
      $this->view->canTag = $canTag = $group->authorization()->isAllowed($viewer, 'tag');
      $this->view->canCommentMemberLevelPermission = Engine_Api::_()->authorization()->getPermission($viewer, 'sesgroup_group', 'comment');
		}
