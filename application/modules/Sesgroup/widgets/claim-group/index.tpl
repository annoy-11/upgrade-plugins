...
<div class="sesgroup_profile_tabs_top claim_request active sesbasic_clearfix">
...
<?php if($checkClaimRequest):?>
	<div class="sesgroup_profile_tabs_top claim_new_group sesbasic_clearfix">
...
<script type="text/javascript">
	en4.core.runonce.add(function() {
		var contentAutocomplete = new Autocompleter.Request.JSON('title', "<?php echo $this->url(array('module' => 'sesgroup', 'controller' => 'index', 'action' => 'get-groups'), 'default', true) ?>", {
			'postVar': 'text',
			'minLength': 1,
			'selectMode': 'pick',
	...
	function validateForm(){
		...
		counter = 0;
		sesJqueryObject('#sesgroup_claim_create input, #sesgroup_claim_create select,#sesgroup_claim_create checkbox,#sesgroup_claim_create textarea,#sesgroup_claim_create radio').each(
			function(index){
	...
	en4.core.runonce.add(function() {
		sesJqueryObject('#sesgroup_claim_create').submit(function(e){
	...
	function sendDataToServer(object){
		...
		sesJqueryObject.ajax({
			type:'POST',
			url: en4.core.baseUrl + "widget/index/mod/sesgroup/name/claim-group/",
			data:formData,
			cache:false,
			contentType: false,
			processData: false,
			success:function(data){
				...
				window.location.href = '<?php echo $this->url(array("action"=>"claim-requests"),"sesgroup_general",true); ?>';
			},
			error: function(data){
			}
		});
