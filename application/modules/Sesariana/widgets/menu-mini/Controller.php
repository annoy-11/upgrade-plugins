...
class Sesariana_Widget_MenuMiniController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if ($viewer->getIdentity()) {
      ...
      $this->view->messageCount = Engine_Api::_()->getApi('message', 'sesariana')->getMessagesUnreadCount($viewer);
      $this->view->requestCount = Engine_Api::_()->getDbtable('notifications', 'sesbasic')->hasNotifications($viewer, 'friend');
    }
    $this->view->poupup = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesariana.popupsign', 1);
    ...
    $this->view->loginsignup_logo = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesariana.loginsignuplogo', '');
    ...
    $defaultoptn = array('search','miniMenu','mainMenu','logo', 'socialshare');
		$loggedinHeaderCondition = $settings->getSetting('sesariana.header.loggedin.options', $defaultoptn);
		$nonloggedinHeaderCondition = $settings->getSetting('sesariana.header.nonloggedin.options',$defaultoptn);
    ...
    if($viewer_id != 0) {
		...
		$this->view->loginsignupbgimage = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesariana.loginsignupbgimage', '');
