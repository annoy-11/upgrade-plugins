...
?>
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesariana/externals/styles/style_login.css'); ?>
<div class="sesariana_login_main sesbasic_bxs">
  <?php if($this->showlogo): ?>
	<div class="sesariana_site_logo">    
    	<?php echo $this->content()->renderWidget('sesariana.menu-logo'); ?>
    </div>
    <?php endif; ?>
    <div class="sesariana_login_form">
	<div class="sesariana_login_tittle">
    	<h3>Sign In</h3>
    </div>
  <?php $settings = Engine_Api::_()->getApi('settings', 'core'); ?>
	<?php if(Engine_Api::_()->getDbTable('modules','core')->isModuleEnabled('sessociallogin')):?>
        <div class="sesariana_social_login_btns">
        <?php  echo $this->partial('_socialLoginIcons.tpl','sessociallogin',array()); ?>
      </div>
      <?php else: ?>
  <div class="sesariana_ligin_with_social">
    <?php
...
if(!sesJqueryObject('#twitter_href_lnk').length && !sesJqueryObject('#fb_href_lnk').length)
  sesJqueryObject('.sesariana_ligin_with_social').remove();
