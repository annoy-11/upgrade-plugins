 ...
 * @version    $Id: AdminManageController.php 2016-11-22 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesariana_AdminManageController extends Core_Controller_Action_Admin {
  ...
  public function miniMenuIconsAction() {
    ...
    $this->view->form = $form = new Sesariana_Form_Admin_MiniMenuIcons();
  ...
  public function footerSettingsAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesariana_admin_main', array(), 'sesariana_admin_main_menusfooter');

    $this->view->form = $form = new Sesariana_Form_Admin_FooterSettings();

    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      $values = $form->getValues();
      foreach ($values as $key => $value) {
				if ($key == 'sesariana_footer_background_image') {
							if(empty($value))
  ...
  public function landingPageSettingsAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesariana_admin_main', array(), 'sesariana_admin_main_menuslanding');
    $this->view->modules = Engine_Api::_()->sesariana()->getModulesEnable();
    if (!empty($_POST)) {
      $values = $_POST;
...
//           continue;
        ...
        if($key == 'sesariana_banner_content' && is_array($value))
          $value = implode(',',$value);
        if($key == 'sesariana_feature_content'){
  ...
  public function headerSettingsAction() {
    ...
    $this->view->form = $form = new Sesariana_Form_Admin_HeaderSettings();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      unset($values['minimenu_frrequest_icons']);
      foreach ($values as $key => $value) {
        if ($key == 'sesariana_header_design' || $key == 'sesariana_sidepanel_effect' || $key == 'sesariana_sidepanel_showhide') {
	        Engine_Api::_()->sesariana()->readWriteXML($key, $value);
		 			Engine_Api::_()->getApi('settings', 'core')->setSetting($key, $value);
        } else {
          if ($key == 'sesariana_header_loggedin_options' || $key == 'sesariana_header_nonloggedin_options' || $key == 'sesariana_header_transparent') {
            if(Engine_Api::_()->getApi('settings', 'core')->hasSetting($key)){
              Engine_Api::_()->getApi('settings', 'core')->removeSetting($key);
            }
          }
					if($key == 'sesariana_menuinformation_img') {
						if(empty($value)) {
							$value = 'public/admin/blank.png';
            }
					}
					if($key == 'sesariana_menu_img') {
						if(empty($value))
							$value = 'public/admin/blank.png';
					}
					//Notification Icon
  ...
  public function setPhoto($photo) {
    ...
    $photo_params = array(
        'parent_id' => 1,
        'parent_type' => "sesariana_minimenu_icons",
    );
  ...
  public function uploadPhotoAction() {
    ...
    $admin_file = APPLICATION_PATH . '/public/adminsesariana';
    ...
    $this->view->photo_url = 'http://' . $_SERVER['HTTP_HOST'] . '/' . Zend_Controller_Front::getInstance()->getBaseUrl() . '/public/adminsesariana/' . $info['name'];
  }
  ...
  public function uploadIconAction() {
    ...
    $this->view->form = new Sesariana_Form_Admin_Icon();

    if ($this->getRequest()->isPost()) {
      if (isset($_FILES['photo']) && is_uploaded_file($_FILES['photo']['tmp_name'])) {
        
        $photoFile = Engine_Api::_()->sesariana()->setPhoto($_FILES['photo'], $id);
      ...
      if ($menuType == 'main')
        $redirectUrl = Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'sesariana', 'controller' => 'admin-manage', 'action' => 'index'), 'default', true);
      else
        $redirectUrl = Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'sesariana', 'controller' => 'admin-manage', 'action' => 'footer-menu'), 'default', true);
  ...
  public function deletePhotoAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $mainPhoto->delete();
        $slideImage = Engine_Api::_()->getItem('sesariana_slideimage', $id);
        $slideImage->delete();
        $db->commit();
      } catch (Exception $e) {
        $db->rollBack();
  ...
  //For write constant in xml file during upgradation
    ...
    if(empty($bodyFontFamily)) {
      Engine_Api::_()->sesariana()->readWriteXML('sesariana_body_fontfamily', 'Arial, Helvetica, sans-serif');
      Engine_Api::_()->sesariana()->readWriteXML('sesariana_body_fontsize', '13px');
    }
    $headingFontFamily = Engine_Api::_()->sesariana()->getContantValueXML('sesariana_heading_fontfamily');
    if(empty($headingFontFamily)) {
      Engine_Api::_()->sesariana()->readWriteXML('sesariana_heading_fontfamily', 'Arial, Helvetica, sans-serif');
      Engine_Api::_()->sesariana()->readWriteXML('sesariana_heading_fontsize', '17px');
    }
    $mainmenuFontFamily = Engine_Api::_()->sesariana()->getContantValueXML('sesariana_mainmenu_fontfamily');
    if(empty($mainmenuFontFamily)) {
      Engine_Api::_()->sesariana()->readWriteXML('sesariana_mainmenu_fontfamily', 'Arial, Helvetica, sans-serif');
      Engine_Api::_()->sesariana()->readWriteXML('sesariana_mainmenu_fontsize', '13px');
    }
    $tabFontFamily = Engine_Api::_()->sesariana()->getContantValueXML('sesariana_tab_fontfamily');
    if(empty($tabFontFamily)) {
      Engine_Api::_()->sesariana()->readWriteXML('sesariana_tab_fontfamily', 'Arial, Helvetica, sans-serif');
      Engine_Api::_()->sesariana()->readWriteXML('sesariana_tab_fontsize', '15px');
    }
