...
class Sesavatar_Plugin_Signup_Avatar extends Core_Plugin_FormSequence_Abstract
{
 	protected $_name = 'avatar';
 
 	protected $_formClass = 'Sesavatar_Form_Signup_Avatar';
 
 	protected $_script = array('index/signupavatar.tpl', 'sesavatar');
 
 	protected $_adminFormClass = 'Sesavatar_Form_Admin_Signup_Avatar';
 
 	protected $_adminScript = array('admin-signup/avatar.tpl', 'sesavatar');
...
//     }
    ...
    if( $this->getForm()->isValid($request->getPost()) &&
      ...
      return true;
	    ...
	    $this->onSubmitNotIsValid();
	    return false;
    } else if( $skip == "skipForm" || (!isset($_SESSION['TemporaryProfileImg']) && $finishForm == "finish") ) { /*echo "4";die;
      $this->setActive(false);
      $this->onSubmitIsValid();
      $this->getSession()->skip = true;
      $this->_skip = true;
  ...
  public function onAdminProcess($form)
  {
    ...
    $values = $form->getValues();
    $settings->sesavatar_signup_photo = $values['require_photo'];
...
//     $step_row->enable = $form->getValue('enable');
//     $step_row->save();
//     $settings->setSetting('sesavatar.signup.documentverification', $step_row->enable);
//     $form->addNotice('Your changes have been saved.');
//   }
}
