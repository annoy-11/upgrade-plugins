...
<div class="sesmaterial_main_navigation">
  <ul>
    <?php foreach( $this->navigation as $navigationMenu ): 
      $class = explode(' ', $navigationMenu->class);
      ?>
      <?php if( $countMenu < $this->max ): ?>
        <?php $mainMenuIcon = Engine_Api::_()->getApi('menus', 'sesmaterial')->getIconsMenu(end($class));?>
    ...
    <?php $countMenu = 0; ?>  
    <li class="sesmaterial_main_nav_more">
      <a href="javascript:void(0);">
        <span><?php echo $this->translate($this->moreText) ?></span>
        <i class="fa fa-angle-down"></i>
      </a>
      <ul class="sesmaterial_main_nav_submenu">
        <?php foreach( $this->navigation as  $navigationMenu ): ?>
          <?php $mainMenuIcon = Engine_Api::_()->getApi('menus', 'sesmaterial')->getIconsMenu(end($class));?>
          <?php if ($countMenu >= $this->max): ?>
            <li <?php if ($navigationMenu->active): ?><?php echo "class='active'";?><?php endif; ?> >
...
<?php $header_fixed = Engine_Api::_()->sesmaterial()->getContantValueXML('sesmaterial_header_fixed_layout'); 
if($header_fixed == '1'):
?>
	<script>
