...
?>
<?php 
$showPopup = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmaterial.popup.enable', 1);
$popShow = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmaterial.popupshow', 1);
?>
  ...
  ?>
    ...
    <?php foreach( $this->navigation as $item ): $class = explode(' ', $item->class); ?>
      ...
      <?php elseif(end($class) == 'core_mini_auth' && $this->viewer->getIdentity() == 0): ?>
        <?php if(empty($popShow)): ?>
	        <li class="sesmaterial_minimenu_link sesmaterial_minimenu_link_login">
        ...
        <?php  elseif(end($class) == 'core_mini_friends'): ?>
           <?php $friendRequestIcon = Engine_Api::_()->getApi('menus', 'sesmaterial')->getIconsMenu('core_mini_friends');?>
           <?php if($this->viewer->getIdentity()):?>
             <li class="sesmaterial_minimenu_request sesmaterial_minimenu_icon" title="<?php echo $this->translate('Friends Invitation');?>">
            <?php if($this->requestCount):?>
              <span id="request_count_new" class="sesmaterial_minimenu_count"><?php echo $this->requestCount ?></span>
            <?php else:?>
          ...
          <span onclick="toggleUpdatesPulldown(event, this, '4', 'friendrequest');" style="display:block;" class="friends_pulldown">
            <div id="friend_request" class="sesmaterial_pulldown_contents_wrapper">
              <div class="dropdown_caret"><span class="caret_outer"></span><span class="caret_inner"></span></div>
              <div class="sesmaterial_pulldown_header">
                <?php echo $this->translate('Requests'); ?>
              </div>
              <div id="sesmaterial_friend_request_content" class="sesmaterial_pulldown_contents">
        ...
        <?php $notificationIcon = Engine_Api::_()->getApi('menus', 'sesmaterial')->getIconsMenu('core_mini_notification');?>
        <?php if($this->viewer->getIdentity()):?>
          <li id='core_menu_mini_menu_update' class="sesmaterial_minimenu_updates sesmaterial_minimenu_icon" title="<?php echo $this->translate('Notifications');?>">
              <?php if($this->notificationCount):?>
                <span id="notification_count_new" class="sesmaterial_minimenu_count">
            ...
            <span onclick="toggleUpdatesPulldown(event, this, '4', 'notifications');" style="display:block;" class="updates_pulldown">
              <div class="sesmaterial_pulldown_contents_wrapper">
                <div class="dropdown_caret"><span class="caret_outer"></span><span class="caret_inner"></span></div>
                <div class="sesmaterial_pulldown_header">
                  <?php echo $this->translate('Notifications'); ?>
                </div>
                <div class="sesmaterial_pulldown_contents pulldown_content_list">
                ...
                <div class="sesmaterial_pulldown_options">
      ...
      <?php elseif(end($class) == 'core_mini_profile'):?>
        <li class="sesmaterial_minimenu_profile" title="<?php echo $this->translate('My Profile');?>">
          <?php echo $this->htmlLink($item->getHref(), $this->itemPhoto($this->viewer(), 'thumb.icon')) ?>
        </li>
      <?php elseif(end($class) == 'core_mini_settings'):?>
        <?php $settingIcon = Engine_Api::_()->getApi('menus', 'sesmaterial')->getIconsMenu('core_mini_settings');?>
        <li class="sesmaterial_minimenu_setting sesmaterial_minimenu_icon" title="<?php echo $this->translate('Settings');?>">
          <span onclick="toggleUpdatesPulldown(event, this, '4', 'settings');" style="display:block;" class="settings_pulldown">
            <div id="user_settings" class="sesmaterial_pulldown_contents_wrapper sm-mini-menu-settings-pulldown">
              <div class="dropdown_caret"><span class="caret_outer"></span><span class="caret_inner"></span></div>
              <div class="sesmaterial_pulldown_header">
                <?php echo $this->translate('Account & Settings');?>
              </div>
              <div id="sesmaterial_user_settings_content" class="sesmaterial_pulldown_contents">
      ...
      <?php elseif(end($class) == 'core_mini_messages'):?>
        <?php $messageIcon = Engine_Api::_()->getApi('menus', 'sesmaterial')->getIconsMenu('core_mini_messages');?>
        <li class="sesmaterial_minimenu_message sesmaterial_minimenu_icon" title="<?php echo $this->translate('Messages');?>">
          <?php if($this->messageCount):?>
            <span id="message_count_new" class="sesmaterial_minimenu_count"><?php echo $this->messageCount ?></span>
          <?php else:?>
          ...
          <span onclick="toggleUpdatesPulldown(event, this, '4', 'message');" style="display:block;" class="messages_pulldown">
            <div id="sesmaterial_user_messages" class="sesmaterial_pulldown_contents_wrapper sm-mini-menu-messages-pulldown">
              <div class="dropdown_caret"><span class="caret_outer"></span><span class="caret_inner"></span></div>
              <div class="sesmaterial_pulldown_header">
              ...
              <div id="sesmaterial_user_messages_content" class="sesmaterial_pulldown_contents">
      ...
      <?php elseif(end($class) == 'core_mini_admin'):?>
        <?php continue;?>
      <?php else:?>
        <li class="sesmaterial_minimenu_link">
...
<script type='text/javascript'>
  ...
  en4.core.runonce.add(function(){
    ...
    (new Request.JSON({
      'url' : en4.core.baseUrl + 'sesmaterial/notifications/hide'
    })).send();
  ...
  var toggleUpdatesPulldown = function(event, element, user_id, menu) {
    ...
    if(event.target.className == 'sesmaterial_pulldown_header')
    return;
    ...
    if(menu == 'notifications' && hideNotification == 0) {
      ...
      if( element.className=='settings_pulldown' ) {
        ...
        document.getElementById('sesmaterial_user_settings_content').style.display = 'block';
      } 
      else {
  ...
  var showNotifications = function() {
    ...
    abortRequest = new Request.HTML({
      'url' : en4.core.baseUrl + 'sesmaterial/notifications/pulldown',
      'data' : {
        'format' : 'html',
        'page' : 1
      },
      'onComplete' : function(responseTree, responseElements, responseHTML, responseJavaScript) {
        ...
        document.getElementById('notification_count_new').removeClass('sesmaterial_minimenu_count');
      }
  ...
  function showSettings() {
    ...
    abortRequest = new Request.HTML({
      url : en4.core.baseUrl + 'sesmaterial/index/general-setting',
      data : {
        format : 'html'
      },
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript)
      {
       document.getElementById('sesmaterial_user_settings_content').innerHTML = responseHTML;
      }
    });
    en4.core.request.send(abortRequest, {
    'force': true
  ...
  function showMessages() {
    ...
    abortRequest = new Request.HTML({
      url : en4.core.baseUrl + 'sesmaterial/index/inbox',
      data : {
        format : 'html'
      },
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript)
      {
       document.getElementById('sesmaterial_user_messages_content').innerHTML = responseHTML;
       document.getElementById('message_count_new').innerHTML = '';
       document.getElementById('message_count_new').removeClass('sesmaterial_minimenu_count');
      }
    }); 
    en4.core.request.send(abortRequest, {
    'force': true
  ...
  function showFriendRequests() {
    ...
    abortRequest = new Request.HTML({
      url : en4.core.baseUrl + 'sesmaterial/index/friendship-requests',
      data : {
        format : 'html'
      },
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript)
      {
       if(responseHTML) {
         document.getElementById('sesmaterial_friend_request_content').innerHTML = responseHTML;
         document.getElementById('request_count_new').innerHTML = '';
         document.getElementById('request_count_new').removeClass('sesmaterial_minimenu_count');
       }
       else {
  ...
  if('<?php echo $this->viewer->getIdentity();?>' == 0) {
			jqueryObjectOfSes(document).ready(function() {
				<?php $forcehide =  $settings->getSetting('sesmaterial.popup.fixed', false); ?>
			...
			jqueryObjectOfSes.magnificPopup.instance.close = function () {
				var day = '<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmaterial.popup.day', 5);?>';
...
window.addEvent('domready', function() {
  $(document.body).addEvent('click', function(event){
    if(event.target.id != 'show_message' && event.target.id != 'show_request' && event.target.id != 'updates_toggle' &&  event.target.id != 'show_settings' && event.target.className != 'sesmaterial_pulldown_header' && event.target.className != 'pulldown_loading') {
      if($$(".updates_pulldown_selected").length > 0)
      $$('.updates_pulldown_selected').set('class', 'updates_pulldown');
...
<?php if($this->viewer->getIdentity() == 0): ?>
  <?php $popUpDesign = Engine_Api::_()->sesmaterial()->getContantValueXML('sesmaterial_popup_design'); ?>
...
<?php if(0) { ?>
  function newFriendRequests() {
    ...
    en4.core.request.send(new Request.JSON({
      url : en4.core.baseUrl + 'sesmaterial/index/new-friend-requests',
      method : 'POST',
      data : {
        format : 'json'
      },
      onSuccess : function(responseJSON) 
      {
        if( responseJSON.requestCount && $("request_count_new") ) {
          ...
          $("request_count_new").style.display = 'block';
         		...
         		$("request_count_new").addClass('sesmaterial_minimenu_count');
  ...
  function newUpdates() {
    en4.core.request.send(new Request.JSON({
      url : en4.core.baseUrl + 'sesmaterial/index/new-updates',
      method : 'POST',
      data : {
        format : 'json'
      },
      onSuccess : function(responseJSON) 
      {
        if( responseJSON.notificationCount && $("notification_count_new") ) {
          ...
          if(responseJSON.notificationCount > 0 && responseJSON.notificationCount != '')
            $("notification_count_new").addClass('sesmaterial_minimenu_count');
        }
      }
    }));
  }  
  function newMessages() {
    en4.core.request.send(new Request.JSON({
      url : en4.core.baseUrl + 'sesmaterial/index/new-messages',
      method : 'POST',
      data : {
        format : 'json'
      },
      onSuccess : function(responseJSON) 
      {
        if( responseJSON.messageCount && $("message_count_new") ) {
          ...
          $("message_count_new").style.display = 'block';
        	...
        	$("message_count_new").addClass('sesmaterial_minimenu_count');
