 ...
 */
class Sesmaterial_Widget_FooterController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->footerlogo = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmaterial.footerlogo', '');

    $this->view->paginator = Engine_Api::_()->getDbTable('socialicons', 'sesmaterial')->getSocialInfo(array('enabled' => 1));
    $this->view->footerlinks = Engine_Api::_()->getDbTable('footerlinks', 'sesmaterial')->getInfo(array('sublink' => 0, 'enabled' => 1));
    ...
    $sesmaterial_footer = Zend_Registry::isRegistered('sesmaterial_footer') ? Zend_Registry::get('sesmaterial_footer') : null;
    if(empty($sesmaterial_footer)) {
    ...
    $this->view->googleplusButton = $this->_getParam('googleplus_url_path', 'http://plus.google.com/');
		...
		if(empty($popShow)) {
			$contentText = '<div class="sesmaterial_footer_top_section sesbasic_clearfix sesbasic_bxs">
  	<p><img src="application/modules/Sesmaterial/externals/images/bottom-text.png" alt=""></p><p><a href="signup">Join Now</a></p></div>';
		} else {
	    $contentText = '<div class="sesmaterial_footer_top_section sesbasic_clearfix sesbasic_bxs">
  	<p><img src="application/modules/Sesmaterial/externals/images/bottom-text.png" alt=""></p><p><a id="popup-signup" data-effect="signup-link mfp-zoom-in" class="popup-with-move-anim" href="#user_signup_form">Join Now</a></p></div>';
  	}
    ...
    if(count($languageList) == '1') {
      $this->view->content = Engine_Api::_()->getApi('settings', 'core')->getSetting("sesmaterial.footertext.en", $contentText);
    } else {
      $this->view->content = Engine_Api::_()->getApi('settings', 'core')->getSetting("sesmaterial.footertext.$language", $contentText);
    }
    ...
    $select = new Zend_Db_Select($db);
		$select->from('engine4_core_content', 'name')
		        ->where('page_id = ?', 1)
		        ->where('name LIKE ?', '%sesmaterial.header%')
		        ->limit(1);
