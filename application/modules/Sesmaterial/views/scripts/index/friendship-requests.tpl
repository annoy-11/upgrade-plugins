 ...
 * @version    $Id: friendship-requests.tpl 2018-07-28 00:00:00 SocialEngineSolutions $
...
<?php if( $this->friendRequests->getTotalItemCount() > 0 ): ?>  
  <ul class='pulldown_content_list' id="notifications_main">
    <?php foreach( $this->friendRequests as $notification ): ?>
      ...
      ?>
      <li id="user-widget-request-<?php echo $notification->notification_id ?>" class="clearfix <?php if( !$notification->sesmaterial_read ): ?>pulldown_content_list_highlighted<?php endif; ?>"  value="<?php echo $notification->getIdentity();?>">
