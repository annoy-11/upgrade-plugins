...
class Sesmaterial_AdminCustomThemeController extends Core_Controller_Action_Admin {

  public function indexAction() {
    
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesmaterial_admin_main', array(), 'sesmaterial_admin_main_customcss');
    
    $writeable = array();
    $writeable['sesmaterial'] = false;
    try {
      foreach(array('theme.css', 'constants.css', 'media-queries.css', 'sesmaterial-custom.css') as $file) {
        if(!file_exists(APPLICATION_PATH . "/application/themes/sesmaterial/$file")) {
          throw new Core_Model_Exception('Missing file');
        } else {
          $this->checkWriteable(APPLICATION_PATH . "/application/themes/sesmaterial/$file");
        }
      }
      $writeable['sesmaterial'] = true;
    } catch(Exception $e) {
      $this->view->errorMessage = $e->getMessage();
    }
    
    $this->view->writeable = $writeable;
    $this->view->activeFileContents = file_get_contents(APPLICATION_PATH . '/application/themes/sesmaterial/sesmaterial-custom.css');
  }

  public function saveAction() {
  
    $theme_id = 'sesmaterial';
    ...
    //Check file
    $basePath = APPLICATION_PATH . '/application/themes/sesmaterial';
