 ...
 * @version    $Id: IndexController.php 2018-07-28 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesmaterial_IndexController extends Core_Controller_Action_Standard {
  ...
  public function inboxAction() {
    ...
    Engine_Api::_()->getApi('message', 'sesmaterial')->setUnreadMessage($viewer);
  }
  ...
  public function newMessagesAction() {
    $this->view->messageCount = Engine_Api::_()->getApi('message', 'sesmaterial')->getMessagesUnreadCount(Engine_Api::_()->user()->getViewer());
  }
  ...
  public function friendshipRequestsAction() {
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->friendRequests = $newFriendRequests = Engine_Api::_()->getDbtable('notifications', 'sesmaterial')->getFriendrequestPaginator($viewer);
    $newFriendRequests->setCurrentPageNumber($this->_getParam('page'));
    Engine_Api::_()->getApi('message', 'sesmaterial')->setUnreadFriendRequest($viewer);
  }

  public function searchAction() {

    $text = $this->_getParam('text', null);
    if (isset($_COOKIE['sesmaterial_commonsearch']))
      $type = $_COOKIE['sesmaterial_commonsearch'];
    else
      $type = '';
