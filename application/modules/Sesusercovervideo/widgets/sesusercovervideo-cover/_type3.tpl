 ...
 * @version    $Id: _type3.tpl 2016-01-29 00:00:00 SocialEngineSolutions $
...
<?php } ?>
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesusercovervideo/externals/styles/cover_layout3.css'); ?>

<div class="sesusercover_main_container sesusercover_cover_type3 sesbasic_bxs <?php if($this->is_fullwidth){?>sesusercovervideo_cover_wrapper_full<?php } ?>">
	<div class="sesusercover_main_container_wrapper">
  ...
  <div class="sesusercovervideo_cover_information_block sesbasic_clearfix">
     ...
     <?php if(in_array('photo',$this->option)){ ?>
      ...
      ?>
    	<div class="sesusercovervideo_profile_img <?php if($this->userphotoround == 1): ?> isround <?php endif; ?>">
           ...
           ?>
            <?php $music_count =  Engine_Api::_()->getApi('core', 'sesusercovervideo')->getModuleRecordsOfUser($musicItem,$this->subject->getIdentity()); ?>
              <div title="<?php echo $this->translate(array('%s music', '%s musics', $music_count), $this->locale()->toNumber($music_count))?>"> 
                ...
                <span class="sesusercovervideo_cover_stat_txt"> <?php echo str_replace(',','',preg_replace('/[0-9]+/', '', $this->translate(array('%s Music', '%s Musics', $music_count), $this->locale()->toNumber($music_count)))); ?></span>
              </div>
         <?php } ?>         
...
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesmember/externals/styles/styles.css'); ?>
            <div class="sesusercovervideo_cover_rating sesbasic_clearfix">
              <div id="video_rating" class="sesbasic_rating_star" onmouseout="rating_out();">
        ...
        <?php if(in_array('recentlyViewedBy',$this->option) && $this->sesmemberenable){ ?>
          <div class="sesusercovervideo_users">
    ...
    <?php if(($this->module == 'user' || $this->module == 'sesmember') && $this->controller == 'profile' && $this->action == 'index'): ?>
      <?php if($this->tab == 'inside'){ ?>
        <div class="sesmember_tabs sesusercovervideo_cover_tabs"></div>
      <?php } ?>
    <?php endif; ?>
  </div>
  </div>
...
<?php if(($this->module == 'user' || $this->module == 'sesmember') && $this->controller == 'profile' && $this->action == 'index'): ?>
  ...
  var tabs = sesJqueryObject('.layout_core_container_tabs').find('.tabs_alt').get(0).outerHTML;
  	//sesJqueryObject('.layout_core_container_tabs').find('.tabs_alt').remove();
		...
		sesJqueryObject('.sesmember_tabs').html(tabs);
  });
...
var heightin = sesJqueryObject('.sesusercovervideo_cover_information_block').height();
var givht = sesJqueryObject('.sesusercover_cover_wrapper').height();
var tot = parseInt(heightin) + parseInt(givht);
sesJqueryObject('.sesusercover_main_container_wrapper').css('height',tot+'px');
});
