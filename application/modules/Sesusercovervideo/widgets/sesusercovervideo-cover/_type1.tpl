 ...
 * @version    $Id: _type1.tpl 2016-01-29 00:00:00 SocialEngineSolutions $
...
<?php } ?>
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesusercovervideo/externals/styles/cover_layout1.css'); ?>

<div class="sesusercovervideo_cover_wrapper sesbasic_bxs sesusercover_cover_type1 <?php if($this->is_fullwidth){?>sesusercovervideo_cover_wrapper_full<?php } ?>" style="height:<?php echo $this->height; ?>px">
<div class="sesusercovervideo_cover_container sesbasic_bxs <?php echo $this->tab == 'inside' ? 'sesusercovervideo_cover_tabs_wrap' : '' ?> " style="height:<?php echo $this->height; ?>px">
  <!--Cover Photo-->
   <?php
	...
	?>
  <div id="sesusercovervideo_cover_default" class="sesusercovervideo_cover_thumbs" style="display:<?php echo !$memberVideoCover ? 'block' : 'none'; ?>;">
 </div>
 	<div id="sesusercovervideo_cover_image" class="sesusercovervideo_cover_image">
  ...
  <span class="sesusercovervideo_cover_fade"></span>
 ...
 <?php if($this->can_edit  && $this->canCreate){ ?>
  <div class="sesusercovervideo_cover_change_cover" id="sesusercovervideo_cover_change">
    ...
    <div class="sesusercovervideo_change_cover_options sesbasic_option_box"> 
      ...
      <?php } ?>
      <a id="removeVideoCover" href="<?php echo 'sesusercovervideo/index/confirmation/'; ?>" class="sessmoothbox" style="display:<?php echo (isset($isUserVideo) && $isUserVideo != 0 && $isUserVideo != '') ? 'block' : 'none' ; ?>;" data-src="<?php echo $isUserVideo; ?>"><i class="fa fa-trash"></i><?php echo $this->translate('Remove Cover Video'); ?></a>
    </div>
  </div>
 <?php } ?>
  <?php if($this->sesmemberenable): ?>
  ...
  <?php endif; ?>
  <div class="sesusercovervideo_cover_inner">
    <div class="sesusercovervideo_cover_cont sesbasic_clearfix">
      <div class="sesusercovervideo_cover_cont_inner">
        <!--Main Photo-->     
        <?php
      ...
      <?php if(in_array('photo',$this->option)){ ?>      
        <div class="sesusercovervideo_cover_main_photo">
        ...
        <?php } ?>
         ...
         <?php } ?>
        <div class="sesusercovervideo_cover_info">
         <?php if(in_array('title',$this->option)){ ?>
          <h2 class="sesusercovervideo_cover_title">
        ...
        <?php if(in_array('membersince',$this->option)){ ?>
          <div class="sesusercovervideo_cover_field sesbasic_clearfix"> 
          ...
          <?php } ?>
            ...
            <!--Member Statics-->
            <div class="sesusercovervideo_cover_stats sesbasic_clearfix clear">
              <?php if(in_array('videocount',$this->option) && (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesvideo')) || Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('video')))){ ?> 
                  ...
                  <div title="<?php echo $this->translate(array('%s video', '%s videos', $video_count), $this->locale()->toNumber($video_count)); ?>"> 
                    <span class="sesusercovervideo_cover_stat_count"><?php echo $video_count; ?></span>
                    <span class="sesusercovervideo_cover_stat_txt"> <?php echo str_replace(',','',preg_replace('/[0-9]+/', '', $this->translate(array('%s Video', '%s Videos', $video_count), $this->locale()->toNumber($video_count)))); ?></span>
                  </div>
              <?php } ?>
              ...
              <?php $photo_count =  Engine_Api::_()->getApi('core', 'sesusercovervideo')->getModuleRecordsOfUser('album',$this->subject->getIdentity()); ?>
                <div title="<?php echo $this->translate(array('%s album', '%s albums', $photo_count), $this->locale()->toNumber($photo_count))?>"> 
                  <span class="sesusercovervideo_cover_stat_count"><?php echo $photo_count; ?></span>
                  <span class="sesusercovervideo_cover_stat_txt"> <?php echo str_replace(',','',preg_replace('/[0-9]+/', '', $this->translate(array('%s Album', '%s Albums', $photo_count), $this->locale()->toNumber($photo_count)))); ?></span>
                </div>
              <?php } ?>
              ...
              <?php $event_count =  Engine_Api::_()->getApi('core', 'sesusercovervideo')->getModuleRecordsOfUser('event',$this->subject->getIdentity()); ?>
                <div title="<?php echo $this->translate(array('%s event', '%s events', $event_count), $this->locale()->toNumber($event_count))?>"> 
                  <span class="sesusercovervideo_cover_stat_count"><?php echo $event_count; ?></span>
                  <span class="sesusercovervideo_cover_stat_txt"> <?php echo str_replace(',','',preg_replace('/[0-9]+/', '', $this->translate(array('%s Event', '%s Events', $event_count), $this->locale()->toNumber($event_count)))); ?></span>
                </div>
              <?php } ?>
              <?php if(in_array('forumcount',$this->option) && Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('forum'))){ ?>
                <?php $forum_count =  Engine_Api::_()->getApi('core', 'sesusercovervideo')->getModuleRecordsOfUser('forum_topic',$this->subject->getIdentity()); ?>
                  <div title="<?php echo $this->translate(array('%s forum', '%s forums', $forum_count), $this->locale()->toNumber($forum_count))?>"> 
                    <span class="sesusercovervideo_cover_stat_count"><?php echo $forum_count; ?></span>
                    <span class="sesusercovervideo_cover_stat_txt"> <?php echo str_replace(',','',preg_replace('/[0-9]+/', '', $this->translate(array('%s Forum', '%s Forums', $forum_count), $this->locale()->toNumber($forum_count)))); ?></span>
                  </div>
              <?php } ?>
                 ...
                 ?>
                  <?php $music_count =  Engine_Api::_()->getApi('core', 'sesusercovervideo')->getModuleRecordsOfUser($musicItem,$this->subject->getIdentity()); ?>
                    <div title="<?php echo $this->translate(array('%s music', '%s musics', $music_count), $this->locale()->toNumber($music_count))?>"> 
                      <span class="sesusercovervideo_cover_stat_count"><?php echo $music_count; ?></span>
                      <span class="sesusercovervideo_cover_stat_txt"> <?php echo str_replace(',','',preg_replace('/[0-9]+/', '', $this->translate(array('%s Music', '%s Music', $music_count), $this->locale()->toNumber($music_count)))); ?></span>
                    </div>
              <?php } ?>
              <?php if(in_array('groupcount',$this->option) && Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('group'))){ ?>
                <?php $group_count =  Engine_Api::_()->getApi('core', 'sesusercovervideo')->getModuleRecordsOfUser('group',$this->subject->getIdentity()); ?>
                  <div title="<?php echo $this->translate(array('%s group', '%s groups', $group_count), $this->locale()->toNumber($group_count))?>"> 
                    <span class="sesusercovervideo_cover_stat_count"><?php echo $group_count; ?></span>
                    <span class="sesusercovervideo_cover_stat_txt"> <?php echo str_replace(',','',preg_replace('/[0-9]+/', '', $this->translate(array('%s Group', '%s Groups', $group_count), $this->locale()->toNumber($group_count)))); ?></span>
                  </div>
              <?php } ?>
              <?php if(in_array('viewcount',$this->option) && $this->sesmemberenable){ ?>
                <div title="<?php echo $this->translate(array('%s view', '%s views', $this->subject->view_count), $this->locale()->toNumber($this->subject->view_count))?>">
                  <span class="sesusercovervideo_cover_stat_count"><?php echo $this->subject->view_count ?></span>
                  <span class="sesusercovervideo_cover_stat_txt"> <?php echo str_replace(',','',preg_replace('/[0-9]+/','',$this->translate(array('%s View', '%s Views', $this->subject->view_count), $this->locale()->toNumber($this->subject->view_count)))); ?></span>
                </div>
                ...
                <div title="<?php echo $this->translate('%s Followers', $followCount); ?>">
                  <span class="sesusercovervideo_cover_stat_count"><?php echo $followCount; ?></span>
                  <span class="sesusercovervideo_cover_stat_txt"> Followers</span>
                </div>
              <?php } ?>
              <?php if(in_array('likecount',$this->option) && $this->sesmemberenable){ ?>
                  <div title="<?php echo $this->translate(array('%s like', '%s likes', $this->subject->like_count), $this->locale()->toNumber($this->subject->like_count))?>">
                    <span class="sesusercovervideo_cover_stat_count"><?php echo $this->subject->like_count; ?></span>
                    <span class="sesusercovervideo_cover_stat_txt"> <?php echo str_replace(',','',preg_replace('/[0-9]+/', '', $this->translate(array('%s Like', '%s Likes', $this->subject->like_count), $this->locale()->toNumber($this->subject->like_count)))); ?></span>
                  </div>      
              <?php } ?>
            </div>
            ...
            <?php if(in_array('rating',$this->option) && $this->sesmemberenable){ ?>
                ...
                <a class="details_btn sessmoothbox" href="<?php echo $this->url(array('action'=>'review-stats','user_id'=>$this->subject->getIdentity()), 'sesmember_general'); ?>" title="Rating Details"><?php echo $this->translate("Rating Details") ;?></a>
              </div>
            <?php } ?>
        ...
        <div class="sesusercovervideo_cover_footer">
       		<div class="sesusercovervideo_cover_footer_inner">     
            <?php if($this->tab == 'inside'){ ?>
              <div class="sesusercovervideo_tabs sesusercovervideo_cover_tabs"></div>
            <?php } ?>
          </div>
        </div>
      </div>
  ...
  <?php if($this->tab == 'inside'){ ?>
    ...
    if (matchMedia('only screen and (min-width: 767px)').matches) {
      ...
      var tabs = sesJqueryObject('.layout_core_container_tabs').find('.tabs_alt').get(0).outerHTML;
        ...
        sesJqueryObject('.sesusercovervideo_tabs').html(tabs);
      });
...
<?php if(isset($this->can_edit)) { ?>
  <script type="application/javascript">
    ...
    function removeCoverPhoto() {
      sesJqueryObject('#removeVideoCover').css('display','none');
      sesJqueryObject('#sesusercovervideo_cover_photo_loading').show();
