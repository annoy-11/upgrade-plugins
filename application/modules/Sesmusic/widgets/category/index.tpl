...
<!--    <li <?php //if(empty($this->category_id)) { ?> class="selected" <?php //} ?>>
      <a class="catlabel <?php //echo $this->image == 0 ? '' : 'noicon' ?>" href="<?php //echo $this->url(array('action' => 'browse'), $route, true) ; ?>" <?php //if($this->image == 0 && $item->cat_icon != '' && !is_null($item->cat_icon)){ ?> style="background-image:url(<?php //echo $this->storage->get($item->cat_icon, '')->getPhotoUrl(); ?>);"<?php //} ?>><?php //echo $this->translate("All Categories"); ?></a>
    </li>-->
    <?php foreach( $this->categories as $item ): ?>
      <li <?php if($this->category_id == $item->category_id) { ?> class="selected" <?php } ?>>
        ...
        <?php endif; ?>
        <?php if($this->storage->get($item->cat_icon, '')){ ?>
          <a class="catlabel <?php echo $this->image == 0 ? '' : 'noicon' ?>" href="<?php echo $this->url(array('action' => 'browse'), $route, true).'?category_id='.urlencode($item->getIdentity()) ; ?>" <?php if($this->image == 0 && $item->cat_icon != '' && !is_null($item->cat_icon)){ ?> style="background-image:url(<?php if($this->storage->get($item->cat_icon, '')){ echo $this->storage->get($item->cat_icon, '')->getPhotoUrl(); } ?>);"<?php } ?>><?php echo $item->category_name; ?></a>
        <?php } else { ?>
          <a class="catlabel <?php echo $this->image == 0 ? '' : 'noicon' ?>" href="<?php echo $this->url(array('action' => 'browse'), $route, true).'?category_id='.urlencode($item->getIdentity()) ; ?>"><?php echo $item->category_name; ?></a>
        <?php } ?>
