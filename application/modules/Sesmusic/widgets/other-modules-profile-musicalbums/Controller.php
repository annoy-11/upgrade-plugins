 ...
 */
class Sesmusic_Widget_OtherModulesProfileMusicalbumsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->resource_id = $resource_id = isset($params['resource_id']) ? $params['resource_id'] : $subject->getIdentity();
    $this->view->resource_type = $resource_type = isset($params['resource_type']) ? $params['resource_type'] : $subject->getType();
    ...
    if ($type == 'profilemusicalbums') {
      ...
      $select = $albumSongTable->select()
              ...
              ->where($albumTableName.'.resource_type =?', $resource_type)
              ->where($albumTableName.'.resource_id =?', $resource_id)
