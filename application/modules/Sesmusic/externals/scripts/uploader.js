...
//     }
    //Hide Add Song Link
    var file_id = fancyUploadFileds.get('value');
    if(file_id) {
      if($('upload_file_link'))
        $('upload_file_link').style.display = 'none';
    }
  },
  ...
  removeFile: function (el) {
    ...
    if ($$('ul#uploaded-file-list li').length === 0) {
      ...
      //show Add song link
      if($('upload_file_link'))
        $('upload_file_link').style.display = 'block';
    }
  },
});
