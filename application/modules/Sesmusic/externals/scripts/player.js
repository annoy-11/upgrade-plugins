...
document.addEventListener('keydown', (event) => {
	if(event.keyCode==music_space_key){
		if(sesJqueryObject("#sesmusic_player_button_play").is(":visible")){
			sesJqueryObject('#sesmusic_player_button_play').trigger('click');
		}
		//event.preventDefault();
	}
	if (sesJqueryObject.inArray(event.keyCode, keys) == -1) {
...
document.addEventListener('keyup', (event) => {
  ...
  // rmeove it
	keys = sesJqueryObject.grep(keys, function(value) {
...
en4.music.playlistAbstract = new Class({
  ...
  initialize: function(el, options) {
    ...
    //Song image work
    this.imgsongs = this.container.getElements('img.item_photo_sesmusic_albumsong');
  ...
  play : function(index) {
   ...
   this.imgsongs = $('sesmusic_player_tracks_contaner').getElements('img.item_photo_sesmusic_albumsong');
    if( !$type(index) ) {
      index = this.selected;
    }
  ...
  create : function(index) {    
    ...
    var soundOptions = {
      ...
      onresume:       function() { 
												if(playingSongId>0){
													setMusicCookie("sesmusic_playSongId", playingSongId, 1);
												}
      ...
      onpause:        function() {  
												...
												for(var i = 0; i < getSongPlaying.length ; i++) {
													...
													setMusicCookie("sesmusic_playSongId", 0, 0);										
      ...
      onstop:         function() { setMusicCookie("sesmusic_playSongId", 0, 0);this.fireEvent('soundmanager_onstop'); }.bind(this), //function() { this.container.getElement('.sesmusic_player_button_play').removeClass('sesmusic_player_button_pause'); }.bind(this),
      onfinish:       function() { setMusicCookie("sesmusic_playSongId", 0, 0);this.fireEvent('soundmanager_onfinish'); }.bind(this), //function() { this.container.getElement('.sesmusic_player_button_play').removeClass('sesmusic_player_button_pause'); this.playNext(); }.bind(this),
  ...
  if(addClassToBody == 1){
   ...
   this._updateDuration();
    ...
    setMusicCookie("sesmusic_playSongId", song_id, 1);
    ...
    //Song image change when click on next, previous and playlist list
    this.imgsongs.each(function(songimage, index) { 
      if( index == this.selected ) {
        var imageSRC = sesJqueryObject(songimage).attr('src');

        $('sesmusic_player_art').innerHTML = '<a href="javascript:void(0);" class="sesmusic_player_tracks_photo"><img class="thumb_icon item_photo_sesmusic_albumsong" src="'+imageSRC+'" alt=""></a>';
      }
    }.bind(this));
  ...
  _attachEvents : function() {
    ...
    this.container.getElement('#sesmusic_player_button_autoPlay').addEvent('click', function(event) {
      ...
      //Song Play work after page refresh from same location
      if(sesmusic_playsongtime) {
        var playSongTime = getMusicCookie("sesmusic_playsongtime");
        if(playSongTime){
          var playMiliSecondTime = playSongTime.split(':');
          var playMiliSecondTimeTotal = 0;
          if(typeof playMiliSecondTime[0] != 'undefined')
            playMiliSecondTimeTotal = playMiliSecondTime[0] * 60 * 1000;
          if(typeof playMiliSecondTime[1] != 'undefined')
            playMiliSecondTimeTotal = playMiliSecondTime[1] * 1000 + playMiliSecondTimeTotal;
          this.sound.setPosition(playMiliSecondTimeTotal);
          
        }
      }
      sesmusic_playsongtime = false;
    ...
    $(document.body).addEvent("click:relay(.sesmusicplaylistmore)", function(event, element) {
      var targetId = event.target.id;
			...
			if(targetId.split('_')[0] != 'delete' && targetId.split('_')[0] != 'store') {
  ...
  _updateDuration : function() {
    ...
    //var d = new Date(ms);
    console.log(msToTime(ms),ms);
    //var hms = d.getMinutes().toString() - 30 +':'+ (d.getSeconds().toString().length==1?'0':'') + d.getSeconds().toString();
    this.container.getElement('div.sesmusic_player_time_total').set('text', msToTime(ms));
  },
  ...
  _updateScrub : function() {
    ...
    setMusicCookie("sesmusic_playsongtime", hms, 1);
    ...
    if( !this.slider.element.hasClass('mousedown') ) {
      ...
      $('sesmusic_player_scrub_cursor').setStyle('width', steps+'px');
