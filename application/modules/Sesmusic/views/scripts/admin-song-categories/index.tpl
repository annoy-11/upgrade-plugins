...
<div class="sesbasic-form sesbasic-categories-form">
  <div>
    <?php if( count($this->subNavigation) ): ?>
      <div class='sesbasic-admin-sub-tabs' style="margin-bottom:0;">
        <?php
    ...
    <div class="sesbasic-form-cont" style="padding-top:15px;">
              ...
              <p class="description">
                <?php echo $this->translate('Song categories can be managed here. To create new categories, use "Add New Category" form below. Below, you can also choose Slug URL, Title, Description, Profile Type to be associated with the category, Icon and Thumbnail. You can also map Categories with the Profile Types, so that questions belonging to the mapped Profile Type will appear to users while creating / editing Songs when they choose the associated Category.<br /><br />To create 2nd-level categories and 3rd-level categories, choose respective 1st-level and 2nd-level category from “Parent Category” dropdown below. Choose this carefully as you will not be able to edit Parent Category later.<br /><br />To reorder the categories, click on their names or row and drag them up or down.') ?>
              </p>
				...
				<div class="sesbasic-form-field-element">
				  <select name="parent" id="parent" class="postform">
				   ...
				   <?php foreach ($this->categories as $category): ?>
					...
					<?php 
					  $subcategory = Engine_Api::_()->getDbTable('categories', 'sesmusic')->getModuleSubcategory(array('column_name' => "*", 'category_id' => $category->category_id));          foreach ($subcategory as $sub_category):  
					?>
            ...
            <div class="sesbasic-categories-listing">
				...
				<tbody>
					  ...
					  <tr id="categoryid-<?php echo $category->category_id; ?>" data-article-id="<?php echo $category->category_id; ?>">
						...
						<td><?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesmusic', 'controller' => 'song-categories', 'action' => 'edit-category', 'id' => $category->category_id), $this->translate('Edit'), array()) ?> 
							<?php if($category->category_id != 0){ ?>
				...
				<tr id="categoryid-<?php echo $sub_category->category_id; ?>" data-article-id="<?php echo $sub_category->category_id; ?>">
					...
					<td><?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesmusic', 'controller' => 'song-categories', 'action' => 'edit-category', 'id' => $sub_category->category_id), $this->translate('Edit'), array()) ?> | <?php echo $this->htmlLink('javascript:void(0);', $this->translate('Delete'), array('class' => 'deleteCat','data-url'=>$sub_category->category_id)) ?> 		</td>
				</tr>
				<?php 
                		//SubSubcategory Work
...
ajaxurl = en4.core.baseUrl+"admin/sesmusic/song-categories/change-order";
