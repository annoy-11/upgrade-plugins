...
class Sesmusic_AdminManageController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $select = $musicAlbumTable->select()
            ...
            ->where($musicAlbumTableName.'.upload_param =?', 'album')
  ...
  //Featured Artist Action
  public function featuredArtistAction() {
    $artist_id = $this->_getParam('id');
    if (!empty($artist_id)) {
      $artist = Engine_Api::_()->getItem('sesmusic_artist', $artist_id);
      $artist->featured = !$artist->featured;
      $artist->save();
    }
    $this->_redirect('admin/sesmusic/settings/artists');
  }
   //Sponsored Artist Action
  public function sponsoredArtistAction() {
    $artist_id = $this->_getParam('id');
    if (!empty($artist_id)) {
      $artist = Engine_Api::_()->getItem('sesmusic_artist', $artist_id);
      $artist->sponsored = !$artist->sponsored;
      $artist->save();
    }
    $this->_redirect('admin/sesmusic/settings/artists');
  }
