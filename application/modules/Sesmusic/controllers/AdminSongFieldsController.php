 ...
 * @version    $Id: AdminFieldsController.php  2018-04-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesmusic_AdminSongFieldsController extends Fields_Controller_AdminAbstract {

  protected $_requireProfileType = true;
  protected $_fieldType = 'sesmusic_albumsong';

  public function indexAction() {
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesmusic_admin_main', array(), 'sesmusic_admin_main_songcategories');
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesmusic_admin_main_songcategories', array(), 'sesmusic_admin_main_songsubfields');
  ...
  public function fieldEditAction() {
    ...
    // Create form
    $this->view->form = $form = new $adminFormClass();
    $form->setTitle('Edit Song Category Profile Question');
    $form->removeElement('show');
    $form->getElement('search')->setLabel('Show on Browse Search Widget?');
    $form->getElement('search')->setMultiOptions(array(
        0 => 'Hide on Browse Search Widget',
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Sesmusic_Form_Admin_Settings_Type();
    if (!$this->getRequest()->isPost()) {
      return;
    }
    if (!$form->isValid($this->getRequest()->getPost())) {
    ...
    if ($form->getValue('duplicate') != 'null') {
      ...
      $field_meta_array = $db->select()
              ->from('engine4_sesmusic_albumsongs_fields_meta')
              ->where('field_id IN (' . implode(', ', $child_id_array) . ')')
              ->query()
              ->fetchAll();
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_sesmusic_albumsongs_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_sesmusic_albumsongs_fields_maps', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      Engine_Api::_()->getDbTable('categories', 'sesmusic')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
