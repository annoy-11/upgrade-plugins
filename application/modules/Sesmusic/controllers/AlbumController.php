...
class Sesmusic_AlbumController extends Core_Controller_Action_Standard {
  ...
  public function editAction() {
    ...
    //Make form
    $this->view->form = $form = new Sesmusic_Form_Edit(['album_id' => $album->getIdentity()]);
    $form->populateWithObject($album);
  ...
  public function viewAction() {
    ...
    //Download Settings
    $this->view->downloadPublic = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmusic.download.publicuser', 0);
...
//     if (empty($values['Filename']) || empty($_FILES['file'])) {
//       $this->view->status = false;
...
//     }
    ...
    try {
      ...
      //Create song
      $file = Engine_Api::_()->getApi('core', 'sesmusic')->createSong($_FILES['file']);
      if (!$file)
