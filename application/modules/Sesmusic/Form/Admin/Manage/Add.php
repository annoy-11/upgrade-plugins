<?php
class Sesmusic_Form_Admin_Manage_Add extends Engine_Form {
  public function init() {
    ...
    if (!$integrateothermoduleId) {
      ...
      if (!empty($module)) {
        ...
        $integrateothermodule = Engine_Api::_()->sesmusic()->getPluginItem($module);
    ...
    if ($param) {
      if (!$integrateothermoduleId) {
        $this->addElement('Select', 'content_type', array(
            ...
            'description' => 'Select the item type of the above chosen plugin which is defined in its manifest.php file. [This item type is the parent to which music albums are associated. For example for groups in SocialEngine Groups plugin, simply choose "groups".]',
            'multiOptions' => $integrateothermodule,
        ));
        
        $this->addElement('Text', 'content_url', array(
          'label' => 'Item Type URL for Browse Music Albums Page',
          'description' => 'Enter the Item Type URL for the Browse Music Albums Page. This is the URL slug which will come after your site url and before the search word music/albums like: "http://www.yourwebsite.com/ITEM_TYPE_URL_SLUG/music/albums". For example for SE - Groups Plugin, enter "groups", so that the URL will be "http://www.yourwebsite.com/groups/music/albums".',
          'required' => true,
          'allowEmpty' => false,
        ));
