...
class Sesmusic_Model_DbTable_Albums extends Engine_Db_Table {
  ...
  public function getPlaylistSelect($params = array()) {
    ...
    $select = $albumTable->select()
            ->from($albumTable)
            ->where($albumTableName . ".song_count != ?", 0)
            ->group("$albumTableName.album_id")
            ->where($albumTableName.'.upload_param =?', 'album');
    ...
    //don't show other module musics
    if(!empty($params['fetchAll']) && isset($params['fetchAll'])){
  ...
  public function widgetResults($params = array()) {
    ...
    $select = $this->select()
                  ->from($albumTableName, $params['column'])
                  ->where($albumTableName . ".song_count != ?", 0)
				  ->where($albumTableName . '.upload_param =?', 'album');
