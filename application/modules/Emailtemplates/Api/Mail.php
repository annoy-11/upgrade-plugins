 ...
 * @version    $Id: Mail.php  2019-01-25 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Emailtemplates_Api_Mail extends Core_Api_Abstract
{
  protected $_enabled;
  ...
  public function sendSystemRaw($recipient, $type, array $params = array())
  {
    ...
    $mailTemplate = $mailTemplateTable->fetchRow($mailTemplateTable->select()->where('type = ?', $type));
		...
		$coreTemplateId = $mailTemplate->mailtemplate_id;
    ...
    foreach( $recipients as $recipient ) {
      ...
      if( !$bodyTextTemplate ) {
        $bodyTextTemplate = $bodyHtmlTemplate;
			...
			$this->view->body = $bodyTextTemplate = strip_tags($bodyTextTemplate);
      $adminEmails = array();
			...
			if($type == 'emailtemplate_test_mail'){
				$select = $templateTable->select()->from($templateTableName)->where('template_id = ?', $templateId);
				...
				$selectemailtemplateTable = $emailtemplateTable->select()->from($emailtemplateTableName,array("signature"));
				$selectemailtemplateTable->where('template_id =?', $templateId);
				$selectemailtemplateResult = $emailtemplateTable->fetchRow($selectemailtemplateTable);
                if(!empty($selectemailtemplateResult->signature) && $selectemailtemplateResult->signature !=''){
                    $is_signature = true;
                    $signature = $selectemailtemplateResult->signature;
                } elseif($signt) {
                    $is_signature = true;
                    $signature = $signt;
                }
					$bodyTextTemplate = '<p><span style="font-weight: 400;">Hello,</span></p>
<p><span style="font-weight: 400;">'.$bodyTextTemplate.'</span></p><p>&nbsp;</p><p><span style="font-weight: 400;">Thank You</span></p>';
			}else{

				$emailtemplateTable = Engine_Api::_()->getDbTable('selecttemplates','emailtemplates');
				$emailtemplateTableName = $emailtemplateTable->info('name');
				$selectemailtemplateTable = $emailtemplateTable->select()->from($emailtemplateTableName,array("template_id","signature"));
				$selectemailtemplateTable->where('core_template_id =?', $coreTemplateId);
				$selectemailtemplateResult = $emailtemplateTable->fetchRow($selectemailtemplateTable);
				//$selectemailtemplateResult = $selectemailtemplateTable->query()->fetchColumn();
				$id = $selectemailtemplateResult->template_id;
				$sign = $selectemailtemplateResult->signature;

				if($id){
					$is_signature = true;
					$select = $templateTable->select()->from($templateTableName)->where('template_id = ?', $id);
					if($sign){
						$signature = $sign;
					}else{
					...
					$select = $templateTable->select()->from($templateTableName)->where('is_active = ?', '1');
					$signt = Engine_Api::_()->getApi('settings', 'core')->getSetting('emailtemplates.signature', '');
					if($signt){
						$is_signature = true;
						$signature = $signt;
			...
			$this->view->result = $result = $templateTable->fetchRow($select);

			$view = Zend_Registry::isRegistered('Zend_View') ? Zend_Registry::get('Zend_View') : null;
			$viewer = Engine_Api::_()->user()->getViewer();
			if(count($result)>0){
				$finalbodyTextTemplate = $view->partial('_emailhtml.tpl','emailtemplates',array('result'=>$result,'body'=>nl2br($bodyTextTemplate),'id'=>$recipient->user_id,'is_signature'=>$is_signature,'signature'=>$signature));
			}
      // Send
      ...
      $mail = $this->create()
        ...
        ->setBodyHtml($finalbodyTextTemplate)
        ->setBodyText($finalbodyTextTemplate);
