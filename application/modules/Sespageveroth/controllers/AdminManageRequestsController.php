...
class sespageveroth_AdminManageRequestsController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespageveroth_admin_main', array(), 'sespageveroth_admin_main_managerequests');

    $this->view->formFilter = $formFilter = new Sespageveroth_Form_Admin_Manage_Filter();
    $page = $this->_getParam('page', 1);

    $table = Engine_Api::_()->getDbtable('pages', 'sespage');
    $tableName = $table->info('name');

    $verificationsTable = Engine_Api::_()->getDbtable('verifications', 'sespageveroth');
...
//     if (!empty($values['displayname']))
         $select->join($verificationsTableName, "$tableName.page_id = $verificationsTableName.resource_id", array('verification_id', 'poster_id', 'resource_id', 'description', 'admin_approved', 'creation_date'));
//     else
...
//       $select->where($tableName.'.user_id = ?', (int) $values['user_id']);
  ...
  public function approveAction() {
    ...
    $this->view->form = $form = new Sespageveroth_Form_Admin_Approve();
    $item = Engine_Api::_()->getItem('sespageveroth_verification', $verification_id);

    $poster = Engine_Api::_()->getItem('user', $item->poster_id);
    $resource = Engine_Api::_()->getItem('sespage_page', $item->resource_id);
    ...
    if ($this->getRequest()->isPost()) {
      ...
      Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($poster, $viewer, $resource, 'sespageveroth_approve');

      Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($resourceOwner, $poster, $resource, 'sespageveroth_verified');
  ...
  public function deleteAction() {
    ...
    $this->view->form = $form = new Sespageveroth_Form_Admin_Delete();
  ...
  public function rejectAction() {
    ...
    $verification_id = $this->_getParam('verification_id');
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->form = $form = new Sespageveroth_Form_Admin_Reject();
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $resource = Engine_Api::_()->getItem('sespage_page', $item->resource_id);
        $item->delete();
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($poster, $viewer, $resource, 'sespageveroth_adminreject');
        $db->commit();
      } catch (Exception $e) {
        $db->rollBack();
        throw $e;
