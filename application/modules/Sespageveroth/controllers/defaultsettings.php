...
//       'order' => 999,
//       'params' => '{"showdetails":["button","badge","details"],"title":"","nomobile":"0","name":"sespageveroth.verify-button-badge"}',
//     ));
//   }
//   if($right_id) {
//     $db->insert('engine4_core_content', array(
...
foreach (Engine_Api::_()->getDbTable('levels', 'authorization')->fetchAll() as $level) {
  $form = new Sespageveroth_Form_Admin_Settings_Level(array(
      'public' => ( in_array($level->type, array('public')) ),
      'moderator' => ( in_array($level->type, array('admin', 'moderator')) ),
  ));
  $values = $form->getValues();
  $valuesForm = $permissionsTable->getAllowed('sespageveroth', $level->level_id, array_keys($form->getValues()));
  ...
  try {
    ...
    $permissionsTable->setAllowed('sespageveroth', $level->level_id, $values, '', $nonBooleanSettings);
    // Commit
    $db->commit();
  } catch (Exception $e) {
    $db->rollBack();
...
("sespageveroth.displaycomment", "1"),
("sespageveroth.enablecomment", "1"),
("sespageveroth.enableverification", "2"),
("sespageveroth.verifybadge", "0");');
