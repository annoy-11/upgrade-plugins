...
class Sesadvminimenu_Api_Message extends Core_Api_Abstract
{
  ...
  public function setUnreadMessage(Core_Model_Item_Abstract $user) {
    ...
    Engine_Api::_()->getDbtable('recipients', 'messages')->update(array('sesadvminimenu_read' => 1), array('`user_id` = ?' => $user->getIdentity(), 'sesadvminimenu_read = ?' => 0, 'inbox_read = ?' => 0));
  }

  public function setUnreadNotification(Core_Model_Item_Abstract $user) {
  
    Engine_Api::_()->getDbtable('notifications', 'activity')->update(array('view_notification' => 1, 'sesadvminimenu_read' => 1), array('user_id = ?' => $user->getIdentity(), 'sesadvminimenu_read = ?' => 0, 'view_notification = ?' => 0));
  }

  public function setUnreadFriendRequest(Core_Model_Item_Abstract $user) {

    Engine_Api::_()->getDbtable('notifications', 'activity')->update(array('view_notification' => 1, 'sesadvminimenu_read' => 1), array('user_id = ?' => $user->getIdentity(), 'sesadvminimenu_read = ?' => 0, 'mitigated = ?' => 0, 'view_notification = ?' => 0));
  }
