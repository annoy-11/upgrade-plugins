 ...
 * @version    $Id: Share.php 2017-07-29 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sessocialshare_Form_Share extends Engine_Form {
  ...
  public function init() {
    ...
    $getResults = Engine_Api::_()->getDbtable('integrateothermodules', 'sesbasic')->getResults(array('enabled' => 1, 'type' => 'socialshare'));

    $enabledModules = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();
    if($getResults) {
      //$allShareContent = array('' => 'Choose Share Type');
      foreach($getResults as $getResult) {
        $module_name = $getResult['module_name']; 
        if(in_array($module_name, $enabledModules )){
          $allShareContent[$getResult['content_type']] = $getResult['title'];
        }
      }
      ...
      if($allShareContent) {
        $this->addElement('Select', 'share_type', array(
          ...
          'allowEmpty' => false,
          'multiOptions' => $allShareContent,
          'onchange' => "sessocialshareType(this.value)",
          'value' => 'self_profile',
        ));
