 ...
 * @version    $Id: EpetitionFieldValueLoop.php 2019-11-07 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */

class Epetition_View_Helper_EpetitionFieldValueLoop extends Fields_View_Helper_FieldAbstract {

  public function epetitionFieldValueLoop($subject, $partialStructure) {
    ...
    if ($module == 'epetition' && $controller == 'admin-manage' && $action == 'view')
      $show_hidden = false;
    else
      ...
      if ($field->type == 'heading') {
        ...
        if (!empty($firstValue->value) && !empty($tmp)) {
          ...
          if (!$isHidden || $show_hidden) {
            ...
            $lastContents= "<span data-field-id={$field->field_id}>{$notice}<span>{$label}: </span><span>{$tmp}</span></span>";
          }
        }
      }
    }
  ...
  protected function _buildLastContents($content, $title) {
    ...
    return "<h4><span>{$title}: </span></h4>{$content}</div>";
  }
