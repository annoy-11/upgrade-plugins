 ...
 * @version    $Id: upgrade.tpl 2019-11-07 00:00:00 socialnetworking.solutions $
...
<?php 
$information = array('description'=>'Package Description','featured'=>'Featured','sponsored'=>'Sponsored','verified'=>'Verified','location'=>'Location','modules'=>'Modules','editor'=>'Rich Editor','custom_fields'=>'Custom Fields','petitioncount'=>'Petition Count');
$showinfo = array('description','featured','sponsored','verified','location','modules','editor','custom_fields','petitioncount');//Engine_Api::_()->getApi('settings', 'core')->getSetting('epetitionpackage.package.information', array_keys($information));
 ?>
<?php $currentCurrency =  Engine_Api::_()->epetition()->getCurrentCurrency(); ?>
<?php if($this->currentPackage){ 
	$package = Engine_Api::_()->getItem('epetitionpackage_package',$this->currentPackage->package_id);
  if($package){
?>
<div class="epetitionpackage_petition_list sesbasic_clearfix sesbasic_bxs">
	<div class="epetitionpackage_slect_plan_petition">
  <div class="heading_petition">
  	<p class="headding"><?php echo $this->translate("Current Package"); ?></p>
    </div>
    <div class="select">
    	<ul>
      <?php if(in_array('petitioncount',$showinfo)){ ?>
        <li><?php echo $this->translate("Petition Count"); ?></li>
     	<?php }else{ ?>
      	<li><?php echo $this->translate("Petition Left"); ?></li>
      <?php } ?>
  ...
  <div class="package_catogery_list">
    ...
    <?php 
            ...
            $curruncySymbol = str_replace('.','',preg_replace('/\d+/u', '', Engine_Api::_()->epetition()->getCurrencyPrice($package->price)));
     ?>
    	<li class="<?php ($package->highlight) ? 'active' : '' ?>">
      	<div class="package-ditals_petition">
        	<div class="headding_petition">
          	<div class="title">
            ...
            <div class="amount">
            	<?php if($package->price > 0){ ?>
            		<span><?php echo $curruncySymbol; ?></span><?php echo Engine_Api::_()->epetition()->getCurrencyPrice($package->price,'','',true); ?><br />
     ...
     ?>
    	<li class="<?php echo ($package->highlight) ? 'active' : '' ?>">
          ...
          <?php if(in_array('description',$showinfo)){ ?> 
            <p class="footer_discprition"><?php echo $this->translate($package->description); ?> </p>
          <?php } ?>
          <a href="<?php echo $this->url(array('epetition_id' => $this->petition->epetition_id,'action'=>'confirm-upgrade','package_id'=>$package->package_id), 'epetitionpackage_general', true); ?>" class="smoothbox">Upgrade Package</a>
          </div>
        </div>
      </li>      
    <?php } ?>
