 ...
 * @version    $Id: petition-role.tpl 2019-11-07 00:00:00 socialnetworking.solutions $
...
<div class="sesbasic_dashboard_content epetition_manage_role_form sesbm sesbasic_clearfix">
	<div class="epetition_manage_role_form_top sesbasic_clearfix">
		<p class="heading_desc"><?php echo $this->translate('Below, you can add members as admins to your petition who will be able create , edit, delete, configure petition settings.');?></p>
		<?php endif; ?>
		<form id="petition_admin_form" action="<?php echo $this->url(array('action' => 'save-petition-admin', 'epetition_id' => $this->petition->epetition_id), 'epetition_dashboard', true) ?>" method="post">
			<div id="manage_admin_input">
				<div class="epetition_manage_roles_item">
					<span class="show_img" id="show_default_img"></span>
          <div class="_input">
            <input type='text' id="petition_admin" name='petition_admin' size='20' placeholder='<?php echo $this->translate('Type Member Name') ?>' />
            <input type="hidden" id="user_id" name="petition_admins[]" value=""/>
					</div>
        </div>
			</div>
			<a href="javascript:void(0);" onclick="addMore();" class="sesbasic_button"><i class="fa fa-plus"></i>&nbsp;<?php echo $this->translate('Add Another Member');?></a>
			<button onclick="saveForm();return false;" id="save_button_admin" disabled><?php echo $this->translate("Save Admin"); ?></button>
	  </form>
<?php if(!$this->is_ajax){ ?>
  </div>
	<div class="epetition_footer_contant">
		<b><?php echo $this->translate('Admins');?></b>
		<p><?php echo $this->translate('Here, you can see a list of admins added by you for this petition.');?></p>
		<div id="manage_admin">
          ...
          <?php if($petitionAdmin->user_id != $this->petition->owner_id):?>
...
<script type="text/javascript">
	function showAutosuggest(petitionAdmin, imageId) {
	  var contentAutocomplete1 =  'contentAutocomplete-'+petitionAdmin
		contentAutocomplete1 = new Autocompleter.Request.JSON(petitionAdmin, "<?php echo $this->url(array('module' => 'epetition', 'controller' => 'dashboard', 'action' => 'get-members', 'epetition_id' => $this->petition->epetition_id), 'default', true) ?>", {
			'postVar': 'text',
			'minLength': 1,
			'selectMode': '',
	...
	en4.core.runonce.add(function() {
	  showAutosuggest('petition_admin','show_default_img');
	});
	
	function saveForm() {
	  var UserIds = $('user_id').value;
		new Request.HTML({
			url : en4.core.baseUrl + 'epetition/dashboard/save-petition-admin/epetition_id/' + <?php echo $this->petition->epetition_id ?>,
			method: 'post',
			data : {
				format : 'html',
				data: UserIds,
	...
	sesJqueryObject(document).on('keyup', 'input[id^="petition_admin"]', function(event) {
	...
	function addMore() {
		...
		if(!currentElem || !sesJqueryObject('#manage_admin_input').children().eq(itemCount).find('input').first().attr('rel'))
			return false;
	  var ColumnId = 'petition_admin_'+count;
	  sesJqueryObject('#manage_admin_input').append('<div class="epetition_manage_roles_item"><span class="show_img" id="show_default_img_'+count+'"'+'></span> <input type="text" placeholder="Type Member Name" size="20" name="'+ColumnId+'"' +'id="'+ColumnId+'"'+'autocomplete="off" rel="'+count+'"><a class="remove_icon" href="javascript:void(0);" onclick="removeInputForm('+"'"+ColumnId+"'"+');"><i class="fa fa-close" id="close_option_'+count+'"'+'></i></a></div>');
	  showAutosuggest('petition_admin_'+count, 'show_default_img_'+count);
	  count = count+1;
	}
  ...
  function removeUser(petitionId, roleId) {
		new Request.JSON({
			url : en4.core.baseUrl + 'epetition/dashboard/delete-petition-admin',
			method: 'post',
			data : {
				format : 'json',
				role_id: roleId,
				epetition_id: petitionId,
				is_ajax: 1,
			},
