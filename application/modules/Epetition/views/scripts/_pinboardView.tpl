 ...
 * @version    $Id: _pinboardView.tpl 2019-11-07 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */
 
?>
<li class="epetition_petition_pinboard_item sesbasic_bxs new_image_pinboard_<?php echo $randonNumber; ?>" style="width:<?php echo is_numeric($this->width_pinboard) ? $this->width_pinboard . 'px' : $this->width_pinboard ?>;">
    <div class="epetition_petition_pinboard_item_inner sesbasic_bg">
        <div class="epetition_petition_pinboard_item_top epetition_thumb sesbasic_clearfix">
            <div class="epetition_item_thumb epetition_listing_thumb">
              <?php $href = $item->getHref(); ?>
	            <?php if(isset($this->imageLabelActive) && !empty($photoPath)) { ?>
                <a href="<?php echo $href; ?>" data-url="<?php echo $item->getType() ?>" class="epetition_thumb_img" style="width:<?php echo is_numeric($this->width_pinboard) ? $this->width_pinboard . 'px' : $this->width_pinboard ?>;">
        ...
        <div class="epetition_item_info">
              ...
              <?php } ?>
                <div class="epetition_item_stats sesbasic_text_light">
                  <?php if (isset($this->likeActive) && isset($item->like_count)) { ?>
                      <span title="<?php echo $this->translate(array('%s like', '%s likes', $item->like_count), $this->locale()->toNumber($item->like_count)); ?>" class="epetition_like_count_<?php echo $item->epetition_id; ?>">
                          <i class="sesbasic_icon_like_o"></i><span><?php echo $item->like_count; ?></span></span>
                  <?php } ?>
          ...
          <?php if (isset($this->descriptionpinboardActive)) { ?>
              <div class="epetition_item_des clear">
                ...
                <?php if (isset($this->signatureLableActive) && isset($item->signature_goal) && !empty($item->signature_goal)) { ?>
                    <div class="epetition_signature_bar_container">
                        ...
                        <div class="epetition_signature_text"><b
                            ...
                            signed of <?php echo $item->signature_goal; ?> goal
                        </div>
                    </div>
                <?php } ?>
                  <div class="epetition_item_btns">
                    ...
                    <?php if ($user_check && (!isset($item->startdate) || (strtotime(date("Y-m-d H:i:s")) >= strtotime($item->startdate))) && (!isset($item->enddate) || (strtotime(date("Y-m-d H:i:s")) <= strtotime($item->enddate)))) { ?>
                        <div class="epetition_list_sign_btn"><a href="<?php echo $this->baseUrl()."/epetition/index/popsignpetition?id=".$item->epetition_id;  ?>"
                                class="epetition_button_outline sesbasic_animation sessmoothbox">Sign This</a></div>
                    <?php } } ?>
                    <?php if ((isset($this->socialSharingActive) && Engine_Api::_()->getApi('settings', 'core')->getSetting('epetition.enable.sharing', 1))): ?>
                      <?php $urlencode = urlencode(((!empty($_SERVER["HTTPS"]) && strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $item->getHref()); ?>
                        <div class="epetition_list_share_wrap">
                            <a href="javascript:void(0)" class="epetition_button_toggle epetition_button_outline sesbasic_animation"><i
      ...
      <?php if (isset($this->enableCommentPinboardActive)): ?>
        <?php $itemType = '';  ?>
        <?php if (isset($item->epetition_id)): ?>
          <?php $item_id = $item->epetition_id; ?>
          <?php $itemType = 'epetition'; ?>
        <?php endif; ?>
        <?php if (($itemType != '')): ?>
              <div class="epetition_item_comments">
