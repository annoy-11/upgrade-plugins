 ...
 * @version    $Id: _showMysignListGrid.tpl 2019-11-07 00:00:00 socialnetworking.solutions $
...
<?php if (!$this->is_ajax) { ?>
    <div class="sesbasic_view_type sesbasic_clearfix clear"
         style="display:<?php echo $this->bothViewEnable ? 'block' : 'none'; ?>;height:<?php echo $this->bothViewEnable ? '' : '0px'; ?>">
...
<div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" class="sesbasic_clearfix sesbasic_bxs clear">
    <ul class="epetition_listing sesbasic_clearfix clear"
        id="tabbed-widget_<?php echo $randonNumber; ?>" style="min-height:50px;">
      <?php }
      ?>
      <?php  $allsetting=$this->allParams;  ?>
      <?php foreach ($this->paginator as $key=>$signitem) {
          $item=Engine_Api::_()->getItem('epetition', $signitem['epetition_id']);
          ?>
        ...
        $category_setting=isset($allsetting['detailsdiplay']) && in_array('category',$allsetting['detailsdiplay']) ? 1 : 0;
        $petitiontitle_setting=isset($allsetting['detailsdiplay']) && in_array('petitiontitle',$allsetting['detailsdiplay']) ? 1 : 0;
        $signaturelocation_setting=isset($allsetting['detailsdiplay']) && in_array('signaturelocation',$allsetting['detailsdiplay']) ? 1 : 0;
        $statement_setting=isset($allsetting['detailsdiplay']) && in_array('category',$allsetting['detailsdiplay']) ? 1 : 0;
        $reason_setting=isset($allsetting['detailsdiplay']) && in_array('reason',$allsetting['detailsdiplay']) ? 1 : 0;
        $submissiondate_setting=isset($allsetting['detailsdiplay']) && in_array('submissiondate',$allsetting['detailsdiplay']) ? 1 : 0;
        ?>

        <?php $photoPath = $item->getPhotoUrl('thumb.normal'); ?>
        <?php if ($this->view_type == 'grid') { ?>
          <?php include APPLICATION_PATH . '/application/modules/Epetition/views/scripts/_mysigngrid.tpl'; ?>
        <?php } else if ($this->view_type == 'list') { ?>
          <?php include APPLICATION_PATH . '/application/modules/Epetition/views/scripts/_mysignlist.tpl'; ?>
        <?php }  ?>
      ...
      <?php if ($this->paginator->getTotalItemCount() == 0 && (empty($this->widgetType)) && $this->view_type != 'map') { ?>
        <?php if (isset($this->category) || isset($this->tag) || isset($this->text)): ?>
              <div class="tip">
      <span>
	<?php echo $this->translate('No Signature.'); ?>
  ...
  <?php if ($this->loadOptionData != 'pagging' && (empty($this->show_limited_data) || $this->show_limited_data == 'no')): ?>
      ...
      <div class="sesbasic_view_more_loading sesbasic_view_more_loading_<?php echo $randonNumber; ?>"
           id="loading_image_<?php echo $randonNumber; ?>" style="display: none;"><img
        ...
        sesJqueryObject(document).on('click', '.selectView_<?php echo $randonNumber; ?>', function () {
            ...
            requestTab_<?php echo $randonNumber; ?> = (new Request.HTML({
                ...
                'data': {
                    ...
                    is_ajax: 1,
                    settingdetails: '<?php echo json_encode($this->allParams); ?>',
