 ...
 * @version    $Id: _gridView.tpl 2019-11-07 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */
 
?>
<li class="epetition_petition_grid_item sesbasic_bxs"
    style="width:<?php echo is_numeric($this->width_grid) ? $this->width_grid . 'px' : $this->width_grid ?>;">
    <article class="sesbasic_bg">
        <div class="epetition_item_thumb epetition_listing_thumb"
             style="height:<?php echo is_numeric($this->height_grid) ? $this->height_grid . 'px' : $this->height_grid ?>;">
        ...
        <div class="epetition_item_info">
          ...
          <?php if (isset($this->descriptiongridActive)) { ?>
              <div class="epetition_item_des"> <?php echo $item->getDescription($this->description_truncation_grid); ?> </div>
          <?php } ?>


          <?php if (isset($this->signatureLableActive) &&!empty($item->signature_goal) && $item->signature_goal <= $signpet) { ?>

            <?php if ($this->victory == 1) {   //petition victory   ?>
            ...
            <?php }
          } else if ($this->victory == 2 && isset($this->signatureLableActive)) { ?>
              <div class="epetition-near-close">
                  <p><?php  echo $this->translate("This petition is closed"); ?></p>
              </div>
          <?php } else {  //petition open ?>
              <div class="epetition_item_footer">
                <?php if (isset($item->signature_goal) && !empty($item->signature_goal) && isset($this->signatureLableActive)) { ?>
                    <div class="epetition_signature_bar_container">
                        ...
                        <div class="epetition_signature_text"><b
                                    class="countincrease acv<?php echo $item->epetition_id; ?>">
                            <?php echo $signpet; ?></b> <?php echo $this->translate("signed of"); ?> <?php echo $item->signature_goal; ?> <?php echo $this->translate("goal"); ?>
                        </div>
                    </div>
                <?php } ?>
                  <div class="epetition_item_btns">
                    ...
                    <?php if ($user_check && (!isset($item->startdate) || (strtotime(date("Y-m-d H:i:s")) >= strtotime($item->startdate))) && (!isset($item->enddate) || (strtotime(date("Y-m-d H:i:s")) <= strtotime($item->enddate)))) { ?>
                        <div class="epetition_list_sign_btn"><a href="<?php echo $this->baseUrl()."/epetition/index/popsignpetition?id=".$item->epetition_id;  ?>"    class="epetition_button_outline sesbasic_animation sessmoothbox">Sign This</a></div>
                    <?php } } ?>
