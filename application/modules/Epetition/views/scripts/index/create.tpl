 ...
 * @version    $Id: create.tpl 2019-11-07 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */
 
?>
<?php
if(!$this->typesmoothbox){ ?>
  <?php
...
<?php } else { ?>
    <script type="application/javascript">
        ...
        Sessmoothbox.css.push("<?php echo $this->layout()->staticBaseUrl . 'application/modules/Epetition/externals/styles/styles.css'; ?>");
       ...
       // Sessmoothbox.javascript.push("<?php //echo $this->layout()->staticBaseUrl . 'externals/scripts/core.js'; ?>");

    </script>

<?php } ?>
<div id="displaycategory"  style="display: none"  >
<?php  $categories_before = Engine_Api::_()->getDbtable('categories', 'epetition')->getCategoriesAssoc(array('member_levels' => 1)); ?>
    <div class="epetition_create_step_container sesbasic_bxs sesbasic_clearfix">
        <h3>Start New Petition</h3>
        <p class="cat_select_text">It's easy to set up. Just choose a Petition category to get started.</p>
        <div class="epetition_create_categories_listing">
            <?php  foreach ($categories_before as $key=>$category)  { ?>
              <?php $categoryItem = Engine_Api::_()->getItem('epetition_category', $key);
	            $imageURL = $categoryItem->getPhotoUrl('../images/transprant-bg.png');
              ?>
            <div class="epetition_create_category">
                <section class="">
                    <div class="_inner">
                        <div class="_step1">
                            <a href="javascript:void(0);" class="sesbasic_linkinherit clickincategory" id="<?php echo $key;  ?>">
                                  <i style="background-image:url(<?php echo $imageURL;?>);"></i>
...
<div id="displayform" <?php if(($this->current_count < $this->quota) || empty($this->quota)) { if($this->quickCreate) { ?>  style="display: none"  <?php  } } ?> >
...
if ($mainPhotoEnable == 1) {
	$required = true; ?>
  <style type='text/css'>
  .epetition_create #tabs_form_petitioncreate-label label:after{content: '*';color: #F00;}
  </style>
<?php } else {
	$required = false;
}
...
<script type="text/javascript">
  ...
  en4.core.runonce.add(function() {
    ...
    sesJqueryObject('#epetition_start_time-wrapper').hide();
...
<?php if (($this->current_count >= $this->quota) && !empty($this->quota)):?>
  <div class="tip">
    <span>
      ...
      <?php echo $this->translate('If you would like to upload a new entry, please <a href="%1$s">delete</a> an old one first.', $this->url(array('action' => 'manage'), 'epetition_general'));?>
    </span>
  </div>
  <br/>
<?php else:?>
  <div class="epetition_create sesbasic_bxs">
...
<script type="text/javascript">
  ...
  jqueryObjectOfSes("#epetitions_create").bind("keypress", function (e) {
  ...
  function validateForm() {
    ...
    jqueryObjectOfSes('#epetitions_create input, #epetitions_create select,#epetitions_create checkbox,#epetitions_create textarea,#epetitions_create radio').each(
      function(index){
	...
	if(jqueryObjectOfSes(this).closest('div').parent().css('display') != 'none' && jqueryObjectOfSes(this).closest('div').parent().find('.form-label').find('label').first().hasClass('required') && jqueryObjectOfSes(this).prop('type') != 'hidden' && jqueryObjectOfSes(this).closest('div').parent().attr('class') != 'form-elements'){
	  ...
	  if(error){
	    ...
	    counter++
			...
			if(sesJqueryObject('#tabs_form_petitioncreate-wrapper').length && sesJqueryObject('.epetition_upload_item_photo').length == 0){
				<?php if($required):?>
					objectError = sesJqueryObject('.epetition_create_form_tabs');
					error = true;
				<?php endif;?>
			}
	  }
...
en4.core.runonce.add(function() {
  jqueryObjectOfSes('#epetitions_create').submit(function(e) {
    ...
    if(!validationFm) {
			...
			if(error != ''){
				sesJqueryObject('#epetition_error_time-wrapper').show();
				sesJqueryObject('#epetition_schedule_error_time-element').text(error);
			 var errorFirstObject = sesJqueryObject('#epetition_start_time-label').parent().parent();
			 sesJqueryObject('html, body').animate({
				scrollTop: errorFirstObject.offset().top
			 }, 2000);
				return false;
			}else{
				sesJqueryObject('#epetition_error_time-wrapper').hide();
			}
		}
    if(validationFm) {
     ...
     return false;
    }else if(sesJqueryObject('.epetition_upload_item_abort').length){
...
en4.core.runonce.add(function() {
  var epetition_create_form_tabsEpetition = jqueryObjectOfSes('#epetition_create_form_tabs li a');
  epetition_create_form_tabsEpetition.click(function() {
...
function createStatusbar(obj,file)
{
     ...
     if(rowCount %2 ==0) row ="even";
		  var checkedId = jqueryObjectOfSes("input[name=cover]:checked");
			this.objectInsert = jqueryObjectOfSes('<div class="epetition_upload_item sesbm '+row+'"></div>');
			this.overlay = jqueryObjectOfSes("<div class='overlay epetition_upload_item_overlay'></div>").appendTo(this.objectInsert);
			this.abort = jqueryObjectOfSes('<div class="abort epetition_upload_item_abort" id="abortPhoto_'+countUploadSes+'"><span><?php echo $this->translate("Cancel Uploading"); ?></span></div>').appendTo(this.objectInsert);
			this.progressBar = jqueryObjectOfSes('<div class="overlay_image progressBar"><div></div></div>').appendTo(this.objectInsert);
			this.imageContainer = jqueryObjectOfSes('<div class="epetition_upload_item_photo"></div>').appendTo(this.objectInsert);
			this.src = jqueryObjectOfSes('<img src="'+en4.core.baseUrl+'application/modules/Epetition/externals/images/blank-img.gif">').appendTo(this.imageContainer);
			this.infoContainer = jqueryObjectOfSes('<div class=epetition_upload_photo_info sesbasic_clearfix"></div>').appendTo(this.objectInsert);
			this.size = jqueryObjectOfSes('<span class="epetition_upload_item_size sesbasic_text_light"></span>').appendTo(this.infoContainer);
			this.filename = jqueryObjectOfSes('<span class="epetition_upload_item_name"></span>').appendTo(this.infoContainer);
			this.option = jqueryObjectOfSes('<div class="epetition_upload_item_options clear sesbasic_clearfix"><span class="epetition_upload_item_radio"><input type="radio" id="main_photo_id'+rowCount+'" name="cover"><label for="main_photo_id'+rowCount+'"><?php echo $this->translate("Main Photo"); ?></label></span><a class="edit_image_upload" href="javascript:void(0);"><i class="fa fa-pencil"></i></a><a class="delete_image_upload" href="javascript:void(0);"><i class="fa fa-trash"></i></a></div>').appendTo(this.objectInsert);
		...
		if (1) {
			if(jqueryObjectOfSes('#show_photo').children('div').length == 1) {
				var idPhoto = jqueryObjectOfSes('#show_photo').eq(0).find('.epetition_upload_item_radio').find('input').attr('id');
...
calculateWidthOfImageContainer();
    ...
    jqueryObjectOfSes(".clickincategory").click(function () {
        selectCat(jqueryObjectOfSes(this).attr('id'));
    });
});
...
function sendFileToServer(formData,status,file,isURL,i)
{
		...
		var url = '&isURL='+urlIs;
    var uploadURL = en4.core.baseUrl + 'epetition/photo/upload' + '?ul=1'+url; //Upload URL
    ...
    processData: false,
		...
		success: function(response){
					...
					if (response.status) {
							...
							status.option.find('.epetition_upload_item_radio').find('input').attr('value',response.photo_id);
...
var dragandrophandlerEpetition = sesJqueryObject('#dragandrophandler');
dragandrophandlerEpetition.click(function(){
	document.getElementById('file_multi').click();
});
...
jqueryObjectOfSes(document).on('click','.edit_image_upload',function(e){
  e.preventDefault();
  var photo_id = jqueryObjectOfSes(this).closest('.epetition_upload_item_options').attr('data-src');
  if(photo_id){
    editImage(photo_id);
  }else
    return false;
...
jqueryObjectOfSes(document).on('click','.delete_image_upload',function(e){
	e.preventDefault();
	jqueryObjectOfSes(this).parent().parent().find('.epetition_upload_item_overlay').css('display','block');
	var sesthat = this;
	var isCover = jqueryObjectOfSes(this).closest('.epetition_upload_item_options').find('.epetition_upload_item_radio').find('input').prop('checked');
	var photo_id = jqueryObjectOfSes(this).closest('.epetition_upload_item_options').attr('data-src');
	if(photo_id){
		request = new Request.JSON({
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'epetition', 'controller' => 'index', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'epetition', 'controller' => 'index', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
  ...
  $$('.core_main_epetition').getParent().addClass('active');
  sesJqueryObject(document).on('click','.icon_close',function(){
    Smoothbox.close();
  });

  function showhidedeadline()
  {
    var values=jqueryObjectOfSes("input[name=petition_deadline]:checked").val();
    if(parseInt(values)==1)
    {
     jqueryObjectOfSes('#epetition_deadline_time-wrapper').hide();
    }
    else
    {
     jqueryObjectOfSes('#epetition_deadline_time-wrapper').show();
    }
  }

  function showStartDate() {
    var values=jqueryObjectOfSes("input[name=show_start_time]:checked").val();
   if(parseInt(values)==1)
    {
     jqueryObjectOfSes('#epetition_start_time-wrapper').hide();
    }
    else
    {
     jqueryObjectOfSes('#epetition_start_time-wrapper').show();
    }
  }
...
<?php if($this->typesmoothbox) { ?>
	...
	function showHideOptionsEpetition(display){
		var elem = sesJqueryObject('.epetition_hideelement_smoothbox');
		for(var i = 0 ; i < elem.length ; i++){
				sesJqueryObject(elem[i]).parent().parent().css('display',display);
		}
	}
	function checkSettingEpetition(first){
		var hideShowOption = sesJqueryObject('#advanced_epetitionoptions').hasClass('active');
			if(hideShowOption){
					showHideOptionsEpetition('none');
					if(typeof first == 'undefined'){
						sesJqueryObject('#advanced_epetitionoptions').html("<i class='fa fa-plus-circle'></i><?php echo $this->translate('Show Advanced Settings') ?>");
					}
					sesJqueryObject('#advanced_epetitionoptions').removeClass('active');
			}else{
					showHideOptionsEpetition('block');
					sesJqueryObject('#advanced_epetitionoptions').html("<i class='fa fa-minus-circle'></i><?php echo $this->translate('Hide Advanced Settings') ?>");
						sesJqueryObject('#advanced_epetitionoptions').addClass('active');
			}
	}
	en4.core.runonce.add(function()
  {
		sesJqueryObject('#advanced_epetitionoptions').click(function(e){
			checkSettingEpetition();
		});
		sesJqueryObject('#advanced_epetitionoptions').html("<i class='fa fa-plus-circle'></i><?php echo $this->translate('Show Advanced Settings') ?>");
		checkSettingEpetition('true');

		tinymce.init({
			mode: "specific_textareas",
			plugins: "table,fullscreen,media,psignature,paste,code,image,textcolor,jbimages,link",
			theme: "modern",
			menubar: false,
			statusbar: false,
			toolbar1:  "undo,redo,removeformat,pastetext,|,code,media,image,jbimages,link,fullscreen,psignature",
...
<script type="application/javascript">
    sesJqueryObject(window).load(function () {
       var display='<?php echo $this->quickCreate;  ?>';
       if(parseInt(display)==1)
       {
           sesJqueryObject("#displaycategory").show();
           sesJqueryObject("#displayform").hide();
       }
       else
       {
