...
class Epetition_Installer extends Engine_Package_Installer_Module {
  ...
  public function onPostInstall() {
    ...
    $singularWord = $db->select()
      ...
      ->where('name = ?', 'epetition.text.singular')
      ->limit(1)
      ->query()
      ->fetchColumn();
    if (!$singularWord)
      $singularWord = 'Petition';

    $pluralWord = $db->select()
      ->from('engine4_core_settings', 'value')
      ->where('name = ?', 'epetition.text.plural')
      ->limit(1)
      ->query()
      ->fetchColumn();
    if (!$pluralWord)
      $pluralWord = 'petitions';

    $oldSigularWord = 'petition';
    $oldPluralWord = 'petitions';
