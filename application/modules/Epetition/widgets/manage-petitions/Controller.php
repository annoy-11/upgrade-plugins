...
class Epetition_Widget_ManagePetitionsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $searchArray=$this->_getAllParams();
    ...
    if (!$is_ajax && is_array($defaultOptionsArray)) {
      foreach ($defaultOptionsArray as $key => $defaultValue) {
				if(!Engine_Api::_()->getApi('settings', 'core')->getSetting('epetition.enable.favourite', 1) && $defaultValue == 'mostSPfavourite')
					continue;
    ...
    $this->view->widgetName = 'manage-petitions';			
    $params = array_merge($params, $value, array('search'=>addslashes($text)));

    // Get Petitions
    $paginator = Engine_Api::_()->getDbtable('epetitions', 'epetition')->getEpetitionsPaginator($value);
