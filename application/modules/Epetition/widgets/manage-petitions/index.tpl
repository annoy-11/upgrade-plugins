      ...
      <?php if($this->tab_option == 'advance'){ ?>
        ...
        <ul>
          ...
          foreach ($this->defaultOptions as $key => $valueOptions) {
            ...
            ?>
              <li <?php if ($this->defaultOpenTab == $key){ ?> class="active" <?php } ?>
                  id="sesTabContainer_<?php echo $randonNumber; ?>_<?php echo $key; ?>">
                  <a href="javascript:void(0)" data-src="<?php echo $key; ?>"
                     onclick='changeTabSes_<?php echo $randonNumber; ?>("<?php echo $key; ?>")'><?php echo $this->translate(($valueOptions)); ?></a>
              </li>
          <?php } ?>
        </ul>
      <?php //echo $this->content()->renderWidget('epetition.browse-menu-quick',array()); ?>
    </div>
          ...
          function changeTabSes_<?php echo $randonNumber; ?>(valueTab) {
              ...
              if (valueTab) {
                  ...
                  requestTab_<?php echo $randonNumber; ?> = new Request.HTML({
                      method: 'post',
                      'url': en4.core.baseUrl + "widget/index/mod/epetition/name/<?php echo $this->widgetName; ?>/openTab/" + valueTab,
                      'data': {
                          format: 'html',
                      ...
                      onSuccess: function (responseTree, responseElements, responseHTML, responseJavaScript) {
                          ...
                          if (sesJqueryObject('.pin_selectView_<?php echo $randonNumber;?>').hasClass('active')) {
                              if (document.getElementById('epetition_pinboard_view_<?php echo $randonNumber;?>'))
                                  document.getElementById('epetition_pinboard_view_<?php echo $randonNumber;?>').style.display = 'block';
