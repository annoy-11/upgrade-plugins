...
class Epetition_Widget_PetitionsMysignatureController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
    $this->view->allParams = $t = $this->_getAllParams();
    if (isset($_POST['settingdetails']) && !empty($_POST['settingdetails'])) {
      $g = $this->view->allParams = json_decode($_POST['settingdetails'], true);
    }
    $searchArray = array();
    ...
    $show_criterias = isset($params['show_criterias']) ? $params['show_criterias'] : $this->_getParam('show_criteria', array('like', 'comment', 'by', 'title', 'featuredLabel', 'sponsoredLabel', 'verifiedLabel', 'category', 'description_list', 'description_grid', 'description_pinboard', 'favouriteButton', 'likeButton', 'socialSharing', 'view', 'creationDate', 'readmore'));
    ...
    $type = '';
    $page_id = Engine_Api::_()->epetition()->getWidgetPageId($identityForWidget = 1448);
    if ($page_id) {
    ...
    $value['status'] = 1;
    $value['search'] = 1;
    $this->view->widgetName = 'petitions-mysignature';
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('signatures', 'epetition')->getSignaturesPaginator(array(
      'owner_id' => $viewer_id,
