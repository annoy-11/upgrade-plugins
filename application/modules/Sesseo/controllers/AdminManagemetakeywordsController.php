 ...
 * @version    $Id: AdminManagemetakeywordsController.php  2019-03-18 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesseo_AdminManagemetakeywordsController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesseo_admin_main', array(), 'sesseo_admin_main_managemetakeywords');

    $this->view->formFilter = $formFilter = new Sesseo_Form_Admin_Manage_Filter();
    ...
    $select = Engine_Api::_()->getDbtable('managemetatags', 'sesseo')->select();
  ...
  public function addAction() {
    ...
    $this->view->form = $form = new Sesseo_Form_Admin_Manage_Add();

    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {

      $values = $form->getValues();

      $managemetatagsTable = Engine_Api::_()->getDbtable('managemetatags', 'sesseo');
  ...
  public function enabledAction() {
    ...
    if (!empty($id)) {
      $item = Engine_Api::_()->getItem('sesseo_managemetatag', $id);
      $item->enabled = !$item->enabled;
      $item->save();
    }
    $this->_redirect('admin/sesseo/managemetakeywords');
  }
  ...
  public function editAction() {
    ...
    $managemetatagTable = Engine_Api::_()->getItem('sesseo_managemetatag', $this->_getParam('managemetatag_id'));

    $page_id = $this->_getParam('page_id', null);
    $corePages = Engine_Api::_()->getItem('core_page', $page_id);

    $form = $this->view->form = new Sesseo_Form_Admin_Manage_Edit();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      try {
        ...
        $corePages->title = $values['meta_title'];
        $corePages->description = $values['meta_description'];
        $corePages->keywords = $values['meta_keywords'];
        $corePages->save();
  ...
  public function setPhoto($photo, $param = null) {
    ...
    $params = array(
        'parent_type' => 'sesseo_managemetakeywords',
  ...
  public function deleteAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db->beginTransaction();
      try {
        $managemetatag = Engine_Api::_()->getItem('sesseo_managemetatag', $this->_getParam('managemetatag_id'));
