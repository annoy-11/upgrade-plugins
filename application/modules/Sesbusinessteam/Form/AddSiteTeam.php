 ...
 */
class Sesbusinessteam_Form_AddSiteTeam extends Engine_Form {
  ...
  public function init() {
    ...
    $business_id = Zend_Controller_Front::getInstance()->getRequest()->getParam('business_id');

    $this
            ->setMethod('POST')
            ->setAttrib('name', 'sesbusinessteam_addteam')
    ...
    if ($type == 'nonsitemember') {
      ...
      $team = Engine_Api::_()->getItem('sesbusinessteam_team', $team_id);
      $photo_id = 0;
    ...
    $designations = Engine_Api::_()->getDbtable('designations', 'sesbusinessteam')->getDesignations();

    $allBusinessDesignations = Engine_Api::_()->getDbtable('designations', 'sesbusinessteam')->getAllDesignations(array('business_id' => $business_id));
    if(count($allBusinessDesignations) > 0) {
      foreach ($allBusinessDesignations as $designation) {
    ...
    $this->addElement('Textarea', "description", array(
        ...
        'description' => "Enter short description about the team member. [The description should be of 300 characters only. This description would be shown on the Team Business.]",
        'maxlength' => '300',
        'filters' => array(
            'StripTags',
    ...
    $this->addElement('TinyMce', 'detail_description', array(
        ...
        'description' => 'Enter detailed description about the team member. [This description would be shown on the Member Profile business.]',
        'editorOptions' => $editorOptions,
    ));

    $this->addElement('Text', "email", array(
        'label' => 'Email',
        'description' => "Enter email address of the team member which will be displayed on the team business.",
    ));
    ...
    $this->addElement('Text', "facebook", array(
        'label' => 'Facebook URL',
        'description' => "Enter URL of the Facebook Profile or Business of the team member.",
    ));
