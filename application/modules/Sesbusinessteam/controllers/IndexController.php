...
class Sesbusinessteam_IndexController extends Core_Controller_Action_Standard {
  ...
  public function getDataAction() {
    ...
    if($business_id) {
      $table = Engine_Api::_()->getDbTable('teams', 'sesbusinessteam');
      $select = $table->select()
  ...
  public function editAction() {
    ...
    if(!$is_ajax) {
      // Prepare form
      $this->view->form = $form = new Sesbusinessteam_Form_EditSiteTeam();
      // Populate form
    ...
    if($is_ajax) {
      ...
      try {
        if ($_POST["designation_id"]) {
          ...
          $designation = Engine_Api::_()->getItem('sesbusinessteam_designations', $_POST["designation_id"])->designation;
        ...
        if (isset($_FILES['photo_id']['name']) && $_FILES['photo_id']['name'] != '') {
          ...
          $filename = $storage->createFile($_FILES['photo_id'], array(
            ...
            'parent_type' => 'sesbusinessteam_team',
            'user_id' => $viewer_id,
          ));
        ...
        $paginator = Engine_Api::_()->getDbTable('teams', 'sesbusinessteam')->getTeamMemers(array('business_id' => $business->business_id));
        $showData =  $this->view->partial('_teams.tpl','sesbusinessteam',array('paginator' => $paginator, 'viewer_id'=> $viewer_id, 'business_id' => $business->business_id, 'is_ajax'=>true));
  ...
  public function addAction() {
    ...
    if(!$is_ajax) {
      //Render Form
      $this->view->form = $form = new Sesbusinessteam_Form_AddSiteTeam();
    ...
    if($is_ajax) {
      ...
      try {
        ...
        if ($values["designation_id"]) {
          ...
          $designation = Engine_Api::_()->getItem('sesbusinessteam_designations', $values["designation_id"])->designation;
  ...
  public function deleteAction() {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->team_id = $team_id = $this->_getParam('team_id');
    $item = Engine_Api::_()->getItem('sesbusinessteam_team', $team_id);
    if(!$is_ajax) {
      $this->view->form = $form = new Sesbusinessteam_Form_Delete();
    }
    if($is_ajax) {
  ...
  public function addDesignationAction() {
    ...
    if(!$is_ajax) {
      //Render Form
      $this->view->form = $form = new Sesbusinessteam_Form_Adddesignation();
    ...
    if($is_ajax) {
      ...
      try {
        ...
        $row->save();
        $db->commit();
        $paginator = Engine_Api::_()->getDbTable('designations', 'sesbusinessteam')->getAllDesignations(array('business_id' => $business->business_id));
        $showData =  $this->view->partial('_designations.tpl','sesbusinessteam',array('designations' => $paginator, 'viewer_id'=> $viewer_id, 'business_id' => $business->business_id, 'is_ajax'=>true));
        echo Zend_Json::encode(array('status' => 1, 'message' => $showData));exit();
      } catch (Exception $e) {
        $db->rollBack();
        throw $e;
  ...
  public function editDesignationAction() {
    ...
    $this->view->designation = $designation = Engine_Api::_()->getItem('sesbusinessteam_designations', $designation_id);

    if(!$is_ajax) {
      // Prepare form
      $this->view->form = $form = new Sesbusinessteam_Form_Editdesignation();
      // Populate form
  ...
  public function deleteDesignationAction() {
    ...
    $this->view->is_ajax = $is_ajax = $this->_getParam('is_ajax', 0);
    $this->view->designation_id = $designation_id = $this->_getParam('designation_id');
    $item = Engine_Api::_()->getItem('sesbusinessteam_designations', $designation_id);
    if(!$is_ajax) {
      $this->view->form = $form = new Sesbusinessteam_Form_DeleteDesignation();
    }
    if($is_ajax) {
      $db = $item->getTable()->getAdapter();
      $db->beginTransaction();
      try {
        ...
        $db->update('engine4_sesbusinessteam_teams', array('designation_id' => '', 'designation' => ''), array('designation_id = ?' => $designation_id));
        $item->delete();
        $db->commit();
      } catch( Exception $e ) {
        $db->rollBack();
