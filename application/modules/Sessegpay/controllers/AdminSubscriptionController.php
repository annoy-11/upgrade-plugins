 ...
 * @version    $Id: AdminSubscriptionController.php  2019-02-02 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sessegpay_AdminSubscriptionController extends Core_Controller_Action_Admin
{
  public function indexAction()
  {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sessegpay_admin_main', array(), 'sessegpay_admin_main_subscriptions');
    ...
    // Make form
    $this->view->formFilter = $formFilter = new Sessegpay_Form_Admin_Subscription_Filter();
    ...
    $tableName = Engine_Api::_()->getDbtable('gateways', 'payment');
    $selectTable = $tableName->select()->where('plugin =?','Sessegpay_Plugin_Gateway_Segpay');
    $res = $tableName->fetchRow($selectTable);
    $gatId = 0;
    if($res){
    ...
    $select->joinRight('engine4_payment_orders', 'engine4_payment_orders.source_id=engine4_payment_subscriptions.subscription_id AND source_type = "payment_subscription"', null)
      ->where('engine4_payment_orders.gateway_id =?',$gatId);
    if( !empty($filterValues['order']) ) {
      if( empty($filterValues['direction']) ) {
        $filterValues['direction'] = 'DESC';
      }
