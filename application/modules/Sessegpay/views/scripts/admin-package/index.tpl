    ...
    <tbody>
      <?php foreach( $this->paginator as $item ): ?>
        <tr>
          ...
          <td class="nowrap">
            <?php echo $item->type == 0 ? $item->getPackageDescription(): $this->locale()->toCurrency($item->initial_price, $currency) .' for '.$item->initial_length.' days then '.$this->locale()->toCurrency($item->recurring_price, $currency).' every '.$item->recurring_length.' days' ; ?>
          </td>
          <td class='admin_table_centered'>
