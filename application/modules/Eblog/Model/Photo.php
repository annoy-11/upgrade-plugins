 ...
 */
class Eblog_Model_Photo extends Core_Model_Item_Collectible {

  protected $_parent_type = 'eblog_album';
  protected $_owner_type = 'user';
  protected $_collection_type = 'eblog_album';
...
//   }
  ...
  public function getAlbum() {
    return Engine_Api::_()->getItem('eblog_album', $this->album_id);
  }
  ...
  public function getAlbumPhotoUrl($type = null) {
    ...
    if (!$photo_id) {
      return 'application/modules/Eblog/externals/images/nophoto_album_thumb_normal.pngc=direct';
    }
  ...
  public function getEblog() {
    return Engine_Api::_()->getItem('eblog', $this->blog_id);
  ...
  public function getAuthorizationItem() {
    return $this->getParent('eblog_blog');
  }
  ...
  public function setPhoto($photo,$isURL = false,$isUploadDirect = false,$viewer = false)
  {
    ...
    $main_height = $core_settings->getSetting('eblog.mainheight', 1600);
    $main_width = $core_settings->getSetting('eblog.mainwidth', 1600);
    $normal_height = $core_settings->getSetting('eblog.normalheight', 500);
    $normal_width = $core_settings->getSetting('eblog.normalwidth', 500);
    // Resize image (main)
    $mainPath = $path . DIRECTORY_SEPARATOR . $base . '_m.' . $extension;
    $image = Engine_Image::factory();
    $image->open($file)
    ...
    if(!$isUploadDirect){
      $enableWatermark = $core_settings->getSetting('eblog.watermark.enable', 0);
...
//      $fileName = $photo;
//    } else {
//      throw new Eblog_Model_Exception('invalid argument passed to setPhoto');
//    }
//
//    if (!$fileName) {
...
//  }
  ...
  protected function _postDelete() {
    ...
    try {
      ...
      $blogItem = Engine_Api::_()->getItem("eblog_blog", $this->blog_id);
      ...
      if (isset($blogItem)) {
        $photoItem = Engine_Api::_()->getItem("eblog_photo", $blogItem->photo_id);
        if (!$photoItem) {
          $blogItem->photo_id = 0;
          $blogItem->save();
        }
