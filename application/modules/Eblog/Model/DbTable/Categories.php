 ...
 */
class Eblog_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Eblog_Model_Category';
  ...
  public function getCategoriesAssoc($params = array()) {
    ...
    if ($viewerId && isset($params['member_levels']) && $params['member_levels'] == 1) {
      ...
      $select->where('CONCAT(engine4_eblog_categories.member_levels," ") LIKE "%' . $levelId . '%"');
    }
  ...
  public function getUserCategoriesAssoc($user) {
    ...
    $stmt = $this->getAdapter()
            ->select()
            ->from('engine4_eblog_categories', array('category_id', 'category_name'))
            ->joinLeft('engine4_eblog_blogs', "engine4_eblog_blogs.category_id = engine4_eblog_categories.category_id")
            ->group("engine4_eblog_categories.category_id")
            ->where('engine4_eblog_blogs.owner_id = ?', $user)
            ->where('engine4_eblog_blogs.draft = ?', "0")
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countBlogs'])) {
      $eblogTable = Engine_Api::_()->getDbTable('blogs', 'eblog')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($eblogTable, "$eblogTable.subcat_id=$tableName.category_id", array("total_blogs_categories" => "COUNT(blog_id)"));
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countBlogs'])) {
      $eblogTable = Engine_Api::_()->getDbTable('blogs', 'eblog')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($eblogTable, "$eblogTable.subsubcat_id=$tableName.category_id", array("total_blogs_categories" => "COUNT(blog_id)"));
  ...
  public function getCategory($params = array(), $customParams = array(), $searchParams = array()) {
    ...
    $blogTable = Engine_Api::_()->getDbTable('blogs', 'eblog')->info('name');
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('eblog_category');
  ...
  public function hasCategoryId($params = array()) {
    ...
    $category_select = $this->select()
            ->from($tableName, 'category_id')
            ->where('seblog_categoryid = ?', $params['seblog_categoryid']);
    return $category_select->query()->fetchColumn();
  }
