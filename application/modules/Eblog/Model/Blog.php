 ...
 * @version    $Id: Blog.php 2016-07-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Eblog_Model_Blog extends Core_Model_Item_Abstract
{
  // Properties
  ...
  public function getHref($params = array()) {
    ...
    $table = Engine_Api::_()->getItemTable('eblog_album');
    $select = $table->select()
	...
	public function cancel()
  {
	 ...
	 if($this->transaction_id){
		...
		if($blog){
			...
			$orderpackage = Engine_Api::_()->getItem('eblogpackage_orderspackage',$this->orderspackage_id);
    ...
    if( !empty($transaction->gateway_id) && !empty($transaction->gateway_profile_id) && empty($transaction->gateway_transaction_id) ) {
      try {
        ...
        $gateway = Engine_Api::_()->getItem('eblogpackage_gateway', $transaction->gateway_id);
        if( $gateway ) {
  ...
  public function getPackage(){
		return Engine_Api::_()->getItem('eblogpackage_package',$this->package_id);
	}
	 public function getTransaction(){
		return Engine_Api::_()->getItem('eblogpackage_transaction',$this->transaction_id);
	}
	 // Events
  ...
  public function setActive($flag = true, $deactivateOthers = null)
  {
	...
	public function changeApprovedStatus($approved = 0){
		...
		if($transaction && $orderPackageId){
				...
				$this->save();
				Engine_Api::_()->getDbtable('blogs', 'eblog')->update(array('is_approved' =>$approved),array('orderspackage_id'=>$orderPackageId));
		}			
	}
  ...
  public function getPhotoUrl($type = null) {
    ...
    if ($photo_id) {
      $file = Engine_Api::_()->getItemTable('storage_file')->getFile($this->photo_id, $type);
		...
		$defaultPhoto = Zend_Registry::get('StaticBaseUrl').$settings->getSetting('eblog_blog_default_photo', 'application/modules/Eblog/externals/images/nophoto_blog_thumb_profile.png');
		return $defaultPhoto;
  }
