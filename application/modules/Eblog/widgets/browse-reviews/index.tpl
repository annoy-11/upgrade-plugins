...
<?php if( $this->paginator->getTotalItemCount() > 0 ){ ?>
  <?php foreach( $this->paginator as $item ): ?>
    <?php $reviewer = Engine_Api::_()->getItem('eblog_blog', $item->blog_id);?>
    <li class="sesbasic_clearfix">
    	<div class="eblog_review_listing_left">
      	<div class="eblog_review_listing_left_photo">
          <?php echo $this->htmlLink($item->getOwner()->getHref(), $this->itemPhoto($item->getOwner(), 'thumb.profile')) ?>
        </div>
        <p class="eblog_review_listing_left_title"><?php echo $this->htmlLink($item->getOwner()->getHref(), $item->getOwner()->getTitle()) ?></p>
          <?php if(isset($this->sponsoredLabelActive) && $item->sponsored):?>
              <div class="eblog_review_sponsored_block">
                  <p><?php echo $this->translate('Sponsored');?></p>
              </div>
          <?php endif;?>
        <?php if(isset($this->featuredLabelActive) && $item->featured):?>
	  <div class="eblog_review_featured_block">
	  <p><?php echo $this->translate('Featured');?></p>
	  </div>
	<?php endif;?>
	<?php if(isset($this->verifiedLabelActive) && $item->verified):?>
	  <div class="eblog_review_verified_block">
	    <p><?php echo $this->translate('Verified');?></p>
	  </div>
	<?php endif;?>
      </div>
      <div class="eblog_review_listing_right sesbasic_clearfix">
        <div class="eblog_review_listing_top sesbasic_clearfix">
          <?php if(in_array('title', $this->stats)): ?>
            <div class='eblog_review_listing_title sesbasic_clearfix'>
        ...
        <div class="eblog_review_listing_top_info sesbasic_clearfix">
          ...
          <div class='eblog_review_listing_top_info_cont'>
            <?php if(in_array('postedBy', $this->stats) || in_array('creationDate', $this->stats)): ?>
              <p class="eblog_review_listing_stats">
                <?php if(in_array('postedBy', $this->stats)): ?>
            ...
            <p class="eblog_review_listing_stats sesbasic_text_light">
          ...
          <div class="eblog_review_show_rating review_ratings_listing">
            ...
            <?php if(in_array('parameter', $this->stats)){ ?>
              <?php $reviewParameters = Engine_Api::_()->getDbtable('parametervalues', 'eblog')->getParameters(array('content_id'=>$item->getIdentity(),'user_id'=>$item->owner_id)); ?>
              <?php if(count($reviewParameters)>0){ ?>
                <div class="eblog_review_show_rating_box sesbasic_clearfix">
            <?php foreach($reviewParameters as $reviewP){ ?>
              <div class="sesbasic_clearfix">
                <div class="eblog_review_show_rating_label"><?php echo $reviewP['title']; ?></div>
                <div class="eblog_review_show_rating_parameters sesbasic_rating_parameter sesbasic_rating_parameter_small">
        ...
        <div class="eblog_review_listing_desc sesbasic_clearfix">
          ...
          <?php endif; ?>
					<?php if(in_array('customfields', $this->stats)): ?>
						<?php $customFieldsData = Engine_Api::_()->eblog()->getCustomFieldMapData($item);?>
						<?php if(count($customFieldsData) > 0):?>
							<?php foreach($customFieldsData as $valueMeta):?>
								<?php if(!$valueMeta['value']):?>	
									<?php continue;?>
								<?php endif;?>
								<?php echo '<p class="eblog_review_view_body"><b class="label">'. $valueMeta['label']. ': </b>'.
								$valueMeta['value'].'</p>';?>
							<?php endforeach;?>   
						<?php endif; ?>
					<?php endif; ?>
	  ...
	  <?php if(in_array('recommended', $this->stats)):?>
	    <p class="eblog_review_listing_recommended"> <b><?php echo $this->translate('Recommend Blog');?><i class="<?php if($item->recommended):?>fa fa-check<?php else:?>fa fa-times<?php endif;?>"></i></b></p>
	  <?php endif;?>
          <p class="eblog_review_listing_more">
            <a href="<?php echo $item->getHref()?>" class=""><?php echo $this->translate('Continue Reading »');?></a>
          </p>
        </div>
        <?php  echo $this->partial('_reviewOptions.tpl','eblog',array('subject'=>$item,'viewer'=>$this->viewer(),'stats'=>$this->stats)); ?>
      </div>
    </li>
  <?php endforeach; ?>
	...
	function viewMoreHide_<?php echo $randonNumber; ?>() {
	 ...
	 function viewMore_<?php echo $randonNumber; ?> () {
			...
			requestViewMore_<?php echo $randonNumber; ?> = new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + "widget/index/mod/eblog/name/browse-reviews",
				'data': {
		format: 'html',
