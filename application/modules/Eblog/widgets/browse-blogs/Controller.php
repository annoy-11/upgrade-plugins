 ...
 */
class Eblog_Widget_BrowseBlogsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $form = new Eblog_Form_Search(array('searchTitle' => 'yes','browseBy' => 'yes','categoriesSearch' => 'yes','searchFor'=>'blog','FriendsSearch'=>'yes','defaultSearchtype'=>'mostSPliked','locationSearch' => 'yes','kilometerMiles' => 'yes','hasPhoto' => 'yes'));
    ...
    $this->view->widgetParams = $widgetParams = Engine_Api::_()->eblog()->getWidgetParams($identityForWidget);
    ...
    $this->view->can_create = Engine_Api::_()->authorization()->isAllowed('eblog_blog', null, 'create');
    if (!$is_ajax) {
     // Make form	
    ...
    $type = '';
    $page_id = Engine_Api::_()->eblog()->getWidgetPageId($identityForWidget);
    if($page_id) {
      ...
      if($pageName) {
        ...
        if(is_numeric($explode[1])) {
          $type = Engine_Db_Table::getDefaultAdapter()->select()
                ->from('engine4_eblog_integrateothermodules', 'content_type')
    ...
    $value['visible'] = "1";
    // Get blogs
    $paginator = Engine_Api::_()->getDbtable('blogs', 'eblog')->getEblogsPaginator($params, $form->getValues());
