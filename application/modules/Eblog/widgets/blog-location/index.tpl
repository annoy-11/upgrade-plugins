...
<?php $randonNumber = "location_widget_eblog"; ?>
  ...
  <?php $location = '';?>
	<?php if($blog->lat): ?>
		...
		<?php if(isset($this->verifiedLabelActive) && $blog->verified == 1) :?>
			<?php $vlabel = "<div class=\"eblog_verified_label\" title=\"VERIFIED\"><i class=\"fa fa-check\"></i></div>";?>
		<?php endif;?>
    ...
    <?php $socialShare = $this->partial('_socialShareIcons.tpl','sesbasic',array('resource' => $blog, 'param' => 'feed', 'socialshare_enable_plusicon' => $this->socialshare_enable_plusicon, 'socialshare_icon_limit' => $this->socialshare_icon_limit)); ?>
     ...
     <?php  $socialshare = '<div class="eblog_list_grid_thumb_btns">'.$socialShare.$likeButton.$favouriteButton.'</div>';?>
    <?php else:?>
...
<?php if(!$this->is_ajax){ ?>
  ...
  <script type="text/javascript">
	...
	'<div class="eblog_large_map_content_title">'+markerArrayData_<?php echo $randonNumber?>[i]['title']+'</div>'+ owner + location+stats +'<div class="eblog_list_stats_btn clearfix clear"><span>'+'</span><span>'+'</span><span>'+'</span><span>'+'</span></div></div></div>'+'<a class="icn close" href="javascript:;" title="Close"><i class="fa fa-close"></i></a>' + 
	'</div>' +
	'</div>' +
    ...
    function callNewMarkersAjax(){
      (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "widget/index/mod/eblog/name/<?php echo $this->widgetName; ?>",
      'data': {
	format: 'html',
	is_ajax : 1,
...
<?php if(!$this->is_ajax){ ?>
	...
	<div id="map-data_location_widget_eblog" style="display:none;"><?php echo json_encode($locationArray,JSON_HEX_QUOT | JSON_HEX_TAG); ?></div>
	<div id="map-canvas-location_widget_eblog" class="map sesbasic_large_map sesbm sesbasic_bxs eblog_browse_map"></div>
