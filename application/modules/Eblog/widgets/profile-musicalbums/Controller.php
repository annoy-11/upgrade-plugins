...
class Eblog_Widget_ProfileMusicalbumsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($_POST['is_ajax'])) {
      ...
      if (!$coreApi->hasSubject('eblog_blog'))
        return $this->setNoRender();

      $subject_blog = $coreApi->getSubject('eblog_blog');
      $this->view->blog_id = $subject_blog->blog_id;

      $subject = $coreApi->getSubject();
			
			$this->view->allow_create = true;
			if(isset($value['is_ajax']) && !$value['is_ajax'] && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('eblogpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblogpackage.enable.package', 1)){
			...
			$this->view->canCreate  = Engine_Api::_()->eblog()->isBlogAdmin($subject, 'music_create');
    }
    ...
    $select = $table->select()
            ...
            ->where($tableName . '.resource_type = ?', 'eblog_blog')
