 ...
 */
class Eblog_Widget_photoViewPageController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
		...
		if(Engine_Api::_()->core()->hasSubject('eblog_photo') && !$is_ajax)
	 	 $photo = Engine_Api::_()->core()->getSubject('eblog_photo');
		else if(isset($_POST['photo_id'])){
		 $photo = Engine_Api::_()->getItem('eblog_photo',$_POST['photo_id']);
		 Engine_Api::_()->core()->setSubject($photo);
		 $photo = Engine_Api::_()->core()->getSubject();
		}else
			 return $this->setNoRender();

		$this->view->blog =	$blog = Engine_Api::_()->getItem('eblog_blog', $photo->blog_id);
	  ...
	  if($viewer->getIdentity()>0){
			...
			$this->view->canCommentMemberLevelPermission = Engine_Api::_()->authorization()->getPermission($viewer, 'eblog_blog', 'comment');
		}
