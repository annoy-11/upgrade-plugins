	...
	<div class="sesbasic_cloud_widget sesbasic_clearfix">  
		...
		<ul>
			...
			<?php foreach($this->paginator as $value):?> 	
				...
				<?php endif;?>	
				<li>
					<a title="<?php echo $value['category_name'] ?>" href="<?php echo $this->url(array('module' =>'eblog','controller' => 'index', 'action' => 'browse'),'eblog_general',true).'?category_id='.$value['category_id']; ?>">
...
<?php elseif($this->showType = 'simple'):?>  
	<ul class="eblog_sidebar_categroies_list">    
		<?php foreach( $this->paginator as $item ): ?>
			<li>
				<?php $subcategory = Engine_Api::_()->getDbtable('categories', 'eblog')->getModuleSubcategory(array('column_name' => "*", 'category_id' => $item->category_id, 'countBlogs' => true)); ?>
				<?php if(counT($subcategory) > 0): ?>  
					<a id="eblog_toggle_<?php echo $item->category_id ?>" class="cattoggel cattoggelright" href="javascript:void(0);" onclick="showCategory('<?php echo $item->getIdentity()  ?>')">
					</a>
				<?php endif; ?>     
				<a class="catlabel' ?>" href="<?php echo $this->url(array('module' =>'eblog','controller' => 'index', 'action' => 'browse'),'eblog_general',true).'?category_id='.$item->category_id; ?>"> 
					...
					<?php else:?>
						<img src="application/modules/Eblog/externals/images/category/icons/image.png" />
					<?php endif;?>
					<span><?php echo $this->translate($item->category_name); ?>
						<span class="eblog_sidebar_cat_count"><?php echo $item->total_blogs_categories;?></span>
					</span>
				</a>
				<ul id="subcategory_<?php echo $item->getIdentity() ?>" style="display:none;">   	
					<?php foreach( $subcategory as $subCat ): ?>
						<li>
							<?php $subsubcategory = Engine_Api::_()->getDbtable('categories', 'eblog')->getModuleSubsubcategory(array('column_name' => "*", 'category_id' => $subCat->category_id, 'countBlogs' => true)); ?>
							<?php if(count($subsubcategory) > 0): ?>   
								<a id="eblog_subcat_toggle_<?php echo $subCat->category_id ?>" class="cattoggel cattoggelright" href="javascript:void(0);" onclick="showCategory('<?php echo $subCat->getIdentity()  ?>')"></a>
							<?php endif; ?> 
...
function showCategory(id) {  
	if($('subcategory_' + id)) {  
      ...
      $('eblog_toggle_' + id).removeClass('cattoggel cattoggeldown');
      $('eblog_toggle_' + id).addClass('cattoggel cattoggelright'); 
			$('subcategory_' + id).style.display = 'none';   
		} else {  
			$('eblog_toggle_' + id).removeClass('cattoggel cattoggelright');
      $('eblog_toggle_' + id).addClass('cattoggel cattoggeldown');
      $('subcategory_' + id).style.display = 'block';
		} 
	}    
	if($('subsubcategory_' + id)) {   
		if ($('subsubcategory_' + id).style.display == 'block') { 
			$('eblog_subcat_toggle_' + id).removeClass('cattoggel cattoggeldown');
      $('eblog_subcat_toggle_' + id).addClass('cattoggel cattoggelright');           
			$('subsubcategory_' + id).style.display = 'none';   
		} else {  
			$('eblog_subcat_toggle_' + id).removeClass('cattoggel cattoggelright'); 
			$('eblog_subcat_toggle_' + id).addClass('cattoggel cattoggeldown');  
