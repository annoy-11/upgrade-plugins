...
<?php endif;?>
  <?php foreach($this->paginatorCategory as $item): ?>
  	<div class="eblog_category_blog sesbasic_clearfix">
      <div class="eblog_category_header sesbasic_clearfix">
        <p class="floatL" <?php echo $this->allignment_seeall == 'right' ?  'class="floatL"' : ''; ?>><a href="<?php echo $item->getBrowseBlogHref(); ?>?category_id=<?php echo $item->category_id ?>" title="<?php echo $this->translate($item->category_name); ?>"><?php echo $this->translate($item->category_name); ?></a></p>
				<?php if(isset($this->seemore_text) && $this->seemore_text != ''): ?>
					<span <?php echo $this->allignment_seeall == 'right' ?  'class="floatR"' : ''; ?>><a href="<?php echo $item->getBrowseBlogHref(); ?>?category_id=<?php echo $item->category_id ?>"><?php $seemoreTranslate = $this->translate($this->seemore_text); ?>
        ...
        <?php	foreach($this->resultArray['blog_data'][$item->category_id] as $item):?>
              <div class="eblog_category_item sesbasic_clearfix" style="width:<?php echo $this->width ?>px;">
                <div class="wrapper_list sesbasic_clearfix">
                  <div class="eblog_entry_img" style="height:<?php echo $this->height ?>px;">
                  ...
                  <?php endif;?>
                   ...
                   <?php if(isset($this->creationDateActive)):?>
                      <div class="entry_meta-date sesbasic_text_light">
                        Posted on <?php echo date('M d, Y',strtotime($item->publish_date));?>
                      </div>
                    <?php endif; ?>
							<div class="eblog_list_readmore"><a href="#" class="eblog_animation">More</a></div>
	...
	<?php if($this->paginatorCategory->getTotalItemCount() == 0 && !$this->is_ajax):?>
		<div class="tip">
			<span>
				<?php echo $this->translate('Nobody has created an blog yet.');?>
				<?php if ($this->can_create):?>
					<?php echo $this->translate('Be the first to %1$screate%2$s one!', '<a href="'.$this->url(array('action' => 'create','module'=>'eblog'), "eblog_general",true).'">', '</a>'); ?>
				<?php endif; ?>
			</span>
		</div>
	<?php endif; ?> 
	<?php if($this->loadOptionData == 'pagging'): ?>
		<?php echo $this->paginationControl($this->paginatorCategory, null, array("_pagging.tpl", "eblog"),array('identityWidget'=>$randonNumber)); ?>
	<?php endif; ?>
<?php if(!$this->is_ajax){ ?>
		</div>
	</div>
