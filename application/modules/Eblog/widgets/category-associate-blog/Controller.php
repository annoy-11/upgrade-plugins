 ...
 */
class Eblog_Widget_CategoryAssociateBlogController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->paginatorCategory = $paginatorCategory = Engine_Api::_()->getDbTable('categories', 'eblog')->getCategory(array('hasBlog' => true, 'criteria' => $criteriaData, 'blogDesc' => 'desc'), array('paginator' => 'yes'));
    ...
    if ($paginatorCategory->getTotalItemCount() > 0) {
      foreach ($paginatorCategory as $key => $valuePaginator) {
        ...
        $blogData['blog_data'][$valuePaginator->category_id] = Engine_Api::_()->getDbTable('blogs', 'eblog')->getEblogsPaginator(array('category_id' => $valuePaginator->category_id, 'status' => 1, 'limit' => $blog_limit, 'popularCol' => $popularCol, 'fixedData' => $fixedData, 'fetchAll' => 0), false);
      }
    } else {
      if (!$is_ajax)
