...
<?php if((!$this->is_ajax) && ($this->allow_create) && (Engine_Api::_()->eblog()->checkBlogAdmin($this->blog))): ?> 
...
<?php endif;?>
 ...
 <div class="eblog_search_result sesbasic_clearfix sesbm" id="<?php echo !$this->is_ajax ? 'paginator_count_eblog' : 'paginator_count_ajax_eblog' ?>"><span id="total_item_count_eblog"> 
...
window.addEvent('domready', function() {
    ...
    <div id="scrollHeightDivSes_<?php echo $randonNumber; ?>">
      <ul class="eblog_album_listings eblog_browse_album_listings sesbasic_bxs sesbasic_clearfix" id="tabbed-widget_<?php echo $randonNumber; ?>">
  <?php endif;?>			 
  <?php foreach( $this->paginator as $album ): ?>
    <?php if($this->view_type == 1){ ?>
      <li id="thumbs-photo-<?php echo $album->photo_id ?>" class="eblog_album_list_grid_thumb eblog_album_list_grid sesea-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sesea-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">  
	<a class="eblog_album_list_grid_img" href="<?php echo Engine_Api::_()->eblog()->getHref($album->getIdentity(),$album->album_id); ?>" style="height:<?php echo is_numeric($this->height) ? $this->height.'px' : $this->height ?>;">
	  ...
	  <div class="ses_image_container" style="display:none;">
	    <?php $image = Engine_Api::_()->eblog()->getAlbumPhoto($album->getIdentity(),$album->photo_id);
	...
	<?php  if((isset($this->socialSharing)  && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.enable.sharing', 1)) || isset($this->likeButton)){  ?>
	  <span class="eblog_album_list_grid_btns">
	    ...
	    if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0 && isset($this->likeButton) && $canComment){  ?>
	      <!--Album Like Button-->
	      <?php $albumLikeStatus = Engine_Api::_()->eblog()->getLikeStatusBlog($album->album_id,'eblog_album'); ?>
	      <a href="javascript:;" data-url='<?php echo $album->album_id; ?>' class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn eblog_albumlike <?php echo ($albumLikeStatus) ? 'button_active' : '' ; ?>">
	    ...
	    <?php } if(isset($this->favouriteButton) && Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.enable.favourite', 1)){ ?>
	     <?php $favStatus = Engine_Api::_()->getDbtable('favourites', 'eblog')->isFavourite(array('resource_type'=>'eblog_album','resource_id'=>'album_id')); ?>
        <a href="javascript:;" data-url='<?php echo $album->album_id; ?>' class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn eblog_albumfavourite <?php echo ($favStatus) ? 'button_active' : '' ; ?>">
          <i class="fa fa-heart"></i>
          <span><?php echo $album->favourite_count; ?></span>
        </a>
        <?php }
        ?>
        <?php if(Engine_Api::_()->eblog()->checkBlogAdmin($this->blog)):?>
					<a href="javascript:void(0);" class="eblog_album_list_grid_option_button sesbasic_icon_btn"><i class="fa fa-ellipsis-v"></i></a>
					<div class="eblog_album_option_box">
						<?php echo $this->htmlLink(array('route' => 'eblog_extended','controller' => 'album','action' => 'editphotos',
						'album_id' => $album->album_id), $this->translate('Manage Photos'), array('class' => 'fa fa-picture-o')); ?>
						<?php echo $this->htmlLink(array('route' => 'eblog_extended','controller' => 'album','action' => 'edit',
						'album_id' => $album->album_id), $this->translate('Edit Settings'), array('class' => 'fa fa-pencil')); ?>
						<?php echo $this->htmlLink(array('route' => 'eblog_extended','controller' => 'album','action' => 'delete',
	...
	<?php if(isset($this->like) || isset($this->favouriteCount) || isset($this->comment) ||  isset($this->view) || isset($this->title) || isset($this->photoCount) ||  isset($this->by)){ ?>
	  <p class="eblog_album_list_grid_info sesbasic_clearfix<?php if(!isset($this->photoCount)) { ?> nophotoscount<?php } ?>">
      ...
      ?>
		...
		<span class="eblog_album_list_grid_likes" title="<?php echo $this->translate(array('%s like', '%s likes', $album->like_count), $this->locale()->toNumber($album->like_count))?>">
		  <i class="fa fa-thumbs-up"></i>
		...
		<span class="eblog_album_list_grid_comment" title="<?php echo $this->translate(array('%s comment', '%s comments', $album->comment_count), $this->locale()->toNumber($album->comment_count))?>">
		  <i class="fa fa-comment"></i>
		  <?php echo $album->comment_count;?>
		</span>
	      <?php } ?>
	      <?php if(isset($this->view)) { ?>
		<span class="eblog_album_list_grid_views" title="<?php echo $this->translate(array('%s view', '%s views', $album->view_count), $this->locale()->toNumber($album->view_count))?>">
		  <i class="fa fa-eye"></i>
		  <?php echo $album->view_count;?>
		</span>
	      <?php } ?>
	      <?php if(isset($this->photoCount)) { ?>
		<span class="eblog_album_list_grid_count" title="<?php echo $this->translate(array('%s photo', '%s photos', $album->count()), $this->locale()->toNumber($album->count()))?>" >
	...
	<?php } ?>
	<?php if(isset($this->photoCount)) { ?>
	  <p class="eblog_album_list_grid_count">
...
<script type="text/javascript">
  ...
  function paggingNumber<?php echo $randonNumber; ?>(pageNum){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "widget/index/mod/eblog/name/profile-photos",
      'data': {
      format: 'html',
      page: pageNum,    
	...
	document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML =  responseHTML;	sesJqueryObject('#paginator_count_eblog').find('#total_item_count_eblog').html(sesJqueryObject('#paginator_count_ajax_eblog').find('#total_item_count_eblog').html());
	sesJqueryObject('#paginator_count_ajax_eblog').remove();
      }
    })).send();
    return false;
  }
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/eblog/name/profile-photos/index/',
      'data': {
	format: 'html',
	...
	sesJqueryObject('#paginator_count_eblog').find('#total_item_count_eblog').html(sesJqueryObject('#paginator_count_ajax_eblog').find('#total_item_count_eblog'));
	sesJqueryObject('#paginator_count_ajax_eblog').remove();
