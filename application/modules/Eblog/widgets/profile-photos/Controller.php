 ...
 */
class Eblog_Widget_ProfilePhotosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $value['blog_id'] = $this->view->blog_id =  $blog_id = $id = Engine_Api::_()->getDbtable('blogs', 'eblog')->getBlogId($id);
    $this->view->blog  = $subject = Engine_Api::_()->getItem('eblog_blog',$id);
    $value['identityForWidget'] = $this->view->identityForWidget = isset($_POST['identity']) ? $_POST['identity'] : '';
		$this->view->allow_create = true;
		if(!$value['is_ajax'] && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('eblogpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblogpackage.enable.package', 1)){
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('albums', 'eblog')->getAlbumSelect($value);
