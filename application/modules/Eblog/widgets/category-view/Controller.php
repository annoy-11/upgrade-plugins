...
class Eblog_Widget_CategoryViewController extends Engine_Content_Widget_Abstract {
  public function indexAction() {
    ...
    if (Engine_Api::_()->core()->hasSubject()) {
      ...
      $this->view->category = $category = Engine_Api::_()->getItem('eblog_category', $params['category_id']);
    ...
    if (!$is_ajax) {
      if ($category->subcat_id == 0 && $category->subsubcat_id == 0) {
        ...
        $innerCatData = Engine_Api::_()->getDbtable('categories', 'eblog')->getModuleSubcategory(array('category_id' => $category->category_id, 'column_name' => '*', 'countBlogs' => true));
        $columnCategory = 'category_id';
      } else if ($category->subsubcat_id == 0) {
        $innerCatData = Engine_Api::_()->getDbtable('categories', 'eblog')->getModuleSubsubcategory(array('countBlogs' => true, 'category_id' => $category->category_id, 'column_name' => '*'));
        $columnCategory = 'subcat_id';
      } else
        $columnCategory = 'subsubcat_id';
      $this->view->innerCatData = $innerCatData;
      //breadcum
      $this->view->breadcrumb = $breadcrumb = Engine_Api::_()->getDbtable('categories', 'eblog')->getBreadcrumb($category);
    }
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('blogs', 'eblog')->getEblogsPaginator(array($columnCategory => $category->category_id, 'status' => 1));
