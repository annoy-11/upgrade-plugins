...
<?php if(isset($this->category->thumbnail) && !empty($this->category->thumbnail)){ ?>
  <div class="eblog_category_cover sesbasic_bxs sesbm">
 		<div class="eblog_category_cover_inner" style="background-image:url(<?php echo  Engine_Api::_()->storage()->get($this->category->thumbnail)->getPhotoUrl('thumb.thumb'); ?>">
    	<div class="eblog_category_cover_content">
        <div class="eblog_category_cover_breadcrumb">
          <!--breadcrumb -->
          <a href="<?php echo $this->url(array('action' => 'browse'), "eblog_category"); ?>"><?php echo $this->translate("Categories"); ?></a>&nbsp;&raquo;
  ...
  <div class="eblog_category_grid sesbasic_clearfix sesbasic_bxs">
    <ul class="">	
      <?php foreach( $this->innerCatData as $item ):  ?>
        <li class=" ">
        <div class="eblog_thumb_contant"  style="height:<?php echo is_numeric($this->heightSubcat) ? $this->heightSubcat.'px' : $this->heightSubcat ?>;width:<?php echo is_numeric($this->widthSubcat) ? $this->widthSubcat.'px' : $this->widthSubcat ?>;">
...
<div class="eblog_subcat_list_head clear sesbasic_clearfix">
...
<?php } ?>
   <?php if($this->viewType == 'list'):?>
		<?php if(!$this->is_ajax){ ?>
		<ul class="eblog_blog_listing sesbasic_clearfix clear" id="tabbed-widget_<?php echo $randonNumber; ?>" style="display:block;">
	<?php } ?>
			<?php foreach($this->paginator as $key=>$blog):?>
				<li class="eblog_list_blog_view sesbasic_clearfix clear">
					<div class="eblog_list_thumb eblog_thumb"  style="width:<?php echo $this->width.'px'; ?>;height:<?php echo $this->height.'px'; ?>">
						<a href="<?php echo $blog->getHref(); ?>" data-url = "<?php echo $blog->getType() ?>" class="eblog_thumb_img">
          ...
          <?php if(isset($this->titleActive)){ ?>
						<div class="eblog_list_info_title">
				  ...
				  <?php } ?>
                      ...
                      <a href="<?php echo $owner->getHref();?>"><?php echo $this->itemPhoto($owner, 'thumb.icon');?></a>
							...
							<?php if(isset($this->creationDateActive)):?>
								<div class="eblog_stats_list sesbasic_text_dark">
									<span>
            ...
            <?php if(isset($this->readmoreActive)){ ?>
							<div class="eblog_list_readmore"><a href="<?php echo $blog->getHref();?>" class="eblog_animation"><?php echo $this->translate('More'); ?></a></div>
						<?php } ?>
            <div class="eblog_cat_list_footer">
            <div class="entry_meta-time sesbasic_text_light floatL">
                 <span><i class="fa fa-clock-o"></i> 2 Min. Read</span>
						...
						<div class="eblog_list_stats sesbasic_text_light">
							...
							<?php } ?>
							<?php if(isset($this->favouriteActive) && isset($blog->favourite_count) && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.enable.favourite', 1)) { ?>
							...
							<?php } ?>
							<?php if(isset($this->ratingActive) && isset($blog->rating) && $blog->rating > 0 && Engine_Api::_()->sesbasic()->getViewerPrivacy('eblog_review', 'view')): ?>
						...
						<?php if(Engine_Api::_()->getApi('core', 'eblog')->allowReviewRating() && isset($this->ratingStarActive)):?>
							<?php echo $this->partial('_blogRating.tpl', 'eblog', array('rating' => $blog->rating, 'class' => 'eblog_list_rating eblog_list_view_ratting floatL', 'style' => ''));?>
						<?php endif;?>
					</div>
				</li>  
		<?php endforeach;?>
			...
			<div class="tip">
				<span>
					...
					<?php if (!$this->can_edit):?>
						<?php echo $this->translate('Be the first to %1$spost%2$s one in this category!', '<a href="'.$this->url(array('action' => 'create'), "eblog_general").'">', '</a>'); ?>
					<?php endif; ?>
				</span>
			</div>
		<?php } ?>    
 ...
 <?php else:?>
	<?php if(!$this->is_ajax){ ?> 
	<ul class="eblog_blog_listing sesbasic_clearfix clear" id="tabbed-widget_<?php echo $randonNumber; ?>">
	<?php } ?>
			<?php foreach($this->paginator as $key=>$blog): ?>
				<li class="eblog_grid eblog_catogery_grid_view sesbasic_bxs" style="width:<?php echo $this->width.'px'; ?>">
						<div class="eblog_grid_inner eblog_thumb"> 
							<div class="eblog_grid_thumb" style="height:<?php echo $this->height.'px'; ?>">
								<a href="<?php echo $blog->getHref(); ?>" data-url = "<?php echo $blog->getType() ?>" class="eblog_thumb_img">
									<span class="" style="background-image:url(<?php echo $blog->getPhotoUrl(); ?>);"></span>
								</a>
								<?php if(isset($this->featuredLabelActive) || isset($this->sponsoredLabelActive) || isset($this->hotLabelActive)){ ?>
						...
						<?php if(Engine_Api::_()->getApi('core', 'eblog')->allowReviewRating() && isset($this->ratingStarActive)):?>
							<?php echo $this->partial('_blogRating.tpl', 'eblog', array('rating' => $blog->rating, 'class' => 'eblog_list_rating eblog_list_view_ratting floatR', 'style' => 'margin-bottom:5px; margin-top:5px;'));?>
						<?php endif;?>
          	<?php if(isset($this->titleActive)){ ?>
          ...
          <?php if(isset($this->readmoreActive)){ ?>
						<div class="eblog_grid_read_btn"><a href="<?php echo $blog->getHref();?>" class="eblog_animation"><?php echo $this->translate('More '); ?></a></div>
					<?php } ?>
					<div class="eblog_grid_stats_footer">  
...
function dynamicWidth(){
	var objectClass = sesJqueryObject('.eblog_cat_blog_list_info');
	for(i=0;i<objectClass.length;i++){
			sesJqueryObject(objectClass[i]).find('div').find('.eblog_cat_blog_list_content').find('.eblog_cat_blog_list_title').width(sesJqueryObject(objectClass[i]).width());
	}
}
dynamicWidth();
<?php } ?>
