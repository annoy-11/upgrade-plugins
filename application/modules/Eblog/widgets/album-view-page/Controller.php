 ...
 */
class Eblog_Widget_AlbumViewPageController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if(Engine_Api::_()->core()->hasSubject('eblog_album')) {
      ...
      $blog = Engine_Api::_()->getItem('eblog_blog', $album->blog_id);
			...
			$album =  Engine_Api::_()->getItem('eblog_album', $_POST['album_id']);
			if(isset($_POST['blog_id']))
        $blog = Engine_Api::_()->getItem('eblog_blog', $_POST['blog_id']);
      else
        $blog = Engine_Api::_()->getItem('eblog_blog', $album['blog_id']);
		}
    ...
    $this->view->blog_id = $param['blog_id'] = $album->blog_id;
		...
		if(!$is_ajax && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('eblogpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblogpackage.enable.package', 1)) {
    ...
    $eblog_photos = Zend_Registry::isRegistered('eblog_photos') ? Zend_Registry::get('eblog_photos') : null;
    if (empty($eblog_photos))
      return $this->setNoRender();
    ...
    $this->view->viewer = $viewer = Engine_Api::_()->user()->getViewer();
		...
		if($viewer->getIdentity() > 0){
			...
			$this->view->canCreateMemberLevelPermission =  Engine_Api::_()->authorization()->getPermission($viewer, 'eblog_blog', 'create');
			$this->view->canEditMemberLevelPermission   =  Engine_Api::_()->authorization()->getPermission($viewer,'eblog_blog', 'edit');
			$this->view->canDeleteMemberLevelPermission  = Engine_Api::_()->authorization()->getPermission($viewer,'eblog_blog', 'delete');
		}
    ...
    if(!$is_ajax){
		...
		$this->view->paginator = $paginator = Engine_Api::_()->getItemTable('eblog_photo')->getPhotoPaginator(array('album' => $album));
