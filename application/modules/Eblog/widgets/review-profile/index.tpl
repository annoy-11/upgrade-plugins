...
<div class="eblog_review_view eblog_profile_review sesbasic_bxs sesbasic_clearfix">
	<div class="eblog_review_view_top sesbasic_clearfix">
    <?php if(in_array('title', $this->stats)): ?>
      <div class="eblog_review_view_title"><?php echo $this->review->getTitle() ?></div>
    <?php endif; ?>
    <div class="eblog_review_view_item_info sesbasic_clearfix">
      <?php if(in_array('postedin', $this->stats)): ?>
      	<div class="eblog_review_view_info_img">
        	<?php echo $this->htmlLink($this->item->getHref(), $this->itemPhoto($this->item, 'thumb.icon')); ?>
        </div>
      <?php endif; ?> 
      <div class="eblog_review_view_info_cont sesbasic_clearfix">
        <?php if(in_array('postedin', $this->stats) && in_array('creationDate', $this->stats)) : ?>
          <p class='eblog_review_view_stats sesbasic_text_light sesbasic_clearfix'>
            <?php if(in_array('postedin', $this->stats)): ?>
        ...
        <?php endif; ?> 
        <p class="eblog_review_view_stats sesbasic_text_light">
    ...
    <div class="eblog_review_show_rating eblog_review_listing_star">
      ...
      <?php $reviewParameters = Engine_Api::_()->getDbtable('parametervalues', 'eblog')->getParameters(array('content_id'=>$this->review->getIdentity(),'user_id'=>$this->review->owner_id)); ?>
  ...
  <div class="eblog_review_contant_disc">
  <?php if(in_array('pros', $this->stats) && $this->review->pros): ?>
    <div class="eblog_review_view_cont_row"><b class="label"><?php echo $this->translate("Pros"); ?></b><?php echo $this->review->pros; ?></div>
  <?php endif; ?>
  <?php if(in_array('cons', $this->stats) && $this->review->cons): ?>
    <div class="eblog_review_view_cont_row"><b class="label"><?php echo $this->translate("Cons"); ?></b><?php echo $this->review->cons; ?></div>
  <?php endif; ?>
  <?php if(in_array('customfields', $this->stats)): ?>
  	<?php $customFieldsData = Engine_Api::_()->eblog()->getCustomFieldMapData($this->review); 
    	if(count($customFieldsData) > 0){ 
         foreach($customFieldsData as $valueMeta){
         if(!$valueMeta['value'])	
         	continue;
          echo '<p class="eblog_review_view_cont_row"><b class="label">'. $valueMeta['label']. ': </b>'.
                $valueMeta['value'].'</p>';
         }     
 			 } ?>
  <?php endif; ?>
  <?php if(in_array('description', $this->stats)): ?>
    <div class='eblog_review_view_cont_row'>
  ...
  <?php if(in_array('recommended', $this->stats)): ?>
  	<p class="eblog_review_view_cont_row eblog_review_view_recommended">
...
<div class="eblog_layout_contant" style="padding:0px;">
	<div class="eblog_footer_two_blog ">
		<div class="eblog_list_share_btns sesbasic_bxs">
        ...
        <?php echo $this->partial('_socialShareIcons.tpl','sesbasic',array('resource' => $this->review, 'socialshare_enable_plusicon' => $this->socialshare_enable_plusicon, 'socialshare_icon_limit' => $this->socialshare_icon_limit)); ?>
			...
			<?php if(in_array('likeButton', $this->stats) && $canComment):?>
				<?php $LikeStatus = Engine_Api::_()->eblog()->getLikeStatus($this->review->review_id,$this->review->getType()); ?>
					<a href="javascript:;" data-url="<?php echo $this->review->review_id ; ?>" class="sesbasic_icon_btn sesbasic_icon_like_btn  eblog_like_eblog_review_<?php echo $this->review->review_id ?> eblog_like_eblog_review_view <?php echo ($LikeStatus) ? 'button_active' : '' ; ?>"><i class="<?php if($LikeStatus):?>fa fa-thumbs-down<?php else:?>fa fa-thumbs-up<?php endif;?>"></i><span><?php if($LikeStatus):?><?php echo $this->translate('Unlike');?><?php else:?><?php echo $this->translate('Like');?><?php endif;?></span></a>
			<?php endif;?>
		</div>
	</div>
</div>