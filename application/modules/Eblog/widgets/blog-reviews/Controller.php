...
class Eblog_Widget_BlogReviewsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $eblog_reviews = Zend_Registry::isRegistered('eblog_reviews') ? Zend_Registry::get('eblog_reviews') : null;
		...
		if (!$is_ajax) {
		  ...
		  return $this->setNoRender();
			...
			if (!Engine_Api::_()->authorization()->getPermission($viewer, 'eblog_review', 'view'))
			return $this->setNoRender();
    
			$this->view->isReview = Engine_Api::_()->getDbtable('reviews', 'eblog')->isReview(array('blog_id' => $subject->getIdentity()));
			$this->view->cancreate = Engine_Api::_()->authorization()->getPermission($viewer, 'eblog_review', 'create'); 
    }
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('reviews', 'eblog')->getBlogReviewSelect($params);
