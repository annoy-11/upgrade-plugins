...
<?php if($this->allowedCreate && $this->cancreate && $this->viewer()->getIdentity() && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.allow.review', 1) && !$this->isReview): ?>
  <div class="sesbasic_profile_tabs_top sesbasic_clearfix">
    <?php echo $this->htmlLink(array('route' => 'eblogreview_extended','action' => 'create', 'blog_id'=>$this->subject()->getIdentity()), $this->translate('Write a Review'), array('class' => 'sesbasic_button fa fa-plus'));?>
  </div>
<?php endif;?>
<?php } ?>
<?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
	<?php if(!$this->is_ajax){ ?>
		<div class="eblog_profile_reviews_filters sesbasic_bxs sesbasic_clearfix">
			<?php echo $this->content()->renderWidget('eblog.browse-review-search',array('reviewSearch'=>1,'reviewStars'=>1,'reviewRecommended'=>1,'review_title'=>0,'view_type'=>'horizontal','isWidget'=>true,'blog_id'=>$this->subject->getIdentity(),'widgetIdentity'=>$this->identity,'allsetting'=>$this->allparam)); ?>
		</div>
		<ul class="eblog_review_listing sesbasic_clearfix sesbasic_bxs" id="eblog_review_listing">
	<?php } ?>
  <?php foreach( $this->paginator as $item ): ?>
  <li class="eblog_owner_review sesbasic_clearfix">
    <div class="eblog_review_listing_top sesbasic_clearfix eblog_review_listing_left_column">
      <?php if(in_array('title', $this->stats)): ?>
      <div class='eblog_review_listing_title'>
      	<?php echo $this->htmlLink($item->getHref(), $item->getTitle()) ?>
      </div>
     <?php endif; ?>
      <div class="eblog_review_listing_top_info sesbasic_clearfix">      
      <?php if(in_array('postedBy', $this->stats)): ?>
        <div class='eblog_review_listing_top_info_img'>
          <?php echo $this->htmlLink($item->getOwner()->getHref(), $this->itemPhoto($item->getOwner(), 'thumb.icon')) ?>
        </div>
       <?php endif; ?>
        <div class='eblog_review_listing_top_info_cont'>
         <?php if(in_array('postedBy', $this->stats)): ?>
        	<p class="eblog_review_listing_stats sesbasic_text_light">by <?php echo $this->htmlLink($item->getOwner()->getHref(), $item->getOwner()->getTitle()) ?>
         <?php endif; ?>
          |
          ...
          <?php endif; ?>
          <p class="sesbasic_text_light eblog_review_listing_stats">
       ...
       <?php if(in_array('rating', $this->stats)): ?>
        <div class="eblog_review_show_rating review_ratings_listing">
        <div class="sesbasic_rating_star">
          ...
          <?php for($i=0; $i<5; $i++){?>
            ...
            <?php else:?>
              <span id="" class="eblog_rating_star eblog_rating_star_disable"></span>
            <?php endif;?>
          <?php }?>
        </div>
        </div>
    ...
    <div class="eblog_review_listing_desc sesbasic_clearfix">
      <?php if(in_array('pros', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.show.pros', 1)): ?>
        <p class="eblog_review_listing_body">
          <b><?php echo $this->translate("Pros: "); ?></b>
          <?php echo $this->string()->truncate($this->string()->stripTags($item->pros), 300) ?>
        </p>
      <?php endif; ?>
      <?php if(in_array('cons', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.show.cons', 1)): ?>
        <p class="eblog_review_listing_body">
      ...
      <p class="eblog_review_listing_more">
    ...
    <div class="eblog_review_listing_footer clear sesbasic_clearfix ">
			...
			<div class='eblog_review_listing_btn_right floatR '>
				<?php if($item->authorization()->isAllowed($this->viewer(), 'edit')) { ?>
					<a class="fa fa-pencil sesbasic_button sesbasic_button_icon " href="<?php echo $this->url(array('slug' => $item->getSlug(), 'action' => 'edit', 'review_id' => $item->getIdentity()), 'eblogreview_view', 'true');?>"><span><i class="fa fa-caret-down"></i><?php echo $this->translate('Edit Review');?></span></a>
				<?php } ?>
				<?php if($item->authorization()->isAllowed($this->viewer(), 'delete')) { ?>
					<a class="smoothbox fa fa-trash sesbasic_button sesbasic_button_icon" href="<?php echo $this->url(array('action' => 'delete', 'review_id' => $item->getIdentity()), 'eblogreview_view', true);?>"><span><i class="fa fa-caret-down"></i><?php echo $this->translate('Delete Review');?></span></a>
				<?php } ?>
				<?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.show.report', 1) && $this->viewer()->getIdentity() && in_array('report', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.enable.report', 1)): ?>
				...
				<?php endif; ?>
				<?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.allow.share', 1) && $this->viewer()->getIdentity() && in_array('share', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.enable.sharing', 1)): ?>
...
<div class="sesbasic_tip clearfix">
    <img src="application/modules/Eblog/externals/images/reviews_icon.png" alt="">
    <span class="sesbasic_text_light">
  ...
  sesJqueryObject("#feed_viewmore_link_<?php echo $randonNumber;?>").removeClass('sesbasic_link_btn fa fa-repeat');
			window.addEvent('load', function() {
				sesJqueryObject(window).scroll( function() {
					var containerId = '#eblog_review_listing';
	...
	<?php if($this->loadOptionData != 'pagging') { ?>
		...
		function viewMore_<?php echo $randonNumber; ?> () {
			...
			requestViewMore_<?php echo $randonNumber; ?> = new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + "widget/index/mod/eblog/name/blog-reviews",
				'data': {
					format: 'html',
				...
				onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
					sesJqueryObject('#eblog_review_listing').append(responseHTML);
	...
	<?php }else{ ?>
		function paggingNumber<?php echo $randonNumber; ?>(pageNum){
			...
			requestViewMore_<?php echo $randonNumber; ?> = (new Request.HTML({
				...
				onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
					sesJqueryObject('#eblog_review_listing').html(responseHTML);
