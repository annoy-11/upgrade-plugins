...
<div class="slide sesbasic_clearfix sesbasic_bxs eblog_blogs_carousel_wrapper <?php echo $this->isfullwidth ? 'isfull_width' : '' ; ?>" style="height:<?php echo $this->height ?>px;">
  <div class="blogslide_<?php echo $this->identity; ?> owl-carousel eblog_featured_sponsored_carousel"  style="height:<?php echo $this->height ?>px;">
    <?php foreach( $this->paginator as $item): ?>
    <div class="eblog_grid_inside sesbasic_clearfix eblog_grid_btns_wrap" style="height:<?php echo $this->height ?>px;">
    	<div class="eblog_grid_inside_inner">
      <div class="eblog_featured_sponsored_item_thumb eblog_thumb">       
        <?php
        $href = $item->getHref();
        $imageURL = $item->getPhotoUrl();
        ?>
        <a href="<?php echo $href; ?>" class="eblog_list_thumb_img"  style="height:<?php echo $this->height ?>px;">
		...
		<?php if((isset($this->socialSharingActive)  && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.enable.sharing', 1)) || isset($this->likeButtonActive) || isset($this->favouriteButtonActive)):?>
         ...
         <?php if(isset($this->creationDateActive)){ ?>
				<div class="eblog_date"><?php echo ' '.date('d M',strtotime($item->publish_date));?></div>
			<?php } ?>
          ...
          <?php if(Engine_Api::_()->getApi('core', 'eblog')->allowReviewRating() && isset($this->ratingStarActive)):?>
							...
							<?php endif;?>
          <div class="eblog_fs_stats">
          <?php if(isset($this->byActive)){ ?>
				<div class="eblog_owner">
...
<script type="text/javascript">
	...
	<?php } ?>
  eblogJqueryObject(".eblog_featured_sponsored_carousel").owlCarousel({
  nav:true,
  dots:false,
  items:1,
	margin:10,
