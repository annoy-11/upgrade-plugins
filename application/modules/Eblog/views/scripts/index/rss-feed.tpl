...
<channel>
  <title><?php echo $this->translate('Adanced Blog Rss Feed');?></title>
    ...
    <item>
      ...
      <link><?php echo "http://".$_SERVER['HTTP_HOST'].$this->url(array('action' => 'view','blog_id'=>$blog->custom_url), 'eblog_entry_view');?></link>
      <?php 
      	$text = strip_tags($blog->body);
        $text = str_replace("&rsquo;","'", $text); 
        $content = preg_replace("/&#?[a-z0-9]{2,8};/i","",$text );
       ?>
      <description><?php echo Engine_Api::_()->eblog()->truncate(htmlspecialchars($content), 200,array('html' => true, 'ending' => '')) ?></description>
    </item>
  <?php endforeach;?>
</channel>
</rss>