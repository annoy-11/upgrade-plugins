 ...
 * @version    $Id: review-rating.tpl 2016-07-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

?>
<?php $settings = Engine_Api::_()->getApi('settings', 'core'); ?>
<div class="form-wrapper eblog_form_rating_star">
  <div class="form-label"><label><?php echo $this->translate("Overall Rating"); ?></label></div>
  <div id="eblog_review_rating" class="sesbasic_rating_star eblog_rating_star_element" onmouseout="rating_out();">
...
function ratingText(rating){
	var text = '';
	if(rating == 1){
			text = "<?php echo $this->translate($settings->getSetting('eblog.rating.stars.one', 'terrible'));?>";
	}else if(rating == 2){
			text = "<?php echo $this->translate($settings->getSetting('eblog.rating.stars.second', 'poor'));?>";
	}else if(rating == 3){
			text = "<?php echo $this->translate($settings->getSetting('eblog.rating.stars.three', 'average'));?>";
	}else if(rating == 4){
			text = "<?php echo $this->translate($settings->getSetting('eblog.rating.stars.four', 'very good'));?>";
	}else if(rating == 5){
			text = "<?php echo $this->translate($settings->getSetting('eblog.rating.stars.five', 'excellent'));?>";
	}else {
...
function validateForm(){
		...
		counter = 0;
		sesJqueryObject('#eblog_review_form input, #eblog_review_form select,#eblog_review_form checkbox,#eblog_review_form textarea,#eblog_review_form radio').each(
				function(index){
			...
			en4.core.runonce.add(function()
				{
			sesJqueryObject('#eblog_review_form').submit(function(e){
					var validationFm = validateForm();
					if(!sesJqueryObject('#rate_value').val()){
						alert('<?php echo $this->translate("Please fill the red mark fields"); ?>');
						 var errorFirstObject = sesJqueryObject('#eblog_review_rating').parent();
