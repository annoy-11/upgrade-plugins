...
<div class="clear eblog_order_view_top" style="margin:10px;">
      <a href="<?php echo $this->blog->getHref(); ?>" class="buttonlink sesbasic_icon_back"><?php echo $this->translate("Back To Blog"); ?></a>
    </div>
<div class="eblog_album_form"> <?php echo $this->form->render($this) ?> </div>
...
<script type="text/javascript">
  ...
  jqueryObjectOfSes('#eblog_create_form_tabs li a').on('click',function(){
...
jqueryObjectOfSes(document).on('click','.delete_image_upload',function(e){
    ...
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'eblog', 'controller' => 'album', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'eblog', 'controller' => 'album', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
</script>