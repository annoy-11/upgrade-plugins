...
<?php if(!$this->is_ajax) {
  echo $this->partial('dashboard/left-bar.tpl', 'eblog', array('blog' => $this->blog));	
?>
...
$showinfo = array('description','featured','sponsored','verified','location','modules','editor','custom_fields','blogcount');//Engine_Api::_()->getApi('settings', 'core')->getSetting('eblogpackage.package.information', array_keys($information));
 ?>
<?php $currentCurrency =  Engine_Api::_()->eblog()->getCurrentCurrency(); ?>
<?php if($this->currentPackage){ 
	$package = Engine_Api::_()->getItem('eblogpackage_package',$this->currentPackage->package_id);
  if($package){
?>
<div class="eblogpackage_blog_list sesbasic_clearfix sesbasic_bxs">
	<div class="eblogpackage_slect_plan_blog">
  ...
  <div class="package_catogery_list">
    ...
    <?php 
            ...
            $curruncySymbol = str_replace('.','',preg_replace('/\d+/u', '', Engine_Api::_()->eblog()->getCurrencyPrice($package->price)));
     ?>
      	...
      	<div class="package-ditals_blog">
            ...
            <div class="amount">
            	<?php if($package->price > 0){ ?>
            		<span><?php echo $curruncySymbol; ?></span><?php echo Engine_Api::_()->eblog()->getCurrencyPrice($package->price,'','',true); ?><br />
     ...
     ?>
    	<li class="<?php echo ($package->highlight) ? 'active' : '' ?>">
          ...
          <?php if(in_array('description',$showinfo)){ ?> 
            <p class="footer_discprition"><?php echo $this->translate($package->description); ?> </p>
          <?php } ?>
          <a href="<?php echo $this->url(array('blog_id' => $this->blog->blog_id,'action'=>'confirm-upgrade','package_id'=>$package->package_id), 'eblogpackage_general', true); ?>" class="smoothbox">Upgrade Package</a>
          </div>
        </div>
      </li>      
    <?php } ?>
