 ...
 * @version    $Id: blog-role.tpl 2016-07-23 00:00:00 SocialEngineSolutions $
...
<?php if(!$this->is_ajax):?> 
	...
	?>
  <?php echo $this->partial('dashboard/left-bar.tpl', 'eblog', array('blog' => $this->blog));	?>
<div class="sesbasic_dashboard_content eblog_manage_role_form sesbm sesbasic_clearfix">
	<div class="eblog_manage_role_form_top sesbasic_clearfix">
		...
		<?php endif; ?>
		<form id="blog_admin_form" action="<?php echo $this->url(array('action' => 'save-blog-admin', 'blog_id' => $this->blog->blog_id), 'eblog_dashboard', true) ?>" method="post">
			<div id="manage_admin_input">
				<div class="eblog_manage_roles_item">
...
<?php if(!$this->is_ajax){ ?>
	...
	<div class="eblog_footer_contant">
...
<script type="text/javascript">
	function showAutosuggest(blogAdmin, imageId) {
	  var contentAutocomplete1 =  'contentAutocomplete-'+blogAdmin
		contentAutocomplete1 = new Autocompleter.Request.JSON(blogAdmin, "<?php echo $this->url(array('module' => 'eblog', 'controller' => 'dashboard', 'action' => 'get-members', 'blog_id' => $this->blog->blog_id), 'default', true) ?>", {
			'postVar': 'text',
			'minLength': 1,
			'selectMode': '',
	...
	function saveForm() {
	  var UserIds = $('user_id').value;
		new Request.HTML({
			url : en4.core.baseUrl + 'eblog/dashboard/save-blog-admin/blog_id/' + <?php echo $this->blog->blog_id ?>,
			method: 'post',
			data : {
				format : 'html',
				data: UserIds,
	...
	function addMore() {
	  ...
	  sesJqueryObject('#manage_admin_input').append('<div class="eblog_manage_roles_item"><span class="show_img" id="show_default_img_'+count+'"'+'></span> <input type="text" placeholder="Type Member Name" size="20" name="'+ColumnId+'"' +'id="'+ColumnId+'"'+'autocomplete="off" rel="'+count+'"><a class="remove_icon" href="javascript:void(0);" onclick="removeInputForm('+"'"+ColumnId+"'"+');"><i class="fa fa-close" id="close_option_'+count+'"'+'></i></a></div>');
  ...
  function removeUser(blogId, roleId) {
		new Request.JSON({
			url : en4.core.baseUrl + 'eblog/dashboard/delete-blog-admin',
			method: 'post',
			data : {
				format : 'json',
				role_id: roleId,
