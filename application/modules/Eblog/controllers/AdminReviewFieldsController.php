...
class Eblog_AdminReviewFieldsController extends Fields_Controller_AdminAbstract {

  protected $_requireProfileType = true;
  protected $_fieldType = 'eblog_review';
  ...
  public function indexAction() {
    ...
    $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('eblog_admin_main_review_cat', array(), 'eblog_admin_main_review_subfields');
  ...
  public function fieldCreateAction() {
    ...
    if ($content_type == 'eblog_category') {
      $content_type = 'eblog_blog';
    }
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Eblog_Form_Admin_Review_Type();
    if (!$this->getRequest()->isPost()) {
      return;
    }
    if (!$form->isValid($this->getRequest()->getPost())) {
    ...
    if ($form->getValue('duplicate') != 'null') {
      ...
      $field_meta_array = $db->select()
              ->from('engine4_eblog_review_fields_meta')
              ->where('field_id IN (' . implode(', ', $child_id_array) . ')')
              ->query()
              ->fetchAll();
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_eblog_review_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_eblog_review_fields_maps', array(
