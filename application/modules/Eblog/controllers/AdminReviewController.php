...
class Eblog_AdminReviewController extends Core_Controller_Action_Admin {
  ...
  public function reviewSettingsAction() {
    ...
    $this->view->subnavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('eblog_admin_main_reviewsettings', array(), 'eblog_admin_main_review_settings');
  ...
  public function manageReviewsAction() {
    ...
    $this->view->subnavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('eblog_admin_main_reviewsettings', array(), 'eblog_admin_main_managereview');

    $this->view->formFilter = $form = new Eblog_Form_Admin_Review_Filter();
    ...
    if ($this->getRequest()->isPost()) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          Engine_Api::_()->getItem('eblog_review', $value)->delete();
        }
      }
    }
  ...
  public function deleteReviewAction() {
    ...
    $this->view->form = new Eblog_Form_Admin_Review_Delete();
  ...
  public function levelSettingsAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('eblog_admin_main_reviewsettings', array(), 'eblog_admin_main_levelsettings');
    ...
    //Make form
    $this->view->form = $form = new Eblog_Form_Admin_Review_Level(array(
        'public' => ( in_array($level->type, array('public')) ),
        'moderator' => ( in_array($level->type, array('admin', 'moderator')) ),
    ));
    $form->level_id->setValue($id);

    $content_type = 'eblog_review';
    $module_name = $this->_getParam('module_name', null);
  ...
  public function featuredAction() {
    ...
    if (!empty($id)) {
      $item = Engine_Api::_()->getItem('eblog_review', $id);
      $item->featured = !$item->featured;
      $item->save();
    }
    $this->_redirect('admin/eblog/review/manage-reviews');
  }
  ...
  public function ofthedayAction() {
    ...
    $this->view->form = $form = new Eblog_Form_Admin_Oftheday();
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db->update('engine4_eblog_reviews', array('starttime' => $values['starttime'], 'endtime' => $values['endtime']), array("review_id = ?" => $id));
      
      if (isset($values['remove']) && $values['remove']) {
        $db->update('engine4_eblog_reviews', array('oftheday' => 0), array("review_id = ?" => $id));
      } else {
        $db->update('engine4_eblog_reviews', array('oftheday' => 1), array("review_id = ?" => $id));
      }
