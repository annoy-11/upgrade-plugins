...
class Eblog_PhotoController extends Core_Controller_Action_Standard {

  public function init() {
    if (!Engine_Api::_()->core()->hasSubject()) {
      if (0 !== ($photo_id = (int) $this->_getParam('photo_id')) &&
              null !== ($photo = Engine_Api::_()->getItem('eblog_photo', $photo_id))) {
        Engine_Api::_()->core()->setSubject($photo);
      } else if (0 !== ($blog_id = (int) $this->_getParam('blog_id')) &&
              null !== ($blog = Engine_Api::_()->getItem('eblog_blog', $event_id))) {
...
//rotate photo action from lightbox and photo view page
  public function rotateAction() {
    if (!$this->_helper->requireSubject('eblog_photo')->isValid())
      return;
    ...
    $photo = Engine_Api::_()->core()->getSubject('eblog_photo');
   ...
   if (!$this->_helper->requireSubject('eblog_photo')->isValid())
    ...
    $this->view->href = $photo->getPhotoUrl();
  	...
  	public function correspondingImageAction(){
		...
		$this->view->paginator = $paginator = Engine_Api::_()->getDbtable('photos', 'eblog')->getPhotoSelect(array('album_id'=>$album_id,'limit_data'=>100));
	}
  ...
  public function uploadPhotoAction() {
    ...
    $eblogPhotoTable = Engine_Api::_()->getDbtable('photos', 'eblog');
    
    $db = $eblogPhotoTable->getAdapter();
    $db->beginTransaction();
		$session = new Zend_Session_Namespace();
    try {
      $viewer = Engine_Api::_()->user()->getViewer();
      if(empty($session->album_id)) {
			$album = Engine_Api::_()->getItemTable('eblog_album')->createRow();
      ...
      $photo = Engine_Api::_()->sesbasic()->setPhoto($uploadSource, $isURL,false,'eblog','eblog_blog',$params,$album);
  ...
  public function editAction()
  {
    ...
    $this->view->form = $form = new Eblog_Form_Photo_Edit();
  ...
  public function deleteAction() {
    $photo = Engine_Api::_()->core()->getSubject();
    $blog = $photo->getParent('eblog_blog');
		$album_id = $photo->album_id;
    if (!$this->_helper->requireAuth()->setAuthParams($blog, null, 'edit')->isValid()) {
      return;
    }

    $this->view->form = $form = new Eblog_Form_Photo_Delete();
