...
class Eblog_AdminManageController extends Core_Controller_Action_Admin {

  public function indexAction() {
  
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('eblog_admin_main', array(), 'eblog_admin_main_manage');

    $this->view->formFilter = $formFilter = new Eblog_Form_Admin_Manage_Filter();
    
    if ($this->getRequest()->isPost()) {
      $values = $this->getRequest()->getPost();
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $item = Engine_Api::_()->getItem('eblog_blog', $value);
					Engine_Api::_()->eblog()->deleteBlog($item);
        }
      }
    }
  ...
  public function claimAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('eblog_admin_main', array(), 'eblog_admin_main_claim');

    $this->view->formFilter = $formFilter = new Eblog_Form_Admin_Manage_ClaimFilter();
      
    if ($this->getRequest()->isPost()) {
      $values = $this->getRequest()->getPost();
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          Engine_Api::_()->getItem('eblog_claim', $value)->delete();
        }
      }
    }
    ...
    $btable = Engine_Api::_()->getDbTable('blogs', 'eblog');
  ...
  public function deleteAction() {
    ...
    if( $this->getRequest()->isPost()) {
      try {
        Engine_Api::_()->eblog()->deleteBlog(Engine_Api::_()->getItem('eblog_blog', $this->_getParam('id')));
      } catch( Exception $e) {
        //slience
      }
  ...
  public function approvedAction() {
    ...
    if (!empty($id)) {
      ...
      $item = Engine_Api::_()->getItem('eblog_blog', $id);
      ...
      if(count($action->toArray()) <= 0 && (!$item->publish_date || strtotime($item->publish_date) <= time())) {
        ...
        $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($viewer, $item, 'eblog_new');
        if($action)
          Engine_Api::_()->getDbtable('actions', 'activity')->attachActivity($action, $item);
      }
    }
    $this->_redirect('admin/eblog/manage');
  }
  ...
  public function ofthedayAction() {
    ...
    $this->view->form = $form = new Eblog_Form_Admin_Manage_Oftheday();
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db->update('engine4_eblog_blogs', array('starttime' => $values['starttime'], 'endtime' => $values['endtime']), array("blog_id = ?" => $id));
      
      if (isset($values['remove']) && $values['remove']) {
        $db->update('engine4_eblog_blogs', array('offtheday' => 0), array("blog_id = ?" => $id));
      } else {
        $db->update('engine4_eblog_blogs', array('offtheday' => 1), array("blog_id = ?" => $id));
      }
      $this->_forward('success', 'utility', 'core', array(
          'smoothboxClose' => 10,
          'parentRefresh' => 10,
  ...
  public function showDetailAction() {
    $this->view->claimItem = Engine_Api::_()->getItem('eblog_claim', $this->_getParam('id'));
  }
  ...
  public function approveClaimAction() {
    ...
    $claimItem = Engine_Api::_()->getItem('eblog_claim', $this->_getParam('id', null));
    $currentOwnerId = $claimItem->user_id;
    
    $item = Engine_Api::_()->getItem('eblog_blog', $claimItem->blog_id);

    $this->view->form = $form = new Eblog_Form_Admin_Manage_Approveform();
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
		...
		if(!empty($_POST['approve_decline']) && ($_POST['approve_decline'] == 'accept')) {
			...
			//upgrade album
			$db->update('engine4_eblog_albums', array('owner_id' => $currentOwnerId), array("blog_id = ?" => $claimItem->blog_id));
			
			//upgrade photos
			$db->update('engine4_eblog_photos', array('user_id' => $currentOwnerId), array("blog_id = ?" => $claimItem->blog_id));
			
			//upgrade favourites
			$db->update('engine4_eblog_favourites', array('user_id' => $currentOwnerId), array("resource_id = ?" => $claimItem->blog_id,'user_id = ?'=>$item->owner_id));
			
			//upgrade reviews
			$db->update('engine4_eblog_reviews', array('owner_id' => $currentOwnerId), array("blog_id = ?" => $claimItem->blog_id,'owner_id = ?'=>$item->owner_id));
			
			//upgrade roles
			$db->update('engine4_eblog_roles', array('user_id' => $currentOwnerId), array("blog_id = ?" => $claimItem->blog_id,'user_id = ?'=>$item->owner_id));
			
			//upgrade extention if exists
			//upgrade video
			if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesvideo')){ 
				$db->update('engine4_video_videos', array('owner_id' => $currentOwnerId), array("parent_id = ?" => $claimItem->blog_id,'parent_type = ?'=>'eblog_blog','owner_id = ?'=>$item->owner_id));
			}
			
			//upgrade music plugin
			if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesmusic')){
				//upgrade video
				$db->update('engine4_sesmusic_albums', array('owner_id' => $currentOwnerId), array("resource_id = ?" => $claimItem->blog_id,'resource_type = ?'=>'eblog_blog','owner_id = ? '=>$item->owner_id));
			}
			//done upgrade extention work
			
			$db->update('engine4_eblog_claims', array('status' => 1), array("claim_id = ?" => $claimItem->claim_id));
			$db->update('engine4_eblog_blogs', array('owner_id' => $currentOwnerId), array("blog_id = ?" => $claimItem->blog_id));
		  ...
		  Engine_Api::_()->getApi('mail', 'core')->sendSystem($claimItem->user_email, 'eblog_claim_owner_approve', $mailDataClaimOwner);
			...
			$blogOwnerId = Engine_Api::_()->getItem('eblog_blog', $claimItem->blog_id)->owner_id;
			...
			Engine_Api::_()->getApi('mail', 'core')->sendSystem($blogOwnerEmail, 'eblog_blog_owner_approve', $mailDataBlogOwner);
			Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($claimOwner, $viewer, $item, 'eblog_claim_approve');
      Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($blogOwner, $viewer, $item, 'eblog_owner_informed');
		} elseif(!empty($_POST['approve_decline']) && ($_POST['approve_decline'] == 'decline')) {
		  $claimOwner = Engine_Api::_()->getItem('user', $currentOwnerId);
		  $db->delete('engine4_eblog_claims', array("claim_id = ?" => $claimItem->claim_id));
		  Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($claimOwner, $viewer, $item, 'eblog_claim_declined');
		} else {
  ...
  public function viewAction() {
    $this->view->item = Engine_Api::_()->getItem('eblog_blog', $this->_getParam('id', null));
  }  
}
