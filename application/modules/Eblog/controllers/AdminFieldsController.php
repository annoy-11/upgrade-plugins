 ...
 */
class Eblog_AdminFieldsController extends Fields_Controller_AdminAbstract {

  protected $_requireProfileType = true;
  protected $_fieldType = 'eblog_blog';
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('eblog_admin_categories', array(), 'eblog_admin_main_subfields');
    parent::indexAction();
  }
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Eblog_Form_Admin_Settings_Type();
    ...
    if ($form->getValue('duplicate') != 'null' && $form->getValue('duplicate') ) {
      // Create New Option in engine4_eblog_contactus_fields_options
      ...
      $field_meta_array = $db->select()
              ->from('engine4_eblog_fields_meta')
      ...
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_eblog_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_eblog_fields_maps', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      Engine_Api::_()->getDbtable('categories', 'eblog')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
