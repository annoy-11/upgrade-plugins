...
class Eblog_AdminIntegrateothermoduleController extends Core_Controller_Action_Admin {

  public function indexAction() {
  
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('eblog_admin_main', array(), 'eblog_admin_main_integrateothermodule');
    $this->view->enabledModules = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();
    $select = Engine_Api::_()->getDbtable('integrateothermodules', 'eblog')->select();
  ...
  public function addmoduleAction() {
    ...
    $this->view->form = $form = new Eblog_Form_Admin_Integrate_Add();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      $integrateothermoduleTable = Engine_Api::_()->getDbtable('integrateothermodules', 'eblog');
      ...
      try {
        ...
        $dbInsert->query('INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `enabled`, `custom`, `order`) VALUES ("eblog_main_browseblog_'.$row->getIdentity().'", "'.$modulename.'", "Browse Blogs", "", \'{"route":"eblog_browseblog_'.$row->getIdentity().'","action":"browse-blogs", "resource_type":"'.$values['content_type'].'"}\', "'.$modulename.'_main", "", 1, 0, 999)');
  ...
  function createBrowseBlogPage($modulename, $id) {
    ...
    if( !$page_id ) {
      ...
      $db->insert('engine4_core_pages', array(
        'name' => 'eblog_index_'.$id,
        'displayname' => 'SES - Advanced Blogs - '.ucfirst($modulename).' Blogs Browse Page',
  ...
  public function deleteAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $inttable = Engine_Api::_()->getItem('eblog_integrateothermodule', $this->_getParam('integrateothermodule_id'));
        $pageName = "eblog_index_".$this->_getParam('integrateothermodule_id');
        if (!empty($pageName)) {
          $page_id = $db->select()
                  ->from('engine4_core_pages', 'page_id')
                  ->where('name = ?', $pageName)
        ...
        Engine_Api::_()->getDbtable('menuItems', 'core')->delete(array('name =?' => 'eblog_main_browseblog_' . $this->_getParam('integrateothermodule_id')));
      
        $integrateothermodule = Engine_Api::_()->getItem('eblog_integrateothermodule', $this->_getParam('integrateothermodule_id'));
  ...
  public function enabledAction() {
    $db = Engine_Db_Table::getDefaultAdapter();
    $db->beginTransaction();
    $content = Engine_Api::_()->getItemTable('eblog_integrateothermodule')->fetchRow(array('integrateothermodule_id = ?' => $this->_getParam('integrateothermodule_id')));
    try {
      Engine_Api::_()->getDbtable('menuItems', 'core')->update(array('enabled' => !$content->enabled), array('name =?' => 'eblog_main_browseblog_' . $this->_getParam('integrateothermodule_id')));
