...
class Eblog_AlbumController extends Core_Controller_Action_Standard {

	public function init() {
    $id = $this->_getParam('album_id', $this->_getParam('id', null));
    if ($id) {
      $album = Engine_Api::_()->getItem('eblog_album', $id);
      if ($album) {
  ...
  public function createAction() {
    ...
    if($album_id){
    	$album = Engine_Api::_()->getItem('eblog_album', $album_id);
			$this->view->blog_id = $blog_id = $album->blog_id;
		}else{
				$this->view->blog_id = $blog_id = $blog_id;
		}
		$blog = $this->view->blog = Engine_Api::_()->getItem('eblog_blog', $blog_id);

		$isBlogAdmin = Engine_Api::_()->eblog()->checkBlogAdmin($blog);
    if(!$isBlogAdmin)
    return $this->_forward('notfound', 'error', 'core');

		if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('eblogpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblogpackage.enable.package', 1) && empty($_POST)){
    ...
    $this->view->current_count =Engine_Api::_()->getDbtable('albums', 'eblog')->getUserAlbumCount($values);
    $this->view->quota = $quota = 0;
    // Get form
    $this->view->form = $form = new Eblog_Form_Album();
    // Render
    ...
    $db = Engine_Api::_()->getItemTable('eblog_album')->getAdapter();
    $db->beginTransaction();
    try {
...
//     }
  ...
  public function getAlbumAction() {
    ...
    $albums = Engine_Api::_()->getDbTable('albums', 'eblog')->getAlbumsAction($value);
  ...
  public function editAction() {
    ...
    return;
		...
		$album_id = $this->_getParam('album_id',false);
    if($album_id)
    $this->view->album = $album = Engine_Api::_()->getItem('eblog_album', $album_id);
	  else
		return;

		$this->view->blog = $blog = Engine_Api::_()->getItem('eblog_blog', $album->blog_id);
		if ($blog)
    ...
    // Make form
    $this->view->form = $form = new Eblog_Form_Album_Edit();
  ...
  public function deleteAction() {
    ...
    $this->view->album = $album = Engine_Api::_()->getItem('eblog_album', $album_id);
	  ...
	  return;
		...
		$blog = Engine_Api::_()->getItem('eblog_blog', $album->blog_id);
		if ($blog)
		Engine_Api::_()->core()->setSubject($blog);
		else
		return;
    ...
    // In smoothbox
    $this->view->form = $form = new Eblog_Form_Album_Delete();
    if (!$album) {
      $this->view->status = false;
    ...
    try {
      ...
      throw $e;
		...
		$tab_id = Engine_Api::_()->sesbasic()->getWidgetTabId(array('name' => 'eblog.profile-photos'));
		return $this->_forward('success' ,'utility', 'core', array('parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'view','blog_id'=>$blog->custom_url, 'tab' => $tab_id), 'eblog_entry_view', true), 'messages' => Array($this->view->message)
		));
  }
  ...
  public function editphotosAction() {
    ...
    $this->view->album = $album = Engine_Api::_()->getItem('eblog_album', $album_id);
    $this->view->content_item = Engine_Api::_()->getItem('eblog_blog', $album->blog_id);
    ...
    // Get albums
    $myAlbums = Engine_Api::_()->getDbtable('albums', 'eblog')->editPhotos();
    $albumOptions = array('' => '');
    ...
    // Make form
    $this->view->form = $form = new Eblog_Form_Album_Photos();
    foreach ($paginator as $photo) {
      $subform = new Eblog_Form_Album_EditPhoto(array('elementsBelongTo' => $photo->getGuid()));
    ...
    try {
      ...
      foreach ($paginator as $photo) {
        ...
        if (isset($values['delete']) && $values['delete'] == '1') {
          ...
          if (($nextPhoto instanceof Eblog_Model_Photo) &&
