...
class Eblog_ReviewController extends Core_Controller_Action_Standard {
	...
	public function init() {
		...
		//Get subject
		if (null !== ($review_id = $this->_getParam('review_id')) && null !== ($review = Engine_Api::_()->getItem('eblog_review', $review_id)) && $review instanceof Eblog_Model_Review && !Engine_Api::_()->core()->hasSubject()) {
			$blog = $review->getParent();
			Engine_Api::_()->core()->setSubject($review);
		}
		if(isset($blog) && $blog && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('eblogpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('eblogpackage.enable.package', 1)){
  ...
  public function createAction() {
		...
		$viewer = Engine_Api::_()->user()->getViewer();
		$this->view->item = $item = Engine_Api::_()->getItem('eblog_blog', $this->getParam('blog_id'));
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('eblog_review', 'create'))
		return $this->_forward('notfound', 'error', 'core');
		if(!$item)
		return $this->_forward('notfound', 'error', 'core');
		//check review exists
		$isReview = Engine_Api::_()->getDbtable('reviews', 'eblog')->isReview(array('blog_id' => $item->getIdentity()));
		if(Engine_Api::_()->getApi('settings', 'core')->getSetting('eblog.allow.owner', 1)){
			$allowedCreate = true;	
		}
		else{
    ...
    $this->view->form = $form = new Eblog_Form_Review_Create(array('defaultProfileId' => $defaultProfileId));
    ...
    $reviews_table = Engine_Api::_()->getDbtable('reviews', 'eblog');
    ...
    try {
      ...
      $review->save();
		  $dbObject = Engine_Db_Table::getDefaultAdapter();
			...
			$table = Engine_Api::_()->getDbtable('parametervalues', 'eblog');
			...
			foreach($_POST as $key => $reviewC){
				...
				if(!is_numeric($key))
					continue;
				$parameter = Engine_Api::_()->getItem('eblog_parameter',$key);
			...
			if(isset($item->rating)){
				$item->rating = Engine_Api::_()->getDbtable('reviews', 'eblog')->getRating($review->blog_id);
				$item->save();
			}
      ...
      $customfieldform->saveValues();
			$review->save();
      $db->commit();
      return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'review_id' => $review->review_id, 'slug' => $review->getSlug()), 'eblogreview_view', true);
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    }
  ...
  public function editAction() {
    ...
    $review_id = $this->_getParam('review_id', null);
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('eblog_review', 'edit'))
    return $this->_forward('notfound', 'error', 'core');
		$this->view->item = $item = Engine_Api::_()->getItem('eblog_blog', $subject->blog_id);
		if (isset($item->category_id) && $item->category_id != 0)
		$this->view->category_id = $item->category_id;
		else
		$this->view->category_id = 0;
    ...
    if(!$review_id || !$subject)
	  return $this->_forward('notfound', 'error', 'core');
    $this->view->defaultProfileId = $defaultProfileId = Engine_Api::_()->getDbTable('reviewmetas', 'eblog')->profileFieldId();
    $this->view->form = $form = new Eblog_Form_Review_Edit(array('defaultProfileId' => $defaultProfileId));
    ...
    try {
      ...
      $subject->save();
			...
			if(isset($item->rating)){
				$item->rating = Engine_Api::_()->getDbtable('reviews', 'eblog')->getRating($subject->blog_id);
				$item->save();
			}
			 //Add fields
      $customfieldform = $form->getSubForm('fields');
      ...
      $customfieldform->saveValues();
			$subject->save();
      $db->commit();
      return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'review_id' => $subject->review_id, 'slug' => $subject->getSlug()), 'eblogreview_view', true);
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    }
  ...
  public function deleteAction() {
    $viewer = Engine_Api::_()->user()->getViewer();
    $review = Engine_Api::_()->getItem('eblog_review', $this->getRequest()->getParam('review_id'));
    $content_item = Engine_Api::_()->getItem('eblog_blog', $review->blog_id);
  ...
  public function viewAction() {
		...
		return $this->_forward('notfound', 'error', 'core');
		$review_id = $this->_getParam('review_id', null);
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('eblog_review', 'view'))
		return $this->_forward('notfound', 'error', 'core');
  ...
  function likeAction() {
    ...
    $itemTable = Engine_Api::_()->getItemTable('eblog_review');
    $tableLike = Engine_Api::_()->getDbtable('likes', 'core');
    $tableMainLike = $tableLike->info('name');
    $select = $tableLike->select()
            ->from($tableMainLike)
            ->where('resource_type = ?', 'eblog_review')
    ...
    if (count($result) > 0) {
      ...
      try {
        ...
        $like->resource_type = 'eblog_review';
