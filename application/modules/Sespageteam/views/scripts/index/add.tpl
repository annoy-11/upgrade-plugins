...
<script type="application/javascript">
  ...
  <?php if($this->type == 'sitemember') { ?>
    sesJqueryObject(document).on('click','#name',function(e) {
      profileOptionAutoComplete('name', "<?php echo $this->url(array('module' =>'sespageteam','controller' => 'index', 'action' => 'get-data', 'page_id' => $this->page_id),'default',true); ?>");
    });
  <?php } ?>
...
//           }
          if(sesJqueryObject('#sespageteam_teams').length) {
            if(sesJqueryObject('#team_tip').length)
              sesJqueryObject('#team_tip').hide();
            sesJqueryObject('#sespageteam_teams').show();
            sesJqueryObject('#sespageteam_teams').html(result.message);
          }
        }
      }
    });
