 ...
 */
 ...
 ?>
<script>
  sesJqueryObject(document).on('submit', '#sespageteam_delete', function(e) {
    e.preventDefault();
    ...
    sesJqueryObject.ajax({
      ...
      success: function(response) {
        ...
        sesJqueryObject('#sessmoothbox_container').html("<div id='teamsuccess_message' class='sespageteam_success_message teamsuccess_message'><i class='fa-check-circle-o'></i><span>Your team is successfully deleted entry.</span></div>");
        
        sesJqueryObject('#teamsuccess_message').fadeOut("slow", function(){
          setTimeout(function() {
            sesJqueryObject('#sespageteam_team_<?php echo $this->team_id ?>').remove();
            sessmoothboxclose();
          }, 1000);
        });
