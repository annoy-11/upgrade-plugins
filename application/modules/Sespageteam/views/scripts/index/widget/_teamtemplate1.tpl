 ...
 * @version    $Id: _teamtemplate1.tpl  2018-11-14 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
 
 ?>
<?php 
$this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sespageteam/externals/styles/styles.css'); 
$this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sespageteam/externals/styles/template1.css');
?>

<div class="sespageteam_temp1_wrap">
  <div class="sespageteam_temp1_list sesbasic_clearfix<?php if ($this->center_block): ?> iscenter<?php endif; ?>">
    <?php foreach( $this->paginator as $user ):
     ...
     <div class="team_box" style="width:<?php echo $this->width ?>px;">
        <div class="team_box_inner">
          ...
          <?php endif; ?>
          <?php if(!empty($this->content_show) && ($user->email || $user->phone || $user->location || $user->description || $user->email || $user->website || $user->facebook || $user->twitter || $user->linkdin || $user->googleplus)): ?>
          <div class="team_box_bottom">
            ...
            <?php endif; ?>
            <div class="sespageteam-social-icon <?php if(empty($this->sespageteam_social_border)): ?>bordernone<?php endif; ?>">
              <?php if($user->email && !empty($this->content_show) && in_array('email', $this->content_show)): ?>
                <a href="mailto:<?php echo $user->email ?>" title="<?php echo $user->email ?>">
                  <i class="fa fa-envelope sesbasic_text_light"></i>
                </a> 
