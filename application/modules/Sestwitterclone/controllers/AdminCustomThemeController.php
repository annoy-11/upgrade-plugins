 ...
 * @version    $Id: AdminCustomThemeController.php 2019-06-15 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sestwitterclone_AdminCustomThemeController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sestwitterclone_admin_main', array(), 'sestwitterclone_admin_main_customcss');

    $writeable = array();
    $writeable['sestwitterclone'] = false;
    try {
      foreach(array('theme.css', 'constants.css', 'media-queries.css', 'sestwitterclone-custom.css') as $file ) {
        if( !file_exists(APPLICATION_PATH . "/application/themes/sestwitterclone/$file") ) {
          throw new Core_Model_Exception('Missing file');
        } else {
          $this->checkWriteable(APPLICATION_PATH . "/application/themes/sestwitterclone/$file");
        }
      }
      $writeable['sestwitterclone'] = true;
    }
    catch( Exception $e ) {
      $this->view->errorMessage = $e->getMessage();
    }
    $this->view->writeable = $writeable;
    $this->view->activeFileContents = file_get_contents(APPLICATION_PATH . '/application/themes/sestwitterclone/sestwitterclone-custom.css');
  }

  public function saveAction() {

    $theme_id = 'sestwitterclone';
    ...
    //Check file
    $basePath = APPLICATION_PATH . '/application/themes/sestwitterclone';
