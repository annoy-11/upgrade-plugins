  ...
  public function init() {
    ...
    $this->getElement('sesiosapp_licensekey')->getDecorator('Description')->setOptions(array('placement' => 'PREPEND', 'escape' => false));
      ...
      foreach ($it as $file) {
          ...
          if (!in_array($extention, array('pem')))
              continue;
          $pemFile['public/admin/' . $name] = $name;
      }
      if (empty($pemFile))
            $pemFile['new'] = "No pem file found.";



      $this->addElement('Select', 'sesiosapp_server_key', array(
          'label' => 'Apple APN Server Certificate for iOS App',
          'description' => 'Apple APN certificate will be used to send Push Notifications from your server. So, a ".pem" file will be required to enable this service. From this setting, choose the appropriate file for the Push Notification certificate which has been generated at the time of app building and submission. If you are not sure what to choose, then please contact our support team from here.',
          'multiOptions' => $pemFile,
      ...
      $this->addElement('Password', 'sesiosapp_server_password', array(
          'label' => 'APN Server Certificate Password',
          'description' => 'At the time of certificate generation, a password has to be set to protect from unauthentic use. Please enter that password in field below.',
          'values' => $settings->getSetting('sesiosapp_server_password', '')
      ));
      $this->addElement('Radio', 'sesiosapp_apn_mode', array(
          'label' => 'Push Notification Mode',
          'description' => 'Choose Push Notification mode for your iOS App.',
          'multiOptions' => array(
              1 => 'Production',
              0 => 'Development'
          ),
          'value' => $settings->getSetting('sesiosapp_apn_mode', 1)
      ));
