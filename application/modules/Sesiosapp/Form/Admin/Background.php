...
class Sesiosapp_Form_Admin_Background extends Engine_Form {
  public function init() {
    ...
    $it = new DirectoryIterator(APPLICATION_PATH . '/public/admin/');
      ...
      $this->addElement('Select', 'sesiosapp_image_video', array(
          'label' => 'Video/Image',
          'description' => 'Choose from below section what you want to show?',
          'multiOptions' => array('1'=>'Video (forgot password + login)','2'=>'Image'),
          'value'=>$settings->getSetting('sesiosapp_image_video', '2'),
      ));
      $this->getElement('sesiosapp_image_video')->getDecorator('Description')->setOptions(array('placement' => 'PREPEND', 'escape' => false));
      
      
      $this->addElement('Select', 'sesiosapp_video_slide', array(
          'label' => 'Video',
          'description' => '',
          'multiOptions' => $videoFile,
          'value'=>$settings->getSetting('sesiosapp_video_slide', ''),
      ));
      $this->getElement('sesiosapp_video_slide')->getDecorator('Description')->setOptions(array('placement' => 'PREPEND', 'escape' => false));
    ...
    if (count($default_photos_main) > 0) {
      ...
      $description = "<div class='tip'><span>" . Zend_Registry::get('Zend_Translate')->_('There are currently no photo for dashboard menu screen. Photo to be chosen for dashboard menu screen should be first uploaded from the "Layout" >> "<a target="_blank" href="' . $fileLink . '">File & Media Manager</a>" section. => There are currently no photo in the File & Media Manager for the dashboard menu screen. Please upload the Photo to be chosen for dashboard menu screen from the "Layout" >> "<a target="_blank" href="' . $fileLink . '">File & Media Manager</a>" section.') . "</span></div>";
      //Add Element: Dummy
