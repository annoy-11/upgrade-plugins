...
class Sesexpose_AdminCustomThemeController extends Core_Controller_Action_Admin {

  public function indexAction() {
    
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesexpose_admin_main', array(), 'sesexpose_admin_main_customcss');
    
    $writeable = array();
    $writeable['sesexpose'] = false;
    try {
      foreach(array('theme.css', 'constants.css', 'media-queries.css', 'sesexpose-custom.css') as $file ) {
        if( !file_exists(APPLICATION_PATH . "/application/themes/sesexpose/$file") ) {
          throw new Core_Model_Exception('Missing file');
        } else {
          $this->checkWriteable(APPLICATION_PATH . "/application/themes/sesexpose/$file");
        }
      }
      $writeable['sesexpose'] = true;
    }
    catch( Exception $e ) {
      $this->view->errorMessage = $e->getMessage();
    }
    $this->view->writeable = $writeable;
    $this->view->activeFileContents = file_get_contents(APPLICATION_PATH . '/application/themes/sesexpose/sesexpose-custom.css');
  }
  ...
  public function saveAction() {
    ...
    //Check file
    $basePath = APPLICATION_PATH . '/application/themes/sesexpose';
