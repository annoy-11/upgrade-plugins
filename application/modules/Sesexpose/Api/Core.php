 ...
 */
class Sesexpose_Api_Core extends Core_Api_Abstract {
...
//       $moduleArray['user'] = 'Members';
  ...
  public function setPhoto($photo, $menuId = null) {
    ...
    $photo_params = array(
        'parent_id' => $menuId,
        'parent_type' => "sesexpose_slideshow_image",
    );
  ...
  public function readWriteXML($keys, $value, $default_constants = null) {
    ...
    //For constant backup
    $filePath_sp = APPLICATION_PATH . "/application/modules/Sesexpose/externals/styles/sesexpose.xml";
    $results_sp = simplexml_load_file($filePath_sp);
    //For constant backup

    if (!empty($keys) && !empty($value) && ($keys != 'exp_body_background_image' || $keys != 'exp_footer_background_image')) {
      $contactsThemeArray = array($keys => $value);
    } elseif (!empty($keys) && ($keys == 'exp_body_background_image' || $keys == 'exp_footer_background_image')) {
      $contactsThemeArray = array($keys => '');
    } elseif ($default_constants) {
      $contactsThemeArray = $default_constants;
    }

    //For constant backup at file path: /application/modules/Sesexpose/externals/styles/sesexpose.xml
