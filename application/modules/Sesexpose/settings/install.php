...
class Sesexpose_Installer extends Engine_Package_Installer_Module {
...
//   }
  ...
  function onEnable() {
    ...
    if (!empty($info)) {
      $db->update('engine4_core_content', array('name' => 'sesexpose.menu-main'), array('name = ?' => $info['name']));
    }
    ...
    $info = $select->query()->fetch();
    if (!empty($info)) {
      $db->update('engine4_core_content', array('name' => 'sesexpose.menu-mini', 'order' => 2), array(       'name = ?' => $info['name']));
    }
    ...
    $info = $select->query()->fetch();
    if (!empty($info)) {
      $db->update('engine4_core_content', array('name' => 'sesexpose.menu-footer'), array('name = ?' => $info['name']));
    }


    //For upgradation only
    $db->query("INSERT IGNORE INTO `engine4_core_settings` (`name`, `value`) VALUES ('sesexpose.themeactive', '1');");
    parent::onEnable();
  }
...
//     $db->query("UPDATE  `engine4_core_menuitems` SET  `enabled` =  '0' WHERE  `engine4_core_menuitems`.`name` ='core_mini_notification';");
    ...
    //Header Work
    $db->query("UPDATE  `engine4_core_content` SET  `name` =  'core.menu-mini' WHERE  `engine4_core_content`.`name` ='sesexpose.header' LIMIT 1");
    $parent_content_id = $db->select()
	  ...
	  //Footer Work
    $db->query("UPDATE  `engine4_core_content` SET  `name` =  'core.menu-footer' WHERE  `engine4_core_content`.`name` ='sesexpose.menu-footer' LIMIT 1");

    $db->query("UPDATE  `engine4_core_themes` SET  `active` =  '0' WHERE  `engine4_core_themes`.`name` ='sesexpose' LIMIT 1");
