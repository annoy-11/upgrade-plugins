...
class Estore_Widget_ProfilePhotosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $value['store_id'] = $this->view->store_id =  $store_id = Engine_Api::_()->getDbTable('stores', 'estore')->getStoreId($id);
    $this->view->storeItem  = $subject = Engine_Api::_()->getItem('stores', $store_id);
    ...
    if (ESTOREPACKAGE == 1) {
	  ...
	  $getStoreRolePermission = Engine_Api::_()->estore()->getStoreRolePermission($subject->getIdentity(), 'post_content','album',false);
    $canUpload = $getStoreRolePermission ? $getStoreRolePermission : $subject->authorization()->isAllowed($viewer, 'album');
    $this->view->canUpload = (ESTOREPACKAGE == 1) ? $allowed : $canUpload;
    $this->view->store_album_count = $store_album_count = Engine_Api::_()->authorization()->getPermission(($viewer->getIdentity() ? $viewer->level_id : 0), 'estore_store', 'store_album_count');

    //Manage Apps Check
    $isCheck = Engine_Api::_()->getDbTable('managestoreapps', 'estore')->isCheck(array('store_id' => $store_id, 'columnname' => 'photos'));
    if(empty($isCheck))
