...
class Estore_ReviewController extends Core_Controller_Action_Standard {

  public function init() {
    if (!Engine_Api::_()->getApi('settings', 'core')->getSetting('estore.allow.review', 1) || !Engine_Api::_()->sesbasic()->getViewerPrivacy('estore_review', 'view'))
      return $this->_forward('notfound', 'error', 'core');

    //Get subject
    if (null !== ($review_id = $this->_getParam('review_id')) && null !== ($review = Engine_Api::_()->getItem('estore_review', $review_id)) && $review instanceof Estore_Model_Review && !Engine_Api::_()->core()->hasSubject()) {
  ...
  public function createAction() {
    ...
    $this->view->item = $userInfoItem = $item = Engine_Api::_()->getItem('stores', $subjectId);
    if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('estore_review', 'create'))
      return $this->_forward('notfound', 'error', 'core');
    if (!$item)
      return $this->_forward('notfound', 'error', 'core');
    //check review exists
  $isReview = Engine_Api::_()->getDbtable('reviews', 'estore')->isReview(array('store_id' => $item->getIdentity(), 'column_name' => 'review_id'));
    ...
    $values['store_id'] = $item->getIdentity();
    ...
    try {
     ...
     $table = Engine_Api::_()->getDbtable('parametervalues', 'estore');
      ...
      if ($item->owner_id != $viewer->getIdentity()) {
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($item->getOwner(), $viewer, $item, 'estore_reviewpost');
      }

      $db->commit();
      $stats = Engine_Api::_()->estore()->getWidgetParams($viewer->getIdentity());
      ...
      if (Engine_Api::_()->sesbasic()->getViewerPrivacy('estore_review', 'edit')) {
        $this->view->form = $form = new Estore_Form_Review_Create(array( 'reviewId' => $reviewObject->review_id, 'storeItem' => $item));
      ...
      $this->view->rating_count = Engine_Api::_()->getDbTable('reviews', 'estore')->ratingCount($item->getIdentity());
  ...
  public function editAction() {
   $viewer = Engine_Api::_()->user()->getViewer();
    ...
    $this->view->item = $item = Engine_Api::_()->getItem('stores', $subject->store_id);
    ...
    try {
      ...
      if (isset($item->rating)) {
        $item->rating = Engine_Api::_()->getDbtable('reviews', 'estore')->getRating($subject->owner_id);
        $item->save();
      }
      $subject->save();
      ...
      $form->rate_value->setvalue($reviewObject->rating);
      $form->setAction(Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'estore', 'controller' => 'review', 'action' => 'edit', 'review_id' => $reviewObject->review_id), 'default', true));
      $this->view->rating_count = Engine_Api::_()->getDbTable('reviews', 'estore')->ratingCount($reviewObject->owner_id);
      $this->view->total_rating_average = Engine_Api::_()->getDbtable('reviews', 'estore')->getRating($reviewObject->owner_id);
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    }
  ...
  public function deleteAction() {
    $viewer = Engine_Api::_()->user()->getViewer();
    $review = Engine_Api::_()->getItem('estore_review', $this->getRequest()->getParam('review_id'));
    $content_item = Engine_Api::_()->getItem('stores', $review->store_id);
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db->beginTransaction();
      try {
        $reviewParameterTable = Engine_Api::_()->getDbTable('parametervalues', 'estore');
  ...
  public function editReviewAction() {
    ...
    $this->view->form = $form = new Estore_Form_Review_Edit(array('reviewId' => $subject->review_id,  'storeItem' => $item));
