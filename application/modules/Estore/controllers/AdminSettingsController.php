...
// Insert main-middle
  ...
  public function resetPageSettingsAction(){
    ...
    try {
      ...
      include_once APPLICATION_PATH . "/application/modules/Estore/controllers/resetPage.php";
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    ...
    if( $this->getRequest()->getParam('format') == 'smoothbox' ) {
      return $this->_forward('success', 'utility', 'core', array(
        ...
        'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'estore', 'controller' => 'settings', 'action' => 'manage-widgetize-page'), 'admin_default', true),
        'smoothboxClose' => true,
      ));
    }
  }
