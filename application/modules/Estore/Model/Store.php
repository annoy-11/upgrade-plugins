...
class Estore_Model_Store extends Core_Model_Item_Abstract {
  ...
  public function getCoverPhotoUrl() {
    ...
    if (ESTOREPACKAGE == 1) {
      ...
      $defaultPhoto = Engine_Api::_()->authorization()->getPermission(Engine_Api::_()->getItem('user', $this->owner_id), 'stores', 'defaultCphoto');
    }
    if (!$defaultPhoto) {
  ...
  public function getPhotoUrl($type = NULL) {
    ...
    if (ESTOREPACKAGE == 1) {
      ...
      if (isset($params['storedefaultphoto']))
        $defaultPhoto = $params['storedefaultphoto'];
      else
        $defaultPhoto = '';
    } else {
      $defaultPhoto = Engine_Api::_()->authorization()->getPermission(Engine_Api::_()->getItem('user', $this->owner_id), 'stores', 'bsdefaultphoto');
    }
  ...
  public function changeApprovedStatus($approved = 0) {
    ...
    if ($transaction && $orderPackageId) {
      ...
      $this->save();
      Engine_Api::_()->getDbtable('stores', 'estore')->update(array('is_approved' => $approved), array('orderspackage_id' => $orderPackageId));
      if ($approved) {
        $activityApi = Engine_Api::_()->getDbtable('actions', 'activity');
        $action = $activityApi->addActivity(Engine_Api::_()->user()->getViewer(), $this, 'estore_create');
        if ($action) {
