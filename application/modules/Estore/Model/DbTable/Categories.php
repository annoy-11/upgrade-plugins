 ...
 */
class Estore_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Estore_Model_Category';
  ...
  public function getCategory($params = array())
  {
    ...
    $category_select = $category_select->joinLeft($storeTableName, "$storeTableName.category_id=$tableName.category_id AND (`is_approved` = 1 AND `" . $storeTableName . "`.`draft` != 0)", array("total_store_categories" => "COUNT($storeTableName.store_id)"));
    $category_select = $category_select->group("$tableName.category_id");
    if (isset($params['criteria']) && $params['criteria'] == 'most_store')
      $category_select->order('total_store_categories DESC');
    if (isset($params['criteria']) && $params['criteria'] != 'most_store')
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('estore_category');
  ...
  public function getCategoriesAssoc($params = array()) {
    ...
    $stmt = $stmt->order('order DESC')
            ...
            ->fetchAll();
    $data = array();
    if (isset($params['module']) && $params['module'] == 'stores') {
      $data[] = '';
    }
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countStores'])) {
      $storeTable = Engine_Api::_()->getDbTable('stores', 'estore')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($storeTable, "$storeTable.subcat_id=$tableName.category_id", array("total_stores_categories" => "COUNT($storeTable.photo_id)"));
      $category_select = $category_select->group("$tableName.category_id");
      $category_select->order('total_stores_categories DESC');
    }
    $category_select = $category_select->order('order DESC');
    if (isset($params['limit']))
      $category_select->limit($params['limit']);
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countStores'])) {
      $storeTable = Engine_Api::_()->getDbTable('stores', 'estore')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($storeTable, "$storeTable.subsubcat_id=$tableName.category_id", array("total_stores_categories" => "COUNT($storeTable.store_id)"));
