 ...
 */
class Estore_Model_DbTable_Recentlyviewitems extends Engine_Db_Table {

  protected $_name = 'estore_recentlyviewitems';
  protected $_rowClass = 'Estore_Model_Recentlyviewitem';

  public function getitem($params = array()) {

    if (isset($params['type']) && $params['type'] == 'estore_album') {
      $itemTable = Engine_Api::_()->getItemTable('estore_album');
      $fieldName = 'album_id';
    } else {
      $itemTable = Engine_Api::_()->getItemTable('stores');
      $fieldName = 'store_id';
    }
    ...
    $select = $this->select()
            ->from(array('engine4_estore_recentlyviewitems' => $subquery))
