 ...
 */
class Estore_Model_Locationphoto extends Core_Model_Item_Abstract {
  ...
  public function setPhoto($photo, $isShareContent = false) {
    if ($photo instanceof Zend_Form_Element_File) {
      ...
      throw new Estore_Model_Exception('Invalid argument passed to setPhoto: ' . print_r($photo, 1));
    }

    $name = basename($fileName);
    $path = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'temporary';
    $params = array(
        'parent_type' => 'estore_locationphoto',
        'parent_id' => $this->getIdentity()
    );

    $core_settings = Engine_Api::_()->getApi('settings', 'core');
    $main_height = $core_settings->getSetting('estore.mainheight', 1600);
    $main_width = $core_settings->getSetting('estore.mainwidth', 1600);
    $normal_height = $core_settings->getSetting('estore.normalheight', 500);
    $normal_width = $core_settings->getSetting('estore.normalwidth', 500);
