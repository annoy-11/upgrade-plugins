...
class Estore_Form_Create extends Engine_Form {
  ...
  public function init() {
    ...
    if ((Engine_Api::_()->authorization()->isAllowed('stores', $viewer, 'enable_price') && ESTOREPACKAGE != 1) || (ESTOREPACKAGE == 1 && isset($params) && $params['enable_price'])) {
      if ((Engine_Api::_()->authorization()->isAllowed('stores', $viewer, 'price_mandatory') && ESTOREPACKAGE != 1) || (ESTOREPACKAGE == 1 && isset($params) && $params['price_mandatory'])) {
        $required = true;
        $allowEmpty = false;
      } else {
        $required = false;
...
//     }
    ...
    if (!isset($store) && $storeMainPhoto) {
      ...
      $this->addElement('Dummy', 'removeimage', array(
          'content' => '<a class="icon_cancel form-link" id="removeimage1" style="display:none; "href="javascript:void(0);" onclick="removeImage();"><i class="far fa-trash-alt"></i>' . $translate->translate('Remove') . '</a>',
      ));
