...
<ul class="estore_photos_flex_view sesbasic_bxs">
      <?php foreach( $this->results as $item ):
        if(isset($item->store_id))
        $store = Engine_Api::_()->getItem('stores', $item->resource_id);
        $album = Engine_Api::_()->getItem('estore_album', $item['album_id']);  ?> 
        <li id="thumbs-photo-<?php echo $album->photo_id ?>" class="estore_album_list_grid_thumb estore_album_list_grid sespa-i-<?php echo (isset($widgetParams['insideOutside']) && $widgetParams['insideOutside'] == 'outside') ? 'outside' : 'inside'; ?> sespa-i-<?php echo (isset($widgetParams['fixHover']) && $widgetParams['fixHover'] == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($widgetParams['width']) ? $widgetParams['width'].'px' : $widgetParams['width'] ?>;"> 
        	<article> 

          <a class="estore_album_list_grid_img" href="<?php echo Engine_Api::_()->estore()->getHref($album->getIdentity(),$album->album_id); ?>" style="height:<?php echo is_numeric($widgetParams['height']) ? $widgetParams['height'].'px' : $widgetParams['height'] ?>;">
          ...
          if(in_array('likeButton', $widgetParams['show_criteria']) && Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && $canComment) {  ?>
            ...
            if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && in_array('favouriteButton', $widgetParams['show_criteria'])){
                  $albumFavStatus = Engine_Api::_()->getDbTable('favourites', 'estore')->isFavourite(array('resource_type'=>$album->getType(),'resource_id'=>$album->getIdentity())); ?>
              <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $album->getType(); ?>" data-url='<?php echo $album->album_id; ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_fav_btn estore_albumFav <?php echo ($albumFavStatus)>0 ? 'button_active' : '' ; ?>">
     ...
     <?php } ?>
      <?php if(in_array('like', $widgetParams['show_criteria']) || in_array('comment', $widgetParams['show_criteria']) || in_array('view', $widgetParams['show_criteria']) || in_array('title', $widgetParams['show_criteria']) || in_array('photoCount', $widgetParams['show_criteria']) || in_array('favouriteCount', $widgetParams['show_criteria'])  || in_array('by', $widgetParams['show_criteria']) || in_array('storeName', $widgetParams['show_criteria'])){ ?>
        ...
        <span class="estore_album_list_grid_stats">
          <?php if(!empty($store) && in_array('storeName', $widgetParams['show_criteria'])) { ?>
