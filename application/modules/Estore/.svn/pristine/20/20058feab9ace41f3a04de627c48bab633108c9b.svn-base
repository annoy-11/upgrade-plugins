 ...
 */
class Estore_Model_DbTable_Followers extends Engine_Db_Table {

  protected $_rowClass = "Estore_Model_Follower";
  protected $_name = "estore_followers";
  ...
  function getCategoryFollowers($resourceId = "") {
    $select = $this->select()
            ->from($this->info('name'), 'owner_id')
            ->where('resource_type = ?', 'estore_category')
            ->where('resource_id = ?', $resourceId);
    return $this->fetchAll($select);
  }
