...
<script type="text/javascript">
  ...
  if(document.getElementById('store_password-wrapper') && sesJqueryObject('#enable_lock').val() == 0){
    document.getElementById('store_password-wrapper').style.display = 'none';	
    sesJqueryObject('#store_password').val('');
  }
  else if(document.getElementById('store_password-wrapper')){
    document.getElementById('store_password-wrapper').style.display = 'block';	
    sesJqueryObject('#store_password').val('<?php echo $this->store->store_password; ?>');
  }
  //trim last -
  ...
  jqueryObjectOfSes('#check_custom_url_availability').click(function(){
    ...
    var obj = this;
    jqueryObjectOfSes.post('<?php echo $this->url(array('controller' => 'ajax','module'=>'estore', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value,store_id:<?php echo $this->store->store_id ?>},function(response){
      sesJqueryObject('#estore_custom_url_loading').hide();
      ...
      if(response.error){
         ...
         jqueryObjectOfSes(obj).find('#estore_custom_url_wrong').css('display','inline-block');
      }else{
         jqueryObjectOfSes('#custom_url').css('border-color','green');
         jqueryObjectOfSes(obj).find('#estore_custom_url_wrong').hide();
         jqueryObjectOfSes(obj).find('#estore_custom_url_correct').css('display','inline-block');
      }
   });
  });
  //tags
