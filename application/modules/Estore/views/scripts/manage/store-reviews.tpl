...
<?php if(!$this->is_ajax || !$this->is_search_ajax){ ?>
    <div class='admin_search estore_reviews_table'>
        ...
        <li class="_active"><a href="<?php echo $this->url(array('action'=>'store-reviews'), 'estore_account', true); ?>" class="sesbasic_dashboard_nopropagate_content dashboard_a_link"><i class="fa fa-star"></i> <?php echo $this->translate('Store Reviews'); ?></a></li>
        <li><a href="<?php echo $this->url(array('action'=>'product-reviews'), 'estore_account', true); ?>" class="sesbasic_dashboard_nopropagate_content dashboard_a_link"><i class="fa fa-star"></i> <?php echo $this->translate('Product Reviews'); ?></a></li>
      </ul>
<?php } ?>
...
sesJqueryObject('#loadingimgestore-wrapper').hide();
 sesJqueryObject(document).on('submit','#filter_form',function(event){
	...
	new Request.HTML({
			method: 'get',
			url :  en4.core.baseUrl + 'estore/manage/store-reviews/',
			data : {
				format : 'html',
