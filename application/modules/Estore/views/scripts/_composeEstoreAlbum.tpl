...
<?php
  ...
  $this->headLink()
    ...
    ->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Estore/externals/styles/styles.css');
...
<script type="text/javascript">
 if(window.location.href.indexOf("messages/compose") > -1 || window.location.href.indexOf("messages/view/id") > -1) {
  var isMessageStore = true;
 }else{
  var isMessageStore = false;
 }

<?php if(!Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('sesadvancedactivity')) { ?>
  en4.core.runonce.add(function() {
    if (Composer.Plugin.Estorephoto)
      return;

    Asset.javascript('<?php echo $this->layout()->staticBaseUrl ?>application/modules/Estore/externals/scripts/composer_photo.js', {
      onLoad:  function() {
        var type = 'wall';
        if (composeInstance.options.type) type = composeInstance.options.type;
        composeInstance.addPlugin(new Composer.Plugin.Estorephoto({
          ...
          requestOptions : {
            'url'  : en4.core.baseUrl + 'estore/album/upload-photo/type/'+type+'/store_id/<?php echo $subject->getIdentity(); ?>',
          },
          fancyUploadOptions : {
            'url'  : en4.core.baseUrl + 'estore/album/compose-upload/format/json/type/'+type,
...
<?php }else{  ?>  
<?php $this->headScript()->appendFile($this->layout()->staticBaseUrl . 'application/modules/Estore/externals/scripts/composer_photo.js'); ?>
...
obj.on('drop', function (e) 
{
   ...
   handleFileUploadEstore(files,obj);
});
...
sesJqueryObject(document).on('click','div[id^="abortPhoto_"]',function(){
    ...
    if(isMessageStore && (sesJqueryObject('#toValues-wrapper').length > 0 || sesJqueryObject('#submit-wrapper').length > 0)){
...
function createStatusbar(obj,file)
{
     ...
     if(rowCount %2 ==0) row ="even";
		  var checkedId = sesJqueryObject("input[name=cover]:checked");
			...
			this.abort = sesJqueryObject('<div class="abort estore_upload_item_abort" id="abortPhoto_'+countUploadSes+'"><span><?php echo $this->translate("Cancel Uploading"); ?></span></div>').appendTo(this.objectInsert);
			...
			this.src = sesJqueryObject('<img src="'+en4.core.baseUrl+'application/modules/Estore/externals/images/blank-img.gif">').appendTo(this.imageContainer);
...
var fdSes = new Array();
function handleFileUploadEstore(files,obj)
{
...
var execute = true;
function executeuploadEstore(){
	if(Object.keys(filesArray).length == 0 && sesJqueryObject('#show_photo').html() != ''){
   if(isMessageStore && (sesJqueryObject('#toValues-wrapper').length > 0 || sesJqueryObject('#submit-wrapper').length > 0)){}else{
    ...
    {
     	sendFileToServerEstore(filesArray[i],statusArray[i],filesArray[i],'upload',i);
			break;
    }			
	 }
	}
...
var jqXHR = new Array();
function sendFileToServerEstore(formData,status,file,isURL,i)
{
		execute = false;
    ...
    var uploadURL =en4.core.baseUrl + 'estore/album/upload-photo/type/'+type+'/store_id/<?php echo $subject->getIdentity(); ?>';
...
function readImageUrlEstore(input) {
	handleFileUploadEstore(input.files,sesJqueryObject('#dragandrophandler'));
}
...
sesJqueryObject(document).on('click','.delete_image_upload',function(e){
    ...
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'estore', 'controller' => 'index', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'estore', 'controller' => 'index', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
</script>
