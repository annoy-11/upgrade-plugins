...
<div class="layout_middle">
  <div class="sesbasic_ext_breadcrumb sesbasic_bxs sesbasic_clearfix">
	...
	<div class="estore_album_form"> <?php echo $this->form->render($this) ?> </div>
</div>
<script type="text/javascript">
  var updateTextFields = function() {
    var fieldToggleStore = ['#title-wrapper', '#description-wrapper'];
    fieldToggleStore = $$(fieldToggleStore.join(','))
    if ($('album').get('value') == 0)
    fieldToggleStore.show();
    else 
    fieldToggleStore.hide();
  }
...
<script type="text/javascript">
  ...
  jqueryObjectOfSes('#estore_create_form_tabs li a').on('click',function(){
...
function createStatusbar(obj,file)
{
     ...
     if(rowCount %2 ==0) row ="even";
		  var checkedId = jqueryObjectOfSes("input[name=cover]:checked");
			this.objectInsert = jqueryObjectOfSes('<div class="estore_upload_item sesbm '+row+'"></div>');
			this.overlay = jqueryObjectOfSes("<div class='overlay estore_upload_item_overlay'></div>").appendTo(this.objectInsert);
			this.abort = jqueryObjectOfSes('<div class="abort estore_upload_item_abort" id="abortPhoto_'+countUploadSes+'"><span><?php echo $this->translate("Cancel Uploading"); ?></span></div>').appendTo(this.objectInsert);
			this.progressBar = jqueryObjectOfSes('<div class="overlay_image progressBar"><div></div></div>').appendTo(this.objectInsert);
			this.imageContainer = jqueryObjectOfSes('<div class="estore_upload_item_photo"></div>').appendTo(this.objectInsert);
			this.src = jqueryObjectOfSes('<img src="'+en4.core.baseUrl+'application/modules/Estore/externals/images/blank-img.gif">').appendTo(this.imageContainer);
			this.infoContainer = jqueryObjectOfSes('<div class=estore_upload_photo_info sesbasic_clearfix"></div>').appendTo(this.objectInsert);
			this.size = jqueryObjectOfSes('<span class="estore_upload_item_size sesbasic_text_light"></span>').appendTo(this.infoContainer);
			this.filename = jqueryObjectOfSes('<span class="estore_upload_item_name"></span>').appendTo(this.infoContainer);
			this.option = jqueryObjectOfSes('<div class="estore_upload_item_options clear sesbasic_clearfix"><span class="estore_upload_item_radio"><input type="radio" id="main_photo_id'+rowCount+'" name="cover"><label for="main_photo_id'+rowCount+'"><?php echo $this->translate("Main Photo"); ?></label></span><a class="edit_image_upload" href="javascript:void(0);"><img src="'+en4.core.baseUrl+'application/modules/Estore/externals/images/edit.png"></a><a class="delete_image_upload" href="javascript:void(0);"><img src="'+en4.core.baseUrl+'application/modules/Estore/externals/images/error.png"></a></div>').appendTo(this.objectInsert);
		...
		if ($('album').get('value') == 0) {
			if(jqueryObjectOfSes('#show_photo').children('div').length == 1) {
				var idPhoto = jqueryObjectOfSes('#show_photo').eq(0).find('.estore_upload_item_radio').find('input').attr('id');
...
function sendFileToServer(formData,status,file,isURL,i)
{
    ...
    var uploadURL =$('form-upload').action + '?ul=1'+url+'&store_id='+store_id; //Upload URL
    ...
    processData: false,
		...
		success: function(response){
					...
					if (response.status) {
							...
							status.option.find('.estore_upload_item_radio').find('input').attr('value',response.photo_id);
...
jqueryObjectOfSes(document).on('click','.edit_image_upload',function(e){
	e.preventDefault();
	var photo_id = jqueryObjectOfSes(this).closest('.estore_upload_item_options').attr('data-src');
	if(photo_id){
		editImage(photo_id);
	}else
		return false;
...
jqueryObjectOfSes(document).on('click','.delete_image_upload',function(e){
	e.preventDefault();
	jqueryObjectOfSes(this).parent().parent().find('.estore_upload_item_overlay').css('display','block');
	var sesthat = this;
	var isCover = jqueryObjectOfSes(this).closest('.estore_upload_item_options').find('.estore_upload_item_radio').find('input').prop('checked');
	var photo_id = jqueryObjectOfSes(this).closest('.estore_upload_item_options').attr('data-src');
	if(photo_id){
		request = new Request.JSON({
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'estore', 'controller' => 'album', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'estore', 'controller' => 'album', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
</script>
