        ...
        <div class="_buttons">      
          ...
          <?php if(Engine_Api::_()->estore()->storePrivacy($store, 'edit')):?>
            <div class="_deactivate_button">
              <a href="<?php echo $this->url(array('action' => 'edit', 'store_id' => $store->custom_url), 'estore_dashboard', 'true');?>" class="estore_button sesbasic_animation"><i class="fa fa-pencil"></i></a>
          ...
          <?php if(ESTOREPACKAGE == 1):?>
            ...
            <?php if(!$package->isFree()): ?>
                  ...
                  <?php if($package->is_renew_link):?>
                    <?php if(!empty($transaction->expiration_date) && $transaction->expiration_date != '3000-00-00 00:00:00'):?>
                      ...
                      <?php if($daysLeft <= $renew_link_days || strtotime($transaction->expiration_date) <= time()): ?>
                        <div><a href="<?php echo $this->url(array('store_id' => $item->store_id,'action'=>'index'), 'estorepackage_payment', true); ?>" class="estore_button sesbasic_animation"><i class="fa fa-money"></i><span><?php echo $this->translate("Review Store Payment"); ?></span></a></div>
                      <?php endif;?>
                    <?php else: ?>
              ...
              <?php else:?>
              	<div>
                	<a href="<?php echo $this->url(array('store_id' => $store->store_id,'action'=>'index'), 'estorepackage_payment', true); ?>" class="estore_button"><i class="fa fa-money"></i><span><?php echo $this->translate("Make Payment")?></span></a>
              	</div>
              <?php endif;?>
            <?php endif;?>
          <?php endif;?>
