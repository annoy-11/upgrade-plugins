  ...
  <?php $ownerSelf = $viewer->getIdentity() == $item->owner_id ? true : false; ?>
	<div class="estore_review_listing_btn_right floatR">
		<?php if($item->authorization()->isAllowed($viewer, 'edit') && $ownerSelf) { ?>
			<a class="fa fa-pencil sesbasic_button sesbasic_button_icon <?php if($ownerSelf) { echo 'estore_own_update_review'; } ?>" href="<?php echo $this->url(array('route' => 'estorereview_view', 'action' => 'edit-review', 'review_id' => $item->review_id,'format' => 'smoothbox'),'estorereview_view',true); ?>" <?php if(!$ownerSelf) { ?> onclick='return opensmoothboxurl(this.href);' <?php  } ?> ><span><i class="fa fa-caret-down"></i><?php echo $this->translate('Edit Review'); ?></span></a>
		<?php } ?>
