 ...
 * @version    $Id: view.tpl  2018-07-13 00:00:00 SocialEngineSolutions $
...
<div class="sesbasic_view_stats_popup">
  ...
  <table>
    <tr>
      ...
      ?>
      <?php else: ?>
      <?php $path = $this->baseUrl() . '/application/modules/Estore/externals/images/nophoto_store_thumb_icon.png'; ?>
      <?php endif; ?>
    ...
    <?php if($this->item->category_id != '' && intval($this->item->category_id) && !is_null($this->item->category_id)):
            $categoryItem = Engine_Api::_()->getItem('estore_category', $this->item->category_id);
     ?>
    <tr>
    ...
    <?php if(strtotime($this->item->enddate) < strtotime(date('Y-m-d')) && $this->item->offtheday == 1){ 
                    Engine_Api::_()->getDbTable('stores', 'estore')->update(array(
                        'offtheday' => 0,
                        'startdate' =>'',
                        'enddate' =>'',
                      ), array(
