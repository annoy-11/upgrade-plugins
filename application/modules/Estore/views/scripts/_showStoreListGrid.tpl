      ...
      <?php if($store->lat): ?>
		...
		<?php if(isset($this->featuredLabelActive) || isset($this->sponsoredLabelActive)):?>
		  ...
		  <?php if(isset($this->featuredLabelActive) && $store->featured == 1):?>
            <?php $labels .= "<span class=\"estore_label_featured\" title='".$this->translate('FEATURED')."'><i class='fa fa-star'></i></span>";?>
          <?php endif;?>
          <?php if(isset($this->sponsoredLabelActive) && $store->sponsored == 1):?>
            <?php $labels .= "<span class=\"estore_label_sponsored\" title='".$this->translate('SPONSORED')."'><i class='fa fa-star'></i></span>";?>
          <?php endif;?>
          <?php $labels .= "<span class=\"estore_label_hot\" title='".$this->translate('HOT')."'><i class='fa fa-star'></i></span>";?>
          <?php $labels .= "</div>";?>
		<?php endif;?>
		<?php $vlabel = '';?>
		...
		if(isset($this->commentActive)){
          $stats .= '<span title="'.$this->translate(array('%s comment', '%s comments', $store->comment_count), $this->locale()->toNumber($store->comment_count)).'"><i class="fa fa-comment"></i>'.$store->comment_count.'</span>';
		}
		if(isset($this->favouriteActive) && Engine_Api::_()->getApi('settings', 'core')->getSetting('estore.enable.favourite', 1)){
          $stats .= '<span title="'.$this->translate(array('%s favourite', '%s favourites', $store->favourite_count), $this->locale()->toNumber($store->favourite_count)).'"><i class="fa fa-heart"></i>'. $store->favourite_count.'</span>';
		}
		if(isset($this->viewActive)){
			$stats .= '<span title="'. $this->translate(array('%s view', '%s views', $store->view_count), $this->locale()->toNumber($store->view_count)).'"><i class="fa fa-eye"></i>'.$store->view_count.'</span>';
		}
		if(isset($this->likeActive)){
			$stats .= '<span title="'.$this->translate(array('%s like', '%s likes', $store->like_count), $this->locale()->toNumber($store->like_count)).'"><i class="fa fa-thumbs-up"></i>'.$store->like_count.'</span> ';
		}
		?>
   <?php if(isset($this->memberActive)){
      $stats .= '<span title=""><i class="fa fa-user"></i>&nbsp; '.$store->member_count.'</span>';
    }
    if(isset($this->totalProductActive)){
      $stats .= '<span title=""><i class="fa fa-shopping-bag"></i>&nbsp;'.$store->product_count.'</span>';
        }
    ?>
	  <?php $stats .= '</div>';?>
      <?php $socialshare = $this->partial('_dataSharing.tpl','estore',array('mapstore' => $store,'contactButton' => isset($this->contactButtonActive) ? 1 : 0, 'joinButton' => isset($this->joinButtonActive) ? 1 : 0, 'socialSharing' => isset($this->socialSharingActive) ? 1 : 0)); ?>
...
<?php if($this->view_type == 'map'):?>
  ...
  <?php if(!$this->view_more || $this->is_search):?>
    <li id="estore_map_view_<?php echo $randonNumber;?>" style="width:100%">
