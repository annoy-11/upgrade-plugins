 ...
 * @version    $Id: review-parameters.tpl 2016-05-25 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

?>
<?php if(!empty($this->store)) { ?>
<?php $reviewParameters = Engine_Api::_()->getDbtable('parameters', 'estore')->getParameterResult(array('category_id'=>$this->store->category_id));?>
<?php } ?>
<?php if(count($reviewParameters)):?>
  <?php foreach($reviewParameters as $value):?>
    <div class="form-wrapper estore_form_review_star">
      <div class="form-label"><label><?php echo $this->translate($value['title']); ?></label></div>
      <div id="estore_review_rating" class="sesbasic_rating_parameter estore_rating_star_element" onmouseout="rating_out_review(<?php echo $value['parameter_id'] ?>);">
...
<script type="text/javascript">
  ...
  window.addEvent('domready', function() {
    var countExistsParam = sesJqueryObject('.estore_review_values');
