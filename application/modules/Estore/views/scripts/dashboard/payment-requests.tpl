...
<div class="estore_request_payment_link ">	
	<a href="<?php echo $this->url(array('store_id' => $this->store->custom_url,'action'=>'payment-request'), 'estore_dashboard', true); ?>" class="openSmoothbox sesbasic_button fa fa-money-bill"><?php echo $this->translate("Make Request For Payment."); ?></a>
</div>
<?php } ?>
        ...
        <tr>
          ...
          <td class="table_options">
          	<?php if ($item->state == 'pending'){ ?>
                <?php echo $this->htmlLink($this->url(array('store_id' => $this->store->custom_url,'action'=>'payment-request','id'=>$item->userpayrequest_id), 'estore_dashboard', true), $this->translate(""), array('class' => 'openSmoothbox fa fa-edit','title'=>$this->translate("Edit Request"))); ?>
