 ...
 * @version    $Id: favourite-store.tpl  2018-07-13 00:00:00 SocialEngineSolutions $
...
<script type="text/javascript">
  ...
  function viewMore() {
    ...
    var id = '<?php echo $this->store_id; ?>';
    
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'estore/ajax/favourite-store/store_id/' + id ,
      'data': {
        format: 'html',
...
<?php if (empty($this->viewmore)): ?>
  <div class="sesbasic_items_listing_popup">
    <div class="sesbasic_items_listing_header">
         <?php echo $this->translate('People who added this store as Favourite') ?>
    ...
    <?php if (!empty($this->paginator) && $this->paginator->count() > 1 && empty($this->viewmore)): ?>
      <?php if ($this->paginator->getCurrentPageNumber() < $this->paginator->count()): ?>
        <div class="sesbasic_view_more sesbasic_load_btn" id="view_more" onclick="viewMore();" >
          <?php echo $this->htmlLink('javascript:void(0);', $this->translate('View More'), array('id' => 'feed_viewmore_link', 'class' => 'sesbasic_animation estore_link_btn fa fa-sync')); ?>
        </div>
