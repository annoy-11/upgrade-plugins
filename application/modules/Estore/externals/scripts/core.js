...
function estore_likefavourite_data(element) {
  ...
  if (clickType == 'like_view') {
    ...
    var elementId = '.estore_favourite_' + contentId;
    if (sesJqueryObject(element).data('status') == 1) {
      sesJqueryObject('.estore_favourite_view_' + contentId).html('<i class="fa fa-heart-o"></i><span>' + en4.core.language.translate("Add to Favorite") + '</span>');
...
//        sesJqueryObject('#estore_sent_meesage').show();
//        sesJqueryObject('#estore_contact_popup').append('<div id="estore_sent_meesage">Bharat Work</div>');
//        window.setTimeout(function() {sesJqueryObject('#estore_sent_meesage').remove()}, 30000);
//				sessmoothboxclose();
      }
    }
  });
