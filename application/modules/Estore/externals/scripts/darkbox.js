...
;(function() {
  sesJqueryObject(function($) {
    ...
    // Call darkbox
    sesJqueryObject(document).on("click", "[data-darkbox],[data-darkbox-store]", function(e) {

      var src = $(this).data("darkbox"),
          isDummy = !src, // (is just a link calling a store)
          storeID = $(this).data("darkbox-store");

      $imagesStore = !storeID ? $(this) : $('[data-darkbox-store="'+ storeID +'"]').filter("[data-darkbox]");
      n = $imagesStore.length;
      c = isDummy ? 0 : $imagesStore.index( this );
