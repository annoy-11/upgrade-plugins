...
class Sesgroupalbum_Widget_BrowseAlbumsController extends Engine_Content_Widget_Abstract
{
	public function indexAction(){
      ...
      default:
		...
		$params = $this->view->params = array('width'=>$value['defaultWidth'],'height'=>$value['defaultHeight'],'limit_data' => $value['limit_data'],'sort'=>$value['sort'],'search'=>$value['search'],'tag_id'=>$value['tag_id'],'load_content'=>$value['load_content'],'show_criterias'=>$value['show_criterias'],'title_truncation' =>$value['title_truncation'],'lat'=>$value['lat'],'lng'=>$value['lng'],'location'=>$value['location'],'miles'=>$value['miles'],'insideOutside' =>$insideOutside,'fixHover'=>$fixHover,'user_id'=>$value['user_id'],'view_type'=>$view_type,'show'=>$value['show']);
