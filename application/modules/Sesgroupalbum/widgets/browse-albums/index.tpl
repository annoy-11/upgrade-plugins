...
<?php if(isset($this->identityForWidget) && !empty($this->identityForWidget)){
 ...
 <div class="sesgroupalbum_search_result sesbasic_clearfix sesbm" id="<?php echo !$this->is_ajax ? 'paginator_count_sesgroupalbum' : 'paginator_count_ajax_sesgroupalbum' ?>"><span id="total_item_count_sesgroupalbum" style="display:inline-block;"><?php echo $this->paginator->getTotalItemCount(); ?></span> <?php echo $this->translate("albums found."); ?></div>
 <?php if(!$this->is_ajax){ ?>
 ...
 <?php }
 				 $allowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.album.rating',1);
					$allowShowPreviousRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.ratealbum.show',1);
          if($allowRating == 0){
        ...
        <li id="thumbs-photo-<?php echo $album->photo_id ?>" class="sesgroupalbum_list_grid_thumb sesgroupalbum_list_grid sesa-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sesa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">  
          ...
          <?php  if(isset($this->socialSharing) || isset($this->likeButton) || isset($this->favouriteButton)){  ?>
            <span class="sesgroupalbum_list_grid_btns">
             ...
             ?>
              <a href="<?php echo 'http://www.facebook.com/sharer/sharer.php?u=' . $urlencode . '&t=' . $album->getTitle(); ?>" onclick="return socialSharingPopUp(this.href,'<?php echo $this->translate('Facebook'); ?>')" class="sesgroupalbum_icon_btn sesgroupalbum_icon_facebook_btn"><i class="fa fa-facebook"></i></a>
              <a href="<?php echo 'https://twitter.com/intent/tweet?url=' . $urlencode . '&text=' . $album->getTitle().'%0a'; ?>" onclick="return socialSharingPopUp(this.href,'<?php echo $this->translate('Twitter')?>')" class="sesgroupalbum_icon_btn sesgroupalbum_icon_twitter_btn"><i class="fa fa-twitter"></i></a>
              <a href="<?php echo 'http://pinterest.com/pin/create/button/?url='.$urlencode; ?>&media=<?php echo urlencode((strpos($album->getPhotoUrl('thumb.main'),'http') === FALSE ? (((!empty($_SERVER["HTTPS"]) && strtolower($_SERVER["HTTPS"] == 'on')) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] ) : $album->getPhotoUrl('thumb.main')) . $album->getPhotoUrl('thumb.main')); ?>&description=<?php echo $album->getTitle();?>" onclick="return socialSharingPopUp(this.href,'<?php echo $this->translate('Pinterest'); ?>')" class="sesgroupalbum_icon_btn sesgroupalbum_icon_pintrest_btn"><i class="fa fa-pinterest"></i></a>
              <?php }
              ...
              if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->likeButton) && $canComment){  ?>
                <!--Album Like Button-->
                <?php $albumLikeStatus = Engine_Api::_()->sesgroupalbum()->getLikeStatus($album->album_id); ?>
                <a href="javascript:;" data-src='<?php echo $album->album_id; ?>' class="sesgroupalbum_icon_btn sesgroupalbum_icon_like_btn sesgroupalbum_albumlike <?php echo ($albumLikeStatus) ? 'button_active' : '' ; ?>">
                  <i class="fa fa-thumbs-up"></i>
                  <span><?php echo $album->like_count; ?></span>
                </a>
              <?php } 
              if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->favouriteButton)){
                    $albumFavStatus = Engine_Api::_()->getDbtable('favourites', 'sesgroupalbum')->isFavourite(array('resource_type'=>'sesgroupalbum_album','resource_id'=>$album->album_id)); ?>
                <a href="javascript:;" data-src='<?php echo $album->album_id; ?>' class="sesgroupalbum_icon_btn sesgroupalbum_icon_fav_btn sesgroupalbum_albumFav <?php echo ($albumFavStatus) ? 'button_active' : '' ; ?>">
         ...
         <?php if(isset($this->like) || isset($this->comment) || isset($this->view) || isset($this->title) || isset($this->rating) || isset($this->photoCount) || isset($this->favouriteCount) || isset($this->downloadCount)  || isset($this->by)){ ?>
              ...
              <span class="sesgroupalbum_list_grid_stats">
                ...
                <?php
                          $user_rate = Engine_Api::_()->getDbtable('ratings', 'sesgroupalbum')->getCountUserRate('album',$album->album_id);
                          $textuserRating = $user_rate == 1 ? 'user' : 'users'; 
                          $textRatingText = $album->rating == 1 ? 'rating' : 'ratings'; ?>
                       <span class="sesgroupalbum_list_grid_rating" title="<?php echo $album->rating.' '.$this->translate($textRatingText.' by').' '.$user_rate.' '.$this->translate($textuserRating); ?>">
              ...
              <span class="sesgroupalbum_list_grid_stats sesbasic_text_light">
               ...
               <?php if(isset($this->favouriteCount)) { ?>
                  <span class="sesgroupalbum_list_grid_fav" title="<?php echo $this->translate(array('%s favourite', '%s favourites', $album->favourite_count), $this->locale()->toNumber($album->favourite_count))?>">
                    <i class="fa fa-heart"></i> 
                    <?php echo $album->favourite_count;?>            
                  </span>
                <?php } ?>
                <?php if(isset($this->downloadCount)) { ?>
                <span class="sesgroupalbum_list_grid_download" title="<?php echo $this->translate(array('%s download', '%s downloads', $album->download_count), $this->locale()->toNumber($album->download_count))?>">
      ...
      <?php }else{ ?>
      		<?php $image = Engine_Api::_()->sesgroupalbum()->getAlbumPhoto($album->getIdentity(),$album->photo_id,3);  ?>
          <?php if(count($image) == 0) {
          	$heightDiv = (str_replace('px','',$this->height)) + 100;
          }else{
          	$heightDiv = $this->height;
          } ?>
          <?php $imageURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref($album); ?>
          <li class="sesgroupalbum_list_thumbnail_view" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">
            <div class="sesgroupalbum_album_thumbnail_view_main_img" style="height:<?php echo is_numeric($heightDiv) ? $heightDiv.'px' : $heightDiv ?>;">
              <a class="ses-image-viewer" onclick="getRequestedAlbumPhotoForImageViewer('<?php echo $album->getPhotoUrl(); ?>','<?php echo $imageURL	; ?>')" href="<?php echo Engine_Api::_()->sesgroupalbum()->getHrefPhoto($album->photo_id,$album->album_id); ?>"><span class="sesgroupalbum_album_thumbnail_view_thumb" style="background-image:url(<?php echo $album->getPhotoUrl('thumb.profile'); ?>);"></span></a>
            </div>
						<?php 
            		if(count($image) != 0) { ?>
                <div class="sesgroupalbum_album_thumbnail_view_thumbs thumbs<?php echo count($image); ?> clear sesbasic_clearfix">
             		<?php	foreach($image as $key=>$valuePhoto){ ?>
                <div>
                	<?php $imageURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref($valuePhoto); ?>
                  <a onclick="getRequestedAlbumPhotoForImageViewer('<?php echo $valuePhoto->getPhotoUrl(); ?>','<?php echo $imageURL	; ?>')" href="<?php echo Engine_Api::_()->sesgroupalbum()->getHrefPhoto($valuePhoto->getIdentity(),$valuePhoto->album_id); ?>" class="ses-image-viewer"><span class="sesgroupalbum_album_thumbnail_view_thumb" style="background-image:url(<?php echo Engine_Api::_()->sesgroupalbum()->photoUrlGet($valuePhoto->photo_id,'thumb.icon');  ?>);"></span></a>
                </div>
            <?php } ?>
            		</div>
            <?php	} ?>
            <div class="sesgroupalbum_album_thumbnail_view_btm clear sesbasic_clearfix">
              <div class="sesgroupalbum_album_thumbnail_owner_photo">
              	<?php $user = Engine_Api::_()->getItem('user',$album->owner_id) ?>
                <a href="<?php echo $user->getHref();; ?>">
                  <?php echo $this->itemPhoto($user, 'thumb.profile'); ?>
                 </a>          
              </div>
              <div class="sesgroupalbum_album_thumbnail_album_info">
                <p class="sesgroupalbum_album_thumbnail_album_name">
                  <?php if(isset($this->title)) { ?>
                   <?php echo $this->htmlLink($album, $this->string()->truncate($album->getTitle(), $this->title_truncation),array('title'=>$album->getTitle())) ; ?>
                  <?php } ?>
                </p>
                <p class="sesgroupalbum_album_thumbnail_album_by sesbasic_clearfix sesbasic_text_light"> 
                  <?php if(isset($this->by)) { ?>
                    <span>
                      <?php echo $this->translate('By');?>
                       <?php echo $this->htmlLink($album->getOwner()->getHref(), $album->getOwner()->getTitle(), array('class' => 'thumbs_author')) ?>
                ...
                <p class="sesgroupalbum_album_thumbnail_album_stats sesgroupalbum_list_stats sesbasic_clearfix sesbasic_text_light">
                	<?php if(isset($this->like)) { ?>
            ...
            <?php  if(isset($this->socialSharing) || isset($this->likeButton) || isset($this->favouriteButton)){  ?>
            	<p class="sesgroupalbum_album_list_btns sesgroupalbum_album_thumbnail_album_btns">
             	<?php if(isset($this->socialSharing)){ 
              //album viewpage link for sharing
                $urlencode = urlencode(((!empty($_SERVER["HTTPS"]) &&  strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $album->getHref());
             ?>
...
function paggingNumber<?php echo $randonNumber; ?>(pageNum){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "widget/index/mod/sesgroupalbum/name/browse-albums",
      'data': {
        format: 'html',
        page: pageNum,    
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        ...
        document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML =  responseHTML;
				sesJqueryObject('#paginator_count_sesgroupalbum').find('#total_item_count_sesgroupalbum').html(sesJqueryObject('#paginator_count_ajax_sesgroupalbum').find('#total_item_count_sesgroupalbum').html());
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/sesgroupalbum/name/browse-albums/index/',
      'data': {
        format: 'html',
