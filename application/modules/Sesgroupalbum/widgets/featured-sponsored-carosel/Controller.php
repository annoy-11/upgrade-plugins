...
class Sesgroupalbum_Widget_FeaturedSponsoredCaroselController extends Engine_Content_Widget_Abstract
{
    ...
    $value['limit'] = $this->_getParam('limit_data',5);
		...
		if($value['type'] == 1 || $value['type'] == 3){
		 ...
		 if($value['type'] == 1){
			...
			$this->setNoRender();
		$itemTable = Engine_Api::_()->getDbTable($tableName, 'sesgroupalbum');
		...
		if($tableName == 'photos'){
			...
			{
				//fecth all
				$photos = $itemTable->fetchAll($select);
				...
				$paginator = Zend_Paginator::factory($tempArray);
			}else
				$paginator = Zend_Paginator::factory($select);
		}else if($tableName != 'photos'){
			if($album_enable_check_privacy){
					//fecth all
				$albums = $itemTable->fetchAll($select);
				//loop over all albums once
