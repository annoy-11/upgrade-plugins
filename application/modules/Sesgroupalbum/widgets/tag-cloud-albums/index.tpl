   ...
   <canvas style="width:100%;height:100%;" id="myCanvas_<?php echo $randonNumber ?>">
    ...
    <ul>
      <?php foreach($this->paginator as $valueTags){ 
          	...
          	continue;
      ?>
      <li><a href="<?php echo $this->url(array('module' =>'sesgroupalbum','controller' => 'index', 'action' => 'browse'),'sesgroupalbum_general',true).'?tag_id='.$valueTags['tag_id'].'&tag_name='.$valueTags['text']  ;?>"><?php echo $valueTags['text'] ?></a></li>
      <?php } ?>
    </ul>
   </canvas>
  </div>
  <a href="<?php echo $this->url(array('action' => 'tags'),'sesgroupalbum_general',true);?>" class="sesbasic_more_link clear"><?php echo $this->translate("See All Tags");?> &raquo;</a>
</div>
