...
class Sesgroupalbum_Widget_FeaturedSponsoredController extends Engine_Content_Widget_Abstract
{
  ...
  {  
		...
		if($value['tableName'] == 'photo'){
		 ...
		 $result = Engine_Api::_()->getDbTable('photos', 'sesgroupalbum')->featuredSponsored($value);
		}else if($value['tableName'] == 'album'){
			$value['tableName'] = 'albums';
			$this->view->typeWidget = 'album';
			$result = Engine_Api::_()->getDbTable('albums', 'sesgroupalbum')->featuredSponsored($value);
		}else
