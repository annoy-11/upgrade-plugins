...
class Sesgroupalbum_Widget_TabbedWidgetController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
	  ...
	  $this->view->albumPhotoOption =  $albumPhotoOption = isset($params['albumPhotoOption']) ? $params['albumPhotoOption'] : $this->_getParam('photo_album', 'photo');
		...
		$paginator = Engine_Api::_()->getDbTable('photos', 'sesgroupalbum')->tabWidgetPhotos($value);
		}else{
			$paginator = Engine_Api::_()->getDbTable('albums', 'sesgroupalbum')->tabWidgetAlbums($value);
		}
    ...
    $paginator->setCurrentPageNumber($page);
		...
		if($is_ajax)
			...
			if( $paginator->getTotalItemCount() <= 0 ) {
				...
				$checkAlbumCount = Engine_Api::_()->getDbTable($albumPhotoOption.'s', 'sesgroupalbum')->$nameFunction();
