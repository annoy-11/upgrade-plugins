...
<div class="tabs_alt tabs_parent" id="sesgroupalbum_tabbed_widget_container_<?php echo $randonNumber; ?>">
<?php } ?>
<!--Advance Tabs-->
<?php if($this->tab_option == 'advance'){ ?>
<div class="sesbasic_tabs_container sesbasic_clearfix sesbasic_bxs">
  <div class="sesbasic_tabs sesbasic_clearfix" id="sesgroupalbum_tabbed_widget_container_<?php echo $randonNumber; ?>">
 <?php } ?>
<!--Filter Tabs-->
<?php if($this->tab_option == 'filter'){ ?>
<div class="sesbasic_filter_tabs_container sesbasic_clearfix sesbasic_bxs">
  <div class="sesbasic_filter_tabs sesbasic_clearfix" id="sesgroupalbum_tabbed_widget_container_<?php echo $randonNumber; ?>">
<?php } ?>
    <ul id="tab-widget-sesgroupalbum-<?php echo $randonNumber; ?>">
      <?php 
...
<script type="application/javascript">
	sesJqueryObject('#sesgroupalbum_tabbed_widget_container_<?php echo $randonNumber; ?>').css('display','none');
</script>
<?php } ?>
  <div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" class="sesbasic_clearfix">
    <ul class="sesgroupalbum_listings sesgroupalbum_tabbed_listings sesgroupalbum_photos_flex_view sesbasic_bxs sesbasic_clearfix" id="tabbed-widget_<?php echo $randonNumber; ?>">
<?php } ?>
          <?php $limit = $this->limit;
           $allowRatingAlbum = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.album.rating',1);
          ...
          if($allowRatingAlbum == 0){
          	...
          	$ratingShowAlbum = true;
          $allowRatingPhoto = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.photo.rating',1);
					$allowShowPreviousRatingPhoto = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.ratephoto.show',1);
         ...
         <?php if($this->view_type != 'masonry'){ ?>
            <li id="thumbs-photo-<?php echo $photo->photo_id ?>" class="ses_album_image_viewer sesgroupalbum_list_grid_thumb sesgroupalbum_list_photo_grid sesgroupalbum_list_grid  sesa-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sesa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">
              <?php $imageURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref($photo,array('status'=>$this->type,'limit'=>$limit)); ?>
            ...
            <span class="sesgroupalbum_list_grid_btns">
              <?php if(isset($this->socialSharing)){ ?>
                ...
                <a href="<?php echo 'http://pinterest.com/pin/create/button/?url='.$urlencode; ?>&media=<?php echo urlencode((strpos($photo->getPhotoUrl('thumb.main'),'http') === FALSE ? (((!empty($_SERVER["HTTPS"]) && strtolower($_SERVER["HTTPS"] == 'on')) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] ) : $photo->getPhotoUrl('thumb.main')) . $photo->getPhotoUrl('thumb.main')); ?>&description=<?php echo $photo->getTitle();?>" onclick="return socialSharingPopUp(this.href,'<?php echo $this->translate('Pinterest'); ?>')" class="sesgroupalbum_icon_btn sesgroupalbum_icon_pintrest_btn"><i class="fa fa-pinterest"></i></a>
              <?php } 
              $canComment =  Engine_Api::_()->getItem('sesgroupalbum_album',$photo->album_id )->authorization()->isAllowed(Engine_Api::_()->user()->getViewer(), 'comment');
         ...
         <?php }else{ ?>
          ...
          if($width >= 500)
          	$imageURL = $photo->getPhotoUrl('thumb.main');?>
         		<li id="thumbs-photo-<?php echo $photo->photo_id ?>" data-w="<?php echo $width ?>" data-h="<?php echo $height; ?>" class="ses_album_image_viewer sesgroupalbum_list_flex_thumb sesgroupalbum_list_photo_grid sesgroupalbum_list_grid sesa-i-inside sesa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?>">
              <?php $imageViewerURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref($photo,array('status'=>$this->type,'limit'=>$limit)); ?>
         ...
         <?php } ?>
          <?php }else if($this->albumPhotoOption == 'photo' && $this->view_type  == 'pinboard'){ ?>
        		  <li class="sesbasic_bxs sesbasic_pinboard_list_item_wrap new_image_pinboard_<?php echo $randonNumber; ?>" style="display:none;">
              	<div class="sesbasic_pinboard_list_item sesbasic_bm">
                	<div class="sesbasic_pinboard_list_item_top">
                     ...
                     <?php $imageURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref($photo,array('status'=>$this->type,'limit'=>$limit)); ?>
                  		<a href="<?php echo Engine_Api::_()->sesgroupalbum()->getHrefPhoto($photo->getIdentity(),$photo->album_id); ?>" onclick="getRequestedAlbumPhotoForImageViewer('<?php echo $photo->getPhotoUrl(); ?>','<?php echo $imageURL	; ?>')" class="sesJqueryObject_thumb_img ses-image-viewer">
                  ...
                  <?php   if(isset($this->socialSharing) || isset($this->likeButton) || isset($this->favouriteButton)){
                    $urlencode = urlencode(((!empty($_SERVER["HTTPS"]) &&  strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $photo->getHref()); ?>
                     <div class="sesbasic_pinboard_list_btns sesgroupalbum_list_grid_btns"> 
                   <?php if(isset($this->socialSharing)){ ?>
                <a href="<?php echo 'http://www.facebook.com/sharer/sharer.php?u=' . $urlencode . '&t=' . $photo->getTitle(); ?>" onclick="return socialSharingPopUp(this.href,'<?php echo $this->translate('Facebook'); ?>')" class="sesgroupalbum_icon_btn sesgroupalbum_icon_facebook_btn"><i class="fa fa-facebook"></i></a>
                <a href="<?php echo 'https://twitter.com/intent/tweet?url=' . $urlencode . '&text=' . $photo->getTitle().'%0a'; ?>" onclick="return socialSharingPopUp(this.href,'<?php echo $this->translate('Twitter')?>')" class="sesgroupalbum_icon_btn sesgroupalbum_icon_twitter_btn"><i class="fa fa-twitter"></i></a>
                <a href="<?php echo 'http://pinterest.com/pin/create/button/?url='.$urlencode; ?>&media=<?php echo urlencode((strpos($photo->getPhotoUrl('thumb.main'),'http') === FALSE ? (((!empty($_SERVER["HTTPS"]) && strtolower($_SERVER["HTTPS"] == 'on')) ? "https://" : "http://") . $_SERVER['HTTP_HOST'] ) : $photo->getPhotoUrl('thumb.main')) . $photo->getPhotoUrl('thumb.main')); ?>&description=<?php echo $photo->getTitle();?>" onclick="return socialSharingPopUp(this.href,'<?php echo $this->translate('Pinterest'); ?>')" class="sesgroupalbum_icon_btn sesgroupalbum_icon_pintrest_btn"><i class="fa fa-pinterest"></i></a>
                      ...
                      <div class="sesbasic_pinboard_list_item_poster sesbasic_text_light sesbasic_clearfix">
                        ...
                        <div class="sesbasic_pinboard_list_item_poster_info">
                          <span class="sesbasic_pinboard_list_item_poster_info_title"><?php echo $this->htmlLink($owner->getHref(),$owner->getTitle() ) ?></span>
                            <div class="sesgroupalbum_list_stats sesbasic_text_light">
        ...
        <?php
            ...
            <li id="thumbs-photo-<?php echo $photo->photo_id ?>" class="sesgroupalbum_list_grid_thumb sesgroupalbum_list_grid sesa-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sesa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">  
              <a class="sesgroupalbum_list_grid_img" href="<?php echo Engine_Api::_()->sesgroupalbum()->getHref($photo->getIdentity(),$photo->album_id); ?>" style="height:<?php echo is_numeric($this->height) ? $this->height.'px' : $this->height ?>;">
              ...
              <div class="ses_image_container" style="display:none;">
                <?php $image = Engine_Api::_()->sesgroupalbum()->getAlbumPhoto($photo->getIdentity(),$photo->photo_id); 
       ...
       ?>
        ...
        $canComment =  $photo->authorization()->isAllowed(Engine_Api::_()->user()->getViewer(), 'comment');
        	if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->likeButton) && $canComment){ ?>
                <!--Album Like Button-->
                <?php $albumLikeStatus = Engine_Api::_()->sesgroupalbum()->getLikeStatus($photo->album_id); ?>
                <a href="javascript:;" data-src='<?php echo $photo->album_id; ?>' class="sesgroupalbum_icon_btn sesgroupalbum_icon_like_btn sesgroupalbum_albumlike <?php echo ($albumLikeStatus) ? 'button_active' : '' ; ?>">
                  <i class="fa fa-thumbs-up"></i>
                  <span><?php echo $photo->like_count; ?></span>
                </a>
              <?php } 
              $canFavourite =  Engine_Api::_()->authorization()->isAllowed('sesgroupalbum_album',Engine_Api::_()->user()->getViewer(), 'favourite_album');
              	if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->favouriteButton) && $canFavourite){
             	 		$albumFavStatus = Engine_Api::_()->getDbtable('favourites', 'sesgroupalbum')->isFavourite(array('resource_type'=>'sesgroupalbum_album','resource_id'=>$photo->album_id)); ?>
              <a href="javascript:;" data-src='<?php echo $photo->album_id; ?>' class="sesgroupalbum_icon_btn sesgroupalbum_icon_fav_btn sesgroupalbum_albumFav <?php echo ($albumFavStatus)>0 ? 'button_active' : '' ; ?>">
                <i class="fa fa-heart"></i>
                <span><?php echo $photo->favourite_count; ?></span>
              </a>
         <?php } ?>
         ...
         <?php if(isset($this->like) || isset($this->comment) || isset($this->view) || isset($this->title) || isset($this->rating) || isset($this->photoCount) || isset($this->favouriteCount) || isset($this->downloadCount)  || isset($this->by)){ ?>
              ...
              <span class="sesgroupalbum_list_grid_stats">
                ...
                <?php }?>
               	<?php if(isset($this->rating) && $ratingShowAlbum) { ?>
                	 <?php
                    $user_rate = Engine_Api::_()->getDbtable('ratings', 'sesgroupalbum')->getCountUserRate('album',$photo->album_id);
                    $textuserRating = $user_rate == 1 ? 'user' : 'users'; 
                    $textRatingText = $photo->rating == 1 ? 'rating' : 'ratings'; ?>
                    <span class="sesgroupalbum_list_grid_rating" title="<?php echo $photo->rating.' '.$this->translate($textRatingText.' by').' '.$user_rate.' '.$this->translate($textuserRating); ?>">
                    <?php if( $photo->rating > 0 ): ?>
              ...
              <?php } ?>
                 <?php if(isset($this->photoCount)) { ?>
               	<span class="sesgroupalbum_list_grid_count" title="<?php echo $this->translate(array('%s photo', '%s photos', $photo->count()), $this->locale()->toNumber($photo->count()))?>" >
                ...
                <?php echo $this->translate("There are currently no ".ucfirst($this->albumPhotoOption)."s.");?>
                 <?php if( $this->canCreate ): ?>
                  <?php echo $this->translate('Be the first to %1$screate%2$s one!', 
                     '<a href="'.$this->url(array('action' => 'create','controller'=>'index'),'sesgroupalbum_general',true).'">', '</a>'); 
                  ?>
                  <?php endif; ?>
              </span>
            </div>    
...
if("<?php echo $this->albumPhotoOption; ?>" == 'photo' && "<?php echo $this->view_type ; ?>" == 'masonry'){
	...
	function paggingNumber<?php echo $randonNumber; ?>(pageNum){
		 ...
		 var valueTab ;
		 sesJqueryObject('#tab-widget-sesgroupalbum-<?php echo $randonNumber; ?> > li').each(function(index){
		 ...
		 if(typeof valueTab == 'undefined')
			...
			(new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + 'widget/index/mod/sesgroupalbum/name/tabbed-widget/openTab/' + valueTab,
				'data': {
					format: 'html',
					page: pageNum,   
...
<?php if($this->loadOptionData != 'pagging'){ ?>
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    if (typeof(requestTab_<?php echo $randonNumber; ?>) != 'undefined') {
		...
		(new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/sesgroupalbum/name/tabbed-widget/openTab/' + openTab_<?php echo $randonNumber; ?>,
      'data': {
        format: 'html',
