...
class Sesgroupalbum_Widget_photoViewPageController extends Engine_Content_Widget_Abstract
{
  public function indexAction()
  {
		...
		if(Engine_Api::_()->core()->hasSubject('sesgroupalbum_photo') && !$is_ajax)
	 	 $photo = Engine_Api::_()->core()->getSubject('sesgroupalbum_photo');
		else if(isset($_POST['photo_id'])){
		 $photo = Engine_Api::_()->getItem('sesgroupalbum_photo',$_POST['photo_id']);
	  ...
	  if($viewer->getIdentity()>0){
			...
			$this->view->canEdit = $canEdit = $group->authorization()->isAllowed($viewer, 'edit');
			$this->view->canComment = $canComment = $album->authorization()->isAllowed($viewer, 'comment');
			$this->view->canDelete = $canDelete = $group->authorization()->isAllowed($viewer, 'delete');
			$this->view->canTag = $canTag = $album->authorization()->isAllowed($viewer, 'tag');
			$this->view->canUntagGlobal = $canUntag = $album->isOwner($viewer);
			$this->view->canCommentMemberLevelPermission = Engine_Api::_()->authorization()->getPermission($viewer, 'sesgroupalbum_album', 'comment');
		}
	...
	if($showRating != 0){
		...
		if(isset($status_like) && $status_like){
			...
			$this->view->paginator_like = $paginator_like = Engine_Api::_()->sesgroupalbum()->likeItemCore($paramData);
		...
		if(isset($status_favourite) && $status_favourite){
			...
			$this->view->album_id = $paramData['id'] = $photo->getIdentity();
			$paramData['type'] = 'album_photo';				
			$this->view->paginator_favourite = $paginator_favourite = Engine_Api::_()->getDbTable('photos', 'sesgroupalbum')->getFavourite(array('resource_id'=>$paramData['id']));
...
// 			$this->view->paginator_tagged = $paginator_tagged = Engine_Api::_()->sesgroupalbum()->tagItemCore($param);		
