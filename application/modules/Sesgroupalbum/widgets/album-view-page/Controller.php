...
class Sesgroupalbum_Widget_AlbumViewPageController extends Engine_Content_Widget_Abstract
{
  ...
  {	
	...
	if(Engine_Api::_()->core()->hasSubject()){
			...
			$album =  Engine_Api::_()->getItem('sesgroupalbum_album', $_POST['album_id']);		
	...
	if(!isset($_POST['is_related'])){
		...
		if($viewer->getIdentity() > 0){
			...
			$this->view->canCreateMemberLevelPermission = Engine_Api::_()->authorization()->getPermission($viewer, 'group', 'create');
			$this->view->canEditMemberLevelPermission   = Engine_Api::_()->authorization()->getPermission($viewer,'group', 'edit');
			$this->view->canDeleteMemberLevelPermission  = Engine_Api::_()->authorization()->getPermission($viewer,'group', 'delete');
		}
    if(!$is_ajax){
			...
			if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock'))  && in_array('sesgroupalbum',$sesprofilelock_enable_module)) {
				...
				if (!Engine_Api::_()->authorization()->getPermission($level, 'album', 'locked') && $album->is_locked) {
		...
		if(!$is_ajax){
			$this->view->canDownload = $canDownload = Engine_Api::_()->authorization()->isAllowed('sesgroupalbum_album',$viewer, 'download');
			...
			/*Get recent Album of user*/
				$this->view->paginatorRecentAlbum = $paginatorRecentAlbum = Engine_Api::_()->getDbTable('albums', 'sesgroupalbum')->profileAlbums(array('userId'=>$album->owner_id,'photo_id'=>false,'widget'=>true,'notInclude'=>$album->album_id));
			...
			/*User like Album*/
				$param['type'] = 'album';
				$this->view->paginatorLike = $paginatorLike = Engine_Api::_()->sesgroupalbum()->likeItemCore($param);
...
// 			/**User tag in Album*/
// 				//$this->view->paginatorTaggedUser = $paginatorTaggedUser = Engine_Api::_()->sesgroupalbum()->tagItemCore(array('id'=>$param['id'],'album'=>true));
...
// 			}
			...
			/**User favourite This Album*/
				$this->view->paginatorFav = $paginatorFav = Engine_Api::_()->getDbTable('albums', 'sesgroupalbum')->getFavourite(array('resource_id'=>$param['id']));
	...
	if(!$is_ajax || isset($_POST['is_related'])){
	 ...
	 // end code related
	  $pageRelated = isset($_POST['pageRelated']) ? $_POST['pageRelated'] : 1 ;
		$this->view->relatedAlbumsPaginator = $relatedAlbumsPaginator = Engine_Api::_()->getDbTable('relatedalbums', 'sesgroupalbum')->getitem(array('album_id' =>$album->album_id));
		...
		if(!$is_ajax){
			// rating code
			$this->view->allowShowRating = $allowShowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.ratealbum.show',1);
			$this->view->allowRating = $allowRating= Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.album.rating',1);
			...
			if($showRating){
				$this->view->canRate = $canRate = Engine_Api::_()->authorization()->isAllowed('sesgroupalbum_album',$viewer, 'rating_album');
				$this->view->allowRateAgain = $allowRateAgain  = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.ratealbum.again',1);
				$this->view->allowRateOwn = $allowRateOwn =  Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.ratealbum.own',1);
				...
				$this->view->rating_type = $rating_type  = 'sesgroupalbum_album';
				$this->view->rating_count = $rating_count = Engine_Api::_()->getDbTable('ratings', 'sesgroupalbum')->ratingCount($album->getIdentity(),$rating_type);
				$this->view->rated = $rated = Engine_Api::_()->getDbTable('ratings', 'sesgroupalbum')->checkRated($album->getIdentity(), $viewer->getIdentity(),$rating_type); 
				$rating_sum  = Engine_Api::_()->getDbTable('ratings', 'sesgroupalbum')->getSumRating($album->getIdentity(),'sesgroupalbum_album');
				if($rating_count != 0){
