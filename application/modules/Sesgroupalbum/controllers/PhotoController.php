...
class Sesgroupalbum_PhotoController extends Core_Controller_Action_Standard {
  ...
  public function init() {
   ...
   if (strpos($_SERVER['REQUEST_URI'], 'ses-compatibility-code') === false && strpos($_SERVER['REQUEST_URI'], 'last-element-data') === false && strpos($_SERVER['REQUEST_URI'], 'allphoto-ses-compatibility-code') === false && strpos($_SERVER['REQUEST_URI'], 'all-photos') === false && strpos($_SERVER['REQUEST_URI'], 'image-viewer-detail') === false  && strpos($_SERVER['REQUEST_URI'], 'o/like') === false) {
      if (!$this->_helper->requireAuth()->setAuthParams('sesgroupalbum_album', null, 'view')->isValid())
		...
		if (0 !== ($photo_id = (int) $this->_getParam('photo_id')) && null !== ($photo = Engine_Api::_()->getItem('sesgroupalbum_photo', $photo_id))) {
    ...
    $value['text'] = $this->_getParam('text','');
		$photos = Engine_Api::_()->getDbtable('photos', 'sesgroupalbum')->getPhoto($value);
...
//function to open third party module photo
  public function sesCompatibilityCodeAction(){
    ...
    $this->view->child_id = $child_id = $getParams[$getModuleData[0]['content_id_photo']];
		...
		/*if(!$parent_item->authorization()->isAllowed($viewer, 'view')){
			$imagePrivateURL = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.private.photo', 1);
		 if(!is_file($imagePrivateURL))
      $imagePrivateURL = 'application/modules/Sesgroupalbum/externals/images/private-photo.jpg';
		 $this->view->imagePrivateURL = $imagePrivateURL;
    }*/
    // get next photo URL
    $this->view->nextPhoto = Engine_Api::_()->sesgroupalbum()->SesNextPreviousPhoto($child_item, '>', $getModuleData[0]['content_type_photo'],$getModuleData[0]['content_id_photo'],$getModuleData[0]['content_id']);
    // get previous photo URL
    $this->view->previousPhoto = Engine_Api::_()->sesgroupalbum()->SesNextPreviousPhoto($child_item, '<', $getModuleData[0]['content_type_photo'],$getModuleData[0]['content_id_photo'],$getModuleData[0]['content_id']);
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
		...
		//FETCH photos
		$paginator = $this->view->allPhotos =  Engine_Api::_()->sesgroupalbum()->SesNextPreviousPhoto($child_item, '>', $getModuleData[0]['content_type_photo'],$getModuleData[0]['content_id_photo'],$getModuleData[0]['content_id'],true);
        ...
        break;
    		...
    		// send extra params to view for extra URL parameters
   			$this->view->params = $params;
				...
				//FETCH photos
				$paginator = $this->view->allPhotos = Engine_Api::_()->getDbTable('photos', 'sesgroupalbum')->getPhotoCustom($photo,$params,'',true);
  			$paginator->setItemCountPerPage(30);
				$this->view->limit = ($page-1)*30;
				$this->view->page = $page ;
				$this->view->is_ajax = $is_ajax ;
  ...
  public function imageViewerDetailAction() {
    ...
    $this->view->album = $album = $photo->getAlbum();
		...
		$cookieData = isset($_COOKIE['sesgroupalbum_lightbox_value']) ? $_COOKIE['sesgroupalbum_lightbox_value'] : array();
		if(@$cookieData && !in_array($album->album_id,explode(',',$cookieData))){
      ...
      //check dependent module sesprofile install or not
			if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock'))  && in_array('sesgroupalbum',$sesprofilelock_enable_module)) {
				...
				if ($viewer->getIdentity() == 0)
					...
					$level = $viewer;
				if (!Engine_Api::_()->authorization()->getPermission($level, 'sesgroupalbum_album', 'locked') && $album->is_locked) {
    ...
    if ($viewer->getIdentity() != 0 && isset($photo->photo_id) && !$viewWidget) {
      ...
      $dbObject->query('INSERT INTO engine4_sesgroupalbum_recentlyviewitems (resource_id, resource_type,owner_id,creation_date ) VALUES ("' . $photo->photo_id . '", "sesgroupalbum_photo","' . $viewer->getIdentity() . '",NOW())	ON DUPLICATE KEY UPDATE	creation_date = NOW()');
    }
    if ($this->getRequest()->getParam('user') != 0) {
      $params = array_merge($params, array('user' => $this->getRequest()->getParam('user')));
    }
    ...
    // get next photo URL
    $this->view->nextPhoto = Engine_Api::_()->sesgroupalbum()->nextPhoto($photo, array_merge($params, $extraParamsNext));
		// get previous photo URL
		$this->view->previousPhoto = Engine_Api::_()->sesgroupalbum()->previousPhoto($photo, array_merge($params, $extraParamsPrevious));
    ...
    $checkAlbum = Engine_Api::_()->getItem('sesgroupalbum_album', $this->_getParam('album_id'));
    ...
    $this->view->canUntagGlobal = $canUntag = $album->isOwner($viewer);
		$this->view->canDownload = Engine_Api::_()->authorization()->isAllowed('sesgroupalbum_album',$viewer, 'download');
		$this->view->canFavourite = Engine_Api::_()->authorization()->isAllowed('sesgroupalbum_album',$viewer, 'favourite_photo');
    // Get tags
    $tags = array();
    ...
    $this->view->tags = $tags;
    // rating code
    $this->view->allowShowRating = $allowShowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.ratephoto.show', 1);
    $this->view->allowRating = $allowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.photo.rating', 1);
    ...
    if ($showRating != 0) {
      $this->view->canRate = $canRate = Engine_Api::_()->authorization()->isAllowed('sesgroupalbum_album', $viewer, 'rating_photo');
      $this->view->allowRateAgain = $allowRateAgain = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.ratephoto.again', 1);
      $this->view->allowRateOwn = $allowRateOwn = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.ratephoto.own', 1);
      ...
      $this->view->rating_count = $rating_count = Engine_Api::_()->getDbTable('ratings', 'sesgroupalbum')->ratingCount($photo->getIdentity(), $rating_type);
      $this->view->rated = $rated = Engine_Api::_()->getDbTable('ratings', 'sesgroupalbum')->checkRated($photo->getIdentity(), $viewer->getIdentity(), $rating_type);
      $rating_sum = Engine_Api::_()->getDbTable('ratings', 'sesgroupalbum')->getSumRating($photo->getIdentity(), 'album_photo');
      if ($rating_count != 0) {
   ...
   $viewer = Engine_Api::_()->user()->getViewer();
	 if($viewer->getIdentity() == 0)
			...
			$level = $viewer;
		$type = Engine_Api::_()->authorization()->getPermission($level,'sesgroupalbum_album', 'imageviewer');
  ...
  public function addAction() {
    ...
    if (intval($photo_id) == 0) {
      echo json_encode(array('status' => 'false', 'error' => 'Invalid argument supplied.'));die;
			...
			$viewer = Engine_Api::_()->user()->getViewer();
    $Fav =  Engine_Api::_()->getDbTable('favourites', 'sesgroupalbum')->getItemfav('sesgroupalbum_photo',$photo_id);
		$photo = Engine_Api::_()->getDbtable('photos', 'sesgroupalbum');
    if (count($Fav) > 0) {
      //delete		
      ...
      $photos = Engine_Api::_()->getItem('sesgroupalbum_photo', $photo_id);
			 Engine_Api::_()->getDbtable('notifications', 'activity')->delete(array('type =?' => "sesgroupalbum_favouritephoto", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $photos->getType(), "object_id = ?" => $photos->getIdentity()));
			 Engine_Api::_()->getDbtable('actions', 'activity')->delete(array('type =?' => "sesgroupalbum_favouritephoto", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $photos->getType(), "object_id = ?" => $photos->getIdentity()));
      ...
      //update
      $db = Engine_Api::_()->getDbTable('favourites', 'sesgroupalbum')->getAdapter();
      $db->beginTransaction();
      try {
        $fav = Engine_Api::_()->getDbTable('favourites', 'sesgroupalbum')->createRow();
        $fav->user_id = Engine_Api::_()->user()->getViewer()->getIdentity();
        $fav->resource_type = 'sesgroupalbum_photo';
      ...
      $favourite_count = Engine_Api::_()->getItem('sesgroupalbum_photo', $photo_id);
			$subject = $favourite_count;
			 $owner = $subject->getOwner();
			 if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
			 $activityTable = Engine_Api::_()->getDbtable('actions', 'activity');
			 Engine_Api::_()->getDbtable('notifications', 'activity')->delete(array('type =?' => "sesgroupalbum_favouritephoto", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $subject->getType(), "object_id = ?" => $subject->getIdentity()));
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $subject, 'sesgroupalbum_favouritephoto');
        $result = $activityTable->fetchRow(array('type =?' => "sesgroupalbum_favouritephoto", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $subject->getType(), "object_id = ?" => $subject->getIdentity()));
        if (!$result) {
          $action = $activityTable->addActivity($viewer, $subject, 'sesgroupalbum_favouritephoto');
          if ($action)
  ...
  function likeAction() {
    ...
    $photo = Engine_Api::_()->getDbtable('photos', 'sesgroupalbum');
    $select = $tableLike->select()->from($tableMainLike)->where('resource_type =?', 'group_photo')->where('poster_id =?', Engine_Api::_()->user()->getViewer()->getIdentity())->where('poster_type =?', 'user')->where('resource_id =?', $photo_id);
    ...
    if (count($Like) > 0) {
      ...
      $like_count = Engine_Api::_()->getItem('sesgroupalbum_photo', $photo_id);
      ...
      try {
        ...
        $like->resource_type = 'sesgroupalbum_photo';
  ...
  public function viewAction() {
		if(!Engine_Api::_()->core()->hasSubject()){
		 ...
		 $url = Engine_Api::_()->sesgroupalbum()->getHref($album_id);
		 header('location:'.$url);
		 die;
		}
    if (!$this->_helper->requireSubject('sesgroupalbum_photo')->isValid())
      return;
    ...
    if ($viewer->getIdentity() != 0 && isset($photo->photo_id)) {
      $dbObject = Engine_Db_Table::getDefaultAdapter();
      $dbObject->query('INSERT INTO engine4_sesgroupalbum_recentlyviewitems (resource_id, resource_type,owner_id,creation_date ) VALUES ("' . $photo->photo_id . '", "album_photo","' . $viewer->getIdentity() . '",NOW())	ON DUPLICATE KEY UPDATE	creation_date = NOW()');
    }
...
//       return;
    $viewer = Engine_Api::_()->user()->getViewer();
    $photo = Engine_Api::_()->core()->getSubject('sesgroupalbum_photo');
    $album = $photo->getParent();
    $this->view->form = $form = new Sesgroupalbum_Form_Photo_Delete();
    ...
    try {
      ...
      $actions = $attachDB->fetchAll($attachDB->select()->where('type = ?', 'sesgroupalbum_photo')->where('id = ?', $photo->photo_id));
      $actionsDB = Engine_Api::_()->getDbtable('actions', 'activity');
      foreach ($actions as $action) {
        $action_id = $action->action_id;
        $attachDB->delete(array('type = ?' => 'sesgroupalbum_photo', 'id = ?' => $photo->photo_id));
    ...
    return $this->_forward('success', 'utility', 'core', array(
                'messages' => array(Zend_Registry::get('Zend_Translate')->_('Photo deleted successfully.')),
                'layout' => 'default-simple',
                'parentRedirect' => Engine_Api::_()->sesgroupalbum()->getHref($ablum_id),
    ));
  }
	//location photo 
...
//       return;
    ...
    $this->view->form = $form = new Sesgroupalbum_Form_Photo_Location();
    ...
    if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
      ...
      $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $this->_getParam('photo_id') . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","sesgroupalbum_photo")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
    }
    $db = $photo->getTable()->getAdapter();
    $db->beginTransaction();
    try {
...
//       return;
    $viewer = Engine_Api::_()->user()->getViewer();
    $photo = Engine_Api::_()->core()->getSubject('sesgroupalbum_photo');
    $this->view->form = $form = new Sesgroupalbum_Form_Photo_Edit();
    $form->populate($photo->toArray());
    if (!$this->getRequest()->isPost()) {
      return;
    }
...
//     }
    ...
    if ($status && !$error) {
      ...
      if ($_POST['lat'] != '' && $_POST['lng'] != '') {
        $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
        $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $_POST['photo_id'] . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","sesgroupalbum_photo")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }
      $db = $photo->getTable()->getAdapter();
      $db->beginTransaction();
      try {
