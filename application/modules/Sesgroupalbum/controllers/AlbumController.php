...
class Sesgroupalbum_AlbumController extends Core_Controller_Action_Standard {
  ...
  public function init() {
    ...
    if (0 !== ($photo_id = (int) $this->_getParam('photo_id')) &&
            null !== ($photo = Engine_Api::_()->getItem('sesgroupalbum_photo', $photo_id))) {
      Engine_Api::_()->core()->setSubject($photo);
    } else if (0 !== ($album_id = (int) $this->_getParam('album_id')) &&
            null !== ($album = Engine_Api::_()->getItem('sesgroupalbum_album', $album_id))) {
  ...
  public function createAction() {
    ...
    //if (!$this->_helper->requireAuth()->setAuthParams('album', null, 'create')->isValid())
     // return;
    $this->view->group_id = $group_id = $this->_getParam('group_id',false); 
    ...
    //$this->view->current_count =Engine_Api::_()->getDbtable('albums', 'sesgroupalbum')->getUserAlbumCount($values);
    //$this->view->quota = $quota = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'album', 'max_albums');
    // Get form
    $this->view->form = $form = new Sesgroupalbum_Form_Album();
    // Render
   $this->_helper->content->setEnabled();		
    ...
    $db = Engine_Api::_()->getItemTable('sesgroupalbum_album')->getAdapter();
    $db->beginTransaction();
    try {
      ...
      if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
        ...
        $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $album->album_id . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","sesgroupalbum_album")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
		...
		$url = Engine_Api::_()->sesgroupalbum()->getHref($album->getIdentity(),$album->album_id); 
    header('location:'.$url);
  }
  ...
  public function uploadPhotoAction() {
   ...
   // $group_id = $this->_getParam('group_id', null);
    ...
    try {
      $viewer = Engine_Api::_()->user()->getViewer();
      $photoTable = Engine_Api::_()->getDbtable('photos', 'sesgroupalbum');
      ...
      $db->commit();
    }catch (Sesgroupalbum_Model_Exception $e) {
      $db->rollBack();
      $this->view->status = false;
  ...
  function relatedAlbumAction() {
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('albums', 'sesgroupalbum')->profileAlbums(array('userId' => $album->owner_id, 'join' => true, 'album_id' => $album->album_id, 'widget' => true, 'notInclude' => $album->album_id));
  ...
  function saveRelatedAlbumAction() {
    ...
    if ($data == 'delete')
      $db->query("Delete FROM `engine4_sesgroupalbum_relatedalbums` WHERE resource_id = " . $album->album_id . " && album_id = " . $albumId);
    //insert related album
    else
      $db->query("INSERT INTO `engine4_sesgroupalbum_relatedalbums` (`resource_id`,`album_id`) VALUES ('" . $album->album_id . "','" . $albumId . "')");
    echo "TRUE";
    die;
  }
  ...
  function favAction() {
    ...
    $Fav = Engine_Api::_()->getDbTable('favourites', 'sesgroupalbum')->getItemfav('sesgroupalbum_album', $album_id);
    if (count($Fav) > 0) {
      //delete		
      ...
      $albumObj = Engine_Api::_()->getItem('sesgroupalbum_album', $album_id);
      // Remove activity attachments for this album
      Engine_Api::_()->getDbtable('notifications', 'activity')->delete(array('type =?' => "sesgroupalbum_favouritealbum", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $albumObj->getType(), "object_id = ?" => $albumObj->getIdentity()));
      Engine_Api::_()->getDbtable('actions', 'activity')->delete(array('type =?' => "sesgroupalbum_favouritealbum", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $albumObj->getType(), "object_id = ?" => $albumObj->getIdentity()));
      ...
      try {
        ...
        $fav->resource_type = 'sesgroupalbum_album';
      ...
      $albumObj = $subject = Engine_Api::_()->getItem('sesgroupalbum_album', $album_id);
      $owner = $subject->getOwner();
      if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
        $activityTable = Engine_Api::_()->getDbtable('actions', 'activity');
        Engine_Api::_()->getDbtable('notifications', 'activity')->delete(array('type =?' => "sesgroupalbum_favouritealbum", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $subject->getType(), "object_id = ?" => $subject->getIdentity()));
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $subject, 'sesgroupalbum_favouritealbum');
        $result = $activityTable->fetchRow(array('type =?' => "sesgroupalbum_favouritealbum", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $subject->getType(), "object_id = ?" => $subject->getIdentity()));
        if (!$result) {
          $action = $activityTable->addActivity($viewer, $subject, 'sesgroupalbum_favouritealbum');
          if ($action)
  ...
  public function locationAction() {
    ...
    if (!$this->_helper->requireSubject('sesgroupalbum_album')->isValid())
      return;
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $album = Engine_Api::_()->core()->getSubject('sesgroupalbum_album');
    $this->view->photo = $album;
    $this->view->form = $form = new Sesgroupalbum_Form_Photo_Location();
    $form->populate($album->toArray());
    $location = Engine_Api::_()->getDbTable('locations', 'sesbasic')->getLocationData('sesgroupalbum_album', $album->album_id);
    if ($location) {
  ...
  function likeAction() {
    ...
    $album = Engine_Api::_()->core()->getSubject();
    $select = $tableLike->select()->from($tableMainLike)->where('resource_type =?', 'sesgroupalbum_album')->where('poster_id =?', Engine_Api::_()->user()->getViewer()->getIdentity())->where('poster_type =?', 'user')->where('resource_id =?', $album_id);
    ...
    if (count($Like) > 0) {
      ...
      try {
        ...
        $like->resource_type = 'sesgroupalbum_album';
  ...
  public function viewAction() {
    ...
    if ($viewer->getIdentity() != 0 && isset($album->album_id)) {
      ...
      $dbObject->query('INSERT INTO engine4_sesgroupalbum_recentlyviewitems (resource_id, resource_type,owner_id,creation_date ) VALUES ("' . $album->album_id . '", "album","' . $viewer->getIdentity() . '",NOW())	ON DUPLICATE KEY UPDATE	creation_date = NOW()');
    }
    // Render
    $this->_helper->content->setEnabled();
  }
  ...
  public function getAlbumAction() {
    ...
    $albums = Engine_Api::_()->getDbTable('albums', 'sesgroupalbum')->getAlbumsAction($value);
  ...
  public function editAction() {
    ...
    foreach ($navigation->getPages() as $page) {
      if ($page->route != 'sesgroupalbum_general' || $page->action != 'manage')
        continue;
      $page->active = true;
    }
    // Make form
    $this->view->form = $form = new Sesgroupalbum_Form_Album_Edit();
		 if ($form->getElement('is_locked'))
      $form->getElement('is_locked')->setValue($album->is_locked);
   	 if ($form->getElement('password'))
      $form->getElement('password')->setValue($album->password);
...
// 			}
      ...
      if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
        $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
        $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $this->_getParam('album_id') . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","sesgroupalbum_album")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }
  ...
  public function deleteAction() {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $album = Engine_Api::_()->getItem('sesgroupalbum_album', $this->getRequest()->getParam('album_id'));
    $group = Engine_Api::_()->getItem('group', $album->group_id);
    if (!$this->_helper->requireAuth()->setAuthParams($group, null, 'delete')->isValid())
      return;
    // In smoothbox
    $this->_helper->layout->setLayout('default-simple');
    $this->view->form = $form = new Sesgroupalbum_Form_Album_Delete();
    if (!$album) {
      $this->view->status = false;
    ...
    return $this->_forward('success', 'utility', 'core', array(
                'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('id' => $group->getIdentity()), 'group_profile', true),
  ...
  public function editphotosAction() {
    ...
    // Get albums
    $myAlbums = Engine_Api::_()->getDbtable('albums', 'sesgroupalbum')->editPhotos();
    $albumOptions = array('' => '');
    ...
    // Make form
    $this->view->form = $form = new Sesgroupalbum_Form_Album_Photos();
    foreach ($paginator as $photo) {
      $subform = new Sesgroupalbum_Form_Album_EditPhoto(array('elementsBelongTo' => $photo->getGuid()));
    ...
    try {
      ...
      foreach ($paginator as $photo) {
        ...
        if (isset($values['delete']) && $values['delete'] == '1') {
          ...
          if (($nextPhoto instanceof Sesgroupalbum_Model_Photo) &&
    ...
    $url = Engine_Api::_()->sesgroupalbum()->getHref($album->getIdentity(),$album->album_id); 
    header('location:'.$url);
   // return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'album_id' => $album->album_id), 'sesgroupalbum_specific', true);
  }
  ...
  public function orderAction() {
    if (!$this->_helper->requireUser()->isValid())
      return;
      $album = Engine_Api::_()->getItem('sesgroupalbum_album', $this->_getParam('album_id', null));
...
//       return;
    ...
    $currentOrder = Engine_Api::_()->getDbtable('photos', 'sesgroupalbum')->order($album);
