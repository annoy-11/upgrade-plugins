...
<div class="pswp__top-bar" style="display:none" id="imageViewerId"> 
  ...
  <div class="pswp__top-bar-action">
    ...
    <div class="pswp__top-bar-share">
      ...
      <a href="javascript:;" class='smoothbox' onclick="openGroupURLinkSmoothBox('<?php echo $this->url(array("action" => "share", "type" => $this->child_item->getType(), "photo_id" => $this->child_item->getIdentity(),"format" => "smoothbox"), 'sesgroupalbum_general', true); ?>')"><?php echo $this->translate('Share'); ?></a>
      <?php } ?>
    </div>
    <div class="pswp__top-bar-more"> 
    	<a href="javascript:;" class="optionGroupOpenImageViewer" id="overlay-model-class" class=""><?php echo $this->translate("Options") ; ?> 
      ...
      <div class="pswp__top-bar-more-tooltip" style="display:none">
        ...
        <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.add.delete',1) == 1){ ?>
        	<a href="javascript:;" class='smoothbox' onclick="openGroupURLinkSmoothBox('<?php echo $this->url(array('controller' => 'photo', 'action' => 'delete-ses', 'photo_id' => $this->child_item->getIdentity(),'item_type'=>$this->child_item->getType(),'module'=>'sesgroupalbum'),'default',true); ?>')"><?php echo $this->translate('Delete'); ?></a>
        <?php } ?>
        <?php } ?>
        <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.add.report',1) == 1 && Engine_Api::_()->user()->getViewer()->getIdentity() != 0){ ?>
        	<a href="javascript:;" class='smoothbox' onclick="openGroupURLinkSmoothBox('<?php echo ($this->url(array('module' => 'core', 'controller' => 'report', 'action' => 'create', 'subject' => $this->child_item->getGuid(), 'format' => 'smoothbox'), 'default', true)); ?>')"><?php echo $this->translate('Report'); ?></a>
        <?php } ?>
        <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0 && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.add.profilepic',1) == 1){ ?>
        	<a href="javascript:;" class='smoothbox' onclick="openGroupURLinkSmoothBox('<?php echo $this->url(array('route' => 'user_extended', 'controller' => 'edit', 'action' => 'external-photo', 'photo' => $this->child_item->getGuid(), 'format' => 'smoothbox'),'user_extended',true); ?>')"><?php echo $this->translate('Make Profile Photo'); ?></a>
        <?php } ?>
        <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.add.download',1) == 1 && isset($this->canDownload) && $this->canDownload == 1){ ?>
        	<a class="ses-album-photo-download" href="<?php echo $this->url(array('module' =>'sesgroupalbum','controller' => 'photo', 'action' => 'download')).'?filePath='.urlencode($this->child_item->getPhotoUrl()) . '&file_id=' . $this->child_item->getIdentity()  ;?>"><?php echo $this->translate('Download'); ?></a>
        <?php } ?>
    ...
    <div class="pswp__top-bar-msg pswp__top-bar-btns">
      <?php $LikeStatus = Engine_Api::_()->sesgroupalbum()->getLikeStatusPhoto($this->child_item->getIdentity(),$this->child_item->getType()); ?>
      <a href="javascript:void(0);" id="sesLightboxLikeUnlikeButton" data-src="albumLike" class="sesbasic_icon_btn nocount sesbasic_icon_like_btn sesgroupalbum_othermodule_like_button<?php echo $LikeStatus ? ' button_active' : '' ;  ?>"><i class="fa fa-thumbs-up"></i><span id="like_unlike_count"></span></a>
    </div>
    <?php } ?>
  </div>
 ...
 <div id="sesgroupalbum_photo_id_data_src" data-src="<?php echo $this->child_item->photo_id; ?>" style="display:none;"></div>
