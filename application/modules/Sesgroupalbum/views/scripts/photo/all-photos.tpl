...
<?php } ?>
      ...
      ?>
       <?php foreach($this->allPhotos as $valuePhoto){
       		  if (!$valuePhoto instanceof Sesgroupalbum_Model_Photo && !isset($this->sesplugins)){
            	$valuePhoto = Engine_Api::_()->getItem('sesgroupalbum_photo', $valuePhoto->photo_id);
       ...
       ?>
          ...
          <?php $imageURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref($valuePhoto,array_merge($this->params,array('limit' => $limit))); ?>
          	<a id="photo-lightbox-id-<?php echo $valuePhoto->photo_id; ?>" onclick="getRequestedAlbumPhotoForImageViewer('<?php echo $valuePhoto->getPhotoUrl(); ?>','<?php echo $imageURL	; ?>')" href="<?php echo Engine_Api::_()->sesgroupalbum()->getHrefPhoto($valuePhoto->getIdentity(),$valuePhoto->album_id); ?>" class="ses-image-viewer ses_ml_photos_panel_photo_thumb">
          <?php }else{ ?>
