...
<div class="ses_media_lightbox_left">
  <div class="ses_media_lightbox_item_wrapper">
    <div class="ses_media_lightbox_item">
      <div id="mainImageContainer">
        ...
        <?php 
         ...
         $cssDisplay = 'block';
         if($this->locked){
         			$imageUrl = 'application/modules/Sesgroupalbum/externals/images/locked-album.jpg';
   ...
   <?php if(isset($this->imagePrivateURL)){
          ...
          ?>
  <span id="image-src-sesgroupalbum-lightbox-hidden"><?php echo $imageUrl; ?></span>
  <span id="image-main-sesgroupalbum-lightbox-hidden"><?php echo $this->photo->getPhotoUrl('','','string'); ?></span>
  <div class="ses_media_lightbox_nav_btns">
       ...
       if(!$taggedUserMap){
        if(!empty($this->previousPhoto))
         ...
         if(!empty($this->previousPhoto[0]['photo_id']))
        	$previousURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref(Engine_Api::_()->getItem('photo',$this->previousPhoto[0]['photo_id']),array_merge($this->params,$this->extraParamsPrevious));
          if(!empty($previousURL))
        ...
        if(!empty($this->previousPhoto))
         ...
         if(!empty($this->previousPhoto->photo_id))
        	$previousURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref(Engine_Api::_()->getItem('photo',$this->previousPhoto->photo_id),array_merge($this->params,$this->extraParamsPrevious));
          if(!empty($previousURL))
    ...
    <?php }     		
          ...
          if(!empty($this->nextPhoto[0]['photo_id']))
  	        $nextURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref(Engine_Api::_()->getItem('photo',$this->nextPhoto[0]['photo_id']),array_merge($this->params,$this->extraParamsNext));
            if(!empty($nextURL))
          ...
          if(!empty($this->nextPhoto->photo_id))
  	        $nextURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref(Engine_Api::_()->getItem('photo',$this->nextPhoto->photo_id),array_merge($this->params,$this->extraParamsNext));
            if(!empty($nextURL))
  ...
  <div class="ses_media_lightbox_options">
    ...
    <div class="ses_media_lightbox_options_name">
      <?php echo $this->translate('In %1$s', $this->htmlLink( Engine_Api::_()->sesgroupalbum()->getHref($this->album->getIdentity()), $this->string()->truncate($this->album->getTitle(),Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.title.truncate',35)))); ?>
    </div>
    ...
    <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0){ ?>
      <div class="ses_media_lightbox_options_btns">
        <a class="sesbasic_icon_btn sesbasic_icon_msg_btn smoothbox" href="<?php echo $this->url(array('module'=> 'sesgroupalbum', 'controller' => 'index', 'action' => 'message','photo_id' => $this->photo->getIdentity(), 'format' => 'smoothbox'),'sesgroupalbum_extended',true); ?>" onclick="openGroupURLinkSmoothBox(this.href);return false;"><i class="fa fa-envelope"></i></a>     
    ...
    <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0 && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupalbum.add.share',1) == 1){ ?>
      <div class="ses_media_lightbox_options_btn ses_media_lightbox_share_btn">
        <a href="javascript:;" class='smoothbox' onclick="openGroupURLinkSmoothBox('<?php echo $this->url(array("action" => "share", "type" => "album_photo", "photo_id" => $this->photo->getIdentity(),"format" => "smoothbox"), 'sesgroupalbum_general', true); ?>')"><?php echo $this->translate('Share'); ?></a>
      </div>
    <?php } ?>
        ...
        <?php } ?>
         ...
         <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0 && (Engine_Api::_()->user()->getViewer()->level_id == 1 || Engine_Api::_()->user()->getViewer()->level_id == 2)): ?>
            ...
            <?php if(strtotime($this->photo->endtime) < strtotime(date("Y-m-d")) && $this->photo->offtheday == 1){$itemofftheday=0;}else{$itemofftheday = $this->photo->offtheday;}
                echo $this->htmlLink(Array("module"=> "sesgroupalbum", "controller" => "index", "action" => "offtheday", "route" => "sesgroupalbum_extended","id" => $this->photo->photo_id, "type" => "album_photo", "param" => (($itemofftheday == 1) ? 0 : 1)),  $this->translate((($itemofftheday == 1) ? "Edit of the Day" : "Make of the Day")), array("class" => "smoothboxGroupOpen")); ?>
    	<?php endif; ?>
      </div>
...
function sespromptPasswordCheck(){
	...
	if(typeof password != 'object' && password.toLowerCase() == '<?php echo strtolower($this->password); ?>'){
			...
			sesJqueryObject('#gallery-img').attr('src',sesJqueryObject('#image-main-sesgroupalbum-lightbox-hidden').html());
