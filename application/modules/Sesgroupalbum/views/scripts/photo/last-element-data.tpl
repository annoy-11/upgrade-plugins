...
<div class="ses_ml_more_popup_container sesbasic_clearfix">
      ...
      <?php  $albumItems = Engine_Api::_()->getDbTable('albums', 'sesgroupalbum')->getAlbums(array('limit_data'=>6,'order'=>'view_count DESC')); 
        	foreach($albumItems as $albumItem){ ?>
          <?php $imageURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref($albumItem); ?>
      	<div class="ses_ml_more_popup_a_list sesbasic_clearfix">
        	<a class="ses-image-viewer" href="<?php echo Engine_Api::_()->sesgroupalbum()->getHref($albumItem->getIdentity(),$albumItem->album_id); ?>" onclick="getRequestedAlbumPhotoForImageViewer('<?php echo $albumItem->getPhotoUrl(); ?>','<?php echo $imageURL	; ?>','change')" href="<?php echo Engine_Api::_()->sesgroupalbum()->getHrefPhoto($albumItem->photo_id,$albumItem->album_id); ?>">
    ...
    <div class="ses_ml_more_popup_rb">
      ...
      <div class="ses_ml_more_popup_bc sesbasic_clearfix">
      	<?php  $photoItems = Engine_Api::_()->getDbTable('photos', 'sesgroupalbum')->getPhotoSelect(array('limit_data'=>9,'order'=>'view_count DESC')); 
        			 foreach($photoItems as $photoItem){ ?>
          <?php $imageURL = Engine_Api::_()->sesgroupalbum()->getImageViewerHref($photoItem); ?>
						<a onclick="getRequestedAlbumPhotoForImageViewer('<?php echo $photoItem->getPhotoUrl(); ?>','<?php echo $imageURL	; ?>','change')" href="<?php echo Engine_Api::_()->sesgroupalbum()->getHrefPhoto($photoItem->getIdentity(),$photoItem->album_id); ?>" class="ses_ml_more_popup_photo_list ses-image-viewer"><span style="background-image:url(<?php echo $photoItem->getPhotoUrl('thumb.normalmain'); ?>);"></span></a>
        <?php } ?>
      </div>
    </div>
  </div>