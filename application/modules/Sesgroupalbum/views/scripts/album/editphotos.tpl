...
<h3>
  <?php echo $this->htmlLink(Engine_Api::_()->sesgroupalbum()->getHref($this->album->getIdentity()), $this->album->getTitle()) ?>
...
<?php endif; ?>
<div class="sesgroupalbum_manage_photos_wrapper sesbasic_clearfix sesbasic_bxs" id="scrollHeightDivSes_<?php echo $randonNumber; ?>">
<form action="<?php echo $this->escape($this->form->getAction()) ?>" name="editPhotos" method="<?php echo $this->escape($this->form->getMethod()) ?>">
  ...
  <ul class='sesgroupalbum_manage_photos' id="tabbed-widget_<?php echo $randonNumber; ?>">
<?php } ?>
    <?php foreach( $this->paginator as $photo ): ?>
      <li class="sesgroupalbum_manage_photos_list" id="thumbs-photo-<?php echo $photo->photo_id ?>">
        <div class="sesbasic_clearfix sesbm">
          <div class="sesgroupalbum_manage_photos_list_photo">
            <?php $url = $photo->getPhotoUrl('thumb.normalmain'); ?>
            <span style="background-image:url(<?php echo $url ?>);"></span>
          </div>
          <div class="sesgroupalbum_manage_photos_list_info">
            <?php
              $key = $photo->getGuid();
              echo $this->form->getSubForm($key)->render($this);
            ?>
            <div class="sesgroupalbums_editphotos_cover">
            ...
            <div class="sesgroupalbum_manage_photos_list_location clear">
              <span>
                <i class="fa fa-map-marker sesbasic_text_light"></i>
                <?php echo $this->htmlLink(Array('module'=> 'sesgroupalbum', 'controller' => 'photo', 'action' => 'location', 'route' => 'sesgroupalbum_extended', 'type' => 'album_photo','album_id' =>$photo->album_id, 'photo_id' => $photo->getIdentity()), $this->translate("Edit Location"), array('onclick'=>"return openGroupURLinkSmoothBox(this.href)")); ?></span>
...
<?php } ?>
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "sesgroupalbum/album/editphotos/album_id/<?php echo $this->album_id; ?>",
      'data': {
        format: 'html',
