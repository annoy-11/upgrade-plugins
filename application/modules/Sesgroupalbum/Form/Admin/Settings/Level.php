...
class Sesgroupalbum_Form_Admin_Settings_Level extends Authorization_Form_Admin_Level_Abstract
{
		...
		if(Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock')) && in_array('sesgroupalbum',$sesprofilelock_enable_module)){
			if($this->isModerator()){
				...
				$this->addElement('Radio', 'locked', array(
						'label' => 'Allow Viewing of Locked Albums?',
						'description' => 'Do you want to let members view locked albums without entering password?',
						'multiOptions' => array(
								1 => 'Yes, allow members to view locked albums.',
								0 => 'No, do not allow members to view locked albums without password.',
						),
						'value' => ( $this->isModerator() ? 1 : 1 ),
				));
			}
			// Element: locked video
      $this->addElement('Radio', 'sesgroupalbum_locked', array(
          'label' => 'Allow User to Lock albums?',
          'description' => 'Do you want to let members to locked albums?',
          'multiOptions' => array(
              1 => 'Yes, allow members to locked albums.',
              0 => 'No, do not allow members to locked albums.',
          ),
          'value' => ( $this->isModerator() ? 1 : 1 ),
      ));
		 }
