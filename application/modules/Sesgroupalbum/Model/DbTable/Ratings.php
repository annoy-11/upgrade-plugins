...
class Sesgroupalbum_Model_DbTable_Ratings extends Engine_Db_Table
{
  protected $_rowClass = "Sesgroupalbum_Model_Rating";
	protected $_name = "sesgroupalbum_ratings";
	// rating functions
	public function getRating($resource_id,$resource_type = 'sesgroupalbum_album')
  {
    ...
    $select = $this->select()
      ...
      ->where('resource_type = ?', $params['type'])
		 ...
		 $albumTableName = Engine_Api::_()->getItemTable('sesgroupalbum_album')->info('name');
		if($params['type'] == 'sesgroupalbum_photo'){
			$photoTableName = Engine_Api::_()->getItemTable('sesgroupalbum_photo')->info('name');
  ...
  public function getRatings($resource_id,$resource_type = 'sesgroupalbum_album')
  {
  ...
  public function checkRated($resource_id, $user_id,$resource_type = 'sesgroupalbum_album')
  {
    $isRated = $this->select()
  ...
  public function setRating($resource_id, $user_id, $rating,$resource_type = 'sesgroupalbum_album'){
    $rName = $this->info('name');
    $row = $this->select()
    ...
    if ($row == 0) {
      // create rating
      Engine_Api::_()->getDbTable('ratings', 'sesgroupalbum')->insert(array(
        ...
        'rating' => $rating,
			 ...
			 Engine_Api::_()->getDbTable('ratings', 'sesgroupalbum')->update(array(
					'rating' => $rating,
				),array(
				'resource_id = ?' => $resource_id,
				'user_id = ?' => $user_id,
  ...
  public function ratingCount($resource_id = NULL,$resource_type = 'sesgroupalbum_album'){
    $rName = $this->info('name');
    return $this->select()
                    ->from($rName,new Zend_Db_Expr('COUNT(rating_id) as total_rating'))
										->where('resource_type =?',$resource_type)
	...
	public function getCountUserRate($resource_type = 'sesgroupalbum_album',$resource_id){
		$rName = $this->info('name');
    $rating_sum = $this->select()
