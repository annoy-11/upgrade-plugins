/* $Id: sesgroupalbumimagevieweradvance.js  2015-6-16 00:00:000 SocialEngineSolutions $ */
	var dataCommentSes = '';
	// store the default browser URL for change state after closing image viewer
	var defaultHashURL = '';
	var requestPhotoSesgroupalbumURL;
  ...
  function getRequestedAlbumPhotoForImageViewer(imageURL,requestedURL,forceAllPhoto,moduleData,sesModuleData){
	...
	if(openPhotoInLightBoxSesgroupalbum == 0){
...
function getRequestedAlbumPhotoForImageViewerParty(imageURL,requestedURL,data1,data2,data3,sesModuleData){
	...
	if(openPhotoInLightBoxSesgroupalbum == 0 || (openGroupPhotoInLightBoxSesgroupalbum == 0 && requestedURL.indexOf("group_id") > -1 ) || (openEventPhotoInLightBoxSesgroupalbum == 0 && requestedURL.indexOf("event_id") > -1) && typeof sesModuleData == 'undefined'){
...
sesJqueryObject(document).on("click", ".thumbs_photo", function (e) {
	...
	if(openPhotoInLightBoxSesgroupalbum == 0 || (openGroupPhotoInLightBoxSesgroupalbum == 0 && requestedURL.indexOf("group_id") > -1 ) || (openEventPhotoInLightBoxSesgroupalbum == 0 && requestedURL.indexOf("event_id") > -1)){
		window.location.href = requestedURL;
		return;
	}
		e.preventDefault();
...
sesJqueryObject(document).on('click','.optionGroupOpenImageViewer',function(){
	if(!sesJqueryObject('#ses_media_lightbox_container').length)
			return;
...
sesJqueryObject(document).on('click','#pswp__button--groupalbuminfo-show', function(){
	if(!sesJqueryObject('#ses_media_lightbox_container').length)
			return;
		if(sesJqueryObject('#pswp__button--groupalbuminfo-show').hasClass('active')){
      sesJqueryObject("#pswp__button--groupalbuminfo-show").removeClass('active');
      ...
      sesJqueryObject("#pswp__button--groupalbuminfo-show").addClass('active');
...
if (window.SESfullScreenApi.supportsFullScreen) {
	document.addEventListener(SESfullScreenApi.fullScreenEventName, function() {
		...
		if (SESfullScreenApi.isFullScreen()) {
			is_fullscreen = 1;
			sesJqueryObject('#ses_media_lightbox_container').addClass('fullscreen');
			sesJqueryObject('.sesgroupalbum_toogle_screen').css('backgroundPosition','-44px 0');
			if(sesJqueryObject('#pswp__button--groupalbuminfo-show').hasClass('active')){
				sesJqueryObject("#pswp__button--groupalbuminfo-show").removeClass('active');
				sesJqueryObject("#pswp__scroll-wrap").removeClass('pswp_info_panel_open');
				sesJqueryObject("#pswp__scroll-wrap").addClass('pswp_info_panel_close');
			...
			sesJqueryObject('.sesgroupalbum_toogle_screen').css('backgroundPosition','0 0');
			...
			slideshow = false;
			sesJqueryObject('#sesgroupalbum_slideshow_playpause').find('i').removeClass('fa-pause');
			sesJqueryObject('#sesgroupalbum_slideshow_playpause').find('i').addClass('fa-play');
			sesJqueryObject('#sesgroupalbum_slideshow_playpause').find('span').html(en4.core.language.translate('Play'));
			is_fullscreen = 0;
			clearInterval(slideShowInterval);
...
sesJqueryObject(document).on('keyup', function (e) {
		...
		// favourite code
		if (e.keyCode === 70) {
			if(sesJqueryObject('#sesgroupalbum_favourite').length > 0)
				sesJqueryObject('#sesgroupalbum_favourite').trigger('click');
		}
...
function getlastElementData(){
	if(document.URL.search('chanel_id') == -1)
		var URL = en4.core.baseUrl+'sesgroupalbum/photo/last-element-data/';
	else
...
function getAllPhoto(requestURL,sesModuleData){
 if(typeof sesModuleData == 'undefined'){
  requestPhotoSesgroupalbumURL = requestURL.replace('image-viewer-detail','all-photos');
	url = '';
 }else{
	url = requestURL;
 	requestPhotoSesgroupalbumURL = en4.core.baseUrl+'sesgroupalbum/photo/allphoto-ses-compatibility-code/';
 }
	imageViewerGetRequest = new Request.HTML({
      url :requestPhotoSesgroupalbumURL,
      data : {
        format : 'html',
				url:url,
      },
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript)
      {
				...
				if( !sesJqueryObject.trim( sesJqueryObject('#ses_media_lightbox_all_photo_id').html() ).length ) 
					...
					var photo_id = sesJqueryObject('#sesgroupalbum_photo_id_data_src').attr('data-src');
...
function getImageViewerObjectData(imageURL,requestedURL,forceAllPhoto,firstPointObject,moduleData,sesModuleData){
      ...
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript)
      {
					...
					if(sesJqueryObject('#sesgroupalbum_check_privacy_album').hasClass('ses-blocked-album')){
						var password = prompt("Enter the password for album '"+sesJqueryObject('#sesgroupalbum_album_title').html()+"'");
						if(typeof password != 'object' && password.toLowerCase() == trim(sesJqueryObject('#sesgroupalbum_album_password').html())){
							sesJqueryObject('.pswp__top-bar-share').show();
							sesJqueryObject('.pswp__top-bar-more').show();
							sesJqueryObject('.pswp__top-bar-msg').show();
							sesJqueryObject('.pswp__button--groupalbuminfo-show').show();
							sesJqueryObject('.pswp__top-bar-tag').show();
							sesJqueryObject('#ses_pswp_information').css('display','');
							setCookieSesgroupalbum(sesJqueryObject('#sesgroupalbum_album_album_id').attr('data-src'));
							changedImage = true;
							var img = document.createElement('img');
							img.onload = function(){
								openPhotoSwipe(sesJqueryObject('#sesgroupalbum_photo_id_data_org').attr('data-src'),this.width,this.height);
							}
							img.src = sesJqueryObject('#sesgroupalbum_photo_id_data_org').attr('data-src');
							...
							changedImage = false;
							sesJqueryObject('.pswp__top-bar-share').hide();
							sesJqueryObject('.pswp__button--groupalbuminfo-show').hide();
							sesJqueryObject('.pswp__top-bar-more').hide();
							sesJqueryObject('.pswp__top-bar-msg').hide();
					...
					if((!sesJqueryObject('#sesgroupalbum_check_privacy_album').hasClass('ses-blocked-album') && !sesJqueryObject('#sesgroupalbum_check_privacy_album').hasClass('ses-private-image')) || changedImage){
						var img = document.createElement('img');
						img.onload = function(){
							openPhotoSwipe(sesJqueryObject('#sesgroupalbum_photo_id_data_org').attr('data-src'),this.width,this.height);
						}
					...
					sesJqueryObject('#sesgroupalbum_check_privacy_album').remove();
					sesJqueryObject('#sesgroupalbum_album_password').remove();
					sesJqueryObject('#sesgroupalbum_album_title').remove();
					sesJqueryObject('#sesgroupalbum_photo_id_data_org').remove();
...
sesJqueryObject(document).on('click','#sesgroupalbum_slideshow_playpause',function(){
...
sesJqueryObject(document).on('click','#sesgroupalbum_slideshow_stop',function(){
		slideshow = false;
		clearInterval(slideShowInterval);
		toogle();
});
...
sesJqueryObject(document).on('click','#editDetailsLink',function(e){
	...
	if(sesJqueryObject('#locationSes').length >0){
		sesJqueryObject('#locationSes').val(trim(sesJqueryObject('#sesgroup_location_data').html()));
		mapLoad = false;
		if(sesJqueryObject('#map-canvas').length)
			sesJqueryObject('#map-canvas').remove();
		initializeSesGroupAlbumMapList();
		//if(sesJqueryObject('#sesgroup_location_data').html())
				//editGroupSetMarkerOnMap();
...
sesJqueryObject(document).on('click','#saveDetailsChanelSes',function(e){
    ...
    type: "POST",  
    url: en4.core.baseUrl+'sesgroupalbum/photo/edit-detail/photo_id/'+photo_id,  
    data: formData,  
    success: function(response) {  
      var data = JSON.parse(response);
			if(data.status && !data.error){
				...
				sesJqueryObject('#sesgroup_location_data').html(sesJqueryObject('#locationSes').val());
				sesJqueryObject('#editDetailsForm').css('display','none')
				sesJqueryObject('#ses_pswp_info').css('display','block');
...
sesJqueryObject(document).on('click','#changeSesPhotoDetails',function(e){
	...
	sesJqueryObject.ajax({  
    type: "POST",  
    url: en4.core.baseUrl+'sesgroupalbum/photo/change-sesdetail/',  
    data: formData,  
    success: function(response) {  
      var data = JSON.parse(response);
			if(data.status && !data.error){
...
sesJqueryObject(document).on('click','#saveDetailsSes',function(e){
	...
	sesJqueryObject.ajax({  
    type: "POST",  
    url: en4.core.baseUrl+'sesgroupalbum/photo/edit-detail/album_id/'+album_id+'/photo_id/'+photo_id,  
    data: formData,  
    success: function(response) {  
      var data = JSON.parse(response);
			if(data.status && !data.error){
...
sesJqueryObject(document).on('click','#comments .comments_options a',function(event){
	...
	if(typeof htmlOnclick != 'undefined' && htmlOnclick.search('comments') != -1 && sesJqueryObject('.sesgroupalbum_othermodule_like_button').length){
			if(sesJqueryObject('.sesgroupalbum_othermodule_like_button').hasClass('button_active')){
				sesJqueryObject('.sesgroupalbum_othermodule_like_button').removeClass('button_active');
				showGroupTooltip(10,10,'<i class="fa fa-thumbs-up"></i><span>'+en4.core.language.translate("Photo removed from like successfully")+'</span>');
				return;
			}else{
				sesJqueryObject('.sesgroupalbum_othermodule_like_button').addClass('button_active');
				showGroupTooltip(10,10,'<i class="fa fa-thumbs-up"></i><span>'+en4.core.language.translate("Photo like successfully")+'</span>', 'sesbasic_liked_notification');
				return ;
			}	
	}
	...
	if(htmlOnclick.search('comments') != -1){
		...
		if(htmlOnclick.search('unlike') != -1){
		 if(sesJqueryObject('#ses_media_lightbox_container').css('display') == 'block'){
			...
			showGroupTooltip(10,10,'<i class="fa fa-thumbs-up"></i><span>'+en4.core.language.translate(itemType+" removed from like successfully")+'</span>');
		}else{
			if(sesJqueryObject('#ses_media_lightbox_container').css('display') == 'block'){
			...
			showGroupTooltip(10,10,'<i class="fa fa-thumbs-up"></i><span>'+en4.core.language.translate(itemType+" like successfully")+'</span>', 'sesbasic_liked_notification');
		}
	}
});
