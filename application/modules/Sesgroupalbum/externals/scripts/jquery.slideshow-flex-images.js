...
(function($){
    ...
    $.fn.flexImagesSlideshow = function(options){
        var o = $.extend({ container: '.slideshow_sesgroupalbum_plugin', object: 'img', rowHeight: 197, maxRows: 0, truncate: false}, options);
        ...
        function _helper(lastRow){
            if (o.maxRows && rows > o.maxRows || o.truncate && lastRow) {
							if(getX == 0){
								...
								if(isComplete)
									return;
								sesJqueryObject('#sesgroupalbum_slideshow'+identitySlideshow).append('<div id="idDiv_'+idDiv+'" style="float:left"></div>');
							 }
							sesJqueryObject('#idDiv_'+idDiv).append(sesJqueryObject('#'+row[x][0].attr('id'))[0].outerHTML);
							getX++;
							if(row[x][0].attr('id') == 'sesgroupalbum_slideshow_id_'+itemCountSlideshow){
								getX = 0;
								row[x][0].remove();
