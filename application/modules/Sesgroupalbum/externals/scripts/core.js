...
en4.album = {
  ...
  rotate : function(photo_id, angle) {
    request = new Request.JSON({
      url : en4.core.baseUrl + 'sesgroupalbum/photo/rotate',
      data : {
        format : 'json',
        photo_id : photo_id,
        angle : angle
  ...
  flip : function(photo_id, direction) {
    request = new Request.JSON({
      url : en4.core.baseUrl + 'sesgroupalbum/photo/flip',
      data : {
        format : 'json',
        photo_id : photo_id,
        direction : direction
  ...
  crop : function(photo_id, x, y, w, h) {
    ...
    request = new Request.JSON({
      url : en4.core.baseUrl + 'sesgroupalbum/photo/crop',
      data : {
        format : 'json',
        photo_id : photo_id,
        x : x,
...
var marker;
var mapLoad = true;
function initializeSesGroupAlbumMap() {
  var mapOptions = {
  ...
  google.maps.event.addListener(autocomplete, 'place_changed', function() {
	...
	google.maps.event.addDomListener(window, 'load', initializeSesGroupAlbumMap);
}
function editGroupSetMarkerOnMap(){
	geocoder = new google.maps.Geocoder();
	var address = trim(document.getElementById('sesgroup_location_data').innerHTML);
...
//list page map 
function initializeSesGroupAlbumMapList() {
if(mapLoad){
  var mapOptions = {
    center: new google.maps.LatLng(-33.8688, 151.2195),
    zoom: 17
...
if(mapLoad){
  ...
  if(mapLoad){
	...
	if(mapLoad){
		google.maps.event.addDomListener(window, 'load', initializeSesGroupAlbumMapList);
	}
}

function editGroupSetMarkerOnMapList(){
	geocoder = new google.maps.Geocoder();
if(mapLoad){
	if(document.getElementById('sesgroup_location_data_list'))
		var address = trim(document.getElementById('sesgroup_location_data_list').innerHTML);
}else{
...
sesJqueryObject(document).on('click','.smoothboxGroupOpen',function(){
	var url = sesJqueryObject(this).attr('href');
	openGroupURLinkSmoothBox(url);
	return false;
});
function openGroupURLinkSmoothBox(openURLsmoothbox){
	Smoothbox.open(openURLsmoothbox);
	parent.Smoothbox.close;
	return false;
}
// ALBUM LIKE ON ALBUM LISTINGS
sesJqueryObject(document).on('click','.sesgroupalbum_albumlike',function(){
		var data = sesJqueryObject(this).attr('data-src');
		var objectDocument = this;
		 (new Request.JSON({
			url : en4.core.baseUrl + 'sesgroupalbum/album/like/album_id/'+data,
			data : {
				format : 'json',
		 ...
		 onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
			 ...
			 if(data.status == 'false'){
              ...
              sesJqueryObject(objectDocument).addClass('button_active');
							...
							showGroupTooltip(10,10,'<i class="fa fa-thumbs-up"></i><span>'+(en4.core.language.translate("Album liked successfully"))+'</span>', 'sesbasic_liked_notification');
					 }else{
              sesJqueryObject(objectDocument).removeClass('button_active');
							sesJqueryObject(objectDocument).find('span').html(data.like_count);
							showGroupTooltip(10,10,'<i class="fa fa-thumbs-up"></i><span>'+en4.core.language.translate("Album Unliked successfully")+'</span>');
					 }						 
				}
			var ObjectIncrem = sesJqueryObject(objectDocument).parent().parent().find('.sesgroupalbum_list_grid_info').find('.sesgroupalbum_list_grid_stats');
			var ObjectLength = sesJqueryObject(ObjectIncrem).children();
			if(ObjectLength.length >0){
					for(i=0;i<ObjectLength.length;i++){
						if(ObjectLength[i].hasClass('sesgroupalbum_list_grid_likes')){
...
function sesgroupalbumRotate(photo_id,rotateAngle){
	var className;
	sesJqueryObject('#ses-rotate-'+rotateAngle).attr('class','icon_loading');
	if(rotateAngle == 90 || rotateAngle == 270){
		if(rotateAngle == 90)
			className = 'sesgroupalbum_icon_photos_rotate_ccw';
		else
			className = 'sesgroupalbum_icon_photos_rotate_cw';		
		rotateSesgroupalbum(photo_id,rotateAngle,className);
	}else{
		if(rotateAngle == 'horizontal')
			className = 'sesgroupalbum_icon_photos_flip_horizontal';
		else
			className = 'sesgroupalbum_icon_photos_flip_vertical';
		flipSesgroupalbum(photo_id,rotateAngle,className);
	}
		
	return false;
}
function flipSesgroupalbum(photo_id,rotateAngle,className){
	request = new Request.JSON({
      url : en4.core.baseUrl + 'groupalbums/photo/flip',
      data : {
        format : 'json',
        photo_id : photo_id,
...
function rotateSesgroupalbum(photo_id,rotateAngle,className){
	request = new Request.JSON({
		url : en4.core.baseUrl + 'groupalbums/photo/rotate',
      data : {
        format : 'json',
        photo_id : photo_id,
        angle : rotateAngle
...
sesJqueryObject(document).on('click','.sesgroupalbum_albumFav',function(){
		var data = sesJqueryObject(this).attr('data-src');
		var objectDocument = this;
		 (new Request.JSON({
			url : en4.core.baseUrl + 'sesgroupalbum/album/fav/album_id/'+data,
			data : {
				format : 'json',
					type : 'sesgroupalbum_album',
				id : data,
		 ...
		 onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
			 ...
			 if(data.status == 'false'){
              ...
              sesJqueryObject(objectDocument).addClass('button_active');
							...
							showGroupTooltip(10,10,'<i class="fa fa-heart"></i><span>'+en4.core.language.translate("Album added as Favourite successfully")+'</span>', 'sesbasic_favourites_notification');
					 }else{
              sesJqueryObject(objectDocument).removeClass('button_active');
							sesJqueryObject(objectDocument).find('span').html(data.favourite_count);
							showGroupTooltip(10,10,'<i class="fa fa-heart"></i><span>'+en4.core.language.translate("Album Unfavourited successfully")+'</span>');
					 }						 
				}
			var ObjectIncrem = sesJqueryObject(objectDocument).parent().parent().find('.sesgroupalbum_list_grid_info').find('.sesgroupalbum_list_grid_stats');
			var ObjectLength = sesJqueryObject(ObjectIncrem).children();
			if(ObjectLength.length >0){
					for(i=0;i<ObjectLength.length;i++){
						if(ObjectLength[i].hasClass('sesgroupalbum_list_grid_fav')){
							var title = sesJqueryObject(ObjectLength[i]).attr('title').replace(/[^0-9]/g, '');	
...
sesJqueryObject(document).on('click','.sesgroupalbum_photoFav ,#sesgroupalbum_favourite',function(){
		var data = sesJqueryObject(this).attr('data-src');
		var objectDocument = this;
		 (new Request.JSON({
			url : en4.core.baseUrl + 'sesgroupalbum/photo/fav/photo_id/'+data,
			data : {
				format : 'json',
				type : 'photo',
				id : data,
		 ...
		 onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
			 ...
			 if(data.status == 'false'){
              ...
              sesJqueryObject(objectDocument).addClass('button_active');
							sesJqueryObject(objectDocument).find('span').html(data.favourite_count);
							showGroupTooltip(10,10,'<i class="fa fa-heart"></i><span>'+en4.core.language.translate("Photo added as Favourite successfully")+'</span>', 'sesbasic_favourites_notification');
					 }else{
              sesJqueryObject(objectDocument).removeClass('button_active');
							sesJqueryObject(objectDocument).find('span').html(data.favourite_count);
							showGroupTooltip(10,10,'<i class="fa fa-heart"></i><span>'+en4.core.language.translate("Photo Unfavourited successfully")+'</span>');
					 }						 
				}
			var ObjectIncrem = sesJqueryObject(objectDocument).parent().parent().find('.sesgroupalbum_list_grid_info').find('.sesgroupalbum_list_grid_stats');
			var ObjectLength = sesJqueryObject(ObjectIncrem).children();
...
sesJqueryObject(document).on('click','.sesgroupalbum_admin_sponsored',function(event){
...
sesJqueryObject(document).on('click','.sesgroupalbum_admin_featured',function(event){
		event.preventDefault();
		var data = sesJqueryObject(this).attr('href');
		sesJqueryObject(this).css('pointer-events','none');
		var objectDocument = this;
...
sesJqueryObject(document).on('click','.sesgroupalbum_photolike',function(){
		var data = sesJqueryObject(this).attr('data-src');
		var objectDocument = this;
		 (new Request.JSON({
			url : en4.core.baseUrl + 'sesgroupalbum/photo/like/photo_id/'+data,
			data : {
				format : 'json',
		 ...
		 onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
			 ...
			 if(data.status == 'false'){
              ...
              sesJqueryObject(objectDocument).addClass('button_active');
							sesJqueryObject(objectDocument).find('span').html(data.like_count);
							showGroupTooltip(10,10,'<i class="fa fa-thumbs-up"></i><span>'+en4.core.language.translate("Photo Liked successfully")+'</span>', 'sesbasic_liked_notification');
					 }else{
              sesJqueryObject(objectDocument).removeClass('button_active');
							sesJqueryObject(objectDocument).find('span').html(data.like_count);
							showGroupTooltip(10,10,'<i class="fa fa-thumbs-up"></i><span>'+en4.core.language.translate("Photo Unliked successfully")+'</span>');
					 }
...
sesJqueryObject(document).on({
	 mouseenter: function(){
			...
			var totalImageCount = sesJqueryObject(this).find('.sesgroupalbum_list_grid_img').find('.ses_image_container').children('.child_image_container').length;
			var changeTime = 2000;
			interval = setInterval(function(){ 
				var imageURL = sesJqueryObject(imageContainerObject).find('.sesgroupalbum_list_grid_img').find('.ses_image_container').children().eq(imageIndex).html();
				sesJqueryObject(imageContainerObject).find('.sesgroupalbum_list_grid_img').find('.main_image_container').css('background-image', 'url(' + imageURL + ')');
	 ...
	 mouseleave: function(){
		 var imageContainerObject = this;
			...
			var totalImageCount = sesJqueryObject(this).find('.sesgroupalbum_list_grid_img').find('.ses_image_container').children('.child_image_container').length;
			var imageURL = sesJqueryObject(imageContainerObject).find('.sesgroupalbum_list_grid_img').find('.ses_image_container').children().eq(totalImageCount-1).html();
			sesJqueryObject(imageContainerObject).find('.sesgroupalbum_list_grid_img').find('.main_image_container').css('background-image', 'url(' + imageURL + ')');
	}
}, '.sesgroupalbum_list_grid');
...
// profile tab resize
sesJqueryObject(document).on('click','.tab_layout_sesgroupalbum_profile_albums',function (event) {
...
sesJqueryObject(document).on('click','ul#main_tabs li.tab_layout_sesgroupalbum_profile_albums',function(event){
		sesJqueryObject(window).trigger('resize');
});
function showGroupTooltip(x, y, contents, className) {
...
// 	return false;
// }
// var sesgroupalbum_cookie_set_value = [];
// function setCookieSesgroupalbum(cvalue) {
//     var d = new Date();
//     d.setTime(d.getTime() + (1*24*60*60*1000));
//     var expires = "expires="+d.toGMTString();
// 		var length = sesgroupalbum_cookie_set_value.length;
// 		var exists = false;
// 		if(length > 0){
// 			for (var i = 0; i < length; i++) {
// 				if (sesgroupalbum_cookie_set_value[i] === cvalue) 
// 				{
// 					exists = true;   
// 					break;
// 				}
...
// 		}
// 		if(!exists)
// 			sesgroupalbum_cookie_set_value.push(cvalue);
//     document.cookie = 'sesgroupalbum_lightbox_value' + "=" + sesgroupalbum_cookie_set_value + "; " + expires+"; path=/"; 
//   }