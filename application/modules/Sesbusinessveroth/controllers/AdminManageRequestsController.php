 ...
 * @version    $Id: AdminManageRequestsController.php  2018-11-02 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class sesbusinessveroth_AdminManageRequestsController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesbusinessveroth_admin_main', array(), 'sesbusinessveroth_admin_main_managerequests');

    $this->view->formFilter = $formFilter = new Sesbusinessveroth_Form_Admin_Manage_Filter();
    $business = $this->_getParam('business', 1);

    $table = Engine_Api::_()->getDbtable('businesses', 'sesbusiness');
    $tableName = $table->info('name');

    $verificationsTable = Engine_Api::_()->getDbtable('verifications', 'sesbusinessveroth');
...
//     if (!empty($values['displayname']))
         $select->join($verificationsTableName, "$tableName.business_id = $verificationsTableName.resource_id", array('verification_id', 'poster_id', 'resource_id', 'description', 'admin_approved', 'creation_date'));
//     else
//         $select->join($verificationsTableName, "$tableName.user_id = $verificationsTableName.poster_id",    array('verification_id', 'poster_id', 'resource_id', 'description', 'admin_approved', 'creation_date'));
    ...
    //Set up select info
    $select->order((!empty($values['order']) ? $values['order'] : 'business_id' ) . ' ' . (!empty($values['order_direction']) ? $values['order_direction'] : 'DESC' ));
...
//       $select->where($tableName.'.user_id = ?', (int) $values['user_id']);
    ...
    $this->view->paginator = $paginator->setCurrentPageNumber($business);
  ...
  public function approveAction() {
    ...
    $this->view->form = $form = new Sesbusinessveroth_Form_Admin_Approve();
    $item = Engine_Api::_()->getItem('sesbusinessveroth_verification', $verification_id);

    $poster = Engine_Api::_()->getItem('user', $item->poster_id);
    $resource = Engine_Api::_()->getItem('businesses', $item->resource_id);
    ...
    if ($this->getRequest()->isPost()) {
      ...
      Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($poster, $viewer, $resource, 'sesbusinessveroth_approve');

      Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($resourceOwner, $poster, $resource, 'sesbusinessveroth_verified');
  ...
  public function deleteAction() {
    ...
    $this->view->form = $form = new Sesbusinessveroth_Form_Admin_Delete();
  ...
  public function rejectAction() {
    ...
    $verification_id = $this->_getParam('verification_id');
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->form = $form = new Sesbusinessveroth_Form_Admin_Reject();
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $resource = Engine_Api::_()->getItem('businesses', $item->resource_id);
        $item->delete();
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($poster, $viewer, $resource, 'sesbusinessveroth_adminreject');
        $db->commit();
      } catch (Exception $e) {
        $db->rollBack();
        throw $e;
