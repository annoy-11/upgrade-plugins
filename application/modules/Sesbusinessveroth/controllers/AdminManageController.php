 ...
 * @version    $Id: AdminManageController.php  2018-11-02 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class sesbusinessveroth_AdminManageController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesbusinessveroth_admin_main', array(), 'sesbusinessveroth_admin_main_manage');

    $this->view->formFilter = $formFilter = new Sesbusinessveroth_Form_Admin_Manage_FilterManage();
...
//       $select->where('featured = ?', $values['featured']);
  ...
  public function enabledAction() {
    ...
    if (!empty($resource_id) && $admin_enabled) {
      Engine_Api::_()->getDbTable('verifications', 'sesbusinessveroth')->update(array('admin_enabled' => 0), array('resource_id = ?' => $resource_id));
    } else {
      Engine_Api::_()->getDbTable('verifications', 'sesbusinessveroth')->update(array('admin_enabled' => 1), array('resource_id = ?' => $resource_id));
    }
    $this->_redirect('admin/sesbusinessveroth/manage');
  }
  ...
  public function removeAction() {
    ...
    $this->view->form = $form = new Sesbusinessveroth_Form_Admin_Remove();
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        Engine_Api::_()->getDbTable('verifications', 'sesbusinessveroth')->delete(array('resource_id' => $resource_id));
        $db->commit();
      } catch (Exception $e) {
        $db->rollBack();
        throw $e;
