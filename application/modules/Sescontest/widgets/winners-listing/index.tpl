...
<?php } ?>
    <?php foreach($this->paginator as $entry):?>
      ...
      <?php endif;?>
      <?php $canComment = Engine_Api::_()->authorization()->isAllowed('participant', $viewer, 'comment');?>
  ...
  <script type="text/javascript">
    ...
    <?php if($this->params['pagging'] != 'pagging'){ ?>
      ...
      function viewMore_<?php echo $randonNumber; ?> (){
        if(typeof requestViewMore_<?php echo $randonNumber; ?> != 'undefined')
          requestViewMore_<?php echo $randonNumber; ?>.cancel();
					if(page<?php echo $randonNumber; ?> > 1){
        ...
        requestViewMore_<?php echo $randonNumber; ?> = new Request.HTML({
          ...
          onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
              ...
              sesJqueryObject('#loading_images_browse_<?php echo $randonNumber; ?>').remove();
							 sesJqueryObject('#tabbed-widget_<?php echo $randonNumber; ?>').find('.sesbasic_loading_container').remove();
              if($('loadingimgsescontest-wrapper'))
              sesJqueryObject('#loadingimgsescontest-wrapper').hide();
                document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML = document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML + responseHTML;
              document.getElementById('loading_image_<?php echo $randonNumber; ?>').style.display = 'none';
							if(sesJqueryObject('#sescontest_pinboard_view_<?php echo $randonNumber; ?>').length > 0){
								if(sesJqueryObject('#sescontest_pinboard_view_<?php echo $randonNumber; ?>').hasClass('active')){
									pinboardLayout_<?php echo $randonNumber ?>();
								}
							}
