		...
		this.titleText.empty().adopt(
			Array.from(content).map(function(item, i){
				if( typeOf(item) == 'element'){
                    ...
                    var newText = fn(item, this.options);
                    var splltText = newText.split(' ');
                    newText = en4.core.language.translate('Ses'+splltText[0])+' '+splltText[1];
                   return new Element('div.column', {text: newText}).addClass('column_' + (i + 1));
            }
