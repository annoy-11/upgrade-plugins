...
<?php if(empty($widgetName)):?>
  <?php if(strlen($entry->getTitle()) > $this->params['list_title_truncation']):?>
...
<?php else:?>
  <?php $canIntegrate = 0;?>
<?php endif;?>
<li class="sescontest_winners_list_item sesbasic_clearfix">
  <article class="sesbasic_clearfix">
    <div class="_thumb" style="height:<?php echo is_numeric($height)?$height.'px':$height?>;width:<?php echo is_numeric($width)?$width.'px':$width ?>;">
      ...
      <div class="sescontest_winners_item_award">
        ...
        <!--<?php if(isset($this->mediaTypeActive)):?>
          <span class="_contestname"><a href="<?php echo $this->url(array('action' => $action),'sescontest_media',true);?>"><?php echo $contestType;?></span>
        <?php endif;?>-->
      </div>
    </div>
    <div class="_info">
      <?php if(isset($this->titleActive)):?>
        <div class="_title"><a href="<?php echo $entry->getHref();?>"><?php echo $title;?></a></div>
      <?php endif;?>
      <div class="_details sesbasic_clearfix">
        ...
        <div>
          ...
          <?php if(isset($this->ownerNameActive)):?><div class="_owner sesbasic_text_light"><?php echo $this->translate('by');?>&nbsp;<?php echo $this->htmlLink($owner->getHref(), $owner->getTitle());?><?php endif;?><?php if($this->contestNameActive):?>&nbsp;<?php echo $this->translate('in');?>&nbsp;<a href="<?php echo $contest->gethref();?>"><?php echo $contest->getTitle();?></a><?php endif;?></div>
          <?php endif;?>
          <div class="_stats sesbasic_text_light">
          	<?php if(isset($this->submitDateActive)):?><span><i class="fa fa-calendar"></i><span title="<?php echo $this->translate('Submitted in');?> <?php echo date('d-m-Y',strtotime($entry->creation_date));?>"><?php echo date('d-m-Y',strtotime($entry->creation_date));?></span>&nbsp;&nbsp;|</span><?php endif;?>
            <?php if(isset($this->voteCountActive) && (!$contest->resulttime || strtotime($contest->resulttime) <= time())):?>
              <span title="<?php echo $this->translate(array('%s Vote', '%s Votes', $entry->vote_count), $this->locale()->toNumber($entry->vote_count)) ?>"><i class="fa fa-hand-o-up"></i><span><?php echo $entry->vote_count;?></span></span>
            <?php endif;?>
            <?php if(isset($this->likeActive)):?>
              <span title="<?php echo $this->translate(array('%s Like', '%s Likes', $entry->like_count), $this->locale()->toNumber($entry->like_count)) ?>"><i class="fa fa-thumbs-up"></i><span><?php echo $entry->like_count;?></span></span>
            <?php endif;?>
            <?php if(isset($this->commentActive)):?>
              <span title="<?php echo $this->translate(array('%s Comment', '%s Comments', $entry->comment_count), $this->locale()->toNumber($entry->comment_count)) ?>"><i class="fa fa-comment"></i><span><?php echo $entry->comment_count;?></span></span>
            <?php endif;?>
            <?php if(isset($this->viewActive)):?>
              <span title="<?php echo $this->translate(array('%s View', '%s Views', $entry->view_count), $this->locale()->toNumber($entry->view_count)) ?>"><i class="fa fa-eye"></i><span><?php echo $entry->view_count;?></span></span>
            <?php endif;?>
            <?php if(isset($this->favouriteActive)):?>
              <span title="<?php echo $this->translate(array('%s Favourite', '%s Favourites', $entry->favourite_count), $this->locale()->toNumber($entry->favourite_count)) ?>"><i class="fa fa-heart"></i><span><?php echo $entry->favourite_count;?></span></span>
            <?php endif;?>
          </div>
        </div>
      </div>
