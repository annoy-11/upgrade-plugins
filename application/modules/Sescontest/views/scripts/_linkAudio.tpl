  ...
  en4.core.runonce.add(function() {
   sesJqueryObject('<div class="sescontest_photo_update_popup sesbasic_bxs" id="sescontest_popup_existing_upload" style="display:none"><div class="sescontest_photo_update_popup_overlay"></div><div class="sescontest_photo_update_popup_container" id="sescontest_popup_container_existing"><div class="sescontest_photo_update_popup_header"><?php echo $this->translate("Select a Audio") ?><a class="fa fa-close" href="javascript:;" onclick="hideContentAudioUpload()" title="<?php echo $this->translate("Close") ?>"></a></div><div class="sescontest_photo_update_popup_content"><div id="sescontest_album_existing_data"></div><div id="sescontest_profile_existing_img" style="display:none;text-align:center;"><img src="application/modules/Sesbasic/externals/images/loading.gif" alt="<?php echo $this->translate("Loading"); ?>" style="margin-top:10px;"  /></div></div></div></div>').appendTo('body');
	...
	$('photouploader-wrapper').style.display = 'none';
       ...
       // if(sesJqueryObject('#contest_link_audio_preview-wrapper').find('iframe').length > 0)
    ...
    sesJqueryObject(document).on('click','#contest_link_audio_preview-wrapper',function(e){
        e.preventDefault();
...
function existingMyAudiosGet(){
  ...
  sesJqueryObject(document).on('click','a[id^="sescontest_profile_upload_existing_photos_"]',function(event){
	...
	if(!id)
      return;
    var url1 = sesJqueryObject(this).data('url');
    var html1 = '<audio controls><source src="'+url1+'" type="audio/mpeg"></audio>';
	$('contest_link_audio_preview-wrapper').style.display = 'none';
    $('contest_link_audio_data-wrapper').style.display = 'block';
	sesJqueryObject('#contest_link_audio_data-wrapper').html(html1);
    sesJqueryObject('#sescontest_link_id').val(id);
    sesJqueryObject('#removelinkaudio').show();
    sesJqueryObject('#remove_link_audio-wrapper').show();
    resetData();
...
function removeLinkAudio() {
    ...
    sesJqueryObject('#contest_link_audio_preview-wrapper').show();
