  ...
  <?php if(count($_POST) == 0 && Engine_Api::_()->getApi('settings','core')->getSetting('sescontest.category.selection', 1)):?>
   ...
   <div id="sescontest_share_content_html" style="display:block;">
      ...
      if (input.files && input.files[0] && (ext == "png" || ext == "jpeg" || ext == "jpg" || ext == 'PNG' || ext == 'JPEG' || ext == 'JPG')){
        ...
        reader.onload = function (e) {
         // $(id+'-wrapper').style.display = 'block';
          ...
          sesJqueryObject('#sescontest-image').css('background-image', 'url(' + e.target.result + ')');
        }
    ...
    function defaultPhoto(){
      ...
      if(sesJqueryObject('#conteststyle-element').length){
        currentTypeSelectedValue = sesJqueryObject("input[name='conteststyle']:checked").val();
      }
      ...
      sesJqueryObject('#sescontest-image').css('background-image', 'url(' + url + ')');
    }
    //validate form
    ...
    function validateForm(){
      ...
      en4.core.runonce.add(function() {
       sesJqueryObject('#sescontest_create_form').append('<button name="submit_share" style="display:none;" id="share_content_btn" type="submit">Save Changes</button>');
        defaultPhoto();
        sesJqueryObject(document).on('submit','#sescontest_create_form',function(e){
      ...
      function createImageOfContent(){
        ...
        //set title
        sesJqueryObject('#sescontest-title').html(sesJqueryObject('#title').val());
        if(sesJqueryObject('#description').css('display') == 'none'){
           var	content = tinymce.get(sesJqueryObject('#description').attr('id')).getContent();
        }else
          var content = sesJqueryObject('#description').val();
        
        //set description
        sesJqueryObject('#sescontest-description').html(sesJqueryObject(content).text());
        //set date
        sesJqueryObject('#sescontest-date').html('on '+sesJqueryObject('#sescontest_start_date').val());
        //set media type
        sesJqueryObject('#sescontest-media-type').html(sesJqueryObject("#contest_type option:selected").text());
        var elem = sesJqueryObject('#content_share_img');
        if(elem.length ==0){
          sesJqueryObject('#sescontest_create_form').prepend('<input type="hidden" name="content_share_img" id="content_share_img">'); 
          sesJqueryObject('#sescontest_create_form').append('<button name="submit_share" style="display:none;" id="share_content_btn" type="submit">Save Changes</button>');
        }
      ...
      function makeImageOfHTML(imageUrl){
         isSubmitTrue = true;
         sesJqueryObject('#content_share_img').val(imageUrl);
          setTimeout(function(){
            sesJqueryObject('#share_content_btn').unbind("submit").submit();
          },1000);    
      }
