...
class Estorepackage_Model_DbTable_Packages extends Engine_Db_Table {

  protected $_rowClass = 'Estorepackage_Model_Package';
  ...
  public function cancelSubscription($params = array()) {
    ...
    $db->query("DELETE FROM engine4_estorepackage_orderspackages WHERE package_id = $packageId && owner_id = " . $viewerId);
    $db->query("DELETE FROM engine4_estorepackage_transactions WHERE package_id = $packageId && owner_id = " . $viewerId);
    //End Work of Package Related Data
    
    Engine_Api::_()->getDbtable('stores', 'estore')->update(array('is_approved' => 0), array('package_id =?' => $packageId, 'owner_id =?' => $viewerId));

    $transactionTable = Engine_Api::_()->getDbTable('transactions', 'estorepackage');
