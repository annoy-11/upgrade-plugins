...
?>
<div class="estore_package_details sesbasic_bxs">
  ...
  <?php }else{ ?>
    ...
    <?php if($package->isOneTime()){
      if($isrenew){
        if(!empty($transaction->expiration_date) && $transaction->expiration_date != '3000-00-00 00:00:00'){
          ...
          <div class="sesbasic_clearfix"><a href="<?php echo $this->url(array('store_id' => $item->store_id,'action'=>'index'), 'estorepackage_payment', true); ?>" class="estore_payment_btn sesbasic_animation"><i class="fa fa-paypal"></i><span><?php echo $this->translate("Reniew Store Payment"); ?></span></a></div>
         <?php 	
          }
        }else{ ?>
    ...
    <?php }else{  ?>
      <div>
        <a href="<?php echo $this->url(array('store_id' => $item->store_id,'action'=>'index'), 'estorepackage_payment', true); ?>" class="estore_payment_btn sesbasic_animation"><i class="fa fa-money"></i><span><?php echo $this->translate("Make Payment"); ?></span></a>
      </div>
    <?php } ?>
  <?php } ?>
</div>  