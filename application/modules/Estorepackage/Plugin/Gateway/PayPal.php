...
class Estorepackage_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function getGateway() {
    if (null === $this->_gateway) {
      ...
      $gateway = new $class(array(
          ...
          'currency' => Engine_Api::_()->estorepackage()->getCurrentCurrency(),
      ));
  ...
  public function createStoreTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $item, Estorepackage_Model_Package $package, array $params = array()) {
    // Process description
    ...
    $defaultCurrency = Engine_Api::_()->estorepackage()->defaultCurrency();
  ...
  Payment_Model_Order $order, array $params = array(),$bankTransfer = false) {
    ...
    if ($package->isOneTime()) {
      ...
      $session = new Zend_Session_Namespace('Payment_Estorepackage');
      ...
      if (!empty($oldOrderPackageId)) {
        $select = $tableStore->select()->from($tableStore->info('name'))->where('orderspackage_id =?', $oldOrderPackageId);
        $totalItemCreated = count($tableStore->fetchAll($select));
        if ($package->item_count >= $totalItemCreated && $package->item_count)
          $leftStore = $package->item_count - $totalItemCreated;
        else if (!$package->item_count)
          $leftStore = -1;
        else
          $leftStore = 0;
      } else
        $leftStore = $package->item_count - 1;
      $tableStore->update(array('orderspackage_id' => $orderPackageId), array('orderspackage_id' => $oldOrderPackageId));
      $packageOrder = Engine_Api::_()->getItem('estorepackage_orderspackage', $orderPackageId);
      $packageOrder->item_count = $leftStore;
      $packageOrder->save();
      ...
      // Get benefit setting
      $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'estorepackage')
      ...
      if (($paymentStatus == 'okay' || $paymentStatus == 'active' ||
        ...
        if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('ecoupon')){
          Engine_Api::_()->ecoupon()->setAppliedCouponDetails($params['couponSessionCode']);
        }
        //For Credit 
  ...
  public function cancelStore($transactionId, $note = null) {
    $profileId = null;
    if ($transactionId instanceof Estorepackage_Model_Transaction) {
