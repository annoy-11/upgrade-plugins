...
class Estorepackage_PaymentController extends Core_Controller_Action_Standard {
  ...
  public function init() {
    ...
    if (Engine_Api::_()->getDbtable('gateways', 'payment')->getEnabledGatewayCount() <= 0 ||
            Engine_Api::_()->getDbtable('packages', 'estorepackage')->getEnabledNonFreePackageCount() <= 0) {
    ...
    $this->_session = new Zend_Session_Namespace('Payment_Estorepackage');
    $this->_session->gateway_id = $this->_getParam('gateway_id', 0);
    $this->_item = Engine_Api::_()->getItem('stores', $this->_getParam('store_id'));
    if (!$this->_item)
  ...
  public function gatewayAction() {
    ...
    // For Coupon 
    $this->view->couponSessionCode = $package->getType().'-'.$package->package_id.'-'.$this->_item->getType().'-'.$this->_item->store_id.'-1';
  ...
  public function processAction($bankTransfer = false) {
    if (!$bankTransfer) {
      ...
      if (!$gatewayId ||
              !($gateway = Engine_Api::_()->getDbtable('gateways', 'estorepackage')->find($gatewayId)->current()) ||
              !($gateway->enabled)) {
    ...
    // For Coupon 
    $couponSessionCode = $package->getType().'-'.$package->package_id.'-'.$this->_item->getType().'-'.$this->_item->store_id.'-1';
    ...
    $this->_session->currency = $currentCurrency = Engine_Api::_()->estorepackage()->getCurrentCurrency();
    ...
    if (!$bankTransfer) {
      ...
      $class = str_replace('Payment', 'Estorepackage', 'Payment_Plugin_Gateway_Paypal');
      Engine_Loader::loadClass($class);
      $gatewayPaypal = Engine_Api::_()->getItem('estorepackage_gateway', 1);
      $plugin = new $class($gatewayPaypal);
      $order = Engine_Api::_()->getItem('payment_order', $order_id);
      try {
        $status = $plugin->onStoreTransactionReturn($order, array(), 'banktransfer');
    ...
    $params['cancel_url'] = $schema . $host
            ...
            . '?order_id=' . $order_id
            . '&state=' . 'cancel';
    $params['ipn_url'] = $schema . $host . $this->view->url(array('action' => 'index', 'controller' => 'ipn', 'module' => 'estorepackage'), 'default') . '?order_id=' . $order_id . '&gateway_id=' . $gatewayId;
    ...
    $creditCode =  'credit'.'-estorepackage-'.$package->package_id.'-'.$this->_item->store_id;
    ...
    if($gateway->plugin == "Sesadvpmnt_Plugin_Gateway_Stripe") {
        $this->view->currency =  $params['currency'] =  $currentCurrency = Engine_Api::_()->estorepackage()->getCurrentCurrency();
        ...
        $paytmParams = $plugin->createStoreTransaction($this->_user, $this->_item, $package, $params);
        ...
        // Process transaction
        $transaction = $plugin->createStoreTransaction($this->_user, $this->_item, $package, $params);
        // Pull transaction params
  ...
  public function returnAction() {
    ...
    if (
        ...
        $order->source_type != 'stores' ||
        !($item = $order->getSource()) ||
        !($package = $this->_item->getPackage()) ||
        !($gateway = Engine_Api::_()->getItem('estorepackage_gateway', $order->gateway_id))) {
        return $this->_helper->redirector->gotoRoute(array('action' => 'manage'), 'estore_general', true);
    }
    // Get gateway plugin
    $this->view->gatewayPlugin = $gatewayPlugin = $gateway->getGateway();
    ...
    $params  = array();
    //For Coupon
    $couponSessionCode = $couponSessionCode = $package->getType().'-'.$package->package_id.'-'.$this->_item->getType().'-'.$this->_item->store_id.'-1';
    $params['amount'] = @isset($_SESSION[$couponSessionCode]) ? round($package->price - $_SESSION[$couponSessionCode]['discount_amount']) : $package->price;
    ...
    try {
      if($gateway->plugin == "Sesadvpmnt_Plugin_Gateway_Stripe") {
          if(isset($_POST['stripeToken'])){
            ...
            $params['currency'] = Engine_Api::_()->estorepackage()->getCurrentCurrency();
            $params['gateway_id'] = $gateway->gateway_id;
            $params['order_id'] = $order->order_id;
            $params['type'] = "estorepackage_gateway";
            if(!$package->isOneTime()) {
          ...
          $status = $plugin->onStoreTransactionReturn($order,$params);
      } else {
        $status = $plugin->onStoreTransactionReturn($order,array_merge($this->_getAllParams(),$params));
      }
    } catch (Payment_Model_Exception $e) {
      $status = 'failure';
      $this->_session->errorMessage = $e->getMessage();
  ...
  public function fillDetailsAction() {
    ...
    $this->view->form = $form = new Estorepackage_Form_Filldetail();
    //Check post
