 ...
 */
class Seseventpdfticket_Installer extends Engine_Package_Installer_Module {
  ...
  public function onPreinstall() {
    ...
    if(!empty($sesevent_Check) && !empty($event_activate[0]['value'])) {
      $plugin_currentversion = '4.8.12p2';
  ...
  public function onEnable() {
		$db = $this->getDb();
		$db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '1' WHERE `engine4_core_menuitems`.`name` = 'sesevent_admin_main_seseventpdfticket';");
    parent::onEnable();
  }
  
  public function onDisable() {
		$db = $this->getDb();
		$db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '0' WHERE `engine4_core_menuitems`.`name` = 'sesevent_admin_main_seseventpdfticket';");
    parent::onDisable();
  }
}