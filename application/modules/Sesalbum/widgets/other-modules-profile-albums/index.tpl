...
<?php } ?>
         ...
         <?php if($this->view_type != 'masonry'){ ?>
            <li id="thumbs-photo-<?php echo $photo->photo_id ?>"  class="ses_album_image_viewer sesalbum_list_grid_thumb sesalbum_list_grid  sesa-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sesa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?> sesbm sesalbum_profile_list_width_<?php echo $randonNumber; ?>">
...
<script type="text/javascript">
	function paggingNumber<?php echo $randonNumber; ?>(pageNum){
		 ...
		 if(typeof valueTab == 'undefined')
			...
			(new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + 'widget/index/mod/sesalbum/name/other-modules-profile-albums/openTab/' +valueTab,
				'data': {
					format: 'html',
					page: pageNum,    
	...
	function changeTabSes_<?php echo $randonNumber; ?>(valueTab){
			if(valueTab == 'albumCreate'){
				var albumcreate = "<?php echo $this->url(array('action' => 'create', 'resource_id' => $this->resource_id, 'resource_type' => $this->resource_type), 'sesalbum_general', true); ?>";
