 ...
 */
class Sesalbum_Widget_OtherModulesProfileAlbumsController extends Engine_Content_Widget_Abstract
{
  ...
  public function indexAction()
  {
		...
		$this->view->resource_id = $resource_id = isset($params['resource_id']) ? $params['resource_id'] :$this->_getParam('resource_id', $subject->getIdentity());
		
		$this->view->resource_type = $resource_type = isset($params['resource_type']) ? $params['resource_type'] :$this->_getParam('resource_type', $subject->getType());
	...
	if(count($show_criterias)){
		...
		if(!$is_ajax){
			...
			foreach($defaultOptionsArray as $key=>$defaultValue){
				if($defaultValue == 'albumCreate' && $viewer->getIdentity() == 0)
					continue;
				...
				if( $this->_getParam($defaultValue.'_label'))
					...
					else if($defaultValue == 'albumCreate')
						$valueLabel = 'Add Album';
				}
