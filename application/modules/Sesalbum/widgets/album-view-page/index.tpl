          ...
          <div class="sesalbum_cover_user_options sesbasic_clearfix">
            <?php if(!empty($this->socialSharingIcons)){ ?>
             ...
             <?php if($this->viewer->getIdentity() != 0){ ?>
              <a title="<?php echo $this->translate('Share'); ?>" href="<?php echo $this->url(array("action" => "share", "type" => "album", "photo_id" => $this->album->getIdentity(),"format" => "smoothbox"), 'sesalbum_general', true); ?>" class="sesalbum_view_share_button smoothbox"><i class="fa fa-share"></i></a></li>
              <?php } ?>
            <?php } ?>
        ...
        <div class="sesalbum_album_other_info_field clear sesbasic_clearfix">
            ...
            <?php $counter = 0;
              foreach($this->albumTags as $tag):
                if($tag->getTag()->text != ''){?> 
               <a href='<?php echo $this->url(array('module' => 'sesalbum','action'=>'browse'), 'sesalbum_general', true) ?>?tag_id=<?php echo $tag->getTag()->tag_id; ?>/&tag_name=<?php echo $tag->getTag()->text; ?>' ><?php echo $tag->getTag()->text ?></a><?php if((count($this->albumTags) - 1) != $counter ) { echo ",&nbsp;"; } ?>
...
sesJqueryObject(document).on('click','a[id^="sesalbum_profile_upload_existing_photos_"]',function(event){
	...
	(new Request.HTML({
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
				...
				sesJqueryObject('#coverChangeSesalbum').html(en4.core.language.translate('Change Cover Photo'));
...
sesJqueryObject('#coverRemoveSesalbum').click(function(){
		...
		var jqXHR=sesJqueryObject.ajax({
			...
			success: function(response){
				sesJqueryObject('#coverChangeSesalbum').html(en4.core.language.translate('Add Cover Photo'));
				//silence
			 }
			}); 
});
...
<?php if(!$this->is_ajax && !$this->is_related){ ?>
		sesJqueryObject(document).on('click','#tab_links_cover > li',function(){
			...
			for(i=0;i<elemLength.length;i++){
				...
				sesJqueryObject('.'+sesJqueryObject(this).attr('data-src')).css('display','flex');
...
<?php if($this->viewer->getIdentity() !=0){ ?>
	sesJqueryObject(document).on('keyup', function (e) {
		...
		if(sesJqueryObject('#ses_media_lightbox_container').css('display') == 'none'){
			// like code
			if (e.keyCode === 76) {
				if(sesJqueryObject('#sesLikeUnlikeButtonSesalbum').length > 0)
				 sesJqueryObject('#sesLikeUnlikeButtonSesalbum').trigger('click');
			}
			// favourite code
			if (e.keyCode === 70) {
