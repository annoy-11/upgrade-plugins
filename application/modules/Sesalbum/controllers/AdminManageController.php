...
class Sesalbum_AdminManageController extends Core_Controller_Action_Admin
{
  public function indexAction() {
    ...
    if( isset($_GET['is_featured']) && $_GET['is_featured'] != '') 
      $select->where($tableAlbumName.'.is_featured = ?', $values['is_featured']);
    
    if( isset($_GET['is_sponsored']) && $_GET['is_sponsored'] != '') 
      $select->where($tableAlbumName.'.is_sponsored = ?', $values['is_sponsored'] );
    ...
    if( !empty($values['creation_date']) ) 
      $select->where('date('.$tableAlbumName.'.creation_date) = ?', $values['creation_date'] );
		 ...
		 if( isset($_GET['location']) && $_GET['location'] != '') {
             if(  $_GET['location'] == 1) {
                 $select->where($tableAlbumName . '.location != ?', '');
             }else{
                 $select->where($tableAlbumName . '.location = ?', '');
             }
         }
      ...
      if ($_GET['rating'] == 1):
        $select->where($tableAlbumName.'.rating != ?', 0);
