...
class Sesalbum_AlbumController extends Core_Controller_Action_Standard {
  ...
  function favAction() {
    ...
    if (count($Fav) > 0) {
      ...
      Engine_Api::_()->getDbtable('notifications', 'activity')->delete(array('type =?' => "sesalbum_album_favourite", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $albumObj->getType(), "object_id = ?" => $albumObj->getIdentity()));
      Engine_Api::_()->getDbtable('actions', 'activity')->delete(array('type =?' => "sesalbum_album_favourite", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $albumObj->getType(), "object_id = ?" => $albumObj->getIdentity()));
      ...
      if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')->delete(array('type =?' => "sesalbum_album_favourite", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $subject->getType(), "object_id = ?" => $subject->getIdentity()));
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $subject, 'sesalbum_album_favourite');
        $result = $activityTable->fetchRow(array('type =?' => "sesalbum_album_favourite", "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $subject->getType(), "object_id = ?" => $subject->getIdentity()));
        if (!$result) {
          $action = $activityTable->addActivity($viewer, $subject, 'sesalbum_album_favourite');
          if ($action)
  ...
  function likeAction() {
    ...
    if (count($Like) > 0) {
      ...
      if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
        ...
        $action = $activityApi->addActivity($viewer, $albumObj, $albumObj->getType() .'_like' , '', array(
          'owner' => $owner->getGuid(),
        ));
        if( $action ) {
          $activityApi->attachActivity($action, $albumObj);
        }
  ...
  public function viewAction() {
    ...
    // Render
		if((Engine_Api::_()->getApi('core', 'sesbasic')->checkAdultContent(array('module'=>'sesalbum')) && $album->adult) || $album->owner_id == $viewer->getIdentity()) {
  ...
  public function editAction() {
    ...
    $form->populate(array(
     ...
     if(empty(Engine_Api::_()->getApi('settings', 'core')->getSetting('enableglocation', 1)) && (Engine_Api::_()->getApi('settings', 'core')->getSetting('sesalbum_enable_location', 1))){
      $albumLocationData = Engine_Api::_()->getDbTable('locations', 'sesbasic')->getLocationData('sesalbum_album', $album->album_id);
     $form->populate(array(
        'city' => $albumLocationData->city,
        'country' => $albumLocationData->country,
        'zip' => $albumLocationData->zip,
        'latValue' => $albumLocationData->lat,
        'lngValue' => $albumLocationData->lng,
        'state' => $albumLocationData->state,
    ));
    }
    //is post
    ...
    try {
      $values = $form->getValues();
    	...
    	//check dependent module sesprofile install or not
			if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock'))  && in_array('sesalbum',$sesprofilelock_enable_module)) {
