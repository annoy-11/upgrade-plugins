...
if (!empty($infoActivityContent)) {
  $db->query('UPDATE engine4_activity_actiontypes set module = "sesalbum" WHERE type = "album_photo_new"');
}
//Album Manage Page
$page_id = $db->select()
...
$db->query('INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `order`) VALUES ("sesalbum_admin_main_integrateothermodule", "sesalbum", "Display Lightbox In Other Plugins", "", \'{"route":"admin_default","module":"sesalbum","controller":"integrateothermodule","action":"index"}\', "sesalbum_admin_main", "", 995);');

$album_table_exist = $db->query('SHOW TABLES LIKE \'engine4_sesalbum_albums\'')->fetch();
if($album_table_exist) {
  $is_locked = $db->query('SHOW COLUMNS FROM engine4_sesalbum_albums LIKE \'is_locked\'')->fetch();
  if (empty($is_locked)) {
    $db->query("ALTER TABLE `engine4_sesalbum_albums` ADD `is_locked` TINYINT(1) NOT NULL DEFAULT '0';");
  }
  $password = $db->query('SHOW COLUMNS FROM engine4_sesalbum_albums LIKE \'password\'')->fetch();
  if (empty($password)) {
    $db->query("ALTER TABLE `engine4_sesalbum_albums` ADD `password` VARCHAR(255) NULL;");
  }
  $adult = $db->query('SHOW COLUMNS FROM engine4_sesalbum_albums LIKE \'adult\'')->fetch();
  if (empty($adult)) {
    $db->query("ALTER TABLE  `engine4_sesalbum_albums` ADD  `adult` TINYINT( 1 ) NOT NULL DEFAULT  '0';");
  }
}
...
("album_like", "sesalbum", \'{item:$subject} liked {item:$owner}\'\'s {item:$object:album}.\', 1, 1, 1, 3, 1, 0),
("album_photo_like", "sesalbum", \'{item:$subject} liked {item:$owner}\'\'s {item:$object:photo}.\', 1, 1, 1, 3, 1, 0);');


$db->query('INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `order`) VALUES ("sesalbum_admin_main_integrateothersmodules", "sesalbum", "Integrate Other Plugins", "", \'{"route":"admin_default","module":"sesalbum","controller":"integrateothersmodules","action":"index"}\', "sesalbum_admin_main", "", 995);
');
$db->query('DROP TABLE IF EXISTS `engine4_sesalbum_integrateothersmodules`;');
$db->query('CREATE TABLE IF NOT EXISTS `engine4_sesalbum_integrateothersmodules` (
...
$db->query('UPDATE `engine4_sesalbum_categories` SET `member_levels` = "1,2,3,4" WHERE `engine4_sesalbum_categories`.`subcat_id` = 0 and  `engine4_sesalbum_categories`.`subsubcat_id` = 0;');

//insert data in album and photo table
$album_table_exist = $db->query('SHOW TABLES LIKE \'engine4_album_albums\'')->fetch();
if($album_table_exist) {
    $db->query("INSERT IGNORE INTO engine4_sesalbum_albums (album_id,title,description,owner_type,owner_id,category_id,creation_date,modified_date,photo_id,view_count,comment_count,like_count,search,type,view_privacy)  SELECT album_id,title,description,owner_type,owner_id,category_id,creation_date,modified_date,photo_id,view_count,comment_count,like_count,search,type,view_privacy FROM engine4_album_albums");
}
$photo_table_exist = $db->query('SHOW TABLES LIKE \'engine4_album_photos\'')->fetch();
if($photo_table_exist) {
    $db->query("INSERT IGNORE INTO engine4_sesalbum_photos (photo_id,album_id,title,description,creation_date,modified_date,`order`,owner_type,owner_id,file_id,view_count,comment_count,like_count)  SELECT photo_id,album_id,title,description,creation_date,modified_date,`order`,owner_type,owner_id,file_id,view_count,comment_count,like_count FROM engine4_album_photos");
}

$db->query('INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `order`) VALUES ("sesalbum_admin_main_managepages", "sesalbum", "Manage Widgetize Page", "", \'{"route":"admin_default","module":"sesalbum","controller":"settings", "action":"manage-widgetize-page"}\', "sesalbum_admin_main", "", 995)
');