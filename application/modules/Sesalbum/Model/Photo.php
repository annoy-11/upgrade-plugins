...
class Sesalbum_Model_Photo extends Core_Model_Item_Abstract
{
   ...
   */
  public function getPhotoUrl($type = null,$data = null,$string = null)
  { 
    $viewer = Engine_Api::_()->user()->getViewer();
    $photo_id = $this->file_id;
    
		if(!Engine_Api::_()->getApi('core', 'sesbasic')->checkAdultContent(array('module'=>'sesalbum')) && $this->getParent()->adult && !$string && $this->owner_id != $viewer->getIdentity()){
			 return Zend_Registry::get('StaticBaseUrl').Engine_Api::_()->getApi('settings', 'core')->getSetting('sesalbum_album_default_adult', 'application/modules/Sesalbum/externals/images/sesalbum_adult.png');	
		}
		
		$cookieData = isset($_COOKIE['sesalbum_lightbox_value']) ? $_COOKIE['sesalbum_lightbox_value'] : '';
		$valid = false;
		if(in_array($this->album_id,explode(',',$cookieData))){
			$valid = true;
		}
		if(!$string && !$valid){
				...
				if ( Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock')) && $this->getParent()->is_locked  && in_array('sesalbum',$sesprofilelock_enable_module) && $this->owner_id != $viewer->getIdentity() && ($viewer->getIdentity() == 0 || $viewer->level_id != 1))
					return Zend_Registry::get('StaticBaseUrl').'application/modules/Sesalbum/externals/images/locked-album.jpg';
		}
    if( !$photo_id ) {
     return Zend_Registry::get('StaticBaseUrl').'application/modules/Sesalbum/externals/images/nophoto_album_thumb_normal.png';
    }
    ...
    $image->open($file)
      ...
      ->destroy();
		...
		if(!$isUploadDirect){
			...
			$watermarkImage = Engine_Api::_()->authorization()->getPermission($viewer->level_id,'album', 'watermark');
				if(is_file($watermarkImage)){
                  ...
                  ->autoRotate()
					...
					$watermarkImageNew = Engine_Api::_()->authorization()->getPermission($viewer->level_id,'album', 'watermarkthumb');
			...
			if($enableWatermark == 1){
			$watermarkImage = Engine_Api::_()->authorization()->getPermission($viewer->level_id,'album', 'watermarkthumb');
				if(is_file($watermarkImage)){
					if(isset($extension))
						$type = $extension;
					else
