...
class Sespaymentapi_Model_Gateway extends Core_Model_Item_Abstract
{
  ...
  public function getPlugin($type = 'ticket')
  {
      ...
      if( null === $this->_plugin ) {
        $class = str_replace('Payment','Sespaymentapi',$this->plugin);
        ...
        $this->_plugin = $plugin;
				...
				if( null === $this->_plugin ) {
				$class = str_replace('Payment','Sespaymentapi',$this->sponsorship);
				Engine_Loader::loadClass($class);
				$plugin = new $class($this);
				if( !($plugin instanceof Engine_Payment_Plugin_Abstract) ) {
					throw new Engine_Exception(sprintf('Payment plugin "%1$s" must ' .
