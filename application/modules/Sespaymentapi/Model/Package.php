...
class Sespaymentapi_Model_Package extends Core_Model_Item_Abstract
{
  ...
  public function getPackageDescription($params = array())
  {
    ...
    $price = $this->price;
    if($params['type'] == 'user') {
      $getCommisionEntry = Engine_Api::_()->getDbTable('commissions', 'sesmembersubscription')->getCommisionEntry(array('from' => $this->price));
      if($getCommisionEntry) {
        if($getCommisionEntry->commission_type == 1) { 
          $commison_amount = ($this->price * $getCommisionEntry->commission_value) / 100;
          $price = $this->price + $commison_amount;
        } else if($getCommisionEntry->commission_type == 2) { 
          $price = $this->price + $getCommisionEntry->commission_value;
        }
      }
    }
    
    $priceStr = $view->locale()->toCurrency($price, $currency);
    ...
    // Plan is recurring
      ...
      if( $this->recurrence == 1 ) { // (Week|Month|Year)ly
        ...
        $str = sprintf($translate->translate('<b>%1$s</b> %2$s'), $priceStr, $typeStr);
        ...
        $str = sprintf($translate->translate('<b>%1$s</b> per %2$s %3$s'), $priceStr,
    ...
    // Plan is one-time
      ...
      $str = sprintf($translate->translate('One-time fee of <b>%1$s</b>'), $priceStr);
    }
    ...
    if( $this->duration > 0 && $this->duration_type != 'forever' ) {
      ...
      $str = sprintf($translate->translate('<b>%1$s</b> for %2$s %3$s'), $str, $this->duration, $typeStr);
    }
