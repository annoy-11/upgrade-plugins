...
class Sespaymentapi_Plugin_Gateway_Refund_PayPal extends Engine_Payment_Plugin_Abstract
{
  ...
  public function getGateway()
  {
    if( null === $this->_gateway ) {
      ...
      $gateway = new $class(array(
        ...
        'currency' => Engine_Api::_()->sespaymentapi()->defaultCurrency(),
      ));
   ...
   * 
	*/
  public function createOrderTransaction($order,$resource,array $params = array()) {

    if($resource->getType() == 'sescrowdfunding_crowdfunding') {
    ...
    // member subscription refund work
    ...
    $this->getGateway()->getLog()->log('ExpressCheckoutDetail: '
      ...
      if( $paymentStatus == 'okay' || ($paymentStatus == 'pending' && $giveBenefit) ) {
        ...
        if($order->source_type == 'sescrowdfunding_userpayrequest') {
          ...
          $orderPayment->save();
				} else if($order->source_type == 'sespaymentapi_refundrequest') {
          // Update order table info
          $orderPayment->gateway_id = 2;
          $orderPayment->gateway_transaction_id = $rdata['PAYMENTINFO'][0]['TRANSACTIONID'];
          $orderPayment->currency_symbol = $rdata['PAYMENTINFO'][0]['CURRENCYCODE'];
        ...
        if($order->source_type == 'sescrowdfunding_userpayrequest') {
          ...
          $tableRemaining = Engine_Api::_()->getDbTable('remainingpayments', 'sesvideosell');
				...
				else if($order->source_type == 'sespaymentapi_userpayrequest') {
          ...
          $select = $tableRemaining->select()->from($tableName)
                                  ->where('user_id =?', $orderPayment->resource_id)
                                  ->where('resource_id =?', $orderPayment->resource_id)
                                  ->where('resource_type =?', $orderPayment->resource_type);
          ...
          if(count($select) > 0) {
            ...
            $tableRemaining->update(array('remaining_payment' => $remainingAmt),array('user_id =?'=>$orderPayment->resource_id, 'resource_id =?' => $orderPayment->resource_id, 'resource_type =?' => $orderPayment->resource_type));
          }
          
          $transaction = Engine_Api::_()->getItem('sespaymentapi_transaction', $orderPayment->transaction_id);
          $transaction->state = 'refund';
          $transaction->save();
          
          $ordersItem = Engine_Api::_()->getItem('sespaymentapi_order', $transaction->order_id);
          $ordersItem->state = 'refund';
          $ordersItem->save();
        }
        
        if($order->source_type == 'sespaymentapi_refundrequest') {
          ...
          if( $orderPayment->state == 'complete' ) {
            ...
            //Notification work
            $resource = Engine_Api::_()->getItem('user', $orderPayment->user_id);
            
            $viewer = Engine_Api::_()->user()->getViewer();
            Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($resource, $viewer, $resource, $resource->getType().'_adminrefundrequestapprove', array());
