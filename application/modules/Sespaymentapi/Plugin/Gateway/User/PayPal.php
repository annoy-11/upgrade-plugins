...
class Sespaymentapi_Plugin_Gateway_User_PayPal extends Engine_Payment_Plugin_Abstract
{
  ...
  public function createOrderTransaction($order,$event,array $params = array()) {
    ...
    if($event->getType() == 'sescrowdfunding_crowdfunding') {
    ...
    // member subscription work
    else if($event->getType() == 'user') {
     $params['driverSpecificParams']['PayPal'] = array(
				'AMT' => @round($order->release_amount, 2),
				'ITEMAMT' => @round($order->release_amount, 2),
				'ITEMS' => array(
      ...
      if( $paymentStatus == 'okay' || ($paymentStatus == 'pending' && $giveBenefit) ) {
        ...
        if($order->source_type == 'sescrowdfunding_userpayrequest') {
          ...
          $select = $tableRemaining->select()->from($tableName)
                                  ->where('user_id =?', $orderPayment->owner_id)
          ...
          $tableRemaining->update(array('remaining_payment' => $remainingAmt),array('user_id =?'=>$orderPayment->owner_id, 'resource_id =?' => $orderPayment->resource_id, 'resource_type =?' => $orderPayment->resource_type));
        }
