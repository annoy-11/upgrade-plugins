 ...
 * @version    $Id: footer-social-icons.tpl 2015-12-12 00:00:00 SocialEngineSolutions $
...
<?php if(count($this->paginator) > 0):?>
  <div class="sesfooter_manage_table">
    <form>
          ...
          <li class="item_label" id="footersocialicons_<?php echo $item->socialicon_id ?>">
            <input type='hidden'  name='order[]' value='<?php echo $item->socialicon_id; ?>'>
              ...
              <?php $link = (preg_match("#https?://#", $item->url) === 0) ? 'http://'.$item->url : $item->url; ?>
				    ...
				    <div style="width:25%;" class="admin_table_centered">
				      <?php echo ( $item->enabled ? $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesfooter', 'controller' => 'manage', 'action' => 'enabled', 'socialicon_id' => $item->socialicon_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sesfooter/externals/images/check.png', '', array('title' => $this->translate('Disable'))), array()) : $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesfooter', 'controller' => 'manage', 'action' => 'enabled', 'socialicon_id' => $item->socialicon_id), $this->htmlImage('application/modules/Sesfooter/externals/images/error.png', '', array('title' => $this->translate('Enable')))) ) ?>
						</div>
						<div style="width:25%;">          
              <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesfooter', 'controller' => 'manage', 'action' => 'edit', 'socialicon_id' => $item->socialicon_id,'type' => 'footer'), $this->translate("Edit"), array('class' => 'smoothbox')) ?>
            </div>
          </li>
        <?php endforeach; ?>
			</ul>
