...
<div class="sesfooter_main sesfooter_clearfix sesfooter_bxs">
  <?php foreach( $this->footerlinks as $item ):  ?>
    <?php if($item->footerlink_id == 1  || $item->footerlink_id == 5 || $item->footerlink_id == 6) continue; ?>
    ...
    <ul>
      <?php foreach( $this->navigation as $item ):
        $attribs = array_diff_key(array_filter($item->toArray()), array_flip(array(
          ...
          'visible', 'label', 'href'))); ?>
        <li><a href="<?php echo $item->getHref(); ?>" class="footer_link"><?php echo $this->translate($item->getLabel()); ?></a>
        </li>  
      <?php endforeach; ?>
    </ul>
  </div>
