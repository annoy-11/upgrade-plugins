 ...
 * @version    $Id: AdminIntegrateothermodulesController.php  2018-12-07 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seslike_AdminIntegrateothermodulesController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('seslike_admin_main', array(), 'seslike_admin_main_integrateothermodule');

    $this->view->enabledModules = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();

    $select = Engine_Api::_()->getDbtable('integrateothersmodules', 'seslike')->select();
  ...
  public function addmoduleAction() {
    ...
    $this->view->form = $form = new Seslike_Form_Admin_AddModules();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      $integrateothersmodulesTable = Engine_Api::_()->getDbtable('integrateothersmodules', 'seslike');
  ...
  public function deleteAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $inttable = Engine_Api::_()->getItem('seslike_integrateothersmodule', $this->_getParam('integrateothersmodule_id'));
        $pageName = "seslike_index_".$this->_getParam('integrateothersmodule_id');
        if (!empty($pageName)) {
          $page_id = $db->select()
        ...
        $integrateothersmodules = Engine_Api::_()->getItem('seslike_integrateothersmodule', $this->_getParam('integrateothersmodule_id'));
  ...
  public function enabledAction() {
    $db = Engine_Db_Table::getDefaultAdapter();
    $db->beginTransaction();
    $content = Engine_Api::_()->getItemTable('seslike_integrateothersmodule')->fetchRow(array('integrateothersmodule_id = ?' => $this->_getParam('integrateothersmodule_id')));
    try {
