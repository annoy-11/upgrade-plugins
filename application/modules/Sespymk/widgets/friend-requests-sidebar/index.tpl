...
<div class="sespymk_requests_sidebar sesbasic_clearfix sesbasic_bxs">
  <?php if (empty($this->viewmore) && $this->linktopage): ?>
    <div class="sespymk_list_more sesbasic_clearfix">
      <a class="floatL" href="friends/requests/"><?php echo $this->translate("Friend Requests") ?></a>
      <?php if( $this->friendRequests->getTotalItemCount() > 0 ): ?>
        <a class="floatR" href="<?php echo $this->url(array('action' => 'index'), 'recent_activity', true) ?>"><?php echo $this->translate("See All") ?></a>
      <?php endif; ?>
    </div>
  <?php endif; ?>
  <?php if( $this->friendRequests->getTotalItemCount() > 0 ): ?>
    <?php if (empty($this->viewmore)): ?>
      <ul class='sespymk_list_sidebar sesbasic_clearfix' id="notifications_main">
    <?php endif; ?>
      <?php foreach( $this->friendRequests as $notification ): ?>
        ...
        ?>
        <li id="user-widget-request-<?php echo $notification->notification_id ?>" value="<?php echo $notification->getIdentity();?>" class="sespymk_user_<?php echo $notification->user_id ?> sespymk_list_sidebar_item sesbasic_clearfix">
          <div class="sespymk_list_sidebar_item_inner">
            ...
            <div class="sespymk_list_item_cont">
              ...
              <div class="sespymk_list_item_btn">
                <div class="sespymk_buttons">
                  <div class="sespymk_add_button">
                    <button type="submit" class="sesbasic_animation" onclick='widget_request_send("confirm", <?php echo $this->string()->escapeJavascript($notification->getSubject()->getIdentity()) ?>, <?php echo $notification->notification_id ?>, event, "<?php echo $tokenName; ?>", "<?php echo $tokenValue; ?>")'><i class="fa fa-user-plus"></i><?php echo $this->translate('Accept Request');?></button>
                  </div>
                </div>
                <a href="javascript:void(0);" onclick='widget_request_send("reject", <?php echo $this->string()->escapeJavascript($notification->getSubject()->getIdentity()) ?>, <?php echo $notification->notification_id ?>, event, "<?php echo $tokenName; ?>", "<?php echo $tokenValue; ?>")'><i class="fa fa-close"></i></a>
              </div>
            </div>
          </div>
        </li>
