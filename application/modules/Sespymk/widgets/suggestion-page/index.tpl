...
<script type="text/javascript">
  function loadMore() {
    ...
    if ($('view_more'))
      $('view_more').style.display = "<?php echo ( $this->peopleyoumayknow->count() == $this->peopleyoumayknow->getCurrentPageNumber() || $this->count == 0 ? 'none' : '' ) ?>";
    ...
    en4.core.request.send(new Request.HTML({
      method: 'post',              
      'url': en4.core.baseUrl + 'widget/index/mod/sespymk/name/suggestion-page',
      'data': {
        format: 'html',
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        document.getElementById('sespymk_user_main').innerHTML = document.getElementById('sespymk_user_main').innerHTML + responseHTML;
...
<?php if( $this->peopleyoumayknow->getTotalItemCount() > 0 ): ?>  
  ...
  <?php else:  ?>
  <?php if (empty($this->viewmore)): ?>
    <ul class='sespymk_list sesbasic_bxs sesbasic_clearfix' id="sespymk_user_main">
  <?php endif; ?>
    <?php foreach( $this->peopleyoumayknow as $notification ): ?>
      <?php $user = Engine_Api::_()->getItem('user', $notification->user_id);?>
        ...
        <div class="sespymk_list_item_inner">
          ...
          <div class="sespymk_list_item_btn rightT">
            <div class="sespymk_buttons">
              <div class="sespymk_add_button">
                <button type="submit" class="sesbasic_animation" onclick='userWidgetRequestSend("add", <?php echo $this->string()->escapeJavascript($user->user_id) ?>, <?php echo $user->user_id ?>, event)'><i class="fa fa-user-plus"></i><?php echo $this->translate('Add Friend');?></button>
              </div>
              <?php if($this->viewer_id && Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('sesmember')) { ?>
                <?php
                  $getFollowResourceStatus = Engine_Api::_()->sesmember()->getFollowResourceStatus($user->user_id);
  ...
  <?php endif; ?>
<?php else:?>
  <div class="tip"><span style="margin:10px 0;"><?php echo $this->translate('There are no friend suggestions.');?></span></div>
<?php endif;?>
