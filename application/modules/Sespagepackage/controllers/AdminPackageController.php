...
class Sespagepackage_AdminPackageController extends Core_Controller_Action_Admin {

  public function indexAction() {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sespage_admin_main', array(), 'sespage_admin_packagesetting');

    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespage_admin_packagesetting', array(), 'sespage_admin_package');
    ...
    // Make form
    $this->view->formFilter = $formFilter = new Sespagepackage_Form_Admin_Package_Filter();
    ...
    // Initialize select
    $table = Engine_Api::_()->getDbtable('packages', 'sespagepackage');
    $select = $table->select();
    ...
    foreach ($paginator as $item) {
      $pageCounts[$item->package_id] = Engine_Api::_()->getDbtable('pages', 'sespage')
              ->select()
              ->from('engine4_sespage_pages', new Zend_Db_Expr('COUNT(*)'))
  ...
  public function settingsAction() {
    ...
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sespage_admin_main', array(), 'sespage_admin_packagesetting');
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespage_admin_packagesetting', array(), 'sespage_admin_subpackagesetting');

    // Make form
    $this->view->form = $form = new Sespagepackage_Form_Admin_Package_Settings();

    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      $values = $form->getValues();
      unset($values['commision']);
      include_once APPLICATION_PATH . "/application/modules/Sespagepackage/controllers/License.php";
      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagepackage.pluginactivated')) {
  ...
  public function createAction() {
    ...
    // Make form
    $this->view->form = $form = new Sespagepackage_Form_Admin_Package_Create(array('customFields' => $customFields));
    ...
    $packageTable = Engine_Api::_()->getDbtable('packages', 'sespagepackage');
    ...
    // Redirect
    return $this->_helper->redirector->gotoRoute(array('module' => 'sespagepackage', 'controller' => 'package'), "admin_default", true);
  }
  ...
  public function editAction() {
    ...
    if (null === ($packageIdentity = $this->_getParam('package_id')) ||
            !($package = Engine_Api::_()->getDbtable('packages', 'sespagepackage')->find($packageIdentity)->current())) {
    ...
    $this->view->customFields = $customFields;
    // Make form
    $this->view->form = $form = new Sespagepackage_Form_Admin_Package_Edit(array('customFields' => $customFields));
  ...
  public function deleteAction() {
    ...
    $this->view->form = $form = new Sespagepackage_Form_Admin_Package_Delete();
    // Check method/data
    if (!$this->getRequest()->isPost()) {
      return;
    }
  ...
  public function manageTransactionAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sespage_admin_packagesetting', array(), 'sespagepackage_admin_main_transaction');

    $this->view->formFilter = $formFilter = new Sespagepackage_Form_Admin_Filter();
  ...
  public function detailAction() {
    ...
    $this->view->item = Engine_Api::_()->getItem('sespage_page', $this->_getParam('page_id'));
  ...
  public function showReceiptAction() {
    ...
    $nama_fail = Engine_Api::_()->getItem('sespagepackage_transaction', $transactionId)->file_path;
    $file = $nama_fail;
    if (file_exists($file)) {
  ...
  public function approveAction() {
    $transactionId = $this->_getParam('transaction_id',0);
    $transaction = Engine_Api::_()->getItem('sespagepackage_transaction', $transactionId);
    $orderPackageId = $transaction->orderspackage_id;
    $package = Engine_Api::_()->getItem('sespagepackage_package',$transaction->package_id);
    $orderPackage = Engine_Api::_()->getItem('sespagepackage_orderspackage', $orderPackageId);
    $pageTable = Engine_Api::_()->getDbTable('pages','sespage');
    $pageId = $pageTable->select()->from($pageTable->info('name'),'page_id')->where('transaction_id =?',$transactionId)->where('orderspackage_id =?',$orderPackageId)->query()->fetchColumn();
    ...
    if (isset($_SERVER['HTTP_REFERER']))
      ...
      $url = 'admin/sespagepackage/manage-transaction';
    $this->_redirect($url);
  }
