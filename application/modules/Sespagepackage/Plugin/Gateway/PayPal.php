...
class Sespagepackage_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function getGateway() {
    if (null === $this->_gateway) {
      ...
      $gateway = new $class(array(
          ...
          'currency' => Engine_Api::_()->sespagepackage()->getCurrentCurrency(),
      ));
  ...
  public function createPageTransaction(User_Model_User $user, Zend_Db_Table_Row_Abstract $item, Sespagepackage_Model_Package $package, array $params = array()) {
    // Process description
          ...
          if ($package->isOneTime()) {
            switch ($rawData['payment_status']) {
              ...
              case 'Pending':
                ...
                // Get benefit setting
                $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sespagepackage')->getBenefitStatus($user);
                if ($giveBenefit) {
                  $item->onPaymentSuccess();
                } else {
                  $item->onPaymentPending();
  ...
  public function cancelPage($transactionId, $note = null) {
    $profileId = null;
    if ($transactionId instanceof Sespagepackage_Model_Transaction) {
