...
?>
<div class="sespage_package_details sesbasic_bxs">
  ...
  <?php }else{ ?>
    ...
    <?php if($package->isOneTime()){
      if($isrenew){
        if(!empty($transaction->expiration_date) && $transaction->expiration_date != '3000-00-00 00:00:00'){
          ...
          <div class="sesbasic_clearfix"><a href="<?php echo $this->url(array('page_id' => $item->page_id,'action'=>'index'), 'sespagepackage_payment', true); ?>" class="sespage_payment_btn sesbasic_animation"><i class="fa fa-paypal"></i><span><?php echo $this->translate("Reniew Page Payment"); ?></span></a></div>
         <?php 	
          }
        }else{ ?>
    ...
    <?php }else{  ?>
      <div>
        <a href="<?php echo $this->url(array('page_id' => $item->page_id,'action'=>'index'), 'sespagepackage_payment', true); ?>" class="sespage_payment_btn sesbasic_animation"><i class="fa fa-money"></i><span><?php echo $this->translate("Make Payment"); ?></span></a>
      </div>
    <?php } ?>
  <?php } ?>
</div>  