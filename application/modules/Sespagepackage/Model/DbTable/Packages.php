...
class Sespagepackage_Model_DbTable_Packages extends Engine_Db_Table {

  protected $_rowClass = 'Sespagepackage_Model_Package';
  ...
  public function cancelSubscription($params = array()) {
    ...
    $db->query("DELETE FROM engine4_sespagepackage_orderspackages WHERE package_id = $packageId && owner_id = " . $viewerId);
    $db->query("DELETE FROM engine4_sespagepackage_transactions WHERE package_id = $packageId && owner_id = " . $viewerId);
    ...
    $transactionTable = Engine_Api::_()->getDbTable('transactions', 'sespagepackage');
