...
<?php if(count($this->package)){ ?>
    ...
    <div class="sespage_packages_table_container">
      <ul class="sespage_packages_table">
        <?php $existing = 0;?>
      	<?php foreach($this->package as $package)	{
              $enableModules = json_decode($package->params,true);
       	?>
         <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagepackage.package.style', 1)):?>
           <?php include APPLICATION_PATH .  '/application/modules/Sespagepackage/views/scripts/_packages.tpl';?> 
         <?php else:?>
           <?php include APPLICATION_PATH .  '/application/modules/Sespagepackage/views/scripts/_packagesHorizontal.tpl';?>
         <?php endif;?>
      	<?php } ?>
