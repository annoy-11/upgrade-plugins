 ...
 * @version    $Id: prayer-popup.tpl  2017-12-12 00:00:00 SocialEngineSolutions $
...
<div class="sesprayer_item_view_popup sesbasic_bxs">
          ...
          <span>
            ...
            <span>
              ...
              <?php if($item->subcat_id) { ?>
                &nbsp;&raquo;
                <?php $subcategory = Engine_Api::_()->getItem('sesprayer_category', $item->subcat_id); ?>
                <a href="<?php echo $this->url(array('action' => 'index'), 'sesprayer_general').'?category_id='.$item->category_id.'&subcat_id='.$item->subcat_id; ?>"><?php echo $subcategory->category_name; ?></a>
              <?php } ?>
              <?php if($item->subsubcat_id) { ?>
                &nbsp;&raquo;
                <?php $subsubcategory = Engine_Api::_()->getItem('sesprayer_category', $item->subsubcat_id); ?>
                <a href="<?php echo $this->url(array('action' => 'index'), 'sesprayer_general').'?category_id='.$item->category_id.'&subcat_id='.$item->subcat_id.'&subsubcat_id='.$item->subsubcat_id; ?>"><?php echo $subsubcategory->category_name; ?></a>
              <?php } ?>
            </span>
          </span>
        <?php } ?>
          ...
          <div class="_options">
            ...
            <div class="_options_pulldown">
              <?php if($this->viewer_id && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesprayer.allowshare', 1)):?>
                <a href="javascript:void(0);" class="sesbasic_icon_share" onclick="sesprayerSmoothBox('<?php echo $this->escape($this->url(array("module" => "activity","controller" => "index","action" => "share", "type" => $item->getType(), "id" => $item->getIdentity(), "format" => "smoothbox"), 'default', true)); ?>'); return false;"><?php echo $this->translate('Share');?></a>
              <?php endif;?>
              <?php if($this->viewer_id && $this->viewer_id != $item->owner_id && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesprayer.allowreport', 1)):?>
              ...
              <?php endif;?>
                ...
                <?php echo $this->htmlLink(array('route' => 'sesprayer_specific', 'action' => 'edit', 'prayer_id' => $item->getIdentity()), $this->translate('Edit Prayer'), array(
                  'class' => 'buttonlink sessmoothbox sesbasic_icon_edit'
                )) ?>
                <a href="javascript:void(0);" class="buttonlink sesbasic_icon_delete" onclick="sesprayerSmoothBox('<?php echo $this->escape($this->url(array("action" => "delete", 'prayer_id'=> $item->getIdentity(), "format" => "smoothbox"), 'sesprayer_specific', true)); ?>'); return false;"><?php echo $this->translate('Delete');?></a>
              <?php endif;?>
            </div>
          </div>
        <?php } ?>
  ...
  <div class="_cont sesprayer_feed_prayer">
  ...
  <div class="_social sesprayer_social_btns sesbasic_clearfix">
  ...
  <?php } else { ?>
    <div class="_comments">
      <?php echo $this->action("list", "comment", "core", array("type" => "sesprayer_prayer", "id" => $item->getIdentity())); ?>
    </div>
  <?php } ?>
</div>
<script type="text/javascript">
	...
	function sesprayerSmoothBox(url) {
  Smoothbox.open(url);
