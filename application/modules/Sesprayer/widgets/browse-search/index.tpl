...
<script>
  ...
  function showSubCategory(cat_id,selected) {
    ...
    new Request.HTML({
      url: en4.core.baseUrl + 'sesprayer/category/subcategory/category_id/' + cat_id,
      data: {
        'selected':selected
      },
  ...
  function showSubSubCategory(cat_id,selected) {
    ...
    (new Request.HTML({
      url: en4.core.baseUrl + 'sesprayer/category/subsubcategory/subcategory_id/' + cat_id,
      data: {
      'selected':selected
      },
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
...
<?php //if($actionName == 'browse') { ?>
  ...
  en4.core.runonce.add(function() {
    sesJqueryObject(document).on('submit','#filter_form',function(e) {
        e.preventDefault();
        sesJqueryObject('#loadingimgsesprayer-wrapper').show();
...
<?php //} ?>
<?php if( $this->form ): ?>
	<div class="sesprayer_browse_search sesbasic_bxs sesbasic_clearfix <?php if($this->viewType == 'horizontal') { ?> sesprayer_browse_search_horrizontal<?php } ?>">
