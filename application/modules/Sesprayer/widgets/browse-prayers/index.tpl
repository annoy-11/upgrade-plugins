...
<script type="text/javascript">
  ...
  function loadMorePRAYER() {
  
    if ($('prayer_view_more'))
      $('prayer_view_more').style.display = "<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() || $this->count == 0 ? 'none' : '' ) ?>";

    if(document.getElementById('prayer_view_more'))
      document.getElementById('prayer_view_more').style.display = 'none';
    
    if(document.getElementById('prayer_loading_image'))
     document.getElementById('prayer_loading_image').style.display = 'block';

    en4.core.request.send(new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/sesprayer/name/browse-prayers',
      'data': {
        format: 'html',
        page: page,
        viewmore: 1,
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        ...
        sesJqueryObject('#loadingimgsesprayer-wrapper').hide();
        ...
        if(document.getElementById('prayer_view_more'))
          document.getElementById('prayer_view_more').destroy();
        if(document.getElementById('prayer_view_more'))
          document.getElementById('prayer_view_more').style.display = 'block';
        if(document.getElementById('prayer_loading_image'))
          document.getElementById('prayer_loading_image').destroy();
        if(document.getElementById('prayer_loading_image'))
         document.getElementById('prayer_loading_image').style.display = 'none';
        if(document.getElementById('prayer_loadmore_list'))
          document.getElementById('prayer_loadmore_list').destroy();
      }
    }));
    return false;
  }
...
<?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
  ...
  <?php if ($allParams['pagging'] == 'button' && !empty($this->paginator) && $this->paginator->count() > 1): ?>
    <?php if ($this->paginator->getCurrentPageNumber() < $this->paginator->count()): ?>
      <div class="clr" id="prayer_loadmore_list"></div>
      <div class="sesbasic_load_btn" id="prayer_view_more" onclick="loadMorePRAYER();" style="display: block;">
      ...
      <div class="sesbasic_load_btn" id="prayer_loading_image" style="display: none;">
...
<?php elseif( $this->category || $this->show == 2 || $this->search ): ?>
  <div class="sesbasic_tip">
    ...
    <span>
      <?php echo $this->translate('Nobody has written a prayer entry with that criteria.');?>
      <?php if (TRUE): // @todo check if user is allowed to create a poll ?>
        <?php echo $this->translate('Be the first to %1$swrite%2$s one!', '<a class="smoothbox" href="'.$this->url(array('action' => 'create'), 'sesprayer_general').'">', '</a>'); ?>
      <?php endif; ?>
    </span>
  </div>
<?php else:?>
  <div class="sesbasic_tip">
    <img src="<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('sesprayer_prayer_no_photo', 'application/modules/Sesprayer/externals/images/prayer-icon.png'); ?>" alt="" />
    <span>
      <?php echo $this->translate('Nobody has written a prayer entry yet.'); ?>
