...
<?php if($this->allParams['viewType'] == 'list') { ?>
  <ul class="sesprayers_sidebar_listing sesbasic_bxs">
    <?php foreach( $this->prayers as $item ): ?>
      <?php $item = Engine_Api::_()->getItem('sesprayer_prayer', $item->resource_id); ?>
...
<?php } else { ?>
  ...
  <ul class="sesprayers_other_listing prelative sesprayers_listing sesbasic_bxs sesbasic_pinboard_<?php echo $randonNumber ; ?>" style="min-height:50px;" id="widget_<?php echo $randonNumber; ?>" >
    <?php foreach( $this->prayers as $item ): ?>
      <?php $item = Engine_Api::_()->getItem('sesprayer_prayer', $item->resource_id); ?>
      <li class="sesbasic_clearfix sesprayers_list_item newwidget_image_pinboard_<?php echo $randonNumber; ?>">
        <section>
