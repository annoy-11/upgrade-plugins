...
class Sesprayer_AdminCategoriesController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    if (isset($_POST['selectDeleted']) && $_POST['selectDeleted']) {
      if (isset($_POST['data']) && is_array($_POST['data'])) {
        ...
        foreach ($_POST['data'] as $key => $valueSelectedcategory) {
          $categoryDelete = Engine_Api::_()->getItem('sesprayer_category', $valueSelectedcategory);
          $deleteCategory = Engine_Api::_()->getDbtable('categories', 'sesprayer')->deleteCategory($categoryDelete);
          if ($deleteCategory) {
            $deleteCategoryIds[] = $categoryDelete->category_id;
            //delete all category level
            $prayerTable->update(array('category_id' => 0), array('category_id = ?' => $categoryDelete->category_id));
            $prayerTable->update(array('subcat_id' => 0), array('subcat_id = ?' => $categoryDelete->category_id));
            $prayerTable->update(array('subsubcat_id' => 0), array('subsubcat_id = ?' => $categoryDelete->category_id));
    ...
    if (isset($_POST['is_ajax']) && $_POST['is_ajax'] == 1) {
      ...
      $slugExists = Engine_Api::_()->getDbtable('categories', 'sesprayer')->slugExists($value['slug']);
      if (!$slugExists) {
      ...
      if ($cat_id != -1) {
        $categoryData = Engine_Api::_()->getItem('sesprayer_category', $cat_id);
        if ($categoryData->subcat_id == 0) {
          $value['subcat_id'] = $cat_id;
          $seprator = '&nbsp;&nbsp;&nbsp;';
          $tableSeprator = '-&nbsp;';
          $parentId = $cat_id;
          $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesprayer')->orderNext(array('subcat_id' => $cat_id));
        } else {
          $value['subsubcat_id'] = $cat_id;
          $seprator = '3';
          $tableSeprator = '--&nbsp;';
          $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesprayer')->orderNext(array('subsubcat_id' => $cat_id));
          $parentId = $cat_id;
        }
      } else {
        $parentId = 0;
        $seprator = '';
        $value['order'] = Engine_Api::_()->getDbtable('categories', 'sesprayer')->orderNext(array('category_id' => true));
        $tableSeprator = '';
      }
      ...
      try {
        ...
        $categoriesTable = Engine_Api::_()->getDbtable('categories', 'sesprayer');
  ...
  public function changeOrderAction() {
    ...
    if ($this->_getParam('id', false) || $this->_getParam('nextid', false)) {
      ...
      $categoryTypeId = '';
      $checkTypeCategory = $dbObject->query("SELECT * FROM engine4_sesprayer_categories WHERE category_id = " . $category_id)->fetchAll();
      ...
      if ($checkTypeCategory)
        $currentOrder = Engine_Api::_()->getDbtable('categories', 'sesprayer')->order($categoryType, $categoryTypeId);
      ...
      $categoryTable = Engine_Api::_()->getDbtable('categories', 'sesprayer');
      ...
      $checkCategoryChildrenCondition = $dbObject->query("SELECT * FROM engine4_sesprayer_categories WHERE subcat_id = '" . $id . "' || subsubcat_id = '" . $id . "' || subcat_id = '" . $nextid . "' || subsubcat_id = '" . $nextid . "'")->fetchAll();
  ...
  public function editCategoryAction() {
    ...
    $this->view->form = $form = new Sesprayer_Form_Admin_Category_Edit();

    $cat_id = $this->_getParam('id');

    $category = Engine_Api::_()->getItem('sesprayer_category', $cat_id);
    ...
    if (!$form->isValid($this->getRequest()->getPost())) {
      ...
      if (isset($_POST['slug'])) {
        $slugExists = Engine_Api::_()->getDbtable('categories', 'sesprayer')->slugExists($value['slug'], $cat_id);
        if (!$slugExists) {
    ...
    return $this->_helper->redirector->gotoRoute(array('module' => 'sesprayer', 'action' => 'index', 'controller' => 'categories'), 'admin_default', true);
  }
  ...
  protected function setPhoto($photo, $cat_id, $resize = false) {
    ...
    $params = array(
      'parent_type' => 'sesprayer_category',
      'parent_id' => $cat_id,
    ...
    try {
      ...
      if ($e->getCode() == Storage_Model_DbTable_Files::SPACE_LIMIT_REACHED_CODE) {
        throw new Sesprayer_Model_Exception($e->getMessage(), $e->getCode());
      } else {
        throw $e;
      }
    }
