sesJqueryObject(document).on('click','.sesprayer_likefavfollow',function(){
	sesprayer_likefavourite_data(this,'sesprayer_likefavfollow');
});

sesJqueryObject(document).on('submit', '#sesprayers_create', function(e) {
  e.preventDefault();
  addPrayer(this);
});
...
function validatePrayerForm() {
  ...
  sesJqueryObject('.sesprayer_create_form input, .sesprayer_create_form select, .sesprayer_create_form checkbox, .sesprayer_create_form textarea, .sesprayer_create_form radio').each(
    function(index){
...
function sesprayer_likefavourite_data(element) {
    ...
    if(clickType == 'like_entry_view') {
      ...
      functionName = 'like';
      itemType = 'sesprayer_participant';
      var contentId = sesJqueryObject(element).attr('data-url');
      var elementId = '.sesprayer_entry_like_'+contentId;
      ...
      functionName = 'like';
      itemType = 'sesprayer_prayer';
      var contentId = sesJqueryObject(element).attr('data-url');
      var elementId = '.sesprayer_like_'+contentId;
      if(sesJqueryObject(elementId).hasClass('button_active')) {
        sesJqueryObject(elementId).find('span').html(parseInt(sesJqueryObject(elementId).find('span').html())-1);
      }
      else {
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'sesprayer/index/' + functionName,
      'data': {
            format: 'html',
            id: contentId,
            type: itemType,
        ...
        if (response.error)
          alert(en4.core.language.translate('Something went wrong,please try again later'));
              ...
              if(canIntegrate == 1 && response.vote_status) {
                sesJqueryObject('#sesprayer_vote_button_'+contentId).html('<i class="fa fa-hand-o-up"></i><span>Voted</span>');
                sesJqueryObject('#sesprayer_vote_button_'+contentId).addClass('disable');
              }
        return true;
      }
    })).send();
