 ...
 */
class Sesprayer_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Sesprayer_Model_Category';
  ...
  public function getCategory($params = array(), $customParams = array()) {
    ...
    $prayerTable = Engine_Api::_()->getDbTable('prayers', 'sesprayer')->info('name');
    ...
    $category_select = $this->select()
            ...
            ->joinLeft($prayerTable, "$prayerTable.category_id=$tableName.category_id", array("total_prayer_categories" => "COUNT($prayerTable.prayer_id)"))
            ->group("$tableName.category_id");

      
    if(isset($params['criteria']) && $params['criteria'] == 'most_prayer')
      $category_select->order('total_prayer_categories DESC');
    ...
    if(isset($params['hasPrayer'])) {
      $category_select = $category_select->having('total_prayer_categories > 0');
    }
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('sesprayer_category');
...
//     }
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countPrayers'])) {
      $prayerTable = Engine_Api::_()->getDbTable('prayers', 'sesprayer')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($prayerTable, "$prayerTable.subcat_id=$tableName.category_id", array("total_prayers_categories" => "COUNT($prayerTable.photo_id)"));
      $category_select = $category_select->group("$tableName.category_id");
      $category_select->order('total_prayers_categories DESC');
    }
    $category_select = $category_select->order('order DESC');
    return $this->fetchAll($category_select);
  }
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countPrayers'])) {
      $prayerTable = Engine_Api::_()->getDbTable('prayers', 'sesprayer')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($prayerTable, "$prayerTable.subsubcat_id=$tableName.category_id", array("total_prayers_categories" => "COUNT($prayerTable.prayer_id)"));
