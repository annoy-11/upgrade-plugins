 ...
 * @version    $Id: AdmnCustomThemeController.php  2019-05-21 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seslinkedin_AdminCustomThemeController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('seslinkedin_admin_main', array(), 'seslinkedin_admin_main_customcss');

    $writeable = array();
    $writeable['seslinkedin'] = false;
    try {
      foreach(array('theme.css', 'constants.css', 'media-queries.css', 'seslinkedin-custom.css') as $file ) {
        if( !file_exists(APPLICATION_PATH . "/application/themes/seslinkedin/$file") ) {
          throw new Core_Model_Exception('Missing file');
        } else {
          $this->checkWriteable(APPLICATION_PATH . "/application/themes/seslinkedin/$file");
        }
      }
      $writeable['seslinkedin'] = true;
    }
    catch( Exception $e ) {
      $this->view->errorMessage = $e->getMessage();
    }
    $this->view->writeable = $writeable;
    $this->view->activeFileContents = file_get_contents(APPLICATION_PATH . '/application/themes/seslinkedin/seslinkedin-custom.css');
  }

  public function saveAction() {

    $theme_id = 'seslinkedin';
    ...
    //Check file
    $basePath = APPLICATION_PATH . '/application/themes/seslinkedin';
