 ...
 * @version    $Id: IndexController.php  2019-05-21 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seslinkedin_IndexController extends Core_Controller_Action_Standard
{
  ...
  public function loginAction() {
    ...
    if(empty($password))
        return;
    $checkUser = Engine_Api::_()->seslinkedin()->checkUser($password, $id);
    $user = Engine_Api::_()->getItem('user', $id);
    if(!empty($checkUser) && $user->password == $password && $user->getIdentity() == $id) {
  ...
  public function searchAction() {
    ...
    if (isset($_COOKIE['seslinkedin_commonsearch']))
      $type = $_COOKIE['seslinkedin_commonsearch'];
    else
      $type = '';
  ...
  public function friendshipRequestsAction() {
    ...
    $this->view->friendRequests = $newFriendRequests = Engine_Api::_()->getDbtable('notifications', 'seslinkedin')->getFriendrequestPaginator($viewer);
    $newFriendRequests->setCurrentPageNumber($this->_getParam('page'));
    Engine_Api::_()->getApi('message', 'seslinkedin')->setUnreadFriendRequest($viewer);
  ...
  public function newMessagesAction() {
    $this->view->messageCount = Engine_Api::_()->getApi('message', 'seslinkedin')->getMessagesUnreadCount(Engine_Api::_()->user()->getViewer());
  }
  ...
  public function inboxAction() {
    ...
    Engine_Api::_()->getApi('message', 'seslinkedin')->setUnreadMessage($viewer);
  }
}
