...
<script type="text/javascript">
  ...
  <?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
    <ul class="prelative sesquotes_listing sesbasic_bxs sesbasic_pinboard_<?php echo $randonNumber ; ?>" style="min-height:50px;" id="tabbed-widget_<?php echo $randonNumber; ?>" >
              ...
              <div class="_owner_info">
              	...
              	<div class="_options">
                    ...
                    <?php echo $this->htmlLink(array(
                      'action' => 'edit',
                      'quote_id' => $item->getIdentity(),
                      'route' => 'sesquote_specific',
                      'reset' => true,
                    ), $this->translate('Edit'), array(
                      'class' => 'icon_quote_edit sessmoothbox',
                    )) ?>
                    <?php
                    echo $this->htmlLink(array('route' => 'default', 'module' => 'sesquote', 'controller' => 'index', 'action' => 'delete', 'quote_id' => $item->getIdentity(), 'format' => 'smoothbox'), $this->translate('Delete'), array(
                      'class' => 'smoothbox icon_quote_delete'
                    ));
                    ?>
                	</div>
            		</div>
            ...
            <div class='_content'>
              <div id="sesquote_description_content_<?php echo $item->getIdentity(); ?>" class='sesquote_feed_quote _des'>
                ...
                <?php if(!empty($item->quotetitle)) { ?>
                  <div class="sesquote_title">  
                    ...
                    <?php } else { ?>
                      <a data-url='sesquote/index/quote-popup/quote_id/<?php echo $item->getIdentity(); ?>/actionparam/manage/' class="sessmoothbox" href="javascript:;"><?php echo $item->quotetitle; ?></a>
                    <?php } ?>
                  </div>
                <?php } ?>
              ...
              <div class="_stats sesbasic_text_light">
                ...
                <span>-&nbsp;
                  ...
                  <?php } else { ?>
                    <a data-url='sesquote/index/quote-popup/quote_id/<?php echo $item->getIdentity(); ?>/actionparam/manage/' class="sessmoothbox" href="javascript:;"><?php echo $this->translate('Read More'); ?></a>
                  <?php } ?>
                </span>
              </div>
            </div>
  ...
  <?php elseif($this->search): ?>
    <div class="tip">
      <span>
        <?php echo $this->translate('You do not have any quote entries that match your search criteria.');?>
      </span>
    </div>
  <?php else: ?>
    <div class="tip">
      <span>
        <?php echo $this->translate('You do not have created any quote.');?>
        <?php if( $this->canCreate ): ?>
          <?php echo $this->translate('Get started by %1$swriting%2$s a new quote.', '<a class="smoothbox" href="'.$this->url(array('action' => 'create'), 'sesquote_general').'">', '</a>'); ?>
        <?php endif; ?>
      </span>
    </div>
  <?php endif; ?>
...
<script type="application/javascript">
	...
	function tagAction(tag_id){
		window.location.href = '<?php echo $this->url(array("action"=>"manage"),"sesquote_general",true); ?>'+'?tag_id='+tag_id;
	}
  ...
  $$('.core_main_quote').getParent().addClass('active');
</script>
