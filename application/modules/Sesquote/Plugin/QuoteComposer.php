 ...
 * @version    $Id: QuoteComposer.php  2017-12-12 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesquote_Plugin_QuoteComposer extends Core_Plugin_Abstract
{
  public function onAttachQuote($data,$location = '',$postData)
  {
    ...
    try {
      ...
      $quote->owner_id = $viewer->getIdentity();
      $quote->owner_type = $viewer->getType();
      $quote->title = $postData['quote-description'];
      $quote->source = $postData['quote-source'];
      $quote->quotetitle = $postData['quote-title'];
      if($postData['category_id'])
        $quote->category_id = $postData['category_id'];
      if($postData['subcat_id'])
        $quote->subcat_id = $postData['subcat_id'];
      if($postData['subsubcat_id'])
        $quote->subsubcat_id = $postData['subsubcat_id'];
      $quote->save();
      ...
      if($postData['video']) {
        ...
        $quote->mediatype = $postData['mediatype'];
        $quote->code = $information['code'];
        $quote->save();
      }

      if(isset($_FILES['photo']['tmp_name']) && !empty($_FILES['photo']['tmp_name']) ) {
        $photo_id = $this->setPhoto($_FILES['photo'], $quote->quote_id);
        $quote->photo_id = $photo_id;
        $quote->save();
      }
  ...
  public function setPhoto($photo, $quote_id)
  {
    if( $photo instanceof Zend_Form_Element_File ) {
      ...
      throw new Sesquote_Model_Exception('invalid argument passed to setPhoto');
    }
