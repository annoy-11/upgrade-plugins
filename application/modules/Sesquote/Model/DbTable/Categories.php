 ...
 */
class Sesquote_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Sesquote_Model_Category';
  ...
  public function getCategory($params = array(), $customParams = array()) {
    ...
    $quoteTable = Engine_Api::_()->getDbTable('quotes', 'sesquote')->info('name');
    ...
    $category_select = $this->select()
            ...
            ->joinLeft($quoteTable, "$quoteTable.category_id=$tableName.category_id", array("total_quote_categories" => "COUNT($quoteTable.quote_id)"))
            ->group("$tableName.category_id");

      
    if(isset($params['criteria']) && $params['criteria'] == 'most_quote')
      $category_select->order('total_quote_categories DESC');
    ...
    if(isset($params['hasQuote'])) {
      $category_select = $category_select->having('total_quote_categories > 0');
    }
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('sesquote_category');
...
//     }
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countQuotes'])) {
      $quoteTable = Engine_Api::_()->getDbTable('quotes', 'sesquote')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($quoteTable, "$quoteTable.subcat_id=$tableName.category_id", array("total_quotes_categories" => "COUNT($quoteTable.photo_id)"));
      $category_select = $category_select->group("$tableName.category_id");
      $category_select->order('total_quotes_categories DESC');
    }
    $category_select = $category_select->order('order DESC');
    return $this->fetchAll($category_select);
  }
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countQuotes'])) {
      $quoteTable = Engine_Api::_()->getDbTable('quotes', 'sesquote')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($quoteTable, "$quoteTable.subsubcat_id=$tableName.category_id", array("total_quotes_categories" => "COUNT($quoteTable.quote_id)"));
