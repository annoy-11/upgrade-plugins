...
class Sesquote_Model_DbTable_Recentlyviewitems extends Engine_Db_Table {

  protected $_name = 'sesquote_recentlyviewitems';
  protected $_rowClass = 'Sesquote_Model_Recentlyviewitem';

  public function getitem($params = array()) {

    $quoteTableName = Engine_Api::_()->getItemTable('sesquote_quote')->info('name');

    $select = $this->select()
            ->setIntegrityCheck(false)
            ->from($this->info('name'), array('*'))
            ->where('resource_type = ?', 'sesquote_quote')
    ...
    $select->joinLeft($quoteTableName, $quoteTableName . ".quote_id =  " . $this->info('name') . '.resource_id', null);
