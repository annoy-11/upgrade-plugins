...
<script type="text/javascript">
  ...
  en4.core.runonce.add(function() {
    ...
    <?php if( !$this->renderOne ): ?>
      ...
      $('sesquote_profile_previous').style.display = '<?php echo ( $this->paginator->getCurrentPageNumber() == 1 ? 'none' : '' ) ?>';
      $('sesquote_profile_next').style.display = '<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() ? 'none' : '' ) ?>';

      $('sesquote_profile_previous').removeEvents('click').addEvent('click', function() {
      ...
      $('sesquote_profile_next').removeEvents('click').addEvent('click', function(){
        en4.core.request.send(new Request.HTML({
          url : en4.core.baseUrl + 'widget/index/content_id/' + <?php echo sprintf('%d', $this->identity) ?>,
          data : {
            format : 'html',
...
<?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
    ...
    <ul class="prelative sesquotes_listing sesbasic_pinboard_<?php echo $randonNumber ; ?>" style="min-height:50px;" id="tabbed-widget_<?php echo $randonNumber; ?>" >
    ...
    <div>
      <div id="sesquote_profile_previous" class="paginator_previous">
        <?php echo $this->htmlLink('javascript:void(0);', $this->translate('Previous'), array(
          'onclick' => '',
          'class' => 'buttonlink icon_previous'
        )); ?>
      </div>
      <div id="sesquote_profile_next" class="paginator_next">
...
<?php else:?>
  <div class="sesbasic_tip">
    ...
    <span>
      <?php echo $this->translate('There are no quote entry yet.'); ?>
    </span>
  </div>
<?php endif; ?>
