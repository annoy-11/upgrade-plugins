...
<script>
  ...
  function showSubCategory(cat_id,selected) {
    ...
    new Request.HTML({
      url: en4.core.baseUrl + 'sesquote/category/subcategory/category_id/' + cat_id,
      data: {
        'selected':selected
      },
  ...
  function showSubSubCategory(cat_id,selected) {
    ...
    (new Request.HTML({
      url: en4.core.baseUrl + 'sesquote/category/subsubcategory/subcategory_id/' + cat_id,
      data: {
      'selected':selected
      },
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
  ...
  <script type="application/javascript">
      en4.core.runonce.add(function() {
        sesJqueryObject(document).on('submit','#filter_form',function(e) {
            e.preventDefault();
            sesJqueryObject('#loadingimgsesquote-wrapper').show();
...
<?php if( $this->form ): ?>
	<div class="sesquote_browse_search sesbasic_bxs sesbasic_clearfix <?php if($this->viewType == 'horizontal') { ?> sesquote_browse_search_horrizontal<?php } ?>">
