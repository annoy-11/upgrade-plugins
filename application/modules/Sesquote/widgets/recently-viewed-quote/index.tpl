...
<?php if($this->allParams['viewType'] == 'list') { ?>
  <ul class="sesquotes_sidebar_listing sesbasic_bxs">
    <?php foreach( $this->quotes as $item ): ?>
      <?php $item = Engine_Api::_()->getItem('sesquote_quote', $item->resource_id); ?>
      <?php if($item) { ?>
...
<?php } else { ?>
  ...
  <ul class="sesquotes_other_listing prelative sesquotes_listing sesbasic_bxs sesbasic_pinboard_<?php echo $randonNumber ; ?>" style="min-height:50px;" id="widget_<?php echo $randonNumber; ?>" >
    <?php foreach( $this->quotes as $item ): ?>
      <?php $item = Engine_Api::_()->getItem('sesquote_quote', $item->resource_id); ?>
      <?php if($item) { ?>
      <li class="sesbasic_clearfix sesquotes_list_item newwidget_image_pinboard_<?php echo $randonNumber; ?>">
        <section>
