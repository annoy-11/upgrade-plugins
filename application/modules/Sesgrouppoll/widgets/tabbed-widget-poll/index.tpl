...
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesgrouppoll/externals/styles/styles.css'); ?>
...
<?php if($this->params['tabOption'] == '0'){ ?>
  <div class="layout_core_container_tabs" <?php if(count($this->defaultOptions) ==1){ ?> style="border-width:0;" <?php } ?>>
  	<div class="tabs_alt tabs_parent sesgrouppoll_tabs" <?php if(count($this->defaultOptions) ==1){ ?> style="display:none" <?php } ?>>
<?php } ?>
...
<p>
   ...
   <?php } else { ?>
      ...
      <ul id="tab-widget-sesgrouppoll-<?php echo $randonNumber; ?>">
         <?php 
           $defaultOptionArray = array();
           foreach($this->defaultOptions as $key=>$valueOptions){ 
           $defaultOptionArray[] = $key;
...
<?php  } ?>
<?php include APPLICATION_PATH . '/application/modules/Sesgrouppoll/views/scripts/_showPollListGrid.tpl'; ?>
<?php if(!$this->is_ajax){ ?>
    </div>
  </div>
<?php } ?>
...
<?php if(!$this->is_ajax):?>
  <script type="application/javascript"> 
	...
	document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML = '<div class="sesbasic_loading_container" id="loading_image_<?php echo $randonNumber; ?>" style="width:100%;"></div>';
	...
	requestTab_<?php echo $randonNumber; ?> = new Request.HTML({
	  method: 'post',
	  'url': en4.core.baseUrl+"widget/index/mod/sesgrouppoll/name/<?php echo $this->widgetName; ?>/openTab/"+valueTab,
	  'data': {
	    format: 'html',  
	  ...
	  onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
	    ...
	    if(document.getElementById('sesgroup_pinboard_view_<?php echo $randonNumber;?>') && sesJqueryObject('.sesbasic_view_type_options_<?php echo $randonNumber; ?>').find('.active').attr('rel') == 'pinboard') {
	      if(document.getElementById('sesgroup_pinboard_view_<?php echo $randonNumber;?>'))
	      document.getElementById('sesgroup_pinboard_view_<?php echo $randonNumber;?>').style.display = 'block';
