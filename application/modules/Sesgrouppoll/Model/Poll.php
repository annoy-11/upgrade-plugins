 ...
 * @version    $Id: Poll.php  2018-11-02 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesgrouppoll_Model_Poll extends Core_Model_Item_Abstract
{
  ...
  {
    $optionsTable = Engine_Api::_()->getDbTable('options', 'sesgrouppoll');
  ...
  public function getRichContent()
  {
    ...
    $view = clone $view;
    $view->clearVars();
    $view->addScriptPath('application/modules/Sesgrouppoll/views/scripts/');
    ...
    $view->showPieChart = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgrouppoll.showpiechart', false);
    $view->canVote = $this->authorization()->isAllowed(null, 'vote');
    $view->canChangeVote = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgrouppoll.canchangevote', false);
    $view->hideLinks = true;
  ...
  {
    return Engine_Api::_()->getDbtable('options', 'sesgrouppoll')->fetchAll(array(
  ...
  {
    $optionTable = Engine_Api::_()->getDbtable('options', 'sesgrouppoll');
    $data = $optionTable
        ->select()
  ...
  public function vote(User_Model_User $user, $option, $group, $owner,$poll)
  {
$item = $poll;
$viewer = Engine_Api::_()->user()->getViewer();
    $votesTable = Engine_Api::_()->getDbTable('votes', 'sesgrouppoll');
    $optionsTable = Engine_Api::_()->getDbtable('options', 'sesgrouppoll');
	...
	$notificationType = 'sesgrouppoll_vote_poll';
    ...
    if( !$vote ) {
	 ...
	 if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $group, $notificationType);
				$action = Engine_Api::_()->getDbTable('actions', 'activity')->addActivity($viewer, $group, 'vote_sesgrouppoll_poll');
      if( $action != null ) {
  ...
  {
    //     delete poll votes
    Engine_Api::_()->getDbtable('votes', 'sesgrouppoll')->delete(array(
      'poll_id = ?' => $this->getIdentity(),
    ));
    ...
    // delete poll options
    Engine_Api::_()->getDbtable('options', 'sesgrouppoll')->delete(array(
      'poll_id = ?' => $this->getIdentity(),
    ));
