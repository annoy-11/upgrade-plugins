 ...
 * @version    $Id: _composeSesgrouppoll.tpl  2018-11-02 00:00:00 SocialEngineSolutions $
...
$canUpload = Engine_Api::_()->authorization()->getPermission($viewer, 'sesgrouppoll_poll', 'create');

if($module){
$gifCount = count(Engine_Api::_()->getDbTable('images', 'sesfeedgif')->getImages(array('fetchAll' => 1, 'limit' => 10)));
}
if(!$this->subject() || ($this->subject() && $this->subject()->getType() != "sesgroup_group"))
  return;
$max_options = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgrouppoll.maxoptions', 15);
?>
...
sesJqueryObject(document).ready(function(){
  ...
  if(window.location.href.indexOf("messages/compose") > -1 || window.location.href.indexOf("messages/view/id") > -1) {
    var isMessageGroup = true;
  }else{
    var isMessageGroup = false;
  }
 <?php if($allowPoll && $canUpload){ ?>
  <?php $this->headScript()->appendFile($this->layout()->staticBaseUrl .
  'application/modules/Sesgrouppoll/externals/scripts/composer_sesgrouppoll.js'); ?>
  en4.core.runonce.add(function() {
    composeInstance.addPlugin(new Composer.Plugin.Sesgrouppoll({
	...
	<?php } ?>
  function checkValidationGroupPoll(){
  ...
  function sesgrouppollappend(){
    var optionsDiv =  '<div class= "pollOptionInput">' +
      '<input type= "text" name="optionsArray[]" class="option_text" placeholder="option" autofocus >' +
      '<div class="sesgrouppoll_uploadmedia"><span class="sesgrouppoll_add_photo"><a href="javascript:void(0)" title="<?php echo $this->string()->escapeJavascript($this->translate("Attach a Photo")); ?>"><input type= "file" name="optionsImage[]" class="'+text+counter+'"  onchange = "previewFile(this,'+counter+');"></a></span>';
      ...
      optionsDiv +='</div><span class="sesgrouppoll_upload_img"><a href="javascript:void(0)" style = "display:none;" id="remove-image'+counter+'" class="fa fa-close remove-image"></a><img src="#"  class="uploadPreviewImage" style="display:none;" id="'+text+counter+'" ></span>';
      optionsDiv +=  '<span  class="sesgrouppoll_upload_img"><a href="javascript:void(0)" style = "display:none;" id="remove-gif'+counter+'" class="fa fa-close remove-gif"></a><img src="#" class="uploadPreviewGif" style="display:none;" id="'+gif+counter+'"></span>' ;
    if(counter > 1){
