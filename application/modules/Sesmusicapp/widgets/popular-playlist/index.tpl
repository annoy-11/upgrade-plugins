...
<?php if(count($this->results) > 0) :?>
  <ul class="sesmusicapp_playlist_grid_listing sesbasic_clearfix">
    <?php foreach( $this->results as $item ):  ?>
      <li class="sesmusicapp_playlist_grid" style="width:<?php echo $this->width ?>px;">
        <div class="sesmusicapp_playlist_grid_top sesbasic_clearfix">
          <?php echo $this->htmlLink($item->getHref(), $this->itemPhoto($item, 'thumbnail')) ?>
          <div>
					 <?php if (!empty($this->information) && in_array('title', $this->information)): ?>
            <div class="sesmusicapp_playlist_grid_title">
              <?php echo $this->htmlLink($item->getHref(), $item->getTitle()) ?>
            </div>
						<?php endif; ?>
            <?php if(!empty($this->information) && in_array('postedby', $this->information)): ?>
            <div class="sesmusicapp_playlist_grid_stats  sesbasic_text_light">
              <?php echo $this->translate('by');?> <?php echo $this->htmlLink($item->getOwner()->getHref(), $item->getOwner()->getTitle()) ?>     
            </div>
            <?php endif; ?>
            <div class="sesmusicapp_playlist_grid_stats sesmusic_list_stats sesbasic_text_light">
              ...
              <?php if (!empty($this->information) && in_array('songCount', $this->information)): ?>
                <span title="<?php echo $this->translate(array('%s song', '%s song', $item->song_count), $this->locale()->toNumber($item->song_count)); ?>"><i class="fa fa-music"></i><?php echo $item->song_count ; ?></span>
              <?php endif; ?>
            </div>
          </div>
        </div>
        ...
        <div class="clear sesbasic_clearfix sesmusicapp_tracks_container sesbasic_custom_scroll">
          <ul class="clear sesmusicapp_tracks_list">
            
            <?php foreach( $songs as $song ):  ?>
             <?php $song = Engine_Api::_()->getItem('sesmusic_albumsong', $song->albumsong_id); ?>
              <li class="sesbasic_clearfix">
                <div class="sesmusicapp_tracks_list_photo">
                  ...
                  <?php if($song->track_id): ?>
                    ...
                    <a href="javascript:void(0);" onclick="play_music('<?php echo $song->albumsong_id ?>', '<?php echo $URL; ?>', '<?php echo $songTitle; ?>', '', '<?php echo $path; ?>');" class="sesmusicapp_songslist_playbutton"><i class="fa fa-play-circle"></i></a>
                <?php else: ?>
                  <a href="javascript:void(0);" onclick="play_music('<?php echo $song->albumsong_id ?>', '<?php echo $song->getFilePath(); ?>', '<?php echo $songTitle; ?>', '', '<?php echo $path; ?>');" class="sesmusicapp_songslist_playbutton"><i class="fa fa-play-circle"></i></a>
                <?php endif; ?>
                  <?php else: ?>
                    <?php if($song->store_link): ?>
                      ...
                      <a href="javascript:void(0);" onclick="play_music('<?php echo $song->albumsong_id ?>', '<?php echo $song->getFilePath(); ?>', '<?php echo $songTitle; ?>', '<?php echo $storeLink ?>', '<?php echo $path; ?>');" class="sesmusicapp_songslist_playbutton"><i class="fa fa-pplay-circlelay"></i></a>
                    <?php else: ?>
                      <a href="javascript:void(0);" onclick="play_music('<?php echo $song->albumsong_id ?>', '<?php echo $song->getFilePath(); ?>', '<?php echo $songTitle; ?>', '', '<?php echo $path; ?>');" class="sesmusicapp_songslist_playbutton"><i class="fa fa-play-circle"></i></a>
                    <?php endif; ?>
                  <?php endif; ?>
                ...
                <div class="sesmusicapp_tracks_list_name" title="<?php echo $song->getTitle() ?>">
...
<?php elseif($this->showType == 'carouselview'): ?>
  ...
  <div class="slide sesmusicapp_carousel_wrapper clearfix <?php if($this->viewType == 'horizontal'): ?> sesmusic_carousel_h_wrapper <?php else: ?> sesmusic_carousel_v_wrapper <?php endif; ?>">
    <div id="playlistslide_<?php echo $randonNumber; ?>">
    <?php foreach( $this->results as $item ):  ?>
      <div class="sesmusicapp_playlist_grid sesbasic_bxs" style="height:<?php echo $this->height ?>px;width:<?php echo $this->width ?>px;">
        <div class="sesmusicapp_playlist_grid_top sesbasic_clearfix">
          <?php echo $this->htmlLink($item->getHref(), $this->itemPhoto($item, 'thumbnail')) ?>
          <div>
						<?php if (!empty($this->information) && in_array('title', $this->information)): ?>
    ...
    <?php if($this->viewType == 'horizontal'): ?>
      <div class="tabs_<?php echo $randonNumber; ?> sesmusic_carousel_nav _playlist">
        <a class="sesmusic_carousel_nav_pre" href="#page-p"><i class="fa fa-angle-left"></i></a>
        <a class="sesmusic_carousel_nav_nxt" href="#page-p"><i class="fa fa-angle-right"></i></a>
      </div>  
    <?php else: ?>
      <div class="tabs_<?php echo $randonNumber; ?> sesmusic_carousel_nav _playlist">
