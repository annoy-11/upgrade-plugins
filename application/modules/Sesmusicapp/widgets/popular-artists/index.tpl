...
<?php if(count($this->results) > 0) :?>
  <ul class="sesmusic_artist_listing">
    ...
    <?php elseif($this->viewType == 'gridview'): ?>
      <li class="sesmusicapp_artist_grid" style="width:<?php echo $this->width ?>px;">
        <div class="sesmusicapp_artist_grid_inner">
        	<div class="sesmusicapp_artist_grid_bg_image" style="height:<?php echo $this->height ?>px;">
          	<?php if($item->artist_photo): ?>
          <?php $img_path = Engine_Api::_()->storage()->get($item->artist_photo, '')->getPhotoUrl();
          $path = $img_path; 
          ?>
          ...
          <div class="sesmusicapp_item_artwork" style="height:<?php echo $this->height ?>px;">
          <?php if($item->artist_photo): ?>
          <?php $img_path = Engine_Api::_()->storage()->get($item->artist_photo, '')->getPhotoUrl();
          $path = $img_path; 
          ?>
          ...
          <div class="hover_box">
        	...
        	<div class="sesmusicapp_artist_info">
          <?php //if(!empty($this->information) && in_array('title', $this->information)): ?>
            <div class="sesmusicapp_artist_title">
              <?php echo $this->htmlLink(array('module'=>'sesmusic', 'controller'=>'artist', 'action'=>'view', 'route'=>'default', 'artist_id' => $item->artist_id), $item->name); ?>
              <?php //echo $this->htmlLink($item->getHref(), $item->getTitle()) ?>
            </div>
          <?php //endif; ?>
          <div class="sesmusicapp_artist_stats sesbasic_text_light">
            ...
            <?php endif; ?>
            <?php if (!empty($this->information) && in_array('rating', $this->information)) : ?>
              <span class="floatR">
                <?php if( $item->rating > 0 ): ?>
                <?php for( $x=1; $x<= $item->rating; $x++ ): ?>
                <span class="sesbasic_rating_star_small fa fa-star"></span>
