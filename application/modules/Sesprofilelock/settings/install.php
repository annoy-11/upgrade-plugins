 ...
 * @version    $Id: install.php 2016-04-30 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesprofilelock_Installer extends Engine_Package_Installer_Module {
...
//   }
  ...
  function onEnable() {
    ...
    $db = $this->getDb();
    $db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '1' WHERE `engine4_core_menuitems`.`name` = 'sesprofilelock_mini_admin';");
    parent::onEnable();
  }

  public function onDisable() {
    $db = $this->getDb();
    $db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '0' WHERE `engine4_core_menuitems`.`name` = 'sesprofilelock_mini_admin';");
    parent::onDisable();
  }
}
