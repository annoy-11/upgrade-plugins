...
<p>
  <?php echo $this->translate('Use this section to add, manage and configure various navigation menus that appear on your social network. When you select the menu you wish to edit, a list of the menu items it contains will be shown. You can drag these items up and down to change their order. Also you can add new menu by clicking on "Add Item" link below') ?>
</p>
<br />
<div class="admin_menus_options">
  ...
  <?php if(count($this->resultsSink) > 0) { ?>
    <?php echo $this->htmlLink(array('route' => 'admin_default', 'module' => 'sesmenu', 'controller' => 'settings', 'action' => 'sink-menus', 'param' => '1'), $this->translate('Sink Menus'), array('class' => 'buttonlink admin_menus_additem smoothbox')) ?>
  <?php } ?>
</div>
<br />
  ...
  <?php foreach($this->menuItems as $menuItem ): ?>
    <li class="admin_menus_item<?php if( isset($menuItem->enabled) && !$menuItem->enabled ) echo ' disabled' ?>" id="admin_menus_item_<?php echo $menuItem->name ?>">
      <span class="item_wrapper">
		...
		<?php if(isset($menuItem->design_cat) && $menuItem->design_cat == 6): ?>
           <?php echo $this->htmlLink(array('reset' => false, 'action' => 'dashboard-links','controller'=>'manage','menu_id'=>$menuItem->id), $this->translate('custom')) ?> | 
          <?php endif; ?> 
          <?php echo $this->htmlLink(array('reset' => false, 'action' => 'edit', 'name' => $menuItem->name,'id'=>$menuItem->id,'menuName' => $this->selectedMenu->name), $this->translate('edit'), array()) ?>
