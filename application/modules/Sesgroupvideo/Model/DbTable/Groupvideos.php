 ...
 * @version    $Id: Groupvideos.php  2018-10-16 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesgroupvideo_Model_DbTable_Groupvideos extends Engine_Db_Table {

  protected $_rowClass = "Sesgroupvideo_Model_Groupvideo";
  protected $_name = 'sesgroupvideo_videos';
  ...
  public function getVideo($params = array(), $paginator = true) {
    ...
    if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo.enable.watchlater', 1)) {
      $watchLaterTable = Engine_Api::_()->getDbTable('watchlaters', 'sesgroupvideo')->info('name');
      //$select = $select->setIntegrityCheck(false);
      $select = $select->joinLeft($watchLaterTable, '(' . $watchLaterTable . '.video_id = ' . $tableName . '.video_id AND ' . $watchLaterTable . '.owner_id = ' . $user_id . ')', array('watchlater_id'));
    }
		$groupTable = Engine_Api::_()->getDbTable('groups', 'sesgroup')->info('name');
    $select = $select->joinLeft($groupTable, '(' . $groupTable . '.group_id = ' . $tableName . '.parent_id )', array('group_id'));
		$select->where($groupTable.'.group_id !=?','');
		if(!empty($params['search_group']))
			$select->where($groupTable . ".title LIKE ?", $params['search_group'] . '%');
    if (isset($params['lat']) && isset($params['miles']) && $params['miles'] != 0 && isset($params['lng']) && $params['lat'] != '' && $params['lng'] != '' && ((isset($params['location']) && $params['location'] != '') || isset($params['locationWidget']))) {
      ...
      $origLon = $params['lng'];
      //$select = $select->setIntegrityCheck(false);
      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sesgroupvideo.search.type', 1) == 1) {
        $searchType = 3956;
      } else
        $searchType = 6371;
      $dist = $params['miles']; // This is the maximum distance (in miles) away from $origLat, $origLon in which to search
      $select->joinLeft($tableLocationName, $tableLocationName . '.resource_id = ' . $tableName . '.video_id AND ' . $tableLocationName . '.resource_type = "groupvideo" ', array('distance'=> new Zend_Db_Expr($searchType . " * 2 * ASIN(SQRT( POWER(SIN(($origLat - abs(lat))*pi()/180/2),2) + COS($origLat*pi()/180 )*COS(abs(lat)*pi()/180) *POWER(SIN(($origLon-lng)*pi()/180/2),2)))"), 'lat', 'lng'));
    ...
    if (!empty($params['tag'])) {
      $select
              ...
              ->where($tmName . '.resource_type = ?', 'groupvideo')
    ...
    if (!empty($params['parent_id']) && $params['parent_type'] == 'sesgroup_group') {
  ...
  public function peopleAlsoLiked($id = 0) {
    ...
    $select = $this->select()
            ...
            ->where($likesTableName . '.resource_type = ?', 'groupvideo')
            ->where('l2.resource_type = ?', 'groupvideo')
...
public function getFavourite($params = array()){
		$tableFav = Engine_Api::_()->getDbtable('favourites', 'sesgroupvideo');
