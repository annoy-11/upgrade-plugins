...
class Sesgroupvideo_Form_Browsesearch extends Engine_Form {
  ...
  public function init() {
    parent::init();
		...
		if($this->getSearchFor() == 'video')
			$searchFor =$view->url(array('controller' => 'index', 'action' => 'browse','module'=>'sesgroupvideo'), 'sesgroupvideo_general', true);
    $this
            ->setAttribs(array(
                'id' => 'filter_form',
    ...
    if ($this->_searchTitle == 'yes') {
      ...
      $this->addElement('Text', 'search', array(
		...
		if ($this->_searchGroupTitle == 'yes' || 1){
			 $valueD = Zend_Registry::get('Zend_Translate')->_('Group Title:');
      $this->addElement('Text', 'search_group', array(
          'label' => $valueD
      ));
    }
    ...
    $this->addElement('Button', 'submit', array(
		...
		$this->addElement('Dummy','loading-img-sesgroupvideo', array(
        'content' => '<img src="application/modules/Core/externals/images/loading.gif" id="sesgroupvideo-category-widget-img" alt="Loading" />',
   ));
  }
