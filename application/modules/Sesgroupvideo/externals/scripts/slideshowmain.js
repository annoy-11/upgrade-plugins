...
var autoPlayId,firstVideoSrc=false,isOnRegister = false,IsfinishVideoNext = false,clearIntvalSesgroupVideoSlideshow1,clearIntvalSesgroupVideoSlideshow2 ;
jQuery(document).ready(function($){
		...
		sliderNav.on('click', 'li', function(event){
			...
			if(!selectedItem.hasClass('selected')) {
				...
				if(slidesWrapper.find('li.selected').hasClass('cd-bg-video')){
					 ...
					 clearInterval(clearIntvalSesgroupVideoSlideshow1);
					 clearInterval(clearIntvalSesgroupVideoSlideshow1);
				}
			}
		});
	...
	function checkVideo(hiddenSlide, container, n) {
		...
		if( visibleVideo.length > 0 ) { 
			clearInterval(autoPlayId);
			clearInterval(clearIntvalSesgroupVideoSlideshow2);
			clearInterval(clearIntvalSesgroupVideoSlideshow1);
			if(!Modernizr.touch){
