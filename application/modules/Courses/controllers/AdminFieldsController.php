...
class Courses_AdminFieldsController extends Fields_Controller_AdminAbstract {
  protected $_requireProfileType = true;
  protected $_fieldType = 'courses';
  ...
  public function indexAction() {
    ...
    $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_coursecat', array(), 'courses_admin_main_subfields');
    parent::indexAction();
  }
  ...
  public function fieldEditAction() {
    ...
    // Create form
    $this->view->form = $form = new $adminFormClass();
    $form->setTitle('Edit Course Category Profile Question');
    $form->removeElement('show');
    $form->getElement('search')->setLabel('Show on Browse Search Widget?');
    $form->getElement('search')->setMultiOptions(array(
        0 => 'Hide on Browse Search Widget',
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Courses_Form_Admin_Settings_Type();
    if (!$this->getRequest()->isPost()) {
      return;
    }
    if (!$form->isValid($this->getRequest()->getPost())) {
    ...
    if ($form->getValue('duplicate') != 'null') {
      ...
      $field_meta_array = $db->select()
              ->from('engine4_courses_fields_meta')
              ->where('field_id IN (' . implode(', ', $child_id_array) . ')')
              ->query()
              ->fetchAll();
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_courses_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_courses_fields_options', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      Engine_Api::_()->getDbTable('categories', 'courses')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
