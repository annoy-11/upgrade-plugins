 ...
 * @version    $Id: Crosssells.php 2019-08-28 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */
class Courses_Model_DbTable_Crosssells extends Engine_Db_Table {
    ...
    public function getCoursessSelect($params = array(), $customFields = array()) {
        ...
        if (!empty($params['subcat_id']))
            $select->where($courseTableName . '.subcat_id =?', $params['subcat_id']);
        if (!empty($params['subsubcat_id']))
            $select->where($courseTableName . '.subsubcat_id =?', $params['subsubcat_id']);
        ...
        if(isset($params['similar_courses']))
