...
class Courses_Model_DbTable_Parametervalues extends Engine_Db_Table {
  protected $_rowClass = 'Courses_Model_Parametervalue';
	protected $_name  = 'courses_review_parametervalues';
	public function getParameters($params = array()){
		$pTable = Engine_Api::_()->getDbtable('parameters', 'courses');
   ...
   // rating functions
    public function getRating($parameter_id) {
        $rating_sum = $this->select()
                ...
                ->query()
                ->fetchColumn(0);
        $total = $this->ratingCount($parameter_id, $resource_type = 'courses_review');
        if ($total)
