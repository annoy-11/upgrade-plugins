...
class Courses_Form_Search extends Engine_Form {
  ...
  public function init() {
    ...
    $this->setAction($view->url(array('module' => 'courses', 'controller' => 'index', 'action' => 'browse'), 'default', true));
    ...
    if ($this->_friendsSearch == 'yes' && $viewer->getIdentity() != 0) {
        ...
        'label' => 'Show',
            'multiOptions' => array(
                '0' => 'Everyone\'s '.ucwords($this->getSearchFor()).'s',
                '1' => 'Only My Friend\'s '.ucwords($this->getSearchFor()).'s',
                '2' => 'Only My Network '.ucwords($this->getSearchFor()).'s',
            ),
        ));
    }
    ...
    if ($this->_locationSearch == 'yes' && $this->getSearchFor() == 'course') {
      /*Location Elements*/
