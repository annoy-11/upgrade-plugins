...
class Courses_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function getGateway() {
    if (null === $this->_gateway) {
      ...
      $gateway = new $class(array(
          ...
          'currency' => Engine_Api::_()->courses()->getCurrentCurrency(),
      ));
  ...
  public function createOrderTransaction(User_Model_User $user, $courseOrder, array $params = array()) {
    // Process description
    ...
    if ($currentCurrency != $defaultCurrency) {
      ...
      $taxAmount = $shippingCost = $totalprice = $courseTotalPrice = 0;

    //get all orders

      $orderTableName = Engine_Api::_()->getDbTable('orders','courses');
      $select = $orderTableName->select()->where('order_id =?',$courseOrder->getIdentity());
      $order = $orderTableName->fetchRow($select);
      $totalprice = $order->total_amount;
      
      //get all order courses
      $courseTableName = Engine_Api::_()->getDbTable('ordercourses','courses');
      $select = $courseTableName->select()->where('order_id =?', $courseOrder->getIdentity());
  ...
  Payment_Model_Order $order, array $params = array()) {
    ...
    $this->getGateway()->getLog()->log('ExpressCheckoutDetail: '
      ...
      if ($paymentStatus == 'okay' || $paymentStatus == 'active' ||
        ...
        // Payment success
        try{
            Engine_Api::_()->courses()->orderComplete($item,$courses);
        }catch(Exception $e){
      ...
      switch ($rawData['txn_type']) {
        ...
        // — adjustment express_checkout new_case
              ...
              case 'Pending':
                ...
                // Get benefit setting
                $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'courses')->getBenefitStatus($user);
                if ($giveBenefit) {
  ...
  */
  ...
  public function onIpn(Engine_Payment_Ipn $ipn) {
    ...
    if ($order) {
      $ipnProcessed = false;
      // Subscription IPN
      if ($order->source_type == 'courses_order') {
