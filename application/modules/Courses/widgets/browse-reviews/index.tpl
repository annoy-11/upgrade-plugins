...
<?php if( $this->paginator->getTotalItemCount() > 0 ){ ?>
  <?php foreach( $this->paginator as $item ): ?>
    <?php $reviewer = Engine_Api::_()->getItem('courses', $item->course_id);
    	if(!$reviewer)
      	continue;
    ?>
    <li class="sesbasic_clearfix">
    	<div class="courses_review_listing_left">
      	<div class="courses_review_listing_left_photo">
          <?php echo $this->htmlLink($item->getOwner()->getHref(), $this->itemPhoto($item->getOwner(), 'thumb.profile')) ?>
        </div>
        <p class="courses_review_listing_left_title"><?php echo $this->htmlLink($item->getOwner()->getHref(), $item->getOwner()->getTitle()) ?></p>
        <?php if(isset($this->featuredLabelActive) && $item->featured):?>
	  <div class="courses_review_featured_block">
	  <p><?php echo $this->translate('Featured');?></p>
	  </div>
	<?php endif;?>
	<?php if(isset($this->verifiedLabelActive) && $item->verified):?>
	  <div class="courses_review_verified_block">
	    <p><?php echo $this->translate('Verified');?></p>
	  </div>
	<?php endif;?>
      </div>
      <div class="courses_review_listing_right sesbasic_clearfix">
        ...
        <div class="courses_review_listing_top_info sesbasic_clearfix">
          ...
          <div class='courses_review_listing_top_info_cont'>
            <?php if(in_array('postedBy', $this->stats) || in_array('creationDate', $this->stats)): ?>
              <p class="courses_review_listing_stats">
                <?php if(in_array('postedBy', $this->stats)): ?>
        ...
        <div class="courses_review_listing_desc sesbasic_clearfix">
          ...
          <?php endif; ?>
					<?php if(in_array('customfields', $this->stats)): ?>
						<?php $customFieldsData = Engine_Api::_()->courses()->getCustomFieldMapData($item);?>
						<?php if(count($customFieldsData) > 0):?>
							<?php foreach($customFieldsData as $valueMeta):?>
								<?php if(!$valueMeta['value']):?>	
									<?php continue;?>
								<?php endif;?>
								<?php echo '<p class="courses_review_view_body"><b class="label">'. $valueMeta['label']. ': </b>'.
								$valueMeta['value'].'</p>';?>
							<?php endforeach;?>   
						<?php endif; ?>
					<?php endif; ?>
	  ...
	  <?php if(in_array('recommended', $this->stats)):?>
	    <p class="courses_review_listing_recommended"> <b><?php echo $this->translate('Recommend');?><i class="<?php if($item->recommended):?>fa fa-check<?php else:?>fa fa-times<?php endif;?>"></i></b></p>
	  <?php endif;?>
          <p class="courses_review_listing_more">
            <a href="<?php echo $item->getHref()?>" class=""><?php echo $this->translate('Continue Reading »');?></a>
          </p>
        </div>
        <?php  echo $this->partial('_reviewOptions.tpl','courses',array('subject'=>$item,'viewer'=>$this->viewer(),'stats'=>$this->stats)); ?>
      </div>
    </li>
  <?php endforeach; ?>
	...
	function viewMoreHide_<?php echo $randonNumber; ?>() {
	 ...
	 function viewMore_<?php echo $randonNumber; ?> () {
			...
			requestViewMore_<?php echo $randonNumber; ?> = new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + "widget/index/mod/courses/name/browse-reviews",
				'data': {
		format: 'html',
