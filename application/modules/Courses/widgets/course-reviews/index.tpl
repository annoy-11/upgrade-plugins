...
var previous_rate_value;
  function showReviewForm() { 
    document.getElementById('courses_review_create_form').style.display = 'block';
		var openObject = sesJqueryObject('#courses_review_create_form');
...
<?php $editReviewPrivacy = Engine_Api::_()->sesbasic()->getViewerPrivacy('coursesreview', 'edit');?>
<?php if($this->viewer()->getIdentity() && Engine_Api::_()->getApi('settings', 'core')->getSetting('courses.allow.review', 1) && $this->allowedCreate):?>
  <div class="sesbasic_profile_tabs_top sesbasic_clearfix courses_review_profile_btn">
    <a id="courses_create_button" href="javascript:void(0)" onclick="showReviewForm();" class="sesbasic_button" style="display:<?php if($this->cancreate && !$this->isReview): ?>block;<?php else:?>none;<?php endif;?>"><i class="sesbasic_icon_add"></i><span><?php echo $this->translate('Write a Review');?></span></a>
    <a id="courses_edit_button" href="javascript:void(0)" onclick="showReviewForm();" class="sesbasic_button" style="display:<?php if($editReviewPrivacy && $this->isReview):?>block;<?php else:?>none;<?php endif;?>"><i class="sesbasic_icon_edit"></i><span><?php echo $this->translate('Update Review');?></span></a>
  </div>
<?php endif;?>
  <ul class="courses_review_listing sesbasic_clearfix sesbasic_bxs" id="courses_review_listing">
<?php } ?>
  <?php if( $this->paginator->getTotalItemCount() > 0 ){ ?>
    <?php foreach( $this->paginator as $item ): ?>
      <li class="sesbasic_clearfix <?php if($item->owner_id == $this->viewer()->getIdentity()):?>courses_owner_review<?php endif;?>">
        <div class="courses_review_listing_top sesbasic_clearfix">
          <?php if(in_array('title', $this->stats)): ?>
            <div class='courses_review_listing_title sesbasic_clearfix'>
          ...
          <div class="courses_review_listing_top_info sesbasic_clearfix">
            ...
            <?php endif; ?>
	    			...
	    			<div class='courses_review_listing_top_info_cont'>
              ...
              <?php endif; ?>
              <p class="sesbasic_text_light courses_review_listing_stats">
          ...
          <div class="courses_review_show_rating review_ratings_listing">
            ...
            <?php if(in_array('parameter', $this->stats)){ ?>
              <?php $reviewParameters = Engine_Api::_()->getDbtable('parametervalues', 'courses')->getParameters(array('content_id'=>$item->getIdentity(),'user_id'=>$item->owner_id));?>
        ...
        <div class="courses_review_listing_desc sesbasic_clearfix">
          <?php if(in_array('pros', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('courses.show.pros', 1)): ?>
            <p class="courses_review_listing_body">
              <b><?php echo $this->translate("Pros"); ?></b>
              <?php echo $this->string()->truncate($this->string()->stripTags($item->pros), 300) ?>
            </p>
          <?php endif; ?>
          <?php if(in_array('cons', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('courses.show.cons', 1)): ?>
            <p class="courses_review_listing_body">
	  ...
	  <p class="courses_review_listing_recommended"> <b><?php echo $this->translate('Recommended');?><i class="<?php if($item->recommended):?>fa fa-check<?php else:?>fa fa-times<?php endif;?>"></i></b></p>
        <?php endif; ?>
          <p class="courses_review_listing_more">
          	<a href="<?php echo $item->getHref(); ?>" class=""><?php echo $this->translate("Continue Reading"); ?> &raquo;</a>
          </p>
        </div>
  			<?php echo $this->partial('_reviewOptions.tpl','courses',array('subject'=>$item,'viewer'=>Engine_Api::_()->user()->getViewer(),'stats'=>$this->stats,'profileWidgets'=>false)); ?>
      </li>
    <?php endforeach; ?>
...
<?php if(!$this->is_ajax){ ?>
  ...
  <div class="sesbasic_load_btn" id="view_more_<?php echo $randonNumber;?>" onclick="viewMore_<?php echo $randonNumber; ?>();" ><a href="javascript:void(0);" class="sesbasic_animation courses_link_btn" id="feed_viewmore_link_<?php echo $randonNumber; ?>"><i class="fa fa-repeat"></i><span><?php echo $this->translate('View More');?></span></a></div>
...
<?php if(($this->allowedCreate && $this->cancreate && $this->viewer()->getIdentity() && Engine_Api::_()->getApi('settings', 'core')->getSetting('courses.allow.review', 1)) || ($editReviewPrivacy)): ?>
  <div id="courses_review_create_form" class="courses_review_form_block" style="display:none;">
...
<script type="text/javascript">
  function closeReviewForm() {
    document.getElementById('courses_review_create_form').style.display = 'none';
		var openObject = sesJqueryObject('.courses_review_profile_btn');
				sesJqueryObject('html, body').animate({
					scrollTop: openObject.offset().top
				}, 2000);
				if(sesJqueryObject('#courses_edit_button').length && previous_rate_value != 'undefined'){
...
<script type="application/javascript">
  <?php if(!$this->is_ajax):?>
	...
	sesJqueryObject(document).on('click','.courses_own_update_review',function(e){
			e.preventDefault();
			showReviewForm();
	});
	...
	var containerId = '#courses_review_listing';
	...
	function viewMoreHide_<?php echo $randonNumber; ?>() {
	 ...
	 function viewMore_<?php echo $randonNumber; ?> () {
			...
			requestViewMore_<?php echo $randonNumber; ?> = new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + "widget/index/mod/courses/name/course-reviews",
				'data': {
		format: 'html',
		...
		loadOptionData : '<?php echo $this->loadOptionData ?>'
				...
				sesJqueryObject('#courses_review_listing').append(responseHTML);
	...
	<?php }else{ ?>
		function paggingNumber<?php echo $randonNumber; ?>(pageNum){
			...
			requestViewMore_<?php echo $randonNumber; ?> = (new Request.HTML({
				...
				onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
					sesJqueryObject('#courses_review_listing').html(responseHTML);
