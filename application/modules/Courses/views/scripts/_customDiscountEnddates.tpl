...
<style>
#courses_discount_end_date{display:block !important;}
</style>
<div class="courses_choose_date">
  <div id="discount_end_date-wrapper" class="form-wrapper">
    ...
    <div id="discount_end_date-element" class="form-element">
      <span class="courses-date-field"><input type="text" class="displayF" name="discount_end_date" id="courses_discount_end_date" value="<?php echo isset($this->end_date) ? ($this->end_date) : ''  ?>" autocomplete="off"></span>
      <span class="courses-time-field"><input type="text" name="discount_end_date_time" id="courses_discount_end_time" value="<?php echo isset($this->end_time) ? ($this->end_time) : ''  ?>" class="ui-timepicker-input" autocomplete="off"></span>
    </div>
  </div>
</div>
...
<script type="application/javascript">
 ...
 en4.core.runonce.add(function() {
	...
	<?php } ?>
	var sesselectedDiscountEndDate =  new Date(sesJqueryObject('#courses_discount_end_date').val());
	var sesFromEndDate;
	sesBasicAutoScroll('#courses_discount_end_time').timepicker({
			'showDuration': true,
			'timeFormat': 'g:ia',
	}).on('changeTime',function(){
		var lastTwoDigitStart = sesBasicAutoScroll('#courses_discount_end_time').val().slice('-2');
		var startDate = new Date(sesBasicAutoScroll('#courses_discount_end_date').val()+' '+sesBasicAutoScroll('#courses_discount_end_time').val().replace(lastTwoDigitStart,'')+':00 '+lastTwoDigitStart);
	...
	sesBasicAutoScroll('#courses_discount_end_date').datepicker({
			format: 'm/d/yyyy',
			weekStart: 1,
			autoclose: true,
