 ...
 * @version    $Id: page.tpl 2019-08-28 00:00:00 socialnetworking.solutions $
...
<?php if(count($this->existingleftpackages)){ ?>
    ...
    <div class="courses_packages_table_container">
      ...
      <?php foreach($this->existingleftpackages as $packageleft)	{
            $package = Engine_Api::_()->getItem('courses_package',$packageleft->package_id);
            $enableModules = json_decode($package->params,true);
       ?>
        <?php include APPLICATION_PATH .  '/application/modules/Courses/views/scripts/_packagesHorizontal.tpl';?>      
      <?php } ?>
      </ul>
    </div>
  </div>
      ...
      <ul class="courses_packages_table">
        <?php $existing = 0;?>
         ...
         <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('courses.package.style', 1)):?>
           <?php include APPLICATION_PATH .  '/application/modules/Courses/views/scripts/_packages.tpl';?> 
         <?php else:?>
           <?php include APPLICATION_PATH .  '/application/modules/Courses/views/scripts/_packagesHorizontal.tpl';?>
         <?php endif;?>
      	<?php } ?>
