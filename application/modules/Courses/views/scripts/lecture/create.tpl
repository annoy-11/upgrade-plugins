...
<?php if (!$this->createLimit): ?>
  <div class="tip">
    <span>
      ...
      <?php echo $this->translate('If you would like to upload a new entry, please <a href="%1$s">delete</a> an old one first.', $this->url(array('action' => 'manage-lectures','course_id'=>$this->course->custom_url), 'courses_dashboard')); ?>
    </span>
  </div>
  <br/>
<?php else: ?>
...
<script>
  function updateTextFields(value) { 
      if(value == 'html'){
        ...
        sesJqueryObject('#upload_file').show();
        sesJqueryObject('#url-wrapper').hide();
