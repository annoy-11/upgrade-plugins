...
<div class='clear'>
  <div class='settings global_form_popup courses_parameter_popup'>
    <?php echo $this->form->render($this); ?>
  </div>
</div>
<script type="application/javascript">
  var alreadyaddedParameter = sesJqueryObject('.courses_added_parameter');
  if(alreadyaddedParameter.length > 0){
    for(var i=0;i<alreadyaddedParameter.length;i++){
      var id = sesJqueryObject(alreadyaddedParameter[i]).attr('id').replace('courses_review_','');
