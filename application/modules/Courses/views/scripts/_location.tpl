...
?>
<?php $location = isset($_POST['location']) ? $_POST['location'] : (isset($this->course) && !empty($this->course->location) ? $this->course->location : '') ; ?>
<?php $venue_name = isset($_POST['venue_name']) ? $_POST['venue_name'] : (isset($this->course) && !empty($this->course->venue_name) ? $this->course->venue_name : '') ; ?>
...
<div id="seslocation-wrapper" class="form-wrapper">
    <div id="location-label" class="form-label">
      <label for="locationSes" class="<?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('courses.location.isrequired', 1)):?>required<?php endif;?>"><?php echo $this->translate("Location"); ?></label>
    </div>
    <div id="location-element" class="form-element">
      <input type="text" name="location" id="locationSes" value="<?php echo $location; ?>" />
    </div>
	</div>
	<div id="online_course-wrapper" class="form-wrapper" style="display:none;">
    <div id="online_course-label" class="form-label">
      <label for="online_course" class="optional"><?php echo $this->translate("Location"); ?></label>
    </div>
    <div id="online_course-element" class="form-element">
      <div class="tip"><span><?php echo $this->translate("This is an online course"); ?></span></div>
    </div>
  </div>
	<div id="courses_location_data-wrapper" class="courses_create_map_box form-wrapper" style="display:none;">
  	<div class="form-label"></div>
    <div class="form-element">  
      <div id="courses_location_map_data" class="courses_create_map_container sesbm" style="display:none;">
        <div id="courses_default_map" class="courses_create_blank_map centerT" style="display:none">
          <i class="fa fa-map-marker sesbasic_text_light"></i>
          <span class="sesbasic_text_light"><?php echo $this->translate("No Map"); ?></span>
        </div>
        <div id="courses_location_map" class="courses_create_map" style="display:none"></div>
      </div>
      <div class="courses_create_location_details sesbasic_bxs">
        <div id="venue_name-wrapper" class="courses_create_location_field">
        ...
        <div id="address-wrapper" class="courses_create_location_field">
        ...
        <div id="address2-wrapper" class="courses_create_location_field floatL">
        ...
        <?php if($this->countrySelect != ''){ ?>
          <div id="country-wrapper" class="courses_create_location_field">
      ...
      <div id="location_options" class="clear _links">
        <a id="courses_enter_address" href="javascript:;" class="form-link" style="display:none;"><i class="fa fa-map-marker"></i><?php echo $this->translate("Enter Address"); ?></a>
        <a id="courses_reset_location" style="display:none" href="javascript:;" class="form-link"><i class="fa fa-refresh"></i><?php echo $this->translate("Reset Location"); ?></a>
      </div>
    </div>
  </div>
...
<script type="application/javascript">
  en4.core.runonce.add(function() {
    if(typeof locationcreatedata == 'undefined'){
      ...
      sesJqueryObject(document).on('click','#courses_enter_address',function(){
        sesJqueryObject('#seslocation-wrapper').show();
        sesJqueryObject('#courses_location_data-wrapper').show();
        sesJqueryObject('#courses_enter_address').hide();
        sesJqueryObject('#courses_reset_location').show();
        var lat = sesJqueryObject('#latSes').val();
        var lng = sesJqueryObject('#lngSes').val();
        if(lat && lng ){
          sesJqueryObject('#courses_location_map_data').show();
          sesJqueryObject('#courses_location_map').show();
          sesJqueryObject('#courses_default_map').hide();
          createClassroomLoadMap();
        }else{
          sesJqueryObject('#courses_location_map_data').show();
          sesJqueryObject('#courses_location_map').hide();
          sesJqueryObject('#courses_default_map').show();
        }
      });
      sesJqueryObject(document).on('click','#courses_reset_location',function(){
        var confirmAc = confirm('Are you sure that you want to reset this location? It will not be recoverable after being deleted.');
        if(confirmAc == true){
          sesJqueryObject('#courses_location_map_data').hide();
          sesJqueryObject('#seslocation-wrapper').show();
          sesJqueryObject('#online_course-wrapper').hide();
          sesJqueryObject('#courses_location_data-wrapper').hide();
          sesJqueryObject('#courses_reset_location').hide();
    ...
    <?php if($location != '' && ($venue_name != '' || $city != '' || $state != '' || $country != '' || $zip != '' || $address != '' || $address2 != '')){ ?>
      checkinD = true;
      sesJqueryObject('#courses_enter_address').trigger('click');
    <?php } ?>
    mapLoad_course = false;
    initializeSesClassroomMapList();
  });
  function createClassroomLoadMap() { 
    var lat = sesJqueryObject('#latSes').val();
    var lng = sesJqueryObject('#lngSes').val();
    if(lat && lng && sesJqueryObject('#courses_location_map_data').css('display') == 'none'){
      sesJqueryObject('#courses_enter_address').trigger('click');
    }
	if(lat && lng && sesJqueryObject('#courses_location_map_data').css('display') == 'block'){ 
      sesJqueryObject('#courses_location_map').show();
      sesJqueryObject('#courses_default_map').hide();
      ...
      var mapLocationCreate = new google.maps.Map(document.getElementById("courses_location_map"), myOptions);
