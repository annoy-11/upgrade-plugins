 ...
 * @version    $Id: classroom-reviews.tpl 2019-08-28 00:00:00 socialnetworking.solutions $
...
<?php if(!$this->is_ajax && !$this->is_search_ajax){ ?>
    <div class='admin_search courses_reviews_table'>
    <?php $navigation = Engine_Api::_()
                    ->getApi('menus', 'core')
                    ->getNavigation('courses_main_myreviews', array()); ?>
    <?php if(count($navigation) > 1): ?>
      <?php  include APPLICATION_PATH .  '/application/modules/Courses/views/scripts/my-reviews.tpl';?>
    <?php endif; ?>
<div class='clear sesbasic-form courses_reviews_search_data'>
<?php } ?>
  <div>
    <div class="sesbasic-form-cont">
      <div class='settings courses_Reviews_form'>
				<h3><?php echo $this->translate("Manage Reviews"); ?></h3>
				<div class="admin_search courses_dashboard_search_form courses_browse_search courses_browse_search_horizontal sesbasic_clearfix sesbasic_bxs">
				  <?php echo $this->reviewFormFilter->render($this); ?>
				</div>
				<div class="courses_dashboard_table sesbasic_bxs">
					<?php $counter = $this->paginator->getTotalItemCount(); ?> 
					<?php if( count($this->paginator) ): ?>
				  <div class="courses_search_reasult" style="margin-bottom: 7px;" data-count="<?php echo $counter; ?>">
              ...
              <div id="error-message_<?php echo $randonNumber;?>">
                <div class="sesbasic_tip clearfix">
                  ...
                  <span class="sesbasic_text_light">
                    <?php echo $this->translate('You have not posted any review on classrooms.') ?>
                  </span>
                </div>
              </div>
          <?php endif; ?>
...
sesJqueryObject('#courses-search-order-img').hide();
 sesJqueryObject(document).on('submit','#filter_form',function(event){
	...
	new Request.HTML({
			method: 'get',
			url :  en4.core.baseUrl + 'courses/manage-account/classroom-reviews/',
			data : {
				format : 'html',
			...
			onComplete: function(response) {
				sesJqueryObject('#courses-search-order-img').hide();
				sesJqueryObject('.courses_reviews_search_data').html(response);
			}
	}).send();
});

function deleteReview(review_id){
var confirmDelete = confirm('<?php echo $this->translate('Are you sure you want to delete the selected Review?'); ?>');
