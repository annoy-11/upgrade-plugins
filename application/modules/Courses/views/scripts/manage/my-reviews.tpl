 ...
 * @version    $Id: my-reviews.tpl 2019-08-28 00:00:00 socialnetworking.solutions $
				      ...
				      <tbody>
				        ...
				        <tr>
				          ...
				          <td><span>
				            ...
				            |
				            <?php //echo $this->htmlLink(array('route' => 'default', 'module' => 'courses', 'controller' => 'manage', 'action' => 'delete-review', 'id' => $item->review_id), $this->translate("Delete"), array('class' => 'smoothbox')) ?>
...
sesJqueryObject('#loadingimgcourses-wrapper').hide();
 sesJqueryObject(document).on('submit','#filter_form',function(event){
	...
	new Request.HTML({
			method: 'post',
			url :  en4.core.baseUrl + 'courses/manage/my-reviews/',
			data : {
				format : 'html',
			...
			onComplete: function(response) {
				sesJqueryObject('#loadingimgcourses-wrapper').hide();
				sesJqueryObject('.courses_reviews_table').html(response);
			}
	}).send();
});
