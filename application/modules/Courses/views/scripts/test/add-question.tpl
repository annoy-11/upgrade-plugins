 ...
 * @version    $Id: add-question.tpl 2019-08-28 00:00:00 socialnetworking.solutions $
...
<script type="application/javascript">
  function addQuestion(formObject) {
    var formData = new FormData(formObject);
    var testId  = '<?php echo $this->test_id ?>';
    formData.append('is_ajax', 1);
    formData.append('test_id', '<?php echo $this->test_id ?>');
    sesJqueryObject.ajax({
      url: "courses/test/add-question/",
      type: "POST",
      contentType:false,
      processData: false,
      cache: false,
      ...
      success: function(response) {
        ...
        if(result.status == 1) {
          sesJqueryObject('#courses_add_question').html("<div id='questionsuccess_message' class='classroomservice_success_message questionsuccess_message'><i class='fa-check-circle-o'></i><span>Your Question is successfully added.</span></div>");
          sesJqueryObject('#questionsuccess_message').fadeOut("slow", function(){
            setTimeout(function() {
              sessmoothboxclose();
            }, 1000);
          });
          sesJqueryObject('#dashboard_test_box_'+testId).html(result.message+sesJqueryObject('#dashboard_test_box_'+testId).html());
        }
      }
    });
}
