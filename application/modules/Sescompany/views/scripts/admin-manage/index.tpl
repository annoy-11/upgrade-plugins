    ...
    <?php foreach ($this->paginator as $item): //print_r($item->toarray());die; ?>
      <?php if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled($item->module)): ?>
        <tr>
          ...
          <td>
            <?php if(!empty($getRow->icon_id) || !empty($getRow->font_icon)):?>
              ...
              <?php if(empty($getRow->icon_type)): ?>
                <img class="company_manangemenu_icon" alt="" src="<?php echo $this->storage->get($getRow->icon_id, '')->getPhotoUrl(); ?>" />
              <?php else: ?>
          ...
          <td>
            <?php echo $this->htmlLink(array('route' => 'default', 'module' => 'sescompany', 'controller' => 'admin-manage', 'action' => 'upload-icon', 'id' => $item->id,'type' => 'main', 'icon_type' => $getRow->icon_type), $label, array('class' => 'smoothbox')); ?>
            <?php if(!empty($getRow->icon_id) || !empty($getRow->font_icon)):?>
              | 
              <?php echo $this->htmlLink(array('route' => 'default', 'module' => 'sescompany', 'controller' => 'admin-manage', 'action' => 'delete-menu-icon', 'id' => $item->id, 'file_id' => $getRow->icon_id, 'icon_type' => $getRow->icon_type), $this->translate("Delete"), array('class' => 'smoothbox')); ?>
            <?php endif;?>
          </td>
        </tr>
      <?php endif; ?>
