...
class sesgroupveroth_AdminManageController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesgroupveroth_admin_main', array(), 'sesgroupveroth_admin_main_manage');

    $this->view->formFilter = $formFilter = new Sesgroupveroth_Form_Admin_Manage_FilterManage();
...
//       $select->where('featured = ?', $values['featured']);
  ...
  public function enabledAction() {
    ...
    if (!empty($resource_id) && $admin_enabled) {
      Engine_Api::_()->getDbTable('verifications', 'sesgroupveroth')->update(array('admin_enabled' => 0), array('resource_id = ?' => $resource_id));
    } else {
      Engine_Api::_()->getDbTable('verifications', 'sesgroupveroth')->update(array('admin_enabled' => 1), array('resource_id = ?' => $resource_id));
    }
    $this->_redirect('admin/sesgroupveroth/manage');
  }
  ...
  public function removeAction() {
    ...
    $this->view->form = $form = new Sesgroupveroth_Form_Admin_Remove();
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        Engine_Api::_()->getDbTable('verifications', 'sesgroupveroth')->delete(array('resource_id' => $resource_id));
        $db->commit();
      } catch (Exception $e) {
        $db->rollBack();
        throw $e;
