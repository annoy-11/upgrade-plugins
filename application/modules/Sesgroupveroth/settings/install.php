...
class Sesgroupveroth_Installer extends Engine_Package_Installer_Module {
  ...
  public function onPreinstall() {
    ...
    $select->from('engine4_core_settings')
            ...
            ->limit(1);
    $group_activate = $select->query()->fetchAll();

    if(!empty($sesgroup_Check) && !empty($group_activate[0]['value'])) {
      $plugin_currentversion = '4.10.3p6';
      $error = include APPLICATION_PATH . "/application/modules/Sesbasic/controllers/checkPluginVersion.php";
      if($error != '1') {
        return $this->_error($error);
      }
    } elseif(!empty($sesgroup_Check) && empty($group_activate[0]['value'])) {
      return $this->_error('<div class="global_form"><div><div><p style="color:red;">The "<a href="https://www.socialenginesolutions.com/social-engine/group-directories-plugin/" target="_blank">Group Directories Plugin</a>" is installed on your website, but is not yet activated. So, please first activate it before installing the Group Verification by Members Extension.</p></div></div></div>');
