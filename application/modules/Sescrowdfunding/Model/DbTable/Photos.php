 ...
 * @version    $Id: Photos.php  2019-01-08 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sescrowdfunding_Model_DbTable_Photos extends Engine_Db_Table {

  protected $_rowClass = 'Sescrowdfunding_Model_Photo';
  
  public function getPhotoSelect($params = array()) {
  	
    $select = $this->select();
    
    if( !empty($params['album']) && $params['album'] instanceof Sescrowdfunding_Model_Album ) {
    ...
    if(!empty($params['file_id'])) {
      $select->where('file_id != ?', $params['file_id']);
    }
