 ...
 */
class Sescrowdfunding_Widget_BrowseCrowdfundingsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $form = new Sescrowdfunding_Form_Search(array('searchTitle' => 'yes','browseBy' => 'yes','categoriesSearch' => 'yes','searchFor'=>'crowdfunding','FriendsSearch'=>'yes','defaultSearchtype'=>'mostSPliked','locationSearch' => 'yes','kilometerMiles' => 'yes'));
    ...
    // Crowdfunding browse page work
    $type = '';
    $page_id = Engine_Api::_()->sescrowdfunding()->getWidgetPageId($identityForWidget);
    if($page_id) {
      $pageName = Engine_Db_Table::getDefaultAdapter()->select()
              ...
              ->fetchColumn();
      if($pageName) {
        $explode = explode('sescrowdfunding_index_', $pageName);
        if(is_numeric($explode[1])) {
          $type = Engine_Db_Table::getDefaultAdapter()->select()
                ->from('engine4_sescrowdfunding_integrateothermodules', 'content_type')
    ...
    $value['status'] = 1;
    $value['search'] = 1;
    $this->view->widgetName = 'browse-crowdfundings';
    $params = array_merge($params, $value,array('search'=>addslashes($text)));

    $value['draft'] = "0";
    $value['visible'] = "1";
    //Get crowdfundings
    $paginator = Engine_Api::_()->getDbtable('crowdfundings', 'sescrowdfunding')->getSescrowdfundingsPaginator($params, $form->getValues());
