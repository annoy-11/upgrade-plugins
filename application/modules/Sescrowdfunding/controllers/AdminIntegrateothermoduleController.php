...
class Sescrowdfunding_AdminIntegrateothermoduleController extends Core_Controller_Action_Admin {

  public function indexAction() {
  
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sescrowdfunding_admin_main', array(), 'sescrowdfunding_admin_main_integrateothermodule');
    
    $this->view->enabledModules = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();
    
    $select = Engine_Api::_()->getDbtable('integrateothermodules', 'sescrowdfunding')->select();
  ...
  public function addmoduleAction() {
    ...
    $this->view->form = $form = new Sescrowdfunding_Form_Admin_Manage_Add();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      $integrateothermoduleTable = Engine_Api::_()->getDbtable('integrateothermodules', 'sescrowdfunding');
      ...
      try {
        ...
        $dbInsert->query('INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `enabled`, `custom`, `order`) VALUES ("sescrowdfunding_main_browsecrowdfunding_'.$row->getIdentity().'", "'.$modulename.'", "Browse Crowdfundings", "", \'{"route":"sescrowdfunding_browsecrowdfunding_'.$row->getIdentity().'","action":"browse-crowdfundings", "resource_type":"'.$values['content_type'].'"}\', "'.$modulename.'_main", "", 1, 0, 999)');
        
        $this->createBrowseCrowdfundingPage($modulename, $row->getIdentity());
  ...
  function createBrowseCrowdfundingPage($modulename, $id) {
    ...
    $page_id = $db->select()
      ...
      ->where('name = ?', 'sescrowdfunding_index_'.$id)
      ->limit(1)
      ->query()
      ->fetchColumn();
    ...
    if( !$page_id ) {
      ...
      $db->insert('engine4_core_pages', array(
        'name' => 'sescrowdfunding_index_'.$id,
        'displayname' => 'SES - Advanced Crowdfundings - '.ucfirst($modulename).' Crowdfundings Browse Page',
        'title' => ucfirst($modulename) .' Crowdfunding Browse',
        'description' => 'This page lists crowdfundings.',
        'custom' => 0,
      ));
  ...
  public function deleteAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $inttable = Engine_Api::_()->getItem('sescrowdfunding_integrateothermodule', $this->_getParam('integrateothermodule_id'));
        $pageName = "sescrowdfunding_index_".$this->_getParam('integrateothermodule_id');
        if (!empty($pageName)) {
          $page_id = $db->select()
                  ->from('engine4_core_pages', 'page_id')
                  ->where('name = ?', $pageName)
        ...
        Engine_Api::_()->getDbtable('menuItems', 'core')->delete(array('name =?' => 'sescrowdfunding_main_browsecrowdfunding_' . $this->_getParam('integrateothermodule_id')));
      
        $integrateothermodule = Engine_Api::_()->getItem('sescrowdfunding_integrateothermodule', $this->_getParam('integrateothermodule_id'));
  ...
  public function enabledAction() {
    $db = Engine_Db_Table::getDefaultAdapter();
    $db->beginTransaction();
    $content = Engine_Api::_()->getItemTable('sescrowdfunding_integrateothermodule')->fetchRow(array('integrateothermodule_id = ?' => $this->_getParam('integrateothermodule_id')));
    try {
      
      Engine_Api::_()->getDbtable('menuItems', 'core')->update(array('enabled' => !$content->enabled), array('name =?' => 'sescrowdfunding_main_browsecrowdfunding_' . $this->_getParam('integrateothermodule_id')));
