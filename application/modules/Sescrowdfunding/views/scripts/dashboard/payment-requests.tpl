...
<div class="sescrowdfunding_dashboard_content_header sesbasic_clearfix">	
  <h3><?php echo $this->translate("Make Payment Request"); ?></h3>
    ...
    <div class="sescrowdfunding_dashboard_threshold_amt"><?php echo $this->translate("Threshold Amount:"); ?> <b><?php echo Engine_Api::_()->sescrowdfunding()->getCurrencyPrice($this->thresholdAmount,$defaultCurrency); ?></b></div>
    <?php } ?>
</div>
...
<div class="sescf_dashboard_btn">	
	<a href="<?php echo $this->url(array('crowdfunding_id' => $this->crowdfunding->custom_url,'action'=>'payment-request'), 'sescrowdfunding_dashboard', true); ?>" class="openSmoothbox sesbasic_button cescf_pr_icon cescf_ic_btn"><?php echo $this->translate("Make Request For Payment."); ?></a>
</div>
<?php } ?>
      ...
      <tbody>
        <?php foreach ($this->paymentRequests as $item): ?>
          <tr>
            ...
            <td class="centerT"><?php echo $item->state != 'pending' ? Engine_Api::_()->sescrowdfunding()->getCurrencyPrice($item->release_amount	,$defaultCurrency) :  "-"; ?></td>
