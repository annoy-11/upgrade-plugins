 ...
 * @version    $Id: Encodesample.php 2015-10-11 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesvideo_Plugin_Job_Encodesample extends Core_Plugin_Job_Abstract {
  ...
  protected function _execute() {
    ...
    // Get video object
    $video = Engine_Api::_()->getItem('sesvideo_samplevideo', $video_id);
    if (!$video || !($video instanceof Sesvideo_Model_Samplevideo)) {
  ...
  private function getTmpDir() {
    ...
    $tmpDir = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'temporary' . DIRECTORY_SEPARATOR . 'samplevideo';
  ...
  private function getVideo($video) {
    ...
    if (is_numeric($video)) {
      $video = Engine_Api::_()->getItem('sesvideo_samplevideo', $video);
    }

    if (!($video instanceof Sesvideo_Model_Samplevideo)) {
