  ...
  <div id="heightOfImageViewerContent">
    <div id="flexcroll" >
      ...
      <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesvideo_enable_location', 1)){ ?>
        ...
        <?php } ?>
        <?php if($this->canEdit){ ?>
        <div class="ses_pswp_item_edit_link"> <a id="editDetailsLink" href="javascript:void(0)" class="sesalbum_button"> <i class="fa fa-edit sesbasic_text_light"></i> <?php echo $this->translate('Edit Details'); ?> </a> </div>
        <?php } ?>
      </div>
      <?php if($this->canEdit){ ?>
      ...
      <?php echo $this->action("list", "comment", "sesadvancedcomment", array("type" => "chanelphoto", "id" => $this->photo->getIdentity()));
        }else{ echo $this->action("list", "comment", "core", array("type" => "chanelphoto", "id" => $this->photo->getIdentity())); } ?></div>
    </div>
  </div>
</div>
</div>
  ...
  <div class="pswp__top-bar-action">
    ...
    <div class="pswp__top-bar-more"> <a href="javascript:;" class="optionOpenImageViewer" id="overlay-model-class" class="">Options <i class="fa fa-angle-down" id="overlay-model-class-down"></i></a>
      <div class="pswp__top-bar-more-tooltip" style="display:none">
        ...
        <a class="ses-album-photo-download" href="<?php echo $this->url(array('module' => 'sesalbum', 'action' => 'download', 'photo_id' => $this->photo->chanelphoto_id,'type'=>'chanelphoto'), 'sesalbum_general', true); ?>"><?php echo $this->translate('Download'); ?></a>
        <?php } ?>
    ...
    <div class="pswp__top-bar-msg pswp__top-bar-btns">      
     <?php if ($this->photo->authorization()->isAllowed($this->viewer, 'comment')){ ?>
      ...
      <?php } ?>
      <a class="sesbasic_icon_btn sesalbum_icon_msg_btn smoothbox" href="<?php echo $this->url(array('module'=> 'sesalbum', 'controller' => 'index', 'action' => 'message','photo_id' => $this->photo->getIdentity(),'type'=>'chanelphoto' ,'format' => 'smoothbox'),'sesalbum_extended',true); ?>" onclick="openURLinSmoothBox(this.href);return false;"><i class="fa fa-envelope"></i></a>
    </div>
    <?php } ?>
  </div>
