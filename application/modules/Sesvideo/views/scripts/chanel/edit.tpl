 ...
 var checkURL = true;
    ...
    new Request.HTML({
				...
				onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
					...
					if(responseHTML == 1){
						...
						sesJqueryObject('.msg').html('<i class="fa fa-times" title="Unavilable"></i>');
   ...
   checkURL = false;
	...
	sesJqueryObject(document).on('click','.add-video-manage',function(){
			...
			sesJqueryObject("<span class='delete_selected_video'><a class='delete-selected' href='javascript:;'><i class='fas fa-times'></i></a></span>").insertAfter(sesJqueryObject('#added_manage_videos').find('#videoId-'+sesJqueryObject(this).attr('data-url')).find('.sesvideo_grid_thumb').find('.sesvideo_thumb_nolightbox'));			
