...
<script type="text/javascript">
  en4.core.runonce.add(function() {
    ...
    var ignoreValidation = window.ignoreValidation = function() {
      $('upload-wrapper').style.display = "inline-block";
    ...
    var updateTextFields = window.updateTextFields = function() {
      ...
      if( video_element.value == 0 ) {
        ...
        url_element.style.display = "none";
				...
				sesJqueryObject('#price-wrapper').hide();
      ...
      /*else if( $('code').value && $('url').value ) {
        ...
        return;
				...
				sesJqueryObject('#price-wrapper').<?php echo $show; ?>();
				sesJqueryObject('#title-wrapper').show();
				sesJqueryObject('#description-wrapper').show();
				if($('photo_id-wrapper'))
					$('photo_id-wrapper').style.display = 'block';
      ...
      iframely : function(url) {
          (new Request.JSON({
            ...
            'onSuccess' : function(response) {
              if( response.valid ) {
                $('upload-wrapper').style.display = "inline-block";
                $('orText').style.display = "inline-block";
                $('validation').style.display = "none";
...
sesJqueryObject("#embedUrl").on("change keyup paste", function() {
		...
		if(sesJqueryObject('#type') != '17' && currentVal != ''){
				$('upload-wrapper').style.display = "inline-block";
				$('validation').style.display = "none";
...
sesJqueryObject('#form-upload').submit(function(e){
 ...
 window.addEvent('domready', function() {	
			...
			<?php if(count($this->data)){ ?>
				sesJqueryObject('#code').val('<?php echo $this->data["code"]; ?>');
				sesJqueryObject('#url').val('<?php echo $this->data["url"]; ?>');
			<?php } ?>
  });
  ...
  function showPaidVideoOptions(value) {
    if(value == 'free') {
      if($('price-wrapper'))
        $('price-wrapper').style.display = 'none';
      if($('sample_file-wrapper'))
        $('sample_file-wrapper').style.display = 'none';
    } else if(value == 'paid') {
      if($('price-wrapper'))
        $('price-wrapper').style.display = 'block';
      if($('sample_file-wrapper'))
        $('sample_file-wrapper').style.display = 'block';
    }
  }
</script> 
