...
class Sesvideo_Model_DbTable_Chanelfollows extends Engine_Db_Table {
  ...
  protected $_name = 'sesvideo_chanelfollows';
  ...
  public function getUserFollowChannel($viewer_id) {
    if (!$viewer_id)
      return;
    $select = $this->select()->from($this->info('name'))->where('owner_id =?',$viewer_id);
    return $this->fetchAll($select);
  }

  public function getFollowChannel($viewer_id) {
    if (!$viewer_id)
      return;
    $select = $this->select()->from($this->info('name'))->where('owner_id !=?',$viewer_id);
    return $this->fetchAll($select);
