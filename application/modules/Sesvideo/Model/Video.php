...
class Sesvideo_Model_Video extends Core_Model_Item_Abstract {
  ...
  public function getRichContent($view = false, $params = array(), $map = false,$autoplay = true) {
    ...
    if ($view == false) {
      ...
      $videoEmbedded = $thumb . '<div id="video_object_' . $this->video_id . '" data-rel="'.$this->type.'" class="sesvideo_object"></div><div class="sesvideo_attachment_info">' . $title . $description . '</div>';
    }
  ...
  public function getPhotoUrl($type = null) {
    ...
    if (!$photo_id && !$this->is_locked && !$this->adult)
      return $defaultPhoto;
		...
		$valid = false;
		if($cookieData && in_array($this->video_id,@explode(',',$cookieData))){
			$valid = true;
		}
    ...
    if (!$valid && $this->is_locked && Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock'))  && in_array('sesvideo',$sesprofilelock_enable_module) && $viewer->getIdentity() != $this->owner_id)
