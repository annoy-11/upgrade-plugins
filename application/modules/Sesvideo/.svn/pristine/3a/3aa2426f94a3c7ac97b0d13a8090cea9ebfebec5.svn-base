  ...
  public function getitem($params = array()) {
    if ($params['type'] == 'sesvideo_chanel') {
      ...
      $not = true;
		...
		$subquery = $this->select()->from($this->info('name'),array('*','MAX(creation_date) as maxcreadate'))->group($this->info('name').".resource_id")->order($this->info('name').".resource_id DESC")->where($this->info('name').'.resource_type =?', $params['type']);		
    $select = $this->select()
