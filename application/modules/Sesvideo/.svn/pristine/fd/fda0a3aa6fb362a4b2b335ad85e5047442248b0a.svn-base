...
class Sesvideo_AdminIntegrateothermoduleController extends Core_Controller_Action_Admin {

  public function indexAction() {
  
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesvideo_admin_main', array(), 'sesvideo_admin_main_integrateothermodule');
    
    $this->view->enabledModules = Engine_Api::_()->getDbtable('modules', 'core')->getEnabledModuleNames();
    
    $select = Engine_Api::_()->getDbtable('integrateothermodules', 'sesvideo')->select();
  ...
  public function addmoduleAction() {
    ...
    $this->view->form = $form = new Sesvideo_Form_Admin_Manage_Add();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      $integrateothermoduleTable = Engine_Api::_()->getDbtable('integrateothermodules', 'sesvideo');
      ...
      try {
        ...
        $dbInsert->query('INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `enabled`, `custom`, `order`) VALUES ("sesvideo_main_browsevideo_'.$row->getIdentity().'", "'.$modulename.'", "Browse Videos", "", \'{"route":"sesvideo_browsevideo_'.$row->getIdentity().'","action":"browse-videos", "resource_type":"'.$values['content_type'].'"}\', "'.$modulename.'_main", "", 1, 0, 999)');
        
        $this->createBrowseVideoPage($modulename, $row->getIdentity());
  ...
  function createBrowseVideoPage($modulename, $id) {
    ...
    if (!$page_id) {
      ...
      $db->insert('engine4_core_pages', array(
          'name' => 'sesvideo_index_'.$id,
          'displayname' => 'SES - Advanced Videos - '.ucfirst($modulename).' Videos Browse Page',
          'title' => ucfirst($modulename) .' Video Browse',
  ...
  public function deleteAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $inttable = Engine_Api::_()->getItem('sesvideo_integrateothermodule', $this->_getParam('integrateothermodule_id'));
        $pageName = "sesvideo_index_".$this->_getParam('integrateothermodule_id');
        if (!empty($pageName)) {
          $page_id = $db->select()
                  ->from('engine4_core_pages', 'page_id')
                  ->where('name = ?', $pageName)
        ...
        Engine_Api::_()->getDbtable('menuItems', 'core')->delete(array('name =?' => 'sesvideo_main_browsevideo_' . $this->_getParam('integrateothermodule_id')));
      
        $integrateothermodule = Engine_Api::_()->getItem('sesvideo_integrateothermodule', $this->_getParam('integrateothermodule_id'));
  ...
  public function enabledAction() {
    $db = Engine_Db_Table::getDefaultAdapter();
    $db->beginTransaction();
    $content = Engine_Api::_()->getItemTable('sesvideo_integrateothermodule')->fetchRow(array('integrateothermodule_id = ?' => $this->_getParam('integrateothermodule_id')));
    try {
      
      Engine_Api::_()->getDbtable('menuItems', 'core')->update(array('enabled' => !$content->enabled), array('name =?' => 'sesvideo_main_browsevideo_' . $this->_getParam('integrateothermodule_id')));
