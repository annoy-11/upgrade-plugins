...
class Sesvideo_AdminSettingsController extends Core_Controller_Action_Admin {
 ...
 // for default installation
  ...
  public function levelChanelphotoAction() {
    ...
    try {
      // Set permissions
      $permissionsTable->setAllowed('chanelphoto', $id, $values);
      // Commit
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
  ...
  'parent_content_id' => $main_middle_id,
  'order' => $widgetOrder++,
  'params' => '{"popularity":"favourite_count","viewType":"gridview","show_criteria":["title","favouriteCount","ratingCount"],"height":"155","width":"155","limit":"14","title":"","nomobile":"0","name":"sesvideo.popular-artists"}',
  ));
  ...
  public function setLandingPageAction(){
    ...
    $form->setTitle("Set This Page As Landing Page?");
      $form->setDescription('Are you sure want to set the Welcome Page of this plugin as the Landing page of your website? For old landing page you will have to manually make changes in the Landing page from Layout Editor. Backup page of your current landing page will get created with the name "SES - Advanced Videos & Channels - Backup - Landing Page".');
  ...
  'params' => '{"bodyclass":"album_landingpage","title":"","nomobile":"0","name":"sesbasic.body-class"}',
      ...
      if ($newbakpage_id) {
        ...
        $db->query('UPDATE `engine4_core_pages` SET `displayname` = "SES - Advanced  Videos & Channels - Backup - Landing Page" WHERE `engine4_core_pages`.`name` = "sesvideo_index_sesbackuplandingppage";');
      }
    }
          }
        } catch (Exception $e) {
