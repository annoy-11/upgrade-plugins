...
class Sesvideo_ChanelController extends Core_Controller_Action_Standard {
  ...
  public function indexAction() {
    ...
    // Render
		if((Engine_Api::_()->getApi('core', 'sesbasic')->checkAdultContent(array('module'=>'video')) && $chanel->adult) || $chanel->owner_id == $viewer->getIdentity()) {
  ...
  public function createAction() {
    ...
    $this->view->quota = $quota = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'sesvideo_chanel', 'maxchannel');
...
//      }
  ...
  public function downloadAction() {
    ...
    $chanelphoto = Engine_Api::_()->getItem('chanelphoto', $this->getRequest()->getParam('photo_id'));
    if (!$chanelphoto)
      return;
  ...
  public function deleteAction() {
    ...
    if ($chanel_id) {
      ...
      $dbObject = Engine_Db_Table::getDefaultAdapter();
      $query = $dbObject->query('DELETE FROM engine4_sesvideo_chanels WHERE  `chanel_id` = ' . $chanel_id);
      $queryVideos = $dbObject->query('DELETE FROM engine4_sesvideo_chanelvideos WHERE  `chanel_id` = ' . $chanel_id);
      $queryFollow = $dbObject->query('DELETE FROM engine4_sesvideo_chanelfollows WHERE  `chanel_id` = ' . $chanel_id);
      $this->view->status = true;
  ...
  public function deletePhotoAction() {
    $viewer = Engine_Api::_()->user()->getViewer();
    $chanelPhoto = Engine_Api::_()->getItem('chanelphoto', $this->getRequest()->getParam('photo_id'));
    if (!$this->_helper->requireAuth()->setAuthParams('chanelphoto', $viewer, 'delete')->isValid())
      return;
    ...
    if ($photo_id) {
      ...
      $dbObject = Engine_Db_Table::getDefaultAdapter();
      $query = $dbObject->query('DELETE FROM engine4_sesvideo_chanelphotos WHERE  `chanelphoto_id` = ' . $photo_id);
      $this->view->status = true;
  ...
  public function photosAction() {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $db = Engine_Api::_()->getItemTable('chanelphoto')->getAdapter();
    $db->beginTransaction();
    try {
  ...
  public function removeAction() {
    ...
    $photo = Engine_Api::_()->getItem('chanelphoto', $photo_id);
    $db = Engine_Api::_()->getDbTable('chanelphotos', 'sesvideo')->getAdapter();
    $db->beginTransaction();
    try {
      $photo->delete();
  ...
  public function locationAction() {
    $this->view->type = $this->_getParam('type', 'chanelphoto');
    ...
    if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
      $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
      $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $photo_id . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","chanelphoto")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
    }
  ...
  public function editPhotoAction() {
    $this->view->photo_id = $photo_id = $this->_getParam('photo_id');
    $this->view->photo = Engine_Api::_()->getItem('chanelphoto', $photo_id);
  }
  ...
  public function editDetailAction() {
    $status = true;
    $error = false;
    if (!$this->_helper->requireAuth()->setAuthParams('chanelphoto', null, 'edit')->isValid()) {
      $status = false;
      $error = true;
    }
    $viewer = Engine_Api::_()->user()->getViewer();
    $photo = Engine_Api::_()->getItem('chanelphoto', $_POST['photo_id']);
    if ($status && !$error) {
      ...
      if ($_POST['lat'] != '' && $_POST['lng'] != '') {
        $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
        $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $_POST['photo_id'] . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","chanelphoto")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }
      $db = $photo->getTable()->getAdapter();
      $db->beginTransaction();
      try {
  ...
  public function editAction() {
    ...
    try {
      ...
      throw $e;
		...
		$dbObject = Engine_Db_Table::getDefaultAdapter();
		$existingVideoChanel = $dbObject->query('SELECT GROUP_CONCAT(video_id) as existing_video_ids FROM engine4_sesvideo_chanelvideos WHERE chanel_id = '.$chanel->chanel_id)->fetchColumn();
    ...
    if (isset($values['delete_video_ids']) && $values['delete_video_ids'] != '' && isset($chanel->chanel_id)) {
      ...
      $query = 'DELETE FROM engine4_sesvideo_chanelvideos WHERE (`video_id`) IN (' . rtrim($ids, ',') . ') AND `chanel_id` = ' . $chanel->chanel_id;
      $stmt = $dbObject->query($query . rtrim($queryString, ','));
    }

      $query = 'DELETE FROM engine4_sesvideo_chanelvideos WHERE chanel_id = '.$chanel->chanel_id;
      $stmt = $dbObject->query($query);
    // Now try to create videos
    if (isset($values['video_ids']) && $values['video_ids'] != '' && isset($chanel->chanel_id)) {
      $explodeIds = explode(',', rtrim($values['video_ids'], ','));
      $queryString = '';
      ...
      if ($runQuery) {
        $query = 'INSERT IGNORE INTO engine4_sesvideo_chanelvideos (`chanel_id`, `video_id` ,`owner_id`,`creation_date`) VALUES ';
        $stmt = $dbObject->query($query . rtrim($queryString, ','));
      }
...
<span style="display:block;"><a href="'.$siteUrl.$user->getHref().'" style="font-family:arial,Arial,sans-serif;font-size:12px;color:#999999;line-height:15px;letter-spacing:0px;text-decoration:none" target="_blank">by '.$user->getTitle().'</a>
			...
			$tickVideo = Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('tickvideo');
			if(count($followerChannel)){
				foreach($followerChannel as $follower){
                    ...
                    if($tickVideo) {
                        ...
                        $notifyApi->addNotification($userObj, $chanel, $chanel, 'sesvideo_channel_content');
                    }
				}
			}
		}
  ...
  function viewAction() {
    ...
    if (!$photo_id)
      return $this->_forward('requireauth', 'error', 'core');
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->photo = $photo = Engine_Api::_()->getItem('chanelphoto', $photo_id);
    ...
    if (!$this->_helper->requireAuth()->setAuthParams('chanelphoto', null, 'view')->isValid())
      return;
  ...
  function likeAction() {
    ...
    $select = $tableLike->select()->from($tableMainLike)->where('resource_type =?', 'chanelphoto')->where('poster_id =?', Engine_Api::_()->user()->getViewer()->getIdentity())->where('poster_type =?', 'user')->where('resource_id =?', $photo_id);
    ...
    if (count($Like) > 0) {
      ...
      $like_count = Engine_Api::_()->getItem('chanelphoto', $photo_id);
      ...
      try {
        ...
        $like->resource_type = 'chanelphoto';
