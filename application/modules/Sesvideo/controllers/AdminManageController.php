...
class Sesvideo_AdminManageController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $formFilter->status->setLabel('State');
    ...
    if (isset($_GET['category_id']) && $_GET['category_id'] != 0)
      $select->where($tableName.'.category_id = ?', $values['category_id']);
    ...
    if ($this->getRequest()->isPost()) {
      ...
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          ...
          $db->query("DELETE FROM engine4_sesvideo_chanelvideos WHERE chanel_id = " . $value);
        }
      }
    }
  ...
  public function deleteChanelAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db = Engine_Db_Table::getDefaultAdapter();
      $db->query("DELETE FROM engine4_sesvideo_chanelvideos WHERE chanel_id = " . $id);
      $this->_forward('success', 'utility', 'core', array(
          'smoothboxClose' => 10,
          'parentRefresh' => 10,
  ...
  public function ofthedayAction() {
    ...
    if ($type == 'video') {
      ...
      $table = 'engine4_sesvideo_chanels';
      $item_id = 'chanel_id';
    ...
    if ($this->getRequest()->isPost()) {
      ...
      if (isset($values['remove']) && $values['remove']) {
        ...
        $db->update($table, array('starttime' => null), array("$item_id = ?" => $id));
        $db->update($table, array('endtime' => null), array("$item_id = ?" => $id));
      } else {
