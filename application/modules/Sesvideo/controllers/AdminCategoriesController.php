...
class Sesvideo_AdminCategoriesController extends Core_Controller_Action_Admin {
        ...
        foreach ($_POST['data'] as $key => $valueSelectedcategory) {
          ...
          if ($deleteCategory) {
            $deleteCategoryIds[] = $categoryDelete->category_id;
						//update channel category data
						$chanelTable->update(array(
								'category_id' => 0,
							), array(
  ...
  public function changeOrderAction() {
    if ($this->_getParam('id', false) || $this->_getParam('nextid', false)) {
      ...
      $categoryTypeId = '';
      $checkTypeCategory = $dbObject->query("SELECT * FROM engine4_sesvideo_categories WHERE category_id = " . $category_id)->fetchAll();
      ...
      $checkCategoryChildrenCondition = $dbObject->query("SELECT * FROM engine4_sesvideo_categories WHERE subcat_id = '" . $id . "' || subsubcat_id = '" . $id . "' || subcat_id = '" . $nextid . "' || subsubcat_id = '" . $nextid . "'")->fetchAll();
