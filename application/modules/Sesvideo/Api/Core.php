...
class Sesvideo_Api_Core extends Core_Api_Abstract {
  ...
  public function checkPrivacySetting($id) {
    ...
    $item = Engine_Api::_()->getItem('sesvideo_video', $id);
  ...
  public function getPluginItem($moduleName) {
    ...
    if (is_file($filePath)) {
      ...
      $manafestFile = include $filePath;
			$resultsArray =  Engine_Api::_()->getDbtable('integrateothermodules', 'sesvideo')->getResults(array('module_name'=>$moduleName));
  ...
  function getVideoViewerHref($getImageViewerData, $paramsExtra = array()) {
	...
	//get chanel photo
  function getChanelPhotos($chanelId = '', $limit = 3) {
    if ($chanelId != '') {
      $chanel = Engine_Api::_()->getItemTable('sesvideo_chanel');
      $chanelTableName = $chanel->info('name');
      $photos = Engine_Api::_()->getItemTable('chanelphoto');
  ...
  public function deleteWatchlaterVideo($params = array()) {
    ...
    if (count($params['video_id'])) {
      ...
      $object = $dbObject->query('INSERT INTO engine4_sesvideo_watchlaters (video_id, owner_id,modified_date,creation_date ) VALUES ("' . $params['video_id'] . '", "' . $user_id . '",NOW(),NOW())	ON DUPLICATE KEY UPDATE 	modified_date = modified_date');
  ...
  public function getCategories($params = array()) {
    $table = Engine_Api::_()->getDbTable('categories', 'sesvideo');
		return Engine_Api::_()->getDbtable('categories', 'sesvideo')->getCategory($params);
  }
  ...
  // handle video upload
  public function createSampleVideo($params, $file, $values,$video_date = false) {
    ...
    if ($file instanceof Storage_Model_File) {
      ...
      // create video item
			if(!$video_date) {
      	$video = Engine_Api::_()->getDbtable('samplevideos', 'sesvideo')->createRow();
      ...
      // Add to jobs
        ...
        Engine_Api::_()->getDbtable('jobs', 'core')->addJob('samplevideo_encode', array(
  ...
  public function deleteVideo($video) {
    ...
    if ($video->status == 1) {
      ...
      if ($video->type == 3)
          Engine_Api::_()->getItem('storage_file', $video->file_id) ? Engine_Api::_()->getItem('storage_file', $video->file_id)->remove() : "";
      if ($video->photo_id)
          Engine_Api::_()->getItem('storage_file', $video->photo_id) ? Engine_Api::_()->getItem('storage_file', $video->photo_id)->remove() : "";
    }
