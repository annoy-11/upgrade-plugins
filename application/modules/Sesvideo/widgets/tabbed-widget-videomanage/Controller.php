  ...
  public function indexAction() {
    ...
    if ($popularCol == 'follow_chanel' && $popularCol == 'my_channel')
      ...
      $type = 'video';
			...
			$value['manageChannel'] = true;
			$value['widgetName'] = 'tabbed-widget-videomanage';
    $this->view->type = $type;
    ...
    if ($popularCol == 'my_videos') {
      ...
      $paginator = Engine_Api::_()->sesvideo()->getLikesContents(array_merge($value, array('resource_type' => 'video')));
      $this->view->getVideoItem = 'getVideoItem';
    } else if ($popularCol == 'rated_videos') {
      $paginator = Engine_Api::_()->getDbTable('ratings', 'sesvideo')->getRatedItems(array_merge($value, array('resource_type' => 'video')));
      $this->view->getVideoItem = 'getVideoItem';
    } else if ($popularCol == 'favourite_videos') {
			$this->view->getVideoItem = 'getVideoItem';
      $paginator = Engine_Api::_()->getDbTable('favourites', 'sesvideo')->getFavourites(array_merge($value, array('resource_type' => 'sesvideo_video')));
      ...
      $this->view->getVideoItem = 'getVideoItem';
      $paginator = Engine_Api::_()->getDbTable('watchlaters', 'sesvideo')->getWatchlaterItems(array_merge($value, array('resource_type' => 'sesvideo_video')));
      ...
      $paginator = Engine_Api::_()->getDbTable('chanels', 'sesvideo')->getChanels(array_merge($value, array('user_id' => $viewer->getIdentity())));
    } else if ($popularCol == 'follow_chanel') {
      $paginator = Engine_Api::_()->getDbTable('chanels', 'sesvideo')->getChanels(array_merge($value, array('follow_id' => $viewer->getIdentity())));
    } else if ($popularCol == 'liked_chanel') {
      $paginator = Engine_Api::_()->sesvideo()->getLikesContents(array_merge($value, array('resource_type' => 'sesvideo_chanel')));
      $this->view->getChanelItem = 'getChanelItem';
    } else if ($popularCol == 'favourite_chanel') {
      $paginator = Engine_Api::_()->getDbTable('favourites', 'sesvideo')->getFavourites(array_merge($value, array('resource_type' => 'sesvideo_chanel', 'user_id' => $viewer->getIdentity())));
      $this->view->getChanelItem = 'getChanelItem';
    } else if ($popularCol == 'featured_chanel') {
      $paginator = Engine_Api::_()->getDbTable('chanels', 'sesvideo')->getChanels(array_merge($value, array('user_id' => $viewer->getIdentity(), 'is_featured' => true)));
    } else if ($popularCol == 'sponsored_chanel') {
      $paginator = Engine_Api::_()->getDbTable('chanels', 'sesvideo')->getChanels(array_merge($value, array('user_id' => $viewer->getIdentity(), 'is_sponsored' => true)));
    } else if ($popularCol == 'hot_chanel') {
      $paginator = Engine_Api::_()->getDbTable('chanels', 'sesvideo')->getChanels(array_merge($value, array('user_id' => $viewer->getIdentity(), 'is_hot' => true)));
    } else if ($popularCol == 'my_playlist') {
      $paginator = Engine_Api::_()->getDbTable('playlists', 'sesvideo')->getPlaylistPaginator(array_merge($value, array('user' => $viewer->getIdentity())));
      $this->view->view_type = 'playlist';
      $this->view->my_playlist = true;
    } else if ($popularCol == 'featured_playlist') {
      $paginator = Engine_Api::_()->getDbTable('playlists', 'sesvideo')->getPlaylistPaginator(array_merge($value, array('user' => $viewer->getIdentity(), 'is_featured' => true)));
      $this->view->view_type = 'playlist';
    } else if ($popularCol == 'sponsored_playlist') {
      $paginator = Engine_Api::_()->getDbTable('playlists', 'sesvideo')->getPlaylistPaginator(array_merge($value, array('user' => $viewer->getIdentity(), 'is_sponsored' => true)));
      $this->view->view_type = 'playlist';
    } else {
      return $this->setNoRender();
    }
