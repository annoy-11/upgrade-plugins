  ...
  <div class="sesvideo_view_links">
    ...
    if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesvideosell') && $this->video->owner_id == $viewer_id): ?>
      <a href="" class="fas fa-video"><?php echo $this->translate("View Sample Video"); ?></a>
		<?php endif; ?>
    <?php if(in_array('openVideoLightbox',$this->allowOptions) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbasic.enable.lightbox', 1)){ ?>
  ...
  <div class="sesvideo_view_statics">
    <?php if(($this->getAllowRating == 0 && $this->allowShowRating == 1 && $this->total_rating_average != 0 ) || ($this->getAllowRating == 1) && in_array('rateCount',$this->allowOptions) ){ ?>
      <div id="album_rating" class="sesbasic_rating_star sesvideo_view_rating" onMouseOut="rating_out();">
        <span id="rate_1" class="rating_star_big_generic" <?php  if ($this->viewer_id && $this->ratedAgain && $this->allowMine &&  $this->allowRating):?>onclick="rate(1);"<?php  endif; ?> onMouseOver="rating_over(1);"></span>
        <span id="rate_2" class="rating_star_big_generic" <?php if ( $this->viewer_id && $this->ratedAgain && $this->allowMine && $this->allowRating):?>onclick="rate(2);"<?php endif; ?> onMouseOver="rating_over(2);"></span>
        <span id="rate_3" class="rating_star_big_generic" <?php if ( $this->viewer_id && $this->ratedAgain  && $this->allowMine && $this->allowRating):?>onclick="rate(3);"<?php endif; ?> onMouseOver="rating_over(3);"></span>
        <span id="rate_4" class="rating_star_big_generic" <?php if ($this->viewer_id && $this->ratedAgain && $this->allowMine && $this->allowRating):?>onclick="rate(4);"<?php endif; ?> onMouseOver="rating_over(4);"></span>
        <span id="rate_5" class="rating_star_big_generic" <?php if ($this->viewer_id && $this->ratedAgain  && $this->allowMine && $this->allowRating):?>onclick="rate(5);"<?php endif; ?> onMouseOver="rating_over(5);"></span>
    ...
    <?php if(in_array('likeCount',$this->allowOptions)){ ?>
      ...
      <?php } ?>
      <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesvideo.allowfavv', 1) && in_array('favouriteCount',$this->allowOptions)){ ?>
      ...
      <span>
        <?php 
          ...
          if($viewer_id && $this->video->price > 0 && $this->video->owner_id != $viewer_id && empty($order_id)) {
            echo $this->htmlLink(array('route' => 'default', 'module' => 'sesvideosell', 'controller' => 'order', 'action' => 'process', 'video_id' => $this->video->video_id, 'gateway_id' => 2), $this->translate("Buy This Video"), array('class' => 'sesvideo_sellbutton sesbasic_animation'));
...
<?php  if($this->video->authorization()->isAllowed(Engine_Api::_()->user()->getViewer(), 'comment')){ ?>
    ...
    <?php } ?>
    <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesvideo.allowfavv', 1) && in_array('favouriteButton',$this->allowOptions) && Engine_Api::_()->user()->getViewer()->getIdentity()){ ?>
    ...
    <?php if( Engine_Api::_()->user()->getViewer()->getIdentity() ): ?>
      ...
      <?php  } ?>
      <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('video.enable.report',1) && in_array('reportVideo',$this->allowOptions) && (Engine_Api::_()->user()->getViewer()->getIdentity() != $this->video->owner_id)){ ?>
...
<div class="sesvideo_view_bottom sesbasic_clearfix">
    ...
    <?php } ?>
    <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesvideo.allowfavv', 1) && in_array('favourite',$this->allowOptions) && $this->viewer()->getIdentity() != 0){  ?>
...
<script type="application/javascript">
 function promptPasswordCheck(){
	 sesJqueryObject('#video_content').hide();
	...
	swal({   
			...
			if (inputValue === false) {
				sesJqueryObject('#video_content').remove();
				sesJqueryObject('#locked_content').show();
			...
			if(inputValue.toLowerCase() == '<?php echo strtolower($this->password); ?>'){
					...
					sesJqueryObject('#video_content').show();
					setCookieSesvideo('<?php echo $this->video->video_id; ?>');
					if(sesJqueryObject('.sesvideo_view_embed').find('iframe')){
					...
					swal.close();
					sesJqueryObject('.layout_sesvideo_video_view_page').show();
			}else{
			...
			if(typeof changeiframe != 'undefined'){
				sesJqueryObject('.sesvideo_view_embed').find('iframe').attr('src',sesJqueryObject('.sesvideo_view_embed').find('iframe').attr('src'));
				var aspect = 16 / 9;
