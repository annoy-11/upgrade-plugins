  ...
  public function indexAction() {
    ...
    if (!$is_ajax && is_array($defaultOptionsArray)) {
      ...
      foreach ($defaultOptionsArray as $key => $defaultValue) {
				if (($defaultValue == 'mySPchanel' || $defaultValue == 'mySPaddChannel') && $viewer->getIdentity() != 0 && !$setting->getSetting('video_enable_chanel', 1)) {
					continue;
				}
        ...
        if ($this->_getParam($defaultValue . '_label'))
          ...
          if ($defaultValue == 'mySPvideo')
            ...
            $valueLabel = 'Playlist';
					else if ($defaultValue == 'mySPaddVideo')
            $valueLabel = 'Add Video';
					else if ($defaultValue == 'mySPaddChannel')
            $valueLabel = 'Add Channel';
        }
    ...
    $sesvideo_widget = Zend_Registry::isRegistered('sesvideo_widget') ? Zend_Registry::get('sesvideo_widget') : null;
    if(empty($sesvideo_widget)) {
      return $this->setNoRender();
    }
    if (isset($_GET['openTab']) || $is_ajax) {
      $this->view->defaultOpenTab = $defaultOpenTab = (isset($_GET['openTab']) && $_GET['openTab'] != 'undefined'  ? $_GET['openTab'] : ($this->_getParam('openTab') != NULL && (isset($_GET['openTab'] ) && $_GET['openTab'] != 'undefined') ? $this->_getParam('openTab') : (isset($params['openTab']) ? $params['openTab'] : '' )));
    }
    ...
    if ($popularCol == 'my_videos') {
      ...
      $paginator = $profileVideo = Engine_Api::_()->getDbTable('videos', 'sesvideo')->getVideo(array('user_id' => $userId));
    ...
    //if( $this->_getParam('titleCount', false) && $profileVideo->getTotalItemCount() > 0 ) {
