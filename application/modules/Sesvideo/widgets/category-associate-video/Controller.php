...
class Sesvideo_Widget_CategoryAssociateVideoController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->identityForWidget = $identityForWidget = isset($_POST['identity']) ? $_POST['identity'] : '';
     $popularity_video = isset($params['popularity_video']) ? $params['popularity_video'] : $this->_getParam('popularity_video', 'like_count');
     if($popularity_video)
    $this->view->popularity_video = $popularity_video = str_replace('SP', '_', $popularity_video);
    ...
    $sesvideo_categories = Zend_Registry::isRegistered('sesvideo_categories') ? Zend_Registry::get('sesvideo_categories') : null;
    if(empty($sesvideo_categories)) {
    ...
    if ($paginatorCategory->getTotalItemCount() > 0) {
      foreach ($paginatorCategory as $key => $valuePaginator) {
        ...
        $videoData['video_data'][$valuePaginator->category_id] = Engine_Api::_()->getDbTable('videos', 'sesvideo')->getVideo(array('category_id' => $valuePaginator->category_id, 'status' => 1, 'limit_data' => $video_limit, 'popularCol' => $popularCol,'search' => 1 ,'fixedData' => $fixedData), false);
      }
    } else {
      if (!$is_ajax)
        return $this->setNoRender();
