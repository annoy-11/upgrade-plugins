...
sesJqueryObject(document).on('click','.sesvideo_home_btn',function () {
    var data = sesJqueryObject(this).data('action');
    if(data){
 ...
 function showtypevalue(value) {
    if(value == 'carouselview')
      sesJqueryObject('#limitCarousel-wrapper').show();
    else
      sesJqueryObject('#limitCarousel-wrapper').hide();
  }
