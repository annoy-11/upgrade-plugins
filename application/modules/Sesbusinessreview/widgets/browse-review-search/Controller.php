 ...
 */
class Sesbusinessreview_Widget_BrowseReviewSearchController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (count($viewOptions))
      ...
      $view = false;
    $this->view->subject_id = $this->_getParam('business_id', 0);
    $this->view->widgetIdentity = $this->_getParam('widgetIdentity', 0);

    $this->view->form = $formFilter = new Sesbusinessreview_Form_Review_Browse(array('reviewTitle' => $this->_getParam('review_title', 1), 'reviewSearch' => $this->_getParam('review_search', 1), 'reviewStars' => $this->_getParam('review_stars', 1), 'reviewRecommended' => $view));
    if ($formFilter) {
