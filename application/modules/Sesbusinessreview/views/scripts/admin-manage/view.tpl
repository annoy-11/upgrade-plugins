...
<div class="sesbasic_view_stats_popup">
  ...
  <table>
    ...
    <tr>
      <td>Rating:</td>
      <td class="sesbusinessreview_view_details_rating">
        ...
        <?php $reviewParameters = Engine_Api::_()->getDbtable('parametervalues', 'sesbusinessreview')->getParameters(array('content_id'=>$this->item->getIdentity(),'business_id'=>$this->item->business_id)); ?>
        <?php if(count($reviewParameters)>0){ ?>
            <?php foreach($reviewParameters as $reviewP){ ?>
            <div class="sesbasic_clearfix">
              <div class="sesbusinessreview_rating_parameter_label"><?php echo $reviewP['title']; ?></div>
