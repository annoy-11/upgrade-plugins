...
class Sesbusinessreview_Form_Admin_Parameter_Add extends Engine_Form {
  ...
  public function init() {
    ...
    if (count($reviewParameters)) {
      foreach ($reviewParameters as $val) {
        $this->addElement('Text', 'sesbusinessreview_review_' . $val['parameter_id'], array(
            'label' => '',
            'class' => 'sesbusinessreview_added_parameter',
            'allowEmpty' => true,
            'value' => $val['title'],
            'required' => false,
            'maxlength' => "255",
