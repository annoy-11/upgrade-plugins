 ...
 * @version    $Id: Polls.php  2018-10-25 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sespagepoll_Model_DbTable_Polls extends Core_Model_Item_DbTable_Abstract
{
  protected $_rowClass = 'Sespagepoll_Model_Poll';
  public function getPollSelect($params = array())
  {
    ...
    $table = Engine_Api::_()->getItemTable('sespagepoll_poll');
    ...
    if(isset($params['searchpage']) && !empty($params['searchpage'])){
      $select->join($pageTableName, $pageTableName . '.page_id = ' . $tableName . '.page_id',null)
        ->where($pageTableName.'.title LIKE ?','%' . $params['searchpage'] . '%');
    }
    if( isset($params['page_id']) && $params['page_id'] != null && $params['page_id'] >0 ) {
