...
<?php if(isset($this->identityForWidget) && !empty($this->identityForWidget)):
	...
	<?php if(!$this->is_ajax): ?>
	<div class="sespage_profile_tab_wrapper sespagepoll_profile_poll sesbasic_bxs">
  ...
  <?php if($this->allowPoll && $this->canUpload && !$this->is_ajax): ?>
		<div class="_addbtn">
			<a class="sesbasic_button sesbasic_icon_add sespage_cbtn" href="<?php echo $this->url(array('module' => 'sespagepoll', 'action' =>
			'create', 'page_id' => $this->page_id), 'sespagepoll_general', true); ?>">
  ...
  <?php endif ?>
	...
	<?php if(!$this->is_ajax): ?>
		<div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" style="position: relative;" class="sesbasic_clearfix sesbasic_bxs clear">
			<ul class="sespagepoll_poll_listing sesbasic_clearfix <?php echo $gridclass; ?> clear" id="tabbed-widget_<?php 	echo $randonNumber; ?>" style="min-height:50px;">
			<?php endif ?>
								...
								<div class="_btnsleft">
									...
									<?php if($viewerId ):?>
										...
										<?php if( Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagepoll.allow.favourite', 1) && in_array("favouriteButton", $this->show_criteria)):?>
											...
											<a href="javascript:;" data-type="favourite_view" data-url="<?php echo $poll->poll_id ; ?>" class="sesbasic_icon_btn favrite sesbasic_icon_btn_count sespagepoll_fav sesbasic_icon_fav_btn sespagepoll_favourite_<?php echo $poll->poll_id ?> sespagepoll_likefavfollow <?php echo ($favouriteStatus) ? 'button_active' : '' ; ?>"><i class="fa fa-heart"></i><span><?php echo $poll->favourite_count;?></span></a>
										<?php endif;?>
									<?php endif;?>
				...
				if($this->load_content == 'pagging' && $this->show_limited_data == 'no'): ?>
					<?php echo $this->paginationControl($this->paginator, null, array("_pagging.tpl", "sespagepoll"),array('identityWidget'=>$randonNumber)); ?>
				<?php endif ?>
			<?php if(!$this->is_ajax): ?>
			</ul>
 ...
 <?php endif ?>
   <script type="text/javascript">
     ...
     function paggingNumber<?php echo $randonNumber; ?>(pageNum){
       (new Request.HTML({
         method: 'post',
         'url': en4.core.baseUrl + 'widget/index/mod/sespagepoll/name/<?php echo $this->widgetName; ?>',
         'data': {
           format: 'html',
           page: pageNum,
 ...
 <?php } ?>
     ...
     function pollSearch() {
       ...
       onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
         ...
         if($('loadingimgsespagepoll-wrapper'))
           sesJqueryObject('#loadingimgsespagepoll-wrapper').hide();
         document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML =  responseHTML;
       }
     }));
