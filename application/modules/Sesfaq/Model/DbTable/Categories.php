...
class Sesfaq_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Sesfaq_Model_Category';
  ...
  public function getCategory($params = array(), $customParams = array()) {
    ...
    $faqTable = Engine_Api::_()->getDbTable('faqs', 'sesfaq')->info('name');
    $category_select = $category_select->setIntegrityCheck(false);
    $category_select = $category_select->joinLeft($faqTable, "$faqTable.category_id=$tableName.category_id", array("total_faq_categories" => "COUNT($faqTable.faq_id)"));

    $category_select  = $category_select->group("$tableName.category_id");
    
    if(isset($params['criteria']) && $params['criteria'] == 'most_faq')
      $category_select->order('total_faq_categories DESC');
    ...
    if(isset($params['hasFaq'])){
      $category_select = $category_select->having('total_faq_categories > 0');
    }
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('sesfaq_category');
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countFaqs'])) {
      $faqTable = Engine_Api::_()->getDbTable('faqs', 'sesfaq')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($faqTable, "$faqTable.subcat_id=$tableName.category_id", array("total_faqs_categories" => "COUNT($faqTable.photo_id)"));
      $category_select = $category_select->group("$tableName.category_id");
      $category_select->order('total_faqs_categories DESC');
    }
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countFaqs'])) {
      $faqTable = Engine_Api::_()->getDbTable('faqs', 'sesfaq')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($faqTable, "$faqTable.subsubcat_id=$tableName.category_id", array("total_faqs_categories" => "COUNT($faqTable.faq_id)"));
