...
<?php $route = 'sesfaq_general'; ?>
<ul class="sesfaq_sidebar_categories sesfaqc_sidebar_widget">
  <?php foreach( $this->categories as $item ): ?>
    <li>
      <?php $subcategory = Engine_Api::_()->getDbtable('categories', 'sesfaq')->getModuleSubcategory(array('column_name' => "*", 'category_id' => $item->category_id)); ?>
      <?php if(counT($subcategory) > 0): ?>
        <a id="sesfaq_toggle_<?php echo $item->category_id ?>" class="cattoggel cattoggelright fa" href="javascript:void(0);" onclick="showCategory('<?php echo $item->getIdentity()  ?>')"></a>
      <?php endif; ?>
      <?php 
        if($item->cat_icon) { 
          ...
          if($cat_icon) {
            ...
            $cat_icon = 'application/modules/Sesfaq/externals/images/category.png';
          }
        } else {
          $cat_icon = 'application/modules/Sesfaq/externals/images/category.png';
        }
      ...
      <ul id="subcategory_<?php echo $item->getIdentity() ?>" style="display:none;">          
        <?php foreach( $subcategory as $subCat ): ?>
          <li>
            <?php $subsubcategory = Engine_Api::_()->getDbtable('categories', 'sesfaq')->getModuleSubsubcategory(array('column_name' => "*", 'category_id' => $subCat->category_id)); ?>
            <?php if(counT($subsubcategory) > 0): ?>
              <a id="sesfaq_subcat_toggle_<?php echo $subCat->category_id ?>" class="cattoggel cattoggelright fa" href="javascript:void(0);" onclick="showCategory('<?php echo $subCat->getIdentity()  ?>')"></a>
            <?php endif; ?> 
...
function showCategory(id) {
  if($('subcategory_' + id)) {
    if ($('subcategory_' + id).style.display == 'block') {
      $('sesfaq_toggle_' + id).removeClass('cattoggel cattoggeldown');
      $('sesfaq_toggle_' + id).addClass('cattoggel cattoggelright');
      $('subcategory_' + id).style.display = 'none';
    } else {
      $('sesfaq_toggle_' + id).removeClass('cattoggel cattoggelright');
      $('sesfaq_toggle_' + id).addClass('cattoggel cattoggeldown');
  ...
  if($('subsubcategory_' + id)) {
    if ($('subsubcategory_' + id).style.display == 'block') {
      $('sesfaq_subcat_toggle_' + id).removeClass('cattoggel cattoggeldown');
      $('sesfaq_subcat_toggle_' + id).addClass('cattoggel cattoggelright');      
      $('subsubcategory_' + id).style.display = 'none';
    } else {
      $('sesfaq_subcat_toggle_' + id).removeClass('cattoggel cattoggelright');
      $('sesfaq_subcat_toggle_' + id).addClass('cattoggel cattoggeldown');
