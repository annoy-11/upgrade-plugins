...
class Sesfaq_AdminManageaskquestionController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesfaq_admin_main', array(), 'sesfaq_admin_main_manageaskquestion');
    
		$this->view->viewer_id = Engine_Api::_()->user()->getViewer()->getIdentity();

    $page = $this->_getParam('page', 1);
    
    $this->view->formFilter = $formFilter = new Sesfaq_Form_Admin_FilterAskQuestion();
    
    if ($this->getRequest()->isPost()) {
      $values = $this->getRequest()->getPost();
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $askquestion = Engine_Api::_()->getItem('sesfaq_question', $value);
          $askquestion->delete();
        }
      }
    }
    ...
    $askquesitonsTable = Engine_Api::_()->getDbtable('askquestions', 'sesfaq');
    ...
    $this->view->form = $form = new Sesfaq_Form_Admin_Answerquestion();
    ...
    if(!$form->isValid($this->getRequest()->getPost())) return;
        ...
        $askquestion->save();
			...
			Engine_Api::_()->getApi('mail', 'core')->sendSystem($askquestion->email, 'SESFAQ_ASKANSWER_EMAIL', array(
  ...
  public function deleteAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db->beginTransaction();
      try {
        $faq = Engine_Api::_()->getItem('sesfaq_askquestion', $id);
        $faq->delete();
        $db->commit();
      } catch (Exception $e) {
        $db->rollBack();
