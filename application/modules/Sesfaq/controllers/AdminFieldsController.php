...
class Sesfaq_AdminFieldsController extends Fields_Controller_AdminAbstract {

  protected $_requireProfileType = true;
  protected $_fieldType = 'sesfaq_faq';
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesfaq_admin_categories', array(), 'sesfaq_admin_main_subfields');
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Sesfaq_Form_Admin_Settings_Type();
    if (!$this->getRequest()->isPost()) {
      return;
    }
    if (!$form->isValid($this->getRequest()->getPost())) {
    ...
    if ($form->getValue('duplicate') != 'null') {
      ...
      $field_meta_array = $db->select()
              ->from('engine4_sesfaq_faq_fields_meta')
              ->where('field_id IN (' . implode(', ', $child_id_array) . ')')
              ->query()
              ->fetchAll();
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_sesfaq_faq_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_sesfaq_faq_fields_maps', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      Engine_Api::_()->getDbtable('categories', 'sesfaq')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
