 ...
 * @version    $Id: editphotos.tpl 2019-08-28 00:00:00 socialnetworking.solutions $
...
<div class="layout_middle">
  ...
  <div class="sesbasic_clearfix">
    <h3>
      <?php echo $this->htmlLink(Engine_Api::_()->eclassroom()->getHref($this->album->getIdentity()), $this->album->getTitle()) ?>
      (<?php echo $this->translate(array('%s photo', '%s photos', $this->album->count()),$this->locale()->toNumber($this->album->count())) ?>)
    </h3>
  <?php if( $this->paginator->count() > 0 ): ?>
  <?php endif; ?>
  <div class="eclassroom_manage_photos_wrapper sesbasic_clearfix sesbasic_bxs" id="scrollHeightDivSes_<?php echo $randonNumber; ?>">
  <form action="<?php echo $this->escape($this->form->getAction()) ?>" name="editPhotos" method="<?php echo $this->escape($this->form->getMethod()) ?>">
    ...
    <ul class='eclassroom_manage_photos' id="tabbed-widget_<?php echo $randonNumber; ?>">
  <?php } ?>
      <?php foreach( $this->paginator as $photo ): ?>
        <li class="eclassroom_manage_photos_list" id="thumbs-photo-<?php echo $photo->photo_id ?>">
          <div class="sesbasic_clearfix sesbm">
            <div class="eclassroom_manage_photos_list_photo">
              <?php $url = $photo->getPhotoUrl('thumb.normalmain'); ?>
              <span style="background-image:url(<?php echo $url ?>);"></span>
            </div>
            <div class="eclassroom_manage_photos_list_info">
              <?php
                $key = $photo->getGuid();
                echo $this->form->getSubForm($key)->render($this);
              ?>
              <div class="eclassroom_editphotos_cover">
  ...
  <?php if(!$this->is_ajax){ ?>
    ...
    <?php echo $this->htmlLink('javascript:void(0);', $this->translate('View More'), array('id' => "feed_viewmore_link", 'class' => 'sesbasic_animation eclassroom_link_btn fa fa-repeat')); ?> </div>
...
<?php } ?>
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "eclassroom/album/editphotos/album_id/<?php echo $this->album_id; ?>",
      'data': {
        format: 'html',
