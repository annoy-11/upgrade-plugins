...
<table class='admin_table'>
  <thead>
    <tr>
      ...
      <th><a href="javascript:void(0);" onclick="javascript:changeOrder('title', 'ASC');"><?php echo $this->translate("Store Title") ?></a></th>
  ...
  <tbody>
    <?php foreach ($this->paginator as $item): ?>
      <tr>
        <?php $classroomItem = Engine_Api::_()->getItem('classroom', $item->classroom_id);?>
        <?php $UserItem = Engine_Api::_()->getItem('user', $item->user_id);?>
        <?php if(!$classroomItem || !$UserItem) continue; ?>
        <td><?php echo $item->getIdentity() ?></td>
        <td><a href="<?php echo $classroomItem->getHref(); ?>" target="_blank"><?php echo $classroomItem->getTitle() ?></a></td>
        <td><a href="<?php echo $UserItem->getHref(); ?>" target="_blank"><?php echo $UserItem->getTitle() ?></a></td>
				<td class="admin_table_centered">
					...
					<?php else: ?>
						<?php echo $this->htmlLink(array('route' => 'default', 'module' => 'eclassroom', 'controller' => 'admin-manage', 'action' => 'approve-claim', 'id' => $item->claim_id), $this->htmlImage($this->layout()->staticBaseUrl . 'application/modules/Sesbasic/externals/images/icons/error.png', '', array('title'=> $this->translate('Approve Claim Request'))), array('class' => 'smoothbox')) ?>
					<?php endif; ?>
				</td> 
        <td><?php echo date('Y-m-d H:i:s',strtotime($item->creation_date)) ?></td>
        <td>
	        <?php echo $this->htmlLink(
	  array('route' => 'default', 'module' => 'eclassroom', 'controller' => 'admin-manage', 'action' => 'show-detail', 'id' => $item->claim_id),
