...
<?php  if($this->createLimit == 1): ?>
  ...
  <?php } ?>
  <?php if(count($_POST) == 0 && Engine_Api::_()->getApi('settings','core')->getSetting('eclassroom.category.selection', 0)): ?>
       <?php if (Engine_Api::_()->core()->hasSubject('eclassroom')):?>
      <?php $classroom = Engine_Api::_()->core()->getSubject();?>
    <?php endif;?>
    <div class="eclassroom_create_step_container sesbasic_bxs sesbasic_clearfix">
      <h3><?php echo $this->translate('Create New Classroom');?></h3>
      <p><?php echo $this->translate("It's easy to set up. Just choose a Classroom category to get started.");?></p>
      <?php $iconType = Engine_Api::_()->getApi('settings','core')->getSetting('eclassroom.category.icon');?>
      <?php if($iconType == 0):?>
      ...
      <?php elseif($iconType == 2):?>
        <?php $icon = 'thumbnail';?>
      <?php endif;?>
      <div class="eclassroom__create_categories_listing">
        <?php foreach($this->categories as $category): ?>
          <div class="eclassroom_create_category">
            <section class="">
              <div class="_inner">
                <div class="_step1">
                  <?php if($this->quickCreate):?>
                    <a href="javascript:;" class="sesbasic_linkinherit eclassroom_create_category_toggle">
                  <?php else:?>
                  ...
                  <?php else:?>
                    <i style="background-image:url(application/modules/Courses/externals/images/classroom-icon-big.png);"></i>
                  <?php endif;?>
                ...
                <?php if($this->quickCreate):?>
                  <div class="_step2">
                    <form class='quick_classroom_create' action="eclassroom/index/create/parent_id/<?php echo $this->parent_id;?>" id="quick_classroom_create_<?php echo $category->category_id;?>" method="post" enctype="multipart/form-data">
                    ...
                    <div><input type="text" class="title" id="title_<?php echo $category->category_id;?>" name='title' class="reset_form_value" placeholder="Classroom Title" /></div>
                    <?php $custom_url_value = isset($classroom->custom_url) ? $classroom->custom_url : (isset($_POST["custom_url"]) ? $_POST["custom_url"] : "");;?>
                    <div class="_url">
                      <input type="text" class="classroom_custom_url reset_form_value" name="custom_url" id="custom_url_<?php echo $category->category_id;?>" value="<?php echo $custom_url_value;?>">
                      <span class="classroom_check_availability_btn">
                          <button id="check_custom_url_availability" class="check_custom_url_availability" type="button" name="check_availability" ><i class="fa fa-check" id="classroom_custom_url_correct" style=""></i><i class="fa fa-close" id="classroom_custom_url_wrong" style="display:none;"></i><img src="application/modules/Core/externals/images/loading.gif" id="classroom_custom_url_loading" alt="Loading" style="display:none;" />
                        <samp class="availability_tip"><?php echo $this->translate("Check Availability");?></samp></button>
                      </span>
                     </div>
                    <?php $subcategory = Engine_Api::_()->getDbTable('categories','eclassroom')->getModuleSubcategory(array('category_id' => $category->category_id, 'column_name' => '*'));?>
                    ...
                    <?php endif;?>
                    <div style='display:none;' class='quick_classroom_subsubcat'><select></select></div>
                    <div style='display:none;' class="_loading" id='classroom_sussubcat_loading'><i class="fa fa-spinner fa-spin"></i></div>
    ...
    <script type="text/javascript">
      sesJqueryObject(document).ready(function(){
        sesJqueryObject(document).on('click',".eclassroom_create_category_toggle",function(){
          checkTitleChnage = false; 
          changeTitle = true;
          sesJqueryObject('.reset_form_value').val('');
          sesJqueryObject('.reset_form_value').trigger('change');
          sesJqueryObject('.eclassroom_create_category').removeClass('openbox');
          sesJqueryObject(this).closest('.eclassroom_create_category').toggleClass("openbox");
        });
      });
    </script>    
  <?php endif; ?>
  <div class="eclassroom_create_container <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('eclassroom.create.form', 1)): ?>eclassroom_create_form<?php endif;?>">
    <?php echo $this->form->render(); ?>
    <div class="classroom_create_loading classroom_create_overlay" style="display:none;">
      <div class="classroom_create_overlay_cont">
        <i class="fa fa-spinner fa-pulse fa-3x fa-fw margin-bottom"></i>
        <span class="_text"><?php echo $this->translate('Creating Classroom ...');?></span>
      </div>
    </div>
    <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('classroom.guidelines', 1) && !empty(Engine_Api::_()->getApi('settings', 'core')->getSetting('classroom.message.guidelines', ''))):?>
      <div id="classroom_create_tips" class="classroom_create_tips">
        ...
        <div class="create_tips_bottom_sec">
          <div class="sesbasic_html_block">
            <?php echo Engine_Api::_()->getApi('settings','core')->getSetting('classroom.message.guidelines', '');?>
          </div>
        </div>
      </div>
    <?php endif; ?>
...
<script type="application/javascript">
  ...
  en4.core.runonce.add(function(){
      if(sesJqueryObject('.eclassroom_create_step_container').length > 0){
        sesJqueryObject('.eclassroom_create_container').hide();
        sesJqueryObject('.eclassroom_create_step_container').show();
      }else{
        sesJqueryObject('.classroom_create_tips').show();
        sesJqueryObject('.eclassroom_create_container').show();
        sesJqueryObject('.eclassroom_create_step_container').hide();
      }
  });
  function selectCat(value){ alert();
    sesJqueryObject('.eclassroom_create_container').show();
    sesJqueryObject('.eclassroom_create_step_container').hide();
    sesJqueryObject('#eclassroom_create_form').find('#category_id').val(value);
    sesJqueryObject('#eclassroom_create_form').find('#category_id').trigger('change');
  ...
  sesJqueryObject(function() {
    ...
    var $el = sesJqueryObject('#classroom_create_tips');
    var positionInitial = sesJqueryObject('#title').offsetTop;
    sesJqueryObject('<style>#classroom_create_tips{top:'+positionInitial+'px;}</style>').appendTo(document.head);
  ...
  en4.core.runonce.add(function() {
    ...
    sesJqueryObject("#title, .title").keyup(function(){
      ...
      if(sesJqueryObject(this).closest('.quick_classroom_create').length){
          sesJqueryObject(sesJqueryObject(this).closest('.quick_classroom_create').find('._url').find('.classroom_custom_url')).val(Text);
      }
      else {
          sesJqueryObject("#custom_url").val(Text);
      }
    });
    sesJqueryObject("#custom_url, .classroom_custom_url").keydown(function(){
      checkTitleChnage = true;
      changeTitle = false;
    });
    var checkTitleChnage = false;
    sesJqueryObject("#title,.title").blur(function(){
    ...
    sesJqueryObject("#custom_url, .classroom_custom_url").blur(function(){
      validUrl = false;
      if(!checkTitleChnage)
        changeTitle = false;
...
sesJqueryObject('#check_custom_url_availability, .check_custom_url_availability').click(function(){
  ...
  if(!custom_url_value)
      return;
  sesJqueryObject(this).find('#classroom_custom_url_wrong').hide();
  sesJqueryObject(this).find('#classroom_custom_url_correct').hide();
  sesJqueryObject(this).find('#classroom_custom_url_loading').css('display','inline-block');
  var obj = this;
  sesJqueryObject.post('<?php echo $this->url(array('module'=>'eclassroom','controller' => 'ajax', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value},function(response){
              sesJqueryObject(obj).find('#classroom_custom_url_loading').hide();
              response = sesJqueryObject.parseJSON(response);
              if(response.error){
                  validUrl = false;
                  sesJqueryObject(obj).find('#classroom_custom_url_correct').hide();
                  sesJqueryObject(obj).find('#classroom_custom_url_wrong').css('display','inline-block');
                  sesJqueryObject(obj).addClass('_notavailable').removeClass('_available');
                      ...
                      sesJqueryObject(obj).find('#classroom_custom_url_wrong').hide();
                      sesJqueryObject(obj).find('#classroom_custom_url_correct').css('display','inline-block');
...
sesJqueryObject(document).on('submit','.quick_classroom_create',function(e) {
  var cat_id = sesJqueryObject(this).attr('id').replace('quick_classroom_create_','');
  if(validation == true) {
    return true;
  }
  e.preventDefault();
  if(sesJqueryObject('#title_'+cat_id).val() == "") {
  ...
  sesJqueryObject.post('<?php echo $this->url(array('controller' => 'ajax','module'=>'eclassroom', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value},function(response){
    response = sesJqueryObject.parseJSON(response);
    if(response.error) {
      ...
      validation = true;
      sesJqueryObject('#quick_classroom_create_'+cat_id).trigger('submit');
      return true;
    }
  });
  return false;
...
var getProfileType = function(category_id) {
  var mapping = <?php echo Zend_Json_Encoder::encode(Engine_Api::_()->getDbTable('categories', 'eclassroom')->getMapping(array('category_id', 'profile_type'))); ?>;
...
en4.core.runonce.add(function() {
  var defaultProfileId = '<?php echo '0_0_' . $this->defaultProfileId ?>' + '-wrapper';
    ...
    if(sesJqueryObject('#sessmoothbox_main').length){
      sesJqueryObject('#eclassroom_create_form').find('div').find('div').find('.form-elements').find('.parent_0').closest('.form-wrapper').hide() ;
    }
  }
});
...
function showQuickSubSubCategory(cat_id,object) {
    sesJqueryObject('#classroom_sussubcat_loading').show();
    ...
    (new Request.HTML({
        ...
        onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        if(responseHTML != ''){
            sesJqueryObject('.quick_classroom_subsubcat').show();
            sesJqueryObject(object).parent().parent().find('.quick_classroom_subsubcat').html("<select><option value=''>"+en4.core.language.translate("3rd-level Category")+"</option>"+responseHTML+"></select>");
        }
        else {
            sesJqueryObject(object).parent().parent().find('.quick_classroom_subsubcat').html("");
            sesJqueryObject('.quick_classroom_subsubcat').hide();
        }
        sesJqueryObject('#classroom_sussubcat_loading').hide();
        }
    })).send();  
}
...
en4.core.runonce.add(function(){
  formObj = sesJqueryObject('#eclassroom_create_form').find('div').find('div').find('div');
...
en4.core.runonce.add(function()
{
  ...
  obj.click(function(e){
      ...
      sesJqueryObject('#classroom_main_photo_preview').attr('src','');
...
function validateForm(){
        ...
        counter = 0;
        sesJqueryObject('#eclassroom_create_form input, #eclassroom_create_form select,#eclassroom_create_form checkbox,#eclassroom_create_form textarea,#eclassroom_create_form radio').each(
        function(index){
...
en4.core.runonce.add(function() {
  sesJqueryObject('#eclassroom_create_form').submit(function(e){
    ...
    if(validationFm) {
      ...
      if(typeof objectError != 'undefined'){
        ...
        <?php }else{ ?>
        sesJqueryObject('#eclassroom_create_form').animate({scrollTop: errorFirstObject.offset().top}, 2000);
        <?php } ?>
      }
      return false;	
    }
      ...
      if(showErrorMessage != ''){
        ...
        sesJqueryObject('#courses_error_time-element').text(showErrorMessage);
        var errorFirstObject = sesJqueryObject('.classroom_choose_date');
        <?php if(!$this->typesmoothbox){ ?>
        sesJqueryObject('html, body').animate({scrollTop: errorFirstObject.offset().top}, 2000);
        <?php }else{ ?>
      ...
      if(!validUrl){
        ...
        sesJqueryObject('.classroom_create_loading').show();
        sesJqueryObject('.eclassroom_create_form').addClass('_success');
  ...
  <?php if($this->typesmoothbox) { ?>
   <script type="application/javascript">
     ...
     en4.core.runonce.add(function() {
       sesJqueryObject('#eclassroom_create_form').attr('action',sesJqueryObject('#eclassroom_create_form').attr('action').replace('?format=html&typesmoothbox=sessmoothbox',''));   
       tinymce.init({
 ...
 <?php die;} ?>
<?php else: ?>
    <div class="tip clearfix classroom_error">
        <img src="application/modules/courses/externals/images/classroom-icon-error.png" alt="">
        <span><?php echo $this->translate("You have reached the limit of Classroom creation. Please contact to the site administrator.");?></span>
    </div>
<?php endif; ?>
