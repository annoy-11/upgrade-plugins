  ...
  <form method="post" id="multidelete_form" onsubmit="return multiDelete();" >
    <table class='admin_table'>
        <thead>
            ...
            <th><?php echo $this->translate("Classroom Name") ?></th>
        ...
        <tbody id="manage_course_table">
            ...
            <tr id="course_id_<?php echo $item->course_id; ?>">
                ...
                <?php $classroom = !empty($item->classroom_id) ? Engine_Api::_()->getItem('classroom', $item->classroom_id) : 0; ?>
                <td>
                  <?php if($classroom): ?>
                      <?php echo $this->htmlLink($classroom->getHref(),$classroom->getTitle(),array('title'=>$classroom->getTitle())  ) ?>
                  <?php else: ?>
                    --
                  <?php endif; ?>
                </td>
                ...
                <td>
                <?php echo $this->htmlLink(array('route' => 'eclassroom_dashboard', 'classroom_id' => $this->classroom->custom_url,'course_id'=>$item->getIdentity(),'action'=>'link-course'), $this->translate('Map Course'), array('target'=> "_blank","class"=>"map_course")) ?> |
...
sesJqueryObject(document).on('click','.map_course',function(event){
	event.preventDefault();
  sesJqueryObject(this).html('<img src="application/modules/Core/externals/images/loading.gif" alt="Loading" />');
  new Request.HTML({
			method: 'post',
			url :  sesJqueryObject(this).attr('href'),
			onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) { 
        var obj = jQuery.parseJSON(responseHTML);
        if(obj.status == "1"){
          sesJqueryObject("#course_id_"+obj.course_id).remove();
        } else {
           sesJqueryObject(this).html("Mapp Course"); 
        }
			}
	}).send();
});
