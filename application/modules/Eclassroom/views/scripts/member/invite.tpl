 ...
 * @version    $Id: invite.tpl 2019-08-28 00:00:00 socialnetworking.solutions $
...
<script type="text/javascript">
  ...
  function viewMore() {
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'eclassroom/member/invite/classroom_id/' + '<?php echo $this->classroom->classroom_id; ?>' ,
      'data': {
        format: 'html',
...
<?php if( $this->paginator->getTotalItemcount() > 0 ): ?>
  ...
  <form method="post" id="eclassroominviteform" data-src="<?php echo $this->classroom->classroom_id; ?>">
    <div class="eclassroom_invite_popup sesbasic_bxs">
      <div class="_header">
        <?php echo $this->translate('Invite Members') ?>
      </div>
      <div class="_content">
      <p class="_des"><?php echo $this->translate('Choose the people you want to invite to this classroom.') ?></p>
      <?php if( $this->paginator->getTotalItemcount() > 1 ): ?>
          ...
          <span class="sesbasic_button _sabtn">
            ...
            <label for="selectall"><?php echo $this->translate('Select All') ?></label>
          </span>
        </div>
      <?php endif;?>
    ...
    <?php if (empty($this->viewmore)): ?>
          ...
          <?php if (!empty($this->paginator) && $this->paginator->count() > 1 && $this->paginator->getCurrentPageNumber() < $this->paginator->count()): ?>
            <div class="sesbasic_load_btn" id="view_more" onclick="viewMore();" >
               <a href="javascript:void(0);" class="sesbasic_animation eclassroom_link_btn" id="feed_viewmore_link"><i class="fa fa-repeat"></i><span><?php echo $this->translate('View More');?></span></a>
            </div>
