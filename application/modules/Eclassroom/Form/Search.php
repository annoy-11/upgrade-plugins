 ...
 */
class Eclassroom_Form_Search extends Engine_Form {
  ...
  public function init() {
    ...
    $hideClass = 'eclassroom_widget_advsearch_hide_' . $identity;
    $params = Engine_Api::_()->eclassroom()->getWidgetParams($identity);
    ...
    $this->setAction($view->url(array('module' => 'eclassroom', 'controller' => 'index', 'action' => 'index'), 'default', true));
    $viewer = Engine_Api::_()->user()->getViewer();
    if (in_array('searchClassroomTitle', $show_criterias)) {
      $this->addElement('Text', 'search', array(
          'label' => $translate->translate('Search Classrooms'),
          'class' => in_array('searchClassroomTitle', $show_advSearch) ? $hideClass : '',
      ));
    }
    ...
    if (isset($show_criterias) && in_array('browseBy', $show_criterias)) {
      switch ($params['default_search_type']) {
        ...
        case 'mostSPfollower':
          ...
          break;
        case 'mostSPcourses':
          $default_search_type = 'course_count';
          break;
        case 'featured':
      ...
      $this->addElement('Select', 'sort', array(
          'label' => $translate->translate('Browse By'),
    ...
    if (in_array('view', $show_criterias)) {
      ...
      foreach ($params['criteria'] as $key => $viewOption) {
        ...
        switch ($columnValue) {
          case '0':
            $value = $translate->translate('All Classrooms');
            break;
          case '1':
            $value = $translate->translate('Only My Friend\'s Classrooms');
            break;
          case '2':
            $value = $translate->translate('Only My Network Classrooms');
            break;
          case '3':
            $value = $translate->translate('Only My Classrooms');
            break;
          case '4':
            $value = $translate->translate('Only 5 Star Reviews');
            break;
          case '5':
            $value = $translate->translate('Only 4 Star Reviews');
            break;
          case '6':
            $value = $translate->translate('Only 3 Star Reviews');
            break;
          case '7':
            $value = $translate->translate('Only 2 Star Reviews');
            break;
          case '8':
            $value = $translate->translate('Only 1 Star Reviews');
            break;
        }
        $filterOptions[$columnValue] = ucwords($value);
      }
    ...
    if (in_array('location', $show_criterias) && Engine_Api::_()->getApi('settings', 'core')->getSetting('eclassroom_enable_location', 1)) {
    ...
    $this->addElement('Hidden', 'classrooms', array(
        'order' => 100
    ));
