 ...
 */
class Eclassroom_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Eclassroom_Model_Category';
  ...
  public function getCategory($params = array())
  {
    ...
    if (isset($params['criteria']) && $params['criteria'] == 'most_classroom')
      $category_select->order('total_classroom_categories DESC');
      
     if (isset($params['category_id']) && !empty($params['category_id']))
      $category_select = $category_select->where($tableName . '.category_id = ?', $params['category_id']);

    $classroomTableName = Engine_Api::_()->getDbTable('classrooms', 'eclassroom')->info('name');
    $category_select = $category_select->setIntegrityCheck(false);
    $category_select = $category_select->joinLeft($classroomTableName, "$classroomTableName.category_id=$tableName.category_id AND (`is_approved` = 1 AND `" . $classroomTableName . "`.`draft` != 0)", array("total_classroom_categories" => "COUNT($classroomTableName.classroom_id)"));
    ...
    if (isset($params['type'])) {
        if (isset($params['criteria']) && $params['criteria'] == 'most_classroom')
            $category_select->order('total_classroom_categories DESC');
        if (isset($params['criteria']) && $params['criteria'] != 'most_classroom')
            $category_select = $category_select->order('total_classroom_categories DESC');
        if (isset($params['category_id']) && !empty($params['category_id']))
            $category_select = $category_select->where($tableName . '.category_id = ?', $params['category_id']);

        if (isset($params['hasClassroom'])) {
            $currentTime = date('Y-m-d H:i:s');
            $tomorrow_date = date("Y-m-d H:i:s", strtotime("+ 1 day"));
            $nextWeekDate = date("Y-m-d H:i:s", strtotime("+ 7 day"));
            $category_select = $category_select->having('total_classroom_categories > 0');
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('eClassroom_category');
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    $classroomTable = Engine_Api::_()->getDbTable('classrooms', 'eclassroom')->info('name');
    $category_select = $category_select->setIntegrityCheck(false);
    $category_select = $category_select->joinLeft($classroomTable, "$classroomTable.subcat_id=$tableName.category_id", array("total_classroom_categories" => "COUNT($classroomTable.photo_id)"));
    $category_select = $category_select->group("$tableName.category_id");
    $category_select->order('total_classroom_categories DESC');
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['category_id']))
      ...
      $classroomTable = Engine_Api::_()->getDbTable('classrooms', 'eclassroom')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($classroomTable, "$classroomTable.subsubcat_id= $tableName.category_id", array("total_classroom_categories" => "COUNT($classroomTable.classroom_id)"));
