 ...
 */
class Eclassroom_Model_DbTable_Followers extends Engine_Db_Table {

  protected $_rowClass = "Eclassroom_Model_Follower";
  protected $_name = "eclassroom_followers";
  ...
  function getCategoryFollowers($resourceId = "") {
    $select = $this->select()
            ->from($this->info('name'), 'owner_id')
            ->where('resource_type = ?', 'eclassroom_category')
            ->where('resource_id = ?', $resourceId);
    return $this->fetchAll($select);
  }
