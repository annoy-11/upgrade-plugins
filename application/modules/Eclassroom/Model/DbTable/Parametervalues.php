 ...
 * @version    $Id: Parametervalues.php 2019-08-28 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */

class Eclassroom_Model_DbTable_Parametervalues extends Engine_Db_Table {
  protected $_rowClass = 'Eclassroom_Model_Parametervalue';
	protected $_name  = 'eclassroom_review_parametervalues';
	public function getParameters($params = array()){
		$pTable = Engine_Api::_()->getDbtable('parameters', 'eclassroom');
   ...
   // rating functions
    public function getRating($parameter_id) {
        $rating_sum = $this->select()
                ...
                ->query()
                ->fetchColumn(0);
        $total = $this->ratingCount($parameter_id, $resource_type = 'eclassroom_review');
        if ($total)
