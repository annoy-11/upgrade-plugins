 ...
 * @version    $Id: AdminFieldsController.php 2019-08-28 00:00:00 socialnetworking.solutions $
 * @author     socialnetworking.solutions
 */

class Eclassroom_AdminFieldsController extends Fields_Controller_AdminAbstract {
  protected $_requireProfileType = true;
  protected $_fieldType = 'classroom';
  ...
  public function indexAction() {
    ...
    $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_class', array(), 'courses_admin_main_classfields');
    parent::indexAction();
  }
  public function fieldCreateAction() {
    ...
    //$form->addElement('hidden', 'search', array('order'=>'9999','value' => 1));
    $form->getElement('search')->setLabel('Show on Browse Search Widget?');
    $form->getElement('search')->setMultiOptions(array(
        0 => 'Hide on Browse Search Widget',
        1 => 'Show on Browse Search Widget',
    ));
  ...
  public function fieldEditAction() {
    ...
    // Create form
    $this->view->form = $form = new $adminFormClass();
    $form->setTitle('Edit Classroom Category Profile Question');
    $form->removeElement('show');
    $form->getElement('search')->setLabel('Show on Browse Search Widget?');
    $form->getElement('search')->setMultiOptions(array(
        0 => 'Hide on Browse Search Widget',
    ...
    $form->addElement('hidden', 'show', array('value' => 0));
    $form->removeElement('display');
    $linkCount = count(Engine_Api::_()->fields()->getFieldsMaps($this->_fieldType)->getRowsMatching('child_id', $field->field_id));
    if ($linkCount >= 2) {
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Eclassroom_Form_Admin_Settings_Type();
    if (!$this->getRequest()->isPost()) {
      return;
    }
    if (!$form->isValid($this->getRequest()->getPost())) {
    ...
    if ($form->getValue('duplicate') != 'null') {
      ...
      $field_meta_array = $db->select()
              ->from('engine4_eclassroom_fields_meta')
              ->where('field_id IN (' . implode(', ', $child_id_array) . ')')
              ->query()
              ->fetchAll();
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_eclassroom_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_eclassroom_fields_options', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      Engine_Api::_()->getDbTable('categories', 'eclassroom')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
