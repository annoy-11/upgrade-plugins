...
class Eclassroom_AdminReviewController extends Core_Controller_Action_Admin {
  ...
  public function reviewSettingsAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_rvwstngs', array(), 'courses_admin_main_review');
     $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_review', array(), 'courses_admin_main_rvwprmstg');
    $this->view->form = $form = new Eclassroom_Form_Admin_Review_ReviewSettings();
  ...
  public function manageReviewsAction() {
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main', array(), 'courses_admin_main_rvwstngs');
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_rvwstngs', array(), 'courses_admin_main_review');
    $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_review', array(), 'courses_admin_main_clsmngrvw');

    $this->view->formFilter = $formFilter = new Eclassroom_Form_Admin_Review_Filter();
    //Process form
    $values = array();
    ...
    if (!empty($_GET['classroom_title'])){
        $tableClassroomName = Engine_Api::_()->getItemTable('classroom')->info('name');
        $select->joinLeft($tableClassroomName, $tableClassroomName . '.classroom_id = ' . $tableName . '.classroom_id',null);
        $select->where($tableClassroomName.'.title LIKE ?', '%' . $values['classroom_title'] . '%');
    }
  ...
  public function viewAction() {
    $this->view->item = Engine_Api::_()->getItem('eclassroom_review', $this->_getParam('id', null));
  }
  //Delete entry
  public function deleteReviewAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db->beginTransaction();
      try {
        $review = Engine_Api::_()->getItem('eclassroom_review', $this->_getParam('review_id',$this->_getParam('id')));
        $review->delete();
        $db->commit();
      } catch (Exception $e) {
        $db->rollBack();
  ...
  public function levelSettingsAction() {
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main', array(), 'courses_admin_main_rvwstngs');
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_rvwstngs', array(), 'courses_admin_main_review');
    $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_review', array(), 'courses_admin_main_rvwlvl');
    //Get level id
    ...
    $id = $level->level_id;
    //Make form
    $this->view->form = $form = new Eclassroom_Form_Admin_Review_Level(array(
        'public' => ( in_array($level->type, array('public')) ),
        'moderator' => ( in_array($level->type, array('admin', 'moderator')) ),
    ));
    $form->level_id->setValue($id);
    $content_type = 'eclass_review';
    ...
    public function reviewParameterAction() {
        $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main', array(), 'courses_admin_main_rvwstngs');
        $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_rvwstngs', array(), 'courses_admin_main_review');
        $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('courses_admin_main_review', array(), 'courses_admin_main_clsrvwprm');
        $this->view->category = Engine_Api::_()->getDbTable('categories', 'eclassroom')->getCategory(array('column_name' => '*', 'profile_type' => true));
    ...
    public function addParameterAction() {
        ...
        // In smoothbox
        $this->_helper->layout->setLayout('admin-simple');
        $this->view->form = $form = new Eclassroom_Form_Admin_Parameter_Add();
        $reviewParameters = Engine_Api::_()->getDbtable('parameters', 'eclassroom')->getParameterResult(array('category_id' => $category_id));
        ...
        $table = Engine_Api::_()->getDbtable('parameters', 'eclassroom');
        $tablename = $table->info('name');
        try {
        $values = $form->getValues();
        ...
        foreach ($_POST as $key => $value) {
            ...
            continue;
            $id = str_replace('eclassroom_review_', '', $key);
  ...
  public function verifyAction() {
    ...
    $item = Engine_Api::_()->getItem('eclassroom_review', $review_id);
    $item->verified = !$item->verified;
    $item->save();
    $this->_redirect('admin/eclassroom/review/manage-reviews');
  }
   //Featured Action
  ...
  public function ofthedayAction() {
    ...
    $item = Engine_Api::_()->getItem('eclassroom_review', $review_id);
    $id = $review_id;
    $dbTable = 'engine4_eclassroom_reviews';
    $item_id = 'review_id';
