...
function doResizeForButton(){
	var width = sesJqueryObject('.eclassroom_view_photo_container_wrapper').width();
  sesJqueryObject('#media_photo').css('max-width',width+'px');
	sesJqueryObject('#media_photo').css('max-height',maxHeight+'px');
	<?php if(Engine_Api::_()->user()->getViewer()->getIdentity() == '0'){ ?>
			return false;
	<?php } ?>
	var topPositionOfParentDiv =  sesJqueryObject(".eclassroom_photo_view_option_btn").offset().top + 35;
	topPositionOfParentDiv = topPositionOfParentDiv+'px';
	var leftPositionOfParentDiv =  sesJqueryObject(".eclassroom_photo_view_option_btn").offset().left - 135;
...
<?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != ''){ ?>
  en4.core.runonce.add(function() {
    var descEls = $$('.eclassroom_view_photo_des');
    if( descEls.length > 0 ) {
    ...
    taggerInstance.addEvents({
      'onBegin' : function() {
				sesJqueryObject('.eclassroom_photo_view_btns').hide();
        $('media_photo_next').erase('href');
      },
      'onEnd' : function() {
				sesJqueryObject('.eclassroom_photo_view_btns').show();
...
<div class='eclassroom_view_photo sesbasic_bxs sesbasic_clearfix'>
  <div class='eclassroom_view_photo_container_wrapper sesbasic_clearfix'>
    <?php if( $this->album->count() > 1 ): ?>
      <div class="eclassroom_view_photo_nav_btns">
        <?php 
        $photoPreviousData = Engine_Api::_()->eclassroom()->getPreviousPhoto($this->album->album_id ,$this->photo->order ) ?  Engine_Api::_()->eclassroom()->getPreviousPhoto($this->album->album_id,$this->photo->order) : null;
        echo $this->htmlLink((isset($photoPreviousData->album_id) ?  $photoPreviousData->getHref() : null ), '<i class="fa fa-angle-left"></i>', array('id' => 'photo_prev','data-url'=>$photoPreviousData->photo_id, 'class' => 'eclassroom_view_photo_nav_prev_btn'));
        $photoNextData = Engine_Api::_()->eclassroom()->getNextPhoto($this->album->album_id  ,$this->photo->order ) ?  Engine_Api::_()->eclassroom()->getNextPhoto($this->album->album_id  ,$this->photo->order ) : null;
         ?>
        <?php echo $this->htmlLink(( isset($photoNextData->album_id) ?  $photoNextData->getHref() : null ), '<i class="fa fa-angle-right"></i>', array('id' => 'photo_next','data-url'=>$photoNextData->photo_id, 'class' => 'eclassroom_view_photo_nav_nxt_btn')) ?>
      </div>
    <?php endif ?>
    <div class='eclassroom_view_photo_container' id='media_photo_div'>
      <?php 
        $imageViewerURL = $this->photo->getHref();
        if($imageViewerURL != ''){
      ?>
        <a href="<?php echo $this->photo->getHref(); ?>" title="<?php echo $this->translate('Open image in image viewer'); ?>" onclick="openLightBoxForSesPlugins('<?php echo $imageViewerURL; ?>','<?php echo $this->photo->getPhotoUrl(); ?>');return false;" class="eclassroom_view_photo_expend seslightbox_no_prop"><i class="fa fa-expand"></i></a>
      <?php } ?>
    ...
    <?php if( $this->canEdit ): ?>
      <div class="eclassroom_view_photo_rotate_btns">          
        <a class="eclassroom_icon_photos_rotate_ccw" id="ses-rotate-90" href="javascript:void(0)" onclick="sesPhotoRotate('<?php echo $this->photo->getIdentity() ?>','90')">&nbsp;</a>
        <a class="eclassroom_icon_photos_rotate_cw" id="ses-rotate-270" href="javascript:void(0)" onclick="sesPhotoRotate('<?php echo $this->photo->getIdentity() ?>','270')">&nbsp;</a>
        <a class="eclassroom_icon_photos_flip_horizontal" id="ses-rotate-horizontal"  href="javascript:void(0)" onclick="sesPhotoRotate('<?php echo $this->photo->getIdentity() ?>','horizontal')">&nbsp;</a>
        <a class="eclassroom_icon_photos_flip_vertical" id="ses-rotate-vertical"  href="javascript:void(0)" onclick="sesPhotoRotate('<?php echo $this->photo->getIdentity() ?>','vertical')">&nbsp;</a>          
      </div>
    <?php endif ?>
    <?php
    ...
    ?>
      <div class="sesbasic_clearfix eclassroom_photo_view_btns">
      ...
      <?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != ''){ ?>
        <?php if($this->canTag){ ?>
        <a title="<?php echo $this->translate('Tag'); ?>" href="javascript:void(0);" onclick="taggerInstance.begin();" class="eclassroom_view_tag_button"><i class="fa fa-tag"></i></a>
        <?php } ?>
        <span class="eclassroom_photo_view_option_btn">
  ...
  <div class="eclassroom_view_photo_count">
	...
	<div class="eclassroom_photo_view_bottom_right">
    <?php if(isset($this->criteria)){ ?>
      <!-- Corresponding photo as per album id -->
      <div class="layout_eclassroom_photo_strip">
        <div class="eclassroom_photos_strip_slider sesbasic_clearfix clear">
          <a id="prevSlide" class="eclassroom_photos_strip_slider_btn btn-prev"><i class="fa fa-angle-left"></i></a>
          <div class="eclassroom_photos_strip_slider_content">
            <div id="eclassroom_corresponding_photo" style="width:257px;">
            <?php if(!$this->is_ajax){ ?>
              <img id="eclassroom_corresponding_photo_image" src="<?php echo $this->layout()->staticBaseUrl; ?>application/modules/Sesbasic/externals/images/loading.gif" alt="" style="margin-top:23px;" />
             <?php } ?>
            </div>
          </div>
          <a id="nextSlide" class="eclassroom_photos_strip_slider_btn btn-nxt"><i class="fa fa-angle-right"></i></a>
        </div>
      </div>
    <?php } ?>
  </div>
	<div class="eclassroom_photo_view_bottom_middle sesbasic_clearfix">
    <?php if( $this->photo->getTitle() ): ?>
      <div class="eclassroom_view_photo_title">
        <?php echo $this->photo->getTitle(); ?>
      </div>
    <?php endif; ?>
    <div class="eclassroom_view_photo_middle_box clear sesbasic_clearfix">
      <div class="eclassroom_view_photo_owner_info sesbasic_clearfix">
        <div class="eclassroom_view_photo_owner_photo">
        ...
        <div class="eclassroom_view_photo_owner_details">
          <span class="eclassroom_view_photo_owner_name sesbasic_text_light">
            by <?php echo $this->htmlLink($albumOwnerDetails->getHref(), $albumOwnerDetails->getTitle()); ?>
          </span>
          <span class="sesbasic_text_light eclassroom_view_photo_date">
...
function sesPhotoRotate(photo_id,rotateAngle){
	...
	if(rotateAngle == 90 || rotateAngle == 270){
		if(rotateAngle == 90)
			className = 'eclassroom_icon_photos_rotate_ccw';
		else
			className = 'eclassroom_icon_photos_rotate_cw';		
		rotatePhotoSes(photo_id,rotateAngle,className);
	}else{
		if(rotateAngle == 'horizontal')
			className = 'eclassroom_icon_photos_flip_horizontal';
		else
			className = 'eclassroom_icon_photos_flip_vertical';
...
function flipPhotoSes(photo_id,rotateAngle,className){
	request = new Request.JSON({
      url : en4.core.baseUrl + 'eclassroom/photo/flip',
      data : {
        format : 'json',
        photo_id : photo_id,
...
function rotatePhotoSes(photo_id,rotateAngle,className){
	request = new Request.JSON({
      url : en4.core.baseUrl + 'eclassroom/photo/rotate',
      data : {
        format : 'json',
        photo_id : photo_id,
        angle : rotateAngle
	...
	/*change next previous button click event*/
	 ...
	 function changeNextPrevious(thisObject){
			...
			var correspondingImageData = sesJqueryObject('#eclassroom_corresponding_photo').html();
			var photo_id = sesJqueryObject(thisObject).attr('data-url');
			(new Request.HTML({
      method: 'post',
      'url':en4.core.baseUrl + 'widget/index/mod/eclassroom/name/photo-view-page/',
      'data': {
        format: 'html',
				 photo_id : photo_id,
				params :'', 
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
				...
				<?php if(Engine_Api::_()->user()->getViewer()->getIdentity() != ''){ ?>
					sesJqueryObject(optionDataForButton).appendTo('body');
				<?php } ?>
					sesJqueryObject('.layout_eclassroom_photo_view_page').html(responseHTML);
					var width = sesJqueryObject('.eclassroom_view_photo_container_wrapper').width();
					sesJqueryObject('#media_photo').css('max-width',width+'px');
					sesJqueryObject('#media_photo').css('max-height',maxHeight+'px');
					sesJqueryObject('#eclassroom_corresponding_photo').html(correspondingImageData);
					sesJqueryObject('#eclassroom_corresponding_photo > a').each(function(index){
					...
					sesJqueryObject('#eclassroom_corresponding_photo > a').eq(3).addClass('slideuptovisible');
					sesJqueryObject('#eclassroom_corresponding_photo').css('width',(countSlide*64)+'px');
      }
    })).send();
    return false;
	}
...
sesJqueryObject(document).on('click','.eclassroom_corresponding_image_album',function(e){
		e.preventDefault();
...
function getCorrespondingImg(){
	(new Request.HTML({
      method: 'post',
      'url':en4.core.baseUrl + 'eclassroom/photo/corresponding-image/album_id/<?php echo $this->album->album_id; ?>',
      'data': {
        format: 'html',
				is_ajax : 1,
      },
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
				if(responseHTML){
					sesJqueryObject('#eclassroom_corresponding_photo').html(responseHTML);
					sesJqueryObject('#eclassroom_corresponding_photo > a').each(function(index){
						sesJqueryObject(this).removeClass('slideuptovisible');
...
sesJqueryObject(document).on('mouseover','#prevSlide',function(e){
	var indexCurrent = 	sesJqueryObject('#eclassroom_corresponding_photo > a.slideuptovisible').index();
...
sesJqueryObject(document).on('click','#nextSlide',function(){
	...
	if((countSlide-1) == indexCurrent || indexCurrent == '-1'){
		...
		var leftAttr = sesJqueryObject('#eclassroom_corresponding_photo').css('left').replace('px','');
		leftAttr = leftAttr.replace('-','');		
		if(slideLeft>3){
			leftAttr = parseInt(leftAttr,10);
			sesJqueryObject('#eclassroom_corresponding_photo').css('left','-'+(leftAttr+(64*4))+'px');
			sesJqueryObject('#eclassroom_corresponding_photo > a').eq(indexCurrent).removeClass('slideuptovisible');
			sesJqueryObject('#eclassroom_corresponding_photo > a').eq((indexCurrent+4)).addClass('slideuptovisible');
		}else{
			leftAttr = parseInt(64*slideLeft,10)+parseInt(leftAttr,10);
			sesJqueryObject('#eclassroom_corresponding_photo').css('left','-'+leftAttr+'px');
			sesJqueryObject('#eclassroom_corresponding_photo > a').eq(indexCurrent).removeClass('slideuptovisible');
			sesJqueryObject('#eclassroom_corresponding_photo > a').eq((indexCurrent+slideLeft)).addClass('slideuptovisible');
		}
	}
});
sesJqueryObject(document).on('click','#prevSlide',function(){
 ...
 if(leftAttr == 0 || countSlide < 4 || indexCurrent == '-1'){
	...
	if(typeof type == 'number' && type > 3 ){
		...
		sesJqueryObject('#eclassroom_corresponding_photo').css('left',leftAttr+'px');
		sesJqueryObject('#eclassroom_corresponding_photo > a').eq(indexCurrent).removeClass('slideuptovisible');
		sesJqueryObject('#eclassroom_corresponding_photo > a').eq(selectedindex).addClass('slideuptovisible');
	}
 }
	return false;
});
