...
<?php if( $this->paginator->getTotalItemCount() > 0 ){ ?>
  <?php foreach( $this->paginator as $item ): ?>
    <?php $reviewer = Engine_Api::_()->getItem('classroom', $item->classroom_id);
    	if(!$reviewer)
      	continue;
    ?>
    <li class="sesbasic_clearfix">
    	<div class="eclassroom_review_listing_left">
      	<div class="eclassroom_review_listing_left_photo">
          <?php echo $this->htmlLink($item->getOwner()->getHref(), $this->itemPhoto($item->getOwner(), 'thumb.profile')) ?>
        </div>
        <p class="eclassroom_review_listing_left_title"><?php echo $this->htmlLink($item->getOwner()->getHref(), $item->getOwner()->getTitle()) ?></p>
        <?php if(isset($this->featuredLabelActive) && $item->featured):?>
	  <div class="eclassroom_review_featured_block">
	  <p><?php echo $this->translate('Featured');?></p>
	  </div>
	<?php endif;?>
	<?php if(isset($this->verifiedLabelActive) && $item->verified):?>
	  <div class="eclassroom_review_verified_block">
	    <p><?php echo $this->translate('Verified');?></p>
	  </div>
	<?php endif;?>
      </div>
      <div class="eclassroom_review_listing_right sesbasic_clearfix">
        ...
        <div class="eclassroom_review_listing_top_info sesbasic_clearfix">
          ...
          <div class='eclassroom_review_listing_top_info_cont'>
            <?php if(in_array('postedBy', $this->stats) || in_array('creationDate', $this->stats)): ?>
              <p class="eclassroom_review_listing_stats">
                <?php if(in_array('postedBy', $this->stats)): ?>
        ...
        <div class="eclassroom_review_listing_desc sesbasic_clearfix">
          <?php if(in_array('pros', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('review.show.pros', 1)): ?>
          ...
          <?php endif; ?>
          <?php if(in_array('cons', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('review.show.cons', 1)): ?>
            <p class="eclassroom_review_listing_body">
          ...
          <?php endif; ?>
					<?php if(in_array('customfields', $this->stats)): ?>
						<?php $customFieldsData = Engine_Api::_()->eclassroom()->getCustomFieldMapData($item);?>
						<?php if(count($customFieldsData) > 0):?>
							<?php foreach($customFieldsData as $valueMeta):?>
								<?php if(!$valueMeta['value']):?>	
									<?php continue;?>
								<?php endif;?>
								<?php echo '<p class="eclassroom_review_view_body"><b class="label">'. $valueMeta['label']. ': </b>'.
								$valueMeta['value'].'</p>';?>
							<?php endforeach;?>   
						<?php endif; ?>
					<?php endif; ?>
	  ...
	  <?php if(in_array('recommended', $this->stats)):?>
	    <p class="eclassroom_review_listing_recommended"> <b><?php echo $this->translate('Recommend');?><i class="<?php if($item->recommended):?>fa fa-check<?php else:?>fa fa-times<?php endif;?>"></i></b></p>
	  <?php endif;?>
          <p class="eclassroom_review_listing_more">
            <a href="<?php echo $item->getHref()?>" class=""><?php echo $this->translate('Continue Reading »');?></a>
          </p>
        </div>
        <?php  echo $this->partial('_reviewOptions.tpl','eclassroom',array('subject'=>$item,'viewer'=>$this->viewer(),'stats'=>$this->stats)); ?>
      </div>
    </li>
  <?php endforeach; ?>
  <?php if($this->loadOptionData == 'pagging'){ ?>
    ...
    <?php } ?>
<?php }else{ ?>
    <?php  if(($this->paginator->getTotalItemCount() == 0) && !isset($_GET)):  ?>
	...
	function viewMoreHide_<?php echo $randonNumber; ?>() {
	 ...
	 function viewMore_<?php echo $randonNumber; ?> () {
			...
			requestViewMore_<?php echo $randonNumber; ?> = new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + "widget/index/mod/eclassroom/name/browse-reviews",
				'data': {
		format: 'html',
