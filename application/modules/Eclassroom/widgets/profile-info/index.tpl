...
<ul class='sesbasic_clearfix sesbasic_bxs sesbasic_sidebar_block eclassroom_sidebar_info_block'>
  <?php if(ECLASSROOMSHOWUSERDETAIL == 1 && in_array('owner',$this->criteria)){ ?>
    <li class="sesbasic_clearfix eclassroom_list_stats eclassroom_list_created_by"> <i title='<?php echo $this->translate("Created by"); ?>' class="fa fa-user"></i> <span><a href="<?php echo $this->subject->getOwner()->getHref(); ?>"><?php echo $this->translate('by ');?><?php echo $this->subject->getOwner()->getTitle(); ?></a></span> </li>
  <?php } ?>
  <?php if(in_array('creationDate',$this->criteria)){ ?>
  <li class="sesbasic_clearfix eclassroom_list_stats eclassroom_list_created_on sesbasic_text_light"> <i title='<?php echo $this->translate("Created on"); ?>' class="fa fa-edit"></i> <span><?php echo $this->translate('%1$s', $this->timestamp($this->subject->creation_date)); ?></span> </li>
  <?php } ?>
  <?php if(in_array('tag',$this->criteria) && count($this->classroomTags)){ ?>
  <li class="sesbasic_clearfix eclassroom_list_stats eclassroom_list_tag"> <i title='<?php echo $this->translate("Tags"); ?>' class="fa fa-tag"></i> <span>
  <?php 
      $counter = 1;
  ...
  <?php if($category){ ?>
  <li class="sesbasic_clearfix eclassroom_list_stats eclassroom_list_category"> <i title="<?php echo $this->translate("Category"); ?>" class="fa fa-tags"></i> <span><a href="<?php echo $category->getHref(); ?>"><?php echo $category->category_name; ?></a>
  ...
  <li class="sesbasic_clearfix sesbasic_text_light"> <i title="<?php echo $this->translate('Statistics'); ?>" class="fa fa-bar-chart"></i> <span>
    ...
    <?php } ?>
    <?php if(in_array('member',$this->criteria)){ ?>
    <span title="<?php echo $this->translate(array('%s member', '%s members', $this->subject->member_count), $this->locale()->toNumber($this->subject->member_count)); ?>"><?php echo $this->translate(array('%s member', '%s members', $this->subject->member_count), $this->locale()->toNumber($this->subject->member_count)); ?></span>,
    <?php } ?>
    </span> </li>
  <?php endif;?>
</ul>
