...
<?php endif;?>
<?php if(!$this->is_ajax): ?>
<div class="eclassroom_profile_tab_wrapper eclassroom_profile_photos sesbasic_bxs">
    ...
    <div class="eclassroom_profile_content_search sesbasic_clearfix">
      ...
      <?php if((!$this->is_ajax) && ($this->paginator->count() > 1 || $this->canUpload )  && (!$this->classroom_album_count || $this->paginator->getTotalItemCount() <= $this->classroom_album_count)):  ?>
      <?php if( $this->canUpload ): ?>
          <div class="_addbtn">
            <?php echo $this->htmlLink(array(
              ...
              'class' => 'sesbasic_button sesbasic_icon_add eclassroom_cbtn'
            )) ?>
          </div>  
        <?php endif; ?>
      <?php endif; ?>
      ...
      <li id="thumbs-photo-<?php echo $album->photo_id ?>" class="eclassroom_album_list_grid_thumb eclassroom_album_list_grid sespa-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sespa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?>" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">
        ...
        <?php  if(isset($this->socialSharing) || isset($this->likeButton)){  ?>
          <span class="eclassroom_album_list_grid_btns">
            ...
            <?php }
            $canComment = $this->classroomItem->authorization()->isAllowed($this->viewer, 'comment');;
            if(Engine_Api::_()->user()->getViewer()->getIdentity() != 0 && isset($this->likeButton) && $canComment){  ?>
              <!--Album Like Button-->
              <?php $albumLikeStatus = Engine_Api::_()->eclassroom()->getLikeStatusClassroom($album->album_id,'eclassroom_album'); ?>
              <a href="javascript:;" data-url='<?php echo $album->album_id; ?>' class="sesbasic_icon_btn sesbasic_icon_btn_count sesbasic_icon_like_btn eclassroom_albumlike <?php echo ($albumLikeStatus) ? 'button_active' : '' ; ?>">
        ...
        <?php if(isset($this->like) || isset($this->comment) || isset($this->view) || isset($this->title) || isset($this->photoCount) ||  isset($this->by)){ ?>
          ...
          <span class="eclassroom_album_list_grid_likes" title="<?php echo $this->translate(array('%s like', '%s likes', $album->like_count), $this->locale()->toNumber($album->like_count))?>">
            <i class="fa fa-thumbs-up"></i>
...
<script type="text/javascript">
  ...
  function paggingNumber<?php echo $randonNumber; ?>(pageNum){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "widget/index/mod/eclassroom/name/profile-photos",
      'data': {
      format: 'html',
      page: pageNum,    
	...
	document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML =  responseHTML;	sesJqueryObject('#paginator_count_eclassroom').find('#total_item_count_eclassroom').html(sesJqueryObject('#paginator_count_ajax_eclassroom').find('#total_item_count_eclassroom').html());
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/eclassroom/name/profile-photos/index/',
      'data': {
	format: 'html',
  ...
  en4.core.runonce.add(function() {
    var url = en4.core.baseUrl + 'widget/index/mod/eclassroom/name/profile-photos/index/';
