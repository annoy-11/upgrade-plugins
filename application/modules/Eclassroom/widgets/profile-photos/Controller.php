...
class Eclassroom_Widget_ProfilePhotosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $value['classroom_id'] = $this->view->classroom_id =  $classroom_id = Engine_Api::_()->getDbTable('classrooms', 'eclassroom')->getClassroomId($id);
    $this->view->classroomItem  = $subject = Engine_Api::_()->getItem('classroom', $classroom_id);
    ...
    $getClassroomRolePermission = Engine_Api::_()->eclassroom()->getClassroomRolePermission($subject->getIdentity(),'post_content','album',false);
    $this->view->canUpload = $canUpload = $getClassroomRolePermission ? $getClassroomRolePermission : $subject->authorization()->isAllowed($viewer, 'album');
    $this->view->classroom_album_count = $classroom_album_count = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'eclassroom_classroom', 'classroom_album_count');
    //Manage Apps Check
    $isCheck = Engine_Api::_()->getDbTable('manageclassroomapps', 'eclassroom')->isCheck(array('classroom_id' => $classroom_id, 'columnname' => 'photos'));
    if(empty($isCheck))
