...
<ul class="eclassroom_photos_flex_view sesbasic_bxs">
      <?php foreach( $this->results as $item ): 
        if(isset($item->classroom_id))
        $classroom = Engine_Api::_()->getItem('classroom', $item->classroom_id);
        $album = Engine_Api::_()->getItem('eclassroom_album', $item->album_id);  ?> 
        <li id="thumbs-photo-<?php echo $album->photo_id ?>" class="eclassroom_album_list_grid_thumb eclassroom_album_list_grid sespa-i-<?php echo (isset($widgetParams['insideOutside']) && $widgetParams['insideOutside'] == 'outside') ? 'outside' : 'inside'; ?> sespa-i-<?php echo (isset($widgetParams['fixHover']) && $widgetParams['fixHover'] == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($widgetParams['width']) ? $widgetParams['width'].'px' : $widgetParams['width'] ?>;"> 
        	<article> 

          <a class="eclassroom_album_list_grid_img" href="<?php echo Engine_Api::_()->eclassroom()->getHref($album->getIdentity(),$album->album_id); ?>" style="height:<?php echo is_numeric($widgetParams['height']) ? $widgetParams['height'].'px' : $widgetParams['height'] ?>;">
          ...
          if(in_array('likeButton', $widgetParams['show_criteria']) && Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && $canComment) {  ?>
            ...
            if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && in_array('favouriteButton', $widgetParams['show_criteria'])){
              ...
              <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $album->getType(); ?>" data-url='<?php echo $album->album_id; ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_fav_btn eclassroom_albumFav <?php echo ($albumFavStatus)>0 ? 'button_active' : '' ; ?>">
      ...
      <?php if(in_array('like', $widgetParams['show_criteria']) || in_array('comment', $widgetParams['show_criteria']) || in_array('view', $widgetParams['show_criteria']) || in_array('title', $widgetParams['show_criteria']) || in_array('photoCount', $widgetParams['show_criteria']) || in_array('favouriteCount', $widgetParams['show_criteria'])  || in_array('by', $widgetParams['show_criteria']) || in_array('classroomName', $widgetParams['show_criteria'])){ ?>
        <p class="eclassroom_album_list_grid_info sesbasic_clearfix<?php if(!in_array('photoCount', $widgetParams['show_criteria'])) { ?> nophotoscount<?php } ?>">
        ...
        <span class="eclassroom_album_list_grid_stats">
          <?php if(!empty($classroom) && in_array('classroomName', $widgetParams['show_criteria'])) { ?>
          ...
          <?php } ?>
          <?php if(ECLASSROOMSHOWUSERDETAIL == 1 && in_array('by', $widgetParams['show_criteria'])) { ?>
            <span class="eclassroom_album_list_grid_owner">
        ...
        <span class="eclassroom_album_list_grid_stats sesbasic_text_light">
         ...
         <?php if(in_array('favouriteCount', $widgetParams['show_criteria'])) { ?>
            <span class="eclassroom_album_list_grid_fav" title="<?php echo $this->translate(array('%s favourite', '%s favourites', $album->favourite_count), $this->locale()->toNumber($album->favourite_count))?>">
              <i class="fa fa-heart"></i> 
