...
class Eclassroom_Widget_RecentlyViewedAlbumsController extends Engine_Content_Widget_Abstract {
  public function indexAction() {
    ...
    $params['type'] = 'eclassroom_album';
    $params['showdefaultalbum'] = $allParams['showdefaultalbum'];
    if(($criteria == 'by_me' || $criteria == 'by_myfriend') && $userId == 0) {
        return $this->setNoRender();
    }
  	$result = Engine_Api::_()->getDbTable('recentlyviewitems', 'eclassroom')->getitem($params);
		if(count($result) == 0)
            return $this->setNoRender();
