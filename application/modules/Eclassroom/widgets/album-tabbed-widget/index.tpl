...
<?php if($this->tab_option == 'default'){ ?>
  <div class="layout_core_container_tabs">
    <div class="tabs_alt tabs_parent" id="eclassroom_tabbed_widget_container_<?php echo $randonNumber; ?>">
<?php } ?>
<!--Advance Tabs-->
<?php if($this->tab_option == 'advance'){ ?>
  <div class="sesbasic_tabs_container sesbasic_clearfix sesbasic_bxs">
    <div class="sesbasic_tabs sesbasic_clearfix" id="eclassroom_tabbed_widget_container_<?php echo $randonNumber; ?>">
 <?php } ?>
<!--Filter Tabs-->
<?php if($this->tab_option == 'filter'){ ?>
  <div class="sesbasic_filter_tabs_container sesbasic_clearfix sesbasic_bxs">
    <div class="sesbasic_filter_tabs sesbasic_clearfix" id="eclassroom_tabbed_widget_container_<?php echo $randonNumber; ?>">
<?php } ?>
    <ul id="tab-widget-eclassroom-<?php echo $randonNumber; ?>">
      <?php 
  ...
  <?php  if(count($this->defaultOptions) == 1) { ?>
    <script type="application/javascript">
      sesJqueryObject('#eclassroom_tabbed_widget_container_<?php echo $randonNumber; ?>').css('display','none');
    </script>
  <?php } ?>
  <div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" class="sesbasic_clearfix">
    <ul class="eclassroom_album_listings eclassroom_tabbed_listings eclassroom_photos_flex_view sesbasic_bxs sesbasic_clearfix" id="album-tabbed-widget_<?php echo $randonNumber; ?>">
<?php } ?>
    <?php $limit = $this->limit;
    			foreach( $this->paginator as $item ): ?>
            <?php $classroom = Engine_Api::_()->getItem('classroom', $item->classroom_id); ?>
            <li id="thumbs-photo-<?php echo $item->photo_id ?>" class="eclassroom_album_list_grid_thumb eclassroom_album_list_grid sespa-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sespa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">
            	<article>  
              <a class="eclassroom_album_list_grid_img" href="<?php echo Engine_Api::_()->eclassroom()->getHref($item->getIdentity(),$item->album_id); ?>" style="height:<?php echo is_numeric($this->height) ? $this->height.'px' : $this->height ?>;">
       ...
       ?>
        ...
        $canComment =  $item->authorization()->isAllowed(Engine_Api::_()->user()->getViewer(), 'comment');
        	if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->likeButton) && $canComment){ ?>
                ...
                <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $item->getType(); ?>" data-url='<?php echo $item->album_id; ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_like_btn eclassroom_albumlike <?php echo ($albumLikeStatus) ? 'button_active' : '' ; ?>">
              ...
              <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $item->getType(); ?>" data-src='<?php echo $item->getIdentity(); ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_fav_btn eclassroom_albumFav <?php echo ($albumFavStatus)>0 ? 'button_active' : '' ; ?>">
         ...
         <?php if(isset($this->like) || isset($this->comment) || isset($this->view) || isset($this->title) || isset($this->rating) || isset($this->photoCount) || isset($this->favouriteCount) || isset($this->downloadCount)  || isset($this->by) || isset($this->classroomName)){ ?>
              ...
              <span class="eclassroom_album_list_grid_stats sesbasic_text_light">
                <?php if(isset($this->like)) { ?>
                  <span class="eclassroom_album_list_grid_likes eclassroom_album_likes_count_<?php echo $item->getIdentity(); ?>" title="<?php echo $this->translate(array('%s like', '%s likes', $item->like_count), $this->locale()->toNumber($item->like_count))?>">
                    <i class="sesbasic_icon_like_o"></i>
                    <?php echo $item->like_count;?>
                  </span>
                <?php } ?>
                <?php if(isset($this->comment)) { ?>
                  <span class="eclassroom_album_list_grid_comment" title="<?php echo $this->translate(array('%s comment', '%s comments', $item->comment_count), $this->locale()->toNumber($item->comment_count))?>">
                    <i class="sesbasic_icon_comment_o"></i>
                    <?php echo $item->comment_count;?>
                  </span>
               <?php } ?>
               <?php if(isset($this->view)) { ?>
                  <span class="eclassroom_album_list_grid_views" title="<?php echo $this->translate(array('%s view', '%s views', $item->view_count), $this->locale()->toNumber($item->view_count))?>">
                    <i class="sesbasic_icon_view"></i>
                    <?php echo $item->view_count;?>
                  </span>
               <?php } ?>
               <?php if(isset($this->favouriteCount)) { ?>
                  <span class="eclassroom_album_list_grid_fav eclassroom_album_favs_count_<?php echo $item->getIdentity(); ?>" title="<?php echo $this->translate(array('%s favourite', '%s favourites', $item->favourite_count), $this->locale()->toNumber($item->favourite_count))?>">
                    <i class="sesbasic_icon_favourite_o"></i> 
                    <?php echo $item->favourite_count;?>            
                  </span>
                <?php } ?>
                 <?php if(isset($this->photoCount)) { ?>
               	<span class="eclassroom_album_list_grid_count" title="<?php echo $this->translate(array('%s photo', '%s photos', $item->count()), $this->locale()->toNumber($item->count()))?>" >
...
var searchParams<?php echo $randonNumber; ?>;
  var truncateRow = false;
	...
	function paggingNumber<?php echo $randonNumber; ?>(pageNum){
		 ...
		 var valueTab ;
		 sesJqueryObject('#tab-widget-eclassroom-<?php echo $randonNumber; ?> > li').each(function(index){
		 ...
		 if(typeof valueTab == 'undefined')
			...
			(new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + 'widget/index/mod/eclassroom/name/album-tabbed-widget/openTab/' + valueTab,
				'data': {
					format: 'html',
					page: pageNum,   
...
<?php if($this->loadOptionData != 'pagging'){ ?>
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    if (typeof(requestTab_<?php echo $randonNumber; ?>) != 'undefined') {
		...
		(new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/eclassroom/name/album-tabbed-widget/openTab/' + openTab_<?php echo $randonNumber; ?>,
      'data': {
        format: 'html',
