...
<?php if(!$this->is_ajax){ ?>
  ...
  var previous_rate_value;
    function showReviewForm() {  
      document.getElementById('sespagereview_review_create_form').style.display = 'block';
      ...
      var openObject = sesJqueryObject('#sespagereview_review_create_form');
  ...
  <?php $editReviewPrivacy = Engine_Api::_()->sesbasic()->getViewerPrivacy('pagereview', 'edit');?>
  <?php if($this->viewer()->getIdentity() && Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagereview.allow.review', 1) && $this->allowedCreate):?>
    <div class="sesbasic_profile_tabs_top sesbasic_clearfix sespagereview_review_profile_btn">
      <a id="sespagereview_create_button" href="javascript:void(0)" onclick="showReviewForm();" class="sesbasic_button" style="display:<?php if($this->cancreate && !$this->isReview): ?>block;<?php else:?>none;<?php endif;?>">
				<i class="fa fa-plus"></i>
				<span><?php echo $this->translate('Write a Review');?></span>
			</a>
      <a id="sespagereview_edit_button" href="javascript:void(0)" onclick="showReviewForm();" class="sesbasic_button" style="display:<?php if($editReviewPrivacy && $this->isReview):?>block;<?php else:?>none;<?php endif;?>">
				<i class="fa fa-plus"></i>
  ...
  <?php if( $this->paginator->getTotalItemCount() > 0 ){ ?>
    <div class="sespagereview_profile_reviews_filters sesbasic_bxs sesbasic_clearfix">
      <?php echo $this->content()->renderWidget('sespagereview.browse-review-search',array('review_search'=>1,'review_stars'=>1,'reviewRecommended'=>1,'review_title'=>0,'view_type'=>'horizontal','isWidget'=>true,'page_id'=>$this->subject->getIdentity(),'widgetIdentity'=>$this->identity)); ?>
    </div>
  <?php } ?>
  <ul class="sespagereview_review_listing sesbasic_clearfix sesbasic_bxs" id="sespagereview_review_listing">
<?php } ?>
  <?php if( $this->paginator->getTotalItemCount() > 0 ){ ?>
    <?php foreach( $this->paginator as $item ): ?>
      <li class="sesbasic_clearfix <?php if($item->owner_id == $this->viewer()->getIdentity()):?>sespagereview_owner_review<?php endif;?>">
        <div class="sespagereview_review_listing_top sesbasic_clearfix">
          ...
          <div class="sespagereview_review_listing_top_info sesbasic_clearfix">
	        ...
	        <div class='sespagereview_review_listing_top_info_cont'>
              ...
              <?php endif; ?>
              <p class="sesbasic_text_light sespagereview_review_listing_stats">
          ...
          <div class="sespagereview_review_show_rating review_ratings_listing">
            ...
            <?php if(in_array('parameter', $this->stats)){ ?>
              <?php $reviewParameters = Engine_Api::_()->getDbtable('parametervalues', 'sespagereview')->getParameters(array('content_id'=>$item->getIdentity(),'page_id'=>$item->page_id)); ?>
        ...
        <div class="sespagereview_review_listing_desc sesbasic_clearfix">
          ...
          <?php endif; ?>
	  <p class="sespagereview_review_listing_recommended"> <b><?php echo $this->translate('Recommended');?><i class="<?php if($item->recommended):?>fa fa-check<?php else:?>fa fa-times<?php endif;?>"></i></b></p>
        ...
        <?php  echo $this->partial('_reviewOptions.tpl','sespagereview',array('subject'=>$item,'viewer'=>Engine_Api::_()->user()->getViewer(),'stats'=>$this->stats,'updateReview' => 1, 'socialshare_enable_plusicon' => $this->socialshare_enable_plusicon,'socialshare_icon_limit' => $this->socialshare_icon_limit)); ?>        
      </li>
    <?php endforeach; ?>
  ...
  <div class="tip">
    <span>
      <?php echo $this->translate('No review have been posted on this page yet.');?>
    </span>
  </div>
  <?php } ?>
<?php if(!$this->is_ajax){ ?>
  ...
  <?php endif;?>
  <?php if(($this->allowedCreate && $this->cancreate && $this->viewer()->getIdentity() && Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagereview.allow.review', 1) && !$this->isReview) || ($editReviewPrivacy)): ?>
    <div id="sespagereview_review_create_form" class="sespagereview_review_form_block" style="display:none;"> 
  ...
  <script type="text/javascript">
    function closeReviewForm() {
      document.getElementById('sespagereview_review_create_form').style.display = 'none';
      var openObject = sesJqueryObject('.sespagereview_review_profile_btn');
      sesJqueryObject('html, body').animate({
          scrollTop: openObject.offset().top
      }, 2000);
      if(sesJqueryObject('#sespagereview_edit_button').length && previous_rate_value != 'undefined'){
...
<script type="application/javascript">
  <?php if(!$this->is_ajax):?>
	sesJqueryObject(document).on('click','.sespagereview_own_update_review',function(e){
      e.preventDefault();
      showReviewForm();
	});
    <?php if($this->loadOptionData == 'auto_load'){ ?>
      window.addEvent('load', function() {
        sesJqueryObject(window).scroll( function() {
          var containerId = '#sespagereview_review_listing';
  ...
  function viewMore_<?php echo $randonNumber; ?> () {
    ...
    requestViewMore_<?php echo $randonNumber; ?> = new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "widget/index/mod/sespagereview/name/page-reviews",
      'data': {
        format: 'html',
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        sesJqueryObject('#sespagereview_review_listing').append(responseHTML);
  ...
  <?php }else{ ?>
    function paggingNumber<?php echo $randonNumber; ?>(pageNum){
      ...
      requestViewMore_<?php echo $randonNumber; ?> = (new Request.HTML({
        ...
        onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
          sesJqueryObject('#sespagereview_review_listing').html(responseHTML);
