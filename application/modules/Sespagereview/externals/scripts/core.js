sesJqueryObject(document).on('submit', '#sespagereview_edit_review',function(e) {
  ...
  var jqXHR=sesJqueryObject.ajax({
    ...
    success: function(response){
      ...
      if(response.status == 'true') {
			sesJqueryObject('#sessmoothbox_container').html("<div id='sespagereview_review_edit_message' class='sesbasic_bxs'><div class='sesbasic_tip clearfix'><img src='application/modules/Sespage/externals/images/success.png' alt=''><span>Review Edit Successfully</span></div></div>");
...
function reviewVotes(elem,type){
  ...
  (new Request.HTML({
    method: 'post',
    'url': en4.core.baseUrl + 'sespagereview/index/review-votes',
    'data': {
      format: 'html',
      id: id,
      type: type,
...
//review votes js
sesJqueryObject(document).on('click', '.sespagereview_review_useful', function (e) {
  reviewVotes(this, '1');
});
sesJqueryObject(document).on('click', '.sespagereview_review_funny', function (e) {
  reviewVotes(this, '2');
});
sesJqueryObject(document).on('click', '.sespagereview_review_cool', function (e) {
  reviewVotes(this, '3');
});

//tooltip code
var sestooltipOrigin;
sesJqueryObject(document).on('mouseover mouseout', '.sespagereview_rating_info_tip', function(event) {
  sesJqueryObject(this).tooltipster({
    ...
    functionBefore: function(origin, continueTooltip) {
      ...
      if(typeof sesJqueryObject(origin).attr('data-rel') == 'undefined')
        ...
        continueTooltip();
      var data = "<div class='sespagereview_rating_info'>"+sesJqueryObject(this).parent().find('.sespagereview_rating_info').html()+"<div>";
...
sesJqueryObject(document).on('click', '.sespagereview_like', function () {
  sespagereview_like_data(this, 'sespagereview_like');
});
//common function for like comment ajax
function sespagereview_like_data(element) {
  if (!sesJqueryObject(element).attr('data-type'))
    return;
  var contentId = sesJqueryObject(element).attr('data-url');
  var elementId = '.sespagereview_like_' + contentId;
  ...
  (new Request.HTML({
    method: 'post',
    'url': en4.core.baseUrl + 'sespagereview/review/like',
    'data': {
      format: 'html',
      id: contentId,
      type: 'pagereview',
