 ...
 */
class Sespagereview_Api_Core extends Core_Api_Abstract {

  public function getWidgetParams($pageStyle = null) {
    if ($pageStyle)
      $pageName = 'sespage_profile_index_' . $pageStyle;
    ...
    $params = $db->select()
            ...
            ->where('name = ?', 'sespagereview.page-reviews')
  ...
  public function allowReviewRating() {
    if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagereview.allow.review', 1)) {
      return 1;
    }
    return 0;
  }
  ...
  function updateTabOnViewPage($pageName = '') {
    ...
    $widget_id = $db->select()->where('type = ?', 'widget')
            ...
            ->where('name = ?', 'sespagereview.profile-reviews')
            ->where('page_id = ?', $page_id)
            ->limit(1)
            ->query()
            ->fetchColumn();
    if ($page_id && !$widget_id) {
      ...
      $order = $db->select()->where('type = ?', 'widget')
              ...
              ->query()
              ->fetchColumn();
      if ($pageName == 'sespage_profile_index_1') {
        $params = '{"stats":["likeCount","commentCount","viewCount","title","share","report","pros","cons","description","recommended","postedBy","parameter","creationDate","rating"],"title":"Reviews","nomobile":"0","name":"sespagereview.page-reviews"}';
      } else if ($pageName == 'sespage_profile_index_2') {
        $params = '{"stats":["likeCount","commentCount","viewCount","title","share","report","pros","cons","description","recommended","postedBy","parameter","creationDate","rating"],"title":"Reviews","nomobile":"0","name":"sespagereview.page-reviews"}';
      } else if ($pageName == 'sespage_profile_index_3') {
        $params = '{"stats":["likeCount","commentCount","viewCount","title","share","report","pros","cons","description","recommended","postedBy","parameter","creationDate","rating"],"title":"Reviews","nomobile":"0","name":"sespagereview.page-reviews"}';
      } else if ($pageName == 'sespage_profile_index_4') {
        $params = '{"stats":["likeCount","commentCount","viewCount","title","share","report","pros","cons","description","recommended","postedBy","parameter","creationDate","rating"],"title":"Reviews","nomobile":"0","name":"sespagereview.page-reviews"}';
      }
