...
this.DatePicker = Picker.Date = new Class({
	...
	constructPicker: function(){
		...
		if (!this.options.rtl){
			this.previous = new Element('div.previous[html=<i class="fa fa-chevron-left"></i>]').inject(this.header);
			this.next = new Element('div.next[html=<i class="fa fa-chevron-right"></i>]').inject(this.header);
		} else {
			this.next = new Element('div.previous[html=<i class="fa fa-chevron-left"></i>]').inject(this.header);
			this.previous = new Element('div.next[html=<i class="fa fa-chevron-right"></i>]').inject(this.header);
		}
	},
...
var renderers = {
	...
	months: function(months, options, currentDate, dateElements, fn){
		...
		months.each(function(_month, i){
			...
			element = new Element('td' + classes, {text: en4.core.language.translate('Ses'+monthsAbbr[i])}).inject(rows[rows.length - 1]);
	...
	days: function(days, options, currentDate, dateElements, fn){
		...
		for (day = options.startDay; day < (options.startDay + 7); day++){
			new Element('th.title.day.day' + (day % 7), {
				text: en4.core.language.translate('Ses'+localeDaysShort[(day % 7)]),
				role: 'columnheader'
			}).inject(titles, where);
		}
		...
		days.each(function(_date, i){
			...
			if (i % 7 == 0){
				...
				if (weeknumbers) new Element('th.day.weeknumber', {text: en4.core.language.translate('Ses'+date.get('week')), scope: 'row', role: 'rowheader'}).inject(weekcontainer);
			}
