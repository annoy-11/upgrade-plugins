...
var previous_rate_value;
  function showReviewForm() {
    document.getElementById('sesmember_review_create_form').style.display = 'block';
		var openObject = sesJqueryObject('#sesmember_review_create_form');
...
<?php if($this->viewer()->getIdentity() && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmember.allow.review', 1) && $this->allowedCreate):?>
  <div class="sesbasic_profile_tabs_top sesbasic_clearfix sesmember_review_profile_btn">
    <a id="sesmember_create_button" href="javascript:void(0)" onclick="showReviewForm();" class="sesbasic_button fa fa-plus" style="display:<?php if($this->cancreate && !$this->isReview): ?>block;<?php else:?>none;<?php endif;?>"><?php echo $this->translate('Write a Review');?></a>
    <a id="sesmember_edit_button" href="javascript:void(0)" onclick="showReviewForm();" class="sesbasic_button fa fa-plus" style="display:<?php if($editReviewPrivacy && $this->isReview):?>block;<?php else:?>none;<?php endif;?>"><?php echo $this->translate('Update Review');?></a>
  </div>
<?php endif;?>
  ...
  function closeReviewForm() {
    document.getElementById('sesmember_review_create_form').style.display = 'none';
		var openObject = sesJqueryObject('.sesmember_review_profile_btn');
				...
				if(sesJqueryObject('#sesmember_edit_button').length && previous_rate_value != 'undefined'){
...
<script type="application/javascript">
  <?php if(!$this->is_ajax):?>
	...
	sesJqueryObject(document).on('click','.sesmember_own_update_review',function(e){
			e.preventDefault();
			showReviewForm();
	});
