  ...
  <?php $ownerSelf = $viewer->getIdentity() == $item->owner_id ? true : false; ?>
	<div class="sesmember_review_listing_btn_right floatR">
		<?php if($item->authorization()->isAllowed($viewer, 'edit')) { ?>     
			<a class="fa fa-pencil sesbasic_button sesbasic_button_icon <?php if($ownerSelf) { echo 'sesmember_own_update_review'; } ?>" href="<?php echo $this->url(array('route' => 'sesmember_review_view', 'action' => 'edit-review', 'review_id' => $item->review_id,'format' => 'smoothbox'),'sesmember_review_view',true); ?>" <?php if(!$ownerSelf) { ?> onclick='return opensmoothboxurl(this.href);' <?php  } ?> ><span><i class="fa fa-caret-down"></i><?php echo $this->translate('Edit Review'); ?></span></a>
		<?php } ?>
		...
		<?php } ?>
		<?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmember.show.report', 1) && $viewer->getIdentity() && in_array('report', $this->stats)): ?>
		...
		<?php endif; ?>
		<?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesmember.allow.share', 1) && $viewer->getIdentity() && in_array('share', $this->stats)): ?>
