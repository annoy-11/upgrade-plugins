...
en4.core.runonce.add(function() {
  ...
  var rating_over = window.rating_over = function(rating) {
    ...
    for(var x=1; x<=5; x++) {
      if(x <= rating) 
      $('rate_'+x).set('class', 'sesmember_rating_star');
      else 
      $('rate_'+x).set('class', 'sesmember_rating_star sesmember_rating_star_disable');
    }
  }
  ...
  function validateForm(){
    ...
    counter = 0;
    sesJqueryObject('#sesmember_review_form input, #sesmember_review_form select,#sesmember_review_form checkbox,#sesmember_review_form textarea,#sesmember_review_form radio').each(
	function(index){
    ...
    return errorPresent ;
      ...
      <?php if(!empty($id)):?>
    sesJqueryObject(document).on('submit','#sesmember_review_form',function(e){
      var validationFm = validateForm();
      if(!sesJqueryObject('#rate_value').val()){
	alert('<?php echo $this->translate("Please fill the red mark fields"); ?>');
	var errorFirstObject = sesJqueryObject('#sesmember_review_rating').parent();
  ...
  function sendDataToServer(object){
    ...
    var url = sesJqueryObject('#sesmember_review_form').attr('action');
      var d = new Date();
      d.setMonth(d.getMonth()+1);
      document.cookie="data="+JSON.stringify(sesJqueryObject('#sesmember_review_form').serialize())+"; expires="+d+"; path=/";
    sesJqueryObject.ajax({
      ...
      success:function(response){
        sesJqueryObject('body').append('<div id="sesmember_review_content" style="display:none;"></div>');
        sesJqueryObject('#sesmember_review_content').html(response);
        var reviewHtml = sesJqueryObject('#sesmember_review_content').find('.sesmember_reviews').html();
        //update cover rating
				if(sesJqueryObject('.sesmember_cover_rating').length){
					pre_rate_cover = sesJqueryObject('#sesmember_review_content').find('.rating_params').find('#total_rating_average').val();
					var ratingtext = sesJqueryObject('#sesmember_review_content').find('.rating_params').find('#rating_text').val();
					window.set_rating_cover(ratingtext);
				}

        if(sesJqueryObject('.sesmember_owner_review').length > 0) {
          var updatedHtmlQuery = sesJqueryObject('ul.sesmember_review_listing li.sesmember_owner_review').index();
          sesJqueryObject('.sesmember_review_listing').children().eq(updatedHtmlQuery).html(reviewHtml);
        }
        else if(!sesJqueryObject('#sesmember_review_rate').length){
	 			 sesJqueryObject('.sesmember_review_listing').prepend('<li class="sesbasic_clearfix sesmember_owner_review">'+reviewHtml+'</li>');
        }
        var editPrivacy = '<?php echo $editPrivacy;?>';
        if(editPrivacy == 1) {
					var editFormHtml = sesJqueryObject('#sesmember_review_content').find('#sesmember_review_create_form').html();
					sesJqueryObject('#sesmember_review_create_form').first().html(editFormHtml);
					sesJqueryObject('#sesmember_create_button').hide();
					sesJqueryObject('#sesmember_edit_button').show();
        }
        else
        	sesJqueryObject('#sesmember_create_button').hide();

        sesJqueryObject('#sesmember_review_content').remove();
        sesJqueryObject('#sesmember_review_create_form').hide();
				sesJqueryObject('.sesmember_review_listing ').show();
				sesJqueryObject('.sesmember_review_listing').parent().find('.tip').hide();
				sesJqueryObject('.sesbasic_loading_cont_overlay').hide();
				var openObject = sesJqueryObject('.sesmember_review_profile_btn');
				sesJqueryObject('html, body').animate({
