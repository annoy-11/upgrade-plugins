...
<div class="clear seslisting_order_view_top" style="margin:10px;">
      <a href="<?php echo $this->listing->getHref(); ?>" class="buttonlink sesbasic_icon_back"><?php echo $this->translate("Back To Listing"); ?></a>
    </div>
<div class="seslisting_album_form"> <?php echo $this->form->render($this) ?> </div>
...
<script type="text/javascript">
  ...
  jqueryObjectOfSes('#seslisting_create_form_tabs li a').on('click',function(){
...
function sendFileToServer(formData,status,file,isURL,i)
{
		...
		var url = '&isURL='+urlIs;
		var listing_id = '<?php  echo $this->listing_id;?>';
    var uploadURL =$('form-upload').action + '?ul=1'+url+'&listing_id='+listing_id; //Upload URL
...
jqueryObjectOfSes(document).on('click','.delete_image_upload',function(e){
    ...
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'seslisting', 'controller' => 'album', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'seslisting', 'controller' => 'album', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
</script>
