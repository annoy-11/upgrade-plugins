 ...
 * @version    $Id: review-parameters.tpl  2019-04-13 00:00:00 SocialEngineSolutions $
...
if($this->item->category_id){
	$reviewParameters = Engine_Api::_()->getDbtable('parameters', 'seslisting')->getParameterResult(array('category_id'=>$this->item->category_id));
  if(count($reviewParameters)){
  foreach($reviewParameters as $value){
?>
<div class="form-wrapper seslisting_form_review_star">
	<div class="form-label"><label><?php echo $this->translate($value['title']); ?></label></div>
  <div id="seslisting_review_rating" class="sesbasic_rating_parameter seslisting_rating_star_element" onmouseout="rating_out_review(<?php echo $value['parameter_id'] ?>);">
...
function ratingTextReview(rating){
  ...
  window.addEvent('domready', function() {
		var countExistsParam = sesJqueryObject('.seslisting_review_values');
