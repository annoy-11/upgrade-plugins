 ...
 * @version    $Id: _pinboardView.tpl  2019-04-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
 
 ?>
<li class="seslisting_pinboard_list sesbasic_bxs new_image_pinboard_<?php echo $randonNumber; ?>">
	<div class="seslisting_pinboard_list_item sesbm <?php if((isset($this->my_listings) && $this->my_listings)){ ?>isoptions<?php } ?>">
		<div class="seslisting_pinboard_list_item_top seslisting_thumb sesbasic_clearfix">
			<div class="seslisting_pinboard_list_item_thumb">
				<a href="<?php echo $item->getHref()?>" data-url = "<?php echo $item->getType() ?>" class="<?php echo $item->getType() != 'seslisting_chanel' ? 'seslisting_thumb_img' : 'seslisting_thumb_nolightbox' ?>">
			...
			<?php if(isset($this->featuredLabelActive) || isset($this->sponsoredLabelActive) || isset($this->verifiedLabel)):?>
				<div class="seslisting_pinboard_labels">
    ...
    <div class="seslisting_pinboard_info_listing sesbasic_clearfix">
      <div class="seslisting_pinboard_list_item_cont sesbasic_clearfix">
        <?php if(isset($this->categoryActive)){ ?>
          <?php if($item->category_id != '' && intval($item->category_id) && !is_null($item->category_id)):?> 
            <?php $categoryItem = Engine_Api::_()->getItem('seslisting_category', $item->category_id);?>
            <?php if($categoryItem):?>
              <div class="seslisting_pinboard_categry seslisting_list_stats sesbasic_text_light">
                <span>
        ...
        <?php if(isset($this->titleActive)):?>
          <div class="seslisting_pinboard_title">
        ...
        <div class="seslisting_pinboard_meta_listing">
          <?php if(isset($this->byActive)):?>
						<div class="seslisting_list_stats sesbasic_text_dark sesbasic_clearfix">
							<?php $owner = $item->getOwner(); ?>
							<?php echo $this->htmlLink($item->getOwner()->getParent(), $this->itemPhoto($item->getOwner()->getParent(), 'thumb.icon')); ?>
							<span class="seslisting_pinboard_list_item_poster_info_title"><?php echo $this->translate('by ');?><?php echo $this->htmlLink($owner->getHref(),$owner->getTitle() ) ?></span>
						</div>
          <?php endif;?>
          <?php if(isset($this->creationDateActive)){ ?>
            <div class="seslisting_list_stats sesbasic_text_dirk">
              <?php if($item->publish_date): ?>
                <span><i class="fa fa-clock-o"></i> <a href="<?php echo $this->url(array('action'=>'browse'),'seslisting_general',true).'?date='.date('Y-m-d',strtotime($item->publish_date)); ?>"><b><?php echo date('M d',strtotime($item->publish_date));?></b></a></span>
              <?php else: ?>
                <span><i class="fa fa-clock-o"></i> <a href="<?php echo $this->url(array('action'=>'browse'),'seslisting_general',true).'?date='.date('Y-m-d',strtotime($item->creation_date)); ?>"><b><?php echo date('M d',strtotime($item->creation_date));?></b></a></span>
              <?php endif; ?>
            </div>
          <?php } ?>
          <?php if(isset($this->locationActive) && isset($item->location) && $item->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('seslisting_enable_location', 1)){ ?>
        ...
        <?php if(isset($this->descriptionpinboardActive)){ ?>
          <div class="seslisting_pinboard_list_item_des clear">
          	<?php echo $item->getDescription($this->description_truncation_pinboard);?>
          </div>
        <?php } ?>
        
        <div class="seslisting_pinboard_list_item_btm sesbm sesbasic_clearfix">
          <div class="seslisting_pinboard_list_item_btm_cont sesbasic_text_light sesbasic_clearfix">
            <?php $owner = $item->getOwner(); ?>
            <div class="seslisting_pinboard_list_statics floatL sesbasic_text_dark">
          ...
          <?php if(isset($this->my_listings) && $this->my_listings ){ ?>
            <?php if($this->can_edit || $this->can_delete){ ?>
              <div class="seslisting_listing_in_grid2_date sesbasic_text_light">
                <span class="seslisting_list_option_toggle fa fa-ellipsis-v sesbasic_text_light"></span>
                <div class="seslisting_listing_options_pulldown">
                  <?php if($this->can_edit){ 
                  echo $this->htmlLink(array('route' => 'seslisting_specific','action' => 'edit','listing_id' => $item->listing_id), $this->translate('Edit Listing')) ; 
                  } ?>
                  <?php if ($this->can_delete){
                  echo $this->htmlLink(array('route' => 'seslisting_specific','action' => 'delete', 'listing_id' => $item->listing_id), $this->translate('Delete Listing'), array('onclick' =>'opensmoothboxurl(this.href);return false;'));
                  } ?>
                </div>
              </div>
            <?php } ?>
          ...
          <?php if(isset($this->enableCommentPinboardActive)):?>
            <?php $itemType = '';?>
            <?php if(isset($item->listing_id)):?> 
              <?php $item_id = $item->listing_id;?>
              <?php $itemType = 'seslisting_listing';?>
            <?php endif; ?>
            <?php if(($itemType != '')): ?>
              <div class="seslisting_pinboard_list_comments">
