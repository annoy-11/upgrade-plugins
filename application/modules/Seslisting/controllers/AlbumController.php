 ...
 * @version    $Id: AlbumController.php  2019-04-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seslisting_AlbumController extends Core_Controller_Action_Standard {

	public function init() {
    $id = $this->_getParam('album_id', $this->_getParam('id', null));
    if ($id) {
      $album = Engine_Api::_()->getItem('seslisting_album', $id);
      if ($album) {
  ...
  public function createAction() {
    ...
    return $this->_forward('upload-photo', null, null, array('format' => 'json'));
     $listing_id = $this->_getParam('listing_id',false);
    ...
    if($album_id){
    	$album = Engine_Api::_()->getItem('seslisting_album', $album_id);
			$this->view->listing_id = $listing_id = $album->listing_id;
				...
				$this->view->listing_id = $listing_id = $listing_id;
		}
		$listing = $this->view->listing = Engine_Api::_()->getItem('seslisting', $listing_id);

		$isListingAdmin = Engine_Api::_()->seslisting()->checkListingAdmin($listing);
    if(!$isListingAdmin)
    return $this->_forward('notfound', 'error', 'core');

		if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('seslistingpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('seslistingpackage.enable.package', 1) && empty($_POST)){
			$package = $listing->getPackage();
			$this->view->photoLeft = $photoLeft = $listing->getPackage()->allowUploadPhoto($listing->orderspackage_id,true);
    ...
    $this->view->current_count =Engine_Api::_()->getDbtable('albums', 'seslisting')->getUserAlbumCount($values);
    $this->view->quota = $quota = 0;
    // Get form
    $this->view->form = $form = new Seslisting_Form_Album();
    // Render
    ...
    $db = Engine_Api::_()->getItemTable('seslisting_album')->getAdapter();
    $db->beginTransaction();
    try {
...
//     }
    ...
    try {
      $viewer = Engine_Api::_()->user()->getViewer();
      $photoTable = Engine_Api::_()->getDbtable('photos', 'seslisting');
	...
	return;
      ...
      $db->commit();
    }catch (Seslisting_Model_Exception $e) {
      $db->rollBack();
      $this->view->status = false;
  ...
  public function getAlbumAction() {
    ...
    $albums = Engine_Api::_()->getDbTable('albums', 'seslisting')->getAlbumsAction($value);
  ...
  public function editAction() {
    ...
    return;
		...
		$album_id = $this->_getParam('album_id',false);
    if($album_id)
    $this->view->album = $album = Engine_Api::_()->getItem('seslisting_album', $album_id);
	  else
		return;

		$this->view->listing = $listing = Engine_Api::_()->getItem('seslisting', $album->listing_id);
		if ($listing)
    ...
    // Make form
    $this->view->form = $form = new Seslisting_Form_Album_Edit();
  ...
  public function deleteAction() {
    ...
    // In smoothbox
    $this->view->form = $form = new Seslisting_Form_Album_Delete();
    if (!$album) {
      $this->view->status = false;
    ...
    try {
      ...
      throw $e;
		...
		$tab_id = Engine_Api::_()->sesbasic()->getWidgetTabId(array('name' => 'seslisting.profile-photos'));
		return $this->_forward('success' ,'utility', 'core', array('parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'view','listing_id'=>$listing->custom_url, 'tab' => $tab_id), 'seslisting_entry_view', true), 'messages' => Array($this->view->message)
		));
  }
  ...
  public function editphotosAction() {
    ...
    // Get albums
    $myAlbums = Engine_Api::_()->getDbtable('albums', 'seslisting')->editPhotos();
    $albumOptions = array('' => '');
    ...
    // Make form
    $this->view->form = $form = new Seslisting_Form_Album_Photos();
    foreach ($paginator as $photo) {
      $subform = new Seslisting_Form_Album_EditPhoto(array('elementsBelongTo' => $photo->getGuid()));
    ...
    try {
      ...
      foreach ($paginator as $photo) {
        ...
        if (isset($values['delete']) && $values['delete'] == '1') {
          ...
          if (($nextPhoto instanceof Seslisting_Model_Photo) &&
