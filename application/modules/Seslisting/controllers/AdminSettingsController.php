 ...
 * @version    $Id: AdminSettingsController.php  2019-04-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seslisting_AdminSettingsController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $db = Zend_Db_Table_Abstract::getDefaultAdapter();

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('seslisting_admin_main', array(), 'seslisting_admin_main_settings');

    $this->view->form  = $form = new Seslisting_Form_Admin_Global();

    if( $this->getRequest()->isPost() && $form->isValid($this->_getAllParams()) ) {

      $values = $form->getValues();
      include_once APPLICATION_PATH . "/application/modules/Seslisting/controllers/License.php";

      $db = Engine_Db_Table::getDefaultAdapter();

      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('seslisting.pluginactivated')) {

        // Listing Design Layout
        if (isset($values['seslisting_chooselayout']))
          $values['seslisting_chooselayout'] = serialize($values['seslisting_chooselayout']);
        else
          $values['seslisting_chooselayout'] = serialize(array());

        //Start Landing page set
        if (isset($_POST['seslisting_changelanding']) && $_POST['seslisting_changelanding'] == 1) {
          $this->landingPageSetup();
				}
        //End Landing Page set

        //START TEXT CHNAGE WORK IN CSV FILE
        $oldSigularWord = Engine_Api::_()->getApi('settings', 'core')->getSetting('seslisting.text.singular', 'listing');
        $oldPluralWord = Engine_Api::_()->getApi('settings', 'core')->getSetting('seslisting.text.plural', 'listings');
        $newSigularWord = @$values['seslisting_text_singular'] ? @$values['seslisting_text_singular'] : 'listing';
        $newPluralWord = @$values['seslisting_text_plural'] ? @$values['seslisting_text_plural'] : 'listings';
...
// 			 $catId = $db->query("SELECT category_id FROM `engine4_seslisting_categories` WHERE category_name = '" . $data[0] . "' AND (thumbnail = 0 OR cat_icon = 0)")->fetchAll();
...
// 					$cat_icon = 0;
//
// 			 $db->query("UPDATE `engine4_seslisting_categories` SET `cat_icon` = '" . $cat_icon . "',`thumbnail` = '" . $thumbnail_icon . "' WHERE category_id = " . $catId);
// 			 $db->query("UPDATE `engine4_seslisting_categories` set `title` = category_name WHERE category_id = ".$catId ." AND ( title IS NULL OR title = '')");
//        $db->query("UPDATE `engine4_seslisting_categories` set `slug` = LOWER(REPLACE(REPLACE(REPLACE(REPLACE(category_name,'&',''),'  ',' '),' ','-'),'/','-'))  WHERE category_id = ".$catId ." AND ( title IS NULL OR slug = '')");
//        $db->query("UPDATE `engine4_seslisting_categories` SET `order` = `category_id`  WHERE category_id = ".$catId ." AND ( `order` IS NULL OR `order` = 0)");
//
// 		}
//
// 	}
  ...
  function setCategoryPhoto($file, $cat_id, $resize = false) {
    ...
    $params = array(
        'parent_type' => 'seslisting_category',
        'parent_id' => $cat_id,
  ...
  public function categoriesAction()
  {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
      ->getNavigation('seslisting_admin_main', array(), 'seslisting_admin_main_categories');

    $this->view->categories = Engine_Api::_()->getItemTable('seslisting_category')->fetchAll();
  }
  ...
  public function addCategoryAction()
  {
    ...
    $form = $this->view->form = new Seslisting_Form_Admin_Category();
    ...
    $categoryTable = Engine_Api::_()->getItemTable('seslisting_category');
  ...
  public function deleteCategoryAction()
  {
    ...
    $this->view->listing_id = $this->view->category_id = $category_id;
  ...
  public function editCategoryAction()
  {
    ...
    $this->_helper->layout->setLayout('admin-simple');
    $category_id = $this->_getParam('id');
    $this->view->listing_id = $this->view->category_id = $id;
    $categoriesTable = Engine_Api::_()->getDbtable('categories', 'seslisting');
    $category = $categoriesTable->find($category_id)->current();
  ...
  //site statis for seslisting plugin
  public function statisticAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('seslisting_admin_main', array(), 'seslisting_admin_main_statistic');

    $listingTable = Engine_Api::_()->getDbtable('seslistings', 'seslisting');
    $listingTableName = $listingTable->info('name');

    $listingPhotoTable = Engine_Api::_()->getDbtable('photos', 'seslisting');
    $listingPhotoTableName = $listingPhotoTable->info('name');

    $listingAlbumTable = Engine_Api::_()->getDbtable('albums', 'seslisting');
    $listingAlbumTableName = $listingAlbumTable->info('name');

    //Total Listings
    $select = $listingTable->select()->from($listingTableName, 'count(*) AS totallisting');
    $this->view->totallisting = $select->query()->fetchColumn();

    //Total approved listing
    $select = $listingTable->select()->from($listingTableName, 'count(*) AS totalapprovedlisting')->where('is_approved =?', 1);
    $this->view->totalapprovedlisting = $select->query()->fetchColumn();

    //Total verified listing
    $select = $listingTable->select()->from($listingTableName, 'count(*) AS totalverified')->where('verified =?', 1);
    $this->view->totallistingverified = $select->query()->fetchColumn();

    //Total featured listing
    $select = $listingTable->select()->from($listingTableName, 'count(*) AS totalfeatured')->where('featured =?', 1);
    $this->view->totallistingfeatured = $select->query()->fetchColumn();

    //Total sponsored listing
    $select = $listingTable->select()->from($listingTableName, 'count(*) AS totalsponsored')->where('sponsored =?', 1);
    $this->view->totallistingsponsored = $select->query()->fetchColumn();

    //Total albums of listing
    $select = $listingAlbumTable->select()->from($listingAlbumTableName, 'count(*) AS totalalbums');
    $this->view->totalalbums = $select->query()->fetchColumn();

    //Total photos of listing
    $select = $listingPhotoTable->select()->from($listingPhotoTableName, 'count(*) AS totalphotos');
    ...
    if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesvideo')) {
			...
			//Total videos of listing
			$select = $videoTable->select()->from($videoTableName, 'count(*) AS totalvideos')->where('parent_type =?', 'seslisting');
			$this->view->totalvideos = $select->query()->fetchColumn();
    }

    //Total favourite listing
    $select = $listingTable->select()->from($listingTableName, 'count(*) AS totalfavourite')->where('favourite_count <>?', 0);
    $this->view->totallistingfavourite = $select->query()->fetchColumn();

    //Total comments listing
    $select = $listingTable->select()->from($listingTableName, 'count(*) AS totalcomment')->where('comment_count <>?', 0);
    $this->view->totallistingcomments = $select->query()->fetchColumn();

     //Total view listing
    $select = $listingTable->select()->from($listingTableName, 'count(*) AS totalview')->where('view_count <>?', 0);
    $this->view->totallistingviews = $select->query()->fetchColumn();

     //Total like listing
    $select = $listingTable->select()->from($listingTableName, 'count(*) AS totallike')->where('like_count <>?', 0);
    $this->view->totallistinglikes = $select->query()->fetchColumn();

    //Total rated listing
    $select = $listingTable->select()->from($listingTableName, 'count(*) AS totalrated')->where('rating <>?', 0);
    $this->view->totallistingrated = $select->query()->fetchColumn();
  }

  public function manageWidgetizePageAction() {

    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('seslisting_admin_main', array(), 'seslisting_admin_main_managepages');

    $this->view->pagesArray = array('seslisting_index_welcome', 'seslisting_index_home', 'seslisting_index_browse', 'seslisting_category_browse', 'seslisting_index_locations', 'seslisting_review_browse', 'seslisting_index_manage', 'seslisting_index_claim', 'seslisting_index_create', 'seslisting_index_view_1', 'seslisting_index_view_2','seslisting_index_view_3','seslisting_index_view_4', 'seslisting_index_tags', 'seslisting_review_view', 'seslisting_album_view', 'seslisting_photo_view', 'seslisting_index_claim-requests', 'seslisting_index_list', 'seslisting_category_index');
  }
  ...
  public function landingPageSetup() {
    ...
    $db->insert('engine4_core_content', array(
      ...
      'parent_content_id' => $main_middle_id,
      'order' => $widgetOrder++,
      'params' => '{"bodysimple":"<div style=\"text-align: center;margin-top:30px; box-shadow:inset 0 1px 0 rgba(255,255,255,.1),0 1px 0 rgba(8,32,84,.1);padding-bottom: 30px;\">\r\n\t<a class=\"seslisting_welcome_btn sesbasic_animation\" href=\"listings\/categories\">Browse All Categories\r\n<\/a><\/div>\r\n<div style=\"font-size: 34px;margin-bottom: 30px;  margin-top: 30px;text-align: center;\">Meet our Top Posters!\r\n<\/span><\/div>","en_US_bodysimple":"<div style=\"text-align: center;margin-top:30px; box-shadow:inset 0 1px 0 rgba(255,255,255,.1),0 1px 0 rgba(8,32,84,.1);padding-bottom: 30px;\">\r\n\t<a class=\"seslisting_welcome_btn sesbasic_animation\" href=\"listings\/categories\">Browse All Categories\r\n<\/a><\/div>\r\n<div style=\"font-size: 34px;margin-bottom: 30px;  margin-top: 30px;text-align: center;\">Meet our Top Posters!\r\n<\/span><\/div>","show_content":"1","title":"","nomobile":"0","name":"sesbasic.simple-html-block"}',
    ));
    $db->insert('engine4_core_content', array(
      'page_id' => $page_id,
      'type' => 'widget',
