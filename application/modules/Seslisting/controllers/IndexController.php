 ...
 * @version    $Id: IndexController.php  2019-04-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seslisting_IndexController extends Core_Controller_Action_Standard
{
  public function init() {
		// only show to member_level if authorized
    if( !$this->_helper->requireAuth()->setAuthParams('seslisting', null, 'view')->isValid() ) return;
    $id = $this->_getParam('listing_id', $this->_getParam('id', null));
  ...
  public function browseListingsAction() {

    $integrateothermodule_id = $this->_getParam('integrateothermodule_id', null);
    $page = 'seslisting_index_' . $integrateothermodule_id;
    //Render
  ...
  public function claimRequestsAction() {
    ...
    $checkClaimRequest = Engine_Api::_()->getDbTable('claims', 'seslisting')->claimCount();
  ...
  public function viewAction() {
    ...
    if( !$seslisting || !$seslisting->getIdentity() || ((!$seslisting->is_approved || $seslisting->draft) && !$seslisting->isOwner($viewer)) )
    return $this->_helper->requireSubject->forward();

    //Privacy: networks and member level based
    if (Engine_Api::_()->authorization()->isAllowed('seslisting', $seslisting->getOwner(), 'allow_levels') || Engine_Api::_()->authorization()->isAllowed('seslisting', $seslisting->getOwner(), 'allow_networks')) {
        $returnValue = Engine_Api::_()->seslisting()->checkPrivacySetting($seslisting->getIdentity());
    ...
    $select = $table->select()
            ->where('type = ?', $seslisting->getType())
            ->where('id = ?', $seslisting->getIdentity())
            ->limit();
    ...
    $view = Zend_Registry::isRegistered('Zend_View') ? Zend_Registry::get('Zend_View') : null;
		...
		if (!empty($getmodule->version) && version_compare($getmodule->version, '4.8.8') >= 0) {
			...
			if($seslisting->seo_title)
        $view->headTitle($seslisting->seo_title, 'SET');
			if($seslisting->seo_keywords)
        $view->headMeta()->appendName('keywords', $seslisting->seo_keywords);
			if($seslisting->seo_description)
        $view->headMeta()->appendName('description', $seslisting->seo_description);
		}

		if($seslisting->style == 1)
		$page = 'seslisting_index_view_1';
		elseif($seslisting->style == 2)
		$page = 'seslisting_index_view_2';
		elseif($seslisting->style == 3)
		$page = 'seslisting_index_view_3';
		elseif($seslisting->style == 4)
		$page = 'seslisting_index_view_4';
  ...
  public function manageAction() {
    ...
    // Prepare data
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->form = $form = new Seslisting_Form_Search();
    $this->view->canCreate = $this->_helper->requireAuth()->setAuthParams('seslisting', null, 'create')->checkRequire();

    $form->removeElement('show');

    // Populate form
    $categories = Engine_Api::_()->getDbtable('categories', 'seslisting')->getCategoriesAssoc();
  ...
  public function listAction() {
    ...
    // Make form
    $form = new Seslisting_Form_Search();
    ...
    // Get paginator
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('seslistings', 'seslisting')->getSeslistingsPaginator($values);
    $items_per_page = Engine_Api::_()->getApi('settings', 'core')->seslisting_page;
  ...
  public function createAction() {
    if( !$this->_helper->requireUser()->isValid() ) return;
    if( !$this->_helper->requireAuth()->setAuthParams('seslisting', null, 'create')->isValid()) return;
      ...
      $this->view->language = $language[0];
		...
		if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('seslistingpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('seslistingpackage.enable.package', 1)){
			$package = Engine_Api::_()->getItem('seslistingpackage_package',$this->_getParam('package_id',0));
			$existingpackage = Engine_Api::_()->getItem('seslistingpackage_orderspackage',$this->_getParam('existing_package_id',0));
			if($existingpackage){
				$package = Engine_Api::_()->getItem('seslistingpackage_package',$existingpackage->package_id);
			}
			if (!$package && !$existingpackage){
				//check package exists for this member level
				$packageMemberLevel = Engine_Api::_()->getDbTable('packages','seslistingpackage')->getPackage(array('member_level'=>$viewer->level_id));
				if(count($packageMemberLevel)){
					//redirect to package page
					return $this->_helper->redirector->gotoRoute(array('action'=>'listing'), 'seslistingpackage_general', true);
				}
			}
		}
    ...
    $parentId = $this->_getParam('parent_id', 0);
		if($parentId && !Engine_Api::_()->getApi('settings', 'core')->getSetting('seslisting.enable.sublisting', 1)){
			return $this->_forward('notfound', 'error', 'core');
		}
    $seslisting_create = Zend_Registry::isRegistered('seslisting_create') ? Zend_Registry::get('seslisting_create') : null;
    if (empty($seslisting_create))
      return $this->_forward('notfound', 'error', 'core');
    $values['user_id'] = $viewer->getIdentity();
    $paginator = Engine_Api::_()->getDbtable('seslistings', 'seslisting')->getSeslistingsPaginator($values);

    $this->view->quota = $quota = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'seslisting', 'max');
    $this->view->current_count = $paginator->getTotalItemCount();

    $this->view->categories = Engine_Api::_()->getDbtable('categories', 'seslisting')->getCategoriesAssoc();

    // Prepare form
    $this->view->form = $form = new Seslisting_Form_Create(array('defaultProfileId' => $defaultProfileId,'smoothboxType'=>$sessmoothbox,));
    ...
    if (isset($_POST['custom_url']) && !empty($_POST['custom_url'])) {
      $custom_url = Engine_Api::_()->getDbtable('seslistings', 'seslisting')->checkCustomUrl($_POST['custom_url']);
      if ($custom_url) {
    ...
    try {
      ...
      $values['subcat_id'] = 0;
			if(isset($package)){
				...
				if($existingpackage){
					...
					$values['is_approved'] = Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('seslisting', $viewer, 'listing_approve');
				}
			}else{
				$values['is_approved'] = Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('seslisting', $viewer, 'listing_approve');
				if(isset($seslisting->package_id) && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('seslistingpackage') ){
					$values['package_id'] = Engine_Api::_()->getDbTable('packages','seslistingpackage')->getDefaultPackage();
				}
			}
      ...
      $seslisting->setFromArray($values);
			...
			if(isset($_FILES['photo_file']) && $_FILES['photo_file']['name'] != ''){
			  $seslisting->photo_id = Engine_Api::_()->sesbasic()->setPhoto($form->photo_file, false,false,'seslisting','seslisting','',$seslisting,true);
      	...
      	$seslisting->publish_date =$starttime;
			...
			if(isset($_POST['start_date']) && $viewer->timezone && $_POST['start_date'] != ''){
				...
				$seslisting->publish_date = date('Y-m-d H:i:s', $start);
			}

			$seslisting->parent_id = $parentId;
      $seslisting->save();
      $listing_id = $seslisting->listing_id;

      if (!empty($_POST['custom_url']) && $_POST['custom_url'] != '')
      $seslisting->custom_url = $_POST['custom_url'];
      else
      $seslisting->custom_url = $seslisting->listing_id;
      $seslisting->save();
      $listing_id = $seslisting->listing_id;
      ...
      $roleTable = Engine_Api::_()->getDbtable('roles', 'seslisting');
        ...
        $seslisting->resource_id = $resource_id;
        $seslisting->resource_type = $resource_type;
        $seslisting->save();
			}

      if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
					Engine_Db_Table::getDefaultAdapter()->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $listing_id . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","seslisting")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }

      if($parentType == 'sesevent_listing') {
        $seslisting->parent_type = $parentType;
        $seslisting->event_id = $event_id;
        $seslisting->save();
        $seseventlisting = Engine_Api::_()->getDbtable('mapevents', 'seslisting')->createRow();
        $seseventlisting->event_id = $event_id;
        $seseventlisting->listing_id = $listing_id;
        $seseventlisting->save();
      }

      if(isset ($_POST['cover']) && !empty($_POST['cover'])) {
				$seslisting->photo_id = $_POST['cover'];
				$seslisting->save();
      }
     ...
     // $seslisting->seo_keywords = implode(',',$tags);
      //$seslisting->seo_title = $seslisting->title;
      $seslisting->save();
      $seslisting->tags()->addTagMaps($viewer, $tags);

      $session = new Zend_Session_Namespace();
      if(!empty($session->album_id)){
				$album_id = $session->album_id;
				if(isset($listing_id) && isset($seslisting->title)){
					Engine_Api::_()->getDbTable('albums', 'seslisting')->update(array('listing_id' => $listing_id,'owner_id' => $viewer->getIdentity(),'title' => $seslisting->title), array('album_id = ?' => $album_id));
					if(isset ($_POST['cover']) && !empty($_POST['cover'])) {
						Engine_Api::_()->getDbTable('albums', 'seslisting')->update(array('photo_id' => $_POST['cover']), array('album_id = ?' => $album_id));
					}
					Engine_Api::_()->getDbTable('photos', 'seslisting')->update(array('listing_id' => $listing_id), array('album_id = ?' => $album_id));
					unset($session->album_id);
				}
      }

      // Add activity only if seslisting is published
      if( $values['draft'] == 0 && $values['is_approved'] == 1 && (!$seslisting->publish_date || strtotime($seslisting->publish_date) <= time())) {
        ...
        //Send notifications for subscribers
      	Engine_Api::_()->getDbtable('subscriptions', 'seslisting')->sendNotifications($seslisting);
      ...
      throw $e;
		...
		$redirect = Engine_Api::_()->getApi('settings', 'core')->getSetting('seslisting.redirect.creation', 0);
    if($parentType) {
      ...
      die;
   	 	...
   	 	return $this->_helper->redirector->gotoRoute(array('action' => 'dashboard','action'=>'edit','listing_id'=>$seslisting->custom_url),'seslisting_dashboard',true);
    } else {
		 	return $this->_helper->redirector->gotoRoute(array('action' => 'view','listing_id'=>$seslisting->custom_url),'seslisting_entry_view',true);
    }
  }
  ...
  function likeAction() {
    ...
    $type = 'seslisting';
    $dbTable = 'seslistings';
    $resorces_id = 'listing_id';
    $notificationType = 'liked';
    $actionType = 'seslisting_like';

		if($this->_getParam('type',false) && $this->_getParam('type') == 'seslisting_album'){
			$type = 'seslisting_album';
	    $dbTable = 'albums';
	    $resorces_id = 'album_id';
	    $actionType = 'seslisting_album_like';
		} else if($this->_getParam('type',false) && $this->_getParam('type') == 'seslisting_photo') {
			$type = 'seslisting_photo';
	    $dbTable = 'photos';
	    $resorces_id = 'photo_id';
	    $actionType = 'seslisting_photo_like';
		}
    ...
    $itemTable = Engine_Api::_()->getDbtable($dbTable, 'seslisting');
    ...
    if (count($result) > 0) {
      ...
      $owner = $subject->getOwner();
	     if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
	       ...
	       if (!$result) {
          if($subject && empty($subject->title) && $this->_getParam('type') == 'seslisting_photo') {
            $album_id = $subject->album_id;
            $subject = Engine_Api::_()->getItem('seslisting_album', $album_id);
          }
  ...
  function favouriteAction() {
    ...
    if ($this->_getParam('type') == 'seslisting') {
      $type = 'seslisting';
      $dbTable = 'seslistings';
      $resorces_id = 'listing_id';
      $notificationType = 'seslisting_favourite';
    } else if ($this->_getParam('type') == 'seslisting_photo') {
      $type = 'seslisting_photo';
      $dbTable = 'photos';
      $resorces_id = 'photo_id';
     // $notificationType = 'sesevent_favourite_playlist';
    }
     else if ($this->_getParam('type') == 'seslisting_album') {
      $type = 'seslisting_album';
      $dbTable = 'albums';
      $resorces_id = 'album_id';
     // $notificationType = 'sesevent_favourite_playlist';
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $Fav = Engine_Api::_()->getDbTable('favourites', 'seslisting')->getItemfav($type, $item_id);

    $favItem = Engine_Api::_()->getDbtable($dbTable, 'seslisting');
    if (count($Fav) > 0) {
      //delete
      ...
      //update
      $db = Engine_Api::_()->getDbTable('favourites', 'seslisting')->getAdapter();
      $db->beginTransaction();
      try {
        $fav = Engine_Api::_()->getDbTable('favourites', 'seslisting')->createRow();
  ...
  public function deleteAction() {
    ...
    $seslisting = Engine_Api::_()->getItem('seslisting', $this->getRequest()->getParam('listing_id'));
    if( !$this->_helper->requireAuth()->setAuthParams($seslisting, null, 'delete')->isValid()) return;

    // In smoothbox
    $this->_helper->layout->setLayout('default-simple');

    $this->view->form = $form = new Seslisting_Form_Delete();

    if( !$seslisting ) {
      $this->view->status = false;
      $this->view->error = Zend_Registry::get('Zend_Translate')->_("Seslisting entry doesn't exist or not authorized to delete");
      return;
    }
    ...
    $db = $seslisting->getTable()->getAdapter();
    $db->beginTransaction();
    ...
    $this->view->message = Zend_Registry::get('Zend_Translate')->_('Your seslisting entry has been deleted.');
    return $this->_forward('success' ,'utility', 'core', array(
      'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'manage'), 'seslisting_general', true),
  ...
  public function styleAction() {
    ...
    if( !$this->_helper->requireUser()->isValid() ) return;
    if( !$this->_helper->requireAuth()->setAuthParams('seslisting', null, 'style')->isValid()) return;
    ...
    // Make form
    $this->view->form = $form = new Seslisting_Form_Style();

    // Get current row
    $table = Engine_Api::_()->getDbtable('styles', 'core');
    $select = $table->select()
      ->where('type = ?', 'user_seslisting') // @todo this is not a real type
    ...
    if( null == $row ) {
      $row = $table->createRow();
      $row->type = 'user_seslisting'; // @todo this is not a real type
  ...
  public function linkListingAction() {
    ...
    $listingMapTable = Engine_Api::_()->getDbTable('mapevents', 'seslisting');
    $listingMapTableName = $listingMapTable->info('name');
    $select = $eventTable->select()
			->setIntegrityCheck(false)
			->from($eventTableName)
		        ->Joinleft($listingMapTableName, "$listingMapTableName.event_id = $eventTableName.event_id", null)
		        ->where($eventTableName.'.event_id !=?', '')
		        ->where($listingMapTableName.'.listing_id !=? or listing_id is null', $listing_id);
   ...
   return;
    ...
    $listingObject = Engine_Api::_()->getItem('seslisting', $listing_id);
    foreach($eventIds as $eventId) {
      $item = Engine_Api::_()->getItem('sesevent_event', $eventId);
      $owner = $item->getOwner();
      $table = Engine_Api::_()->getDbtable('mapevents', 'seslisting');
      $db = $table->getAdapter();
      $db->beginTransaction();
      try {
        $seseventlisting = $table->createRow();
	$seseventlisting->event_id = $eventId;
	$seseventlisting->listing_id = $listing_id;
	$seseventlisting->request_owner_listing = 1;
	$seseventlisting->approved = 0;
	$seseventlisting->save();
	$listingPageLink = '<a href="' . $listingObject->getHref() . '">' . ucfirst($listingObject->getTitle()) . '</a>';
	Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $item, 'seslisting_link_event', array("listingPageLink" => $listingPageLink));
  ...
  public function listingRequestAction() {
    ...
    $mapListingTable = Engine_Api::_()->getDbtable('mapevents', 'seslisting');
    $mapListingTableName = $mapListingTable->info('name');
    $viewerId = Engine_Api::_()->user()->getViewer()->getIdentity();
    $select = $ListingTable->select()
                        ->setIntegrityCheck(false)
			->from($ListingTableName, array('owner_id', 'listing_id'))
		        ->Joinleft($mapListingTableName, "$mapListingTableName.listing_id = $ListingTableName.listing_id", array('event_id','approved'))
		        ->where($ListingTableName.'.owner_id =?', $viewerId)
		        ->where($mapListingTableName.'.approved =?', 0)
		         ->where($mapListingTableName.'.request_owner_event =? and request_owner_event IS NOT null', 1);
    $page = isset($_POST['page']) ? $_POST['page'] : 1;
    $this->view->paginator = $paginator = Zend_Paginator::factory($select);
    $paginator->setCurrentPageNumber($page);
    $paginator->setItemCountPerPage(10);
  ...
  public function approvedAction() {
    ...
    $mapListingTable = Engine_Api::_()->getDbtable('mapevents', 'seslisting');
    $selectMapTable = $mapListingTable->select()->where('event_id =?', $event_id)->where('listing_id =?', $listing_id)->where('request_owner_event =?', 1);
    $mapResult = $mapListingTable->fetchRow($selectMapTable);
    if (!empty($listing_id)) {
      $listing = Engine_Api::_()->getItem('seslisting', $event_id);
      ...
      $db->update('engine4_seslisting_mapevents', array(
      'approved' => $approved,
      ), array(
	'event_id = ?' => $event_id,
    ...
    $this->_redirect('seslistings/listing-request');
  }
  ...
  public function rejectRequestAction() {
    ...
    try {
      $mapResult->delete();
      $listingPageLink = '<a href="' . $listingObject->getHref() . '">' . ucfirst($listingObject->getTitle()) . '</a>';
      Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $eventObject, 'seslisting_reject_event_request', array("listingPageLink" => $listingPageLink));
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
  ...
  protected function setPhoto($photo, $id) {
    ...
    try {
      ...
      if ($e->getCode() == Storage_Model_DbTable_Files::SPACE_LIMIT_REACHED_CODE) {
        throw new Seslisting_Model_Exception($e->getMessage(), $e->getCode());
      } else {
        throw $e;
      }
    }
  ...
  public function subcategoryAction() {
    ...
    if ($category_id) {
      $categoryTable = Engine_Api::_()->getDbtable('categories', 'seslisting');
  ...
  public function editPhotoAction() {
    ...
    $this->view->photo = Engine_Api::_()->getItem('seslisting_photo', $photo_id);
  }
  ...
  public function saveInformationAction() {
    ...
    Engine_Api::_()->getDbTable('photos', 'seslisting')->update(array('title' => $title, 'description' => $description), array('photo_id = ?' => $photo_id));
  }
  ...
  public function removeAction() {
    ...
    $db->beginTransaction();
    try {
      Engine_Api::_()->getDbtable('photos', 'seslisting')->delete(array('photo_id =?' => $photo_id));
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
  ...
  public function getListingAction() {
    ...
    $value['search'] = 1;
		$value['fetchAll'] = true;
		$value['getlisting'] = true;
    $listings = Engine_Api::_()->getDbtable('seslistings', 'seslisting')->getSeslistingsSelect($value);
    foreach ($listings as $listing) {
      $video_icon = $this->view->itemPhoto($listing, 'thumb.icon');
      $sesdata[] = array(
  ...
  public function shareAction() {
    ...
    $typeItem = ucwords(str_replace(array('seslisting_'), '', $attachment->getType()));
  ...
  public function locationAction() {
    ...
    $this->view->type = $this->_getParam('type', 'listing');
    $this->view->listing_id = $listing_id = $this->_getParam('listing_id');
    $this->view->listing = $listing = Engine_Api::_()->getItem('seslisting', $listing_id);
    if (!$listing)
      return;
    $this->view->form = $form = new Seslisting_Form_Location();
  ...
  public function customUrlCheckAction(){
    ...
    $custom_url = Engine_Api::_()->getDbtable('seslistings', 'seslisting')->checkCustomUrl($value,$listing_id);
    if($custom_url){
  ...
  function sanitize($string, $force_lowercase = true, $anal = false) {
	...
	public function getListingsAction() {
		...
		$selectClaimTable = $listingClaimTable->select()
		->from($listingClaimTableName, 'listing_id')
		->where('user_id =?', $viewerId);
		$claimedListings = $listingClaimTable->fetchAll($selectClaimTable);
		...
		->where($listingTableName .'.title  LIKE ? ', '%' .$text. '%');
		if(count($claimedListings) > 0)
		$select->where('listing_id NOT IN(?)', $selectClaimTable);
		$select->order('listing_id ASC')->limit('40');
		$listings = $listingTable->fetchAll($select);
		foreach ($listings as $listing) {
			$listing_icon_photo = $this->view->itemPhoto($listing, 'thumb.icon');
			$sesdata[] = array(
			'id' => $listing->listing_id,
			'label' => $listing->title,
			'photo' => $listing_icon_photo
			);
		}
		return $this->_helper->json($sesdata);
	}
