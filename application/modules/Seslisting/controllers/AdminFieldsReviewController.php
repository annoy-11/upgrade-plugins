 ...
 * @version    $Id: AdminFieldsReviewController.php  2019-04-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seslisting_AdminFieldsReviewController extends Fields_Controller_AdminAbstract {

  protected $_requireProfileType = true;
  protected $_fieldType = 'seslistingreview';
  ...
  public function indexAction() {
    ...
    $this->view->subnavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('seslisting_admin_main_rwsettings', array(), 'seslisting_admin_main_subreviewparameters');
  ...
  public function fieldEditAction() {
    ...
    // Create form
    $this->view->form = $form = new $adminFormClass();
    $form->setTitle('Edit Listing Category Review Profile Question');
    $form->removeElement('show');
    $form->removeElement('search');
    $form->addElement('hidden', 'show', array('value' => 0));
    $form->removeElement('display');
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Seslisting_Form_Admin_Review_Type();
    if (!$this->getRequest()->isPost()) {
      return;
    }
    if (!$form->isValid($this->getRequest()->getPost())) {
    ...
    if ($form->getValue('duplicate') != 'null') {
      ...
      $field_meta_array = $db->select()
              ->from('engine4_seslistingreview_fields_meta')
              ->where('field_id IN (' . implode(', ', $child_id_array) . ')')
              ->query()
              ->fetchAll();
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_seslistingreview_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_seslistingreview_fields_maps', array(
