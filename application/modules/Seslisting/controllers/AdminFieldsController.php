 ...
 * @version    $Id: AdminFieldsController.php  2019-04-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Seslisting_AdminFieldsController extends Fields_Controller_AdminAbstract {

  protected $_requireProfileType = true;
  protected $_fieldType = 'seslisting';
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('seslisting_admin_categories', array(), 'seslisting_admin_main_subfields');
    parent::indexAction();
  }
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Seslisting_Form_Admin_Settings_Type();
    ...
    if ($form->getValue('duplicate') != 'null' && $form->getValue('duplicate') ) {
      // Create New Option in engine4_seslisting_contactus_fields_options
      ...
      $field_meta_array = $db->select()
              ->from('engine4_seslisting_fields_meta')
      ...
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_seslisting_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_seslisting_fields_maps', array(
  ...
  public function typeDeleteAction() {
    ...
    if (!empty($option_id)) {
      Engine_Api::_()->getDbtable('categories', 'seslisting')->update(array('profile_type' => 0), array('profile_type = ?' => $option_id));
    }
