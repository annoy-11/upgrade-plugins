...
class Seslisting_ReviewController extends Core_Controller_Action_Standard {
	...
	public function init() {
		...
		if($listing_id = $this->_getParam('listing_id')){
			$listing = Engine_Api::_()->getItem('seslisting',$listing_id);
		}

		//Get subject
		if (null !== ($review_id = $this->_getParam('review_id')) && null !== ($review = Engine_Api::_()->getItem('seslistingreview', $review_id)) && $review instanceof Seslisting_Model_Review && !Engine_Api::_()->core()->hasSubject()) {
			$listing = $review->getParent();
			Engine_Api::_()->core()->setSubject($review);
		}
		if(isset($listing) && $listing && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('seslistingpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('seslistingpackage.enable.package', 1)){
  ...
  public function createAction() {
		...
		$viewer = Engine_Api::_()->user()->getViewer();
		$this->view->item = $item = Engine_Api::_()->getItem('seslisting', $this->getParam('listing_id'));
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('seslistingreview', 'create'))
		return $this->_forward('notfound', 'error', 'core');
		if(!$item)
		return $this->_forward('notfound', 'error', 'core');
		//check review exists
		$isReview = Engine_Api::_()->getDbtable('seslistingreviews', 'seslisting')->isReview(array('listing_id' => $item->getIdentity()));
		if(Engine_Api::_()->getApi('settings', 'core')->getSetting('seslisting.allow.owner', 1)){
			$allowedCreate = true;
		}
		else{
    ...
    $this->view->form = $form = new Seslisting_Form_Review_Create(array('defaultProfileId' => $defaultProfileId));
    ...
    $values['listing_id'] = $item->getIdentity();
    $reviews_table = Engine_Api::_()->getDbtable('seslistingreviews', 'seslisting');
    ...
    try {
      ...
      $review->save();
		  $dbObject = Engine_Db_Table::getDefaultAdapter();
			...
			$table = Engine_Api::_()->getDbtable('parametervalues', 'seslisting');
			...
			foreach($_POST as $key => $reviewC){
				...
				if(!is_numeric($key))
					continue;
				$parameter = Engine_Api::_()->getItem('seslisting_parameter',$key);
			...
			if(isset($item->rating)){
				$item->rating = Engine_Api::_()->getDbtable('seslistingreviews', 'seslisting')->getRating($review->listing_id);
				$item->save();
			}
      ...
      $customfieldform->saveValues();
			$review->save();
      $db->commit();
      return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'review_id' => $review->review_id, 'slug' => $review->getSlug()), 'seslistingreview_view', true);
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    }
  ...
  public function editAction() {
    ...
    $review_id = $this->_getParam('review_id', null);
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('seslistingreview', 'edit'))
    return $this->_forward('notfound', 'error', 'core');
		$this->view->item = $item = Engine_Api::_()->getItem('seslisting', $subject->listing_id);
		if (isset($item->category_id) && $item->category_id != 0)
		$this->view->category_id = $item->category_id;
		else
		$this->view->category_id = 0;
    ...
    if(!$review_id || !$subject)
	  return $this->_forward('notfound', 'error', 'core');
    $this->view->defaultProfileId = $defaultProfileId = Engine_Api::_()->getDbTable('reviewmetas', 'seslisting')->profileFieldId();
    $this->view->form = $form = new Seslisting_Form_Review_Edit(array('defaultProfileId' => $defaultProfileId));
    ...
    try {
      ...
      $subject->save();
			...
			if(isset($item->rating)){
				$item->rating = Engine_Api::_()->getDbtable('seslistingreviews', 'seslisting')->getRating($subject->listing_id);
				$item->save();
			}
			 //Add fields
      $customfieldform = $form->getSubForm('fields');
      ...
      $customfieldform->saveValues();
			$subject->save();
      $db->commit();
      return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'review_id' => $subject->review_id, 'slug' => $subject->getSlug()), 'seslistingreview_view', true);
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    }
  ...
  public function deleteAction() {
    $viewer = Engine_Api::_()->user()->getViewer();
    $review = Engine_Api::_()->getItem('seslistingreview', $this->getRequest()->getParam('review_id'));
    $content_item = Engine_Api::_()->getItem('seslisting', $review->listing_id);
  ...
  public function viewAction() {
		...
		return $this->_forward('notfound', 'error', 'core');
		$review_id = $this->_getParam('review_id', null);
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('seslistingreview', 'view'))
		return $this->_forward('notfound', 'error', 'core');
  ...
  function likeAction() {
    ...
    $itemTable = Engine_Api::_()->getItemTable('seslistingreview');
    $tableLike = Engine_Api::_()->getDbtable('likes', 'core');
    $tableMainLike = $tableLike->info('name');
    $select = $tableLike->select()
            ->from($tableMainLike)
            ->where('resource_type = ?', 'seslistingreview')
    ...
    if (count($result) > 0) {
      ...
      try {
        ...
        $like->resource_type = 'seslistingreview';
