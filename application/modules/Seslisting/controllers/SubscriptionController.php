 ...
 * @version    $Id: SubscriptionController.php  2019-04-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Seslisting_SubscriptionController extends Core_Controller_Action_Standard {
  ...
  public function init() {
    ...
    // Get subject
    if( ($seslisting_id = $this->_getParam('listing_id')) &&
        ($seslisting = Engine_Api::_()->getItem('seslisting', $seslisting_id)) instanceof Seslisting_Model_Listing ) {
      $subject = $seslisting->getOwner('user');
  ...
  public function addAction() {
    ...
    if( $subscriptionTable->checkSubscription($user, $viewer) ) {
      ...
      $this->view->message = Zend_Registry::get('Zend_Translate')
          ->_('You are already subscribed to this member\'s listing.');
    ...
    $this->view->form = $form = new Core_Form_Confirm(array(
      'title' => 'Subscribe?',
      'description' => 'Would you like to subscribe to this member\'s listing?',
    ...
    $this->view->status = true;
    $this->view->message = Zend_Registry::get('Zend_Translate')
        ->_('You are now subscribed to this member\'s listing.');
  ...
  public function removeAction() {
    ...
    if( !$subscriptionTable->checkSubscription($user, $viewer) ) {
      $this->view->status = true;
      $this->view->message = Zend_Registry::get('Zend_Translate')
          ->_('You are already not subscribed to this member\'s listing.');
    ...
    $this->view->form = $form = new Core_Form_Confirm(array(
      'title' => 'Unsubscribe?',
      'description' => 'Would you like to unsubscribe from this member\'s listing?',
      'class' => 'global_form_popup',
    ...
    $this->view->status = true;
    $this->view->message = Zend_Registry::get('Zend_Translate')
        ->_('You are no longer subscribed to this member\'s listing.');
