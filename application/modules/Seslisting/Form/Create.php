...
class Seslisting_Form_Create extends Engine_Form
{
  public $_error = array();
  ...
  public function init() {
    if($this->getSmoothboxType())
      $hideClass = 'seslisting_hideelement_smoothbox';
    else
      $hideClass = '';
  ...
  $translate = Zend_Registry::get('Zend_Translate');
	 ...
	 if($package_id)
		...
		$existing_package = Engine_Api::_()->getItem('seslistingpackage_orderspackage',$existing_package_id);
		$package = Engine_Api::_()->getItem('seslistingpackage_package',$existing_package->package_id);
	}
    ...
    $this->setTitle('Write New Entry')
      ->setDescription('Compose your new listing entry below, then click "Post Entry" to publish the entry to your listing.')
      ->setAttrib('name', 'seslistings_create');

		if($this->getSmoothboxType())
			$this->setAttrib('class','global_form seslisting_smoothbox_create');
    ...
    $custom_url_value = isset($listing->custom_url) ? $listing->custom_url : (isset($_POST["custom_url"]) ? $_POST["custom_url"] : "");
    // Custom Url
    $this->addElement('Dummy', 'custom_url_listing', array(
	'label' => 'Custom Url',
	'content' => '<input type="text" name="custom_url" id="custom_url" value="' . $custom_url_value . '"><i class="fa fa-check" id="seslisting_custom_url_correct" style="display:none;"></i><i class="fa fa-close" id="seslisting_custom_url_wrong" style="display:none;"></i><span class="seslisting_check_availability_btn"><img src="application/modules/Core/externals/images/loading.gif" id="seslisting_custom_url_loading" alt="Loading" style="display:none;" /><button id="check_custom_url_availability" type="button" name="check_availability" >Check Availability</button></span> <p id="suggestion_tooltip" class="check_tooltip" style="display:none;">'.$translate->translate("You can use letters, numbers and periods.").'</p>',
    ));
    ...
    // prepare categories
    $categories = Engine_Api::_()->getDbtable('categories', 'seslisting')->getCategoriesAssoc(array('member_levels' => 1));
   ...
   if(((isset($modulesEnable) && array_key_exists('modules',$modulesEnable) && in_array('photo',$modulesEnable['modules'])) || empty($modulesEnable)) && $photoLeft){
			...
			$this->addElement('Dummy', 'tabs_form_listingcreate', array(
			'label' => 'Upload photos',
			//'required'=>$required,
			//'allowEmpty'=>$allowEmpty,
...
//                 continue;
  ...
  //       ));
    ...
    if($this->getSmoothboxType()) {
			$this->addElement('Cancel', 'advanced_seslistingoptions', array(
