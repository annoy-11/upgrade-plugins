	...
	<div class="sesbasic_cloud_widget sesbasic_clearfix">  
		...
		<ul>
			...
			<?php foreach($this->paginator as $value):?> 	
				...
				<?php endif;?>	
				<li>
					<a title="<?php echo $value['category_name'] ?>" href="<?php echo $this->url(array('module' =>'seslisting','controller' => 'index', 'action' => 'browse'),'seslisting_general',true).'?category_id='.$value['category_id']; ?>">
...
<?php elseif($this->showType = 'simple'):?>  
	<ul class="seslisting_sidebar_categroies_list">    
		<?php foreach( $this->paginator as $item ): ?>
			<li>
				<?php $subcategory = Engine_Api::_()->getDbtable('categories', 'seslisting')->getModuleSubcategory(array('column_name' => "*", 'category_id' => $item->category_id, 'countListings' => true)); ?>
				<?php if(counT($subcategory) > 0): ?>  
					<a id="seslisting_toggle_<?php echo $item->category_id ?>" class="cattoggel cattoggelright" href="javascript:void(0);" onclick="showCategory('<?php echo $item->getIdentity()  ?>')">
					</a>
				<?php endif; ?>     
				<a class="catlabel' ?>" href="<?php echo $this->url(array('module' =>'seslisting','controller' => 'index', 'action' => 'browse'),'seslisting_general',true).'?category_id='.$item->category_id; ?>"> 
					...
					<?php else:?>
						<img src="application/modules/Seslisting/externals/images/category/icons/image.png" />
					<?php endif;?>
					<span><?php echo $this->translate($item->category_name); ?>
						<?php echo ' ('.$item->total_listings_categories.')';?>
					</span>
				</a>
				<ul id="subcategory_<?php echo $item->getIdentity() ?>" style="display:none;">   	
					<?php foreach( $subcategory as $subCat ): ?>
						<li>
							<?php $subsubcategory = Engine_Api::_()->getDbtable('categories', 'seslisting')->getModuleSubsubcategory(array('column_name' => "*", 'category_id' => $subCat->category_id, 'countListings' => true)); ?>
							<?php if(count($subsubcategory) > 0): ?>   
								<a id="seslisting_subcat_toggle_<?php echo $subCat->category_id ?>" class="cattoggel cattoggelright" href="javascript:void(0);" onclick="showCategory('<?php echo $subCat->getIdentity()  ?>')"></a>
							<?php endif; ?> 
							<a class="catlabel" href="<?php echo $this->url(array('action' => 'browse'), $route, true).'?category_id='.urlencode($item->category_id) . '&subcat_id='.urlencode($subCat->category_id) ; ?>">
								...
								<span>
									...
									<?php echo ' ('.$subCat->total_listings_categories.')';?>
								</span>
							</a>                  
									...
									<li>
										<a class="catlabel" href="<?php echo $this->url(array('action' => 'browse'), $route, true).'?category_id='.urlencode($item->category_id) . '&subcat_id='.urlencode($subCat->category_id) .'&subsubcat_id='.urlencode($subSubCat->category_id) ; ?>">
											...
											<span>
												...
												<?php echo ' ('.$subSubCat->total_listings_categories.')';?>
											</span>
										</a>
									</li>
								<?php endforeach; ?>                
...
function showCategory(id) {  
	if($('subcategory_' + id)) {  
      ...
      $('seslisting_toggle_' + id).removeClass('cattoggel cattoggeldown');
      $('seslisting_toggle_' + id).addClass('cattoggel cattoggelright'); 
			$('subcategory_' + id).style.display = 'none';   
		} else {  
			$('seslisting_toggle_' + id).removeClass('cattoggel cattoggelright');
      $('seslisting_toggle_' + id).addClass('cattoggel cattoggeldown');
      $('subcategory_' + id).style.display = 'block';
		} 
	}    
	if($('subsubcategory_' + id)) {   
		if ($('subsubcategory_' + id).style.display == 'block') { 
			$('seslisting_subcat_toggle_' + id).removeClass('cattoggel cattoggeldown');
      $('seslisting_subcat_toggle_' + id).addClass('cattoggel cattoggelright');           
			$('subsubcategory_' + id).style.display = 'none';   
		} else {  
			$('seslisting_subcat_toggle_' + id).removeClass('cattoggel cattoggelright'); 
			$('seslisting_subcat_toggle_' + id).addClass('cattoggel cattoggeldown');  
