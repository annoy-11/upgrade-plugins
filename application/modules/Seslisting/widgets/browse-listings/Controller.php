 ...
 */
class Seslisting_Widget_BrowseListingsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $form = new Seslisting_Form_Search(array('searchTitle' => 'yes','browseBy' => 'yes','categoriesSearch' => 'yes','searchFor'=>'listing','FriendsSearch'=>'yes','defaultSearchtype'=>'mostSPliked','locationSearch' => 'yes','kilometerMiles' => 'yes','hasPhoto' => 'yes'));
    ...
    $value['ordering'] = isset($searchArray['ordering']) ? $searchArray['ordering'] : (isset($_GET['ordering']) ? $_GET['ordering'] : (isset($params['ordering']) ? $params['ordering'] : $this->_getParam('ordering', '1')));
    ...
    // Listing browse page work
    $type = '';
    $page_id = Engine_Api::_()->seslisting()->getWidgetPageId($identityForWidget);
    if($page_id) {
      $pageName = Engine_Db_Table::getDefaultAdapter()->select()
              ...
              ->fetchColumn();
      if($pageName) {
        $explode = explode('seslisting_index_', $pageName);
        if(is_numeric($explode[1])) {
          $type = Engine_Db_Table::getDefaultAdapter()->select()
                ->from('engine4_seslisting_integrateothermodules', 'content_type')
    ...
    $value['status'] = 1;
    $value['search'] = 1;
    $this->view->widgetName = 'browse-listings';
    ...
    // Get listings
    $paginator = Engine_Api::_()->getDbtable('seslistings', 'seslisting')->getSeslistingsPaginator($params, $form->getValues());
