 ...
 */
class Seslisting_Widget_ManageListingsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (!$is_ajax && is_array($defaultOptionsArray)) {
      foreach ($defaultOptionsArray as $key => $defaultValue) {
				if(!Engine_Api::_()->getApi('settings', 'core')->getSetting('seslisting.enable.favourite', 1) && $defaultValue == 'mostSPfavourite')
					continue;
    ...
    $this->view->widgetName = 'manage-listings';
    $params = array_merge($params, $value, array('search'=>addslashes($text)));

    // Get Listings
    $paginator = Engine_Api::_()->getDbtable('seslistings', 'seslisting')->getSeslistingsPaginator($value);
