...
class Seslisting_Widget_ListingReviewsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $seslisting_reviews = Zend_Registry::isRegistered('seslisting_reviews') ? Zend_Registry::get('seslisting_reviews') : null;
		...
		if (!$is_ajax) {
		  ...
		  return $this->setNoRender();
			...
			if (!Engine_Api::_()->authorization()->getPermission($viewer, 'seslistingreview', 'view'))
			return $this->setNoRender();

			$this->view->isReview = Engine_Api::_()->getDbtable('seslistingreviews', 'seslisting')->isReview(array('listing_id' => $subject->getIdentity()));

			$this->view->cancreate = Engine_Api::_()->authorization()->getPermission($viewer, 'seslistingreview', 'create');
    ...
    if (empty($seslisting_reviews))
      return $this->setNoRender();
		...
		$this->view->listing_id = $params['listing_id'] = isset($_POST['listing_id']) ? $_POST['listing_id'] : $subject->getIdentity();
		$params['paginator'] = true;

    $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('seslistingreviews', 'seslisting')->getListingReviewSelect($params);
