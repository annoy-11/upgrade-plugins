...
<div class="seslisting_review_view seslisting_profile_review sesbasic_bxs sesbasic_clearfix">
	<div class="seslisting_review_view_top sesbasic_clearfix">
    <?php if(in_array('title', $this->stats)): ?>
      <div class="seslisting_review_view_title"><?php echo $this->review->getTitle() ?></div>
    <?php endif; ?>
    <div class="seslisting_review_view_item_info sesbasic_clearfix">
      <?php if(in_array('postedin', $this->stats)): ?>
      	<div class="seslisting_review_view_info_img">
        	<?php echo $this->htmlLink($this->item->getHref(), $this->itemPhoto($this->item, 'thumb.icon')); ?>
        </div>
      <?php endif; ?> 
      <div class="seslisting_review_view_info_cont sesbasic_clearfix">
        <?php if(in_array('postedin', $this->stats) && in_array('creationDate', $this->stats)) : ?>
          <p class='seslisting_review_view_stats sesbasic_text_light sesbasic_clearfix'>
            <?php if(in_array('postedin', $this->stats)): ?>
        ...
        <?php endif; ?> 
        <p class="seslisting_review_view_stats sesbasic_text_light">
    ...
    <div class="seslisting_review_show_rating seslisting_review_listing_star">
      ...
      <?php $reviewParameters = Engine_Api::_()->getDbtable('parametervalues', 'seslisting')->getParameters(array('content_id'=>$this->review->getIdentity(),'user_id'=>$this->review->owner_id)); ?>
      <?php if(count($reviewParameters)>0){ ?>
        <div class="seslisting_review_show_rating_box sesbasic_clearfix">
          <?php foreach($reviewParameters as $reviewP){ ?>
            <div class="sesbasic_clearfix">
              <div class="seslisting_review_show_rating_label"><?php echo $reviewP['title']; ?></div>
              <div class="seslisting_review_show_rating_parameters sesbasic_rating_parameter sesbasic_rating_parameter_small">
  ...
  <div class="seslisting_review_contant_disc">
  <?php if(in_array('pros', $this->stats) && $this->review->pros): ?>
    <div class="seslisting_review_view_cont_row"><b class="label"><?php echo $this->translate("Pros"); ?></b><?php echo $this->review->pros; ?></div>
  <?php endif; ?>
  <?php if(in_array('cons', $this->stats) && $this->review->cons): ?>
    <div class="seslisting_review_view_cont_row"><b class="label"><?php echo $this->translate("Cons"); ?></b><?php echo $this->review->cons; ?></div>
  <?php endif; ?>
  <?php if(in_array('customfields', $this->stats)): ?>
  	<?php $customFieldsData = Engine_Api::_()->seslisting()->getCustomFieldMapData($this->review); 
    	if(count($customFieldsData) > 0){ 
         foreach($customFieldsData as $valueMeta){
         if(!$valueMeta['value'])	
         	continue;
          echo '<p class="seslisting_review_view_cont_row"><b class="label">'. $valueMeta['label']. ': </b>'.
                $valueMeta['value'].'</p>';
         }     
 			 } ?>
  <?php endif; ?>
  <?php if(in_array('description', $this->stats)): ?>
    <div class='seslisting_review_view_cont_row'>
  ...
  <?php if(in_array('recommended', $this->stats)): ?>
  	<p class="seslisting_review_view_cont_row seslisting_review_view_recommended">
...
<div class="seslisting_layout_contant" style="padding:0px;">
	<div class="seslisting_footer_two_listing ">
		<div class="seslisting_shear_listing sesbasic_bxs">
        ...
        <?php echo $this->partial('_socialShareIcons.tpl','sesbasic',array('resource' => $this->review, 'socialshare_enable_plusicon' => $this->socialshare_enable_plusicon, 'socialshare_icon_limit' => $this->socialshare_icon_limit)); ?>
			...
			<div class="seslisting_static_icons sesbasic_clearfix">
			...
			<?php if(in_array('likeButton', $this->stats) && $canComment):?>
				<?php $LikeStatus = Engine_Api::_()->seslisting()->getLikeStatus($this->review->review_id,$this->review->getType()); ?>
				<div class="seslisting_static_icons sesbasic_clearfix">
					<a href="javascript:;" data-url="<?php echo $this->review->review_id ; ?>" class="sesbasic_icon_btn sesbasic_icon_like_btn  seslisting_like_seslisting_review_<?php echo $this->review->review_id ?> seslisting_like_seslisting_review_view <?php echo ($LikeStatus) ? 'button_active' : '' ; ?>"><i class="<?php if($LikeStatus):?>fa fa-thumbs-down<?php else:?>fa fa-thumbs-up<?php endif;?>"></i><span><?php if($LikeStatus):?><?php echo $this->translate('Unlike');?><?php else:?><?php echo $this->translate('Like');?><?php endif;?></span></a>
				</div>
			<?php endif;?>
		</div>
	</div>
