...
class Seslisting_Widget_ProfileMusicalbumsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($_POST['is_ajax'])) {
      ...
      if (!$coreApi->hasSubject('seslisting'))
        return $this->setNoRender();

      $subject_listing = $coreApi->getSubject('seslisting');
      $this->view->listing_id = $subject_listing->listing_id;

      $subject = $coreApi->getSubject();

			$this->view->allow_create = true;
			if(isset($value['is_ajax']) && !$value['is_ajax'] && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('seslistingpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('seslistingpackage.enable.package', 1)){
			...
			$this->view->canCreate  = Engine_Api::_()->seslisting()->isListingAdmin($subject, 'music_create');
    }
    ...
    $this->view->listing_id = $listing_id = isset($params['listing_id']) ? $params['listing_id'] : $subject->listing_id;
    ...
    $params = $this->view->params = array('height' => $defaultHeight, 'width' => $defaultWidth, 'limit_data' => $limit_data, 'albumPhotoOption' => 'album', 'openTab' => $defaultOpenTab, 'pagging' => $loadOptionData, 'show_criterias' => $show_criterias, "informationAlbum" => $informationAlbum, 'listing_id' => $listing_id);
    ...
    $select = $table->select()
            ...
            ->where($tableName . '.resource_id = ?', $listing_id)
            ->where($tableName . '.resource_type = ?', 'seslisting')
