 ...
 */
class Seslisting_Widget_ProfileVideosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($_POST['is_ajax'])) {
      ...
      if (!$subject->authorization()->isAllowed($viewer, 'view'))
        return $this->setNoRender();
			...
			$subject = Engine_Api::_()->getItem('seslisting', $params['parent_id']);
		}
    ...
    $this->view->description_truncation_list = $description_truncation_list = isset($params['description_truncation_list']) ? $params['description_truncation_list'] : $this->_getParam('description_truncation_list', '100');
		$viewer = Engine_Api::_()->user()->getViewer();
		$this->view->canUpload = $canUpload = Engine_Api::_()->seslisting()->isListingAdmin($subject, 'create');
    ...
    $this->view->isListingAdmin = Engine_Api::_()->seslisting()->checkListingAdmin($subject);
