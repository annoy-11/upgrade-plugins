 ...
 */
class Seslisting_Widget_ProfilePhotosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->album_parent_id =  $id = isset($params['album_parent_id']) ? $params['album_parent_id'] : Zend_Controller_Front::getInstance()->getRequest()->getParam('listing_id', null);

    $value['listing_id'] = $this->view->listing_id =  $listing_id = $id = Engine_Api::_()->getDbtable('seslistings', 'seslisting')->getListingId($id);
    $this->view->listing  = $subject = Engine_Api::_()->getItem('seslisting',$id);
    $value['identityForWidget'] = $this->view->identityForWidget = isset($_POST['identity']) ? $_POST['identity'] : '';
		$this->view->allow_create = true;
		if(!$value['is_ajax'] && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('seslistingpackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('seslistingpackage.enable.package', 1)){
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('albums', 'seslisting')->getAlbumSelect($value);
