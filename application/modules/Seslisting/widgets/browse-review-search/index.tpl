...
<?php if(!$this->widgetIdentity) { ?>
	<div class="seslisting_browse_reviews_search sesbasic_bxs sesbasic_clearfix <?php echo $this->view_type == 'horizontal' ? 'seslisting_browse_review_search_horizontal' : 'seslisting_browse_review_search_vertical'; ?>">
<?php } ?>
...
<?php if($controllerName == 'review' && $actionName == 'browse'){ ?>
  <?php $identity = Engine_Api::_()->sesbasic()->getIdentityWidget('seslisting.browse-reviews','widget','seslisting_review_browse'); ?>
	<?php if($identity):?>
		<script type="application/javascript">
			sesJqueryObject(document).ready(function(){
				sesJqueryObject('#filter_form_review').submit(function(e){	
					if(sesJqueryObject('.seslisting_review_listing').length > 0){
						e.preventDefault();
						sesJqueryObject('#loadingimgseslistingreview-wrapper').show();
						...
						if(typeof paggingNumber<?php echo $identity; ?> == 'function'){
							...
							sesJqueryObject('#seslisting_review_listing').html('');
...
<script type="text/javascript">
	var Searchurl = "<?php echo $this->url(array('module' =>'seslisting','controller' => 'index', 'action' => 'get-review'),'default',true); ?>";
