...
<?php if(!$this->is_ajax):?>
  <div id="scrollHeightDivSes_<?php echo $randonNumber;?>" class="sesbasic_bxs"> 
		<div id="category-listing-widget_<?php echo $randonNumber; ?>" class="sesbasic_clearfix">
<?php endif;?>
  <?php foreach( $this->paginatorCategory as $item): ?>
  	<div class="seslisting_category_listing sesbasic_clearfix">
      <div class="seslisting_category_header sesbasic_clearfix">
        <p class="floatL"><a href="<?php echo $item->getBrowseListingHref(); ?>?category_id=<?php echo $item->category_id ?>" title="<?php echo $this->translate($item->category_name); ?>"><?php echo $this->translate($item->category_name); ?></a></p>
				<?php if(isset($this->seemore_text) && $this->seemore_text != ''): ?>
					<span <?php echo $this->allignment_seeall == 'right' ?  'class="floatR"' : ''; ?>><a  class="sesbasic_link_btn" href="<?php echo $item->getBrowseListingHref(); ?>?category_id=<?php echo $item->category_id ?>"><?php $seemoreTranslate = $this->translate($this->seemore_text); ?>
					<?php echo str_replace('[category_name]',$this->translate($item->category_name),$seemoreTranslate); ?></a></span>
				<?php endif;?>
      </div>
      <?php if(isset($this->resultArray['listing_data'][$item->category_id])):?>
        <?php $bigBlg = 0;?>
        <?php	foreach($this->resultArray['listing_data'][$item->category_id] as $item):?>
            <?php if(!$bigBlg):?>
            <div class="seslisting_category_item_single">
            <div class="seslisting_category_item_single_info">
              <div class="seslisting_entry_img">
                <a href="<?php echo $item->getHref();?>"><img src="<?php echo $item->getPhotoUrl('thumb.main'); ?>" /></a>
              </div>
              <div class="seslisting_category_item_single_content">
            ...
            <div class="seslisting-category_item_list sesbasic_custom_scroll">
            <?php else:?>
              <div class="seslisting_category_item sesbasic_clearfix">
  ...
  <?php endforeach;?>
	<?php if($this->paginatorCategory->getTotalItemCount() == 0 && !$this->is_ajax):?>
		<div class="tip">
			<span>
				<?php echo $this->translate('Nobody has created an listing yet.');?>
				<?php if ($this->can_create):?>
					<?php echo $this->translate('Be the first to %1$screate%2$s one!', '<a href="'.$this->url(array('action' => 'create','module'=>'seslisting'), "seslisting_general",true).'">', '</a>'); ?>
				<?php endif; ?>
			</span>
		</div>
	<?php endif; ?> 
	<?php if($this->loadOptionData == 'pagging'): ?>
		<?php echo $this->paginationControl($this->paginatorCategory, null, array("_pagging.tpl", "seslisting"),array('identityWidget'=>$randonNumber)); ?>
	<?php endif; ?>
<?php if(!$this->is_ajax){ ?>
		</div>
	</div>
	...
	function viewMore_<?php echo $randonNumber; ?> (){
		...
		en4.core.request.send(new Request.HTML({
			...
			onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
				document.getElementById('category-listing-widget_<?php echo $randonNumber; ?>').innerHTML = document.getElementById('category-listing-widget_<?php echo $randonNumber; ?>').innerHTML + responseHTML;
		...
		function paggingNumber<?php echo $randonNumber; ?>(pageNum){
			...
			en4.core.request.send(new Request.HTML({
				...
				onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
					...
					document.getElementById('category-listing-widget_<?php echo $randonNumber; ?>').innerHTML =  responseHTML;
          jqueryObjectOfSes(".sesbasic_custom_scroll").mCustomScrollbar({
          theme:"minimal-dark"
          });
					dynamicWidth();
