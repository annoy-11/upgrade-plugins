 ...
 * @version    $Id: Listing.php  2019-04-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Seslisting_Model_Listing extends Core_Model_Item_Abstract
{
  // Properties
  ...
  public function getHref($params = array()) {
    ...
    $params = array_merge(array(
      'route' => 'seslisting_entry_view',
      'reset' => true,
     // 'user_id' => $this->owner_id,
      'listing_id' => $this->custom_url,
      //'slug' => $slug,
    ), $params);
    $route = $params['route'];
    $reset = $params['reset'];
    ...
    if ($photo instanceof Zend_Form_Element_File) {
      ...
      throw new Seslisting_Model_Exception('invalid argument passed to setPhoto');
    }
    ...
    $this->save();
		if($direct == ''){
			...
			$listingAlbum = $this->getSingletonAlbum();
			$listingAlbum->title = Zend_Registry::get('Zend_Translate')->_('Untitled');
			$listingAlbum->owner_id = Engine_Api::_()->user()->getViewer()->getIdentity();
			$listingAlbum->save();
			$photoItem = $photoTable->createRow();
			$photoItem->setFromArray(array(
					'listing_id' => $this->getIdentity(),
					'album_id' => $listingAlbum->getIdentity(),
					'user_id' => $viewer->getIdentity(),
					'file_id' => $iMain->getIdentity(),
					'collection_id' => $listingAlbum->getIdentity(),
					'user_id' => $viewer->getIdentity(),
			));
			$photoItem->save();
		}
    ...
    $table = Engine_Api::_()->getItemTable('seslisting_album');
    $select = $table->select()
	...
	public function cancel()
  {
	 ...
	 //update transaction_id to other listing of same package listing
	 if($this->transaction_id){
		$transaction = $this->getTransaction();
		$table = Engine_Api::_()->getDbTable('seslistings','seslisting');
		$tableName = $table->info('name');
		//select listing in package with our transaction id.
		$select = $table->select()->from($tableName)->where('transaction_id =?','')->where('orderspackage_id =?',$this->orderspackage_id);
		$listing = $table->fetchRow($select);
		if($listing){
			$listing->transaction_id = $this->transaction_id;
			$listing->save();
			//update order
			$order_id = $transaction->order_id;
			$order = Engine_Api::_()->getItem('payment_order',$order_id);
			if($order){
				$order->source_id = $listing->getIdentity();
				$order->save();
			}
			//update item count in order package
			$orderpackage = Engine_Api::_()->getItem('seslistingpackage_orderspackage',$this->orderspackage_id);
    ...
    if( !empty($transaction->gateway_id) && !empty($transaction->gateway_profile_id) && empty($transaction->gateway_transaction_id) ) {
      try {
        ...
        $gateway = Engine_Api::_()->getItem('seslistingpackage_gateway', $transaction->gateway_id);
        if( $gateway ) {
          $gatewayPlugin = $gateway->getPlugin();
          if( method_exists($gatewayPlugin, 'cancelListing') ) {
            $gatewayPlugin->cancelListing($transaction->gateway_profile_id);
          }
        }
      } catch( Exception $e ) {
        // Silence?
  ...
  public function getPackage(){
		return Engine_Api::_()->getItem('seslistingpackage_package',$this->package_id);
	}
	 public function getTransaction(){
		return Engine_Api::_()->getItem('seslistingpackage_transaction',$this->transaction_id);
	}
	 // Events
  ...
  public function setActive($flag = true, $deactivateOthers = null)
  {
	...
	public function changeApprovedStatus($approved = 0){
		...
		if($transaction && $orderPackageId){
				...
				$this->save();
				Engine_Api::_()->getDbtable('seslistings', 'seslisting')->update(array('is_approved' =>$approved),array('orderspackage_id'=>$orderPackageId));
		}
	}
     ...
     if( in_array($transaction->state, array('initial', 'trial', 'pending', 'active')) ) {
      ...
      if( $expiration ) {
        $expiration_date = date('Y-m-d H:i:s', $expiration);
				...
				//make it a future listing(never expired)
  ...
  public function getPhotoUrl($type = null) {
    ...
    if ($photo_id) {
      $file = Engine_Api::_()->getItemTable('storage_file')->getFile($this->photo_id, $type);
		...
		$defaultPhoto = Zend_Registry::get('StaticBaseUrl').$settings->getSetting('seslisting_default_photo', 'application/modules/Seslisting/externals/images/nophoto_listing_thumb_profile.png');
		return $defaultPhoto;
  }
