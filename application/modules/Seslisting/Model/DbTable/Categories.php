 ...
 * @version    $Id: Categories.php  2019-04-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Seslisting_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Seslisting_Model_Category';
  ...
  public function getCategoriesAssoc($params = array()) {
    ...
    if ($viewerId && isset($params['member_levels']) && $params['member_levels'] == 1) {
      ...
      $stmt->where('CONCAT(engine4_seslisting_categories.member_levels," ") LIKE "%' . $levelId . '%"');
    }
  ...
  public function getUserCategoriesAssoc($user) {
    ...
    $stmt = $this->getAdapter()
            ->select()
            ->from('engine4_seslisting_categories', array('category_id', 'category_name'))
            ->joinLeft('engine4_seslisting_listings', "engine4_seslisting_listings.category_id = engine4_seslisting_categories.category_id")
            ->group("engine4_seslisting_categories.category_id")
            ->where('engine4_seslisting_listings.owner_id = ?', $user)
            ->where('engine4_seslisting_listings.draft = ?', "0")
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countListings'])) {
      $seslistingTable = Engine_Api::_()->getDbTable('seslistings', 'seslisting')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($seslistingTable, "$seslistingTable.subcat_id=$tableName.category_id", array("total_listings_categories" => "COUNT(listing_id)"));
      $category_select = $category_select->group("$tableName.category_id");
      $category_select->order('total_listings_categories DESC');
    }
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countListings'])) {
      $seslistingTable = Engine_Api::_()->getDbTable('seslistings', 'seslisting')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($seslistingTable, "$seslistingTable.subsubcat_id=$tableName.category_id", array("total_listings_categories" => "COUNT(listing_id)"));
  ...
  public function getCategory($params = array(), $customParams = array(), $searchParams = array()) {
    ...
    $listingTable = Engine_Api::_()->getDbTable('seslistings', 'seslisting')->info('name');

    if ((isset($params['hasListing']) && $params['hasListing']) || isset($params['countListings']) || (isset($params['criteria']) && $params['criteria'] == 'most_listing')) {
      $take = true;
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($listingTable, "$listingTable.category_id=$tableName.category_id", array("total_listings_categories" => "COUNT(listing_id)"));
      if (empty($params['countListings'])) {
        $category_select = $category_select->having("COUNT($listingTable.category_id) > 0")
                ->group("$listingTable.category_id");
        if (isset($params['listingDesc'])) {
          if ($params['criteria'] && $params['criteria'] == 'most_listing') {
            $category_select->order('total_listings_categories DESC');
          }
        }
      } else {
    ...
    if (isset($params['listingRequired'])) {
      if (empty($take)) {
        $category_select = $category_select->setIntegrityCheck(false);
        $category_select = $category_select->joinLeft($listingTable, "$listingTable.category_id=$tableName.category_id", array("total_listings_categories" => "COUNT(listing_id)"));
      }
  ...
  public function getCloudCategory($params = array()) {
    ...
    $category_select = $this->select()
            ...
            ->joinLeft($listingTableName, "$listingTableName.category_id=$tableName.category_id", array("total_listings_categories" => "COUNT(listing_id)"))
            ->where($tableName . '.subcat_id = ?', 0)
            ->where($tableName . '.subsubcat_id = ?', 0)
            ->group("$listingTableName.category_id")
            ->order('order DESC');
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('seslisting_category');
  ...
  public function hasCategoryId($params = array()) {
    ...
    $category_select = $this->select()
            ->from($tableName, 'category_id')
            ->where('selisting_categoryid = ?', $params['selisting_categoryid']);
    return $category_select->query()->fetchColumn();
  }
