 ...
 * @version    $Id: SignupController.php  2018-11-26 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Quicksignup_SignupController extends Core_Controller_Action_Standard
{
    ...
    public function indexAction()
    {
        ...
        $signupForms[] = array('class'=>'Quicksignup_Plugin_Signup_Account','order'=>1);
        $allowedActions = array('Payment_Plugin_Signup_Subscription'=>999,'Otpsms_Plugin_Signup_Otpsms'=>998);

        foreach( Engine_Api::_()->getDbtable('signup', 'user')->fetchAll() as $row ) {
            if( $row->enable == 1 ) {
                if(isset($allowedActions[$row->class])) {
                    $class = $row->class;
                    $signupForms[] = array('class'=>$row->class,'order'=>$allowedActions[$row->class]);
                }
            }
        }

        foreach ($signupForms as $form){
            $formSequenceHelper->setPlugin(new $form['class'], $form['order']);
        }
