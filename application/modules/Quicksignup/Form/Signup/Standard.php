 ...
 * @version    $Id: Standards.php  2018-11-26 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Quicksignup_Form_Signup_Standard extends Engine_Form
{
    /* Custom */
    ...
    public function generate()
    {
        ...
        $order = Engine_Api::_()->getDbTable('enablefields','quicksignup')->getField('profile_types');
        ...
        foreach( $struct as $fskey => $map )
        {
            ...
            if($alias == "birthdate"){
                $element
                    ->getDecorator('HtmlTag2')->setOption('class', 'form-wrapper birthday-cnt');
            }
            if( $field->isHeading() ) {
...
//        }
    ...
    public function isValid($data)
    {
        ...
        foreach ($accountElements as $key => $element) {
            if (isset($data[$key])) {
                $valid = $element->isValid($data[$key], $data) && $valid;
            } else {
