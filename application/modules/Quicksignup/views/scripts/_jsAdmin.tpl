 ...
 * @version    $Id: _jsAdmin.tpl  2018-11-26 00:00:00 SocialEngineSolutions $
...
<script type="text/javascript">
  ...
  var urls = {
      enableDisable:{
          enable : '<?php echo $this->url(array('action' => 'enable')) ?>',
          disable : '<?php echo $this->url(array('action' => 'disable')) ?>'
      },
    option : {
  ...
  var registerEvents = function() {
    ...
    // Attach edit field
    $$('.field > .item_options > a:first-child').removeEvents().addEvent('click', enableField);

    // Attach delete field
    $$('.field > .item_options > a + a').removeEvents().addEvent('click', disableField);

    // Attach heading edit
    $$('.heading > .item_options > a:first-child').removeEvents().addEvent('click', enableField);

    // Attach heading edit
    $$('.heading > .item_options > a:last-child').removeEvents().addEvent('click', disableField);
  ...
  var enableField = function (spec) {
      ...
      var elem = element.getParent('.admin_field');
      var ids = readIdentifiers(elem.get('id'));
      var url = urls.enableDisable.enable;
      url += '/field_id/' + ids.child_id;
      window.location.href = url;
  }
  var disableField = function (spec) {
      if ($type(spec) == 'event' || $type(spec) == 'domevent') {
          element = $(spec.target);
      } else if ($type(spec) == 'element' || $type(spec) == 'domelement') {
          element = $(element);
      ...
      var elem = element.getParent('.admin_field');
      var ids = readIdentifiers(elem.get('id'));
      var url = urls.enableDisable.disable;
      url += '/field_id/' + ids.child_id;
      window.location.href = url;
  }
