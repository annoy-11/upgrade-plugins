 ...
 * @version    $Id: Favourites.php  2018-10-29 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesbusinesspoll_Model_DbTable_Favourites extends Engine_Db_Table {

  protected $_rowClass = "Sesbusinesspoll_Model_Favourite";
  ...
  public function getItemfav($resource_type, $itemId) {
    $tableFav = Engine_Api::_()->getDbTable('favourites', 'sesbusinesspoll');
    $tableMainFav = $tableFav->info('name');
    $select = $tableFav->select()->from($tableMainFav)->where('resource_type =?', $resource_type)->where('user_id =?',
      Engine_Api::_()->user()->getViewer()->getIdentity())->where('resource_id =?', $itemId);
