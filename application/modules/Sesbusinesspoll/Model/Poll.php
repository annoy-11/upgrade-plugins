 ...
 * @version    $Id: Poll.php  2018-10-29 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesbusinesspoll_Model_Poll extends Core_Model_Item_Abstract
{
  ...
  {
    $optionsTable = Engine_Api::_()->getDbTable('options', 'sesbusinesspoll');
  ...
  public function getRichContent()
  {
    ...
    $view = clone $view;
    $view->clearVars();
    $view->addScriptPath('application/modules/Sesbusinesspoll/views/scripts/');
    ...
    $view->showPieChart = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinesspoll.showpiechart', false);
    $view->canVote = $this->authorization()->isAllowed(null, 'vote');
    $view->canChangeVote = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinesspoll.canchangevote', false);
    $view->hideLinks = true;
  ...
  {
    return Engine_Api::_()->getDbtable('options', 'sesbusinesspoll')->fetchAll(array(
  ...
  {
    $optionTable = Engine_Api::_()->getDbtable('options', 'sesbusinesspoll');
    $data = $optionTable
        ->select()
  ...
  public function vote(User_Model_User $user, $option, $business, $owner,$poll)
  {
	  $viewer = Engine_Api::_()->user()->getViewer();
    $votesTable = Engine_Api::_()->getDbTable('votes', 'sesbusinesspoll');
    $optionsTable = Engine_Api::_()->getDbtable('options', 'sesbusinesspoll');
	...
	$notificationType = 'sesbusinesspoll_vote_poll';
  ...
  {
    //     delete poll votes
    Engine_Api::_()->getDbtable('votes', 'sesbusinesspoll')->delete(array(
      'poll_id = ?' => $this->getIdentity(),
    ));
    ...
    // delete poll options
    Engine_Api::_()->getDbtable('options', 'sesbusinesspoll')->delete(array(
      'poll_id = ?' => $this->getIdentity(),
    ));
