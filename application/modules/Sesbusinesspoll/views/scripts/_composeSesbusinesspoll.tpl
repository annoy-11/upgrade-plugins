 ...
 * @version    $Id: _composeSesbusinesspoll.tpl  2018-10-29 00:00:00 SocialEngineSolutions $
...
$canUpload = Engine_Api::_()->authorization()->getPermission($viewer, 'sesbusinesspoll_poll', 'create');

if($module){
$gifCount = count(Engine_Api::_()->getDbTable('images', 'sesfeedgif')->getImages(array('fetchAll' => 1, 'limit' => 10)));
}
if(!$this->subject() || ($this->subject() && $this->subject()->getType() != "businesses"))
  return;
$max_options = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinesspoll.maxoptions', 15);
?>
...
sesJqueryObject(document).ready(function(){
	var div = '<div class="gif_businesspoll_content ses_emoji_container sesbasic_bxs'+ '_gif_businesspoll_content notclose">'+
  ...
  'application/modules/Sesbusinesspoll/externals/scripts/composer_sesbusinesspoll.js'); ?>
  en4.core.runonce.add(function() {
    composeInstance.addPlugin(new Composer.Plugin.Sesbusinesspoll({
	...
	<?php } ?>
  function checkValidationBusinessPoll(){
    ...
    sesJqueryObject( ".pollOptionInput" ).each(function( index ) {
      ...
      var Gifsrc = sesJqueryObject(this).find('span').eq(2).find('.uploadPreviewGif').attr('src');
  ...
  function sesbusinesspollappend(){
    var optionsDiv =  '<div class= "pollOptionInput">' +
      '<input type= "text" name="optionsArray[]" class="option_text" >' +
      '<div class="sesbusinesspoll_uploadmedia"><span class="sesbusinesspoll_add_photo"><a href="javascript:void(0)" title="<?php echo $this->string()->escapeJavascript($this->translate("Attach a Photo")); ?>"><input type= "file" name="optionsImage[]" class="'+text+counter+'"  onchange = "previewFile(this,'+counter+');"></a></span>';
      ...
      optionsDiv +='</div><span class="sesbusinesspoll_upload_img"><a href="javascript:void(0)" style = "display:none;" id="remove-image'+counter+'" class="fa fa-close remove-image"></a><img src="#"  class="uploadPreviewImage" style="display:none;" id="'+text+counter+'" ></span>';
      optionsDiv +=  '<span  class="sesbusinesspoll_upload_img"><a href="javascript:void(0)" style = "display:none;" id="remove-gif'+counter+'" class="fa fa-close remove-gif"></a><img src="#" class="uploadPreviewGif" style="display:none;" id="'+gif+counter+'"></span>' ;
    if(counter > 1){
...
var pollOptionObject;
	sesJqueryObject(document).on('click','.gif_poll_select',function() {
	  ...
	  sesJqueryObject('.gif_businesspoll_content').removeClass('from_bottom');
	  ...
	  if(sesJqueryObject(this).hasClass('activity_gif_content_a') && typeof sesadvancedactivityDesign != 'undefined' && sesadvancedactivityDesign == 2){
		...
		sesJqueryObject('.gif_businesspoll_content').css('top',topPositionOfParentDiv+'px');
		sesJqueryObject('.gif_businesspoll_content').css('left',leftPositionOfParentDiv).css('z-index',100);
		sesJqueryObject('.gif_businesspoll_content').show();
		...
		if(availableSpace < 400 && !sesJqueryObject('#ses_media_lightbox_container').length){
		  sesJqueryObject('.gif_businesspoll_content').addClass('from_bottom');
		}

		if(sesJqueryObject(this).hasClass('active')){
		  sesJqueryObject(this).removeClass('active');
		  sesJqueryObject('.gif_businesspoll_content').hide();
    ...
    var url = en4.core.baseUrl+'sesbusinesspoll/index/gif/',
    requestComentGif = new Request.HTML({
      ...
      onSuccess : function(responseTree, responseElements, responseHTML, responseJavaScript) {
        sesJqueryObject('.gif_businesspoll_content').find('.ses_gif_container_inner').find('.ses_gif_holder').html(responseHTML);
        sesJqueryObject(that).addClass('complete');
        sesJqueryObject('.gif_businesspoll_content').css('display','block');
        //complitionRequestTrigger();
