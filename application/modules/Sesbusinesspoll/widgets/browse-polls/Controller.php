 ...
 */
class Sesbusinesspoll_Widget_BrowsePollsController extends Engine_Content_Widget_Abstract {
...
$this->view->params = $params = !$is_ajax ? $this->_getAllParams() : json_decode($this->_getParam('params'),true);
   ...
   $this->view->show_criteria = $show_criterias = isset($params['show_criteria']) ? $params['show_criteria'] : $this->_getParam('show_criteria', array('like','description','in', 'comment','by','favourite','title', 'favouriteButton', 'likeButton', 'socialSharing', 'view'));
  ...
  //  $this->view->canUpload = $canUpload = $subject->authorization()->isAllowed($viewer, 'poll');
	...
	$values = array_merge($searchArray,$value);
    // initialize type variable type
    $paginator = Engine_Api::_()->getDbTable('polls', 'sesbusinesspoll')->getPollsPaginator($searchArray);
