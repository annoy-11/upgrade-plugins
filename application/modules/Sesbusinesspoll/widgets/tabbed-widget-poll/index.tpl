...
<p>
   ...
   <?php } else { ?>
      ...
      <ul id="tab-widget-sesbusinesspoll-<?php echo $randonNumber; ?>">
         <?php 
           $defaultOptionArray = array();
           foreach($this->defaultOptions as $key=>$valueOptions){ 
           $defaultOptionArray[] = $key;
...
<?php  } ?>
<?php include APPLICATION_PATH . '/application/modules/Sesbusinesspoll/views/scripts/_showPollListGrid.tpl'; ?>
<?php if(!$this->is_ajax){ ?>
    </div>
  </div>
<?php } ?>
...
<?php if(!$this->is_ajax):?>
  <script type="application/javascript"> 
	...
	requestTab_<?php echo $randonNumber; ?> = new Request.HTML({
	  method: 'post',
	  'url': en4.core.baseUrl+"widget/index/mod/sesbusinesspoll/name/<?php echo $this->widgetName; ?>/openTab/"+valueTab,
	  'data': {
	    format: 'html',  
	  ...
	  onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
	    ...
	    if(document.getElementById('sesbusiness_pinboard_view_<?php echo $randonNumber;?>') && sesJqueryObject('.sesbasic_view_type_options_<?php echo $randonNumber; ?>').find('.active').attr('rel') == 'pinboard') {
