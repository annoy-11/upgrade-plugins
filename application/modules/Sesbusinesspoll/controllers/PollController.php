 ...
 * @version    $Id: PollController.php  2018-10-29 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesbusinesspoll_PollController extends Core_Controller_Action_Standard{
  ...
  public function closeAction(){
    ...
    $poll = Engine_Api::_()->getItem('sesbusinesspoll_poll', $this->_getParam('poll_id'));
    if( !Engine_Api::_()->core()->hasSubject('sesbusinesspoll_poll') ) {
      Engine_Api::_()->core()->setSubject($poll);
    }
    $this->view->poll = $poll;
    // Check auth
    ...
    if( !$this->_helper->requireAuth()->setAuthParams('sesbusinesspoll', $viewer, 'edit')->isValid() ) {
      return;
    }
  ...
  public function favouriteAction() {
    ...
    $type = 'sesbusinesspoll_poll';
    $dbTable = 'polls';
    $resorces_id = 'poll_id';
    $notificationType = 'sesbusinesspoll_favourite_poll';
    $item_id = $this->_getParam('id',null);
    if (intval($item_id) == 0) {
      echo json_encode(array('status' => 'false', 'error' => 'Invalid argument supplied.'));die;
    }
    $viewer = Engine_Api::_()->user()->getViewer();
    $Fav = Engine_Api::_()->getDbTable('favourites', 'sesbusinesspoll')->getItemfav($type, $item_id);
    $favItem = Engine_Api::_()->getDbtable($dbTable, 'sesbusinesspoll');
$item = Engine_Api::_()->getItem('sesbusinesspoll_poll', $item_id);
    ...
    if (count($Fav) > 0) {
      ...
      Engine_Api::_()->getDbtable('notifications', 'activity')->delete(array('type =?' => $notificationType, "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $business->getType(), "object_id = ?" => $business->getIdentity()));
      Engine_Api::_()->getDbtable('actions', 'activity')->delete(array('type =?' => $notificationType, "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $business->getType(), "object_id = ?" => $business->getIdentity()));
      ...
      //update
      $db = Engine_Api::_()->getDbTable('favourites', 'sesbusinesspoll')->getAdapter();
      $db->beginTransaction();
      try {
        $fav = Engine_Api::_()->getDbTable('favourites', 'sesbusinesspoll')->createRow();
      ...
      $item = Engine_Api::_()->getItem(@$type, @$item_id);
      if ($this->_getParam('type') != 'sesbusinesspoll_artist') {
        $subject = $item;
  ...
  public function likeAction() {
    ...
    $dbTable = 'polls';
    $resorces_id = 'poll_id';
    $notificationType = 'sesbusinesspoll_like_poll';
    ...
    $itemTable = Engine_Api::_()->getDbtable($dbTable, 'sesbusinesspoll');
    ...
    if (count($Like) > 0) {
      ...
      if ($owner->getType() == 'user' && $owner->getIdentity() != $viewer->getIdentity()) {
        ...
        Engine_Api::_()->getDbtable('notifications', 'activity')->delete(array('type =?' => $notificationType, "subject_id =?" => $viewer->getIdentity(), "object_type =? " => $business->getType(), "object_id = ?" => $business->getIdentity()));
        Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $viewer, $business, $notificationType);
        
           $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($viewer, $business, $notificationType);
          if ($action != null){
  ...
  public function deleteAction(){
    $viewer = Engine_Api::_()->user()->getViewer();
    $poll = Engine_Api::_()->getItem('sesbusinesspoll_poll', $this->getRequest()->getParam('poll_id'));
    if( !$this->_helper->requireAuth()->setAuthParams('sesbusinesspoll', $viewer, 'delete')->isValid()) return;
    // In smoothbox
    $this->_helper->layout->setLayout('default-simple');
    $this->view->form = $form = new Sesbusinesspoll_Form_Delete();
    if( !$poll ) {
    ...
    return $this->_forward('success' ,'utility', 'core', array(
      'parentRedirect' => Zend_Controller_Front::getInstance()->getRouter()->assemble(array('action' => 'home'), 'sesbusinesspoll_general', true),
...
$this->_helper->content
    ...
    if( !$this->_helper->requireAuth()->setAuthParams('sesbusinesspoll_poll', null, 'edit')->isValid()) {
       return;
    }
    // Setup
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->poll = $poll = Engine_Api::_()->core()->getSubject('sesbusinesspoll_poll');
    ...
    // Get form
    $this->view->form = $form = new Sesbusinesspoll_Form_Edit();
    ...
    foreach( $roles as $role ) {
      if( 1 === $auth->isAllowed('sesbusinesspoll_poll', $role, 'view') ) {
        $form->auth_view->setValue($role);
      }
      if( 1 === $auth->isAllowed('sesbusinesspoll_poll', $role, 'comment') ) {
    ...
    if(!empty($IdsDiffrence)){
      foreach($IdsDiffrence as $index=>$value){
        $diffItem = $optionItem = Engine_Api::_()->getItem('sesbusinesspoll_option', $value);
        if(!empty($diffItem)){
    ...
    if($this->getParam('is_image_delete',0)==1){
      foreach($ids as $k=>$value){
        $Item = Engine_Api::_()->getItem('sesbusinesspoll_option', $value);
        if($Item){
    ...
    try{
      ...
      foreach($options as $optionKey=>$optionValue) {
        $optionItem = Engine_Api::_()->getItem('sesbusinesspoll_option', $ids[$optionKey]);
  ...
  public function viewAction(){
    // Check auth
    if( !$this->_helper->requireSubject('sesbusinesspoll_poll')->isValid() ) {
      return;
    }
  ...
  public function voteAction(){
   ...
   // }
    ...
    $poll = Engine_Api::_()->core()->getSubject('sesbusinesspoll_poll');
	$business = Engine_Api::_()->getItem('businesses', $poll->business_id);
    $viewer = Engine_Api::_()->user()->getViewer();
    if( !$poll ) {
    ...
    $hashElement = $this->view->sesBusinessVoteHash($poll)->getElement();
    if( $poll->closed ) {
      $this->view->success = false;
    ...
    $db->beginTransaction();
    try {
      $poll->vote($viewer, $option_id,$business,$owner,$poll);
      $db->commit();
    } catch( Exception $e ) {
      $db->rollback();
      $this->view->success = false;
    ...
    $this->view->token = $this->view->sesBusinessVoteHash($poll)->generateHash();
  ...
  public function moreAction(){
    if($optionId = $this->_getParam('option_id'))
      $option = Engine_Api::_()->getItem('sesbusinesspoll_option', $this->_getParam('option_id'));
    if(!$option)
      return;
    $this->view->moreuser = $optionUserObj = Engine_Api::_()->getDbtable('votes', 'sesbusinesspoll')->getVotesPaginator
