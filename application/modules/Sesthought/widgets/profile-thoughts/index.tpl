...
<script type="text/javascript">
  ...
  en4.core.runonce.add(function() {
    ...
    <?php if( !$this->renderOne ): ?>
      ...
      $('sesthought_profile_previous').style.display = '<?php echo ( $this->paginator->getCurrentPageNumber() == 1 ? 'none' : '' ) ?>';
      $('sesthought_profile_next').style.display = '<?php echo ( $this->paginator->count() == $this->paginator->getCurrentPageNumber() ? 'none' : '' ) ?>';

      $('sesthought_profile_previous').removeEvents('click').addEvent('click', function() {
      ...
      $('sesthought_profile_next').removeEvents('click').addEvent('click', function(){
        en4.core.request.send(new Request.HTML({
          url : en4.core.baseUrl + 'widget/index/content_id/' + <?php echo sprintf('%d', $this->identity) ?>,
          data : {
            format : 'html',
        ...
        <li class="sesthoughts_list_item new_image_pinboard_<?php echo $randonNumber; ?>" >
            ...
            <div class='_content'>
              <div id="sesthought_description_content_<?php echo $item->getIdentity(); ?>" class='sesthought_feed_thought _des'>
                ...
                <?php } ?>
                <?php if(in_array('title', $this->allParams['stats']) && !empty($item->thoughttitle)) { ?>
              ...
              <?php if(in_array('likecount', $this->allParams['stats']) || in_array('commentcount', $this->allParams['stats']) || in_array('viewcount', $this->allParams['stats']) || in_array('category', $this->allParams['stats']) || in_array('permalink', $this->allParams['stats'])): ?>
                <div class="_stats sesbasic_text_light">
                  ...
                  <?php } ?>
                  <?php if(in_array('category', $this->allParams['stats']) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesthought.enablecategory', 1) && $item->category_id) { ?>
                    <span> 
            ...
            <div class="_footer sesbasic_clearfix sesthought_social_btns">
              <?php if(in_array('socialSharing', $this->allParams['stats']) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesthought.allowshare', 1)):?>
...
<?php else:?>
  <div class="sesbasic_tip">
    <img src="<?php echo Engine_Api::_()->getApi('settings', 'core')->getSetting('sesthought_thought_no_photo', 'application/modules/Sesthought/externals/images/thought-icon.png'); ?>" alt="" />
    <span>
