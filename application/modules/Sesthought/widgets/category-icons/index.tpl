...
<ul class="sesthought_cat_iconlist_container sesbasic_clearfix clear sesbasic_bxs <?php echo ($allParams['alignContent'] == 'left' ? 'gridleft' : ($allParams['alignContent'] == 'right' ? 'gridright' : '')) ?>">	

  <?php foreach( $this->paginator as $item ): ?>

    <li class="sesthought_cat_iconlist" style="height:<?php echo is_numeric($allParams['height']) ? $allParams['height'].'px' : $allParams['height'] ?>; width:<?php echo is_numeric($allParams['width']) ? $allParams['width'].'px' : $allParams['width'] ?>;">
      ...
      <a href="<?php echo $this->url(array('action' => 'index'), 'sesthought_general').'?category_id='.$item->category_id; ?>">
        ...
        <?php if(in_array('countThoughts', $allParams['showStats'])){ ?>
