...
class Seseventsponsorship_Installer extends Engine_Package_Installer_Module {
  ...
  public function onEnable() {
		$db = $this->getDb();
		$db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '1' WHERE `engine4_core_menuitems`.`name` = 'sesevent_admin_main_sponsorshippaymentrequest';");
		$db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '1' WHERE `engine4_core_menuitems`.`name` = 'sesevent_admin_main_managesponsorship';");
		$db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '1' WHERE `engine4_core_menuitems`.`name` = 'sesevent_admin_main_managesopnshorshipeventowner';");
    parent::onEnable();
  }
  
  public function onDisable() {
		$db = $this->getDb();
		$db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '0' WHERE `engine4_core_menuitems`.`name` = 'sesevent_admin_main_sponsorshippaymentrequest';");
		$db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '0' WHERE `engine4_core_menuitems`.`name` = 'sesevent_admin_main_managesponsorship';");
		$db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '0' WHERE `engine4_core_menuitems`.`name` = 'sesevent_admin_main_managesopnshorshipeventowner';");
    parent::onDisable();
  }
}