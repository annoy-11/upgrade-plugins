 ...
 * @version    $Id: edit-education.tpl  2019-02-06 00:00:00 SocialEngineSolutions $
...
<script>
  ...
  en4.core.runonce.add(function() {
    <?php for($i=1;$i<=3;$i++) { ?>
      var edu_i = '<?php echo $i; ?>';
      <?php $upload = 'upload_'.$i;
      if($this->education->$upload) { ?>
        $('upload_'+edu_i+'-wrapper').style.display = 'block';
        
      <?php } else { ?>
        $('upload_'+edu_i+'-wrapper').style.display = 'none';
      <?php } ?>
    <?php } ?>
  });
  ...
  function submitCompliment(formObject) {
    ...
    sesJqueryObject.ajax({
      url: "sesprofilefield/index/edit-education/",
      type: "POST",
      contentType:false,
      processData: false,
      cache: false,
...
<div class="sesprofilefield_add_details_form edit_education_form">
