 ...
 * @version    $Id: edit-course.tpl  2019-02-06 00:00:00 SocialEngineSolutions $
...
<script>
  ...
  function validateForm() {
    ...
    sesJqueryObject('#sesprofilefield_addcourse input, #sesprofilefield_addcourse select,#sesprofilefield_addcourse checkbox,#sesprofilefield_addcourse textarea,#sesprofilefield_addcourse radio').each(
      function(index){
...
//         }
  ...
  function submitCompliment(formObject) {
    ...
    sesJqueryObject.ajax({
      url: "sesprofilefield/index/edit-course/",
      type: "POST",
      contentType:false,
      processData: false,
      cache: false,
      ...
      success: function(response) {
        ...
        if(result.status == 1) {
          ...
          if(sesJqueryObject('#course_count').length) {
            sesJqueryObject('#course_count').html(result.count);
          }
          
          if(sesJqueryObject('#sesprofilefield_courses').length) {
            if(sesJqueryObject('#course_tip').length)
              sesJqueryObject('#course_tip').hide();
            sesJqueryObject('#sesprofilefield_courses').show();
            sesJqueryObject('#sesprofilefield_courses').html(result.message);
          }
        }
      }
    });
...
<div class="sesprofilefield_add_details_form edit_course_form">
