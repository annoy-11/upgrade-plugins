...
class Eiosstories_Form_Admin_Global extends Engine_Form {
  ...
  public function init() {
    ...
    if ($settings->getSetting('eiosstories.pluginactivated')) {
        ...
        $this->addElement('Text', "eiosstories_videouplimit", array(
            'label' => 'Video Story Upload Limit',
            'description' => "Enter the limit of upload story video (in MB).",
            'allowEmpty' => false,
            'required' => true,
            'value' => $settings->getSetting('eiosstories.videouplimit', '10'),
        ));

        $this->addElement('Text', "eiosstories_storyviewtime", array(
            'label' => 'Story Video Time',
            'description' => "Enter the story view time (in seconds).",
            'allowEmpty' => false,
            'required' => true,
            'value' => $settings->getSetting('eiosstories.storyviewtime', '5'),
        ));
