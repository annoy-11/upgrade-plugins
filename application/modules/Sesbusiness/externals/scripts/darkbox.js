...
;(function() {
  sesJqueryObject(function($) {
    ...
    function showImage() {
      ...
      sesJqueryObject("<img/>").on("load", function() {
    ...
    // Call darkbox
    sesJqueryObject(document).on("click", "[data-darkbox],[data-darkbox-business]", function(e) {

      var src = $(this).data("darkbox"),
          isDummy = !src, // (is just a link calling a business)
          businessID = $(this).data("darkbox-business");

      $imagesBusiness = !businessID ? $(this) : $('[data-darkbox-business="'+ businessID +'"]').filter("[data-darkbox]");
      n = $imagesBusiness.length;
      c = isDummy ? 0 : $imagesBusiness.index( this );
