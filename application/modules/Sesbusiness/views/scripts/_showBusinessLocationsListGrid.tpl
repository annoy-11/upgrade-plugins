 ...
 * @version    $Id: _showBusinessLocationsListGrid.tpl  2018-07-13 00:00:00 SocialEngineSolutions $
...
<?php $counter = 0;?>
<?php if(!$this->is_ajax){ ?>
  <a href="javascript:;" id="sesbusiness_browse_location" class="sessmoothbox" style="display:none;">MAP</a>
  <style>
...
<?php if(!isset($this->bothViewEnable) && !$this->is_ajax){ ?>
 ...
 <?php } ?>
<?php if(!$this->is_ajax){ ?>
<div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" class="sesbasic_clearfix sesbasic_bxs sesbusiness_locations_conteiner">
	<div class="_listing" id="sesbusinesslocation_content">
    <?php if($this->paginator->getTotalItemCount() > 0 && isset($this->params['show_item_count']) && $this->params['show_item_count']){ ?>
      <div class="sesbasic_clearfix sesbm _result" style="display:<?php !$this->is_ajax ? 'block' : 'none'; ?>" id="<?php echo !$this->is_ajax ? 'paginator_count_sesbusiness' : 'paginator_count_ajax_sesbusiness_entry' ?>"><span id="total_item_count_sesbusiness_entry"><?php echo $this->paginator->getTotalItemCount(); ?></span> <?php echo $this->paginator->getTotalItemCount() == 1 ?  $this->translate("business found.") : $this->translate("businesses found."); ?></div>
    <?php } ?>
  <ul class="sesbusiness_business_listing sesbasic_clearfix clear  <?php if($this->view_type == 'pinboard'):?>sesbasic_pinboard_<?php echo $randonNumber;?><?php endif;?>" id="tabbed-widget_<?php echo $randonNumber; ?>">
<?php } ?>
...
<?php if( $this->paginator->getTotalItemCount() > 0 ) { ?>
  <div class="_map" id="sesbusinesslocation_map">
    ...
    <?php if(!$this->view_more || $this->is_search): ?>
      <div class="_map_inner" id="sesbusiness_map_view_<?php echo $randonNumber;?>" style="width:100%">
  ...
  <script type="application/javascript">
    ...
    function newMarkerLayout_<?php echo $randonNumber?>(dataLenth) {
      ...
      for(i=0;i<markerArrayData_<?php echo $randonNumber?>.length;i++){
        ...
        var marker_html = '<div data-url="sesbusiness/index/mapmarkercontent/business_id/'+resource_id+'" data-id="'+resource_id+'" class="'+animateClass+'pin public sesbusiness_browse_location marker_'+countMarker_<?php echo $randonNumber;?>+'" data-lat="'+ markerArrayData_<?php echo $randonNumber?>[i]['lat']+'" data-lng="'+ markerArrayData_<?php echo $randonNumber?>[i]['lng']+'">' +
        '<div class="wrapper">' +
        '<div class="small">' +
        ...
        oms_<?php echo $randonNumber;?>.addListener('click', function(marker) {
          ...
          sesJqueryObject('#sesbusiness_browse_location').attr('data-url', sesJqueryObject('.marker_'+ id).attr('data-url'));
          sesJqueryObject('#sesbusiness_browse_location').trigger('click');
          sesJqueryObject('.sesbusiness_manage_listing_item').removeClass('_browse_location_heightlight');
          sesJqueryObject('#sesbusiness_manage_listing_item_'+sesJqueryObject('.marker_'+ id).attr('data-id')).addClass('_browse_location_heightlight');
        });

        $$('.sesbusiness_browse_location_' + resource_id).each(function(markers_<?php echo $randonNumber;?>) {
          markers_<?php echo $randonNumber;?>.addEvent('mouseover',function(marker) {

            //sesJqueryObject('#sesbusiness_browse_location').attr('data-url', 'sesbusiness/index/mapmarkercontent/business_id/'+resource_id);
            //sesJqueryObject('#sesbusiness_browse_location').trigger('click');
          });
        });
