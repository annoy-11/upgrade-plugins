...
<?php if($this->createLimit == 1):?>
  ...
  <?php }else{ ?>
    <script type="application/javascript">
      Sessmoothbox.css.push("<?php echo $this->layout()->staticBaseUrl . 'application/modules/Sesbusiness/externals/styles/styles.css'; ?>");
  ...
  <?php } ?>
  <?php if(count($_POST) == 0 && Engine_Api::_()->getApi('settings','core')->getSetting('sesbusiness.category.selection', 0)):?>
    <?php if (Engine_Api::_()->core()->hasSubject('businesses')):?>
      <?php $business = Engine_Api::_()->core()->getSubject();?>
    <?php endif;?>
    <div class="sesbusiness_create_step_container sesbasic_bxs sesbasic_clearfix">
      <h3><?php echo $this->translate('Create New Business');?></h3>
      <p><?php echo $this->translate("It's easy to set up. Just choose a Business category to get started.");?></p>
      <?php $iconType = Engine_Api::_()->getApi('settings','core')->getSetting('sesbusiness.category.icon');?>
      <?php if($iconType == 0):?>
      ...
      <?php elseif($iconType == 2):?>
        <?php $icon = 'thumbnail';?>
      <?php endif;?>
      <div class="sesbusiness_create_categories_listing">
        <?php foreach($this->categories as $category):?>
          <div class="sesbusiness_create_category">
            <section class="">
              <div class="_inner">
                <div class="_step1">
                  <?php if($this->quickCreate):?>
                    <a href="javascript:;" class="sesbasic_linkinherit sesbusiness_create_category_toggle">
                  <?php else:?>
                  ...
                  <?php else:?>
                    <i style="background-image:url(application/modules/Sesbusiness/externals/images/business-icon-big.png);"></i>
                  <?php endif;?>
                ...
                <?php if($this->quickCreate):?>
                  <div class="_step2">
                    <form class='quick_business_create' action="sesbusiness/index/create/parent_id/<?php echo $this->parent_id;?>" id="quick_business_create_<?php echo $category->category_id;?>" method="post" enctype="multipart/form-data">
                    ...
                    <div><input type="text" class="sesbusiness_title" id="sesbusiness_title_<?php echo $category->category_id;?>" name='sesbusiness_title' class="reset_form_value" placeholder="Business Title" /></div>
                    <?php $custom_url_value = isset($business->custom_url) ? $business->custom_url : (isset($_POST["custom_url"]) ? $_POST["custom_url"] : "");;?>
                    <div class="_url">
                      <input type="text" class="sesbusiness_custom_url reset_form_value" name="custom_url" id="custom_url_<?php echo $category->category_id;?>" value="<?php echo $custom_url_value;?>">
                      <span class="sesbusiness_check_availability_btn">
                    ...
                    <?php $subcategory = Engine_Api::_()->getDbTable('categories','sesbusiness')->getModuleSubcategory(array('category_id' => $category->category_id, 'column_name' => '*'));?>
                    ...
                    <?php endif;?>
                    <div style='display:none;' class='quick_business_subsubcat'><select></select></div>
                    <div style='display:none;' class="_loading" id='sesbusiness_sussubcat_loading'><i class="fa fa-spinner fa-spin"></i></div>
    ...
    <script type="text/javascript">
      //sesJqueryObject(document).ready(function(){
        sesJqueryObject(document).on('click',".sesbusiness_create_category_toggle",function(){
          checkTitleChnage = false; 
          changeTitle = true;
          sesJqueryObject('.reset_form_value').val('');
          sesJqueryObject('.reset_form_value').trigger('change');
          sesJqueryObject('.sesbusiness_create_category').removeClass('openbox');
          sesJqueryObject(this).closest('.sesbusiness_create_category').toggleClass("openbox");
        });
      //});
    </script>    
  <?php endif; ?>
  <div class="sesbusiness_create_container <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusiness.create.form', 1)):?>sesbusiness_create_form<?php endif;?>">
    <?php echo $this->form->render();?>
    <div class="sesbusiness_create_loading sesbusiness_create_overlay">
      <div class="sesbusiness_create_overlay_cont">
        <i class="fa fa-spinner fa-pulse fa-3x fa-fw margin-bottom"></i>
        <span class="_text"><?php echo $this->translate('Creating Business ...');?></span>
      </div>
    </div>
    <?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusiness.guidelines', 1) && !empty(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusiness.message.guidelines', ''))):?>
      <div id="sesbusiness_create_tips" class="sesbusiness_create_tips">
        ...
        <div class="create_tips_bottom_sec">
          <div class="sesbasic_html_block">
            <?php echo Engine_Api::_()->getApi('settings','core')->getSetting('sesbusiness.message.guidelines', '');?>
          </div>
        </div>
      </div>
    <?php endif;?>
    ...
    <script type="application/javascript">
      ...
      en4.core.runonce.add(function(){
         if(sesJqueryObject('.sesbusiness_create_step_container').length > 0){
           sesJqueryObject('.sesbusiness_create_container').hide();
           sesJqueryObject('.sesbusiness_create_step_container').show();
         }else{
           sesJqueryObject('.sesbusiness_create_tips').show();
           sesJqueryObject('.sesbusiness_create_container').show();
           sesJqueryObject('.sesbusiness_create_step_container').hide();
         }
      });
      ...
      sesJqueryObject(function() {
        ...
        var $el = sesJqueryObject('#sesbusiness_create_tips');
        var positionInitial = sesJqueryObject('#title').offsetTop;
        sesJqueryObject('<style>#sesbusiness_create_tips{top:'+positionInitial+'px;}</style>').appendTo(document.head);
      ...
      en4.core.runonce.add(function() {
        ...
        sesJqueryObject("#title, .sesbusiness_title").keyup(function(){
          ...
          if(sesJqueryObject(this).closest('.quick_business_create').length){
              sesJqueryObject(sesJqueryObject(this).closest('.quick_business_create').find('._url').find('.sesbusiness_custom_url')).val(Text);   
      }
      else {
          sesJqueryObject("#custom_url").val(Text);   
      }
        });
        sesJqueryObject("#custom_url, .sesbusiness_custom_url").keydown(function(){
         checkTitleChnage = true; 
         changeTitle = false;
        });
        var checkTitleChnage = false;
        sesJqueryObject("#title,.sesbusiness_title").blur(function(){
        ...
        sesJqueryObject("#custom_url, .sesbusiness_custom_url").blur(function(){
          validUrl = false;
          if(!checkTitleChnage)
           changeTitle = false;
      ...
      sesJqueryObject('#check_custom_url_availability, .check_custom_url_availability').click(function(){
        ...
        if(!custom_url_value)
            return;
        sesJqueryObject(this).find('#sesbusiness_custom_url_wrong').hide();
        sesJqueryObject(this).find('#sesbusiness_custom_url_correct').hide();
        sesJqueryObject(this).find('#sesbusiness_custom_url_loading').css('display','inline-block');
        var obj = this;
        sesJqueryObject.post('<?php echo $this->url(array('controller' => 'ajax','module'=>'sesbusiness', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value},function(response){
                    sesJqueryObject(obj).find('#sesbusiness_custom_url_loading').hide();
                    response = sesJqueryObject.parseJSON(response);
                    if(response.error){
                        validUrl = false;
                        sesJqueryObject(obj).find('#sesbusiness_custom_url_correct').hide();
                        sesJqueryObject(obj).find('#sesbusiness_custom_url_wrong').css('display','inline-block');
                            ...
                            sesJqueryObject(obj).find('#sesbusiness_custom_url_wrong').hide();
                            sesJqueryObject(obj).find('#sesbusiness_custom_url_correct').css('display','inline-block');
    ...
    <script type="text/javascript">
      ...
      sesJqueryObject(document).on('submit','.quick_business_create',function(e) {
        var cat_id = sesJqueryObject(this).attr('id').replace('quick_business_create_','');
        if(validation == true) {
          return true;
        }
        e.preventDefault();
        if(sesJqueryObject('#sesbusiness_title_'+cat_id).val() == "") {
        ...
        sesJqueryObject.post('<?php echo $this->url(array('controller' => 'ajax','module'=>'sesbusiness', 'action' => 'custom-url-check'), 'default', true) ?>',{value:custom_url_value},function(response){
          ...
          if(response.error) {
           ...
           validation = true;
           sesJqueryObject('#quick_business_create_'+cat_id).trigger('submit');
           return true;
          }
        });
        return false;
        ...
        var defaultProfileId = '<?php echo '0_0_' . $this->defaultProfileId ?>' + '-wrapper';
         if ($type($(defaultProfileId)) && typeof $(defaultProfileId) != 'undefined') {
          ...
          if(sesJqueryObject('#sessmoothbox_main').length){
            sesJqueryObject('#sesbusiness_create_form').find('div').find('div').find('.form-elements').find('.parent_0').closest('.form-wrapper').hide() ;
          }
        }
      });
      ...
      function showQuickSubSubCategory(cat_id,object) {
       sesJqueryObject('#sesbusiness_sussubcat_loading').show();
        ...
        (new Request.HTML({
          ...
          onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
            if(responseHTML != ''){
              sesJqueryObject('.quick_business_subsubcat').show();
              sesJqueryObject(object).parent().parent().find('.quick_business_subsubcat').html("<select><option value=''>"+en4.core.language.translate("3rd-level Category")+"</option>"+responseHTML+"></select>");
            }
            else {
              sesJqueryObject(object).parent().parent().find('.quick_business_subsubcat').html("");
              sesJqueryObject('.quick_business_subsubcat').hide();
            }
            sesJqueryObject('#sesbusiness_sussubcat_loading').hide();
          }
        })).send();  
      }
       ...
       en4.core.runonce.add(function(){
                formObj = sesJqueryObject('#sesbusiness_create_form').find('div').find('div').find('div');
    ...
    obj.on('drop', function (e) 
    {
         ...
         handleFileBackgroundUpload(files,'business_main_photo_preview');
    });
       ...
       while (idx < maxBytes - 2) {
         ...
         switch (uint16) {
           ...
           case 0x0112: // Orientation tag
             ...
             // See business 102 at the following URL
   ...
   var sesbusinessidparam = "";
    function handleFileBackgroundUpload(input,id) {
      ...
      if (input.files && input.files[0] && (ext == "png" || ext == "jpeg" || ext == "jpg" || ext == 'PNG' || ext == 'JPEG' || ext == 'JPG')){
       /* var reader = new FileReader();
        ...
        sesbusinessidparam = id;
        if (files) {
           orientation(files, function(base64img, value) {
             //$(id+'-wrapper').attr('src', base64img);
             sesJqueryObject(sesbusinessidparam).closest('.form-wrapper').show();;
             var rotated = sesJqueryObject(sesbusinessidparam).attr('src', base64img);
             if (value) {
               sesJqueryObject(sesbusinessidparam).css('transform', rotation[value]);
             }
           });
         }
        ...
        $('business_main_photo_preview').style.display = 'block';
        $('business_main_photo_preview-wrapper').style.display = 'block';
    ...
    function removeImage() {
        ...
        $('business_main_photo_preview').style.display = 'none';
        $('business_main_photo_preview-wrapper').style.display = 'none';
        $('business_main_photo_preview').src = '';
        $('MAX_FILE_SIZE').value = '';
        $('removeimage2').value = '';
        $('photo').value = '';
    }
        ...
        sesJqueryObject('#sesbusiness_create_form').submit(function(e){
          ...
          if(validationFm) {
            ...
            if(typeof objectError != 'undefined'){
             ...
             <?php }else{ ?>
              sesJqueryObject('#sesbusiness_create_form').animate({scrollTop: errorFirstObject.offset().top}, 2000);
             <?php } ?>
            }
            return false;	
          }
            ...
            if(showErrorMessage != ''){
              sesJqueryObject('#business_error_time-wrapper').show();
              sesJqueryObject('#business_error_time-element').text(showErrorMessage);
              var errorFirstObject = sesJqueryObject('.sesbusiness_choose_date');
              <?php if(!$this->typesmoothbox){ ?>
              sesJqueryObject('html, body').animate({scrollTop: errorFirstObject.offset().top}, 2000);
             <?php }else{ ?>
             ...
             <?php } ?>
              ...
              sesJqueryObject('#business_error_time-wrapper').hide();
            }
            if(!validUrl){
             ...
             <?php } ?>
              ...
              sesJqueryObject('.sesbusiness_create_loading').show();
              sesJqueryObject('.sesbusiness_create_form').addClass('_success');
  ...
  <?php if($this->typesmoothbox) { ?>
   <script type="application/javascript">
     ...
     en4.core.runonce.add(function() {
       ...
       sesJqueryObject('#sesbusiness_create_form').attr('action',sesJqueryObject('#sesbusiness_create_form').attr('action').replace('?format=html&typesmoothbox=sessmoothbox',''));   
       tinymce.init({
 ...
 <?php die;} ?>
<?php else:?>
  <div class="sesbasic_tip clearfix sesbusiness_error">
    <img src="application/modules/Sesbusiness/externals/images/business-icon-error.png" alt="">
    <span><?php echo $this->translate("You have reached the limit of business creation. Please contact to the site administrator.");?></span>
  </div>
<?php endif;?>
