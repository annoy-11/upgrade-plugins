...
<?php
  ...
  $this->headLink()
    ...
    ->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesbusiness/externals/styles/styles.css');
...
<script type="text/javascript">
 if(window.location.href.indexOf("messages/compose") > -1 || window.location.href.indexOf("messages/view/id") > -1) {
  var isMessageBusiness = true;
 }else{
  var isMessageBusiness = false;
 }

<?php if(!Engine_Api::_()->getDbTable('modules', 'core')->isModuleEnabled('sesadvancedactivity')) { ?>
  en4.core.runonce.add(function() {
    if (Composer.Plugin.Sesbusinessphoto)
      return;
    //Hide Album plugin menu
    $('compose-photo-activator').style.display = 'none';
    Asset.javascript('<?php echo $this->layout()->staticBaseUrl ?>application/modules/Sesbusiness/externals/scripts/composer_photo.js', {
      onLoad:  function() {
        var type = 'wall';
        if (composeInstance.options.type) type = composeInstance.options.type;
        composeInstance.addPlugin(new Composer.Plugin.Sesbusinessphoto({
          ...
          requestOptions : {
            'url'  : en4.core.baseUrl + 'sesbusiness/album/upload-photo/type/'+type+'/business_id/<?php echo $subject->getIdentity(); ?>',
          },
          fancyUploadOptions : {
            'url'  : en4.core.baseUrl + 'sesbusiness/album/compose-upload/format/json/type/'+type,
...
<?php }else{  ?>  
<?php $this->headScript()->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sesbusiness/externals/scripts/composer_photo.js'); ?>
...
obj.on('drop', function (e) 
{
   ...
   handleFileUploadSesbusiness(files,obj);
});
...
sesJqueryObject(document).on('click','div[id^="abortPhoto_"]',function(){
		...
		if(typeof jqXHR[id] != 'undefined'){
				...
				executeuploadSesbusiness();
		}else{
				delete filesArray[id];	
				sesJqueryObject(this).parent().remove();
		}
    if(isMessageBusiness && (sesJqueryObject('#toValues-wrapper').length > 0 || sesJqueryObject('#submit-wrapper').length > 0)){
...
function createStatusbar(obj,file)
{
     ...
     if(rowCount %2 ==0) row ="even";
		  var checkedId = sesJqueryObject("input[name=cover]:checked");
			...
			this.abort = sesJqueryObject('<div class="abort sesbusiness_upload_item_abort" id="abortPhoto_'+countUploadSes+'"><span><?php echo $this->translate("Cancel Uploading"); ?></span></div>').appendTo(this.objectInsert);
			...
			this.src = sesJqueryObject('<img src="'+en4.core.baseUrl+'application/modules/Sesbusiness/externals/images/blank-img.gif">').appendTo(this.imageContainer);
			this.infoContainer = sesJqueryObject('<div class=advact_compose_photo_item_info sesbasic_clearfix"></div>').appendTo(this.objectInsert);
			 this.size = sesJqueryObject('<span class="sesbusiness_upload_item_size sesbasic_text_light"></span>').appendTo(this.infoContainer);
			 this.filename = sesJqueryObject('<span class="sesbusiness_upload_item_name"></span>').appendTo(this.infoContainer);
			this.option = sesJqueryObject('<div class="sesbusiness_upload_item_options clear sesbasic_clearfix"><span class="sesbusiness_upload_item_radio"></span><a class="edit_image_upload" href="javascript:void(0);"><i class="fa fa-pencil"></i></a><a class="delete_image_upload" href="javascript:void(0);"><i class="fa fa-close"></i></a></div>').appendTo(this.objectInsert);
...
var fdSes = new Array();
function handleFileUploadSesbusiness(files,obj)
{
...
var execute = true;
function executeuploadSesbusiness(){
	if(Object.keys(filesArray).length == 0 && sesJqueryObject('#show_photo').html() != ''){
   if(isMessageBusiness && (sesJqueryObject('#toValues-wrapper').length > 0 || sesJqueryObject('#submit-wrapper').length > 0)){}else{
    ...
    {
     	sendFileToServerSesbusiness(filesArray[i],statusArray[i],filesArray[i],'upload',i);
			break;
    }			
	 }
	}
...
var jqXHR = new Array();
function sendFileToServerSesbusiness(formData,status,file,isURL,i)
{
		execute = false;
    ...
    var uploadURL =en4.core.baseUrl + 'sesbusiness/album/upload-photo/type/'+type+'/business_id/<?php echo $subject->getIdentity(); ?>';
...
function readImageUrlSesbusiness(input) {
	handleFileUploadSesbusiness(input.files,sesJqueryObject('#dragandrophandler'));
}
...
sesJqueryObject(document).on('click','.delete_image_upload',function(e){
    ...
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'sesbusiness', 'controller' => 'index', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'sesbusiness', 'controller' => 'index', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
</script>
