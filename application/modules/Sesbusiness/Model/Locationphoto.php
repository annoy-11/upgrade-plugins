 ...
 * @version    $Id: Locationphoto.php  2018-07-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesbusiness_Model_Locationphoto extends Core_Model_Item_Abstract {
  ...
  public function setPhoto($photo, $isShareContent = false) {
    if ($photo instanceof Zend_Form_Element_File) {
      ...
      throw new Sesbusiness_Model_Exception('Invalid argument passed to setPhoto: ' . print_r($photo, 1));
    }

    $name = basename($fileName);
    $path = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'temporary';
    $params = array(
        'parent_type' => 'sesbusiness_locationphoto',
        'parent_id' => $this->getIdentity()
    );

    $core_settings = Engine_Api::_()->getApi('settings', 'core');
    $main_height = $core_settings->getSetting('sesbusiness.mainheight', 1600);
    $main_width = $core_settings->getSetting('sesbusiness.mainwidth', 1600);
    $normal_height = $core_settings->getSetting('sesbusiness.normalheight', 500);
    $normal_width = $core_settings->getSetting('sesbusiness.normalwidth', 500);
