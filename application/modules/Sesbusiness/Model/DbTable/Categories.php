 ...
 * @version    $Id: Categories.php  2018-07-13 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sesbusiness_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Sesbusiness_Model_Category';
  ...
  public function getCategory($params = array())
  {
    ...
    $businessTableName = Engine_Api::_()->getDbTable('businesses', 'sesbusiness')->info('name');
    $category_select = $category_select->setIntegrityCheck(false);
    $category_select = $category_select->joinLeft($businessTableName, "$businessTableName.category_id=$tableName.category_id AND (`is_approved` = 1 AND `" . $businessTableName . "`.`draft` != 0)", array("total_business_categories" => "COUNT($businessTableName.business_id)"));
    $category_select = $category_select->group("$tableName.category_id");
    if (isset($params['criteria']) && $params['criteria'] == 'most_business')
      $category_select->order('total_business_categories DESC');
    if (isset($params['criteria']) && $params['criteria'] != 'most_business')
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('sesbusiness_category');
  ...
  public function getCategoriesAssoc($params = array()) {
    ...
    $stmt = $stmt->order('order DESC')
            ...
            ->fetchAll();
    $data = array();
    if (isset($params['module']) && $params['module'] == 'businesses') {
      $data[] = '';
    }
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countBusinesses'])) {
      $businessTable = Engine_Api::_()->getDbTable('businesses', 'sesbusiness')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($businessTable, "$businessTable.subcat_id=$tableName.category_id", array("total_businesses_categories" => "COUNT($businessTable.photo_id)"));
      $category_select = $category_select->group("$tableName.category_id");
      $category_select->order('total_businesses_categories DESC');
    }
    $category_select = $category_select->order('order DESC');
    if (isset($params['limit']))
      $category_select->limit($params['limit']);
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countBusinesses'])) {
      $businessTable = Engine_Api::_()->getDbTable('businesses', 'sesbusiness')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($businessTable, "$businessTable.subsubcat_id=$tableName.category_id", array("total_businesses_categories" => "COUNT($businessTable.business_id)"));
