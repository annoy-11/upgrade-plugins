 ...
 */
class Sesbusiness_Model_DbTable_Albums extends Engine_Db_Table {

  protected $_rowClass = "Sesbusiness_Model_Album";
  ...
  public function editPhotos(){
    ...
    $select = $this->select()
										...
										->join($tablebusinessName, $tablebusinessName . '.business_id = ' . $albumTableName . '.business_id',null);
...
//       $value['order'] = 'modified_date';
    ...
    if(isset($value['business_id']) && intval($value['business_id']))
      $select->where($albumTableName.'.business_id = ?',$value['business_id']);
...
// 		    ->from($albumTableName)
// 		    ->where('search =?',1)
// 		    ->where($albumTableName.'.business_id =?',$value['business_id'])
// 		    ->business($albumTableName.'.album_id');
//     return Zend_Paginator::factory($select);
//   }
		public function getSpecialAlbum(User_Model_User $user, $type,$business_id)
  {
    if( !in_array($type, array('wall')) ) {
      //throw new Sesbusiness_Model_Exception('Unknown special album type');
    }
    $select = $this->select()
