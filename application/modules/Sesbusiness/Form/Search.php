 ...
 */
class Sesbusiness_Form_Search extends Engine_Form {
  ...
  public function init() {
    ...
    $hideClass = 'sesbusiness_widget_advsearch_hide_' . $identity;

    $params = Engine_Api::_()->sesbusiness()->getWidgetParams($identity);
    ...
    $this->setAction($view->url(array('module' => 'sesbusiness', 'controller' => 'index', 'action' => 'index'), 'default', true));
    $viewer = Engine_Api::_()->user()->getViewer();
    if (in_array('searchBusinessTitle', $show_criterias)) {
      $this->addElement('Text', 'search', array(
          'label' => 'Search Businesses',
          'class' => in_array('searchBusinessTitle', $show_advSearch) ? $hideClass : '',
      ));
    }
    ...
    if (in_array('view', $show_criterias)) {
      ...
      foreach ($params['criteria'] as $key => $viewOption) {
        ...
        switch ($columnValue) {
          case '0':
            $value = 'All Business Directories';
            break;
          case '1':
            $value = 'Only My Friend\'s Business Directories';
            break;
          case '2':
            $value = 'Only My Network Business Directories';
            break;
          case '3':
            $value = 'Only My Businesses Directories';
            break;
          case '4':
            $value = 'Open Businesses';
            break;
          case '5':
            $value = 'Closed Business Directories';
            break;
        }
        $filterOptions[$columnValue] = ucwords($value);
      }
    ...
    if (count($categories) > 0 && in_array('Categories', $show_criterias)) {
      ...
      if (in_array('customFields', $show_criterias)) {
        ...
        $customFields = new Sesbusiness_Form_Custom_Fields(array(
            'packageId' => '',
            'resourceType' => '',
    ...
    if (in_array('location', $show_criterias) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusiness_enable_location', 1)) {
      ...
      if (isset($show_criterias) && in_array('miles', $show_criterias)) {
    ...
    if (in_array('closebusiness', $show_criterias)) {
      $this->addElement('Checkbox', 'is_close_business', array(
          'label' => 'Include Closed Business Directories',
          'class' => in_array('closebusiness', $show_advSearch) ? $hideClass : '',
              //'decorators' => array(
//              'ViewHelper',
//              array('Description', array()),
//             // array('Label', array('placement' => 'APPEND', 'tag' => 'label')),
//              // array('HtmlTag', array('tag' => 'li', 'class' => 'is_close_business'))
//          ),
      ));
    }
    ...
    $this->addElement('Hidden', 'businesses', array(
        'order' => 100
    ));
