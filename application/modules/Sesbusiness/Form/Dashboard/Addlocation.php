 ...
 */
class Sesbusiness_Form_Dashboard_Addlocation extends Engine_Form {

  public function init() {

    $this->setTitle('Add New Location')
            ->setAttrib('id', 'sesbusiness_add_location')
            ->setAttrib('enctype', 'multipart/form-data')
            ->setMethod("POST")
            ->setAttrib('class', 'global_form sesbusiness_smoothbox_create');

    $request = Zend_Controller_Front::getInstance()->getRequest();
    $actionName = $request->getActionName();
    if ($actionName == 'edit-location') {
      $location = Engine_Api::_()->getItem('sesbusiness_location', $request->getParam('location_id'));
    }
    ...
    if (count($territories)) {
      ...
      if (isset($location)) {
        $business = $location;
        $itemlocation = Engine_Api::_()->getDbTable('locations', 'sesbusiness')->getLocationData(array('location_id' => $location->location_id));
        if ($itemlocation)
