...
<div class="sesbusiness_profile_tabs_top claim_request active sesbasic_clearfix">
...
<?php if($checkClaimRequest):?>
	<div class="sesbusiness_profile_tabs_top claim_new_business sesbasic_clearfix">
...
<script type="text/javascript">
	en4.core.runonce.add(function() {
		var contentAutocomplete = new Autocompleter.Request.JSON('title', "<?php echo $this->url(array('module' => 'sesbusiness', 'controller' => 'index', 'action' => 'get-businesses'), 'default', true) ?>", {
			'postVar': 'text',
			'minLength': 1,
			'selectMode': 'pick',
		...
		contentAutocomplete.addEvent('onSelection', function(element, selected, value, input) {
			$('business_id').value = selected.retrieve('autocompleteChoice').id;
		});
	});
	...
	function validateForm(){
		...
		counter = 0;
		sesJqueryObject('#sesbusiness_claim_create input, #sesbusiness_claim_create select,#sesbusiness_claim_create checkbox,#sesbusiness_claim_create textarea,#sesbusiness_claim_create radio').each(
			function(index){
	...
	en4.core.runonce.add(function() {
		sesJqueryObject('#sesbusiness_claim_create').submit(function(e){
	...
	function sendDataToServer(object){
		...
		sesJqueryObject.ajax({
			type:'POST',
			url: en4.core.baseUrl + "widget/index/mod/sesbusiness/name/claim-business/",
			data:formData,
			cache:false,
			contentType: false,
			processData: false,
			success:function(data){
				...
				window.location.href = '<?php echo $this->url(array("action"=>"claim-requests"),"sesbusiness_general",true); ?>';
			},
			error: function(data){
			}
		});
