...
<script>var Searchurl = "<?php echo $this->url(array('module' =>'sesbusiness','controller' => 'album', 'action' => 'get-album'),'default',true); ?>";</script>
<?php if($controllerName == 'album' && $actionName == 'browse') { ?>
  <?php $identity = Engine_Api::_()->sesbusiness()->getIdentityWidget('sesbusiness.browse-albums','widget','sesbusiness_album_browse'); ?>
  <?php if($identity){ ?>
    <script type="application/javascript">
      sesJqueryObject(document).ready(function(){
        sesJqueryObject('#filter_form').submit(function(e){
          if(sesJqueryObject('.sesbusiness_browse_album_listings').length > 0){
