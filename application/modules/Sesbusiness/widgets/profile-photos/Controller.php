...
class Sesbusiness_Widget_ProfilePhotosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
	...
	$getBusinessRolePermission = Engine_Api::_()->sesbusiness()->getBusinessRolePermission($subject->getIdentity(),'post_content','album',false);
    $levelId = !empty($viewer->getIdentity()) ? $viewer->level_id : 5;
    $this->view->canUpload = $canUpload = $getBusinessRolePermission ? $getBusinessRolePermission : $subject->authorization()->isAllowed($levelId, 'album');

    $this->view->business_album_count = $business_album_count = Engine_Api::_()->authorization()->getPermission($levelId, 'sesbusiness_business', 'business_album_count');

    //Manage Apps Check
    $isCheck = Engine_Api::_()->getDbTable('managebusinessapps', 'sesbusiness')->isCheck(array('business_id' => $business_id, 'columnname' => 'photos'));
    if(empty($isCheck))
