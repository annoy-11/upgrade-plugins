...
<?php if($this->tab_option == 'default'){ ?>
  <div class="layout_core_container_tabs">
    <div class="tabs_alt tabs_parent" id="sesbusiness_tabbed_widget_container_<?php echo $randonNumber; ?>">
<?php } ?>
<!--Advance Tabs-->
<?php if($this->tab_option == 'advance'){ ?>
  <div class="sesbasic_tabs_container sesbasic_clearfix sesbasic_bxs">
    <div class="sesbasic_tabs sesbasic_clearfix" id="sesbusiness_tabbed_widget_container_<?php echo $randonNumber; ?>">
 <?php } ?>
<!--Filter Tabs-->
<?php if($this->tab_option == 'filter'){ ?>
  <div class="sesbasic_filter_tabs_container sesbasic_clearfix sesbasic_bxs">
    <div class="sesbasic_filter_tabs sesbasic_clearfix" id="sesbusiness_tabbed_widget_container_<?php echo $randonNumber; ?>">
<?php } ?>

    <ul id="tab-widget-sesbusiness-<?php echo $randonNumber; ?>">
      <?php 
  ...
  <?php  if(count($this->defaultOptions) == 1) { ?>
    <script type="application/javascript">
      sesJqueryObject('#sesbusiness_tabbed_widget_container_<?php echo $randonNumber; ?>').css('display','none');
    </script>
  <?php } ?>
  <div id="scrollHeightDivSes_<?php echo $randonNumber; ?>" class="sesbasic_clearfix">
    <ul class="sesbusiness_album_listings sesbusiness_tabbed_listings sesbusiness_photos_flex_view sesbasic_bxs sesbasic_clearfix" id="album-tabbed-widget_<?php echo $randonNumber; ?>">
<?php } ?>
          ...
          <?php $limit = $this->limit;
            ...
            <li id="thumbs-photo-<?php echo $item->photo_id ?>" class="sesbusiness_album_list_grid_thumb sesbusiness_album_list_grid sespa-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sespa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">
            	<article>  
              <a class="sesbusiness_album_list_grid_img" href="<?php echo Engine_Api::_()->sesbusiness()->getHref($item->getIdentity(),$item->album_id); ?>" style="height:<?php echo is_numeric($this->height) ? $this->height.'px' : $this->height ?>;">
       ...
       ?>
        ...
        $canComment =  $item->authorization()->isAllowed(Engine_Api::_()->user()->getViewer(), 'comment');
        	if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->likeButton) && $canComment){ ?>
                ...
                <a href="javascript:;" data-contenttype="album" data-src='<?php echo $item->album_id; ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_like_btn sesbusiness_albumlike <?php echo ($albumLikeStatus) ? 'button_active' : '' ; ?>">
              ...
              <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $item->getType(); ?>" data-src='<?php echo $item->getIdentity(); ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_fav_btn sesbusiness_albumFav <?php echo ($albumFavStatus)>0 ? 'button_active' : '' ; ?>">
...
var searchParams<?php echo $randonNumber; ?>;
  var truncateRow = false;
	...
	function paggingNumber<?php echo $randonNumber; ?>(pageNum){
		 ...
		 var valueTab ;
		 sesJqueryObject('#tab-widget-sesbusiness-<?php echo $randonNumber; ?> > li').each(function(index){
		 ...
		 if(typeof valueTab == 'undefined')
			...
			(new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + 'widget/index/mod/sesbusiness/name/album-tabbed-widget/openTab/' + valueTab,
				'data': {
					format: 'html',
					page: pageNum,   
...
<?php if($this->loadOptionData != 'pagging'){ ?>
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    if (typeof(requestTab_<?php echo $randonNumber; ?>) != 'undefined') {
		...
		(new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/sesbusiness/name/album-tabbed-widget/openTab/' + openTab_<?php echo $randonNumber; ?>,
      'data': {
        format: 'html',
