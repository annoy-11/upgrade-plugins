...
<?php if(isset($this->identityForWidget) && !empty($this->identityForWidget)){
 ...
 <div class="sesbusinessa_album_search_result sesbasic_clearfix sesbm" id="<?php echo !$this->is_ajax ? 'paginator_count_sesbusiness' : 'paginator_count_ajax_sesbusiness' ?>">	
 		<span id="total_item_count_sesbusiness" style="display:inline-block;"><?php echo $this->paginator->getTotalItemCount(); ?></span> <?php echo $this->translate("albums found."); ?>
 </div>
 <?php if(!$this->is_ajax){ ?>
  <div id="scrollHeightDivSes_<?php echo $randonNumber; ?>">
    <ul class="sesbusiness_album_listings sesbusiness_browse_album_listings sesbasic_bxs sesbasic_clearfix" id="tabbed-widget_<?php echo $randonNumber; ?>">
 <?php }
    foreach( $this->paginator as $album ): ?>
      <?php $business = Engine_Api::_()->getItem('businesses', $album->business_id); ?>
      <?php if($this->view_type == 1){ ?>
        <li id="thumbs-photo-<?php echo $album->photo_id ?>" class="sesbusiness_album_list_grid_thumb sesbusiness_album_list_grid sespa-i-<?php echo (isset($this->insideOutside) && $this->insideOutside == 'outside') ? 'outside' : 'inside'; ?> sespa-i-<?php echo (isset($this->fixHover) && $this->fixHover == 'fix') ? 'fix' : 'over'; ?> sesbm" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">  
          <article>
          <a class="sesbusiness_album_list_grid_img" href="<?php echo Engine_Api::_()->sesbusiness()->getHref($album->getIdentity(),$album->album_id); ?>" style="height:<?php echo is_numeric($this->height) ? $this->height.'px' : $this->height ?>;">
            ...
            <div class="ses_image_container" style="display:none;">
              <?php $image = Engine_Api::_()->sesbusiness()->getAlbumPhoto($album->getIdentity(),$album->photo_id); 
            ...
            <span class="sesbusiness_album_list_grid_btns">
             ...
             ?>
              ...
              if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->favouriteButton)){
                $albumFavStatus = Engine_Api::_()->getDbTable('favourites', 'sesbusiness')->isFavourite(array('resource_type'=>'sesbusiness_album','resource_id'=>$album->getIdentity())); ?>
                <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $album->getType(); ?>" data-url='<?php echo $album->getIdentity(); ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_fav_btn sesbusiness_albumFav <?php echo ($albumFavStatus) ? 'button_active' : '' ; ?>">
         ...
         <?php } ?>
         <?php if(isset($this->like) || isset($this->comment) || isset($this->view) || isset($this->title) || isset($this->rating) || isset($this->photoCount) || isset($this->favouriteCount) || isset($this->downloadCount)  || isset($this->by) || isset($this->businessname)){ ?>
              <p class="sesbusiness_album_list_grid_info sesbasic_clearfix<?php if(!isset($this->photoCount)) { ?> nophotoscount<?php } ?>">
              ...
              <span class="sesbusiness_album_list_grid_stats">
                <?php if(isset($this->businessname)) { ?>
                ...
                <?php }?>
                <?php if(SESBUSINESSSHOWUSERDETAIL == 1 && isset($this->by)) { ?>
                  <span class="sesbusiness_album_list_grid_owner">
      ...
      <?php }else{ ?>
      		<?php $image = Engine_Api::_()->sesbusiness()->getAlbumPhoto($album->getIdentity(),$album->photo_id,3);  ?>
          ...
          <li class="sesbusiness_album_list_thumbnail_view" style="width:<?php echo is_numeric($this->width) ? $this->width.'px' : $this->width ?>;">
            <div class="sesbusiness_album_thumbnail_view_main_img" style="height:<?php echo is_numeric($heightDiv) ? $heightDiv.'px' : $heightDiv ?>;">
              <?php $albumPhoto = Engine_Api::_()->getItem('sesbusiness_photo', $album->photo_id); ?>
              <a class="ses-image-viewer" onclick="openLightBoxForSesPlugins('<?php echo $albumPhoto->getHref()	; ?>', '<?php echo $album->getPhotoUrl(); ?>')" href="<?php echo $albumPhoto->getHref(); ?>"><span class="sesbusiness_album_thumbnail_view_thumb" style="background-image:url(<?php echo $albumPhoto->getPhotoUrl('thumb.normalmain'); ?>);"></span></a>
            </div>
						<?php 
            		if(count($image) != 0) { ?>
                <div class="sesbusiness_album_thumbnail_view_thumbs thumbs<?php echo count($image); ?> clear sesbasic_clearfix">
             		<?php	foreach($image as $key=>$valuePhoto) { ?>
                <div>
                  <?php $imageURL = $valuePhoto->getPhotoUrl('thumb.normalmain'); ?>
                  <a onclick="openLightBoxForSesPlugins('<?php echo $valuePhoto->getHref(); ?>', '<?php echo $valuePhoto->getPhotoUrl()	; ?>')" href="<?php echo $valuePhoto->getHref(); ?>" class="ses-image-viewer"><span class="sesbusiness_album_thumbnail_view_thumb" style="background-image:url(<?php echo $imageURL;  ?>);"></span></a>
                </div>
            <?php } ?>
            		</div>
            <?php	} ?>
            <div class="sesbusiness_album_thumbnail_view_btm clear sesbasic_clearfix">
              <div class="sesbusiness_album_thumbnail_owner_photo">
              	<?php $user = Engine_Api::_()->getItem('user',$album->owner_id) ?>
                <a href="<?php echo $user->getHref();; ?>">
                  <?php echo $this->itemPhoto($user, 'thumb.profile'); ?>
                 </a>          
              </div>
              <div class="sesbusiness_album_thumbnail_album_info">
                <p class="sesbusiness_album_thumbnail_album_name">
                  <?php if(isset($this->title)) { ?>
                   <?php echo $this->htmlLink($album, $this->string()->truncate($album->getTitle(), $this->title_truncation),array('title'=>$album->getTitle())) ; ?>
                  <?php } ?>
                </p>
                <p class="sesbusiness_album_thumbnail_album_by sesbasic_clearfix sesbasic_text_light"> 
                  <?php if(isset($this->businessname)) { ?>
                    <span>
                      <?php echo $this->translate('in ');?>
                       <?php echo $this->htmlLink($business->getHref(), $business->getTitle(), array('class' => 'thumbs_author')) ?>
               ...
               <?php } ?>
                <p class="sesbusiness_album_thumbnail_album_stats sesbusiness_album_list_stats sesbasic_clearfix sesbasic_text_light">
                	<?php if(isset($this->like)) { ?>
            ...
            <?php  if(isset($this->socialSharing) || isset($this->likeButton) || isset($this->favouriteButton)){  ?>
            	<p class="sesbusiness_album_list_btns sesbusiness_album_thumbnail_album_btns">
             	<?php if(isset($this->socialSharing)){ 
              //album viewpage link for sharing
                $urlencode = urlencode(((!empty($_SERVER["HTTPS"]) &&  strtolower($_SERVER["HTTPS"]) == 'on') ? "https://" : "http://") . $_SERVER['HTTP_HOST'] . $album->getHref());
             ?>
              ...
              $canComment = $album->authorization()->isAllowed($this->viewer, 'comment');
                if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->likeButton) && $canComment){  ?>
              	...
              	<?php } 
                 if(Engine_Api::_()->user()->getViewer()->getIdentity() !=0 && isset($this->favouriteButton)){
                  $albumFavStatus = Engine_Api::_()->getDbTable('favourites', 'sesbusiness')->isFavourite(array('resource_type'=>'album','resource_id'=>$album->album_id)); ?>
                    <a href="javascript:;" data-contenttype="album" data-resource-type="<?php echo $album->getType(); ?>" data-url='<?php echo $album->album_id; ?>' class="sesbasic_icon_btn_count sesbasic_icon_btn sesbasic_icon_fav_btn sesbusiness_albumFav <?php echo ($albumFavStatus) ? 'button_active' : '' ; ?>">
                      <i class="fa fa-heart"></i>
                      <span><?php echo $album->favourite_count; ?></span>
                    </a>
                <?php } ?>
...
function paggingNumber<?php echo $randonNumber; ?>(pageNum){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + "widget/index/mod/sesbusiness/name/browse-albums",
      'data': {
        format: 'html',
        page: pageNum,    
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        ...
        document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML =  responseHTML;
				sesJqueryObject('#paginator_count_sesbusiness').find('#total_item_count_sesbusiness').html(sesJqueryObject('#paginator_count_ajax_sesbusiness').find('#total_item_count_sesbusiness').html());
				sesJqueryObject('#paginator_count_ajax_sesbusiness').remove();
      }
    })).send();
    return false;
}
  ...
  function viewMore_<?php echo $randonNumber; ?> (){
    ...
    (new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/sesbusiness/name/browse-albums/index/',
      'data': {
        format: 'html',
      ...
      onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
        ...
        document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML = document.getElementById('tabbed-widget_<?php echo $randonNumber; ?>').innerHTML + responseHTML;
				sesJqueryObject('#paginator_count_sesbusiness').find('#total_item_count_sesbusiness').html(sesJqueryObject('#paginator_count_ajax_sesbusiness').find('#total_item_count_sesbusiness'));
				sesJqueryObject('#paginator_count_ajax_sesbusiness').remove();
