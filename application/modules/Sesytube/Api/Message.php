 ...
 * @version    $Id: Message.php  2019-02-01 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesytube_Api_Message extends Core_Api_Abstract
{
  ...
  public function setUnreadMessage(Core_Model_Item_Abstract $user) {
    $userLastNotification = Engine_Api::_()->getApi('message', 'sesytube')->getUserLastMessage($user);
    ...
    //Engine_Api::_()->getDbtable('recipients', 'messages')->update(array('ytube_read' => 1), array('`user_id` = ?' => $user->getIdentity(), 'ytube_read = ?' => 0, 'inbox_read = ?' => 0));
  }
  ...
  public function setUnreadFriendRequest(Core_Model_Item_Abstract $user) {
   ...
   // Engine_Api::_()->getDbtable('notifications', 'activity')->update(array('view_notification' => 1), array('user_id = ?' => $user->getIdentity(), 'ytube_read = ?' => 0, 'mitigated = ?' => 0, 'view_notification = ?' => 0));
  }
