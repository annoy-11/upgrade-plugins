 ...
 * @version    $Id: AdminManageController.php  2019-02-01 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesytube_AdminManageController extends Core_Controller_Action_Admin {
  ...
  public function miniMenuIconsAction() {
    ...
    $this->view->form = $form = new Sesytube_Form_Admin_MiniMenuIcons();
  ...
  public function landingPageSettingsAction() {
    ...
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesytube_admin_main', array(), 'sesytube_admin_main_menuslanding');
    $this->view->modules = Engine_Api::_()->sesytube()->getModulesEnable();
    if (!empty($_POST)) {
      $values = $_POST;
...
//           continue;
        ...
        if($key == 'sesytube_feature_content'){
  ...
  public function headerSettingsAction() {
    ...
    $this->view->form = $form = new Sesytube_Form_Admin_HeaderSettings();
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      $values = $form->getValues();
      ...
      unset($values['minimenu_frrequest_icons']);
      foreach ($values as $key => $value) {
          if ($key == 'sesytube_header_loggedin_options' || $key == 'sesytube_header_nonloggedin_options') {
            if(Engine_Api::_()->getApi('settings', 'core')->hasSetting($key)){
              Engine_Api::_()->getApi('settings', 'core')->removeSetting($key);
            }
          }
  ...
  public function setPhoto($photo) {
    ...
    $photo_params = array(
        'parent_id' => 1,
        'parent_type' => "sesytube_minimenu_icons",
    );
  ...
  public function uploadPhotoAction() {
    ...
    $admin_file = APPLICATION_PATH . '/public/adminsesytube';
    ...
    $this->view->photo_url = 'http://' . $_SERVER['HTTP_HOST'] . '/' . Zend_Controller_Front::getInstance()->getBaseUrl() . '/public/adminsesytube/' . $info['name'];
  }
  ...
  public function uploadIconAction() {
    ...
    $this->view->form = new Sesytube_Form_Admin_Icon();

    if ($this->getRequest()->isPost()) {
      if (isset($_FILES['photo']) && is_uploaded_file($_FILES['photo']['tmp_name'])) {

        $photoFile = Engine_Api::_()->sesytube()->setPhoto($_FILES['photo'], $id);
      ...
      if (isset($_FILES['activeicon']) && is_uploaded_file($_FILES['activeicon']['tmp_name'])) {

        $photoFile = Engine_Api::_()->sesytube()->setPhoto($_FILES['activeicon'], $id);
        if (!empty($photoFile->file_id)) {
          $previousFile = Engine_Api::_()->getDbTable('menusicons','sesbasic')->getRow($menu->id);
          $previous_file_id = !empty($previousFile->activeicon) ? $previousFile->activeicon : 0;
          Engine_Api::_()->getDbTable('menusicons','sesbasic')->addActiveSave($menu->id,$photoFile->file_id, '', '', 'sesytube');
      ...
      if ($menuType == 'main')
        $redirectUrl = Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'sesytube', 'controller' => 'admin-manage', 'action' => 'index'), 'default', true);
      else
        $redirectUrl = Zend_Controller_Front::getInstance()->getRouter()->assemble(array('module' => 'sesytube', 'controller' => 'admin-manage', 'action' => 'footer-menu'), 'default', true);
  ...
  public function deletePhotoAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      try {
        ...
        $mainPhoto->delete();
        $slideImage = Engine_Api::_()->getItem('sesytube_slideimage', $id);
        $slideImage->delete();
        $db->commit();
      } catch (Exception $e) {
        $db->rollBack();
