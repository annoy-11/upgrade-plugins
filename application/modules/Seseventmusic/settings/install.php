 ...
 */
class Seseventmusic_Installer extends Engine_Package_Installer_Module {
  ...
  public function onInstall() {
    ...
    //SE mobile menu
    $db->query('INSERT IGNORE INTO `engine4_core_menuitems` (`name`, `module`, `label`, `plugin`, `params`, `menu`, `submenu`, `order`) VALUES ("mobi_main_seseventmusic", "seseventmusic", "Music", "", \'{"route":"seseventmusic_general","action":"home"}\', "mobi_browse", "", 100);');
    ...
    $page_id = $db->select()
            ->from('engine4_core_pages', 'page_id')
            ->where('name = ?', 'seseventmusic_artist_view')
            ->limit(1)
            ->query()
            ->fetchColumn();
    if (!$page_id) {
      ...
      $db->insert('engine4_core_pages', array(
          'name' => 'seseventmusic_artist_view',
