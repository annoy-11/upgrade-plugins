 ...
 */
class Seseventmusic_AdminManagesongsController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('seseventmusic_admin_main', array(), 'seseventmusic_admin_main_managesongs');

    $ratingTable = Engine_Api::_()->getDbtable('ratings', 'seseventmusic');
    $favouriteTable = Engine_Api::_()->getDbtable('favourites', 'seseventmusic');

    $this->view->formFilter = $formFilter = new Seseventmusic_Form_Admin_Manage_FilterSongs();
    if ($this->getRequest()->isPost()) {
      $values = $this->getRequest()->getPost();
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          
          $ratingTable->delete(array('resource_type =?' => 'seseventmusic_albumsong', 'resource_id =?' => $value));
          $favouriteTable->delete(array('resource_type =?' => 'seseventmusic_albumsong', 'resource_id =?' => $value));
          $musicAlbum = Engine_Api::_()->getItem('seseventmusic_albumsong', $value);
          $musicAlbum->delete();
        }
      }
    }
    ...
    $musicAlbumSongs = Engine_Api::_()->getDbTable('albumsongs', 'seseventmusic');
    $musicAlbumSongsName = $musicAlbumSongs->info('name');
    $musicAlbums = Engine_Api::_()->getDbTable('albums', 'seseventmusic');
  ...
  public function featuredAction() {
    ...
    $this->_redirect('admin/seseventmusic/managesongs');
  }
  ...
  public function deleteAction() {
    ...
    if ($this->getRequest()->isPost()) {
      ...
      $db->beginTransaction();
      try {
        $musicAlbumSong = Engine_Api::_()->getItem('seseventmusic_albumsong', $id);
        
        $ratingTable->delete(array('resource_type =?' => 'seseventmusic_albumsong', 'resource_id =?' => $id));
        $favouriteTable->delete(array('resource_type =?' => 'seseventmusic_albumsong', 'resource_id =?' => $id));
  ...
  public function viewAction() {
    $this->view->item = $albumsong = Engine_Api::_()->getItem('seseventmusic_albumsong', $this->_getParam('id', null));
