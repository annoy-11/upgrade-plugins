<?php

/**
 * SocialEngineSolutions
 *
 * @category   Application_Seseventmusic
 * @package    Seseventmusic
 * @copyright  Copyright 2015-2016 SocialEngineSolutions
 * @license    http://www.socialenginesolutions.com/license/
 * @version    $Id: Controller.php 2015-03-30 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Seseventmusic_Widget_ProfileOptionsController extends Engine_Content_Widget_Abstract {

  public function indexAction() {

    $viewPageType = $this->_getParam('viewPageType', 'album');
    $this->view->viewType = $this->_getParam('viewType', 'vertical');
    $coreMenuApi = Engine_Api::_()->getApi('menus', 'core');

    if ($viewPageType == 'album') {

      if (!Engine_Api::_()->core()->hasSubject('seseventmusic_album'))
        return $this->setNoRender();

      $this->view->navigation = $coreMenuApi->getNavigation('seseventmusic_profile');
    } elseif ($viewPageType == 'song') {

      if (!Engine_Api::_()->core()->hasSubject('seseventmusic_albumsong'))
        return $this->setNoRender();

      $this->view->navigation = $coreMenuApi->getNavigation('seseventmusic_song_profile');
    }
  }

}