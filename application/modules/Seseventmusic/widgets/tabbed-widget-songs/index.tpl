...
<?php if($this->defaultOptions): ?>
  ...
  <div id="scrollHeightDivSes_<?php echo $randonNumber; ?>">    
    <ul class="seseventmusic_browse_listing" id="tabbed-widget-songs_<?php echo $randonNumber; ?>">
    <?php } ?>
          ...
          <li id="thumbs-photo-<?php echo $item->photo_id ?>" class="seseventmusic_item_grid" style="width:<?php echo str_replace('px','',$this->width).'px'; ?>;">            
              <div class="seseventmusic_item_artwork" style="height:<?php echo str_replace('px','',$this->height).'px'; ?>;">
                ...
                <div class="hover_box">
                  ...
                  <?php $songTitle = str_replace("'", '', $songTitle); ?>
				          <?php if($item->track_id): ?>
				            ...
				            <?php $URL = "http://api.soundcloud.com/tracks/$item->track_id/stream?consumer_key=$consumer_key"; ?>
				              <a class="seseventmusic_play_button" href="javascript:void(0);" onclick="play_music('<?php echo $item->albumsong_id ?>', '<?php echo $URL; ?>', '<?php echo $songTitle; ?>');"><i class="fa fa-play-circle"></i></a>
				            <?php else: ?>
				              <a class="seseventmusic_play_button" href="javascript:void(0);"><i class="fa fa-play-circle"></i></a>
				            <?php endif; ?>
				          <?php else: ?>
				            <a class="seseventmusic_play_button" href="javascript:void(0);" onclick="play_music('<?php echo $item->albumsong_id ?>', '<?php echo $item->getFilePath(); ?>', '<?php echo $songTitle; ?>');"><i class="fa fa-play-circle"></i></a>
				          <?php endif; ?>
                  ...
                  <div class="hover_box_options">
                    ...
                    <?php $isFavourite = Engine_Api::_()->getDbTable('favourites', 'seseventmusic')->isFavourite(array('resource_type' => "seseventmusic_albumsong", 'resource_id' => $item->albumsong_id)); ?>              
                    ...
                    <a class="share-white" title="Share" href="javascript:void(0);" onclick="showPopUp('<?php echo $this->escape($this->url(array('module'=>'activity', 'controller'=>'index', 'action'=>'share', 'route'=>'default', 'type'=>'seseventmusic_albumsong', 'id' => $item->albumsong_id, 'format' => 'smoothbox'), 'default' , true)); ?>'); return false;" ><i class="fa fa-share"></i></a>
                    <?php endif; ?>
                    <?php endif; ?>
                  </div>
                </div>
...
<script type="text/javascript">
	function changeTabSes_<?php echo $randonNumber; ?>(valueTab) {
		...
		if(valueTab){
			 ...
			 requestTab_<?php echo $randonNumber; ?> = new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + 'widget/index/mod/seseventmusic/name/tabbed-widget-songs/openTab/' + valueTab,
				'data': {
					format: 'html',
					page:  1,    
  ...
  function viewMore_<?php echo $randonNumber; ?> () {
    ...
    en4.core.request.send(new Request.HTML({
      method: 'post',
      'url': en4.core.baseUrl + 'widget/index/mod/seseventmusic/name/tabbed-widget-songs/openTab/' + openTab_<?php echo $randonNumber; ?>,
      'data': {
        format: 'html',
