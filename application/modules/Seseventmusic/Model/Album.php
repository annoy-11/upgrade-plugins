 ...
 */
class Seseventmusic_Model_Album extends Core_Model_Item_Abstract {
...
//  }
  ...
  public function getRichContent($view = false, $params = array()) {
    ...
    if (!$view) {
      ...
      $desc = "<div class='seseventmusic_feed_desc'>" . (Engine_String::strlen($desc) > 255 ? Engine_String::substr($desc, 0, 255) . '...' : $desc) . "</div>";
      $view = Zend_Registry::get('Zend_View');
      $view->playlist = $this;
      $view->songs = $this->getSongs();
      $view->short_player = true;
      $view->hideStats = true;
      $videoEmbedded = $desc . $view->render('application/modules/Seseventmusic/views/scripts/_Player.tpl');
    }
  ...
  public function getSong($file_id) {
    ...
    return Engine_Api::_()->getDbtable('albumsongs', 'seseventmusic')->fetchRow(array(
  ...
  public function addSong($file_id, $params = array()) {
    ...
    if ($file_id instanceof Seseventmusic_Model_Albumsong)
    ...
    if ($file) {
      $albumSongs = Engine_Api::_()->getDbtable('albumsongs', 'seseventmusic')->createRow();
  ...
  public function setProfile() {
    $table = Engine_Api::_()->getDbtable('albums', 'seseventmusic')->update(array(
        'profile' => 0,
            ), array(
  ...
  public function setPhoto($photo) {
    ...
    if ($photo instanceof Zend_Form_Element_File)
      ...
      throw new Seseventmusic_Model_Exception('Invalid argument passed to setPhoto: ' . print_r($photo, 1));

    $name = basename($file);
    
    $path = APPLICATION_PATH . DIRECTORY_SEPARATOR . 'temporary';
    $params = array(
        'parent_type' => 'seseventmusic_album',
