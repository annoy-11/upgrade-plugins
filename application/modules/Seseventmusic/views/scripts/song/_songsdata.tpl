...
<?php else: ?>
<ul class="clear seseventmusic_songslist_container playlist">
  <?php foreach( $this->paginator as $song ):  ?>
    <?php $song = Engine_Api::_()->getItem('seseventmusic_albumsong', $song->resource_id); ?>
    <?php if( !empty($song) ): ?>
            ...
            <div class="seseventmusic_songslist_options seseventmusic_options_buttons">
              <?php if( $this->viewer()->getIdentity()): ?>
                <?php if($this->canAddPlaylistAlbumSong): ?>
                  <a title="<?php echo $this->translate('Add to Playlist');?>" href="javascript:void(0);" onclick="showPopUp('<?php echo $this->escape($this->url(array('action'=>'append','albumsong_id' => $song->albumsong_id, 'format' => 'smoothbox'), 'seseventmusic_albumsong_specific' , true)); ?>'); return false;" class="fa fa-plus"><?php echo $this->translate('Add to Playlist');?></a>
                <?php endif; ?>
