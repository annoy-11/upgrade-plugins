 ...
 */
class Seseventmusic_Api_Core extends Core_Api_Abstract {
  ...
  public function createSong($file, $params = array()) {
    ...
    if (is_array($file)) {
      if (!is_uploaded_file($file['tmp_name']))
        throw new Seseventmusic_Model_Exception('Invalid upload or file too large');
...
//Check file extension
    if (!preg_match('/\.(mp3|m4a|aac|mp4)$/iu', $filename))
      throw new Seseventmusic_Model_Exception('Invalid file type');

//Upload to storage system
    if(isset($name) && !empty($name)) {
    $params = array_merge(array('type' => 'song', 'name' => $name, 'parent_type' => 'seseventmusic_albumsongs', 'parent_id' => Engine_Api::_()->user()->getViewer()->getIdentity(), 'user_id' => Engine_Api::_()->user()->getViewer()->getIdentity(), 'extension' => substr($filename, strrpos($filename, '.') + 1)), $params);
    } else {
      $params = array_merge(array('type' => 'song', 'name' => $filename, 'parent_type' => 'seseventmusic_albumsongs', 'parent_id' => Engine_Api::_()->user()->getViewer()->getIdentity(), 'user_id' => Engine_Api::_()->user()->getViewer()->getIdentity(), 'extension' => substr($filename, strrpos($filename, '.') + 1)), $params);
    }
