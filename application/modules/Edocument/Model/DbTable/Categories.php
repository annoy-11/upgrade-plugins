 ...
 * @version    $Id: Categories.php 2016-07-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Edocument_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Edocument_Model_Category';
  ...
  public function getCategoriesAssoc($params = array()) {
    ...
    if ($viewerId && isset($params['member_levels']) && $params['member_levels'] == 1) {
      ...
      $stmt->where('CONCAT(engine4_edocument_categories.member_levels," ") LIKE "%' . $levelId . '%"');
    }
  ...
  public function getUserCategoriesAssoc($user) {
    ...
    $stmt = $this->getAdapter()
            ->select()
            ->from('engine4_edocument_categories', array('category_id', 'category_name'))
            ->joinLeft('engine4_edocument_documents', "engine4_edocument_documents.category_id = engine4_edocument_categories.category_id")
            ->group("engine4_edocument_categories.category_id")
            ->where('engine4_edocument_documents.owner_id = ?', $user)
            ->where('engine4_edocument_documents.draft = ?', "0")
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countDocuments'])) {
      $edocumentTable = Engine_Api::_()->getDbTable('edocuments', 'edocument')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($edocumentTable, "$edocumentTable.subcat_id=$tableName.category_id", array("total_documents_categories" => "COUNT(edocument_id)"));
      $category_select = $category_select->group("$tableName.category_id");
      $category_select->order('total_documents_categories DESC');
    }
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countDocuments'])) {
      $edocumentTable = Engine_Api::_()->getDbTable('edocuments', 'edocument')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($edocumentTable, "$edocumentTable.subsubcat_id=$tableName.category_id", array("total_documents_categories" => "COUNT(edocument_id)"));
  ...
  public function getCategory($params = array(), $customParams = array(), $searchParams = array()) {
    ...
    $documentTable = Engine_Api::_()->getDbTable('edocuments', 'edocument')->info('name');

    if ((isset($params['hasDocument']) && $params['hasDocument']) || isset($params['countDocuments']) || (isset($params['criteria']) && $params['criteria'] == 'most_document')) {
      $take = true;
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($documentTable, "$documentTable.category_id=$tableName.category_id", array("total_documents_categories" => "COUNT(edocument_id)"));
      if (empty($params['countDocuments'])) {
        $category_select = $category_select->having("COUNT($documentTable.category_id) > 0")
                ->group("$documentTable.category_id");
        if (isset($params['documentDesc'])) {
          if ($params['criteria'] && $params['criteria'] == 'most_document') {
            $category_select->order('total_documents_categories DESC');
          }
        }
      } else {
    ...
    if (isset($params['documentRequired'])) {
      if (empty($take)) {
        $category_select = $category_select->setIntegrityCheck(false);
        $category_select = $category_select->joinLeft($documentTable, "$documentTable.category_id=$tableName.category_id", array("total_documents_categories" => "COUNT(edocument_id)"));
      }
  ...
  public function getCloudCategory($params = array()) {
    ...
    $category_select = $this->select()
            ...
            ->joinLeft($documentTableName, "$documentTableName.category_id=$tableName.category_id", array("total_documents_categories" => "COUNT(edocument_id)"))
            ->where($tableName . '.subcat_id = ?', 0)
            ->where($tableName . '.subsubcat_id = ?', 0)
            ->group("$documentTableName.category_id")
            ->order('order DESC');
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('edocument_category');
  ...
  public function hasCategoryId($params = array()) {
    ...
    $category_select = $this->select()
            ->from($tableName, 'category_id')
            ->where('sedocument_categoryid = ?', $params['sedocument_categoryid']);
    return $category_select->query()->fetchColumn();
  }
}
