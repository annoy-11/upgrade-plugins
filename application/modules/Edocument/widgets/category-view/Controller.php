...
class Edocument_Widget_CategoryViewController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $show_subcatcriterias = isset($params['show_subcatcriteria']) ? $params['show_subcatcriteria'] : $this->_getParam('show_subcatcriteria', array('countDocument', 'icon', 'title'));
    ...
    $this->view->textDocument = $textDocument = isset($params['textDocument']) ? $params['textDocument'] : $this->_getParam('textDocument', 'Documents we love');
    $params = array('viewType' => $this->view->viewType,'document_limit' => $limit_data, 'description_truncation' => $descriptionLimit, 'pagging' => $loadOptionData, 'show_criterias' => $show_criterias,'category_id' => $categoryId, 'width' => $width, 'height' => $height, 'show_subcat' => $show_subcat, 'show_subcatcriteria' => $show_subcatcriterias, 'widthSubcat' => $widthSubcat, 'heightSubcat', $heightSubcat,'textDocument'=>$textDocument);
    if (Engine_Api::_()->core()->hasSubject()) {
      $this->view->category = $category = Engine_Api::_()->core()->getSubject();
      $category_id = $category->category_id;
    } else {
      $this->view->category = $category = Engine_Api::_()->getItem('edocument_category', $params['category_id']);
    ...
    if (!$is_ajax) {
      if ($category->subcat_id == 0 && $category->subsubcat_id == 0) {
        ...
        $innerCatData = Engine_Api::_()->getDbtable('categories', 'edocument')->getModuleSubcategory(array('category_id' => $category->category_id, 'column_name' => '*', 'countDocuments' => true));
        $columnCategory = 'category_id';
      } else if ($category->subsubcat_id == 0) {
        $innerCatData = Engine_Api::_()->getDbtable('categories', 'edocument')->getModuleSubsubcategory(array('countDocuments' => true, 'category_id' => $category->category_id, 'column_name' => '*'));
        $columnCategory = 'subcat_id';
      } else
        $columnCategory = 'subsubcat_id';
      $this->view->innerCatData = $innerCatData;
      //breadcum
      $this->view->breadcrumb = $breadcrumb = Engine_Api::_()->getDbtable('categories', 'edocument')->getBreadcrumb($category);
    }
    $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('edocuments', 'edocument')->getEdocumentsPaginator(array($columnCategory => $category->category_id, 'status' => 1));
