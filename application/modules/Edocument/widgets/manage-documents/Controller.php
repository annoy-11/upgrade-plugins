 ...
 */
class Edocument_Widget_ManageDocumentsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (!$is_ajax && is_array($defaultOptionsArray)) {
      foreach ($defaultOptionsArray as $key => $defaultValue) {
        if(!Engine_Api::_()->getApi('settings', 'core')->getSetting('edocument.enable.favourite', 1) && $defaultValue == 'mostSPfavourite')
            continue;
    ...
    // Get Documents
    $paginator = Engine_Api::_()->getDbtable('edocuments', 'edocument')->getEdocumentsPaginator($value);
