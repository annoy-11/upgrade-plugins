 ...
 */
class Edocument_Widget_CategoryCarouselController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $show_criterias = isset($values['show_criterias']) ? $values['show_criterias'] : $this->_getParam('show_criteria', array( 'title', 'countDocuments', 'icon','socialshare'));

    foreach ($show_criterias as $show_criteria)
      $this->view->{$show_criteria . 'Active'} = $show_criteria;

    $this->view->criteria =$params['criteria'] = $this->_getParam("criteria",array('alphabetical','most_document','admin_order'));

    $params['limit'] = $this->_getParam('limit_data', 15);
