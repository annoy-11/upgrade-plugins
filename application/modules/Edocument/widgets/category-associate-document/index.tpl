...
<?php if(!$this->is_ajax):?>
  <div id="scrollHeightDivSes_<?php echo $randonNumber;?>" class="sesbasic_bxs"> 
		<div id="category-document-widget_<?php echo $randonNumber; ?>" class="sesbasic_clearfix">
<?php endif;?>
  <?php foreach( $this->paginatorCategory as $item): ?>
  	<div class="edocument_category_document sesbasic_clearfix">
      <div class="edocument_category_header sesbasic_clearfix">
        <p class="floatL"><a href="<?php echo $item->getBrowseDocumentHref(); ?>?category_id=<?php echo $item->category_id ?>" title="<?php echo $this->translate($item->category_name); ?>"><?php echo $this->translate($item->category_name); ?></a></p>
				<?php if(isset($this->seemore_text) && $this->seemore_text != ''): ?>
					<span <?php echo $this->allignment_seeall == 'right' ?  'class="floatR"' : ''; ?>><a  class="sesbasic_link_btn" href="<?php echo $item->getBrowseDocumentHref(); ?>?category_id=<?php echo $item->category_id ?>"><?php $seemoreTranslate = $this->translate($this->seemore_text); ?>
      ...
      <?php if(isset($this->resultArray['document_data'][$item->category_id])):?>
        <?php $bigBlg = 0;?>
        <?php	foreach($this->resultArray['document_data'][$item->category_id] as $item):?>
            <?php if(!$bigBlg):?>
            <div class="edocument_category_item_single">
            <div class="edocument_category_item_single_info">
              <div class="edocument_entry_img">
                <a href="<?php echo $item->getHref();?>"><img src="<?php echo $item->getPhotoUrl('thumb.main'); ?>" /></a>
              </div>
              <div class="edocument_category_item_single_content">
            ...
            <div class="edocument-category_item_list sesbasic_custom_scroll">
            <?php else:?>
              <div class="edocument_category_item sesbasic_clearfix">
		...
		<div class="tip">
			<span>
				...
				<?php if ($this->can_create):?>
					<?php echo $this->translate('Be the first to %1$screate%2$s one!', '<a href="'.$this->url(array('action' => 'create','module'=>'edocument'), "edocument_general",true).'">', '</a>'); ?>
				<?php endif; ?>
			</span>
		</div>
	<?php endif; ?> 
	<?php if($this->loadOptionData == 'pagging'): ?>
		<?php echo $this->paginationControl($this->paginatorCategory, null, array("_pagging.tpl", "edocument"),array('identityWidget'=>$randonNumber)); ?>
	<?php endif; ?>
<?php if(!$this->is_ajax){ ?>
		</div>
	</div>
...
<script type="text/javascript">
	...
	function viewMore_<?php echo $randonNumber; ?> (){
		...
		en4.core.request.send(new Request.HTML({
			method: 'post',
			'url': en4.core.baseUrl + "widget/index/mod/edocument/name/<?php echo $this->widgetName; ?>",
			'data': {
			format: 'html',
			...
			onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
				document.getElementById('category-document-widget_<?php echo $randonNumber; ?>').innerHTML = document.getElementById('category-document-widget_<?php echo $randonNumber; ?>').innerHTML + responseHTML;
		...
		function paggingNumber<?php echo $randonNumber; ?>(pageNum){
			...
			en4.core.request.send(new Request.HTML({
				...
				onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
					...
					document.getElementById('category-document-widget_<?php echo $randonNumber; ?>').innerHTML =  responseHTML;
          jqueryObjectOfSes(".sesbasic_custom_scroll").mCustomScrollbar({
          theme:"minimal-dark"
          });
					dynamicWidth();
