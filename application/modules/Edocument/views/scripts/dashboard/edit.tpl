...
<?php if(!$this->is_ajax){ ?>
  <?php
  echo $this->partial('dashboard/left-bar.tpl', 'edocument', array(
  'document' => $this->document,
  ));	
  ?>
  <div class="sesbasic_dashboard_content sesbm sesbasic_clearfix">
<?php } ?>
<div class="sesbasic_dashboard_form edocument_create_form sesbasic_bxs">
...
//prdocument form submit on enter
...
var counter = 0;
  function validateForm(){
    ...
    jqueryObjectOfSes('#edocuments_edit input, #edocuments_edit select,#edocuments_edit checkbox,#edocuments_edit textarea,#edocuments_edit radio').each(
      function(index){
  ...
  jqueryObjectOfSes('#edocuments_edit').submit(function(e){
    var validationFm = validateForm();
		if(!validationFm && sesJqueryObject('#edocument_schedule_time').length > 0 ) {
