...
<style>
#edocument_schedule_date{display:block !important;}
</style>
<div class="edocument_choose_date">
  <div id="event_start_time-wrapper" class="form-wrapper">
    ...
    <div id="event_start_time-element" class="form-element">
      <span class="edocument-date-field"><input type="text" class="displayF" name="start_date" id="edocument_schedule_date" value="<?php echo isset($this->start_date) ? ($this->start_date) : ''  ?>"></span>
      <span class="edocument-time-field"><input type="text" name="start_time" id="edocument_schedule_time" value="<?php echo isset($this->start_time) ? ($this->start_time) : ''  ?>" class="ui-timepicker-input" autocomplete="off"></span>
    </div>
  </div>
</div>
<div id="event_error_time-wrapper" class="form-wrapper" style="display:none;">
  <div class="form-element tip"><span id="edocument_schedule_error_time-element"></span></div>
</div>
<script type="application/javascript">
 ...
 en4.core.runonce.add(function() {
	...
	<?php } ?>
	var sesselectedDate =  new Date(sesJqueryObject('#edocument_schedule_date').val());
	var sesFromEndDate;
	sesBasicAutoScroll('#edocument_schedule_time').timepicker({
			'showDuration': true,
			'timeFormat': 'g:ia',
	}).on('changeTime',function(){
		var lastTwoDigitStart = sesBasicAutoScroll('#edocument_schedule_time').val().slice('-2');
		var startDate = new Date(sesBasicAutoScroll('#edocument_schedule_date').val()+' '+sesBasicAutoScroll('#edocument_schedule_time').val().replace(lastTwoDigitStart,'')+':00 '+lastTwoDigitStart);
		var error = checkDateTime(startDate);
		if(error != ''){
			sesBasicAutoScroll('#event_error_time-wrapper').show();
			sesBasicAutoScroll('#edocument_schedule_error_time-element').text(error);
		}else{
			sesBasicAutoScroll('#event_error_time-wrapper').hide();
		}
	});
	sesBasicAutoScroll('#edocument_schedule_date').datepicker({
			format: 'm/d/yyyy',
			weekStart: 1,
			autoclose: true,
...
function checkDateTime(startdate){
	...
	if(<?php echo $this->subject ? 1 : 0 ?> == 0 && currentTime.valueOf() > startdate.valueOf() && sesBasicAutoScroll('#edocument_schedule_date').val() && 1 == '<?php echo $this->start_time_check; ?>'){
		errorMessage = "<?php echo $this->translate('Document Schedule date is in the past. Please enter an document scheduled date greater than or equal to today\'s date.')?>";	
	}
	return errorMessage;
	}
</script>
