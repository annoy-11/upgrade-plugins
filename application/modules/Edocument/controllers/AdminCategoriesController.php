 ...
 * @version    $Id: AdminCategoriesController.php 2016-07-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Edocument_AdminCategoriesController extends Core_Controller_Action_Admin {

  public function indexAction() {

    $catTable = Engine_Api::_()->getDbtable('categories', 'edocument');

    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('edocument_admin_categories', array(), 'edocument_admin_main_subcategories');
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('edocument_admin_main', array(), 'edocument_admin_main_categories');
    ...
    if (isset($_POST['selectDeleted']) && $_POST['selectDeleted']) {
      if (isset($_POST['data']) && is_array($_POST['data'])) {
        ...
        foreach ($_POST['data'] as $key => $valueSelectedcategory) {
          $categoryDelete = Engine_Api::_()->getItem('edocument_category', $valueSelectedcategory);
          $deleteCategory = $catTable->deleteCategory($categoryDelete);
          if ($deleteCategory) {
    ...
    if (isset($_POST['is_ajax']) && $_POST['is_ajax'] == 1) {
      ...
      $slugExists = $catTable->slugExists($value['slug']);
      if (!$slugExists) {
        echo json_encode(array('slugError' => true));
        die;
      }
      if ($cat_id != -1) {
        $categoryData = Engine_Api::_()->getItem('edocument_category', $cat_id);
        if ($categoryData->subcat_id == 0) {
          $value['subcat_id'] = $cat_id;
          $seprator = '&nbsp;&nbsp;&nbsp;';
          $tableSeprator = '-&nbsp;';
          $parentId = $cat_id;
          $value['order'] = $catTable->orderNext(array('subcat_id' => $cat_id));
        } else {
          $value['subsubcat_id'] = $cat_id;
          $seprator = '3';
          $tableSeprator = '--&nbsp;';
          $value['order'] = $catTable->orderNext(array('subsubcat_id' => $cat_id));
          $parentId = $cat_id;
        }
      } else {
        $parentId = 0;
        $seprator = '';
        $value['order'] = $catTable->orderNext(array('category_id' => true));
        $tableSeprator = '';
      }
      ...
      try {
        ...
        if (isset($_FILES['icon']['name']) && $_FILES['icon']['name'] != '') {
          $row->cat_icon = $row->setPhoto($_FILES['icon'], $row->category_id);
        }
        if (isset($_FILES['thumbnail']['name']) && $_FILES['thumbnail']['name'] != '') {
          $row->thumbnail = $row->setPhoto($_FILES['thumbnail'], $row->category_id, true);
        }
        $row->save();
        $db->commit();
      } catch (Exception $e) {
    ...
    //profile types
    $profiletype = array();
    $topStructure = Engine_Api::_()->fields()->getFieldStructureTop('edocument');
    if (count($topStructure) == 1 && $topStructure[0]->getChild()->type == 'profile_type') {
      $profileTypeField = $topStructure[0]->getChild();
      $options = $profileTypeField->getOptions();
      $options = $profileTypeField->getElementParams('edocument');
    ...
    //Get all categories
    $this->view->categories = $catTable->getCategory(array('column_name' => '*', 'profile_type' => true));
  }
  ...
  public function changeOrderAction() {
    ...
    if ($this->_getParam('id', false) || $this->_getParam('nextid', false)) {
      ...
      $categoryTypeId = '';
      $checkTypeCategory = $dbObject->query("SELECT * FROM engine4_edocument_categories WHERE category_id = " . $category_id)->fetchAll();
      ...
      if ($checkTypeCategory)
        $currentOrder = Engine_Api::_()->getDbtable('categories', 'edocument')->order($categoryType, $categoryTypeId);
      ...
      $checkCategoryChildrenCondition = $dbObject->query("SELECT * FROM engine4_edocument_categories WHERE subcat_id = '" . $id . "' || subsubcat_id = '" . $id . "' || subcat_id = '" . $nextid . "' || subsubcat_id = '" . $nextid . "'")->fetchAll();
  ...
  public function editCategoryAction() {
    ...
    $this->view->form = $form = new Edocument_Form_Admin_Category_Add();

    $cat_id = $this->_getParam('id');
    $category = Engine_Api::_()->getItem('edocument_category', $cat_id);
    ...
    if (!$form->isValid($this->getRequest()->getPost())) {
      if (isset($_POST['slug'])) {
        $slugExists = Engine_Api::_()->getDbtable('categories', 'edocument')->slugExists($value['slug'], $cat_id);
        if (!$slugExists) {
    ...
    try {
      ...
      if (isset($_FILES['cat_icon']) && !empty($_FILES['cat_icon']['name'])) {
        $Icon = $category->setPhoto($form->cat_icon, $cat_id);
        if (!empty($Icon)) {
      ...
      if (isset($_FILES['thumbnail']) && !empty($_FILES['thumbnail']['name'])) {
        $Thumbnail = $category->setPhoto($form->thumbnail, $cat_id, true);
        if (!empty($Thumbnail)) {
    ...
    return $this->_helper->redirector->gotoRoute(array('module' => 'edocument', 'action' => 'index', 'controller' => 'categories'), 'admin_default', true);
  }
}
