...
class Booking_Model_DbTable_Usergateways extends Engine_Db_Table
{
  protected $_rowClass = 'Booking_Model_Usergateway';
  ...
  public function getEnabledGatewayCount()
  {
	...
	public function getUserGateway($params = array()){
			$select = $this->select()->from($this->info('name'))->where('professional_id =?',$params['professional_id']);
			if(!isset($params['enabled']))
				$select->where('enabled = 1 OR test_mode = 1');
