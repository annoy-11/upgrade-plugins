 ...
 * @version    $Id: Categories.php  2019-03-19 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Booking_Model_DbTable_Categories extends Engine_Db_Table {

  protected $_rowClass = 'Booking_Model_Category';
  ...
  public function getCategory($params = array(), $customParams = array()) {
    ...
    $serviceTable = Engine_Api::_()->getDbTable('services', 'booking')->info('name');
  ...
  public function order($categoryType = 'category_id', $categoryTypeId) {
    ...
    $table = Engine_Api::_()->getItemTable('booking_category');
  ...
  public function getCategoriesAssoc($params = array()) {
    ...
    if ($viewerId && isset($params['member_levels']) && $params['member_levels'] == 1) {
      ...
      $stmt->where('CONCAT(engine4_booking_categories.member_levels," ") LIKE "%' . $levelId . '%"');
    }
  ...
  public function getModuleSubcategory($params = array()) {
    ...
    if (isset($params['countEvents'])) {
      $serviceTable = Engine_Api::_()->getDbTable('services', 'booking')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($serviceTable, "$serviceTable.subcat_id=$tableName.category_id", array("total_events_categories" => "COUNT($serviceTable.photo_id)"));
      $category_select = $category_select->group("$tableName.category_id");
  ...
  public function getModuleSubsubcategory($params = array()) {
    ...
    if (isset($params['countEvents'])) {
      $serviceTable = Engine_Api::_()->getDbTable('services', 'booking')->info('name');
      $category_select = $category_select->setIntegrityCheck(false);
      $category_select = $category_select->joinLeft($serviceTable, "$serviceTable.subsubcat_id=$tableName.category_id", array("total_events_categories" => "COUNT($serviceTable.service_id)"));
