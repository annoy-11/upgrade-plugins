...
<div class="sescommunitads_details sesbasic_bxs">
  ...
  <?php }else{ ?>
      ...
      if($isrenew){
        if($item->ad_type == "perclick"){
          if($item->ad_limit != "-1" && $timePeriod <= $item->ad_limit - $item->click_count){ ?>
            <div class="sesbasic_clearfix"><a href="<?php echo $this->url(array('sescommunityad_id' => $item->sescommunityad_id,'action'=>'index'), 'sescomminityads_payment', true); ?>" class="sescontest_payment_btn sesbasic_animation"><i class="fa fa-paypal"></i><span><?php echo $this->translate("Reniew Advertisement Payment"); ?></span></a></div>
        <?php
          }
        ...
        <?php
          ...
          if(!empty($transaction->expiration_date) && $transaction->expiration_date != '3000-00-00 00:00:00'){
            ...
            <div class="sesbasic_clearfix"><a href="<?php echo $this->url(array('sescommunityad_id' => $item->sescommunityad_id,'action'=>'index'), 'sescomminityads_payment', true); ?>" class="sescmads_payment_btn sesbasic_animation"><i class="fa fa-paypal"></i><span><?php echo $this->translate("Reniew Advertisement Payment"); ?></span></a></div>
           <?php 	
            }
          }
        }
    ...
    <?php }else{  ?>
      <div>
        <a href="<?php echo $this->url(array('sescommunityad_id' => $item->sescommunityad_id,'action'=>'index'), 'sescomminityads_payment', true); ?>" class="sescmads_payment_btn sesbasic_animation"><i class="fa fa-money"></i><span><?php echo $this->translate("Make Payment"); ?></span></a>
      </div>
    <?php } ?>
  <?php } ?>
</div>  
