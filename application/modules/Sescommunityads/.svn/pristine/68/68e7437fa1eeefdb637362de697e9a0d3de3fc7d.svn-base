 ...
 * @version    $Id: Package.php  2018-10-09 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sescommunityads_Model_Package extends Core_Model_Item_Abstract
{
  ...
  public function getPackageDescription()
  {
    ...
    else if($this->package_type == "nonRecurring"){
      if($this->click_type == "perday")
        $type = "Days";
      else if($this->click_type == "perview")
        $type = "Views";
      else
        $type = "Clicks";
      
      $clickLimit = $this->click_limit ? $this->click_limit : "Unlimited";
      $str = sprintf($translate->translate('%1$s One-time fee for %2$s %3$s'),$priceStr, $clickLimit,$type);      
    }
    // Plan is recurring
  ...
  public function getExpirationDate($rel = null)
  {
    ...
    if($this->package_type == "nonRecurring"){
      if($this->click_type == "perday"){
        return date('Y-m-d H:i:s',strtotime(date('Y-m-d H:i:s')." + ".$this->click_limit." days"));  
      }else{ 
        return "";  
      }
    }
