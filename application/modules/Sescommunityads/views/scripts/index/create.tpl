...
<div class="sescommunity_create_cnt ads_preview_page" rel="3" style="display:none">
  ...
  <?php 
    $count = 5;
    if((!$this->package->targetting || count($this->formField) < 2) && (!$this->package->networking || !array_key_exists('network_enable',$this->targetFields) || ($this->package->networking && !count($this->networks)))){
      $count = 4;
    }
  ?>   
  ...
  <div class="sescmads_create_main">
          ...
          <div class ="tabcontent sescmads_create_targetting" id="choose_targetting">
            <div class="target_inner">
              <div class="sescmads_create_targetting_tabs">
                <ul>
                  <?php if($this->package->targetting  && count($this->formField) > 1){ ?>
                  <?php 
                     $counter = 0;
                  ...
                  ?>
                  <?php } ?>
                  <?php if($this->package->networking && count($this->networks) && array_key_exists('network_enable',$this->targetFields)){ ?>
              ...
              <div class="sescommunityads_packages sesbasic_bg">
                ...
                <div class="sescommunity_network_targetting sesbasic_clearfix">
                  <select multiple name="network[]" id="networks">
                    ...
                    <option value="<?php echo $network->getIdentity(); ?>" <?php echo !empty($this->targetData) && in_array($network->getIdentity(),$this->targetData) ? "selected" : "";  ?>><?php echo $network->getTitle(); ?></option>
                    <?php } ?>
                  </select>
                </div>
                <?php } ?>
      ...
      <?php if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesmember') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sescommunityads_enable_location',1) && Engine_Api::_()->getApi('settings', 'core')->getSetting('enableglocation', 1)){ ?>
       ...
       <?php if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesinterest') && !empty($this->package->interests)) { ?>
          ...
          <?php if(count($getResults) > 0) { ?>
						<div class="ad_scheduling sescmads_create_step">
                            ...
                            <li>
  														<input type="checkbox" id="intrest_<?php echo $getResult->interest_id; ?>" name="interest_enable" value="<?php echo $getResult->interest_id; ?>" <?php echo !empty($this->interestTargetData) && in_array($getResult->interest_id,$this->interestTargetData) ? "checked" : "";  ?>><label for="intrest_<?php echo $getResult->interest_id; ?>"><?php echo $getResult->interest_name; ?></label>
                            </li>
													<?php } ?>
												</ul>
											</div>  
            ...
            <div class="sescommunityads_left_text sesbasic_clearfix">
              <div class="sescmads_format_inner">
                <div class="sescommunityads_select_content">
                  ...
                  <input type="text" class="displayF" name="start_date" id="sescomm_start_date" value="<?php echo isset($this->ad->startdate) ? date('m/d/Y',strtotime($this->ad->startdate)) : date('m/d/Y')  ?>">
                  <input type="text" name="start_time" id="sescomm_start_time" value="<?php echo isset($this->ad->startdate) ? date('H:ia',strtotime($this->ad->startdate)) : date('H:ia',strtotime('+46 hours',time()))  ?>" class="ui-timepicker-input" autocomplete="off">
                </div>
                <div class="sescommunityads_select_content sescomm_end_date_div" style="display:<?php echo isset($this->ad->enddate) && $this->ad->enddate != "0000-00-00 00:00:00" ? 'block' : (empty($this->ad) ? 'block' : "none"); ?>">
                  ...
                  <input type="text" class="displayF" name="end_date" id="sescomm_end_date" value="<?php echo isset($this->ad->enddate) && $this->ad->enddate != "0000-00-00 00:00:00" ? date('m/d/Y',strtotime($this->ad->enddate))  : (empty($this->ad) ? date('m/d/Y',strtotime('+5 day',time())) : "");  ?>">
                  <input type="text" name="end_time" id="sescomm_end_time" value="<?php echo isset($this->ad->enddate) && $this->ad->enddate != "0000-00-00 00:00:00" ? date('H:ia',strtotime($this->ad->enddate)) : (empty($this->ad) ?  date('H:ia',strtotime('+46 hours',time())) : "" )  ?>" class="ui-timepicker-input" autocomplete="off">
                </div>
...
sesJqueryObject(document).ready(function(){
  <?php if(!empty($this->ad)){ ?>
    ...
    sesJqueryObject('a[rel='+typeValue+']').trigger('click');
    sesJqueryObject('.sescomm_back_btn').remove();
    sesJqueryObject('#communityAds_campaign').trigger('change');
  ...
  <?php }elseif(!empty($this->widgetid)){ ?>
    sesJqueryObject('a[rel='+'promote_website_cnt'+']').trigger('click');
    sesJqueryObject('.sescomm_back_btn').remove();
  <?php }else{ ?>
    sesJqueryObject('.sescomm_stp_1').show();
  <?php } ?>
});
...
sesJqueryObject(document).on('click','.tablinks',function(){
   var totalTabs = sesJqueryObject('.tablinks');
   for(i=0;i<totalTabs.length;i++){
     document.getElementById(sesJqueryObject(totalTabs[i]).data('rel')).style.display = "none"; 
   }
   sesJqueryObject('.tablinks').removeClass('active');
   document.getElementById(sesJqueryObject(this).data('rel')).style.display = "block";
...
sesJqueryObject(function(){
  sesJqueryObject(window).scroll(function(){
    return;
    var sescmads_create_preview_video = sesJqueryObject('.sescmads_create_preview_video');
    var sescmads_ad_preview_boost_post = sesJqueryObject('.sescmads_ad_preview_boost_post');
    var sescmads_create_preview_image = sesJqueryObject('.sescmads_create_preview_image');
    var sescmads_create_preview_carousel = sesJqueryObject('.sescmads_create_preview_carousel');
    var rightElement = sesJqueryObject('.sescmads_create_main_right').find('.sescmads_create_step').eq(0);    
    if(sescmads_create_preview_video.css('display') != "none" || sescmads_ad_preview_boost_post.css('display') != "none" || sescmads_create_preview_image.css('display') != "none" || sescmads_create_preview_carousel.css('display') != "none"){
      if(typeof leftElementOrgHeight == "undefined")
        leftElementOrgHeight = sesJqueryObject('.sescmads_create_main_left').height();
      scrollTopPosition = sesJqueryObject(this).scrollTop() + 20;
      var getElementTopPosition = (rightElement.offset().top - sesJqueryObject(document).scrollTop()) + rightElement.height();     
      var fixedHeader = sesJqueryObject('.layout_page_header').css('position');
      if(fixedHeader != "fixed"){
          fixedHeader = sesJqueryObject('.global_header').css('position');
      }
      var height = '0';
      if(fixedHeader == "fixed"){
      ...
      rightElement.css('width',sesJqueryObject('.sescommunityads_right_preview').width()+"px");     
