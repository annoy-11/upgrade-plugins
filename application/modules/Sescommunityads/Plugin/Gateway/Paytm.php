...
class Sescommunityads_Plugin_Gateway_Paytm extends Engine_Payment_Plugin_Abstract
{
  ...
  public function createAdTransaction(User_Model_User $user,
  ...
  public function onAdTransactionReturn(
    ...
    $taxId = isset($params['TXNID']) ? $params['TXNID'] : '';
    // One-time
    if($package->isOneTime()) {
       $order->state = $orderStatus;
      $order->gateway_transaction_id = $taxId;
      $order->save();
      ...
      if ($paymentStatus == 'okay' || $paymentStatus == 'active' ||
        ...
        $transaction->gateway_profile_id = $taxId;
        $transaction->save();
        // Payment success
