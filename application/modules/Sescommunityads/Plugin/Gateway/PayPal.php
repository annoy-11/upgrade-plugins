 ...
 * @version    $Id: PayPal.php  2018-10-09 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
class Sescommunityads_Plugin_Gateway_PayPal extends Engine_Payment_Plugin_Abstract {
  ...
  public function getGateway() {
    if (null === $this->_gateway) {
      ...
      $gateway = new $class(array(
          ...
          'currency' => Engine_Api::_()->sescommunityads()->getCurrentCurrency(),
      ));
  ...
  public function onAdTransactionIpn(
  Payment_Model_Order $order, Engine_Payment_Ipn $ipn) {
      ...
      switch ($rawData['txn_type']) {
          ...
          if ($package->isOneTime()) {
            switch ($rawData['payment_status']) {
              ...
              case 'Pending':
                ...
                // Get benefit setting
                $giveBenefit = Engine_Api::_()->getDbtable('transactions', 'sescommunityads')->getBenefitStatus($user);
                if ($giveBenefit) {
                  $item->onPaymentSuccess();
                } else {
                  $item->onPaymentPending();
              ...
              case 'Expired': // Not sure about this one
                ...
                Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $owner, $item, 'sescommunityads_adsexpired', array("adsLink" => $link));


                //Send email to user
                Engine_Api::_()->getApi('mail', 'core')->sendSystem($owner->email, 'sescommunityads_adsexpired', array('host' => $_SERVER['HTTP_HOST'], 'queue' => false, 'title' => $item->title, 'description' => $item->description, 'ad_link' => $link));
        ...
        case 'recurring_payment_outstanding_payment':
          ...
          // send notification
            ...
            Engine_Api::_()->getDbtable('notifications', 'activity')->addNotification($owner, $owner, $item, 'sescommunityads_adsoverdue', array("adsLink" => $link));

            //Send email to user
            Engine_Api::_()->getApi('mail', 'core')->sendSystem($owner->email, 'sescommunityads_adsoverdue', array('host' => $_SERVER['HTTP_HOST'], 'queue' => false, 'title' => $item->title, 'description' => $item->description, 'ad_link' => $link));
  ...
  public function cancelAd($transactionId, $note = null) {
    $profileId = null;
    if ($transactionId instanceof Sescommunityads_Model_Transaction) {
  ...
  public function onIpn(Engine_Payment_Ipn $ipn) {
    ...
    if ($order) {
      $ipnProcessed = false;
      // Subscription IPN
      if ($order->source_type == 'sescommunityads') {
        $this->onAdTransactionIpn($order, $ipn);
        $ipnProcessed = true;
      }
