 ...
 * @version    $Id: PaymentController.php  2018-10-09 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sescommunityads_PaymentController extends Core_Controller_Action_Standard
{
  /**
   * @var User_Model_User
   */
  ...
  public function init()
  {
    ...
    if( Engine_Api::_()->getDbtable('gateways', 'payment')->getEnabledGatewayCount() <= 0 ||
        Engine_Api::_()->getDbtable('packages', 'sescommunityads')->getEnabledNonFreePackageCount() <= 0 ) {
      return $this->_helper->redirector->gotoRoute(array('action'=>'manage'), 'sescommunityads_general', true);
    }

    // Get user and session
    $this->_user = Engine_Api::_()->user()->getViewer();
    $this->_session = new Zend_Session_Namespace('Payment_Sescommunityads');
		$this->_session->gateway_id = $this->_getParam('gateway_id',0);
    $this->_item = Engine_Api::_()->getItem('sescommunityads',$this->_getParam('sescommunityad_id'));
		if(!$this->_item)
			 return $this->_helper->redirector->gotoRoute(array('action'=>'create'), 'sescommunityads_general', true);
  ...
  public function gatewayAction()
  {
    ...
    if( !$this->_user ||
        ...
        !($package = Engine_Api::_()->getItem('sescommunityads_packages', $item->package_id)) ) {
      return $this->_helper->redirector->gotoRoute(array('action' => 'manage'),'sescommunityads_general',true);
    }
    ...
    // For Coupon 
    $this->view->couponSessionCode = $package->getType().'-'.$package->package_id.'-'.$this->_item->getType().'-'.$this->_item->sescommunityad_id.'-1';
  ...
  public function processAction()
  {
    ...
    if( !$gatewayId ||
        !($gateway = Engine_Api::_()->getDbtable('gateways', 'sescommunityads')->find($gatewayId)->current()) ||
        !($gateway->enabled) ) {
    ...
    if( $this->_checkItemStatus($this->_item) ) {
     ...
     // For Coupon 
    $couponSessionCode = $package->getType().'-'.$package->package_id.'-'.$this->_item->getType().'-'.$this->_item->sescommunityad_id.'-1';
    ...
    $params['cancel_url'] = $schema . $host
      ...
      . '?order_id=' . $order_id
      . '&state=' . 'cancel';
    $params['ipn_url'] = $schema . $host . $this->view->url(array('action' => 'index', 'controller' => 'ipn', 'module' => 'sescommunityads'), 'default') . '?order_id=' . $order_id.'&gateway_id='.$gatewayId;
        //For Coupon
    ...
    $creditCode =  'credit'.'-sescommunityads-'.$package->package_id.'-'.$this->_item->sescommunityad_id;
    ...
    if($gateway->plugin == "Sesadvpmnt_Plugin_Gateway_Stripe") {
        ...
        $paytmParams = $plugin->createAdTransaction($this->_user,$this->_item, $package, $params);
      ...
      $transaction = $plugin->createAdTransaction($this->_user,
  ...
  public function returnAction()
  {
    ...
    if( !$this->_user ||
        ...
        $order->source_type != 'sescommunityads' ||
        !($item = $order->getSource()) ||
        !($package = $this->_item->getPackage()) ||
        !($gateway = Engine_Api::_()->getItem('sescommunityads_gateway', $order->gateway_id)) ) {
      return $this->_helper->redirector->gotoRoute(array('action'=>'manage'), 'sescommunityads_general', true);
    }
    // Get gateway plugin
    $this->view->gatewayPlugin = $gatewayPlugin = $gateway->getGateway();
    ...
    //For Coupon
    $couponSessionCode = $couponSessionCode = $package->getType().'-'.$package->package_id.'-'.$this->_item->getType().'-'.$this->_item->sescommunityad_id.'-1';
    $params['amount'] = @isset($_SESSION[$couponSessionCode]) ? round($package->price - $_SESSION[$couponSessionCode]['discount_amount']) : $package->price;
    ...
    try {
        if($gateway->plugin == "Sesadvpmnt_Plugin_Gateway_Stripe") {
          if(isset($_POST['stripeToken'])){
            ...
            $params['currency'] = Engine_Api::_()->sescommunityads()->getCurrentCurrency();
            $params['type'] = "sescommunityads";
            if(!$package->isOneTime()) {
            ...
            $transaction = $plugin->createAdTransaction($this->_user, $this->_item, $package, $params);
          }
          $params['transaction'] = $transaction;
          $status = $plugin->onAdTransactionReturn($order,$params);
      } else {
        $status = $plugin->onAdTransactionReturn($order,array_merge($this->_getAllParams(),$params));
      }
  ...
  public function finishAction()
  {
    ...
    $this->view->error = $this->_session->errorMessage;
		$this->view->sescommunityad_id = $sescommunityad_id = $this->_getParam('sescommunityad_id');
