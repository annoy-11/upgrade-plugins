...
class Sesforum_AdminManagePostsController extends Core_Controller_Action_Admin
{
  public function indexAction()
  {
    $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
      ->getNavigation('sesforum_admin_main', array(), 'sesforum_admin_main_manageposts');

    $this->view->formFilter = $formFilter = new Sesforum_Form_Admin_FilterPost();
    if ($this->getRequest()->isPost()) {
      $values = $this->getRequest()->getPost();
      foreach ($values as $key => $value) {
        if ($key == 'delete_' . $value) {
          $sesforum = Engine_Api::_()->getItem('sesforum_post', $value);
          $sesforum->delete();
        }
      }
    }
  ...
  public function deleteAction()
  {
    ...
    if( $this->getRequest()->isPost() )
    {
      ...
      try
      {
        $sesforum = Engine_Api::_()->getItem('sesforum_post', $id);
