...
class Sesforum_TopicController extends Core_Controller_Action_Standard
{
  public function init()
  {
    if( 0 !== ($topic_id = (int) $this->_getParam('topic_id')) &&
        null !== ($topic = Engine_Api::_()->getItem('sesforum_topic', $topic_id)) &&
        $topic instanceof Sesforum_Model_Topic ) {
  ...
  public function editAction()
  {
    ...
    $this->view->form = $form = new Sesforum_Form_Topic_Edit();
 ...
 public function stickyAction()
  {
    ...
    $this->view->topic = $topic = Engine_Api::_()->core()->getSubject('sesforum_topic');
    $this->view->sesforum = $sesforum = $topic->getParent();
    if( !$this->_helper->requireAuth()->setAuthParams('sesforum_forum', null, 'topic.edit')->isValid() ) {
      return;
    }
