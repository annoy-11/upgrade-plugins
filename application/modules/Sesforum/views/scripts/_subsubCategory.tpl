 ...
 * @version    $Id: _subsubCategory.tpl 2019-06-05 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
 
 ?>
<?php  if($this->cat3rdShow) { ?> 
<?php $subsubCategories = Engine_Api::_()->getItemTable('sesforum_category')->fetchAll(Engine_Api::_()->getItemTable('sesforum_category')->select()->where('subsubcat_id = ?', $subCategorie->category_id)->where("privacy LIKE ? ", '%' . $levelId. '%')->order('order ASC')); ?>
    <?php if(count($subsubCategories) > 0) { ?>
        <?php foreach($subsubCategories as $subsubCategory) { ?>
            <div class="sesforum_main_category_item_header">
            <?php if(!empty($subsubCategory->cat_icon)) { ?>
                <div class="_icon">
                <?php $cat_icon = Engine_Api::_()->storage()->get($subsubCategory->cat_icon, '');
                if($cat_icon) {
            ...
            <div class="_info">
                <div class="_title">
                <a href="<?php echo $subsubCategory->getHref(); ?>"> 
                    <?php echo $this->translate($subsubCategory->getTitle()); ?>
                </a>
            </div>
                <p> 
                    <?php if(strlen($subsubCategory->description) > $this->description_truncation_category): ?>
                        <?php $title = mb_substr($subsubCategory->description,0,$this->description_truncation_category).'...';?>
                        <?php echo $this->translate($title); ?>
                    <?php else: ?>
                        <?php echo $this->translate($subsubCategory->description); ?>
                    <?php endif;?>
                </p>
            </div>
            </div>
            ...
            <?php if(isset($this->forumShow) && $this->forumShow != 0) { ?>
                <?php foreach( $this->sesforums[$subsubCategory->category_id] as $sesforum ):
