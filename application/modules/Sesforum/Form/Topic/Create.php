...
class Sesforum_Form_Topic_Create extends Engine_Form
{
  public function init()
  {
    ...
    $this->setMethod("POST");
    $this->setAttrib('name', 'sesforum_post_create');
    $this->addElement('Text', 'title', array(
      'label' => 'Topic Title',
      'allowEmpty' => false,
      'required' => true,
    ));
     $tag = $settings->getSetting('sesforum.tag.mandatory', 1);
     if($tag == 1) {
        $required = true;
        $allowEmpty = false;
     } else {
    ...
    $allowHtml = (bool) $settings->getSetting('sesforum.html', 0);
    $allowBbcode = (bool) $settings->getSetting('sesforum.bbcode', 0);
