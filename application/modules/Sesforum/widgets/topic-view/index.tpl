...
<script type="text/javascript">
  en4.core.runonce.add(function() {
    ...
    var rate = window.rate = function(rating) {
      ...
      (new Request.JSON({
        'format': 'json',
        'url' : '<?php echo $this->url(array('module' => 'sesforum', 'controller' => 'index', 'action' => 'rate'), 'default', true) ?>',
        'data' : {
          'format' : 'json',
          'rating' : rating,
          'topic_id': topic_id
...
<div class="sesforum_topic_view sesbasic_bxs<?php if( $this->topic->closed ): ?> sesforum_topic_closed<?php endif; ?>">
  <div class="sesforum_topic_view_header sesbasic_clearfix">
    <div class="sesforum_topic_title">
      <h3><?php echo $this->topic->getTitle() ?></h3>
    </div>
    <div class="sesforum_topic_header_options">
      <?php if(isset($this->backToTopicButton)): ?>
        <?php echo $this->htmlLink($this->sesforum->getHref(), $this->translate('Back To Topics'), array(
      ...
      <?php if( $this->canPost && isset($this->postReply)): ?>
        <?php echo $this->htmlLink($this->topic->getHref(array('action' => 'post-create')), $this->translate('Post Reply'), array(
          'class' => 'sesforum_icon_post_reply sesbasic_button'
        )) ?>
      <?php endif; ?>
      <?php if( $this->viewer->getIdentity() && $this->settings->getSetting('sesforum.subscribe', 1)): ?>
            <?php if(!$this->isWatching ): ?>
            <?php echo $this->htmlLink($this->url(array('action' => 'watch', 'watch' => '1')), $this->translate('Subscribe'), array(
                'class' => 'sesforum_icon_topic_watch sesbasic_button'
            )) ?>
            <?php else: ?>
            <?php echo $this->htmlLink($this->url(array('action' => 'watch', 'watch' => '0')), $this->translate('Unsubscribe'), array('class' => 'sesforum_icon_topic_unwatch sesbasic_button')); ?>
            <?php endif; ?>
        <?php if(0) { ?>
        <?php $isSubscribe = Engine_Api::_()->getDbTable('subscribes', 'sesforum')->isSubscribe(array('resource_id' => $this->topic->getIdentity())); ?>
            <a class="active sesbasic_icon_subscribe sesbasic_button" id="<?php echo $this->topic->getType(); ?>_unsubscribe_<?php echo $this->topic->getIdentity(); ?>" style ='display:<?php echo $isSubscribe ? "inline-block" : "none" ?>' href = "javascript:void(0);" onclick = "sesforumSubscribe('<?php echo $this->topic->getIdentity(); ?>', '<?php echo $this->topic->getType(); ?>');"><?php echo $this->translate("Unsubscribe") ?></a>
  ...
  <div class="sesforum_topic_options sesbasic_clearfix">
    <div class="sesforum_topic_options_top sesbasic_clearfix">
      <div class="sesforum_topic_options_left sesbasic_text_light">
        <?php if($viewer_id && $this->settings->getSetting('sesforum.rating', 1) && isset($this->ratings)) { ?>      
      ...
      <div class="sesforum_topic_options_right">
        <?php if($viewer_id && isset($this->likeButton)) { ?>
          ...
          <?php if ($canLike && !empty($viewer_id)): ?>
            <div>
              <a class="active sesbasic_icon_like sesbasic_button" id="<?php echo $this->topic->getType(); ?>_unlike_<?php echo $this->topic->getIdentity(); ?>" style ='display:<?php echo $isLike ? "inline-block" : "none" ?>' href = "javascript:void(0);" onclick = "sesforumLike('<?php echo $this->topic->getIdentity(); ?>', '<?php echo $this->topic->getType(); ?>');"><?php echo $this->translate("Unlike") ?></a>
              <a class="sesbasic_icon_like sesbasic_button"" id="<?php echo $this->topic->getType(); ?>_like_<?php echo $this->topic->getIdentity(); ?>" style ='display:<?php echo $isLike ? "none" : "inline-block" ?>' href = "javascript:void(0);" onclick = "sesforumLike('<?php echo $this->topic->getIdentity(); ?>', '<?php echo $this->topic->getType(); ?>');"><?php echo $this->translate("Like") ?></a>
              ...
              <ul class="_isicon">
                <?php if(($this->canEdit && $this->topic->user_id == $this->viewer()->getIdentity()) || $this->canEdit == 2): ?>
                  <?php if( !$this->topic->sticky ): ?>
                    <li><?php echo $this->htmlLink(array('action' => 'sticky', 'sticky' => '1', 'reset' => false), $this->translate('Make Sticky'), array('class' => 'sesforum_icon_post_stick')) ?></li>
                  <?php else: ?>
                    <li><?php echo $this->htmlLink(array('action' => 'sticky', 'sticky' => '0', 'reset' => false), $this->translate('Remove Sticky'), array('class' => 'sesforum_icon_post_unstick')) ?></li>
                  <?php endif; ?>
                  <?php if( !$this->topic->closed ): ?>
                    <li><?php echo $this->htmlLink(array('action' => 'close', 'close' => '1', 'reset' => false), $this->translate('Close'), array('class' => 'sesforum_icon_post_close')) ?></li>
                  <?php else: ?>
                    <li><?php echo $this->htmlLink(array('action' => 'close', 'close' => '0', 'reset' => false), $this->translate('Open'), array('class' => 'sesforum_icon_post_unclose')) ?></li>
                  <?php endif; ?>
                    <li><?php echo $this->htmlLink(array('action' => 'rename', 'reset' => false), $this->translate('Rename'), array('class' => 'smoothbox sesforum_icon_post_rename')) ?></li>
                    <li><?php echo $this->htmlLink(array('action' => 'move', 'reset' => false), $this->translate('Move'), array('class' => 'smoothbox sesforum_icon_post_move')) ?></li>
                <?php endif; ?>
      ...
      <?php $valueTags = Engine_Api::_()->sesforum()->tagCloudItemCore('fetchAll',null,$this->topic->getIdentity()); ?>
      <?php if($this->tags && count($valueTags) >0): ?>
      <div class="sesforum_topic_view_tags sesbasic_tags">   
        <?php foreach($valueTags as $valueTag): ?>
            <a href="<?php echo $this->url(array('module' => 'sesforum', 'controller' => 'index', 'action' => 'search'), 'default', true).'?tag_id='.$valueTag['tag_id'].'&tag_name='.$valueTag['text']  ;?>" ><?php echo $valueTag['text'] ?></a>
        <?php endforeach; ?>
      </div>
      <?php endif; ?>
    </div> 
<!--    <div class="">
  ...
  <?php if( $this->topic->closed ): ?>
    <div class="sesforum_topic_close_msg">
      <i class="fa fa-lock"></i>
      <span><?php echo $this->translate('This topic has been closed.');?></span>
    </div>
  <?php endif; ?>
<!--  <div class="sesforum_topic_pages">
  ...
  <script type="text/javascript">
    en4.core.runonce.add(function() {
      $$('.sesforum_topic_posts_info_body').enableLinks();
  
      // Scroll to the selected post
      var post_id = <?php echo sprintf('%d', $this->post_id) ?>;
      if( post_id > 0 ) {
        window.scrollTo(0, $('sesforum_post_' + post_id).getPosition().y);
      }
    });
  </script>
  
  <ul class="sesforum_topic_view_posts">
    <?php foreach( $this->paginator as $i => $post ): ?>
      <?php $user = $this->user($post->user_id); ?>
      <?php $signature = $post->getSignature(); ?>
      <?php $isModeratorPost = $this->sesforum->isModerator($post->getOwner()) ?>
      <li id="sesforum_post_<?php echo $post->post_id ?>" class="sesforum_topic_view_post_item sesforum_nth_<?php echo $i % 2 ?><?php if( $isModeratorPost ): ?> sesforum_moderator_post<?php endif ?>">
        <div class="sesforum_topic_view_post_author_photo">
          <?php echo $this->htmlLink($user->getHref(), $this->itemPhoto($user, 'thumb.icon')) ?>
          
          <div class="sesforum_topic_view_post_author_stats sesbasic_text_light">
            <?php if($this->settings->getSetting('sesforum.thanks', 1) && isset($this->thanksCount)) { ?>
              <?php $thanks = Engine_Api::_()->getDbTable('thanks', 'sesforum')->getAllUserThanks($post->user_id); ?>
              <?php if($thanks) { ?>
                <span title='<?php echo $this->translate("Thanks")?>'><i class="sesforum_icon_thanks"></i><span><?php echo $this->translate(array('%s thanks', '%s thanks', $thanks), $this->locale()->toNumber($thanks)); ?></span></span>
              <?php } ?>
            <?php } ?>
            <?php if($this->settings->getSetting('sesforum.reputation', 1) && isset($this->reputationCount)) { ?>
              <?php $getIncreaseReputation = Engine_Api::_()->getDbTable('reputations', 'sesforum')->getIncreaseReputation(array('user_id'=>$post->user_id)); ?>
              <?php $getDecreaseReputation = Engine_Api::_()->getDbTable('reputations', 'sesforum')->getDecreaseReputation(array('user_id'=>$post->user_id)); ?>
              <span title='<?php echo $this->translate("Reputation")?>'><i class="sesforum_icon_reputation"></i><span><?php echo $getIncreaseReputation; ?>&nbsp;-&nbsp;<?php echo $getDecreaseReputation; ?></span></span>
            <?php } ?>
            <?php if($signature && isset($this->postsCount)): ?>
              <span><i class="sesforum_icon_post"></i><span><?php echo $signature->post_count; ?> <?php echo $this->translate('posts');?></span></span>
            <?php endif; ?>
          </div>
        </div>
        <div class="sesforum_topic_view_post_info">
          <div class="sesforum_topic_view_post_header sesbasic_clearfix sesbasic_text_light">
            <span class="sesforum_author_name">
              <?php echo $user->__toString(); ?>
              <?php if( $post->user_id != 0 ): ?>
                <?php if( $post->getOwner() ): ?>
                  <?php if( $isModeratorPost ): ?>
                    <span class="_label"><?php echo $this->translate('Moderator') ?></span>
                  <?php endif; ?>
                <?php endif; ?>
              <?php endif; ?>
            </span>
            <span class="sesforum_post_date sesbasic_text_light">
              <a href="<?php echo $post->getHref() ?>"><i class="sesforum_icon_post"></i></a>
          ...
          <div class="sesforum_topic_view_post_body sesbasic_html_block">
            <?php
              $body = $post->body; 
              $doNl2br = false;
            ...
            <?php if( $post->edit_id && !empty($post->modified_date) ):?>
              <div class="sesforum_topic_view_post_msg sesbasic_lbg">
          ...
          <?php endif;?>
          <div class="sesforum_topic_view_post_footer sesbasic_clearfix">
            <?php if( $this->canPost && isset($this->quote) && !$this->topic->closed): ?>
              <div><?php echo $this->htmlLink(array('route' => 'sesforum_topic', 'action' => 'post-create', 'topic_id'=>$this->subject()->getIdentity(), 'quote_id'=>$post->getIdentity()), $this->translate('Quote'), array('class' => 'sesforum_icon_post_quote')) ?></div>
            <?php endif;?>
            <?php if($viewer_id) { ?>
            ...
            <?php } ?>
            <?php $isReputation = Engine_Api::_()->getDbTable('reputations', 'sesforum')->isReputation(array('post_id' => $post->getIdentity(), 'resource_id' => $post->user_id)); ?>
            <?php if($this->settings->getSetting('sesforum.reputation', 1) && empty($isReputation) && $viewer_id != $post->user_id) { ?>
              <div><?php echo $this->htmlLink(array('module'=> 'sesforum','controller' => 'index', 'action' => 'add-reputation', 'route' => 'default','resource_id' => $post->user_id,'resource_type' =>$post->getType(),'post_id' => $post->getIdentity(),'format' => 'smoothbox'),$this->translate("Add Reputation"), array('class' => 'smoothbox sesforum_icon_reputation')); ?></div>
            <?php } ?>
            <?php } ?>
            <!--Like Button-->
            <?php  
            if(isset($this->likeButton)) {
              ...
              <?php if ($canLike && !empty($viewer_id)): ?>
                  <div>
                  <a class="sesbasic_icon_like active" id="<?php echo $post->getType(); ?>_unlike_<?php echo $post->getIdentity(); ?>" style ='display:<?php echo $isLike ? "inline-block" : "none" ?>' href = "javascript:void(0);" onclick = "sesforumLike('<?php echo $post->getIdentity(); ?>', '<?php echo $post->getType(); ?>');"><?php echo $this->translate("Unlike") ?></a>
                  <a class="sesbasic_icon_like" id="<?php echo $post->getType(); ?>_like_<?php echo $post->getIdentity(); ?>" style ='display:<?php echo $isLike ? "none" : "inline-block" ?>' href = "javascript:void(0);" onclick = "sesforumLike('<?php echo $post->getIdentity(); ?>', '<?php echo $post->getType(); ?>');"><?php echo $this->translate("Like") ?></a>
                  <input type="hidden" id="<?php echo $post->getType(); ?>_likehidden_<?php echo $post->getIdentity(); ?>" value='<?php echo $isLike ? $isLike : 0; ?>' />
                  </div>
              <?php endif; ?>
            <?php } ?>
            <!--Thanks Button-->
            <?php if($this->settings->getSetting('sesforum.thanks', 1)) { ?>
             <?php
            ...
            <div>
              <a class="sesforum_icon_thanks" id="<?php echo $post->getType(); ?>_thank_<?php echo $post->post_id; ?>" style ='display:<?php echo $isThank ? "none" : "inline-block" ?>' href = "javascript:void(0);" onclick = "sesTopicThank('<?php echo $post->post_id; ?>', '<?php echo $post->getType(); ?>', '<?php echo $post->user_id; ?>');"><?php echo $this->translate("Say Thanks") ?></a>
                ...
                <ul class="_isicon">
                  ...
                  <?php if(!$post->isOwner($this->viewer)): ?>
                   <?php if($this->canEdit_Post == 2): ?>
                      <li><a href="<?php echo $this->url(array('post_id'=>$post->getIdentity(), 'action'=>'edit'), 'sesforum_post'); ?>" class="sesbasic_icon_edit"><?php echo $this->translate('Edit');?></a></li>
                   <?php endif; ?>
                    <?php if($this->canDelete_Post == 2): ?>
                      <li><a href="<?php echo $this->url(array('post_id'=>$post->getIdentity(), 'action'=>'delete'), 'sesforum_post');?>" class="smoothbox sesbasic_icon_delete"><?php echo $this->translate('Delete');?></a></li>
                   <?php endif; ?>
  ...
  <?php if(($this->canPost) && $this->form && !$this->topic->closed): ?>
    <div class="sesforum_topic_view_reply">
      <div class="sesforum_topic_view_reply_author_photo sesbasic_clearfix">
        <?php echo $this->htmlLink($viewer->getHref(), $this->itemPhoto($viewer, 'thumb.icon')) ?>
      </div>
      <div class="sesforum_topic_view_reply_form_box">
        <?php echo $this->form->setAttrib('class', 'sesforum_topic_view_reply_form')->render($this) ?>
      </div>
    </div>
  <?php endif; ?>
</div>
