 ...
 * @version    $Id: Video.php  2018-10-16 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesbusinessvideo_Form_Video extends Engine_Form {
  ...
  public function init() {
    ...
    $setting = Engine_Api::_()->getApi('settings', 'core');
		...
		$allowedUploadOption = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('businessvideo', $viewer, 'sesvdeo_upld');
    ...
    //My Computer
    if ($myComputer) {
      $allowed_upload = 1;//Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('businessvideo', $user, 'upload');
    ...
    $this->addElement('Text', 'title', array(
		...
		if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinessvideo_enable_location', 1)){
    ...
    $viewOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('businessvideo', $user, 'auth_view');
    ...
    // Comment
    $commentOptions = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('businessvideo', $user, 'auth_comment');
    ...
    if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('sesprofilelock')) && Engine_Api::_()->authorization()->getPermission($viewer, 'businessvideo', 'video_locked')) {
    ...
    $uploadoption = $settings->getSetting('sesbusinessvideo.uploadphoto', '0');
    if ($uploadoption == 1) {
      if (isset($video) && $video->photo_id) {
        ...
        if (isset($path) && !empty($path)) {
          $this->addElement('Image', 'cover_photo_preview sesbd', array(
              'src' => $path,
              'class' => 'sesbusinessvideo_channel_thumb_preview sesbd',
          ));
