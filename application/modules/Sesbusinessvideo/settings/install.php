...
class Sesbusinessvideo_Installer extends Engine_Package_Installer_Module {
  ...
  public function onEnable() {
    $db = $this->getDb();
    $db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '1' WHERE `engine4_core_menuitems`.`name` = 'sesbusiness_admin_main_sesbusinessvideo';");
    parent::onEnable();
  }

  public function onDisable() {
    $db = $this->getDb();
    $db->query("UPDATE `engine4_core_menuitems` SET `enabled` = '0' WHERE `engine4_core_menuitems`.`name` = 'sesbusiness_admin_main_sesbusinessvideo';");
    parent::onDisable();
  }
}
