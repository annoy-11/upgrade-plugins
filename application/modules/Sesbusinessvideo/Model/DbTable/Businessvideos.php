 ...
 * @version    $Id: Businessvideos.php  2018-10-16 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesbusinessvideo_Model_DbTable_Businessvideos extends Engine_Db_Table {

  protected $_rowClass = "Sesbusinessvideo_Model_Businessvideo";
  protected $_name = 'sesbusinessvideo_videos';
  ...
  public function getWatchLaterStatus($video_id) {
    ...
    $watchLaterTable = Engine_Api::_()->getDbTable('watchlaters', 'sesbusinessvideo')->info('name');
  ...
  public function getVideo($params = array(), $paginator = true) {
    ...
    $select = $select->joinLeft($businessTable, '(' . $businessTable . '.business_id = ' . $tableName . '.parent_id )', array('business_id'));
		$select->where($businessTable.'.business_id !=?','');
		if(!empty($params['search_business']))
			$select->where($businessTable . ".title LIKE ?", $params['search_business'] . '%');
    ...
    if (!empty($params['parent_id']) && $params['parent_type'] == 'businesses') {
  ...
  public function peopleAlsoLiked($id = 0) {
    ...
    $select = $this->select()
            ...
            ->where($likesTableName . '.resource_type = ?', 'businessvideo')
            ->where('l2.resource_type = ?', 'businessvideo')
