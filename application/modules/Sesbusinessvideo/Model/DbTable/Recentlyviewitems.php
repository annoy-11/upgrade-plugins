...
class Sesbusinessvideo_Model_DbTable_Recentlyviewitems extends Engine_Db_Table {
  protected $_name = 'sesbusinessvideo_recentlyviewitems';
  protected $_rowClass = 'Sesbusinessvideo_Model_Recentlyviewitem';
  public function getitem($params = array()) {

      $itemTable = Engine_Api::_()->getItemTable('businessvideo');
    ...
    $select = $this->select()
            ...
            ->from(array('engine4_sesbusinessvideo_recentlyviewitems' => $subquery))
            ->where($this->info('name') .'.resource_type = ?', $params['type'])
            ->order('engine4_sesbusinessvideo_recentlyviewitems.creation_date DESC')
