...
var autoPlayId,firstVideoSrc=false,isOnRegister = false,IsfinishVideoNext = false,clearIntvalSesbusinessVideoSlideshow1,clearIntvalSesbusinessVideoSlideshow2 ;
jQuery(document).ready(function($){
		...
		sliderNav.on('click', 'li', function(event){
			...
			if(!selectedItem.hasClass('selected')) {
				...
				if(slidesWrapper.find('li.selected').hasClass('cd-bg-video')){
					 ...
					 clearInterval(clearIntvalSesbusinessVideoSlideshow1);
					 clearInterval(clearIntvalSesbusinessVideoSlideshow1);
				}
			}
		});
	...
	function checkVideo(hiddenSlide, container, n) {
		...
		if( visibleVideo.length > 0 ) { 
			clearInterval(autoPlayId);
			clearInterval(clearIntvalSesbusinessVideoSlideshow2);
			clearInterval(clearIntvalSesbusinessVideoSlideshow1);
			if(!Modernizr.touch){
