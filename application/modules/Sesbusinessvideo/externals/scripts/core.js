...
var marker;
var mapLoad = true;
function initializeSesBusinessVideoMap() {
  var mapOptions = {
  ...
  google.maps.event.addDomListener(window, 'load', initializeSesBusinessVideoMap);
}
function editMarkerOnMapSesbusinessVideoEdit(){
...
function editSetMarkerOnMapSesbusinessVideo(){
	geocoder = new google.maps.Geocoder();
...
//list page map
function initializeSesbusinessVideoMapList () {
  ...
  if(mapLoad){
	...
	if(mapLoad){
		google.maps.event.addDomListener(window, 'load', initializeSesbusinessVideoMapList);
	}
}

function editSetMarkerOnMapListSesbusinessVideo() {
...
sesJqueryObject(document).on('click','#openBusinessVideoInLightbox',function(){
	var getImageHref = window.location.href;
	videoURLsesbasic = videoURLsesbusinessvideo;
	moduleName = 'sesbusinessvideo';
	...
	var image = sesJqueryObject('#sesbusinessvideo_image_video_url').attr('data-src');
...
function checkFunctionEmbed(){
	if( typeof flashembed == 'function'){
		//silence
		 ...
		 if(sesJqueryObject('.sesbusinessvideo_attachment_info').length){
				var href = sesJqueryObject('.sesbusinessvideo_attachment_info').find('a').attr('href');
...
sesJqueryObject(document).on('click','.sesbusinessvideo_list_option_toggle',function(){
...
sesJqueryObject(document).on('click','.sesbusinessvideo_watch_later',function(){
		var that = this;
		...
		if(sesJqueryObject(this).hasClass('selectedWatchlater')){
		 ...
		 (new Request.HTML({
      method: 'post',
      'url':  en4.core.baseUrl + 'sesbusinessvideo/watchlater/add/',
      'data': {
        format: 'html',
...
function sesbusinessvideo_like_favourite_data(element,functionName,itemType,likeNoti,unLikeNoti,className){
		...
		if(sesJqueryObject(element).hasClass('button_active')){
		 ...
		 (new Request.HTML({
      method: 'post',
      'url':  en4.core.baseUrl + 'sesbusinessvideo/index/'+functionName,
      'data': {
        format: 'html',
...
sesJqueryObject(document).on('click','.sesbusinessvideo_favourite_businessvideo',function(){
	sesbusinessvideo_like_favourite_data(this,'favourite','businessvideo','<i class="fa fa-heart"></i><span>'+(en4.core.language.translate("Video added as Favourite successfully"))+'</span>','<i class="fa fa-heart"></i><span>'+(en4.core.language.translate("Video Unfavourited successfully"))+'</span>','sesbasic_favourites_notification');
});
sesJqueryObject(document).on('click','.sesbusinessvideo_like_businessvideo',function(){
	sesbusinessvideo_like_favourite_data(this,'like','businessvideo','<i class="fa fa-thumbs-up"></i><span>'+(en4.core.language.translate("Video Liked successfully"))+'</span>','<i class="fa fa-thumbs-up"></i><span>'+(en4.core.language.translate("Video Unliked successfully"))+'</span>','sesbasic_liked_notification');
});
sesJqueryObject(document).on("click", '.sesbusinessvideo_thumb_img', function (e) {
