 ...
 */
class Sesbusinessvideo_Widget_ProfileVideosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($_POST['is_ajax'])) {
      ...
      if (!$subject->authorization()->isAllowed($viewer, 'view'))
        return $this->setNoRender();
			...
			$subject = Engine_Api::_()->getItem('businesses', $params['parent_id']);
		}

		$viewer = Engine_Api::_()->user()->getViewer();

	  //Privacy Check
    $this->view->allowVideo  = Engine_Api::_()->authorization()->isAllowed('businesses', $viewer, 'video');

		$this->view->canUpload = $canUpload = $subject->authorization()->isAllowed($viewer, 'video');

		if($viewer->getIdentity()) {
      $this->view->can_edit = Engine_Api::_()->authorization()->getPermission($viewer, 'businessvideo', 'edit');
      $this->view->can_delete = Engine_Api::_()->authorization()->getPermission($viewer, 'businessvideo', 'delete');
		}
    ...
    $paginator = Engine_Api::_()->getDbTable('videos', 'sesbusinessvideo')->getVideo(array('parent_id' => $parent_id, 'parent_type' => $parent_type, 'criteria' => 10, 'text' => $search, 'sort' => $sort));
