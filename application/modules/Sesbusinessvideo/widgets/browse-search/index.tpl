...
<?php if($this->search_for == 'video'){ ?>
  <script type="application/javascript">var Searchurl = "<?php echo $this->url(array('module' =>'sesbusinessvideo','controller' => 'index', 'action' => 'get-video'),'default',true); ?>";</script>
  <?php if($controllerName == 'index' && $actionName != 'locations'){ ?>

    <?php if($actionName == 'browse'): ?>
      <?php $identity = Engine_Api::_()->sesbusiness()->getIdentityWidget('sesbusinessvideo.browse-video','widget','sesbusinessvideo_index_browse'); ?>
    <?php endif; ?>
    ...
    <?php if(@$identity) { ?>
      ...
      sesJqueryObject(document).ready(function(){
        sesJqueryObject('#filter_form').submit(function(e){
          e.preventDefault();
          if(sesJqueryObject('.sesbusinessvideo_video_listing').length > 0){
            ...
            sesJqueryObject('#loadingimgsesbusinessvideo-wrapper').show();
...
if(sesJqueryObject('#lat-wrapper').length > 0){
  ...
  initializeSesBusinessVideoMapList();
}
});
