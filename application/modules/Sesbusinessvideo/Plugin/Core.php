...
class Sesbusinessvideo_Plugin_Core {
  public function onStatistics($event) {
    $table = Engine_Api::_()->getDbTable('videos', 'sesbusinessvideo');
    $select = new Zend_Db_Select($table->getAdapter());
    $select->from($table->info('name'), 'COUNT(*) AS count');
    $event->addResponse($select->query()->fetchColumn(0), 'businessvideo');
  }
...
EOF;
    $script .=
						"var videoURLsesbusinessvideo = '" . Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinessvideo.video.manifest', 'businessvideo') . "';
