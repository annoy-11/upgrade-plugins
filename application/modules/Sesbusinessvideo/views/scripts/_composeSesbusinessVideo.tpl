 ...
 * @version    $Id: _composeSesbusinessVideo.tpl  2018-10-16 00:00:00 SocialEngineSolutions $
...
//        if(!Engine_Api::_()->authorization()->isAllowed('businesses', $viewer, 'video') || !$subject->authorization()->isAllowed(null, 'video') )
//        		return;
?>
<?php
   $this->headScript()
      ->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sesbusinessvideo/externals/scripts/composer_video.js');

  $allowed = 0;
  $user = Engine_Api::_()->user()->getViewer();
  $is_allowed_option = (array) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('businessvideo', $user, 'sesvdeo_upld');
...
// 	$dailymotion = (bool) in_array('dailymotion',$is_allowed_option) ? 1 : 0;
  ...
  $allowed_upload = 1;//(bool) Engine_Api::_()->authorization()->getAdapter('levels')->getAllowed('sesbusinessvideo', $user, 'upload');
  $ffmpeg_path = (bool) Engine_Api::_()->getApi('settings', 'core')->sesbusinessvideo_ffmpeg_path;
  $youtubeEnabled = (bool) Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinessvideo.youtube.apikey');
...
<script type="text/javascript">
  en4.core.runonce.add(function() {
    ...
    composeInstance.addPlugin(new Composer.Plugin.Sesbusinessvideo({
      ...
      lang : {
        ...
        'To upload a video from your computer, please use our full uploader.': '<?php echo $this->string()->escapeJavascript($this->translate('To upload a video from your computer, please use our <a href="%1$s">full uploader</a>.', $this->url(array('action' => 'create', 'type'=>3,'parent_id'=>$subject->getIdentity()), 'sesbusinessvideo_general'))) ?>'
      },
      ...
      requestOptions : {
        'url' : en4.core.baseUrl + 'sesbusinessvideo/index/compose-upload/format/json/c_type/'+type
      }
    }));
  });
</script>
