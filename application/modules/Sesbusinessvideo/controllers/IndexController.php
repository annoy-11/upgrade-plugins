...
class Sesbusinessvideo_IndexController extends Core_Controller_Action_Standard {
  ...
  public function searchAction() {
    ...
    $sesbusinessvideo_commonsearch = $this->_getParam('sesbusinessvideo_commonsearch', 'video');
    if ($sesbusinessvideo_commonsearch && $actonType == 'browse') {
      $type = $sesbusinessvideo_commonsearch;
    } else {
      if (isset($_COOKIE['sesbusinessvideo_commonsearch']))
        $type = $_COOKIE['sesbusinessvideo_commonsearch'];
      else
        $type = 'video';
    }
  ...
  public function editDetailAction() {
    ...
    if ($status && !$error) {
      ...
      if ($_POST['lat'] != '' && $_POST['lng'] != '') {
        ...
        $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $_POST['video_id'] . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","businessvideo")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
      }
  ...
  public function locationAction() {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->video = $video = Engine_Api::_()->getItem('businessvideo', $video_id);
    if (!$video)
      return;
    $this->view->form = $form = new Sesbusinessvideo_Form_Location();
    ...
    if (isset($_POST['lat']) && isset($_POST['lng']) && $_POST['lat'] != '' && $_POST['lng'] != '') {
      $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
      $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $video_id . '", "' . $_POST['lat'] . '","' . $_POST['lng'] . '","businessvideo")	ON DUPLICATE KEY UPDATE	lat = "' . $_POST['lat'] . '" , lng = "' . $_POST['lng'] . '"');
    }
    $db = $video->getTable()->getAdapter();
    $db->beginTransaction();
    try {
  ...
  public function imageviewerdetailAction() {
    ...
    $this->view->user = $user = Engine_Api::_()->getItem('user', $user_id);
		...
		if (!is_file($privateImageURL))
			$privateImageURL = 'application/modules/Sesbusinessvideo/externals/images/private-video.jpg';
    ...
    if ($viewer->getIdentity() != 0 && isset($video->video_id)) {
      ...
      $dbObject->query('INSERT INTO engine4_sesbusinessvideo_recentlyviewitems (resource_id, resource_type,owner_id,creation_date ) VALUES ("' . $video->video_id . '", "businessvideo","' . $viewer->getIdentity() . '",NOW())	ON DUPLICATE KEY UPDATE	creation_date = NOW()');
    }
		...
		$this->view->customParamsArray = $customParamsArray;
		 // get next video URL
    $this->view->nextVideo = Engine_Api::_()->getDbTable('videos', 'sesbusinessvideo')->videoLightBox($video, '>','','',$type,$this->_getParam('item_id',''));
    // get previous video URL
    $this->view->previousVideo = Engine_Api::_()->getDbTable('videos', 'sesbusinessvideo')->videoLightBox($video, '<','','',$type,$this->_getParam('item_id',''));
    ...
    if (Engine_Api::_()->getApi('core', 'sesbasic')->isModuleEnable(array('seslock'))) {
      ...
      if ($viewer->getIdentity() == 0)
        ...
        $level = $viewer;
      if (!Engine_Api::_()->authorization()->getPermission($level, 'businessvideo', 'locked') && $video->is_locked) {
      ...
      $this->view->locked = false;
    // rating code
    $this->view->allowShowRating = $allowShowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinessvideo.ratevideo.show', 1);
    $this->view->allowRating = $allowRating = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinessvideo.video.rating', 1);
    ...
    if ($showRating != 0) {
      $this->view->canRate = $canRate = Engine_Api::_()->authorization()->isAllowed('businessvideo', $viewer, 'rating');
      $this->view->allowRateAgain = $allowRateAgain = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinessvideo.ratevideo.again', 1);
      $this->view->allowRateOwn = $allowRateOwn = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinessvideo.ratevideo.own', 1);
      ...
      $this->view->rating_type = $rating_type = 'businessvideo';
      $this->view->rating_count = $rating_count = Engine_Api::_()->getDbTable('ratings', 'sesbusinessvideo')->ratingCount($video->getIdentity(), $rating_type);
      $this->view->rated = $rated = Engine_Api::_()->getDbTable('ratings', 'sesbusinessvideo')->checkRated($video->getIdentity(), $viewer->getIdentity(), $rating_type);
  ...
  public function getVideoAction() {
    ...
    $value['search'] = 1;
    $videos = Engine_Api::_()->getDbtable('videos', 'sesbusinessvideo')->getVideo($value);
  ...
  public function allVideosAction() {
    ...
    //FETCH photos
		...
		$this->view->customParamsArray = $customParamsArray;
    $paginator = $this->view->allVideos = Engine_Api::_()->getDbTable('videos', 'sesbusinessvideo')->videoLightBox($video, '', true, true,$type,$this->_getParam('item_id',''));
...
//     }
  ...
  public function shareAction() {
    ...
    $typeItem = ucwords(str_replace(array('sesbusinessvideo_'), '', $attachment->getType()));
  ...
  public function importVideoFromYoutubePlaylist($playlistId = null, $leftvideos, $values, $form, $limitYoutubePlaylist, $googleApiKey,$approve) {
    ...
    $videoIds = array();
    $counter = 1;
    $key = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinessvideo.youtube.apikey');
  ...
  public function uploadYoutubePlaylistVideos($values, $form, $leftVideos, $data, $limitYoutubePlaylist,$approve) {
    ...
    $values['parent_type'] = $parent_type = 'businesses';
      ...
      foreach ($data['items'] as $videoId) {
        ...
        if ($valid_thumb && $thumbnail && $ext && $thumbnail_parsed && in_array($ext, array('jpg', 'jpeg', 'gif', 'png'))) {
          ...
          $video->save();
            ...
            $dbGetInsert = Engine_Db_Table::getDefaultAdapter();
            $dbGetInsert->query('INSERT INTO engine4_sesbasic_locations (resource_id, lat, lng , resource_type) VALUES ("' . $video->video_id . '", "' . $values['lat'] . '","' . $values['lng'] . '","businessvideo")	ON DUPLICATE KEY UPDATE	lat = "' . $values['lat'] . '" , lng = "' . $values['lng'] . '"');
          }
  ...
  public function createAction() {
    ...
    $this->view->quota = $quota = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'businessvideo', 'max');
    ...
    //Create form
    $this->view->form = $form = new Sesbusinessvideo_Form_Video();
    ...
    try {
      if ($approve) {
        ...
        if($parent_id && $parent_type) {
		      $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity($owner, $parentItem, 'businesses_editeventvideo');
	        if ($action != null) {
	          Engine_Api::_()->getDbtable('actions', 'activity')->attachActivity($action, $video);
	        }
        } else {
      ...
      throw $e;
		...
		$widgetId = Engine_Api::_()->sesbusiness()->getIdentityWidget('sesbusinessvideo.profile-videos','widget','sesbusiness_profile_index_'.$parentItem->businessestyle);
		if($widgetId)
  ...
  public function deleteAction() {
    ...
    $viewer = Engine_Api::_()->user()->getViewer();
    $video = Engine_Api::_()->getItem('businessvideo', $this->getRequest()->getParam('video_id'));
  ...
  public function editAction() {
    ...
    $this->view->form = $form = new Sesbusinessvideo_Form_Edit();
    ...
    return $this->_helper->redirector->gotoRoute(array('user_id' => $viewer->getIdentity(), 'video_id' => $video->getIdentity(), 'slug' => $video->getSlug()), 'sesbusinessvideo_view', true);
  }
  public function lastElementDataAction() {
		$this->view->type = $this->_getParam('type','businessvideo');
...
//     }
    ...
    $values['user_id'] = $viewer->getIdentity();
    $paginator = Engine_Api::_()->getApi('core', 'sesbusinessvideo')->getVideosPaginator($values);
    $quota = Engine_Api::_()->authorization()->getPermission($viewer->level_id, 'businessvideo', 'max');
...
//     $this->view->valid = $valid;
  ...
  public function checkYouTubePlaylist($code) {
    ...
    require_once 'Google/Service/YouTube.php';
    $client = new Google_Client();
    $client->setDeveloperKey(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinessvideo.youtube.apikey', 0));
    $youtube = new Google_Service_YouTube($client);
    $nextPageToken = '';
    $playlistItemsResponse = $youtube->playlistItems->listPlaylistItems('snippet', array(
        'playlistId' => $code,
  ...
  public function favouriteItemAction() {
    ...
    if ($item_type == 'video'){
      ...
      $paginator = Engine_Api::_()->getDbTable('videos', 'sesbusinessvideo')->getFavourite($param);
		}
    else
      $paginator = Engine_Api::_()->getDbTable('chanels', 'sesbusinessvideo')->getFavourite($param);
  ...
  public function likeItemAction() {
    ...
    $paginator = Engine_Api::_()->sesbusinessvideo()->likeItemCore($param);
    $this->view->item_id = $item_id;
    $this->view->paginator = $paginator;
    // Set item count per page and current page number
    $paginator->setItemCountPerPage(10);
