...
class Sesbusinessvideo_AdminSettingsController extends Core_Controller_Action_Admin {
  ...
  public function indexAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesbusinessvideo_admin_main', array(), 'sesbusinessvideo_admin_main_settings');


    // Check ffmpeg path for correctness
    if (function_exists('exec')) {
      $ffmpeg_path = Engine_Api::_()->getApi('settings', 'core')->sesbusinessvideo_ffmpeg_path;
      ...
      if ($ffmpeg_path != $ffmpeg_path_original) {
        Engine_Api::_()->getApi('settings', 'core')->sesbusinessvideo_ffmpeg_path = $ffmpeg_path;
      }
    }

    // Make form
    $this->view->form = $form = new Sesbusinessvideo_Form_Admin_Settings_Global();
    ...
    if ($this->getRequest()->isPost() && $form->isValid($this->getRequest()->getPost())) {
      ...
      if (!empty($values['sesbusinessvideo_youtube_apikey']) && $values['sesbusinessvideo_youtube_apikey'] != $oldYoutubeApikey) {
        $response = $this->verifyYotubeApiKey($values['sesbusinessvideo_youtube_apikey']);
      ...
      // Check ffmpeg path
      if (!empty($values['sesbusinessvideo_ffmpeg_path'])) {
        if (function_exists('exec')) {
          $ffmpeg_path = $values['sesbusinessvideo_ffmpeg_path'];
          $output = null;
          $return = null;
          exec($ffmpeg_path . ' -version', $output, $return);

          if ($return > 0 && $output != NULL) {
            $form->sesbusinessvideo_ffmpeg_path->addError('FFMPEG path is not valid or does not exist');
            $values['sesbusinessvideo_ffmpeg_path'] = '';
          }
        } else {
          $form->sesbusinessvideo_ffmpeg_path->addError('The exec() function is not available. The ffmpeg path has not been saved.');
          $values['sesbusinessvideo_ffmpeg_path'] = '';
        }
      }

      include_once APPLICATION_PATH . "/application/modules/Sesbusinessvideo/controllers/License.php";
      if (Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinessvideo.pluginactivated')) {
  ...
  public function levelAction() {
    ...
    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesbusinessvideo_admin_main', array(), 'sesbusinessvideo_admin_main_level');
    ...
    // Make form
    $this->view->form = $form = new Sesbusinessvideo_Form_Admin_Settings_Level(array(
    ...
    $form->populate($permissionsTable->getAllowed('businessvideo', $id, array_keys($form->getValues())));
    // Check post
    ...
    try {
      // Set permissions
      $permissionsTable->setAllowed('businessvideo', $id, $values);
      // Commit
      $db->commit();
    } catch (Exception $e) {
      $db->rollBack();
  ...
  public function utilityAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
            ->getNavigation('sesbusinessvideo_admin_main', array(), 'sesbusinessvideo_admin_main_utility');
    $ffmpeg_path = Engine_Api::_()->getApi('settings', 'core')->sesbusinessvideo_ffmpeg_path;
  ...
  //site statis for sesbusinessvideo plugin
  public function statisticAction() {

    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesbusiness_admin_main', array(), 'sesbusiness_admin_main_sesbusinessvideo');

    $this->view->subNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesbusinessvideo_admin_main', array(), 'sesbusinessvideo_admin_main_statistic');

    $videoTable = Engine_Api::_()->getDbtable('videos', 'sesbusinessvideo');
  ...
  public function manageWidgetizePageAction() {
    ...
    $this->view->subNavigation = $navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesbusinessvideo_admin_main', array(), 'sesbusinessvideo_admin_main_managebusinesses');

    $this->view->pagesArray = array('sesbusinessvideo_index_home','sesbusinessvideo_index_browse','sesbusinessvideo_index_create','sesbusinessvideo_index_edit','sesbusinessvideo_index_view');
  }
}
