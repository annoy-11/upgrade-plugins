...
class Sespwa_AdminSettingsController extends Core_Controller_Action_Admin {
  ...
  public function miniMenuIconsAction() {
    ...
    public function sinkPagesAction() {
        ...
        if(count($results) > 0) {
            $db->query('INSERT INTO engine4_sespwa_pages SELECT * FROM engine4_core_pages where page_id > '.$pageId.';');
            $db->query('INSERT INTO engine4_sespwa_content SELECT * FROM engine4_core_content where content_id > '.$contentId.';');
        }
