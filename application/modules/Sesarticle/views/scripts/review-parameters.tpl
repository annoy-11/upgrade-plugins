...
if($this->item->category_id){
	$reviewParameters = Engine_Api::_()->getDbtable('parameters', 'sesarticle')->getParameterResult(array('category_id'=>$this->item->category_id));
  if(count($reviewParameters)){
  foreach($reviewParameters as $value){
?>
<div class="form-wrapper sesarticle_form_review_star">
	<div class="form-label"><label><?php echo $this->translate($value['title']); ?></label></div>
  <div id="sesarticle_review_rating" class="sesbasic_rating_parameter sesarticle_rating_star_element" onmouseout="rating_out_review(<?php echo $value['parameter_id'] ?>);">
...
function ratingTextReview(rating){
  ...
  window.addEvent('domready', function() {
		var countExistsParam = sesJqueryObject('.sesarticle_review_values');
