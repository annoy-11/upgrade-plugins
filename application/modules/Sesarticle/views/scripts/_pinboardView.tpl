...
?>
<li class="sesarticle_pinboard_list sesbasic_bxs new_image_pinboard_<?php echo $randonNumber; ?>">
	<div class="sesarticle_pinboard_list_item sesbm <?php if((isset($this->my_articles) && $this->my_articles)){ ?>isoptions<?php } ?>">
		<div class="sesarticle_pinboard_list_item_top sesarticle_thumb sesbasic_clearfix">
			<div class="sesarticle_pinboard_list_item_thumb">
				<a href="<?php echo $item->getHref()?>" data-url = "<?php echo $item->getType() ?>" class="<?php echo $item->getType() != 'sesarticle_chanel' ? 'sesarticle_thumb_img' : 'sesarticle_thumb_nolightbox' ?>">
			...
			<?php if(isset($this->featuredLabelActive) || isset($this->sponsoredLabelActive) || isset($this->verifiedLabel)):?>
				<div class="sesarticle_pinboard_labels">
    ...
    <div class="sesarticle_pinboard_info_article sesbasic_clearfix">
      <div class="sesarticle_pinboard_list_item_cont sesbasic_clearfix">
        <?php if(isset($this->categoryActive)){ ?>
          <?php if($item->category_id != '' && intval($item->category_id) && !is_null($item->category_id)):?> 
            <?php $categoryItem = Engine_Api::_()->getItem('sesarticle_category', $item->category_id);?>
            <?php if($categoryItem):?>
              <div class="sesarticle_pinboard_categry sesarticle_list_stats sesbasic_text_light">
                <span>
        ...
        <?php if(isset($this->titleActive)):?>
          <div class="sesarticle_pinboard_title">
        ...
        <div class="sesarticle_pinboard_meta_article">
          <?php if(isset($this->byActive)):?>
						<div class="sesarticle_list_stats sesbasic_text_dark sesbasic_clearfix">
							<?php $owner = $item->getOwner(); ?>
							<?php echo $this->htmlLink($item->getOwner()->getParent(), $this->itemPhoto($item->getOwner()->getParent(), 'thumb.icon')); ?>
							<span class="sesarticle_pinboard_list_item_poster_info_title"><?php echo $this->translate('by ');?><?php echo $this->htmlLink($owner->getHref(),$owner->getTitle() ) ?></span>
						</div>
          <?php endif;?>
          <?php if(isset($this->creationDateActive)){ ?>
            <div class="sesarticle_list_stats sesbasic_text_dirk">
              <?php if($item->publish_date): ?>
                <span><i class="fa fa-clock-o"></i> <a href="<?php echo $this->url(array('action'=>'browse'),'sesarticle_general',true).'?date='.date('Y-m-d',strtotime($item->publish_date)); ?>"><b><?php echo date('M d',strtotime($item->publish_date));?></b></a></span>
              <?php else: ?>
                <span><i class="fa fa-clock-o"></i> <a href="<?php echo $this->url(array('action'=>'browse'),'sesarticle_general',true).'?date='.date('Y-m-d',strtotime($item->creation_date)); ?>"><b><?php echo date('M d',strtotime($item->creation_date));?></b></a></span>
              <?php endif; ?>
            </div>
          <?php } ?>
          <?php if(isset($this->locationActive) && isset($item->location) && $item->location && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticle_enable_location', 1)){ ?>
        ...
        <?php if(isset($this->descriptionpinboardActive)){ ?>
          <div class="sesarticle_pinboard_list_item_des clear">
            <?php echo $item->getDescription($this->description_truncation_pinboard);?>
          </div>
        <?php } ?>
        
        <div class="sesarticle_pinboard_list_item_btm sesbm sesbasic_clearfix">
          <div class="sesarticle_pinboard_list_item_btm_cont sesbasic_text_light sesbasic_clearfix">
            <?php $owner = $item->getOwner(); ?>
            <div class="sesarticle_pinboard_list_statics floatL sesbasic_text_dark">
          ...
          <?php if(isset($this->my_articles) && $this->my_articles ){ ?>
            <?php if($this->can_edit || $this->can_delete){ ?>
              <div class="sesarticle_listing_in_grid2_date sesbasic_text_light">
                <span class="sesarticle_list_option_toggle fa fa-ellipsis-v sesbasic_text_light"></span>
                <div class="sesarticle_listing_options_pulldown">
                  <?php if($this->can_edit){ 
                  echo $this->htmlLink(array('route' => 'sesarticle_specific','action' => 'edit','article_id' => $item->article_id), $this->translate('Edit Article')) ; 
                  } ?>
                  <?php if ($this->can_delete){
                  echo $this->htmlLink(array('route' => 'sesarticle_specific','action' => 'delete', 'article_id' => $item->article_id), $this->translate('Delete Article'), array('onclick' =>'opensmoothboxurl(this.href);return false;'));
                  } ?>
                </div>
              </div>
            <?php } ?>
          ...
          <?php if(isset($this->enableCommentPinboardActive)):?>
            <?php $itemType = '';?>
            <?php if(isset($item->article_id)):?> 
              <?php $item_id = $item->article_id;?>
              <?php $itemType = 'sesarticle';?>
            <?php endif; ?>
            <?php if(($itemType != '')): ?>
              <div class="sesarticle_pinboard_list_comments">
