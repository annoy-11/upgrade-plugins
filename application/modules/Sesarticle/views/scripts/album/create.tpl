...
<div class="clear sesarticle_order_view_top" style="margin:10px;">
      <a href="<?php echo $this->article->getHref(); ?>" class="buttonlink sesbasic_icon_back"><?php echo $this->translate("Back To Article"); ?></a>
    </div>
<div class="sesarticle_album_form"> <?php echo $this->form->render($this) ?> </div>
...
<script type="text/javascript">
  ...
  jqueryObjectOfSes('#sesarticle_create_form_tabs li a').on('click',function(){
...
function sendFileToServer(formData,status,file,isURL,i)
{
		...
		var url = '&isURL='+urlIs;
		var article_id = '<?php  echo $this->article_id;?>';
    var uploadURL =$('form-upload').action + '?ul=1'+url+'&article_id='+article_id; //Upload URL
...
jqueryObjectOfSes(document).on('click','.delete_image_upload',function(e){
    ...
    'format' : 'json',
    'url' : '<?php echo $this->url(Array('module' => 'sesarticle', 'controller' => 'album', 'action' => 'remove'), 'default') ?>',
    'data': {
      'photo_id' : photo_id
    },
...
<?php } ?>
  function editImage(photo_id) {
    var url = '<?php echo $this->url(Array('module' => 'sesarticle', 'controller' => 'album', 'action' => 'edit-photo'), 'default') ?>' + '/photo_id/'+ photo_id;
    Smoothbox.open(url);
  }
</script>