 ...
 * @version    $Id: ReviewController.php 2016-07-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesarticle_ReviewController extends Core_Controller_Action_Standard {
	...
	public function init() {
		...
		if($article_id = $this->_getParam('article_id')){
			$article = Engine_Api::_()->getItem('sesarticle',$article_id);
		}
		 
		//Get subject
		if (null !== ($review_id = $this->_getParam('review_id')) && null !== ($review = Engine_Api::_()->getItem('sesarticlereview', $review_id)) && $review instanceof Sesarticle_Model_Review && !Engine_Api::_()->core()->hasSubject()) {
          $article = $review->getParent();
          Engine_Api::_()->core()->setSubject($review);
		}
		if(isset($article) && $article && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesarticlepackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticlepackage.enable.package', 1)){
  ...
  public function createAction() {
		...
		$viewer = Engine_Api::_()->user()->getViewer();
		$this->view->item = $item = Engine_Api::_()->getItem('sesarticle', $this->getParam('article_id'));
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('sesarticlereview', 'create'))
		return $this->_forward('notfound', 'error', 'core');
		if(!$item)
		return $this->_forward('notfound', 'error', 'core');
		//check review exists
		$isReview = Engine_Api::_()->getDbtable('sesarticlereviews', 'sesarticle')->isReview(array('article_id' => $item->getIdentity()));
		if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticle.allow.owner', 1)){
			$allowedCreate = true;	
		}
		else{
    ...
    $this->view->form = $form = new Sesarticle_Form_Review_Create(array('defaultProfileId' => $defaultProfileId));
    ...
    $values['article_id'] = $item->getIdentity();
    $reviews_table = Engine_Api::_()->getDbtable('sesarticlereviews', 'sesarticle');
    ...
    try {
      ...
      $review->save();
		  $dbObject = Engine_Db_Table::getDefaultAdapter();
			...
			$table = Engine_Api::_()->getDbtable('parametervalues', 'sesarticle');
			...
			foreach($_POST as $key => $reviewC){
				...
				if(!is_numeric($key))
					continue;
				$parameter = Engine_Api::_()->getItem('sesarticle_parameter',$key);
			...
			if(isset($item->rating)){
				$item->rating = Engine_Api::_()->getDbtable('sesarticlereviews', 'sesarticle')->getRating($review->article_id);
				$item->save();
			}
      ...
      $customfieldform->saveValues();
			$review->save();
      $db->commit();
      return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'review_id' => $review->review_id, 'slug' => $review->getSlug()), 'sesarticlereview_view', true);
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    }
  ...
  public function editAction() {
    ...
    $review_id = $this->_getParam('review_id', null);
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('sesarticlereview', 'edit'))
    return $this->_forward('notfound', 'error', 'core');
		$this->view->item = $item = Engine_Api::_()->getItem('sesarticle', $subject->article_id);
		if (isset($item->category_id) && $item->category_id != 0)
		$this->view->category_id = $item->category_id;
		else
		$this->view->category_id = 0;
    ...
    if(!$review_id || !$subject)
	  return $this->_forward('notfound', 'error', 'core');
    $this->view->defaultProfileId = $defaultProfileId = Engine_Api::_()->getDbTable('reviewmetas', 'sesarticle')->profileFieldId();
    $this->view->form = $form = new Sesarticle_Form_Review_Edit(array('defaultProfileId' => $defaultProfileId));
    ...
    try {
      ...
      $subject->save();
			...
			if(isset($item->rating)){
				$item->rating = Engine_Api::_()->getDbtable('sesarticlereviews', 'sesarticle')->getRating($subject->article_id);
				$item->save();
			}
			 //Add fields
      $customfieldform = $form->getSubForm('fields');
      ...
      $customfieldform->saveValues();
			$subject->save();
      $db->commit();
      return $this->_helper->redirector->gotoRoute(array('action' => 'view', 'review_id' => $subject->review_id, 'slug' => $subject->getSlug()), 'sesarticlereview_view', true);
    } catch (Exception $e) {
      $db->rollBack();
      throw $e;
    }
  ...
  public function deleteAction() {
    $viewer = Engine_Api::_()->user()->getViewer();
    $review = Engine_Api::_()->getItem('sesarticlereview', $this->getRequest()->getParam('review_id'));
    $content_item = Engine_Api::_()->getItem('sesarticle', $review->article_id);
  ...
  public function viewAction() {
		...
		return $this->_forward('notfound', 'error', 'core');
		$review_id = $this->_getParam('review_id', null);
		if (!Engine_Api::_()->sesbasic()->getViewerPrivacy('sesarticlereview', 'view'))
		return $this->_forward('notfound', 'error', 'core');
  ...
  function likeAction() {
    ...
    $itemTable = Engine_Api::_()->getItemTable('sesarticlereview');
    $tableLike = Engine_Api::_()->getDbtable('likes', 'core');
    $tableMainLike = $tableLike->info('name');
    $select = $tableLike->select()
            ->from($tableMainLike)
            ->where('resource_type = ?', 'sesarticlereview')
    ...
    if (count($result) > 0) {
      ...
      try {
        ...
        $like->resource_type = 'sesarticlereview';
