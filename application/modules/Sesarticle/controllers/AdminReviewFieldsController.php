 ...
 * @version    $Id: AdminReviewFieldsController.php 2016-07-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */
 
class Sesarticle_AdminReviewFieldsController extends Fields_Controller_AdminAbstract {

  protected $_requireProfileType = true;
  protected $_fieldType = 'sesarticlereview';
  ...
  public function indexAction() {
    ...
    $this->view->subsubNavigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesarticle_admin_main_revwcat', array(), 'sesarticle_admin_main_review_subfields');
  ...
  public function fieldCreateAction() {
    ...
    if ($content_type == 'sesarticle_category') {
      $content_type = 'sesarticle';
    }
  ...
  public function typeCreateAction() {
    ...
    // Create form
    $this->view->form = $form = new Sesarticle_Form_Admin_Review_Type();
    if (!$this->getRequest()->isPost()) {
      return;
    }
    if (!$form->isValid($this->getRequest()->getPost())) {
    ...
    if ($form->getValue('duplicate') != 'null') {
      ...
      $field_meta_array = $db->select()
              ->from('engine4_sesarticlereview_fields_meta')
              ->where('field_id IN (' . implode(', ', $child_id_array) . ')')
              ->query()
              ->fetchAll();
      // Copy each row
      for ($c = 0; $c < $field_map_array_count; $c++) {
        $db->insert('engine4_sesarticlereview_fields_meta', array(
      ...
      for ($i = 0; $i < $map_count; $i++) {
        $db->insert('engine4_sesarticlereview_fields_maps', array(
