...
class Sesarticle_Model_DbTable_Parametervalues extends Engine_Db_Table {
  protected $_rowClass = 'Sesarticle_Model_Parametervalue';
	protected $_name  = 'sesarticle_review_parametervalues';
	public function getParameters($params = array()){
		$pTable = Engine_Api::_()->getDbtable('parameters', 'sesarticle');
  ...
  public function getRating($parameter_id) {
    $rating_sum = $this->select()
            ...
            ->fetchColumn(0)
    ;
    $total = $this->ratingCount($parameter_id, $resource_type = 'sesarticle_event');
    if ($total)
