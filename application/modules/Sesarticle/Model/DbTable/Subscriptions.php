...
class Sesarticle_Model_DbTable_Subscriptions extends Engine_Db_Table {

  public function sendNotifications(Sesarticle_Model_Article $sesarticle) {
  
    if( !empty($sesarticle->draft) || $sesarticle->owner_type != 'user' ) {
      return $this;
    }

    // Get sesarticle owner
    $owner = $sesarticle->getOwner('user');
    ...
    $identities = $this->select()
      ...
      ->where('user_id = ?', $sesarticle->owner_id)
      ->query()
    ...
    foreach( $users as $user ) {
      $notificationTable->addNotification($user, $owner, $sesarticle, 'sesarticle_subscribed_new');
    }
