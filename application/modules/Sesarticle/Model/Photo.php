...
class Sesarticle_Model_Photo extends Core_Model_Item_Collectible
{
  protected $_parent_type = 'sesarticle_album';

  protected $_owner_type = 'user';

  protected $_collection_type = 'sesarticle_album';
...
//   }
  ...
  public function setAlbumPhoto($photo,$isURL = false,$isUploadDirect = false) {
   ...
   public function getAlbum() {
    return Engine_Api::_()->getItem('sesarticle_album', $this->album_id);
  }
    //get next photo
  ...
  */
  public function getAlbumPhotoUrl($type = null) { 
    ...
    if( !$photo_id ) {
      return 'application/modules/Sesarticle/externals/images/nophoto_album_thumb_normal.pngc=direct';
    }
  ...
  {
      return Engine_Api::_()->getItem('sesarticle',$this->article_id);
        //return $this->getParent('sesarticle');
  }
  ...
  public function setPhoto($photo)
  {
    if( $photo instanceof Zend_Form_Element_File ) {
      ...
      throw new Sesarticle_Model_Exception('invalid argument passed to setPhoto');
    }
  ...
  protected function _postDelete()
  {
    ...
    try
    {
	  ...
	  $articleItem = Engine_Api::_()->getItem("sesarticle", $this->article_id);
      if( (int) $album->photo_id == (int) $this->getIdentity() ) {
		if($this->getNextCollectible()) 
		$album->photo_id = $this->getNextCollectible()->getIdentity();
		$album->save();
      }
	  $photoItem = Engine_Api::_()->getItem("sesarticle_photo", $articleItem->photo_id);
	  if(!$photoItem) {
		  $articleItem->photo_id = 0;
	      $articleItem->save();
	  }
    }
    catch( Exception $e )
