 ...
 */
class Sesarticle_Widget_ProfilePhotosController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->album_parent_id =  $id = isset($params['album_parent_id']) ? $params['album_parent_id'] : Zend_Controller_Front::getInstance()->getRequest()->getParam('article_id', null);
		
    $value['article_id'] = $this->view->article_id =  $article_id = $id = Engine_Api::_()->getDbtable('sesarticles', 'sesarticle')->getArticleId($id);
    $this->view->article  = $subject = Engine_Api::_()->getItem('sesarticle',$id);
    $value['identityForWidget'] = $this->view->identityForWidget = isset($_POST['identity']) ? $_POST['identity'] : '';
		$this->view->allow_create = true;
		if(!$value['is_ajax'] && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesarticlepackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticlepackage.enable.package', 1)){
    ...
    $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('albums', 'sesarticle')->getAlbumSelect($value);
