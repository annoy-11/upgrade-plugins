...
class Sesarticle_Widget_ProfileMusicalbumsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (empty($_POST['is_ajax'])) {
      ...
      $subject_article = $coreApi->getSubject('sesarticle');
      $this->view->article_id = $subject_article->article_id;

      $subject = $coreApi->getSubject();
			
			$this->view->allow_create = true;
			if(isset($value['is_ajax']) && !$value['is_ajax'] && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesarticlepackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticlepackage.enable.package', 1)){
			...
			$this->view->canCreate  = Engine_Api::_()->sesarticle()->isArticleAdmin($subject, 'music_create');
    }
    ...
    $this->view->article_id = $article_id = isset($params['article_id']) ? $params['article_id'] : $subject->article_id;
    ...
    $params = $this->view->params = array('height' => $defaultHeight, 'width' => $defaultWidth, 'limit_data' => $limit_data, 'albumPhotoOption' => 'album', 'openTab' => $defaultOpenTab, 'pagging' => $loadOptionData, 'show_criterias' => $show_criterias, "informationAlbum" => $informationAlbum, 'article_id' => $article_id);
    ...
    $select = $table->select()
            ...
            ->where($tableName . '.resource_id = ?', $article_id)
            ->where($tableName . '.resource_type = ?', 'sesarticle')
