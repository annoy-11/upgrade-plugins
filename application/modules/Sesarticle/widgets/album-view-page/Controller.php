 ...
 */
class Sesarticle_Widget_AlbumViewPageController extends Engine_Content_Widget_Abstract
{
...
// 	}
	if(Engine_Api::_()->core()->hasSubject()){
			...
			$album =  Engine_Api::_()->getItem('sesarticle_album', $_POST['album_id']);
			$article =  Engine_Api::_()->getItem('sesarticle', $_POST['article_id']);		
		}
		$this->view->allow_create = true;
		$this->view->article = $article;
		 $this->view->album = $album;
		 $this->view->album_id = $param['id'] = $album->album_id;
		 $this->view->article_id = $param['article_id'] = $album->article_id;
		if(!$is_ajax && Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('sesarticlepackage') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticlepackage.enable.package', 1)){
			$package = $article->getPackage();
			$viewAllowed = $package->getItemModule();
			if(!$viewAllowed)
				return $this->setNoRender();
			//allow upload photo
			$this->view->allow_create = $allow_create = $package->allowUploadPhoto($article);
		}
    $sesarticle_photos = Zend_Registry::isRegistered('sesarticle_photos') ? Zend_Registry::get('sesarticle_photos') : null;
	 ...
	 $photoTable = Engine_Api::_()->getItemTable('sesarticle_photo');
		...
		if($viewer->getIdentity() > 0){
			$this->view->canEdit = $viewPermission = $article->authorization()->isAllowed($viewer, 'edit');
			$this->view->canComment =  $article->authorization()->isAllowed($viewer, 'comment');
			$this->view->canCreateMemberLevelPermission =  Engine_Api::_()->authorization()->getPermission($viewer, 'sesarticle', 'create');
			
			$this->view->canEditMemberLevelPermission   =  Engine_Api::_()->authorization()->getPermission($viewer,'sesarticle', 'edit');
			$this->view->canDeleteMemberLevelPermission  = Engine_Api::_()->authorization()->getPermission($viewer,'sesarticle', 'delete');
		}
    if (empty($sesarticle_photos))
      return $this->setNoRender();
    if(!$is_ajax){
			// Prepare data
