...
<div class="slide sesbasic_clearfix sesbasic_bxs sesarticle_carousel_wrapper <?php echo $this->isfullwidth ? 'isfull_width' : '' ; ?>" style="height:<?php echo $this->height ?>px;display:none;" id="sesarticle_carousel_<?php echo $this->identity; ?>">
  <div class="articleslide_<?php echo $this->identity; ?>">
    <?php foreach( $this->paginator as $item): ?>
    <div class="sesarticle_grid_inside sesarticle_category_carousel_item sesbasic_clearfix sesarticle_grid_btns_wrap" style="height:<?php echo $this->height ?>px;width:<?php echo $this->width ?>px;">
    	<div class="sesarticle_grid_inside_inner">
      <div class="sesarticle_category_carousel_item_thumb sesarticle_thumb" style="height:<?php echo $this->height ?>px;">       
        <?php
        $href = $item->getHref();
		...
		<?php if((isset($this->socialSharingActive)  && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticle.enable.sharing', 1)) || isset($this->likeButtonActive) || isset($this->favouriteButtonActive)):?>
			...
			<?php endif;?> 
				...
				<div class="sesarticle_grid_inside_info sesbasic_clearfix ">
            ...
            <span class="sesarticle_category_carousel_item_info_title">
              ...
              <?php } ?>
							<?php if(Engine_Api::_()->getApi('core', 'sesarticle')->allowReviewRating() && isset($this->ratingStarActive)):?>
								<?php echo $this->partial('_articleRating.tpl', 'sesarticle', array('rating' => $item->rating, 'class' => 'sesarticle_list_rating sesarticle_list_view_ratting', 'style' => 'margin-bottom:0px; margin-top:5px;'));?>
							<?php endif;?>
            </span>
          <?php } ?>
      ...
      <?php if(isset($this->creationDateActive)){ ?>
				<div class="admin_teg sesarticle_list_stats sesbasic_text_dark"><i class="fa fa-calendar"></i> <?php echo ' '.date('M d, Y',strtotime($item->publish_date));?></div>
			<?php } ?>
      <div class="sesarticle_list_stats">
        ...
        <?php echo $item->favourite_count;?> </span>
			...
			<?php if(isset($this->viewActive) && isset($item->view_count)) { ?>
				<span title="<?php echo $this->translate(array('%s view', '%s views', $item->view_count), $this->locale()->toNumber($item->view_count))?>"><i class="fa fa-eye"></i><?php echo $item->view_count; ?>
			<?php } ?>
...
<script type="text/javascript">
  window.addEvent('domready', function () {
		<?php if($this->isfullwidth){ ?>
			...
			htmlElement.addClass('sesarticle_carousel');
		<?php } ?>
