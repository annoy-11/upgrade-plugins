 ...
 */
class Sesarticle_Widget_ManageArticlesController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    if (!$is_ajax && is_array($defaultOptionsArray)) {
      foreach ($defaultOptionsArray as $key => $defaultValue) {
				if(!Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticle.enable.favourite', 1) && $defaultValue == 'mostSPfavourite')
					continue;
    ...
    $sesarticle_tabbed = Zend_Registry::isRegistered('sesarticle_tabbed') ? Zend_Registry::get('sesarticle_tabbed') : null;
		//simple list view
    ...
    $this->view->widgetName = 'manage-articles';			
    $params = array_merge($params, $value, array('search'=>addslashes($text)));

    // Get Articles
    $paginator = Engine_Api::_()->getDbtable('sesarticles', 'sesarticle')->getSesarticlesPaginator($value);
