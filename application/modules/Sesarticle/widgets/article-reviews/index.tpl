...
<?php if($this->allowedCreate && $this->cancreate && $this->viewer()->getIdentity() && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticle.allow.review', 1) && !$this->isReview): ?>
  <div class="sesbasic_profile_tabs_top sesbasic_clearfix">
    <?php echo $this->htmlLink(array('route' => 'sesarticlereview_extended','action' => 'create', 'article_id'=>$this->subject()->getIdentity()), $this->translate('Write a Review'), array('class' => 'sesbasic_button fa fa-plus'));?>
  </div>
<?php endif;?>
<?php } ?>
<?php if( $this->paginator->getTotalItemCount() > 0 ): ?>
	<?php if(!$this->is_ajax){ ?>
		<div class="sesarticle_profile_reviews_filters sesbasic_bxs sesbasic_clearfix">
			<?php echo $this->content()->renderWidget('sesarticle.browse-review-search',array('review_search'=>1,'review_stars'=>1,'reviewRecommended'=>1,'review_title'=>0,'view_type'=>'horizontal','isWidget'=>true,'article_id'=>$this->subject->getIdentity(),'widgetIdentity'=>$this->identity)); ?>
		</div>
		<ul class="sesarticle_review_listing sesbasic_clearfix sesbasic_bxs" id="sesarticle_review_listing">
	<?php } ?>
  <?php foreach( $this->paginator as $item ): ?>
  <li class="sesarticle_owner_review sesbasic_clearfix">
    <div class="sesarticle_review_listing_top sesbasic_clearfix sesarticle_review_listing_left_column">
      <?php if(in_array('title', $this->stats)): ?>
      <div class='sesarticle_review_listing_title'>
      	<?php echo $this->htmlLink($item->getHref(), $item->getTitle()) ?>
      </div>
     <?php endif; ?>
      <div class="sesarticle_review_listing_top_info sesbasic_clearfix">      
      <?php if(in_array('postedBy', $this->stats)): ?>
        <div class='sesarticle_review_listing_top_info_img'>
          <?php echo $this->htmlLink($item->getOwner()->getHref(), $this->itemPhoto($item->getOwner(), 'thumb.icon')) ?>
        </div>
       <?php endif; ?>
        <div class='sesarticle_review_listing_top_info_cont'>
         <?php if(in_array('postedBy', $this->stats)): ?>
        	<p class="sesarticle_review_listing_stats sesbasic_text_light">by <?php echo $this->htmlLink($item->getOwner()->getHref(), $item->getOwner()->getTitle()) ?>
         <?php endif; ?>
          |
          ...
          <?php endif; ?>
          <p class="sesbasic_text_light sesarticle_review_listing_stats">
       ...
       <?php if(in_array('rating', $this->stats)): ?>
        <div class="sesarticle_review_show_rating review_ratings_listing">
    ...
    <div class="sesarticle_review_listing_desc sesbasic_clearfix">
      <?php if(in_array('pros', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticle.show.pros', 1)): ?>
        <p class="sesarticle_review_listing_body">
          <b><?php echo $this->translate("Pros: "); ?></b>
          <?php echo $this->string()->truncate($this->string()->stripTags($item->pros), 300) ?>
        </p>
      <?php endif; ?>
      <?php if(in_array('cons', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticle.show.cons', 1)): ?>
        <p class="sesarticle_review_listing_body">
      ...
      <p class="sesarticle_review_listing_more">
    ...
    <div class="sesarticle_review_listing_footer clear sesbasic_clearfix ">
			...
			<div class='sesarticle_review_listing_btn_right floatR '>
				<?php if($item->authorization()->isAllowed($this->viewer(), 'edit')) { ?>
					<a class="fa fa-pencil sesbasic_button sesbasic_button_icon " href="<?php echo $this->url(array('slug' => $item->getSlug(), 'action' => 'edit', 'review_id' => $item->getIdentity()), 'sesarticlereview_view', 'true');?>"><span><i class="fa fa-caret-down"></i><?php echo $this->translate('Edit Review');?></span></a>
				<?php } ?>
				<?php if($item->authorization()->isAllowed($this->viewer(), 'delete')) { ?>
					<a class="smoothbox fa fa-trash sesbasic_button sesbasic_button_icon" href="<?php echo $this->url(array('action' => 'delete', 'review_id' => $item->getIdentity()), 'sesarticlereview_view', true);?>"><span><i class="fa fa-caret-down"></i><?php echo $this->translate('Delete Review');?></span></a>
				<?php } ?>
				<?php if(Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticle.show.report', 1) && $this->viewer()->getIdentity() && in_array('report', $this->stats) && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesarticle.enable.report', 1)): ?>
...
<div class="sesbasic_tip clearfix">
    <img src="application/modules/Sesarticle/externals/images/reviews_icon.png" alt="">
    <span class="sesbasic_text_light">
      <?php echo $this->translate('No review have been posted in this article yet.');?>    </span>
  </div>
<?php endif; ?>
<?php if(!$this->is_ajax){ ?>
		...
		<?php if($this->loadOptionData == 'auto_load'){ ?>
			window.addEvent('load', function() {
				sesJqueryObject(window).scroll( function() {
					var containerId = '#sesarticle_review_listing';
	...
	<?php if($this->loadOptionData != 'pagging') { ?>
		...
		function viewMore_<?php echo $randonNumber; ?> () {
			...
			requestViewMore_<?php echo $randonNumber; ?> = new Request.HTML({
				method: 'post',
				'url': en4.core.baseUrl + "widget/index/mod/sesarticle/name/article-reviews",
				'data': {
					format: 'html',
				...
				onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
					sesJqueryObject('#sesarticle_review_listing').append(responseHTML);
					sesJqueryObject('.sesbasic_view_more_loading_<?php echo $randonNumber;?>').hide();
					sesJqueryObject('#loadingimgsesarticlereview-wrapper').hide();
					viewMoreHide_<?php echo $randonNumber; ?>();
				}
			});
	...
	<?php }else{ ?>
		function paggingNumber<?php echo $randonNumber; ?>(pageNum){
			...
			requestViewMore_<?php echo $randonNumber; ?> = (new Request.HTML({
				...
				onSuccess: function(responseTree, responseElements, responseHTML, responseJavaScript) {
					sesJqueryObject('#sesarticle_review_listing').html(responseHTML);
