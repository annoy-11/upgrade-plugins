...
class Sesarticle_Widget_ArticleReviewsController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $sesarticle_reviews = Zend_Registry::isRegistered('sesarticle_reviews') ? Zend_Registry::get('sesarticle_reviews') : null;
		...
		if (!$is_ajax) {
		  ...
		  return $this->setNoRender();
			...
			if (!Engine_Api::_()->authorization()->getPermission($viewer, 'sesarticlereview', 'view'))
			return $this->setNoRender();
    
			$this->view->isReview = Engine_Api::_()->getDbtable('sesarticlereviews', 'sesarticle')->isReview(array('article_id' => $subject->getIdentity()));
			$this->view->cancreate = Engine_Api::_()->authorization()->getPermission($viewer, 'sesarticlereview', 'create'); 
    }
    ...
    if (empty($sesarticle_reviews))
      return $this->setNoRender();
		...
		$this->view->article_id = $params['article_id'] = isset($_POST['article_id']) ? $_POST['article_id'] : $subject->getIdentity();
		$params['paginator'] = true;

    $this->view->paginator = $paginator = Engine_Api::_()->getDbTable('sesarticlereviews', 'sesarticle')->getArticleReviewSelect($params);
