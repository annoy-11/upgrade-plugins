...
class Sesarticle_Widget_BrowseReviewSearchController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->subject_id = $this->_getParam('article_id', 0);
    $this->view->widgetIdentity = $this->_getParam('widgetIdentity', 0);
    $reviewSearch = $this->_getParam('review_search', 1);
    $this->view->form = $formFilter = new Sesarticle_Form_Review_Browse(array('reviewTitle' => $this->_getParam('review_title', 1), 'reviewSearch' => $reviewSearch, 'reviewStars' => $this->_getParam('review_stars', 1), 'reviewRecommended' => $this->_getParam('review_recommendation', 1)));
   if($reviewSearch){
