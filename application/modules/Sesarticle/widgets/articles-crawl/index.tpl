...
<div class="sesarticle_ticker_container sesbasic_clearfix sesbasic_bxs">
	<div class="sesarticle_ticker_title">
		<?php echo $this->translate($this->title); ?>
  </div>
  <div class="sesarticle_ticker">
    <?php foreach( $this->paginatorRight as $item): ?>
	    <div class="sesarticle_ticker_item sesbasic_clearfix">
        <?php if($this->showCreationDate): ?>
        <div class="sesarticle_ticker_item_date sesbasic_text_light">
        	<?php echo ' '.date('M d, Y',strtotime($item->publish_date));?>
        </div>
        <?php endif; ?>
        <div class="sesarticle_ticker_item_title">
...
<?php if($this->enableSlideshow): ?>
  <script type="text/javascript">
    sesarticleJqueryObject(document).ready(function() {
      sesarticleJqueryObject('.sesarticle_ticker').owlCarousel({
        loop:true,
        items:1,
        margin:0,
        autoHeight:true,
