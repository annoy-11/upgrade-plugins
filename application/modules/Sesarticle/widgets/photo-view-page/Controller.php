 ...
 */
class Sesarticle_Widget_photoViewPageController extends Engine_Content_Widget_Abstract
{ 
  public function indexAction()
  {
		...
		if(Engine_Api::_()->core()->hasSubject('sesarticle_photo') && !$is_ajax)
	 	 $photo = Engine_Api::_()->core()->getSubject('sesarticle_photo');
		else if(isset($_POST['photo_id'])){
		 $photo = Engine_Api::_()->getItem('sesarticle_photo',$_POST['photo_id']);
		 Engine_Api::_()->core()->setSubject($photo); 
		 $photo = Engine_Api::_()->core()->getSubject();
		}else
			 return $this->setNoRender();
		
		$this->view->article =	$article = Engine_Api::_()->getItem('sesarticle', $photo->article_id);
	  ...
	  if($viewer->getIdentity()>0){
			$this->view->canEdit = $canEdit = $article->authorization()->isAllowed($viewer, 'edit');
			$this->view->canComment = $canComment = $article->authorization()->isAllowed($viewer, 'comment');
			$this->view->canDelete = $canDelete = $article->authorization()->isAllowed($viewer, 'delete');
			$this->view->canTag = $canTag = $article->authorization()->isAllowed($viewer, 'tag');
			$this->view->canCommentMemberLevelPermission = Engine_Api::_()->authorization()->getPermission($viewer, 'sesarticle', 'comment');
		}
