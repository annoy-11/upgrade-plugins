 ...
 */
class Sesarticle_Form_Custom_Fields extends Engine_Form {
  /* Custom */
  ...
  public function saveValues($params = '') {
    ...
    foreach ($fVals as $key => $value) {
      ...
      // Array mode
      if (is_array($value) || ($this->getElement($key) && $this->getElement($key)->isArray())) {
    ...
    if (!empty($this->_topLevelId))
      $selected[$this->_topLevelId] = $this->_topLevelValue;
		if(isset($_POST['category_id']) && $_POST['category_id'] != 0){
			$category_map_id_data	=  Engine_Api::_()->getDbtable('categories', 'sesarticle')->getMapId($_POST['category_id']);
		...
		if(isset($_POST['subcat_id']) && $_POST['subcat_id'] != 0){
			$subcategory_map_id_data	=  Engine_Api::_()->getDbtable('categories', 'sesarticle')->getSubCatMapId($_POST['subcat_id']);
		...
		if(isset($_POST['subsubcat_id'])  && $_POST['subsubcat_id'] != 0){
			$subsubcategory_map_id_data	=  Engine_Api::_()->getDbtable('categories', 'sesarticle')->getSubSubCatMapId($_POST['subsubcat_id']);
