...
class Sesarticle_Form_Review_Browse extends Engine_Form {
  ...
  public function init() {
    ...
    $article_id = Zend_Controller_Front::getInstance()->getRequest()->getParam('article_id', 0);
    $this->setAttribs(array('id' => 'filter_form_review', 'class' => 'global_form_box'))->setMethod('GET');
    if($article_id)
    $this->setAction(Zend_Controller_Front::getInstance()->getRouter()->assemble(array('article_id' => $article_id), 'sesarticle_entry_view', true));
    else
    $this->setAction(Zend_Controller_Front::getInstance()->getRouter()->assemble(array(), 'sesarticle_review', true));
    ...
    $this->addElement('Dummy', 'loading-img-sesarticle-review', array(
