 ...
 */
class Sesspectromedia_Api_Core extends Core_Api_Abstract {
  ...
  public function setPhoto($photo, $menuId = null) {
    ...
    $params = array(
        'parent_id' => $menuId,
        'parent_type' => "Sesspectromedia_images",
    );
  ...
  public function readWriteXML($keys, $value, $default_constants = null) {
    ...
    //For constant backup
    $filePath_sp = APPLICATION_PATH . "/application/modules/Sesspectromedia/externals/styles/sesspectromedia.xml";
    $results_sp = simplexml_load_file($filePath_sp);
    //For constant backup

    if (!empty($keys) && !empty($value) && ($keys != 'sm_body_background_image' || $keys != 'sm_footer_background_image')) {
      $contactsThemeArray = array($keys => $value);
    } elseif (!empty($keys) && ($keys == 'sm_body_background_image' || $keys == 'sm_footer_background_image')) {
      $contactsThemeArray = array($keys => '');
    } elseif ($default_constants) {
      $contactsThemeArray = $default_constants;
    }

    //For constant backup at file path: /application/modules/Sesspectromedia/externals/styles/sesspectromedia.xml
