 ...
 * @version    $Id: AdminCustomThemeController.php 2016-11-22 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesspectromedia_AdminCustomThemeController extends Core_Controller_Action_Admin {

  public function indexAction() {
    
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesspectromedia_admin_main', array(), 'sesspectromedia_admin_main_customcss');
    
    $writeable = array();
    $writeable['sesspectromedia'] = false;
    try {
      foreach(array('theme.css', 'constants.css', 'media-queries.css', 'sesspectromedia-custom.css') as $file) {
        if(!file_exists(APPLICATION_PATH . "/application/themes/sesspectromedia/$file")) {
          throw new Core_Model_Exception('Missing file');
        } else {
          $this->checkWriteable(APPLICATION_PATH . "/application/themes/sesspectromedia/$file");
        }
      }
      $writeable['sesspectromedia'] = true;
    } catch(Exception $e) {
      $this->view->errorMessage = $e->getMessage();
    }
    
    $this->view->writeable = $writeable;
    $this->view->activeFileContents = file_get_contents(APPLICATION_PATH . '/application/themes/sesspectromedia/sesspectromedia-custom.css');
  }

  public function saveAction() {
  
    $theme_id = 'sesspectromedia';
    ...
    //Check file
    $basePath = APPLICATION_PATH . '/application/themes/sesspectromedia';
