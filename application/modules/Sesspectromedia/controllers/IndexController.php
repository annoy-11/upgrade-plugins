...
class Sesspectromedia_IndexController extends Core_Controller_Action_Standard {
  ...
  public function inboxAction() {
    ...
    Engine_Api::_()->getApi('message', 'sesspectromedia')->setUnreadMessage($viewer);
  }
  ...
  public function newMessagesAction() {
    $this->view->messageCount = Engine_Api::_()->getApi('message', 'sesspectromedia')->getMessagesUnreadCount(Engine_Api::_()->user()->getViewer());
  }
  ...
  public function friendshipRequestsAction() {
    $viewer = Engine_Api::_()->user()->getViewer();
    $this->view->friendRequests = $newFriendRequests = Engine_Api::_()->getDbtable('notifications', 'sesspectromedia')->getFriendrequestPaginator($viewer);
    $newFriendRequests->setCurrentPageNumber($this->_getParam('page'));
    Engine_Api::_()->getApi('message', 'sesspectromedia')->setUnreadFriendRequest($viewer);
  }

  public function searchAction() {

    $text = $this->_getParam('text', null);
    if (isset($_COOKIE['sesspectromedia_commonsearch']))
      $type = $_COOKIE['sesspectromedia_commonsearch'];
    else
      $type = '';
