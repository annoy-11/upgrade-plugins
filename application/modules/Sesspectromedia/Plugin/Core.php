...
class Sesspectromedia_Plugin_Core extends Zend_Controller_Plugin_Abstract {

  public function routeShutdown(Zend_Controller_Request_Abstract $request) {
  
    if (substr($request->getPathInfo(), 1, 5) == "admin") {
      $params = $request->getParams();
      if($params['module'] == 'sesspectromedia' &&  $params['controller'] == "admin-menu") {
        $request->setModuleName('sesbasic');
        $request->setControllerName('admin-menu');
        $request->setActionName($params['action']);
        $request->setParam('moduleName', 'sesspectromedia');
      }
    }
  }
  ...
  public function onRenderLayoutDefault($event) {
    ...
    $usegoogleFont = $settings->getSetting('sesspectromedia.googlefonts', 0);
    ...
    $enable_responseive_leyout = Engine_Api::_()->sesspectromedia()->getContantValueXML('sm_responsive_layout');
    if ($theme_name == 'sesspectromedia' && $enable_responseive_leyout == '1') {
      include APPLICATION_PATH . '/application/modules/Sesspectromedia/views/scripts/responsive_layout.tpl';
