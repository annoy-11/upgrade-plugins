 ...
 * @version    $Id: AdminManageController.php 2018-08-14 00:00:00 socialnetworking.solutions $
...
class User_AdminManageController extends Core_Controller_Action_Admin
{
  public function indexAction()
  {
    ...
    $select = $table->select()->from($table->info('name'),array('*','pointSeven'=>new Zend_Db_Expr("(SELECT count(*) FROM ".$creditTableName.' WHERE '.$creditTableName.'.user_id = '.$table->info('name').'.user_id').' AND '.$creditTableName.'.creation_date >= DATE(NOW()) - INTERVAL 7 DAY)'));
    ...
    $startid = $this->_getParam('startid');
    $values = array_merge(array(
      'order' => 'SUM(pointSeven)',
    ...
    $select->setIntegrityCheck(false)->joinLeft($creditTableName,$creditTableName.'.user_id = '.$table->info('name').'.user_id',array('points'=>new Zend_Db_Expr("SUM(credit_point)")))
    ->group($table->info('name').'.user_id');
    ...
    $this->view->openUser = (bool) ( $this->_getParam('open') && $paginator->getTotalItemCount() == 1 );
      ...
      if(isset($_POST['submitNote'])) {
          Engine_Api::_()->getApi('settings', 'core')->setSetting('user.note.adminend', $_POST['note'] ? $_POST['note'] : '');

      }
    if(isset($_POST['submitStart'])){
        Engine_Api::_()->getApi('settings', 'core')->setSetting('user.export.adminend', $startid ? $startid : '');
        $select = Engine_Api::_()->getDbTable('users','user')->select()->order('user_id DESC');

        if($startid){
            $select->where('user_id >?',$startid);
        }

        $users = Engine_Api::_()->getDbTable('users','user')->fetchAll($select);
        ...
        if(!empty($users))
            foreach($users as $row) {
                $valueVal['ID'] = $row->user_id;
                $valueVal['Display Names'] = $row->displayname;
                $valueVal['Username'] = $row->username;
                $valueVal['Email'] = $row->email;
                $valueVal['SignUp Date'] = $row->creation_date;
                $valueVal['Last Login'] = $row->lastlogin_date;
                $counter++;
                if(!$heading) {
