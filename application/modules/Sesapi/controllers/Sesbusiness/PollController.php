 ...
 */
class Sesbusiness_PollController extends Sesapi_Controller_Action_Standard
{
    public function init(){
        $poll_id = $this->_getParam('poll_id');
        $poll = null;
        $poll = Engine_Api::_()->getItem('sesbusinesspoll_poll', $poll_id);
        if ($poll) {
            Engine_Api::_()->core()->setSubject($poll);
        }
    }
    public function browseAction(){
        $form = new Sesbusinesspoll_Form_Search();
        $form->populate($_POST);
        $params = $form->getValues();
        if ($params['closed'] == '')
            $params['closed'] = 0;
        $paginator = Engine_Api::_()->getDbTable('polls', 'sesbusinesspoll')->getPollsPaginator($params);
    ...
    public function moreAction(){
        ...
        if(!$option)
            return;
		$result = array();
        $paginator = Engine_Api::_()->getDbtable('votes', 'sesbusinesspoll')->getVotesPaginator($option->poll_option_id);
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        $data =array();
        if(count($paginator)){
    ...
    public function getPolls($paginator){
        ...
        foreach ($paginator as $poll){
            ...
            $result[$counter]['content_id'] = $business_id;
            $user_id = $poll->user_id;
            $user = Engine_Api::_()->getItem('user', $user_id);
            $result[$counter]['owner_title'] = $poll->getOwner()->getTitle();
            $result[$counter]['owner_title'] = $poll->getOwner()->getTitle();
            $likeStatus = Engine_Api::_()->sesbusiness()->getLikeStatus($poll->poll_id,'sesbusinesspoll_poll');
            $can_fav = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinesspoll.allow.favourite', 1);
            ...
            if($business)
                $result[$counter]['content_title'] = $business->title;
            if($viewer_id)
    ...
    public function searchAction(){
        ...
        // Get form
        $form = new Sesbusinesspoll_Form_Search(array('searchTitle'=>$this->_getParam('search_title')));
        ...
        if ($this->_getParam('getForm')) {
            ...
            $this->generateFormFields($formFields, array('resources_type' => 'sesbusinesspoll_poll'));
        } else {
    ...
    public function businessPollAction(){
        $params = array();
        $result = array();
        $viewer = Engine_Api::_()->user()->getViewer();
        ...
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        $allowPoll  = Engine_Api::_()->authorization()->isAllowed('businesses', $viewer, 'poll');
        $canUpload =  Engine_Api::_()->authorization()->isAllowed('sesbusinesspoll_poll',$viewer, 'create');
    ...
    public function likeAction(){
        if (!Engine_Api::_()->core()->hasSubject())
            $subject = Engine_Api::_()->getItem('sesbusinesspoll_poll', $this->_getParam('id', null));
        else
    ...
    public function favouriteAction(){
        ...
        $dbTable = 'polls';
        $resorces_id = 'poll_id';
        $notificationType = 'sespgpoll_fav';
        $item_id = $subject->poll_id;
        if (intval($item_id) == 0) {
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('Invalid argument supplied.'), 'result' => array()));
        }
    ...
    public function deleteAction(){
        ...
        if (!Engine_Api::_()->core()->hasSubject())
            $poll = Engine_Api::_()->getItem('sesbusinesspoll_poll', $this->_getParam('id', null));
        else
    ...
    public function voteAction(){
        $viewer = Engine_Api::_()->user()->getViewer();
        if( !$this->_helper->requireUser()->isValid() || !$this->_helper->requireAuth()->setAuthParams('sesbusinesspoll_poll', $viewer, 'view')->isValid() ) {
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('user_not_autheticate'), 'result' => array()));
        }
        if( !$this->_helper->requireAuth()->setAuthParams('sesbusinesspoll_poll', $viewer, 'vote')->isValid() ) {
        ...
        $data = array();
        $data['token'] = $this->view->sesBusinessVoteHash($poll)->generateHash();
        $data['status'] = true;
    ...
    public function createAction(){
        ...
        $options = array();
        $maxOptions = $max_options = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinesspoll.maxoptions', 15);
        $form = new Sesbusinesspoll_Form_Create();
        $viewer = Engine_Api::_()->user()->getViewer();
        $gifCount =0;
        ...
        try {
            $business = Engine_Api::_()->getItem('businesses',$business_id);
            $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity(Engine_Api::_()->user()->getViewer(), $business, 'businesses_createpoll');
            if( $action ) {
    ...
    public function editAction(){
        ...
        $pollData['max_options'] = $max_options = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesbusinesspoll.maxoptions', 15);
        $form = new Sesbusinesspoll_Form_Edit();
