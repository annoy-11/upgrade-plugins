...
class Courses_IndexController extends Sesapi_Controller_Action_Standard {
  ...
  public function createAction() { 
    ...
    try {
      ...
      if (isset($_FILES['photo']) && $_FILES['photo']['name'] != '') {
        if(Engine_Api::_()->getDbtable('modules', 'core')->isModuleEnabled('coursesalbum')) {
          ...
          $courses->photo_id = Engine_Api::_()->sesapi()->setPhoto($form->photo, false,false,'courses','courses','',$courses,true);
        }
      }
      if(empty($_POST['show_start_time'])){
    ...
    try {
      ...
      Engine_Api::_()->getApi('response','sesapi')->sendResponse(array('error'=>'0','error_message'=>'', 'result' => array('course_id' => $courses->getIdentity(),'message' => $this->view->translate('Courses created successfully.'))));
    } catch (Exception $e) {
      $db->rollBack();
...
//         if($class != 'courses_main_browse' && $class != 'courses_main_categories' && $class != 'courses_main_browsecategory' && $class != 'courses_main_browse'  && $class != 'courses_main_create' && $class != 'courses_main_account' && $class != 'courses_main_browseplaylist')
//           continue;
//         if($class == 'courses_main_browselocations')
//           continue;
//         if($class == 'eclassroom_main_location')
//           continue;
//         if($class == 'courses_main_pinboard')
//           continue;
//         if($class == 'courses_main_storealbumhome')
//           continue;
    ...
    if(!$courseId)
		...
		if (!Engine_Api::_()->sesapi()->getViewerPrivacy('courses_review', 'view'))
			Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('permission_error'), 'result' => array()));
		...
		$cancreate = Engine_Api::_()->sesapi()->getViewerPrivacy('courses_review', 'create');
		$viewerId = $viewer->getIdentity();
		$levelId = ($viewerId) ? $viewer->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
		$editReviewPrivacy = Engine_Api::_()->sesapi()->getViewerPrivacy('courses_review', 'edit');
    ...
    if ($isReview || !$allowedCreate)
      Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('permission_error'), 'result' => array()));
    if ($hasReview && Engine_Api::_()->sesapi()->getViewerPrivacy('courses_review', 'edit')) {
  ...
  try {
      ...
      if (Engine_Api::_()->sesapi()->getViewerPrivacy('courses_review', 'edit')) {
    ...
    if (!$viewer->isSelf($review->getOwner())) {
		...
		$optionCounter = 0;
		if(Engine_Api::_()->getApi('settings', 'core')->getSetting('courses.show.report', 1) && $viewerId && $viewerId != $owner){
	...
	public function editAction(){
		...
		if( !$this->_helper->requireAuth()->setAuthParams('courses', $viewer, 'edit')->isValid() ) 
			Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('permission_error'), 'result' => array()));
		$form = new Courses_Form_Course_Edit(array('defaultProfileId' => $defaultProfileId));
		// Populate form
...
// 		$form->getElement('productstyle')->setValue($course->style);
    ...
    foreach ($paginator as $wishlist) {
        ...
        $result[$counter]['images'] = array('main' => Engine_Api::_()->getApi('settings', 'core')->getSetting('courses_wishlist_default_image', Zend_Registry::get('StaticBaseUrl')."application/modules/Courses/externals/images/nophoto_wishlist_thumb_profile.png"));
			}
  ...
  public function browseAction() {
      ...
      if (!isset($params['category_id']) || empty($params['category_id'])) {
        ...
        if (!isset($params['category_id'])) {
          ...
          if (count($this->getPopularCourses($paginator))) {
              $result['popularCourses'] = $this->getPopularCourses($paginator);
          }
        }
      }
      ...
      if (($this->_getParam('page', 1) == 1) && $paginator->getTotalItemCount() > 0){
        $result['hot_courses'] = $this->hotAction();
        $result['featured_courses'] = $this->featuredAction();
        $result['verified_courses'] = $this->verifiedAction();
        $result['sponsored_courses'] = $this->sponsoredAction();
      }
      $extraParams['pagging']['total_page'] = $paginator->getPages()->pageCount;
      $extraParams['pagging']['total'] = $paginator->getTotalItemCount();
      $extraParams['pagging']['current_page'] = $paginator->getCurrentPageNumber();
    ...
    public function manageAction()
    {
        ...
        if ($paginator) {
            ...
            foreach ($paginator as $course) {
                ...
                if (!$canFavourite)
                    unset($courseArray['favourite_count']);
                if (!$canFollow)
                    unset($courseArray['follow_count']);
                $result[$courseCounter] = $courseArray;
                $statsCounter = 0;
                $image = Engine_Api::_()->sesapi()->getPhotoUrls($course, '', "");
                if (image) {
                    $result[$courseCounter]['images'] = $image;
                } else {
                    $result[$courseCounter]['images'] = $image;
                }
                $courseCounter++;
    ...
    public function getCourses($paginator) {
      $result = array();
      $counter = 0;
      $canFavourite = Engine_Api::_()->getApi('settings', 'core')->getSetting('courses.enable.favourite', 0);
      ...
      foreach ($paginator as $course) {
        ...
        if ($course->is_approved) {
          if(Engine_Api::_()->getApi('settings', 'core')->getSetting('courses.enable.wishlist', 1)) {
            $result[$counter]['is_content_wishlist'] = true;
          }
          if($course->discount && $priceDiscount = Engine_Api::_()->courses()->courseDiscountPrice($course)){
            $result[$counter]["price_with_discount"] = $priceDiscount['discountPrice'];
            $result[$counter]['course_price'] = $price = Engine_Api::_()->courses()->getCurrencyPrice($priceDiscount['discountPrice']);
            $afterDiscount = Engine_Api::_()->courses()->getCurrentCurrency() . '<strike>' . $course->price . '</strike>';
            $result[$counter]['discount_price'] = $afterDiscount;
            if($course->discount_type == 0) {
              $result[$counter]['course_price'] = $this->view->translate("%s%s OFF",str_replace('.00','', $course->percentage_discount_value),"%");
            } else {
              $result[$counter]['course_price'] = $this->view->translate("%s OFF",Engine_Api::_()->courses()->getCurrencyPrice($course->fixed_discount_value));
            }
          } else {
            $result[$counter]['course_price'] = $price = $course->price > 0 ? Engine_Api::_()->courses()->getCurrencyPrice($course->price) : $this->view->translate('FREE');
          }
          if ($viewerId != 0) {
              ...
              if ($canFavourite) {
                  $result[$counter]['is_content_favourite'] = Engine_Api::_()->sesapi()->contentFavoutites($course, 'favourites', 'courses', 'courses', 'user_id');
                  $result[$counter]['content_favourite_count'] = (int)Engine_Api::_()->sesapi()->getContentFavouriteCount($course, 'favourites', 'courses', 'courses', 'user_id');
              }
          }
          $counter++;
        }
  ...
  public function getCourse($course){
    ...
    $coursedata['currency'] = $curArr[$currency];
      ...
      if ($location) {
		...
		if($course->discount && $priceDiscount = Engine_Api::_()->courses()->courseDiscountPrice($course)){
			$coursedata["price_with_discount"] = $priceDiscount['discountPrice'];
			if($course->discount_type == 0){ 
				$coursedata["course_price"] = $this->view->translate("%s%s OFF",str_replace('.00','',$course->percentage_discount_value),"%");
			}else{
				$coursedata["course_price"] = $this->view->translate("%s OFF",Engine_Api::_()->courses()->getCurrencyPrice($course->fixed_discount_value));
			}
		}else{
			$coursedata["course_price"] = $course->price > 0 ? Engine_Api::_()->courses()->getCurrencyPrice($course->price) : $this->view->translate('FREE');
		}
  ...
  public function infoAction(){
    ...
    $classroom = Engine_Api::_()->getItem('classroom',$course->classroom_id);
    if(!empty($classroom)) {
      $result['classroom_logo'] = $this->getBaseUrl(true,$classroom->getPhotoUrl('thumb.icon'));
      $result['classroom_title'] = $classroom->title;
		}
		if($course->category_id != '' && intval($course->category_id) && !is_null($course->category_id)) {
      $category = Engine_Api::_()->getItem('courses_category', $course->category_id);
        if ($category) {
    ...
    //$result['related_courses'] = $this->relatedCourses($course);
    return $result;
  }
  ...
  public function overviewAction(){
    ...
    try
    {
        Engine_Api::_()->courses()->deleteCourses($course);
        $db->commit();
  ...
  public function myCartAction(){
    ...
    foreach($cartData['cartCourses'] as $cart) { 
      ...
      $result['cartData'][$counter]['title'] = $course->getTitle();
      $result['cartData'][$counter]['quantity'] = $quantity;
      if(!empty($priceData['discountPrice'])){
        $result['cartData'][$counter]['price'] = Engine_Api::_()->courses()->getCurrencyPrice(round($priceData['discountPrice'],2)) ;
      } else {
        $result['cartData'][$counter]['price'] = 'FREE';
      }
      $images = Engine_Api::_()->sesapi()->getPhotoUrls($course,'',"");
      if(!count($images))
        $images['main'] = $this->getBaseUrl(true,$course->getPhotoUrl());
        $result['cartData'][$counter]['course_images'] = $images;
      ...
      $result['cartData'][$counter]['taxes'] = $taxes;
      ...
      $menucounter++;
      //Menus
      $result['cartData'][$counter]['buttons'] = $menuoptions;
      $counter++;
    }
    $extraParams = array();
  ...
  public function createTestAction() { 
    ...
    try {
        ...
        $viewer = Engine_Api::_()->user()->getViewer();
        if (isset($_FILES['photo']) && $_FILES['photo']['name'] != '') {
          $test->photo_id = Engine_Api::_()->sesapi()->setPhoto($form->photo, false,false,'courses','courses','',$test,true);
        }
  ...
  public function joinTestAction(){
    ...
    if(!$testId)
		...
		$formData = $this->_getParam('formData',false);
		$testDetails = array();
		$counter = 0;
		if($this->_getParam('getForm')) {
      foreach($paginator as $question):
        ...
        elseif($question->answer_type == 1): 
          ...
          $testDetails['formFields'][$counter]['multiOptions'][1] = $this->view->translate('TRUE');
      ...
      $testDetails['test']['usertest'] = $this->getTestQuestion();
		}
		$extraParams = array();
    $testDetails['test'] = $test->toArray();    
    $extraParams['pagging']['total'] = $paginator->getTotalItemCount();
