...
class Courses_ClassroomController extends Sesapi_Controller_Action_Standard
{
    ...
    public function createAction(){
      ...
      if ($allowClassroomCount != 0 && $totalClassroom >= $allowClassroomCount) {
        ...
        $quckCreate = 0;
        $form = new Eclassroom_Form_Classroom_Create();
      ...
      Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('Classroom created successfully.'), 'result' => array()));
    }
    ...
    public function editAction() {
      ...
      $form = new Eclassroom_Form_Classroom_Edit(array('defaultProfileId' => $defaultProfileId));
    ...
    public function deleteAction()
    {
      ...
      try {
          ...
          $db->commit();
          $status['status'] = true;
          Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '0', 'error_message' => '', 'result' => array('success_message' => $this->view->translate('You have successfully deleted to this classroom.'), $status)));
      } catch (Exception $e) {
          $db->rollBack();
          Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $e->getMessage(), 'result' => array()));
      }
  ...
  public function browseAction(){
    ...
    if ($page == 1 && $paginator->getTotalItemCount() > 0){
      $result['hot_classroom'] = $this->hotAction();
      $result['featured_classroom'] = $this->featuredAction();
      $result['verified_classroom'] = $this->verifiedAction();
      $result['verified_classroom'] = $this->sponsoredAction();
    }
  ...
  public function getPopularClassrooms($classroomPaginator){
      $classroomid = array();
      foreach ($classroomPaginator as $classroom) {
          $classroomid[] = $classroom->classroom_id;
      }
      $params['info'] = 'most_viewed';
      $params['notClassroomId'] = $classroomid;
      $paginator = Engine_Api::_()->getDbTable('classrooms', 'eclassroom')->getClassroomPaginator($params);
  ...
  public function getclassroom($classroom)
  {
      ...
      $shareType = Engine_Api::_()->getApi('settings', 'core')->getSetting('eclassroom.allow.share', 1);
      $canFavourite = Engine_Api::_()->getApi('settings', 'core')->getSetting('eclassroom.allow.favourite', 0);
      $canFollow = Engine_Api::_()->getApi('settings', 'core')->getSetting('eclassroom.allow.follow', 0);
      ...
      if($canFavourite){
        $classroomdata['is_content_follow'] = $favouriteStatus >0 ? true : false;
      }
      if ($classroom->category_id) {
          $category = Engine_Api::_()->getItem('eclassroom_category', $classroom->category_id);
          if ($category) {
    ...
    $classroomid = array();
    $classroomid[] = $classroom->classroom_id;
    $relatedParams['notClassroomId'] = $classroomid;
    ...
    if ($isClassroomEdit) {
        ...
        if ($canUploadCover) {
            ...
            if (isset($classroom->cover) && $classroom->cover != 0 && $classroom->cover != '') {
                ...
                $i++;
                $classroomdata['updateCoverPhoto'][$i]['label'] = $this->view->translate('View Cover Photo');
                $classroomdata['updateCoverPhoto'][$i]['name'] = 'view';
                $i++;
            } else {
    ...
    foreach ($navigation as $link) {
        ...
        if ($class != "eclassroom_profile_addtoshortcut") {
          ...
          if ($class == 'eclassroom_profile_dashboard') {
              ...
              $pluralurl = Engine_Api::_()->getApi('settings', 'core')->getSetting('eclassroom_classroom_manifest', null);
              if ($pluralurl) {
  ...
  public function reviewAction(){
    ...
    if(!$classroomId)
		...
		$cancreate = Engine_Api::_()->sesapi()->getViewerPrivacy('eclassroom_review', 'create');
		$viewerId = $viewer->getIdentity();
		$levelId = ($viewerId) ? $viewer->level_id : Engine_Api::_()->getDbtable('levels', 'authorization')->getPublicLevel()->level_id;
		...
		$isReview = $hasReview = $reviewTable->isReview(array('classroom_id' => $classroom->getIdentity(), 'column_name' => 'review_id'));
    ...
    //check review exists
    $isReview = Engine_Api::_()->getDbtable('reviews', 'eclassroom')->isReview(array('classroom_id' => $classroom->getIdentity(), 'column_name' => 'review_id'));
    if (Engine_Api::_()->getApi('settings', 'core')->getSetting('eclassroom.allow.owner.review', 1)) {
        $allowedCreate = true;
    } else {
  ...
  try {
    ...
    if (!$viewer->isSelf($review->getOwner())) {
		...
		$optionCounter = 0;
		if(Engine_Api::_()->getApi('settings', 'core')->getSetting('eclassroom.show.report', 1) && $viewerId && $viewerId != $owner){
...
//           }
  ...
  public function getInformation($classroom){
		...
		if($classroom->category_id != '' && intval($classroom->category_id) && !is_null($classroom->category_id)) {
      $category = Engine_Api::_()->getItem('eclassroom_category', $classroom->category_id);
        if ($category) {
            $result['category_title'] = $category->category_name;
            if ($category->subcat_id) {
                $subcat = Engine_Api::_()->getItem('eclassroom_category', $category->subcat_id);
                if ($subcat) {
...
//    die;
  ...
  public function uploadcoverAction()
  {
      ...
      $file['main'] = $classroom->getCoverPhotoUrl();
