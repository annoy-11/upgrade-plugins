...
class Activity_IndexController extends Sesapi_Controller_Action_Standard
{
  ...
  function sesadvancedactivity()
  {
	...
	if (!$subject) {
	  $allownetworkprivacy = Engine_Api::_()->getApi('settings', 'core')->getSetting('sesadvancedactivity.allownetworkprivacy', 1);
	...
	if (Engine_Api::_()->sesapi()->isModuleEnable('Sesfeedbg') && Engine_Api::_()->getApi('settings', 'core')->getSetting('sesfeedbg.enablefeedbg', 1) && $viewer->getIdentity()) {
	  $sesfeedbg_enablefeedbg = Engine_Api::_()->authorization()->isAllowed('sesfeedbg', null, 'enablefeedbg');
  ...
  function friendBirthdayHTML($birthdayuser)
  {
	...
	if (Engine_Api::_()->sesapi()->hasCheckMessage($birthdayuser)) {
	  $message = '<a href="' . $this->getBaseUrl(true, "messages/compose/to/" . $birthdayuser->user_id) . '?module=composeMessage" class="smoothbox"><i class="fa fa-comments-o"></i><span>' . $this->view->translate('Send Message') . '</span></a>';
	}
	return array('html' => "<link href=\"" . $this->getBaseUrl(true, 'application/modules/Sesapi/externals/styles/activity_messages.css') . "\" type=\"text/css\" rel=\"stylesheet\">" . '<div class="sesact_tip_box sesbasic_clearfix sesbm sesact_sendwish sesbasic_bxs parent_notification_sesadv">' . str_replace($token, $replace, Engine_Api::_()->getApi('settings', 'core')->getSetting('sesadvancedactivity.friendnotificationbirthdaytext', '')) . '</div><div class="sesact_sendwish_btns"><a href="' . $this->getBaseUrl(true, $birthdayuser->getHref()) . '?module=postonwall"><i class="fa fa-pencil-square-o"></i><span>' . $this->view->translate("Post on Wall") . '</span></a>' . $message . '</div>', 'message' => Engine_Api::_()->sesapi()->hasCheckMessage($birthdayuser) ? true : false, 'user_id' => $birthdayuser->user_id, 'user_title' => $birthdayuser->getTitle());
  }
