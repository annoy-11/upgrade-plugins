...
class Sespage_PollController extends Sesapi_Controller_Action_Standard
{
    public function init(){
        $poll_id = $this->_getParam('poll_id');
        $poll = null;
        $poll = Engine_Api::_()->getItem('sespagepoll_poll', $poll_id);
        if ($poll) {
    ...
    {
        $form = new Sespagepoll_Form_Search();
        $form->populate($_POST);
        $params = $form->getValues();
        if ($params['closed'] == '')
            $params['closed'] = 0;
        $paginator = Engine_Api::_()->getDbTable('polls', 'sespagepoll')->getPollsPaginator($params);
    ...
    public function moreAction(){
        ...
        if(!$option)
            return;
        $paginator = Engine_Api::_()->getDbtable('votes', 'sespagepoll')->getVotesPaginator($option->poll_option_id);
        $paginator->setItemCountPerPage($this->_getParam('limit', 10));
        $paginator->setCurrentPageNumber($this->_getParam('page', 1));
        $data =array();
        if(count($paginator)){
    ...
    public function getPolls($paginator){
        ...
        foreach ($paginator as $poll){
            ...
            $result[$counter]['content_id'] = $page_id;
    ...
    public function searchAction(){
        ...
        // Get form
        $form = new Sespagepoll_Form_Search(array('searchTitle'=>$this->_getParam('search_title')));
        ...
        if ($this->_getParam('getForm')) {
            ...
            $this->generateFormFields($formFields, array('resources_type' => 'sespagepoll_poll'));
        } else {
    ...
    public function pagePollAction(){
        $params = array();
        $result = array();
        $viewer = Engine_Api::_()->user()->getViewer();
    ...
    public function likeAction(){
        if (!Engine_Api::_()->core()->hasSubject())
            $subject = Engine_Api::_()->getItem('sespagepoll_poll', $this->_getParam('id', null));
        else
    ...
    public function deleteAction(){
        ...
        if (!Engine_Api::_()->core()->hasSubject())
            $poll = Engine_Api::_()->getItem('sespagepoll_poll', $this->_getParam('id', null));
        else
    ...
    public function voteAction(){
        $viewer = Engine_Api::_()->user()->getViewer();
        if( !$this->_helper->requireUser()->isValid() || !$this->_helper->requireAuth()->setAuthParams('sespagepoll_poll', $viewer, 'view')->isValid() ) {
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('user_not_autheticate'), 'result' => array()));
        }
        if( !$this->_helper->requireAuth()->setAuthParams('sespagepoll_poll', $viewer, 'vote')->isValid() ) {
        ...
        $data = array();
        $data['token'] = $this->view->sesVoteHash($poll)->generateHash();
        $data['status'] = true;
       ...
       // }
        ...
        $db->beginTransaction();
        try {
            $poll->vote($viewer,$option_id,$page,$poll->getOwner(),$poll);
            $db->commit();
            $data['token'] = $this->view->sesVoteHash($poll)->generateHash();
            $data['status'] = true;
    ...
    public function createAction(){
        ...
        if( !$this->_helper->requireAuth()->setAuthParams('sespagepoll_poll', null, 'create')->isValid() ) {
            Engine_Api::_()->getApi('response', 'sesapi')->sendResponse(array('error' => '1', 'error_message' => $this->view->translate('permission_error'), 'result' => array()));
        }
        ...
        $options = array();
        $maxOptions = $max_options = Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagepoll.maxoptions', 15);
        $form = new Sespagepoll_Form_Create();
        $viewer = Engine_Api::_()->user()->getViewer();
        $gifCount =0;
        ...
        try {
            $page = Engine_Api::_()->getItem('sespage_page',$page_id);
            $action = Engine_Api::_()->getDbtable('actions', 'activity')->addActivity(Engine_Api::_()->user()->getViewer(), $page, 'sespage_page_createpoll');
            if( $action ) {
    ...
    public function editAction(){
        ...
        $pollData['max_options'] = $max_options = Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagepoll.maxoptions', 15);
        $form = new Sespagepoll_Form_Edit();
        ...
        foreach( $roles as $role ) {
            if( 1 === $auth->isAllowed('sespagepoll_poll', $role, 'view') ) {
                $form->auth_view->setValue($role);
            }
            if( 1 === $auth->isAllowed('sespagepoll_poll', $role, 'comment') ) {
    ...
    public function viewAction()
    {
        ...
        if($canEdit && !$poll_is_voted){
            ...
            $optionData[$counterOpt]['label'] = $this->view->translate('Edit Poll');
            $counterOpt++;
        }
        if($canDelete){
            $optionData[$counterOpt]['name'] = 'delete';
            $optionData[$counterOpt]['label'] = $this->view->translate('Delete Poll');
            $counterOpt++;
        }
        if($shareType){
        ...
        if($owner->isSelf($viewer) && !$poll->closed){
            ...
            $optionData[$counterOpt]['label'] = $this->view->translate('Close Poll');
            $counterOpt++;
        }elseif ($owner->isSelf($viewer) && $poll->closed){
            $optionData[$counterOpt]['name'] = 'open';
            $optionData[$counterOpt]['label'] = $this->view->translate('Open Poll');
            $counterOpt++;
        }
