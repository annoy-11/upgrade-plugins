 ...
 */
class Sespagethm_Api_Core extends Core_Api_Abstract {
  ...
  public function readWriteXML($keys, $value, $default_constants = null) {
    ...
    //For constant backup
    $filePath_sp = APPLICATION_PATH . "/application/modules/Sespagethm/externals/styles/sespagethm.xml";
    $results_sp = simplexml_load_file($filePath_sp);
    //For constant backup

    if (!empty($keys) && !empty($value) && ($keys != 'sespagethm_body_background_image' || $keys != 'sespagethm_footer_background_image')) {
      $contactsThemeArray = array($keys => $value);
    } elseif (!empty($keys) && ($keys == 'sespagethm_body_background_image' || $keys == 'sespagethm_footer_background_image')) {
      $contactsThemeArray = array($keys => '');
    } elseif ($default_constants) {
      $contactsThemeArray = $default_constants;
    }

    //For constant backup at file path: /application/modules/Sespagethm/externals/styles/sespagethm.xml
