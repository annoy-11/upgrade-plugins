 ...
 */
class Sespagethm_Widget_FooterController extends Engine_Content_Widget_Abstract {
  ...
  public function indexAction() {
    ...
    $this->view->footerlogo = Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagethm.footerlogo', '');

    $this->view->paginator = Engine_Api::_()->getDbTable('socialicons', 'sespagethm')->getSocialInfo(array('enabled' => 1));
    ...
    $sespagethm_footer = Zend_Registry::isRegistered('sespagethm_footer') ? Zend_Registry::get('sespagethm_footer') : null;
    if(empty($sespagethm_footer)) {
	...
	$popShow = Engine_Api::_()->getApi('settings', 'core')->getSetting('sespagethm.popupshow', 1);
		if(empty($popShow)) {
			$contentText = '<div class="sespagethm_footer_top_section sesbasic_clearfix sesbasic_bxs">
  	<p><img src="application/modules/Sespagethm/externals/images/bottom-text.png" alt=""></p><p><a href="signup">Join Now</a></p></div>';
		} else {
	    $contentText = '<div class="sespagethm_footer_top_section sesbasic_clearfix sesbasic_bxs">
  	<p><img src="application/modules/Sespagethm/externals/images/bottom-text.png" alt=""></p><p><a id="popup-signup" data-effect="signup-link mfp-zoom-in" class="popup-with-move-anim" href="#user_signup_form">Join Now</a></p></div>';
  	}
    ...
    if(count($languageList) == '1') {
      $this->view->content = Engine_Api::_()->getApi('settings', 'core')->getSetting("sespagethm.footertext.en", $contentText);
    } else {
      $this->view->content = Engine_Api::_()->getApi('settings', 'core')->getSetting("sespagethm.footertext.$language", $contentText);
    }
    ...
    $select = new Zend_Db_Select($db);
		$select->from('engine4_core_content', 'name')
		        ->where('page_id = ?', 1)
		        ->where('name LIKE ?', '%sespagethm.header%')
		        ->limit(1);
