...
<div class="user_dashboard_links sesbasic_bxs sesbasic_clearfix">
	<div class="dashboard_explore_links_v sesbasic_clearfix">
  	<ul>
        ...
        <span><a onclick="showHideInformation('sespagethm_information');" href="javascript:void(0);" class="fa fa-ellipsis-h sesbasic_text_light" title='<?php echo $this->translate("Edit Profile")?>'></a></span>
        <ul class="edit_profile_dropdown" style="display:none;" id="sespagethm_information">
      ...
      <li class="_newsfeed<?php if($moduleName = 'user' && $controllerName == 'index' && $actionName == 'home') { ?> active<?php } ?>">
        <a href="">
          <img src="application/modules/Sespagethm/externals/images/newsfeeed.png" />
          <span><?php echo $this->translate("News Feed");?></span>
        </a>
      </li>
      <li class="_message">
        <a class="profile_link" href="/messages/inbox">
          <img src="application/modules/Sespagethm/externals/images/message.png" />
  ...
  <?php foreach( $this->dashboardlinks as $item ): ?>
    <div class="<?php if(!empty($item->type) && $item->type == 'horizontal') { ?> dashboard_explore_links_h <?php } else { ?> dashboard_explore_links_v <?php } ?>sesbasic_clearfix">
      <?php $footersubresults = Engine_Api::_()->getDbTable('dashboardlinks', 'sespagethm')->getInfo(array('sublink' => $item->dashboardlink_id, 'enabled' => 1)); ?>
