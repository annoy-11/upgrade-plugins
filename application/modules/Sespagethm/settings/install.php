...
class Sespagethm_Installer extends Engine_Package_Installer_Module {
  ...
  function onEnable() {
    ...
    $parent_content_id = $db->select()
		        ...
		        ->query()
		        ->fetchColumn();
    if (!empty($info) && !empty($info1) && !empty($info2) && !empty($info3)) {
    ...
    if($themeActive) {
	    ...
	    $db->query("UPDATE  `engine4_core_themes` SET  `active` =  '1' WHERE  `engine4_core_themes`.`name` ='sespagethm' LIMIT 1");
    }
  ...
  public function onDisable() {
    ...
    //Header Work
    $db->query("UPDATE  `engine4_core_content` SET  `name` =  'core.menu-mini' WHERE  `engine4_core_content`.`name` ='sespagethm.header' LIMIT 1");
    $parent_content_id = $db->select()
            ->from('engine4_core_content', 'content_id')
            ->where('type = ?', 'container')
            ->where('page_id = ?', '1')
	...
	//Footer Work
    $db->query("UPDATE  `engine4_core_content` SET  `name` =  'core.menu-footer' WHERE  `engine4_core_content`.`name` ='sespagethm.footer' LIMIT 1");

    $db->query("UPDATE  `engine4_core_themes` SET  `active` =  '0' WHERE  `engine4_core_themes`.`name` ='sespagethm' LIMIT 1");
