...
<?php $this->headLink()->appendStylesheet($this->layout()->staticBaseUrl . 'application/modules/Sesdbslide/externals/styles/styles.css'); ?>
<?php $this->headScript()->appendFile($this->layout()->staticBaseUrl . 'application/modules/Sesdbslide/externals/scripts/typed.js'); ?>
...
// end header work
   ...
   <?php }?>
     ...
     <?php if($this->banner_full_width == 1){ ?>
            var body = document.body;
            body.classList.add("sesdbslide_full_width");
    <?php }?>
      ...
      <?php if($this->header_insight_out == 2){ ?>
        <div class="sesdbslide_header sesbasic_bxs">
          <header class="sesbasic_clearfix">
            ...
            <?php } ?>
            <div class="sesdbslide_slider_logo" id="sesdbslide_slider_logo<?php echo "_".$identity ?>"></div>
            <?php if($this->main_navigation && count($this->navigation) && ($this->show_menu)): ?>
              <nav class="sesdbslide_slider_nav sesdbslide_slider_nav<?php echo "_".$identity ?>">
              	<div class="_browsermenu">
                    ...
                    <?php if(count($this->navigation)> $this->max) : ?>
                      ...
                      <ul class="sesdbslide_slider_nav_dropdown">
                        <?php  $countMenu = 0; ?>
                        <?php foreach( $this->navigation as $navigationMenu ): ?>
                          <?php if($countMenu >= $this->max):  ?>
                            <li>
                ...
                <div class="txtcontent">
                    <section>
                        ...
                        <div class="_btns">
                            ...
                            ($slide->video_video_file_url);  }else{ echo " "; } ?>' class="_mb sesdbslide_video_button" onMouseOut="this.style.backgroundColor='#<?php echo $slide->video_background_color ?>'; this.style.color='#<?php echo $slide->video_text_color ?>';"  onmouseover="this.style.backgroundColor='#<?php echo $slide->video_mouseover_background_color ?>'; this.style.color='#<?php echo $slide->video_mouseover_text_color ?>'" style='background-color:#<?php echo $slide->video_background_color ?> ;color:#<?php echo $slide->video_text_color ?> ; ' ><?php echo $this->translate($slide->video_Button_label) ?></a>
                            <?php }?>
                        </div>
                    </section>
                </div>
...
<div id="videopopup" class="sesdoublebnr_videobox sesdbslide_video_cnt">
...
<script type='text/javascript'>
  ...
  <?php  if($this->logo)
          {
            if($this->logo_url){ ?>
                  sesJqueryObject('#sesdbslide_slider_logo<?php echo "_".$identity ?>').html('<div id="sesdbslide_slider_logo" class="logo"><div><a href="/"><img src="<?php echo $this->logo_url; ?>"></a></div></div>');
            <?php }
          }
          else{ ?>
              sesJqueryObject('#sesdbslide_slider_logo<?php echo "_".$identity ?>').html(logoSeshtml<?php echo "_".$identity ?>);
          <?php } ?>
</script>
<?php } ?>
...
<script>
    sesJqueryObject('.sesdbslide_video_button').on('click', function () {
        ...
        if(!videobuttonurl)
            return;
            if(videotype == 4){
                sesJqueryObject('.sesdbslide_video_cnt').show();
                sesJqueryObject('.sesdbslide_video_cnt').find('.sesdoublebnr_videobox_inner').append('<iframe src="'+videobuttonurl+'"></iframe>');
            }else{
                sesJqueryObject('.sesdbslide_video_cnt').show();
                sesJqueryObject('.sesdbslide_video_cnt').find('.sesdoublebnr_videobox_inner').append(videobuttonurl);
            }
         });
         sesJqueryObject('.cancel-video, .sesdoublebnr_videobox').on('click', function () {
           sesJqueryObject('.sesdoublebnr_videobox_inner').html('');
           sesJqueryObject('.sesdbslide_video_cnt').hide();
        });
