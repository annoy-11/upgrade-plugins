 ...
 */
class Sesdbslide_Widget_SlideshowController extends Engine_Content_Widget_Abstract {
    ...
    public function indexAction(){
      ...
      $sesdbslide_widgets = Zend_Registry::isRegistered('sesdbslide_widgets') ? Zend_Registry::get('sesdbslide_widgets') : null;
      if (empty($sesdbslide_widgets)) {
      ...
      if (!$galleryId)
        return $this->setNoRender();
      $gallery = Engine_Api::_()->getItem('sesdbslide_gallery', $galleryId);
      if (!$gallery)
        return $this->setNoRender();
			if($gallery->status != 1)
				return $this->setNoRender();
      ...
      $this->view->paginator = $paginator = $itemTable = Engine_Api::_()->getDbtable('slides', 'sesdbslide')->getDbslidePaginator($params);
