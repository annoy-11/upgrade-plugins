...
<?php if($this->allowedCreate && $this->cancreate && $this->viewer()->getIdentity() && Engine_Api::_()->getApi('settings', 'core')->getSetting('seseventreview.allow.review', 1) && !$this->isReview): ?>
  ...
  <a href="#" class="sesbasic_button" onclick="opensmoothboxurl('<?php echo $this->url(array('action' => 'create','controller'=>'index','type'=>$this->subject()->getGuid()),'seseventreview_extended',true); ?>');return false;" >
    <i class="fa fa-plus"></i><?php echo $this->translate('Write a Review') ?>
    </a>
    </div>
<?php endif;?>
  ...
  <li class="sesbasic_clearfix">
    <div class="sesevent_review_listing_left_column">
      ...
      <?php if($item->authorization()->isAllowed($this->viewer(), 'edit')) { ?>
        <div>
        	<?php echo $this->htmlLink(array('slug' => $item->getSlug(), 'action' => 'edit', 'review_id' => $item->getIdentity(), 'route' => 'seseventreview_view', 'reset' => true), $this->translate('Edit Review'), array('class' => 'fa fa-edit')) ?>
        </div>
        <?php } ?>
       ...
       <?php if(in_array('rating', $this->stats)): ?>
        <div class="sesbasic_rating_star">
          ...
          <?php for($i=0; $i<5; $i++){?>
            ...
            <?php else:?>
              <span id="" class="far fa-star star-disable"></span>
            <?php endif;?>
          <?php }?>
        </div>
       <?php endif ?>
