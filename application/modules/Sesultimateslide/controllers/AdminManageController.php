...
class Sesultimateslide_AdminManageController extends Core_Controller_Action_Admin {

    public function indexAction() {
        // Admin navigation Menus
        $this->view->navigation = $navigation = Engine_Api::_()->getApi('menus', 'core')
                ->getNavigation('sesultimateslide_admin_main', array(), 'sesultimateslide_admin_main_manage');
        $page = $this->_getParam('page', 1);
        // galleries's paginator
        $this->view->paginator = Engine_Api::_()->getDbtable('galleries', 'sesultimateslide')->getDbslidePaginator(array(
            'orderby' => 'gallery_id',
        ));
          ...
          $values = $this->getRequest()->getPost();
            foreach ($values as $key => $value) {
                if ($key == 'delete_' . $value) {
                  // delete gallery
                  $gallery = Engine_Api::_()->getItem('sesultimateslide_gallery', $value)->delete();
                    $db->query("DELETE FROM engine4_sesultimateslide_slides WHERE gallery_id = " . $value);
                }
            }
        }
    ...
    public function createGalleryAction() {
        ...
        // gallery form
        $this->view->form = $form = new Sesultimateslide_Form_Admin_Gallery();
        if ($id) {
              $form->setTitle("Edit Slidshow Title");
              $form->submit->setLabel('Save Changes');
              $gallery = Engine_Api::_()->getItem('sesultimateslide_gallery', $id);
        ...
        if ($this->getRequest()->isPost()) {
          ...
          if (!$form->isValid($this->getRequest()->getPost()))
                return;
            $db = Engine_Api::_()->getDbtable('galleries', 'sesultimateslide')->getAdapter();
            $db->beginTransaction();
            try {
    ...
    public function deleteGalleryAction() {
        ...
        // Check post
        if ($this->getRequest()->isPost()) {
            $chanel = Engine_Api::_()->getItem('sesultimateslide_gallery', $id)->delete();
            $db = Engine_Db_Table::getDefaultAdapter();
            $db->query("DELETE FROM engine4_sesultimateslide_slides WHERE gallery_id = " . $id);
            $this->_forward('success', 'utility', 'core', array(
                'smoothboxClose' => 10,
                'parentRefresh' => 10,
    ...
    public function manageAction() {
        if ($this->getRequest()->isPost()) {
            ...
            foreach ($values as $key => $value) {
                if ($key == 'delete_' . $value) {
                    $slide = Engine_Api::_()->getItem('sesultimateslide_slide', $value);
        ...
        if (!$id)
            return;
        $this->view->paginator = $paginator = Engine_Api::_()->getDbtable('slides', 'sesultimateslide')->getDbslidePaginator($id, 'show_all');
        $page = $this->_getParam('page', 1);
    ...
    public function createSlideAction() {
        ...
        if (!$id)
            return;
        $this->view->form = $form = new Sesultimateslide_Form_Admin_Createslide();
        if ($slide_id) {
            ...
            $slide = Engine_Api::_()->getItem('sesultimateslide_slide', $slide_id);
        ...
        if ($this->getRequest()->isPost()) {
            if (!$form->isValid($this->getRequest()->getPost()))
                return;
            $db = Engine_Api::_()->getDbtable('slides', 'sesultimateslide')->getAdapter();
            $db->beginTransaction();
            try {
                $table = Engine_Api::_()->getDbtable('slides', 'sesultimateslide');
                $values = $form->getValues();
                ...
                if (isset($_FILES['video_upload']['name']) && $_FILES['video_upload']['name'] != '') {
                    ...
                    $filename = $storage->createFile($form->video_upload, array(
                        ...
                        'parent_type' => 'sesultimateslide_slide',
                ...
                if (isset($_POST['save']))
                    return $this->_helper->redirector->gotoRoute(array('module' => 'sesultimateslide', 'controller' => 'manage', 'action' => 'create-slide','slide_id'=>$slideId, 'id' => $id), 'admin_default', true);
                else
                    return $this->_helper->redirector->gotoRoute(array('module' => 'sesultimateslide', 'controller' => 'manage', 'action' => 'manage', 'id' => $id), 'admin_default', true);
            } catch (Exception $e) {
                $db->rollBack();
                throw $e;
            }
    ...
    public function deleteSlideAction() {
        ...
        // Check post
        if ($this->getRequest()->isPost()) {
            $slide = Engine_Api::_()->getItem('sesultimateslide_slide', $id);
    ...
    public function enabledAction() {
        ...
        if (!empty($id)) {
            if (!empty($gallery_id))
                $item = Engine_Api::_()->getItem('sesultimateslide_slide', $id);
            else
                $item = Engine_Api::_()->getItem('sesultimateslide_gallery', $id);
            $item->status = !$item->status;
            $item->save();
        }
        if (!empty($gallery_id))
            $this->_redirect('admin/sesultimateslide/manage/manage/id/' . $gallery_id);
        else
            $this->_redirect('admin/sesultimateslide/manage');
    }
	 public function enabledgalleryAction() {
        $gallery_id = $this->_getParam('gallery_id', 0);
        if (!empty($gallery_id)) {
			$item = Engine_Api::_()->getItem('sesultimateslide_gallery', $gallery_id);
            $item->status = !$item->status;
            $item->save();
        }
        $this->_redirect('admin/sesultimateslide/manage');
    ...
    public function orderAction() {
        if (!$this->getRequest()->isPost())
            return;
        $slidesTable = Engine_Api::_()->getDbtable('slides', 'sesultimateslide');
    ...
    public function addSlideshowAction() {
        $form = $this->view->form = new Sesultimateslide_Form_Admin_Slideshow_Addslideshow();
