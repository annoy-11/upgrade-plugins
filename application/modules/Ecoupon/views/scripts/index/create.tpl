...
jqueryObjectOfSes(document).on('click','div[id^="abortPhoto_"]',function(){
  ...
  function checkAvailsbility(submitform) {
    ...
    jqueryObjectOfSes.post('<?php echo $this->url(array('action' => 'check-availability','subject'=>$this->resource_type,'resource_id' => $this->resource_id), "ecoupon_general") ?>',{coupon_code:coupon_code},function(response){
  ...
  function changeDiscountType(value){ console.log(value);
    if(value == 1){ 
...
<?php  if($this->typesmoothbox) { ?>
	...
	function showHideOptionsCoupon(display){
