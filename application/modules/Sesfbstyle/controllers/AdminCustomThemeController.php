 ...
 * @version    $Id: AdminCustomThemeController.php  2017-09-23 00:00:00 SocialEngineSolutions $
 * @author     SocialEngineSolutions
 */

class Sesfbstyle_AdminCustomThemeController extends Core_Controller_Action_Admin {

  public function indexAction() {
    
    $this->view->navigation = Engine_Api::_()->getApi('menus', 'core')->getNavigation('sesfbstyle_admin_main', array(), 'sesfbstyle_admin_main_customcss');
    
    $writeable = array();
    $writeable['sesfbstyle'] = false;
    try {
      foreach(array('theme.css', 'constants.css', 'media-queries.css', 'sesfbstyle-custom.css') as $file ) {
        if( !file_exists(APPLICATION_PATH . "/application/themes/sesfbstyle/$file") ) {
          throw new Core_Model_Exception('Missing file');
        } else {
          $this->checkWriteable(APPLICATION_PATH . "/application/themes/sesfbstyle/$file");
        }
      }
      $writeable['sesfbstyle'] = true;
    }
    catch( Exception $e ) {
      $this->view->errorMessage = $e->getMessage();
    }
    $this->view->writeable = $writeable;
    $this->view->activeFileContents = file_get_contents(APPLICATION_PATH . '/application/themes/sesfbstyle/sesfbstyle-custom.css');
  }
  ...
  public function saveAction() {
    ...
    //Check file
    $basePath = APPLICATION_PATH . '/application/themes/sesfbstyle';
