...
Composer = new Class({
  ...
  initialize : function(element, options) {
    ...
    this.getForm().addEvent('submit', function(e) {
      ...
      if (typeof this.getContent() == "undefined"){ return false;  }
      if( this.pluginReady ) {
  ...
  getContent: function(){
    if( this._supportsContentEditable() ) {
      document.getElementById("activity_body").value=this.cleanup(this.elements.body.get('html'));
      return this.cleanup(this.elements.body.get('html'));
    } else {
      return this.cleanup(this.elements.body.get('value'));
      document.getElementById("activity_body").value=this.cleanup(this.elements.body.get('value'));
    }
  },
  ...
  editorKeyPress: function(e){
    if(e.key == "enter" && e.shift ==false){ sesJqueryObject("#chatmessages_form").submit(); }
    this.keyListener(e);
...
Composer.Plugin.Interface = new Class({
  ...
  makeActivator : function() {
    if( !this.elements.activator ) {
			this.elements.activator = new Element('div', {
				'html' :  '',
				'class': 'composer-box-option-item composer-box-option-item-'+this.getName(),
			});
			
      this.elements.mActivator = new Element('a', {
        'id' : 'compose-' + this.getName() + '-activator',
        'class' : 'composer-box-option-item-btn',
        ...
        'events' : {
          'click' : this.activate.bind(this)
			...
			this.elements.activator.inject($('compose-menu'),'before');
    }
  },
